function replyTicket(leafId, url, securityToken, ticketId, userId, isAdmin, solve, loadingText, loadingCompleteText, loadingErrorText) {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if (data.success == true) {
                ...
                append = append + "<div class=\"alert alert-info\">";

                append = append + "<img width=\"50\" height=\"50\" style=\"float:left;border:1px solid #000000;\" src=\"./package/portal/mini/images/" + data.userAvatar + "\">";
                append = append + "<p>";
                append = append + "&nbsp;&nbsp;&nbsp;<img src=\"./images/icons/user.png\"> <b>" + data.userName + "</b>";
                append = append + "<br>";
                append = append + "&nbsp;&nbsp;&nbsp;<img src=\"./images/icons/balloon.png\">" + data.ticketText;
                append = append + "<br>";
                append = append + "&nbsp;&nbsp;&nbsp;<img src=\"./images/icons/clock-history.png\"> <abbr class=\"timeago\" title=\"" + data.isoDate + "\">" + data.detailDate + "</abbr>";
                append = append + "</p>";
                append = append + "<div class=\"btn-group\">";
                if (isAdmin == 1) {
                    append = append + "<a href=\"#\" class=\"btn btn-warning\">Review</a>";
                    append = append + "<a href=\"#\" class=\"btn btn-success\">Solve</a>";
                }
                append = append + "</div></div>";
                append = append + " </div>";
                append = append + " </div>";
                $("#ticketReply").append(append);
                sleep(500);
                window.scrollTo(0, document.body.scrollHeight - 50); // so to obvious below
...
function showTicketThread(leafId, url, securityToken, ticketId, loadingText, loadingCompleteText, loadingErrorText) {
// unlimited for searching because  lazy paging.
    sleep(500);
    $.ajax({
        type: 'POST',
        ...
        success: function(data) {
            ...
            if (data.success == false) {
                ...
                $('#centerViewport').html('<div class=\'alert alert-error  col-md-12\'><img src=\'./images/icons/smiley-roll.png\'>' + decodeURIComponent(t['loadingCompleteTextLabel']) + '</div>');
...
function showFormUpdate(leafId, url, securityToken, ticketId, loadingText, loadingCompleteText, loadingErrorText) {
    sleep(500);
    // unlimited for searching because  lazy paging.
    $.ajax({
        type: 'POST',
...
function showModalDelete(ticketId, userIdFrom, userIdTo, ticketText) {
...
function deleteGridRecordCheckbox(leafId, url, urlList, securityToken, loadingText, loadingCompleteText, loadingErrorText, deleteRecordText, deleteCheckboxText) {
    var stringText = '';
    var counter = 0;
    $('input:checkbox[name="ticketId[]"]').each(function() {
        stringText = stringText + "&ticketId[]=" + $(this).val();
    });
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($('#previousRecordCounter').val()) > 0 && parseFloat($('#previousRecordCounter').val()) < parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    ticketId: $('#previousRecordCounter').val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($('#nextRecordCounter').val()) < parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    ticketId: $('#nextRecordCounter').val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
