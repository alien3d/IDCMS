function checkDuplicate(leafId, page, securityToken) {
    var $notificationCode = $("#notificationCode");
    if ($notificationCode.val().length === 0) {
    ...
    $.ajax({
        ...
        url: page,
        data: {
            notificationCode: $notificationCode.val(),
            method: 'duplicate',
...
function showFormUpdate(leafId, url, urlList, securityToken, notificationId, updateAccess, deleteAccess) {
    sleep(500);
...
function showModalDelete(notificationId, notificationTypeId, staffId, notificationFrom, notificationMessage) {
    // clear first old record if exist
    $('#notificationIdPreview').val('').val(decodeURIComponent(notificationId));
    $('#notificationTypeIdPreview').val('').val(decodeURIComponent(notificationTypeId));
    $('#staffIdPreview').val('').val(decodeURIComponent(staffId));
    $('#notificationFromPreview').val('').val(decodeURIComponent(notificationFrom));
    $('#notificationMessagePreview').val('').val(decodeURIComponent(notificationMessage));
...
function deleteGridRecord(leafId, url, urlList, securityToken) {
    $.ajax({
        ...
        data: {
            method: 'delete',
            output: 'json',
            notificationId: $('#notificationIdPreview').val(),
            securityToken: securityToken,
            leafId: leafId
        },
        beforeSend: function() {
...
function deleteGridRecordCheckbox(leafId, url, urlList, securityToken) {
    var stringText = '';
    var counter = 0;
    $('input:checkbox[name="notificationId[]"]').each(function() {
        stringText = stringText + "&notificationId[]=" + $(this).val();
    });
...
function newRecord(leafId, url, urlList, securityToken, type, createAccess, updateAccess, deleteAccess) {
    ...
    var $notificationTypeId = $('#notificationTypeId');
    var $staffId = $('#staffId');
    var $notificationFrom = $('#notificationFrom');
    var $notificationMessage = $('#notificationMessage');
    if (css.search('disabled') > 0) {
        return false;
    } else {
        if (type === 1) {
            if ($notificationTypeId.val().length === 0) {
                $('#notificationTypeIdHelpMe')
                        .html('').empty()
                        .html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['notificationTypeIdLabel'] + " </span>");
                $('#notificationTypeIdForm')
                        .removeClass().addClass('form-group has-error');
                $notificationTypeId.focus();
                return false;
            }
            ...
            if ($notificationFrom.val().length === 0) {
                $('#notificationFromHelpMe')
                        .html('').empty()
                        .html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['notificationFromLabel'] + " </span>");
                $('#notificationFromForm')
                        .removeClass().addClass('form-group has-error');
                $notificationFrom.focus();
                return false;
            }
            if ($notificationMessage.val().length === 0) {
                $('#notificationMessageHelpMe')
                        .html('').empty()
                        .html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['notificationMessageLabel'] + " </span>");
            ...
            $.ajax({
                ...
                data: {
                    method: 'create',
                    output: 'json',
                    notificationTypeId: $notificationTypeId.val(),
                    staffId: $staffId.val(),
                    notificationFrom: $notificationFrom.val(),
        ...
        if (type === 1) {
            ...
            if ($notificationTypeId.val().length === 0) {
                $('#notificationTypeIdHelpMe')
                        .html('').empty()
                        .html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['notificationTypeIdLabel'] + " </span>");
                $('#notificationTypeIdForm').removeClass().addClass('form-group has-error');
                $notificationTypeId.focus();
                return false;
            }
            if ($staffId.val().length === 0) {
            ...
            if ($notificationFrom.val().length === 0) {
                $('#notificationFromHelpMe')
                        .html('').empty()
                        .html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['notificationFromLabel'] + " </span>");
                $('#notificationFromForm').removeClass().addClass('form-group has-error');
                $notificationFrom.focus();
                return false;
            }
            if ($notificationMessage.val().length === 0) {
                $('#notificationMessageHelpMe')
                        .html('').empty()
                        .html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['notificationMessageLabel'] + " </span>");
                $('#notificationMessageForm').removeClass().addClass('form-group has-error');
                $notificationMessage.focus();
                return false;
            }
            if ($infoPanel.is(':hidden')) {
                ...
                if (success === true) {
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if (success === true) {
                                $('#notificationId').val(data.data.notificationId);
                                $('#notificationTypeId').val(data.data.notificationTypeId).trigger("chosen:updated");
                                $('#staffId').val(data.data.staffId);
                                $('#notificationFrom').val(data.data.notificationFrom);
                                $('#notificationMessage').val(data.data.notificationMessage);
                                if (nextRecord > 0) {
                                    $('#previousRecordButton')
                                            .removeClass().addClass('btn btn-default disabled')
                                            .attr('onClick', '');
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($previousRecordCounter.val()) > 0 && parseFloat($previousRecordCounter.val()) < parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    notificationId: $previousRecordCounter.val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($nextRecordCounter.val()) <= parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    notificationId: $nextRecordCounter.val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
