...
function checkDuplicate(leafId, page, securityToken) {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if (success === true) {
                if (total !== 0) {
                    $("#locationCode").val('').focus();
                    $("#locationCodeForm").removeClass().addClass("form-group has-error");
                ...
                $("#locationForm").removeClass().addClass("form-group has-error");
            }
            if ($infoPanel.is(':hidden')) {
                $infoPanel.show();
            }
...
function showFormUpdate(leafId, url, urlList, securityToken, locationId, updateAccess, deleteAccess) {
    sleep(500);
...
function showModalDelete(locationId, branchId, departmentId, locationCode, locationDescription) {
    ...
    $('#locationCodePreview').val('').val(decodeURIComponent(locationCode));
    $('#locationDescriptionPreview').val('').val(decodeURIComponent(locationDescription));
...
function deleteGridRecord(leafId, url, urlList, securityToken) {
    $.ajax({
        ...
        data: {
            method: 'delete',
            output: 'json',
            locationId: $('#locationIdPreview').val(),
            securityToken: securityToken,
            leafId: leafId
        },
        beforeSend: function() {
...
function deleteGridRecordCheckbox(leafId, url, urlList, securityToken) {
    var stringText = '';
    var counter = 0;
    $('input:checkbox[name="locationId[]"]').each(function() {
        stringText = stringText + "&locationId[]=" + $(this).val();
    });
...
function newRecord(leafId, url, urlList, securityToken, type, createAccess, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        if (type === 1) {
            ...
            if ($('#locationCode').val().length === 0) {
                $('#locationCodeHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['locationCodeLabel'] + " </span>");
                $('#locationCodeForm').removeClass().addClass('form-group has-error');
                $('#locationCode').focus();
                return false;
            }
            if ($('#locationDescription').val().length === 0) {
                $('#locationDescriptionHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['locationDescriptionLabel'] + " </span>");
                $('#locationDescriptionForm').removeClass().addClass('form-group has-error');
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (success === true) {
                        ...
                        $('#locationCode').val('');
                        $('#locationCode').val('');
                        $('#locationCodeHelpMe').empty().html('');
                        $('#locationDescription').val('');
                        $('#locationDescriptionForm').removeClass().addClass('form-group');
                        $('#locationDescription').val('');
                        $('#locationDescriptionHelpMe').empty().html('');
                    } else if (success === false) {
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if (success === true) {
                                ...
                                ;
                                $('#locationCode').val(data.data.locationCode);
                                $('#locationDescription').val(data.data.locationDescription);
                                if (nextRecord > 0) {
                                    $('#previousRecordButton').removeClass().addClass('btn btn-default disabled').attr('onClick', '');
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($previousRecordCounter.val()) > 0 && parseFloat($previousRecordCounter.val()) < parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    locationId: $previousRecordCounter.val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($nextRecordCounter.val()) <= parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    locationId: $nextRecordCounter.val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
