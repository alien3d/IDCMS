...
function checkDuplicate(leafId, page, securityToken) {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if (success === true) {
                if (total !== 0) {
                    $("#departmentCode").val('').focus();
                    $("#departmentCodeForm").removeClass().addClass("form-group has-error");
                ...
                $("#departmentForm").removeClass().addClass("form-group has-error");
            }
            if ($infoPanel.is(':hidden')) {
                $infoPanel.show();
            }
...
function showFormUpdate(leafId, url, urlList, securityToken, departmentId, updateAccess, deleteAccess) {
    sleep(500);
...
function showModalDelete(departmentId, branchId, departmentCode, departmentDescription) {
    $('#departmentIdPreview').val('').val(decodeURIComponent(departmentId));
    $('#branchIdPreview').val('').val(decodeURIComponent(branchId));
    $('#departmentCodePreview').val('').val(decodeURIComponent(departmentCode));
    $('#departmentDescriptionPreview').val('').val(decodeURIComponent(departmentDescription));
...
function deleteGridRecord(leafId, url, urlList, securityToken) {
    $.ajax({
        ...
        data: {
            method: 'delete',
            output: 'json',
            departmentId: $('#departmentIdPreview').val(),
            securityToken: securityToken,
            leafId: leafId
        },
        beforeSend: function() {
...
function deleteGridRecordCheckbox(leafId, url, urlList, securityToken) {
    ...
    $('input:checkbox[name="departmentId[]"]').each(function() {
        stringText = stringText + "&departmentId[]=" + $(this).val();
    });
...
function newRecord(leafId, url, urlList, securityToken, type, createAccess, updateAccess, deleteAccess) {
    ...
    var $departmentCode = $('#departmentCode');
    var $departmentDescription = $('#departmentDescription');
    if (css.search('disabled') > 0) {
        return false;
    } else {
        if (type === 1) {
            ...
            if ($departmentCode.val().length === 0) {
                $('#departmentCodeHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['departmentCodeLabel'] + " </span>");
                $('#departmentCodeForm').removeClass().addClass('form-group has-error');
                $departmentCode.focus();
                return false;
            }
            if ($departmentDescription.val().length === 0) {
                $('#departmentDescriptionHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['departmentDescriptionLabel'] + " </span>");
                $('#departmentDescriptionForm').removeClass().addClass('form-group has-error');
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (success === true) {
                        ...
                        $departmentCode.val('');
                        $('#departmentCodeHelpMe').html('').empty();
                        $departmentDescription.val('');
                        $('#departmentDescriptionForm').removeClass().addClass('form-group');
                        $('#departmentDescriptionHelpMe').html('').empty();
                    } else if (success === false) {
                        $infoPanel.html('').empty().html("<span class='label label-important'>&nbsp;" + message + "</span>");
                        if ($infoPanel.is(':hidden')) {
                            $infoPanel.show();
...
function resetRecord(leafId, url, urlList, securityToken, createAccess, updateAccess, deleteAccess) {
    ...
    $('#branchId').trigger("chosen:updated");
    $("#departmentCode").val('');
    $("#departmentCodeHelpMe").empty().html('');
    $("#departmentDescription").val('');
    $("#departmentDescriptionHelpMe").empty().html('');
    $('#departmentDescription').empty().val('');
}
function postRecord() {
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if (success === true) {
                                ...
                                $('#departmentDescription').val(data.data.departmentDescription);
                                if (nextRecord > 0) {
                                    $('#previousRecordButton').removeClass().addClass('btn btn-default disabled').attr('onClick', '');
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($previousRecordCounter.val()) > 0 && parseFloat($previousRecordCounter.val()) < parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    departmentId: $previousRecordCounter.val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($nextRecordCounter.val()) <= parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    departmentId: $nextRecordCounter.val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
