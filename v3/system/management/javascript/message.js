    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if (data.success == false) {
                ...
                $('#centerViewport').html('').empty().removeClass().append(data);
                $('#infoPanel').empty();
                $('#infoPanel').html('');
                $('#infoPanel').html("<span class='label label-success'>&nbsp;<img src='./images/icons/smiley-lol.png'> " + decodeURIComponent(t['loadingCompleteTextLabel']) + "</span>").delay(1000).fadeOut();
                if ($('#infoPanel').is(':hidden')) {
...
function showModalDelete(messageId, staffIdFrom, staffIdTo, messageTitle, messageDescription, messageFile, isRead) {
    ...
    $('#messageTitlePreview').val('');
    $('#messageTitlePreview').val(decodeURIComponent(messageTitle));

    $('#messageDescriptionPreview').val('');
    $('#messageDescriptionPreview').val(decodeURIComponent(messageDescription));

    $('#messageFilePreview').val('');
    $('#messageFilePreview').val(decodeURIComponent(messageFile));

    $('#isReadPreview').val('');
    $('#isReadPreview').val(decodeURIComponent(isRead));
    ...
    if (css.search('disabled') > 0) {
        ...
        if (type == 1) {
            ...
            if ($('#messageTitle').val().length == 0) {
                $('#messageTitleHelpMe').empty();
                $('#messageTitleHelpMe').html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['messageTitleLabel'] + " </span>");
                $('#messageTitleForm').addClass('form-group has-error');
                $('#messageTitle').focus();
                return false;
            }
            if ($('#messageDescription').val().length == 0) {
                $('#messageDescriptionHelpMe').empty();
                $('#messageDescriptionHelpMe').html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['messageDescriptionLabel'] + " </span>");
                $('#messageDescriptionForm').addClass('form-group has-error');
            ...
            if ($('#isRead').val().length == 0) {
                $('#isReadHelpMe').empty();
                $('#isReadHelpMe').html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['isReadLabel'] + " </span>");
                $('#isReadForm').addClass('form-group has-error');
                $('#isRead').focus();
                return false;
            }
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (data.success == true) {
                        ...
                        $('#messageTitle').val('');
                        $('#messageTitleHelpMe').empty();
                        $('#messageTitleHelpMe').html('');
                        $('#messageDescription').val('');
                        $('#messageDescription').val('');
                        $('#messageDescriptionHelpMe').empty();
                        $('#messageDescriptionHelpMe').html('');
                        $('#messageFile').val('');
                        $('#messageFile').val('');
                        $('#messageFileHelpMe').empty();
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if (data.success == true) {
                                ...
                                $('#messageTitle').val(data.data.messageTitle);
                                $('#messageDescription').val(data.data.messageDescription);
                                $('#messageDescription').val(data.data.messageDescription);
                                $('#messageFile').val(data.data.messageFile);
                                $('#messageFile').val(data.data.messageFile);
                                $('#isRead').val(data.data.isRead);
