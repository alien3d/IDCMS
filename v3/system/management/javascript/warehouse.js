function getBranch(leafId, url, securityToken) {
    $.ajax({
        ...
        success: function(data) {
            ...
            if (success === false) {
                $infoPanel.html('').empty().html("<span class='label label-important'>&nbsp;<img src='" + smileyRoll + "'>" + data.message + "</spam>");
            } else {
...
function checkDuplicate(leafId, page, securityToken) {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if (success === true) {
                if (total !== 0) {
                    $("#warehouseCode").val('').focus();
                    $("#warehouseCodeForm").removeClass().addClass("form-group has-error");
                ...
                $("#warehouseForm").removeClass().addClass("form-group has-error");
            }
            if ($infoPanel.is(':hidden')) {
                $infoPanel.show();
            }
...
function showFormUpdate(leafId, url, urlList, securityToken, warehouseId, updateAccess, deleteAccess) {
    sleep(500);
...
function showModalDelete(warehouseId, branchId, warehouseCode, warehouseDescription, warehouseAddress, warehousePostcode) {
    $('#warehouseIdPreview').val('').val(decodeURIComponent(warehouseId));
    $('#branchIdPreview').val('').val(decodeURIComponent(branchId));
    $('#warehouseCodePreview').val('').val(decodeURIComponent(warehouseCode));
    $('#warehouseDescriptionPreview').val('').val(decodeURIComponent(warehouseDescription));
    $('#warehouseAddressPreview').val('').val(decodeURIComponent(warehouseAddress));
    $('#warehousePostcodePreview').val('').val(decodeURIComponent(warehousePostcode));
...
function deleteGridRecord(leafId, url, urlList, securityToken) {
    $.ajax({
        ...
        data: {
            method: 'delete',
            output: 'json',
            warehouseId: $('#warehouseIdPreview').val(),
            securityToken: securityToken,
            leafId: leafId
        },
        beforeSend: function() {
...
function deleteGridRecordCheckbox(leafId, url, urlList, securityToken) {
    var stringText = '';
    var counter = 0;
    $('input:checkbox[name="warehouseId[]"]').each(function() {
        stringText = stringText + "&warehouseId[]=" + $(this).val();
    });
...
function newRecord(leafId, url, urlList, securityToken, type, createAccess, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        if (type === 1) {
            ...
            if ($('#warehouseCode').val().length === 0) {
                $('#warehouseCodeHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['warehouseCodeLabel'] + " </span>");
                $('#warehouseCodeForm').removeClass().addClass('form-group has-error');
                $('#warehouseCode').focus();
                return false;
            }
            if ($('#warehouseDescription').val().length === 0) {
                $('#warehouseDescriptionHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['warehouseDescriptionLabel'] + " </span>");
                $('#warehouseDescriptionForm').removeClass().addClass('form-group has-error');
                $('#warehouseDescription').focus();
                return false;
            }
            if ($('#warehouseAddress').val().length === 0) {
                $('#warehouseAddressHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['warehouseAddressLabel'] + " </span>");
                $('#warehouseAddressForm').removeClass().addClass('form-group has-error');
                $('#warehouseAddress').focus();
                return false;
            }
            if ($('#warehousePostcode').val().length === 0) {
                $('#warehousePostcodeHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['warehousePostcodeLabel'] + " </span>");
                $('#warehousePostcodeForm').removeClass().addClass('form-group has-error');
            ...
            $.ajax({
                ...
                data: {
                    ...
                    warehouseCode: $('#warehouseCode').val(),
                    warehouseDescription: $('#warehouseDescription').val(),
                    warehouseAddress: $('#warehouseAddress').val(),
                    warehousePostcode: $('#warehousePostcode').val(),
                    securityToken: securityToken,
                    leafId: leafId
                },
                beforeSend: function() {
                ...
                success: function(data) {
                    ...
                    if (success === true) {
                        ...
                        $('#warehouseCode').val('');
                        $('#warehouseCode').val('');
                        $('#warehouseCodeHelpMe').empty().html('');
                        $('#warehouseDescription').val('');
                        $('#warehouseDescriptionForm').removeClass().addClass('form-group');
                        $('#warehouseDescription').val('');
                        $('#warehouseDescriptionHelpMe').empty().html('');
                        $('#warehouseAddress').val('');
                        $('#warehouseAddressForm').removeClass().addClass('form-group');
                        $('#warehouseAddress').val('');
                        $('#warehouseAddressHelpMe').empty().html('');
                        $('#warehousePostcode').val('');
                        $('#warehousePostcodeForm').removeClass().addClass('form-group');
                        $('#warehousePostcode').val('');
                        $('#warehousePostcodeHelpMe').empty().html('');
                    } else if (success === false) {
                        $infoPanel.html('').empty().html("<span class='label label-important'>&nbsp;" + data.message + "</span>");
                        if ($infoPanel.is(':hidden')) {
                            $infoPanel.show();
                        }
                    }
...
function resetRecord(leafId, url, urlList, securityToken, createAccess, updateAccess, deleteAccess) {
    ...
    $("#warehouseDescription").val('');
    $("#warehouseDescriptionHelpMe").empty().html('');
    $('#warehouseDescription').empty().val('');
    $("#warehouseAddress").val('');
    $("#warehouseAddressHelpMe").empty().html('');
    $('#warehouseAddress').empty().val('');
    $("#warehousePostcode").val('');
    $("#warehousePostcodeHelpMe").empty().html('');
    $('#warehousePostcode').empty().val('');
}
function postRecord() {
                ...
                if (success === true) {
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            var $infoPanel = $('#infoPanel');
                            if (success === true) {
                                $('#warehouseId').val(data.data.warehouseId);
                                $('#branchId').val(data.data.branchId).trigger("chosen:updated");
                                ;
                                $('#warehouseCode').val(data.data.warehouseCode);
                                $('#warehouseDescription').val(data.data.warehouseDescription);
                                $('#warehouseAddress').val(data.data.warehouseAddress);
                                $('#warehousePostcode').val(data.data.warehousePostcode);
                                if (nextRecord > 0) {
                                    $('#previousRecordButton').removeClass().addClass('btn btn-default disabled').attr('onClick', '');
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($previousRecordCounter.val()) > 0 && parseFloat($previousRecordCounter.val()) < parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    warehouseId: $previousRecordCounter.val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($nextRecordCounter.val()) <= parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    warehouseId: $nextRecordCounter.val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
