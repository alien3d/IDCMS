function checkDuplicate(leafId, page, securityToken) {
    var $todoCategoryCode = $("#todoCategoryCode");
    if ($todoCategoryCode.val().length === 0) {
    ...
    $.ajax({
        ...
        url: page,
        data: {
            todoCategoryCode: $todoCategoryCode.val(),
            method: 'duplicate',
...
function showFormUpdate(leafId, url, urlList, securityToken, todoCategoryId, updateAccess, deleteAccess) {
    sleep(500);
...
function showModalDelete(todoCategoryId, todoCategoryDescription) {
    // clear first old record if exist
    $('#todoCategoryIdPreview').val('').val(decodeURIComponent(todoCategoryId));
    $('#todoCategoryDescriptionPreview').val('').val(decodeURIComponent(todoCategoryDescription));
...
function deleteGridRecord(leafId, url, urlList, securityToken) {
    $.ajax({
        ...
        data: {
            method: 'delete',
            output: 'json',
            todoCategoryId: $('#todoCategoryIdPreview').val(),
            securityToken: securityToken,
            leafId: leafId
        },
        beforeSend: function() {
...
function deleteGridRecordCheckbox(leafId, url, urlList, securityToken) {
    var stringText = '';
    var counter = 0;
    $('input:checkbox[name="todoCategoryId[]"]').each(function() {
        stringText = stringText + "&todoCategoryId[]=" + $(this).val();
    });
...
function newRecord(leafId, url, urlList, securityToken, type, createAccess, updateAccess, deleteAccess) {
    ...
    var $todoCategoryDescription = $('#todoCategoryDescription');
    if (css.search('disabled') > 0) {
        return false;
    } else {
        if (type === 1) {
            if ($todoCategoryDescription.val().length === 0) {
                $('#todoCategoryDescriptionHelpMe')
                        .html('').empty()
                        .html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['todoCategoryDescriptionLabel'] + " </span>");
                $('#todoCategoryDescriptionForm')
                        .removeClass().addClass('form-group has-error');
                $todoCategoryDescription.focus();
                return false;
            }
            $.ajax({
                type: 'POST',
                ...
                data: {
                    method: 'create',
                    output: 'json',
                    todoCategoryDescription: $todoCategoryDescription.val(),
                    securityToken: securityToken,
                    leafId: leafId
                },
                beforeSend: function() {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (success === true) {
                        ...
                        $('#todoCategoryId').val(data.todoCategoryId);
                        $('#newRecordButton1')
                                .removeClass().addClass('btn btn-success disabled');
                        $('#newRecordButton2')
                                .removeClass().addClass('btn dropdown-toggle btn-success disabled');
        ...
        if (type === 1) {
            ...
            if ($todoCategoryDescription.val().length === 0) {
                $('#todoCategoryDescriptionHelpMe')
                        .html('').empty()
                        .html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['todoCategoryDescriptionLabel'] + " </span>");
                $('#todoCategoryDescriptionForm').removeClass().addClass('form-group has-error');
                $todoCategoryDescription.focus();
                return false;
            }
            if ($infoPanel.is(':hidden')) {
                ...
                if (success === true) {
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if (success === true) {
                                $('#todoCategoryId').val(data.data.todoCategoryId);
                                $('#todoCategoryDescription').val(data.data.todoCategoryDescription);
                                if (nextRecord > 0) {
                                    $('#previousRecordButton')
                                            .removeClass().addClass('btn btn-default disabled')
                                            .attr('onClick', '');
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($previousRecordCounter.val()) > 0 && parseFloat($previousRecordCounter.val()) < parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    todoCategoryId: $previousRecordCounter.val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($nextRecordCounter.val()) <= parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    todoCategoryId: $nextRecordCounter.val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
