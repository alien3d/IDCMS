...
function getMessage(leafId, url, securityToken) {
    $.ajax({
        type: 'GET',
        url: url,
        data: {
...
function checkDuplicate(leafId, page, securityToken) {
    var $messageAttachmentCode = $("#messageAttachmentCode");
    if ($messageAttachmentCode.val().length === 0) {
    ...
    $.ajax({
        ...
        url: page,
        data: {
            messageAttachmentCode: $messageAttachmentCode.val(),
            method: 'duplicate',
            securityToken: securityToken,
            leafId: leafId
        },
...
function showFormUpdate(leafId, url, urlList, securityToken, messageAttachmentId, updateAccess, deleteAccess) {
    sleep(500);
...
function showModalDelete(messageAttachmentId, documentAttachmentId, messageId) {
    // clear first old record if exist
    $('#messageAttachmentIdPreview').val('').val(decodeURIComponent(messageAttachmentId));
    $('#documentAttachmentIdPreview').val('').val(decodeURIComponent(documentAttachmentId));
    $('#messageIdPreview').val('').val(decodeURIComponent(messageId));
...
function deleteGridRecord(leafId, url, urlList, securityToken) {
    $.ajax({
        ...
        data: {
            method: 'delete',
            output: 'json',
            messageAttachmentId: $('#messageAttachmentIdPreview').val(),
            securityToken: securityToken,
            leafId: leafId
        },
        beforeSend: function() {
...
function deleteGridRecordCheckbox(leafId, url, urlList, securityToken) {
    var stringText = '';
    var counter = 0;
    $('input:checkbox[name="messageAttachmentId[]"]').each(function() {
        stringText = stringText + "&messageAttachmentId[]=" + $(this).val();
    });
    ...
    if (css.search('disabled') > 0) {
        ...
        if (type === 1) {
            if ($documentAttachmentId.val().length === 0) {
                $('#documentAttachmentIdHelpMe')
                        .html('').empty()
                        .html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['documentAttachmentIdLabel'] + " </span>");
                $documentAttachmentId.data('chosen').activate_action();
                return false;
            }
            if ($messageId.val().length === 0) {
                $('#messageIdHelpMe')
                        .html('').empty()
                        .html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['messageIdLabel'] + " </span>");
                $messageId.data('chosen').activate_action();
                return false;
            }
            $.ajax({
                type: 'POST',
                ...
                success: function(data) {
                    ...
                    if (success === true) {
                        ...
                        $('#documentAttachmentIdHelpMe')
                                .html('').empty();
                        $messageId.val('');
                        $messageId.trigger("chosen:updated");
                        $('#messageIdHelpMe')
                                .html('').empty();
                    } else if (success === false) {
                        $infoPanel
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (success === true) {
                        ...
                        $('#messageAttachmentId').val(data.messageAttachmentId);
                        $('#newRecordButton1')
                                .removeClass().addClass('btn btn-success disabled');
                        $('#newRecordButton2')
                                .removeClass().addClass('btn dropdown-toggle btn-success disabled');
...
function updateRecord(leafId, url, urlList, securityToken, type, deleteAccess) {
    ...
    var $messageAttachmentId = $('#messageAttachmentId');
    var $documentAttachmentId = $('#documentAttachmentId');
    var $messageId = $('#messageId');
    if (css.search('disabled') > 0) {
        return false;
        ...
        if (type === 1) {
            ...
            $.ajax({
                ...
                data: {
                    method: 'save',
                    output: 'json',
                    messageAttachmentId: $messageAttachmentId.val(),
                    documentAttachmentId: $documentAttachmentId.val(),
                    messageId: $messageId.val(),
                    securityToken: securityToken,
                    leafId: leafId
    ...
    if (css.search('disabled') > 0) {
        ...
        if (deleteAccess === 1) {
            if (confirm(decodeURIComponent(t['deleteRecordMessageLabel']))) {
                var value = $messageAttachmentId.val();
                if (!value) {
                    $infoPanel
                            .html('').empty()
...
function resetRecord(leafId, url, urlList, urlPurchaseInvoiceCreditNoteDetail, securityToken, createAccess,updateAccess,deleteAccess) {
    ...
    $("#messageAttachmentId").val('');
    $("#messageAttachmentIdHelpMe")
            .empty().html('');
            ...
            success: function(data) {
                ...
                if (success === true) {
                    $.ajax({
                        ...
                        data: {
                            method: 'read',
                            messageAttachmentId: firstRecord,
                            output: 'json',
                            securityToken: securityToken,
                            leafId: leafId
                        },
                        ...
                        success: function(data) {
                            ...
                            if (success === true) {
                                $('#messageAttachmentId').val(data.data.messageAttachmentId);
                                $('#documentAttachmentId').val(data.data.documentAttachmentId).trigger("chosen:updated");
                                $('#messageId').val(data.data.messageId).trigger("chosen:updated");
                                if (nextRecord > 0) {
                                    $('#previousRecordButton')
                                            .removeClass().addClass('btn btn-default disabled')
                                            .attr('onClick', '');
            ...
            success: function(data) {
                ...
                if (success === true) {
                    $.ajax({
                        ...
                        data: {
                            method: 'read',
                            messageAttachmentId: lastRecord,
                            output: 'json',
                            securityToken: securityToken,
                            leafId: leafId
                        },
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($previousRecordCounter.val()) > 0 && parseFloat($previousRecordCounter.val()) < parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    messageAttachmentId: $previousRecordCounter.val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($nextRecordCounter.val()) <= parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    messageAttachmentId: $nextRecordCounter.val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
