    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if (success === true) {
                if (total !== 0) {
                    $("#unitCode").val('').focus();
                    $("#unitCodeForm").removeClass().addClass("form-group has-error");
                ...
                $("#unitForm").removeClass().addClass("form-group has-error");
            }
            if ($infoPanel.is(':hidden')) {
                $infoPanel.show();
            }
...
function showFormUpdate(leafId, url, urlList, securityToken, unitId, updateAccess, deleteAccess) {
    sleep(500);
...
function showModalDelete(unitId, branchId, departmentId, unitCode, unitDescription) {
    $('#unitIdPreview').val('').val(decodeURIComponent(unitId));
    $('#branchIdPreview').val('').val(decodeURIComponent(branchId));
    $('#departmentIdPreview').val('').val(decodeURIComponent(departmentId));
    $('#unitCodePreview').val('').val(decodeURIComponent(unitCode));
    $('#unitDescriptionPreview').val('').val(decodeURIComponent(unitDescription));
...
function deleteGridRecordCheckbox(leafId, url, urlList, securityToken) {
    var stringText = '';
    var counter = 0;
    $('input:checkbox[name="unitId[]"]').each(function() {
        stringText = stringText + "&unitId[]=" + $(this).val();
    });
...
function newRecord(leafId, url, urlList, securityToken, type, createAccess, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        if (type === 1) {
            ...
            if ($('#unitCode').val().length === 0) {
                $('#unitCodeHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['unitCodeLabel'] + " </span>");
                $('#unitCodeForm').removeClass().addClass('form-group has-error');
                $('#unitCode').focus();
                return false;
            }
            if ($('#unitDescription').val().length === 0) {
                $('#unitDescriptionHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['unitDescriptionLabel'] + " </span>");
                $('#unitDescriptionForm').removeClass().addClass('form-group has-error');
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (success === true) {
                        ...
                        $('#unitCode').val('');
                        $('#unitCode').val('');
                        $('#unitCodeHelpMe').empty().html('');
                        $('#unitDescription').val('');
                        $('#unitDescriptionForm').removeClass().addClass('form-group');
                        $('#unitDescription').val('');
                        $('#unitDescriptionHelpMe').empty().html('');
                    } else if (success === false) {
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if (success === true) {
                                ...
                                ;
                                $('#unitCode').val(data.data.unitCode);
                                $('#unitDescription').val(data.data.unitDescription);
                                if (nextRecord > 0) {
                                    $('#previousRecordButton').removeClass().addClass('btn btn-default disabled').attr('onClick', '');
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($previousRecordCounter.val()) > 0 && parseFloat($previousRecordCounter.val()) < parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    unitId: $previousRecordCounter.val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
