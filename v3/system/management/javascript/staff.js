function getRole(leafId, url, securityToken) {
    $.ajax({
        type: 'GET',
        url: url,
        data: {
...
function getLanguage(leafId, url, securityToken) {
    $.ajax({
        type: 'GET',
        url: url,
        data: {
...
function getTheme(leafId, url, securityToken) {
    $.ajax({
        type: 'GET',
        url: url,
        data: {
...
function showFormUpdate(leafId, url, urlList, securityToken, staffId, updateAccess, deleteAccess) {
    sleep(500);
...
function showModalDelete(staffId, roleId, languageId, themeId, staffName, staffPassword, staffEmail, staffAvatar) {
    ...
    $('#staffNamePreview').val('').val(decodeURIComponent(staffName));

    $('#staffPasswordPreview').val('').val(decodeURIComponent(staffPassword));

    $('#staffEmailPreview').val('').val(decodeURIComponent(staffEmail));

    $('#staffAvatarPreview').val('').val(decodeURIComponent(staffAvatar));
...
function deleteGridRecordCheckbox(leafId, url, urlList, securityToken) {
    ...
    $('input:checkbox[name="staffId[]"]').each(function() {
        stringText = stringText + "&staffId[]=" + $(this).val();
    });
...
function newRecord(leafId, url, urlList, securityToken, type, createAccess, updateAccess, deleteAccess) {
    ...
    var $staffPassword = $('#staffPassword');
    var $staffEmail = $('#staffEmail');
    var $staffAvatar = $('#staffAvatar');
    if (css.search('disabled') > 0) {
        return false;
    } else {
        if (type === 1) {
            if ($roleId.val().length === 0) {
                ...
                $roleId.data('chosen').activate_action();
                return false;
            }
            ...
            if ($themeId.val().length === 0) {
                $('#themeIdHelpMe')
                        .html('').empty()
                        .html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['themeIdLabel'] + " </span>");
                $themeId.data('chosen').activate_action();
                return false;
            }
            if ($staffName.val().length === 0) {
                $('#staffNameHelpMe')
                        .html('').empty()
                        .html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['staffNameLabel'] + " </span>");
                $('#staffNameForm')
                        .removeClass().addClass('form-group has-error');
                $staffName.focus();
                return false;
            }
            if ($staffPassword.val().length === 0) {
                $('#staffPasswordHelpMe')
                        .html('').empty()
                        .html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['staffPasswordLabel'] + " </span>");
                $('#staffPasswordForm')
                        .removeClass().addClass('form-group has-error');
                $staffPassword.focus();
                return false;
            }
            if ($staffEmail.val().length === 0) {
                $('#staffEmailHelpMe')
                        .html('').empty()
                        .html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['staffEmailLabel'] + " </span>");
                $('#staffEmailForm')
                        .removeClass().addClass('form-group has-error');
                $staffEmail.focus();
                return false;
            }
            if ($staffAvatar.val().length === 0) {
                $('#staffAvatarHelpMe')
                        .html('').empty()
                        .html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['staffAvatarLabel'] + " </span>");
                $('#staffAvatarForm')
                        .removeClass().addClass('form-group has-error');
                $staffAvatar.focus();
                return false;
                ...
                success: function(data) {
                    ...
                    if (success === true) {
                        ...
                        $('#languageIdHelpMe')
                                .html('').empty();
                        $themeId.val('');
                        $themeId.trigger("chosen:updated");
                        $('#themeIdHelpMe')
                                .html('').empty();
                        $staffName.val('');
                        $('#staffNameForm')
        ...
        if (type === 1) {
            ...
            if ($staffName.val().length === 0) {
                $('#staffNameHelpMe')
                        .html('').empty()
                        .html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['staffNameLabel'] + " </span>");
                $('#staffNameForm').removeClass().addClass('form-group has-error');
                $staffName.focus();
                return false;
            }
            if ($staffPassword.val().length === 0) {
                $('#staffPasswordHelpMe')
                        .html('').empty()
                        .html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['staffPasswordLabel'] + " </span>");
                $('#staffPasswordForm').removeClass().addClass('form-group has-error');
                $staffPassword.focus();
                return false;
            }
            if ($staffEmail.val().length === 0) {
                $('#staffEmailHelpMe')
                        .html('').empty()
                        .html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['staffEmailLabel'] + " </span>");
                $('#staffEmailForm').removeClass().addClass('form-group has-error');
                $staffEmail.focus();
                return false;
            }
            if ($staffAvatar.val().length === 0) {
                $('#staffAvatarHelpMe')
                        .html('').empty()
                        .html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['staffAvatarLabel'] + " </span>");
                $('#staffAvatarForm').removeClass().addClass('form-group has-error');
                $staffAvatar.focus();
                return false;
            }
            if ($infoPanel.is(':hidden')) {
                        ...
                        success: function(data) {
                            ...
                            if (success === true) {
                                ...
                                $('#roleId').val(data.data.roleId).trigger("chosen:updated")
                                        ;
                                $('#languageId').val(data.data.languageId).trigger("chosen:updated")
                                        ;
                                $('#themeId').val(data.data.themeId).trigger("chosen:updated")
                                        ;
                                $('#staffName').val(data.data.staffName);
                                $('#staffPassword').val(data.data.staffPassword);
                                $('#staffEmail').val(data.data.staffEmail);
                                $('#staffAvatar').val(data.data.staffAvatar);
                                if (nextRecord > 0) {
                                    $('#previousRecordButton')
                                            .removeClass().addClass('btn btn-info disabled')
                                            .attr('onClick', '');
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($previousRecordCounter.val()) > 0 && parseFloat($previousRecordCounter.val()) < parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    staffId: $previousRecordCounter.val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
