...
namespace Core\System\Management\Unit\Controller;

use Core\ConfigClass;
use Core\System\Management\Unit\Model\UnitModel;
use Core\System\Management\Unit\Service\UnitService;
...
require_once($newFakeDocumentRoot . "v3/system/management/model/unitModel.php");
require_once($newFakeDocumentRoot . "v3/system/management/service/unitService.php");

/**
 * Class Unit
 * this is unit controller files.
 * @name IDCMS
 * @version 2
 * @author hafizan
 * @package  Core\System\Management\Unit\Controller
 * @subpackage Management
 * @link http://www.hafizan.com
 * @license http://www.gnu.org/copyleft/lesser.html LGPL
 */
class UnitClass extends ConfigClass {
    ...
    /**
     * Model
     * @var \Core\System\Management\Unit\Model\UnitModel
     */
    public $model;

    /**
     * Service-Business Application Process or other ajax request
     * @var \Core\System\Management\Unit\Service\UnitService
     */
    public $service;
    ...
    function __construct() {
        ...
        $this->setViewPath("./v3/system/management/view/unit.php");
        $this->setControllerPath("./v3/system/management/controller/unitController.php");
        $this->setServicePath("./v3/system/management/service/unitService.php");
    }
    ...
    public function create() {
        ...
        if ($this->getVendor() == self::MYSQL) {
            ...
            (
                 ...
                 '" . $this->model->getUnitCode() . "',
                 '" . $this->model->getUnitDescription() . "',
    ...
    public function read() {
        ...
        if (isset($_SESSION['isAdmin'])) {
            if ($_SESSION['isAdmin'] == 0) {
                if ($this->getVendor() == self::MYSQL) {
                    $this->setAuditFilter(
                            " `unit`.`isActive` = 1  AND `unit`.`companyId`='" . $this->getCompanyId() . "' "
                    );
                } else {
                    if ($this->getVendor() == self::MSSQL) {
                        $this->setAuditFilter(
                ...
                if ($_SESSION['isAdmin'] == 1) {
                    if ($this->getVendor() == self::MYSQL) {
                        $this->setAuditFilter("   `unit`.`companyId`='" . $this->getCompanyId() . "'	");
                    } else {
                        if ($this->getVendor() == self::MSSQL) {
                            $this->setAuditFilter(" [unit].[companyId]='" . $this->getCompanyId() . "' ");
                        } else {
       ...
       SELECT                    `unit`.`unitId`,
		  ...
		  FROM      `unit`
		  JOIN      `staff`
		  ON        `unit`.`executeBy` = `staff`.`staffId`
	JOIN	`company`
	ON		`company`.`companyId` = `unit`.`companyId`
	JOIN	`branch`
	ON		`branch`.`branchId` = `unit`.`branchId`
	JOIN	`department`
	ON		`department`.`departmentId` = `unit`.`departmentId`
		  WHERE     " . $this->getAuditFilter();
            if ($this->model->getUnitId(0, 'single')) {
                $sql .= " AND `unit`.`" . $this->model->getPrimaryKeyName() . "`='" . $this->model->getUnitId(
                                0, 'single'
                        ) . "'";
            }
            if ($this->model->getBranchId()) {
                $sql .= " AND `unit`.`branchId`='" . $this->model->getBranchId() . "'";
            }
            if ($this->model->getDepartmentId()) {
                $sql .= " AND `unit`.`departmentId`='" . $this->model->getDepartmentId() . "'";
            }
        } else {
            if ($this->getVendor() == self::MSSQL) {
	...
	ON		[department].[departmentId] = [unit].[departmentId]
		  WHERE     " . $this->getAuditFilter();
                if ($this->model->getUnitId(0, 'single')) {
                    $sql .= " AND [unit].[" . $this->model->getPrimaryKeyName(
                            ) . "]		=	'" . $this->model->getUnitId(0, 'single') . "'";
                }
                if ($this->model->getBranchId()) {
                    $sql .= " AND [unit].[branchId]='" . $this->model->getBranchId() . "'";
                }
		  ...
		  SELECT                    UNIT.UNITID AS \"unitId\",
                    ...
                    UNIT.COMPANYID AS \"companyId\",
                    BRANCH.branchName AS  \"branchName\",
                    UNIT.BRANCHID AS \"branchId\",
                    DEPARTMENT.DEPARTMENTDESCRIPTION AS  \"departmentDescription\",
                    UNIT.DEPARTMENTID AS \"departmentId\",
                    UNIT.UNITCODE AS \"unitCode\",
                    UNIT.UNITDESCRIPTION AS \"unitDescription\",
                    UNIT.ISDEFAULT AS \"isDefault\",
                    UNIT.ISNEW AS \"isNew\",
                    UNIT.ISDRAFT AS \"isDraft\",
                    UNIT.ISUPDATE AS \"isUpdate\",
                    UNIT.ISDELETE AS \"isDelete\",
                    UNIT.ISACTIVE AS \"isActive\",
                    UNIT.ISAPPROVED AS \"isApproved\",
                    UNIT.ISREVIEW AS \"isReview\",
                    UNIT.ISPOST AS \"isPost\",
                    UNIT.EXECUTEBY AS \"executeBy\",
                    UNIT.EXECUTETIME AS \"executeTime\",
	...
	ON		DEPARTMENT.DEPARTMENTID = UNIT.DEPARTMENTID
         WHERE     " . $this->getAuditFilter();
                    if ($this->model->getUnitId(0, 'single')) {
                        $sql .= " AND UNIT. " . strtoupper(
                                        $this->model->getPrimaryKeyName()
                                ) . "='" . $this->model->getUnitId(
                                        0, 'single'
                                ) . "'";
                    }
                    if ($this->model->getBranchId()) {
        ...
        if ($this->getCharacterQuery()) {
            if ($this->getVendor() == self::MYSQL) {
                $sql .= " AND `unit`.`" . $this->model->getFilterCharacter() . "` like '" . $this->getCharacterQuery(
                        ) . "%'";
            } else {
                if ($this->getVendor() == self::MSSQL) {
                    $sql .= " AND [unit].[" . $this->model->getFilterCharacter(
                            ) . "] like '" . $this->getCharacterQuery() . "%'";
                } else {
                    if ($this->getVendor() == self::ORACLE) {
                        $sql .= " AND Initcap(UNIT." . strtoupper(
        ...
        if ($this->getDateRangeStartQuery()) {
            if ($this->getVendor() == self::MYSQL) {
                $sql .= $this->q->dateFilter(
                        'unit', $this->model->getFilterDate(), $this->getDateRangeStartQuery(), $this->getDateRangeEndQuery(), $this->getDateRangeTypeQuery(), $this->getDateRangeExtraTypeQuery()
                );
            } else {
                if ($this->getVendor() == self::MSSQL) {
                    $sql .= $this->q->dateFilter(
        ...
        $tableArray = null;
        if ($this->getVendor() == self::MYSQL) {
            $tableArray = array('staff', 'unit', 'company', 'branch', 'department');
        } else {
            if ($this->getVendor() == self::MSSQL) {
                $tableArray = array('staff', 'unit', 'company', 'branch', 'department');
            } else {
        ...
        while (($row = $this->q->fetchAssoc()) == true) {
            ...
            if ($this->model->getUnitId(0, 'single')) {
                ...
                $row['previousRecord'] = $this->previousRecord('value', $this->model->getUnitId(0, 'single'));
                $row['nextRecord'] = $this->nextRecord('value', $this->model->getUnitId(0, 'single'));
                ...
                if ($this->model->getUnitId(0, 'single')) {
                            ...
                            array("[", "]"), "", json_encode(
                                    array(
                                        ...
                                        'previousRecord' => $this->previousRecord(
                                                'value', $this->model->getUnitId(0, 'single')
                                        ),
                                        'nextRecord' => $this->nextRecord('value', $this->model->getUnitId(0, 'single')),
                    ...
                    echo json_encode(
                            array(
                                ...
                                'nextRecord' => $this->recordSet->nextRecord('value', $this->model->getUnitId(0, 'single')),
    ...
    function update() {
        ...
        if ($this->getVendor() == self::MYSQL) {
           ...
           FROM 	`unit`
           WHERE  	`" . $this->model->getPrimaryKeyName() . "` = '" . $this->model->getUnitId(0, 'single') . "' ";
        } else {
            if ($this->getVendor() == self::MSSQL) {
                $sql = "
           SELECT	[" . $this->model->getPrimaryKeyName() . "]
           ...
           FROM 	UNIT
           WHERE  	" . strtoupper($this->model->getPrimaryKeyName()) . " = '" . $this->model->getUnitId(
                                    0, 'single'
                            ) . "' ";
                }
            }
        ...
        if ($total == 0) {
            ...
            if ($this->getVendor() == self::MYSQL) {
               ...
               UPDATE `unit` SET 
                       ...
                       `unitCode` = '" . $this->model->getUnitCode() . "',
                       `unitDescription` = '" . $this->model->getUnitDescription() . "',
               ...
               WHERE    `unitId`='" . $this->model->getUnitId('0', 'single') . "'";
            } else {
                if ($this->getVendor() == self::MSSQL) {
                    $sql = "
                UPDATE [unit] SET
  ...
  /** 
    ...
    function duplicate() {
        ...
        if ($this->getVendor() == self::MYSQL) {
           ...
           SELECT  `unitCode` 
           FROM    `unit` 
           WHERE   `unitCode` 	= 	'" . $this->model->getUnitCode() . "' 
           AND     `isActive`  =   1
    ...
    function excel() {
        ...
        $this->excel->getActiveSheet()->setCellValue('E3', $this->translate['unitCodeLabel']);
        $this->excel->getActiveSheet()->setCellValue('F3', $this->translate['unitDescriptionLabel']);
        ...
        while (($row = $this->q->fetchAssoc()) == true) {
            ...
            $this->excel->getActiveSheet()->setCellValue('E' . $loopRow, strip_tags($row ['unitCode']));
            $this->excel->getActiveSheet()->setCellValue('F' . $loopRow, strip_tags($row ['unitDescription']));
        ...
        switch ($this->getReportMode()) {
            case 'excel':
                ...
                $extension = '.xlsx';
                $folder = 'excel';
                $filename = "unit" . rand(0, 10000000) . $extension;
...
if (isset($_POST ['method'])) {
    if (isset($_POST['output'])) {
        ...
        if ($_POST['securityToken'] != $unitObject->getSecurityToken()) {
            header('Content-Type:application/json; charset=utf-8');
        ...
        if (isset($_POST ['leafId'])) {
            $unitObject->setLeafId($_POST ['leafId']);
        }
        if (isset($_POST ['offset'])) {
            $unitObject->setStart($_POST ['offset']);
        }
        if (isset($_POST ['limit'])) {
            $unitObject->setLimit($_POST ['limit']);
        }
        $unitObject->setPageOutput($_POST['output']);
        $unitObject->execute();
        /*
...
if (isset($_GET ['method'])) {
    $unitObject = new UnitClass ();
    if ($_GET['securityToken'] != $unitObject->getSecurityToken()) {
        header('Content-Type:application/json; charset=utf-8');
        echo json_encode(array("success" => false, "message" => "Something wrong with the system.Hola hackers"));
        exit();
    }
    ...
    if (isset($_GET ['leafId'])) {
        $unitObject->setLeafId($_GET ['leafId']);
    }
    /*
     *  Load the dynamic value
     */
    ...
    if ($_GET ['method'] == 'updateStatus') {
        $unitObject->updateStatus();
    }
    /*
     *  Checking Any Duplication  Key 
     */
    if ($_GET['method'] == 'duplicate') {
        $unitObject->duplicate();
    }
    if ($_GET ['method'] == 'dataNavigationRequest') {
        if ($_GET ['dataNavigation'] == 'firstRecord') {
            $unitObject->firstRecord('json');
        }
        if ($_GET ['dataNavigation'] == 'previousRecord') {
            $unitObject->previousRecord('json', 0);
        }
        if ($_GET ['dataNavigation'] == 'nextRecord') {
            $unitObject->nextRecord('json', 0);
        }
        if ($_GET ['dataNavigation'] == 'lastRecord') {
            $unitObject->lastRecord('json');
        }
    }
    /*
     * Excel Reporting  
     */
    if (isset($_GET ['mode'])) {
        $unitObject->setReportMode($_GET['mode']);
        if ($_GET ['mode'] == 'excel' || $_GET ['mode'] == 'pdf' || $_GET['mode'] == 'csv' || $_GET['mode'] == 'html' || $_GET['mode'] == 'excel5' || $_GET['mode'] == 'xml') {
            $unitObject->excel();
        }
    }
    if (isset($_GET ['filter'])) {
        $unitObject->setServiceOutput('option');
        if (($_GET['filter'] == 'branch')) {
            $unitObject->getBranch();
        }
        if (($_GET['filter'] == 'department')) {
            $unitObject->getDepartment();
        }
    }
}
?>
