...
namespace Core\System\Management\Location\Controller;

use Core\ConfigClass;
use Core\System\Management\Location\Model\LocationModel;
use Core\System\Management\Location\Service\LocationService;
...
require_once($newFakeDocumentRoot . "v3/system/management/model/locationModel.php");
require_once($newFakeDocumentRoot . "v3/system/management/service/locationService.php");

/**
 * Class Location
 * this is location controller files.
 * @name IDCMS
 * @version 2
 * @author hafizan
 * @package  Core\System\Management\Location\Controller
 * @subpackage Management
 * @link http://www.hafizan.com
 * @license http://www.gnu.org/copyleft/lesser.html LGPL
 */
class LocationClass extends ConfigClass {
    ...
    /**
     * Model
     * @var \Core\System\Management\Location\Model\LocationModel
     */
    public $model;

    /**
     * Service-Business Application Process or other ajax request
     * @var \Core\System\Management\Location\Service\LocationService
     */
    public $service;
    ...
    function __construct() {
        ...
        $this->setViewPath("./v3/system/management/view/location.php");
        $this->setControllerPath("./v3/system/management/controller/locationController.php");
        $this->setServicePath("./v3/system/management/service/locationService.php");
    }
    ...
    public function create() {
        ...
        if ($this->getVendor() == self::MYSQL) {
            ...
            (
                 ...
                 '" . $this->model->getLocationCode() . "',
                 '" . $this->model->getLocationDescription() . "',
        ...
        $locationId = $this->q->lastInsertId();
        $this->q->commit();
        $end = microtime(true);
        $time = $end - $start;
        echo json_encode(
    ...
    public function read() {
        ...
        if (isset($_SESSION['isAdmin'])) {
            if ($_SESSION['isAdmin'] == 0) {
                if ($this->getVendor() == self::MYSQL) {
                    $this->setAuditFilter(
                            " `location`.`isActive` = 1  AND `location`.`companyId`='" . $this->getCompanyId() . "' "
                    );
                } else {
                    if ($this->getVendor() == self::MSSQL) {
                        $this->setAuditFilter(
                ...
                if ($_SESSION['isAdmin'] == 1) {
                    if ($this->getVendor() == self::MYSQL) {
                        $this->setAuditFilter("   `location`.`companyId`='" . $this->getCompanyId() . "'	");
                    } else {
                        if ($this->getVendor() == self::MSSQL) {
                            $this->setAuditFilter(" [location].[companyId]='" . $this->getCompanyId() . "' ");
                        } else {
       ...
       SELECT                    `location`.`locationId`,
                    ...
                    `location`.`isPost`,
                    `location`.`executeBy`,
                    `location`.`executeTime`,
                    `staff`.`staffName`
		  FROM      `location`
		  JOIN      `staff`
		  ON        `location`.`executeBy` = `staff`.`staffId`
	JOIN	`company`
	ON		`company`.`companyId` = `location`.`companyId`
	JOIN	`branch`
	ON		`branch`.`branchId` = `location`.`branchId`
	JOIN	`department`
	ON		`department`.`departmentId` = `location`.`departmentId`
		  WHERE     " . $this->getAuditFilter();
            if ($this->model->getLocationId(0, 'single')) {
                $sql .= " AND `location`.`" . $this->model->getPrimaryKeyName() . "`='" . $this->model->getLocationId(
                                0, 'single'
                        ) . "'";
            }
            if ($this->model->getBranchId()) {
                $sql .= " AND `location`.`branchId`='" . $this->model->getBranchId() . "'";
            }
            if ($this->model->getDepartmentId()) {
                $sql .= " AND `location`.`departmentId`='" . $this->model->getDepartmentId() . "'";
            }
        } else {
            if ($this->getVendor() == self::MSSQL) {
	...
	ON		[department].[departmentId] = [location].[departmentId]
		  WHERE     " . $this->getAuditFilter();
                if ($this->model->getLocationId(0, 'single')) {
                    $sql .= " AND [location].[" . $this->model->getPrimaryKeyName(
                            ) . "]		=	'" . $this->model->getLocationId(0, 'single') . "'";
                }
                if ($this->model->getBranchId()) {
                    $sql .= " AND [location].[branchId]='" . $this->model->getBranchId() . "'";
                }
		  ...
		  SELECT                    LOCATION.LOCATIONID AS \"locationId\",
                    ...
                    LOCATION.BRANCHID AS \"branchId\",
                    DEPARTMENT.DEPARTMENTDESCRIPTION AS  \"departmentDescription\",
                    LOCATION.DEPARTMENTID AS \"departmentId\",
                    LOCATION.LOCATIONCODE AS \"locationCode\",
                    LOCATION.LOCATIONDESCRIPTION AS \"locationDescription\",
                    LOCATION.ISDEFAULT AS \"isDefault\",
                    LOCATION.ISNEW AS \"isNew\",
                    LOCATION.ISDRAFT AS \"isDraft\",
                    LOCATION.ISUPDATE AS \"isUpdate\",
                    LOCATION.ISDELETE AS \"isDelete\",
                    LOCATION.ISACTIVE AS \"isActive\",
                    LOCATION.ISAPPROVED AS \"isApproved\",
                    LOCATION.ISREVIEW AS \"isReview\",
                    LOCATION.ISPOST AS \"isPost\",
                    LOCATION.EXECUTEBY AS \"executeBy\",
                    LOCATION.EXECUTETIME AS \"executeTime\",
	...
	ON		DEPARTMENT.DEPARTMENTID = LOCATION.DEPARTMENTID
        ...
        if ($this->getCharacterQuery()) {
            if ($this->getVendor() == self::MYSQL) {
                $sql .= " AND `location`.`" . $this->model->getFilterCharacter(
                ...
                if ($this->getVendor() == self::MSSQL) {
                    ...
                    if ($this->getVendor() == self::ORACLE) {
                        $sql .= " AND Initcap(LOCATION." . strtoupper(
        ...
        if ($this->getDateRangeStartQuery()) {
            if ($this->getVendor() == self::MYSQL) {
                $sql .= $this->q->dateFilter(
                        'location', $this->model->getFilterDate(), $this->getDateRangeStartQuery(), $this->getDateRangeEndQuery(), $this->getDateRangeTypeQuery(), $this->getDateRangeExtraTypeQuery()
                );
            } else {
                if ($this->getVendor() == self::MSSQL) {
                    $sql .= $this->q->dateFilter(
        ...
        $tableArray = null;
        if ($this->getVendor() == self::MYSQL) {
            $tableArray = array('staff', 'location', 'company', 'branch', 'department');
        } else {
            if ($this->getVendor() == self::MSSQL) {
                $tableArray = array('staff', 'location', 'company', 'branch', 'department');
            } else {
        ...
        while (($row = $this->q->fetchAssoc()) == true) {
            ...
            if ($this->model->getLocationId(0, 'single')) {
                ...
                $row['previousRecord'] = $this->previousRecord('value', $this->model->getLocationId(0, 'single'));
                $row['nextRecord'] = $this->nextRecord('value', $this->model->getLocationId(0, 'single'));
                            ...
                            array("[", "]"), "", json_encode(
                                    array(
                                        ...
                                        'previousRecord' => $this->previousRecord(
                                                'value', $this->model->getLocationId(0, 'single')
                                        ),
                                        'nextRecord' => $this->nextRecord('value', $this->model->getLocationId(0, 'single')),
    ...
    function update() {
        ...
        if ($this->getVendor() == self::MYSQL) {
           ...
           FROM 	`location`
           WHERE  	`" . $this->model->getPrimaryKeyName() . "` = '" . $this->model->getLocationId(
                            0, 'single'
                    ) . "' ";
        } else {
            if ($this->getVendor() == self::MSSQL) {
           ...
           FROM 	LOCATION
           WHERE  	" . strtoupper($this->model->getPrimaryKeyName()) . " = '" . $this->model->getLocationId(
                                    0, 'single'
                            ) . "' ";
                }
            }
        ...
        if ($total == 0) {
            ...
            if ($this->getVendor() == self::MYSQL) {
               ...
               UPDATE `location` SET 
                       ...
                       `locationCode` = '" . $this->model->getLocationCode() . "',
                       `locationDescription` = '" . $this->model->getLocationDescription() . "',
               ...
               WHERE    `locationId`='" . $this->model->getLocationId('0', 'single') . "'";
            } else {
                if ($this->getVendor() == self::MSSQL) {
                    $sql = "
                UPDATE [location] SET
   ...
   /** 
    ...
    function duplicate() {
        ...
        if ($this->getVendor() == self::MYSQL) {
           ...
           SELECT  `locationCode` 
           FROM    `location` 
           WHERE   `locationCode` 	= 	'" . $this->model->getLocationCode() . "' 
           AND     `isActive`  =   1
    ...
    public function getDepartment() {
        $this->service->setServiceOutput($this->getServiceOutput());
        if ($this->model->getBranchId()) {
            return $this->service->getDepartment($this->model->getBranchId());
        } else {
    ...
    function excel() {
        ...
        $this->excel->getActiveSheet()->setCellValue('E3', $this->translate['locationCodeLabel']);
        $this->excel->getActiveSheet()->setCellValue('F3', $this->translate['locationDescriptionLabel']);
        ...
        while (($row = $this->q->fetchAssoc()) == true) {
            ...
            $this->excel->getActiveSheet()->setCellValue('E' . $loopRow, strip_tags($row ['locationCode']));
        ...
        switch ($this->getReportMode()) {
            case 'excel':
                ...
                $extension = '.xlsx';
                $folder = 'excel';
                $filename = "location" . rand(0, 10000000) . $extension;
...
if (isset($_POST ['method'])) {
    if (isset($_POST['output'])) {
        ...
        if ($_POST['securityToken'] != $locationObject->getSecurityToken()) {
            header('Content-Type:application/json; charset=utf-8');
        ...
        if (isset($_POST ['leafId'])) {
            $locationObject->setLeafId($_POST ['leafId']);
        }
        if (isset($_POST ['offset'])) {
            $locationObject->setStart($_POST ['offset']);
        }
        if (isset($_POST ['limit'])) {
            $locationObject->setLimit($_POST ['limit']);
        }
        $locationObject->setPageOutput($_POST['output']);
        $locationObject->execute();
        /*
...
if (isset($_GET ['method'])) {
    $locationObject = new LocationClass ();
    if ($_GET['securityToken'] != $locationObject->getSecurityToken()) {
        header('Content-Type:application/json; charset=utf-8');
        echo json_encode(array("success" => false, "message" => "Something wrong with the system.Hola hackers"));
        exit();
    }
    ...
    if (isset($_GET ['leafId'])) {
        $locationObject->setLeafId($_GET ['leafId']);
    }
    /*
     *  Load the dynamic value
     */
    ...
    if ($_GET ['method'] == 'updateStatus') {
        $locationObject->updateStatus();
    }
    /*
     *  Checking Any Duplication  Key 
     */
    if ($_GET['method'] == 'duplicate') {
        $locationObject->duplicate();
    }
    if ($_GET ['method'] == 'dataNavigationRequest') {
        if ($_GET ['dataNavigation'] == 'firstRecord') {
            $locationObject->firstRecord('json');
        }
        if ($_GET ['dataNavigation'] == 'previousRecord') {
            $locationObject->previousRecord('json', 0);
        }
        if ($_GET ['dataNavigation'] == 'nextRecord') {
            $locationObject->nextRecord('json', 0);
        }
        if ($_GET ['dataNavigation'] == 'lastRecord') {
            $locationObject->lastRecord('json');
        }
    }
    /*
     * Excel Reporting  
     */
    if (isset($_GET ['mode'])) {
        $locationObject->setReportMode($_GET['mode']);
        if ($_GET ['mode'] == 'excel' || $_GET ['mode'] == 'pdf' || $_GET['mode'] == 'csv' || $_GET['mode'] == 'html' || $_GET['mode'] == 'excel5' || $_GET['mode'] == 'xml') {
            $locationObject->excel();
        }
    }
    if (isset($_GET ['filter'])) {
        $locationObject->setServiceOutput('option');
        if (($_GET['filter'] == 'branch')) {
            $locationObject->getBranch();
        }
        if (($_GET['filter'] == 'department')) {
            $locationObject->getDepartment();
        }
    }
}
?>
