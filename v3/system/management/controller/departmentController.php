...
use Core\System\Management\Department\Model\DepartmentModel;
use Core\System\Management\Department\Service\DepartmentService;
...
require_once($newFakeDocumentRoot . "v3/system/management/model/departmentModel.php");
require_once($newFakeDocumentRoot . "v3/system/management/service/departmentService.php");

/**
 * Class Department
 * this is department controller files.
 * @name IDCMS
 * @version 2
 * @author hafizan
 * @package  Core\System\Management\Department\Controller
...
class DepartmentClass extends ConfigClass {
    ...
    /**
     * Model
     * @var \Core\System\Management\Department\Model\DepartmentModel
     */
    public $model;

    /**
     * Service-Business Application Process or other ajax request
     * @var \Core\System\Management\Department\Service\DepartmentService
     */
    public $service;
    ...
    function __construct() {
        ...
        $this->setViewPath("./v3/system/management/view/department.php");
        $this->setControllerPath("./v3/system/management/controller/departmentController.php");
        $this->setServicePath("./v3/system/management/service/departmentService.php");
    }
    ...
    public function create() {
        ...
        if ($this->getVendor() == self::MYSQL) {
            ...
            (
                 ...
                 '" . $this->model->getDepartmentDescription() . "',
        ...
        $departmentId = $this->q->lastInsertId();
        $this->q->commit();
        $end = microtime(true);
        $time = $end - $start;
        echo json_encode(
            ...
            if ($_SESSION['isAdmin'] == 0) {
                ...
                if ($this->getVendor() == self::MYSQL) {
                    if ($_SESSION['isBranch'] == 0) {
                        $this->setAuditFilter(" `department`.`isActive` = 1  AND `department`.`companyId`='" . $this->getCompanyId() . "' ");
                    } else if ($_SESSION['isBranch'] == 1) {
                        $this->setAuditFilter(" `department`.`isActive` = 1  AND `department`.`companyId`='" . $this->getCompanyId() . "' AND `branchId`	=	'" . $this->getBranchId() . "' ");
                    }
                } else if ($this->getVendor() == self::MSSQL) {
                    if ($_SESSION['isBranch'] == 0) {
                        $this->setAuditFilter(" [department].[isActive] = 1 AND [department].[companyId]='" . $this->getCompanyId() . "' ");
                ...
                if ($this->getVendor() == self::MYSQL) {
                    $this->setAuditFilter("   `department`.`companyId`='" . $this->getCompanyId() . "'	");
                } else if ($this->getVendor() == self::MSSQL) {
                    $this->setAuditFilter(" [department].[companyId]='" . $this->getCompanyId() . "' ");
                } else if ($this->getVendor() == self::ORACLE) {
                    $this->setAuditFilter(" DEPARTMENT.COMPANYID='" . $this->getCompanyId() . "' ");
	...
	JOIN	`company`
	ON		`company`.`companyId` = `department`.`companyId`
	JOIN	`branch`
	ON		`branch`.`branchId` = `department`.`branchId`
		  WHERE     " . $this->getAuditFilter();
            if ($this->model->getDepartmentId(0, 'single')) {
                $sql .= " AND `department`.`" . $this->model->getPrimaryKeyName(
                        ) . "`='" . $this->model->getDepartmentId(0, 'single') . "'";
            }
            if ($this->model->getBranchId()) {
                $sql .= " AND `department`.`branchId`='" . $this->model->getBranchId() . "'";
            }
        } else {
            if ($this->getVendor() == self::MSSQL) {
	...
	ON		[branch].[branchId] = [department].[branchId]
		  ...
		  SELECT                    DEPARTMENT.DEPARTMENTID AS \"departmentId\",
                    ...
                    DEPARTMENT.BRANCHID AS \"branchId\",
	...
	ON		BRANCH.BRANCHID = DEPARTMENT.BRANCHID
        ...
        if ($this->getCharacterQuery()) {
            if ($this->getVendor() == self::MYSQL) {
                $sql .= " AND `department`.`" . $this->model->getFilterCharacter(
                ...
                if ($this->getVendor() == self::MSSQL) {
                    ...
                    if ($this->getVendor() == self::ORACLE) {
                        $sql .= " AND Initcap(DEPARTMENT." . strtoupper(
        ...
        if ($this->getDateRangeStartQuery()) {
            if ($this->getVendor() == self::MYSQL) {
                $sql .= $this->q->dateFilter(
                        'department', $this->model->getFilterDate(), $this->getDateRangeStartQuery(), $this->getDateRangeEndQuery(), $this->getDateRangeTypeQuery(), $this->getDateRangeExtraTypeQuery()
                );
            } else {
                if ($this->getVendor() == self::MSSQL) {
                    $sql .= $this->q->dateFilter(
        ...
        $tableArray = null;
        if ($this->getVendor() == self::MYSQL) {
            $tableArray = array('staff', 'department', 'branch');
        } else {
            if ($this->getVendor() == self::MSSQL) {
                $tableArray = array('staff', 'department', 'branch');
            } else {
        ...
        while (($row = $this->q->fetchAssoc()) == true) {
            ...
            if ($this->model->getDepartmentId(0, 'single')) {
                ...
                $row['previousRecord'] = $this->previousRecord('value', $this->model->getDepartmentId(0, 'single'));
                $row['nextRecord'] = $this->nextRecord('value', $this->model->getDepartmentId(0, 'single'));
                            ...
                            array("[", "]"), "", json_encode(
                                    array(
                                        ...
                                        'previousRecord' => $this->previousRecord(
                                                'value', $this->model->getDepartmentId(0, 'single')
                                        ),
                                        'nextRecord' => $this->nextRecord('value', $this->model->getDepartmentId(0, 'single')),
    ...
    function update() {
        ...
        if ($this->getVendor() == self::MYSQL) {
           ...
           FROM 	`department`
           WHERE  	`" . $this->model->getPrimaryKeyName() . "` = '" . $this->model->getDepartmentId(
                            0, 'single'
                    ) . "' ";
        } else {
            if ($this->getVendor() == self::MSSQL) {
           ...
           FROM 	DEPARTMENT
           WHERE  	" . strtoupper($this->model->getPrimaryKeyName()) . " = '" . $this->model->getDepartmentId(
                                    0, 'single'
                            ) . "' ";
                }
            }
        ...
        if ($total == 0) {
            ...
            if ($this->getVendor() == self::MYSQL) {
               ...
               UPDATE `department` SET 
                       ...
                       `departmentDescription` = '" . $this->model->getDepartmentDescription() . "',
 ...
 /** 
    ...
    function duplicate() {
        ...
        if ($this->getVendor() == self::MYSQL) {
           ...
           AND     [companyId] =	'" . $this->getCompanyId() . "'";
                ...
                if ($this->getVendor() == self::ORACLE) {
                    $sql = "
               SELECT  DEPARTMENTCODE as \"departmentCode\"
               FROM    DEPARTMENT
               WHERE   DEPARTMENTCODE	= 	'" . $this->model->getDepartmentCode() . "'
               AND     ISACTIVE    =   1
               AND     COMPANYID   =   '" . $this->getCompanyId() . "'";
    ...
    function excel() {
        ...
        $this->excel->getActiveSheet()->setCellValue('D3', $this->translate['departmentCodeLabel']);
        $this->excel->getActiveSheet()->setCellValue('E3', $this->translate['departmentDescriptionLabel']);
        ...
        while (($row = $this->q->fetchAssoc()) == true) {
            ...
            $this->excel->getActiveSheet()->setCellValue('D' . $loopRow, strip_tags($row ['departmentCode']));
            $this->excel->getActiveSheet()->setCellValue('E' . $loopRow, strip_tags($row ['departmentDescription']));
        ...
        switch ($this->getReportMode()) {
            case 'excel':
                ...
                $extension = '.xlsx';
                $folder = 'excel';
                $filename = "department" . rand(0, 10000000) . $extension;
...
if (isset($_POST ['method'])) {
    if (isset($_POST['output'])) {
        ...
        if ($_POST['securityToken'] != $departmentObject->getSecurityToken()) {
            header('Content-Type:application/json; charset=utf-8');
        ...
        if (isset($_POST ['offset'])) {
            $departmentObject->setStart($_POST ['offset']);
        }
        if (isset($_POST ['limit'])) {
            $departmentObject->setLimit($_POST ['limit']);
        }
        $departmentObject->setPageOutput($_POST['output']);
...
if (isset($_GET ['method'])) {
    $departmentObject = new DepartmentClass ();
    if ($_GET['securityToken'] != $departmentObject->getSecurityToken()) {
        header('Content-Type:application/json; charset=utf-8');
        echo json_encode(array("success" => false, "message" => "Something wrong with the system.Hola hackers"));
        exit();
    }
     ...
     */
    if (isset($_GET ['mode'])) {
        $departmentObject->setReportMode($_GET['mode']);
        if ($_GET ['mode'] == 'excel' || $_GET ['mode'] == 'pdf' || $_GET['mode'] == 'csv' || $_GET['mode'] == 'html' || $_GET['mode'] == 'excel5' || $_GET['mode'] == 'xml') {
            $departmentObject->excel();
        }
    }
    if (isset($_GET ['filter'])) {
        $departmentObject->setServiceOutput('option');
        if (($_GET['filter'] == 'branch')) {
            $departmentObject->getBranch();
        }
    }
}
?>
