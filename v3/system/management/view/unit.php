...
require_once($newFakeDocumentRoot . "v3/system/management/controller/unitController.php");
...
$translator->setCurrentTable('unit');
...
if (isset($_POST)) {
    if (isset($_POST['method'])) {
        $unit = new \Core\System\Management\Unit\Controller\UnitClass();
        define('LIMIT', 10);
        ...
        if (isset($_POST ['query'])) {
            $unit->setFieldQuery($_POST ['query']);
        }
        if (isset($_POST ['filter'])) {
            $unit->setGridQuery($_POST ['filter']);
        }
        if (isset($_POST ['character'])) {
            $unit->setCharacterQuery($_POST['character']);
        }
        if (isset($_POST ['dateRangeStart'])) {
            $unit->setDateRangeStartQuery($_POST['dateRangeStart']);
            //explode the data to get day,month,year
            $start = explode('-', $_POST ['dateRangeStart']);
            $unit->setStartDay($start[2]);
            $unit->setStartMonth($start[1]);
            $unit->setStartYear($start[0]);
        }
        if (isset($_POST ['dateRangeEnd']) && (strlen($_POST['dateRangeEnd']) > 0)) {
            $unit->setDateRangeEndQuery($_POST['dateRangeEnd']);
            //explode the data to get day,month,year
            $start = explode('-', $_POST ['dateRangeEnd']);
            $unit->setEndDay($start[2]);
            $unit->setEndMonth($start[1]);
            $unit->setEndYear($start[0]);
        }
        if (isset($_POST ['dateRangeType'])) {
            $unit->setDateRangeTypeQuery($_POST['dateRangeType']);
        }
        if (isset($_POST ['dateRangeExtraType'])) {
            $unit->setDateRangeExtraTypeQuery($_POST['dateRangeExtraType']);
        }
        $unit->setServiceOutput('html');
        $unit->setLeafId($leafId);
        $unit->execute();
        $branchArray = $unit->getBranch();
        $departmentArray = $unit->getDepartment();
        if ($_POST['method'] == 'read') {
            $unit->setStart($offset);
            $unit->setLimit($limit); // normal system don't like paging..
            $unit->setPageOutput('html');
            $unitArray = $unit->read();
            if (isset($unitArray [0]['firstRecord'])) {
                $firstRecord = $unitArray [0]['firstRecord'];
            }
            if (isset($unitArray [0]['nextRecord'])) {
                $nextRecord = $unitArray [0]['nextRecord'];
            }
            if (isset($unitArray [0]['previousRecord'])) {
                $previousRecord = $unitArray [0]['previousRecord'];
            }
            if (isset($unitArray [0]['lastRecord'])) {
                $lastRecord = $unitArray [0]['lastRecord'];
                $endRecord = $unitArray [0]['lastRecord'];
            }
            $navigation = new \Core\Paging\HtmlPaging();
            $navigation->setLeafId($leafId);
            $navigation->setViewPath($unit->getViewPath());
            ...
            if (isset($unitArray [0]['total'])) {
                $total = $unitArray [0]['total'];
            } else {
                $total = 0;
            }
                ...
                <div class="control-label col-md-1">
                    <div class="pull-right">
                        <button class="delete btn btn-warning" type="button" 
                                ...
                                echo $unit->getControllerPath();
                                ?>', '<?php echo $unit->getViewPath(); ?>', '<?php echo $securityToken; ?>');">
                        ...
                        <div class="modal-dialog">
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <input type="hidden" name="unitIdPreview" id="unitIdPreview">
                                    ...
                                    <div class="form-group" id="unitCodeDiv">
                                        <label for="unitCodePreview"
                                               class="control-label col-xs-4 col-sm-4 col-md-4"><?php echo $leafTranslation['unitCodeLabel']; ?></label>

                                        <div class="col-xs-8 col-sm-8 col-md-8">
                                            <input class="form-control" type="text" name="unitCodePreview"
                                                   id="unitCodePreview">
                                        </div>
                                    </div>
                                    <div class="form-group" id="unitDescriptionDiv">
                                        <label for="unitDescriptionPreview"
                                               class="control-label col-xs-4 col-sm-4 col-md-4"><?php echo $leafTranslation['unitDescriptionLabel']; ?></label>

                                        <div class="col-xs-8 col-sm-8 col-md-8">
                                            <input class="form-control" type="text" name="unitDescriptionPreview"
                                                   id="unitDescriptionPreview">
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div class="col-xs-12 col-sm-12 col-md-12">
                            <div class="panel panel-default">
                                <table class="table table table-bordered table-striped table-condensed table-hover smart-form has-tickbox" id="tableData">
                                    ...
                                    <th><?php echo ucwords($leafTranslation['unitDescriptionLabel']); ?></th>
                                    <th width="100px">
                                    ...
                                    <tbody id="tableBody">
                                        ...
                                        if ($_POST['method'] == 'read' && $_POST['type'] == 'list' && $_POST['detail'] == 'body') {
                                            if (is_array($unitArray)) {
                                                $totalRecord = intval(count($unitArray));
                                                if ($totalRecord > 0) {
                                                    $counter = 0;
                                                    for ($i = 0; $i < $totalRecord; $i++) {
                                                        ...
                                                        ?>
                                                        <tr <?php
                                                        if ($unitArray[$i]['isDelete'] == 1) {
                                                            echo "class=\"danger\"";
                                                        } else {
                                                            if ($unitArray[$i]['isDraft'] == 1) {
                                                                echo "class=\"warning\"";
                                                            }
                                                        }
                                                        ?>>
                                                            ...
                                                            <td align="center">
                                                                <div class="btn-group" align="center">
                                                                    ...
                                                                    ?>', '<?php
                                                                    echo rawurlencode(
                                                                            $unitArray [$i]['branchName']
                                                                    );
                                                                    ?>', '<?php
                                                                    echo rawurlencode(
                                                                            $unitArray [$i]['departmentDescription']
                                                                    );
                                                                    ?>', '<?php
                                                                    echo rawurlencode(
                                                                            $unitArray [$i]['unitCode']
                                                                    );
                                                                    ?>', '<?php
                                                                    echo rawurlencode(
                                                                            $unitArray [$i]['unitDescription']
                                                                    );
                                                                    ?>', '<?php
                                                                    echo rawurlencode(
                                                                            $unitArray [$i]['isDefault']
                                                                    );
                                                                    ?>', '<?php
                                                                    echo rawurlencode(
                                                                            $unitArray [$i]['isNew']
                                                                    ...
                                                                    ?>', '<?php
                                                                    echo rawurlencode(
                                                                            $unitArray [$i]['isUpdate']
                                                                    );
                                                                    ?>', '<?php
                                                                    echo rawurlencode(
                                                                            $unitArray [$i]['isDelete']
                                                                    );
                                                                    ?>', '<?php
                                                                    echo rawurlencode(
                                                                            $unitArray [$i]['isActive']
                                                                    );
                                                                    ?>', '<?php
                                                                    echo rawurlencode(
                                                                            $unitArray [$i]['isApproved']
                                                                    );
                                                                    ?>', '<?php
                                                                    echo rawurlencode(
                                                                            $unitArray [$i]['isReview']
                                                                    );
                                                                    ?>', '<?php
                                                                    echo rawurlencode(
                                                                            $unitArray [$i]['isPost']
                                                                    ...
                                                                    ?>', '<?php
                                                                    echo rawurlencode(
                                                                            $unitArray [$i]['executeBy']
                                                                    );
                                                                    ?>', '<?php
                                                                    echo rawurlencode(
                                                                            $unitArray [$i]['executeTime']
                                                                    );
                                                                    ?>');">
                                                                        <i class="glyphicon glyphicon-trash glyphicon-white"></i></button>
                                                                </div>
                                                            </td>
                        <?php if (isset($unitArray[$i]['branchName'])) { ?>
                                                                <td>
                                                                    <div class="pull-left">
                                                                        <?php
                                                                        if (strpos(
                                                                                        $unitArray[$i]['branchName'], $_POST['query']
                                                                                ) !== false
                                                                        ) {
                                                                            echo str_replace(
                                                                                    $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $unitArray[$i]['branchName']
                                                                            );
                                                                        } else {
                                                                            echo $unitArray[$i]['branchName'];
                                                                        }
                                                                        ?>
                                                                    </div>
                                                                </td>
                                                            <?php } else { ?>
                                                                <td>&nbsp;</td>
                                                            <?php } ?>
                        <?php if (isset($unitArray[$i]['departmentDescription'])) { ?>
                                                                <td>
                                                                    <div class="pull-left">
                                                                        <?php
                                                                        if (strpos(
                                                                                        $unitArray[$i]['departmentDescription'], $_POST['query']
                                                                                ) !== false
                                                                        ) {
                                                                            echo str_replace(
                                                                                    $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $unitArray[$i]['departmentDescription']
                                                                            );
                                                                        } else {
                                                                            echo $unitArray[$i]['departmentDescription'];
                                                                        }
                                                                        ?>
                                                                    </div>
                                                                </td>
                                                            <?php } else { ?>
                                                                <td>&nbsp;</td>
                                                            <?php } ?>
                        <?php if (isset($unitArray[$i]['unitDescription'])) { ?>
                                                                <td>
                                                                    <div class="pull-left">
                                                                        <?php
                                                                        if (strpos(
                                                                                        $unitArray[$i]['unitDescription'], $_POST['query']
                                                                                ) !== false
                                                                        ) {
                                                                            echo str_replace(
                                                                                    $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $unitArray[$i]['unitDescription']
                                                                            );
                                                                        } else {
                                                                            echo $unitArray[$i]['unitDescription'];
                                                                        }
                                                                        ?>
                                                                    </div>
                                                                </td>                             <?php } else { ?>
                                                                <td>&nbsp;</td>
                                                            <?php } ?>
                        <?php if (isset($unitArray[$i]['executeBy'])) { ?>
                                                                <td>
                                                                    <div align="center">
                                                                        <?php
                                                                        if (strpos(
                                                                                        $unitArray[$i]['staffName'], $_POST['query']
                                                                                ) !== false
                                                                        ) {
                                                                            echo str_replace(
                                                                                    $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $unitArray[$i]['staffName']
                                                                            );
                                                                        } else {
                                                                            echo $unitArray[$i]['staffName'];
                                                                        }
                                                                        ?>
                                                                    </div>
                                                                </td>                             <?php } else { ?>
                                                            ...
                                                            <?php } ?>
                                                            <?php
                                                            if (isset($unitArray[$i]['executeTime'])) {
                                                                $valueArray = $unitArray[$i]['executeTime'];
                                                            ...
                                                            ?>
                                                            <td>
                                                                <input style="display:none;" type="checkbox" name="unitId[]"
                                                                       value="<?php echo $unitArray[$i]['unitId']; ?>">
                                                                <input <?php echo $checked; ?> type="checkbox" name="isDelete[]"
                                                                                               value="<?php echo $unitArray[$i]['isDelete']; ?>">
                                                            </td>
                                                        </tr>
                                                        <?php
                                                    }
                                                    ...
                                                    ?>
                                                    <tr>
                                                        <td colspan="6"><?php $unit->exceptionMessage($t['recordNotFoundLabel']); ?></td>
                                                    </tr>
                                                    <?php
                                                }
                                            } else {
                                            ...
                                            ?>
                                            <tr>
                                                <td colspan="6"><?php $unit->exceptionMessage($t['loadFailureLabel']); ?></td>
                                            </tr>
                                            <?php
                                        }
                                        ?>
...
if ((isset($_POST['method']) == 'new' || isset($_POST['method']) == 'read') && $_POST['type'] == 'form') {
        ...
        <div class="row">
            <div class="col-xs-12 col-sm-12 col-md-12">
                <div class="panel panel-info">
                    ...
                    <div class="panel-body">
                        <input type="hidden" name="unitId" id="unitId" value="<?php
                        ...
                        <div class="row">
                            <div class="col-xs-12 col-sm-12 col-md-12">
                                <div class="control-label col-xs-2 col-sm-2 col-md-2">
                                    <label for="branchId"><?php echo $leafTranslation['branchIdLabel']; ?></label></div>
                                <div class="col-md-4 form-group" id="branchIdForm">
                                    <select name="branchId" id="branchId" class="chzn-select form-control"
                                        ...
                                        if (is_array($branchArray)) {
                                            ...
                                            if ($totalRecord > 0) {
                                                for ($i = 0; $i < $totalRecord; $i++) {
                                                    if ($unitArray[0]['branchId'] == $branchArray[$i]['branchId']) {
                                                        $selected = "selected";
                                                    } else {
                                                        $selected = null;
                                                    }
    ...
    <?php } ?>
                                ...
                                <div class="control-label col-xs-2 col-sm-2 col-md-2">
                                    <label for="departmentId"><?php echo $leafTranslation['departmentIdLabel']; ?></label></div>
                                <div class="col-md-4 form-group" id="departmentIdForm">
                                    <select name="departmentId" id="departmentId" class="chzn-select form-control"
                                            onChange="removeMeError('departmentId');">
                                        <option value=""></option>
                                        <?php
                                        if (is_array($departmentArray)) {
                                            ...
                                            if ($totalRecord > 0) {
                                                for ($i = 0; $i < $totalRecord; $i++) {
                                                    if ($unitArray[0]['departmentId'] == $departmentArray[$i]['departmentId']) {
                                                        $selected = "selected";
                                                    } else {
                                                        $selected = null;
                                                    }
    ...
    <?php } ?>
                        ...
                        <div class="row">
                            <div class="col-xs-12 col-sm-12 col-md-12">
                                <label for="unit_code"
                                       class="control-label col-xs-2 col-sm-2 col-md-2"><?php echo $leafTranslation['unitCodeLabel']; ?></label>
                            </div>
                            <div class="col-md-4 form-group" id="unitCodeForm">
                                <input type="text" class="form-control" name="unitCode" id="unitCode"
                                       onKeyUp="removeMeError('unitCode');" value="<?php
                                       if (isset($unitArray) && is_array(
                                                       $unitArray
                                               )
                                       ) {
                                           echo $unitArray[0]['unitCode'];
                                       }
                                       ?>" maxlength="16">
                            </div>
                            <div class="control-label col-xs-2 col-sm-2 col-md-2"></div>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-xs-12 col-sm-12 col-md-12">
                            <div class="form-group" id="unitDescriptionForm">
                                <textarea class="form-control" name="unitDescription" id="unitDescription"><?php
                                    if (isset($unitArray) && is_array($unitArray)) {
                                        echo $unitArray[0]['unitDescription'];
                                    }
                                    ?></textarea>
                            </div>
                        </div>
    ...
    <?php if ($leafAccess['leafAccessCreateValue'] == 1) { ?>
                ...
                if (e.which === 78 && e.which === 18  && e.shiftKey) {
                    ...
                    newRecord(<?php echo $leafId; ?>, '<?php echo $unit->getControllerPath(); ?>', '<?php echo $unit->getViewPath(); ?>', '<?php echo $securityToken; ?>', 1);
    ...
    <?php if ($leafAccess['leafAccessUpdateValue'] == 1) { ?>
                ...
                if (e.which === 83 && e.which === 18  && e.shiftKey) {
                    ...
                    updateRecord(<?php echo $leafId; ?>, '<?php echo $unit->getControllerPath(); ?>', '<?php echo $unit->getViewPath(); ?>', '<?php echo $securityToken; ?>', 1, <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
    ...
    <?php if ($leafAccess['leafAccessDeleteValue'] == 1) { ?>
                ...
                if (e.which === 88 && e.which === 18 && e.shiftKey) {
                    ...
                    deleteRecord(<?php echo $leafId; ?>, '<?php echo $unit->getControllerPath(); ?>', '<?php echo $unit->getViewPath(); ?>', '<?php echo $securityToken; ?>', <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
    ...
    <?php } ?>
            switch (e.keyCode) {
                case 37:
                    previousRecord(<?php echo $leafId; ?>, '<?php echo $unit->getControllerPath(); ?>', '<?php echo $unit->getViewPath(); ?>', '<?php echo $securityToken; ?>', <?php echo $leafAccess['leafAccessUpdateValue']; ?>, <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
                    
                    return false;
                    break;
                case 39:
                    nextRecord(<?php echo $leafId; ?>, '<?php echo $unit->getControllerPath(); ?>', '<?php echo $unit->getViewPath(); ?>', '<?php echo $securityToken; ?>', <?php echo $leafAccess['leafAccessUpdateValue']; ?>, <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
    ...
    <?php if ($_POST['method'] == "new") { ?>
        ...
        <?php if ($leafAccess['leafAccessCreateValue'] == 1) { ?>
                    $('#newRecordButton1').removeClass().addClass('btn btn-success').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $unit->getControllerPath(); ?>','<?php echo $unit->getViewPath(); ?>','<?php echo $securityToken; ?>',1)");
                    $('#newRecordButton2').removeClass().addClass('btn dropdown-toggle btn-success');
                    $('#newRecordButton3').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $unit->getControllerPath(); ?>','<?php echo $unit->getViewPath(); ?>','<?php echo $securityToken; ?>',1)");
                    $('#newRecordButton4').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $unit->getControllerPath(); ?>','<?php echo $unit->getViewPath(); ?>','<?php echo $securityToken; ?>',2)");
                    $('#newRecordButton5').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $unit->getControllerPath(); ?>','<?php echo $unit->getViewPath(); ?>','<?php echo $securityToken; ?>',3)");
                    $('#newRecordButton6').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $unit->getControllerPath(); ?>','<?php echo $unit->getViewPath(); ?>','<?php echo $securityToken; ?>',4)");
                    $('#newRecordButton7').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $unit->getControllerPath(); ?>','<?php echo $unit->getViewPath(); ?>','<?php echo $securityToken; ?>',5)");
        <?php } else { ?>
        ...
        if ($_POST['unitId']) {
            ...
            <?php if ($leafAccess['leafAccessUpdateValue'] == 1) { ?>
                        $('#updateRecordButton1').removeClass().addClass('btn btn-info').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $unit->getControllerPath(); ?>','<?php echo $unit->getViewPath(); ?>','<?php echo $securityToken; ?>',1,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                        $('#updateRecordButton2').removeClass().addClass('btn dropdown-toggle btn-info');
                        $('#updateRecordButton3').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $unit->getControllerPath(); ?>','<?php echo $unit->getViewPath(); ?>','<?php echo $securityToken; ?>',1,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                        $('#updateRecordButton4').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $unit->getControllerPath(); ?>','<?php echo $unit->getViewPath(); ?>','<?php echo $securityToken; ?>',2,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                        $('#updateRecordButton5').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $unit->getControllerPath(); ?>','<?php echo $unit->getViewPath(); ?>','<?php echo $securityToken; ?>',3,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
            <?php } else { ?>
                        $('#updateRecordButton1').removeClass().addClass(' btn btn-info disabled').attr('onClick', '');
                        $('#updateRecordButton2').removeClass().addClass('btn dropdown-toggle btn-info disabled');
                        $('#updateRecordButton3').attr('onClick', '');
            ...
            <?php } ?>
            <?php if ($leafAccess['leafAccessDeleteValue'] == 1) { ?>
                        $('#deleteRecordButton').removeClass().addClass('btn btn-danger').attr('onClick', "deleteRecord(<?php echo $leafId; ?>,'<?php echo $unit->getControllerPath(); ?>','<?php echo $unit->getViewPath(); ?>','<?php echo $securityToken; ?>',<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
            <?php } else { ?>
                        $('#deleteRecordButton').removeClass().addClass('btn btn-danger disabled').attr('onClick', '');
            <?php } ?>
            <?php
...
<?php } ?>
<script type="text/javascript" src="./v3/system/management/javascript/unit.js"></script>
<hr>
