function filterGrid(leafId, url, securityToken, offset, limit) {
    $.ajax({type: 'POST', url: url, data: {offset: offset, limit: limit, method: 'read', output: 'table', securityToken: securityToken, leafId: leafId, defaultLabelId: $("#defaultLabelId").val(), languageId: $("#languageId").val()}, beforeSend: function() {
...
function updateRecordInline(leafId, url, securityToken, id) {
    $.ajax({type: 'POST', url: url, data: {method: 'save', securityToken: securityToken, leafId: leafId, output: 'json', defaultLabelTranslateId: id, defaultLabelNative: $("#defaultLabelNative" + id).val()}, beforeSend: function() {
            var smileyRoll = './images/icons/smiley-roll.png';
            var $infoPanel = $('#infoPanel');
            ...
            var smileyLol = './images/icons/smiley-lol.png';
            if (success === false) {
                $centerViewPort.html('').empty().html("<span class=\'label label-important col-md-12\'><img src='" + smileyRoll + "'>" + message + "</span>");
                ...
                $centerViewPort.html('').empty().html('<span class=\'label label-important \'><img src=\'./images/icons/smiley-roll.png\'>' + message + "</span>");
            }
        }, error: function(xhr) {
            var smileyRollSweat = './images/icons/smiley-roll-sweat.png';
            $('#infoError').html('').empty().html("<span class='alert alert-error col-md-12'><img src='" + smileyRollSweat + "'><strong>" + xhr.status + "</strong> : " + decodeURIComponent(t['loadingErrorTextLabel']) + "</span>");
