function filterGrid(leafId, url, securityToken, offset, limit) {
    $.ajax({type: 'POST', url: url, data: {offset: offset, limit: limit, method: 'read', output: 'table', securityToken: securityToken, leafId: leafId, applicationId: $("#applicationId").val(), languageId: $("#languageId").val()}, beforeSend: function() {
...
function showForm(leafId, url, securityToken) {
    ...
    $.ajax({type: 'POST', url: url, data: {method: 'new', type: 'form', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
            ...
            if (success === false) {
                ...
                $centerViewPort.html('').empty().append(data.data);
                $infoPanel.html('').empty().html("<span class=\"label label-success\"><img src='" + smileyLol + "'>" + decodeURIComponent(t['loadingCompleteTextLabel']) + "</span>").delay(1000).fadeOut();
                if ($infoPanel.is(':hidden')) {
                    $infoPanel.show();
                }
...
function updateRecordInline(leafId, url, securityToken, id) {
    $.ajax({type: 'POST', url: url, data: {method: 'save', securityToken: securityToken, leafId: leafId, output: 'json', applicationTranslateId: id, applicationNative: $("#applicationNative" + id).val()}, beforeSend: function() {
            var smileyRoll = './images/icons/smiley-roll.png';
            var $infoPanel = $('#infoPanel');
