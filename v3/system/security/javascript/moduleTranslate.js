function filterForm(leafId, url, securityToken, offset, limit) {
    $.ajax({type: 'GET', url: url, data: {offset: offset, limit: limit, method: 'read', output: 'option', filter: 'moduleId', applicationId: $("#applicationId").val(), securityToken: securityToken, languageId: $("#languageId").val(), leafId: leafId}, beforeSend: function() {
            ...
            if (success === false) {
                ...
                $('#moduleId').html('').empty().html(data).trigger("chosen:updated");
...
function filterGrid(leafId, url, securityToken, offset, limit) {
    $.ajax({type: 'POST', url: url, data: {offset: offset, limit: limit, method: 'read', output: 'table', securityToken: securityToken, leafId: leafId, applicationId: $("#applicationId").val(), moduleId: $("#moduleId").val(), languageId: $("#languageId").val()}, beforeSend: function() {
            var smileyRoll = './images/icons/smiley-roll.png';
            var $infoPanel = $('#infoPanel');
            $infoPanel.html('').empty().html("<span class='label label-warning'><img src='" + smileyRoll + "'>&nbsp;" + decodeURIComponent(t['loadingTextLabel']) + "....</span>");
            if ($infoPanel.is(':hidden')) {
...
function showGrid(leafId, page, securityToken, offset, limit) {
    $.ajax({type: 'POST', url: page, data: {offset: offset, limit: limit, method: 'read', type: 'list', detail: 'body', securityToken: securityToken, languageId: $("#languageId").val(), leafId: leafId}, beforeSend: function() {
            var smileyRoll = './images/icons/smiley-roll.png';
            var $infoPanel = $('#infoPanel');
            $infoPanel.html('').empty().html("<span class='label label-warning'><img src='" + smileyRoll + "'>&nbsp;" + decodeURIComponent(t['loadingTextLabel']) + "....</span>");
            if ($infoPanel.is(':hidden')) {
            ...
            if (success === false) {
                ...
                $infoPanel.html('').empty().empty().html("<span class='label label-success'>&nbsp;<img src='" + smileyLol + "'> " + decodeURIComponent(t['loadingCompleteTextLabel']) + "</span>").delay(1000).fadeOut();
                if ($infoPanel.is(':hidden')) {
                    $infoPanel.show();
                }
            }
...
function showForm(leafId, url, securityToken) {
    ...
    $.ajax({type: 'POST', url: url, data: {method: 'new', type: 'form', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
            ...
            if (success === false) {
                ...
                $centerViewport.html('').empty().append(data);
                $infoPanel.html('').html('').empty().html("<span class='label label-success'>&nbsp;<img src='" + smileyLol + "'> " + decodeURIComponent(t['loadingCompleteTextLabel']) + "</span>").delay(1000).fadeOut();
            }
        }, error: function(xhr) {
            var smileyRollSweat = './images/icons/smiley-roll-sweat.png';
            $('#infoError').html('').empty().html("<span class='alert alert-error col-md-12'><img src='" + smileyRollSweat + "'><strong>" + xhr.status + "</strong> : " + decodeURIComponent(t['loadingErrorTextLabel']) + "</span>");
...
function updateRecordInline(leafId, url, securityToken, id) {
    $.ajax({type: 'POST', url: url, data: {method: 'save', securityToken: securityToken, leafId: leafId, output: 'json', moduleTranslateId: id, moduleNative: $("#moduleNative" + id).val()}, beforeSend: function() {
            var smileyRoll = './images/icons/smiley-roll.png';
            var $infoPanel = $('#infoPanel');
