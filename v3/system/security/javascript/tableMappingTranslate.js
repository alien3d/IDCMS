function filterForm(leafId, url, securityToken, offset, limit, field) {
    var $tableMappingName = $("#tableMappingName");
    var $tableMappingId = $("#tableMappingId");
    if (field === 'tableMappingName') {
        $tableMappingName.val('').attr("disabled", "disabled");
        $tableMappingId.val('').attr("disabled", "disabled");
    } else if (field === 'tableMappingId') {
        $tableMappingId.val('').attr("disabled", "disabled");
    }
    $.ajax({type: 'GET', url: url, data: {offset: offset, limit: limit, method: 'read', output: 'table', securityToken: securityToken, leafId: leafId, tableMappingId: $tableMappingId.val(), tableMappingName: $tableMappingName.val(), languageId: $("#languageId").val(), filter: field}, beforeSend: function() {
...
function filterGrid(leafId, url, securityToken, offset, limit) {
    $.ajax({type: 'POST', url: url, data: {offset: offset, limit: limit, method: 'read', output: 'table', securityToken: securityToken, leafId: leafId, databaseName: $("#databaseName").val(), tableMappingId: $("#tableMappingId").val(), tableMappingName: $("#tableMappingName").val(), languageId: $("#languageId").val()}, beforeSend: function() {
            var smileyRoll = './images/icons/smiley-roll.png';
            var $infoPanel = $('#infoPanel');
            $infoPanel.html('').empty().html("<span class='label label-warning'><img src='" + smileyRoll + "'>&nbsp;" + decodeURIComponent(t['loadingTextLabel']) + "....</span>");
            if ($infoPanel.is(':hidden')) {
...
function updateRecordInline(leafId, url, securityToken, id) {
    $.ajax({type: 'POST', url: url, data: {method: 'save', securityToken: securityToken, leafId: leafId, output: 'json', tableMappingTranslateId: id, tableMappingNative: $("#tableMappingNative" + id).val()}, beforeSend: function() {
            var smileyRoll = './images/icons/smiley-roll.png';
            var $infoPanel = $('#infoPanel');
