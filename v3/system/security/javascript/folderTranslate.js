function filterForm(leafId, url, securityToken, offset, limit, field) {
    $.ajax({type: 'GET', url: url, data: {offset: offset, limit: limit, method: 'read', output: 'table', securityToken: securityToken, leafId: leafId, applicationId: $("#applicationId").val(), moduleId: $("#moduleId").val(), folderId: $("#folderId").val(), languageId: $("#languageId").val(), filter: field}, beforeSend: function() {
...
function filterGrid(leafId, url, securityToken, offset, limit) {
    $.ajax({type: 'POST', url: url, data: {offset: offset, limit: limit, method: 'read', output: 'table', securityToken: securityToken, leafId: leafId, applicationId: $("#applicationId").val(), moduleId: $("#moduleId").val(), folderId: $("#folderId").val(), languageId: $("#languageId").val()}, beforeSend: function() {
            var smileyRoll = './images/icons/smiley-roll.png';
            var $infoPanel = $('#infoPanel');
            $infoPanel.html('').empty().html("<span class='label label-warning'><img src='" + smileyRoll + "'>&nbsp;" + decodeURIComponent(t['loadingTextLabel']) + "....</span>");
            if ($infoPanel.is(':hidden')) {
...
function updateRecordInline(leafId, url, securityToken, id) {
    $.ajax({type: 'POST', url: url, data: {method: 'save', securityToken: securityToken, leafId: leafId, output: 'json', folderTranslateId: id, folderNative: $("#folderNative" + id).val()}, beforeSend: function() {
            var smileyRoll = './images/icons/smiley-roll.png';
            var $infoPanel = $('#infoPanel');
            ...
            if (success === false) {
                ...
                $infoPanel.html('').empty().html("<span class='label label-success'>&nbsp;<img src='" + smileyLol + "'> " + decodeURIComponent(t['loadingCompleteTextLabel']) + "</span>").delay(1000).fadeOut();
                $('#infoPanelMini' + id).empty().html('').html("<img src='" + smileyLol + "'>" + decodeURIComponent(t['updateRecordTextLabel'])).delay(1000).fadeOut();
            } else {
                $centerViewPort.html('').empty().html("<span class=\"label label-success\"><img src='" + smileyRoll + "'>" + message + "</span>");
            }
        }, error: function(xhr) {
