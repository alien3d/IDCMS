...
require_once($newFakeDocumentRoot . "v3/system/common/controller/religionController.php");
...
$translator->setCurrentTable('religion');
...
if (isset($_POST)) {
    if (isset($_POST['method'])) {
        $religion = new \Core\System\Common\Religion\Controller\ReligionClass();
        define('LIMIT', 10);
        ...
        if (isset($_POST ['query'])) {
            $religion->setFieldQuery($_POST ['query']);
        }
        if (isset($_POST ['filter'])) {
            $religion->setGridQuery($_POST ['filter']);
        }
        if (isset($_POST ['character'])) {
            $religion->setCharacterQuery($_POST['character']);
        }
        if (isset($_POST ['dateRangeStart'])) {
            $religion->setDateRangeStartQuery($_POST['dateRangeStart']);
            //explode the data to get day,month,year
            $start = explode('-', $_POST ['dateRangeStart']);
            $religion->setStartDay($start[2]);
            $religion->setStartMonth($start[1]);
            $religion->setStartYear($start[0]);
        }
        if (isset($_POST ['dateRangeEnd']) && (strlen($_POST['dateRangeEnd']) > 0)) {
            $religion->setDateRangeEndQuery($_POST['dateRangeEnd']);
            //explode the data to get day,month,year
            $start = explode('-', $_POST ['dateRangeEnd']);
            $religion->setEndDay($start[2]);
            $religion->setEndMonth($start[1]);
            $religion->setEndYear($start[0]);
        }
        if (isset($_POST ['dateRangeType'])) {
            $religion->setDateRangeTypeQuery($_POST['dateRangeType']);
        }
        if (isset($_POST ['dateRangeExtraType'])) {
            $religion->setDateRangeExtraTypeQuery($_POST['dateRangeExtraType']);
        }
        $religion->setServiceOutput('html');
        $religion->setLeafId($leafId);
        $religion->execute();
        if ($_POST['method'] == 'read') {
            $religion->setStart($offset);
            $religion->setLimit($limit); // normal system don't like paging..
            $religion->setPageOutput('html');
            $religionArray = $religion->read();
            if (isset($religionArray [0]['firstRecord'])) {
                $firstRecord = $religionArray [0]['firstRecord'];
            }
            if (isset($religionArray [0]['nextRecord'])) {
                $nextRecord = $religionArray [0]['nextRecord'];
            }
            if (isset($religionArray [0]['previousRecord'])) {
                $previousRecord = $religionArray [0]['previousRecord'];
            }
            if (isset($religionArray [0]['lastRecord'])) {
                $lastRecord = $religionArray [0]['lastRecord'];
                $endRecord = $religionArray [0]['lastRecord'];
            }
            $navigation = new \Core\Paging\HtmlPaging();
            $navigation->setLeafId($leafId);
            $navigation->setViewPath($religion->getViewPath());
            ...
            if (isset($religionArray [0]['total'])) {
                $total = $religionArray [0]['total'];
            } else {
                $total = 0;
            }
                        ...
                        <div class="btn-group">
                            ...
                            <ul class="dropdown-menu" style="text-align:left">
                                <li>
                                    ...
                                    echo $religion->getControllerPath();
                    ...
                    <div class="panel panel-default">
                        <div class="panel-body">
                                ...
                                <div class="input-group"><input type="text" class="form-control"
                                                                name="dateRangeStart" id="dateRangeStart"
                                                                value="<?php
                                                                if (isset($_POST['dateRangeStart'])) {
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <form class="form-horizontal">
                                        <input type="hidden" name="religionIdPreview" id="religionIdPreview">

                                        <div class="form-group" id="religionCodeDiv">
                                            <label for="religionCodePreview"
                                                   class="control-label col-xs-4 col-sm-4 col-md-4"><?php echo $leafTranslation['religionCodeLabel']; ?></label>

                                            <div class="col-xs-8 col-sm-8 col-md-8">
                                                <input class="form-control" type="text" name="religionCodePreview"
                                                       id="religionCodePreview">
                                            </div>
                                        </div>
                                        <div class="form-group" id="religionDescriptionDiv">
                                            <label for="religionDescriptionPreview"
                                                   class="control-label col-xs-4 col-sm-4 col-md-4"><?php echo $leafTranslation['religionDescriptionLabel']; ?></label>

                                            <div class="col-xs-8 col-sm-8 col-md-8">
                                                <input class="form-control" type="text" name="religionDescriptionPreview"
                                                       id="religionDescriptionPreview">
                                            </div>
                                        </div>
                                    </form>
                                </div>
                        ...
                        <div class="col-xs-12 col-sm-12 col-md-12">
                            <div class="panel panel-default">
                                <table class="table table table-bordered table-striped table-condensed table-hover smart-form has-tickbox" id="tableData">
                                    ...
                                    <th width="100px">
                                    <div align="center"><?php echo ucwords($leafTranslation['religionCodeLabel']); ?></div>
                                    </th>
                                    <th><?php echo ucwords($leafTranslation['religionDescriptionLabel']); ?></th>
                                    <th width="100px">
                                            ...
                                            if (is_array($religionArray)) {
                                                ...
                                                if ($totalRecord > 0) {
                                                    ...
                                                    for ($i = 0; $i < $totalRecord; $i++) {
                                                        ...
                                                        ?>
                                                        <tr <?php
                                                        if ($religionArray[$i]['isDelete'] == 1) {
                                                            echo "class=\"danger\"";
                                                        } else {
                                                            if ($religionArray[$i]['isDraft'] == 1) {
                                                                echo "class=\"warning\"";
                                                            }
                                                        }
                                                        ?>>
                                                            ...
                                                            <td align="center">
                                                                <div class="btn-group" align="center">
																	<?php if($religionArray [$i]['religionCode'] !='UNBL') { ?>
                                                                    ...
                                                                    ?>', '<?php
                                                                    echo rawurlencode(
                                                                            $religionArray [$i]['religionCode']
                                                                    );
                                                                    ?>', '<?php
                                                                    echo rawurlencode(
                                                                            $religionArray [$i]['religionDescription']
                                                                    );
                                                                    ?>');">
                                                            ...
                                                            <td>
                                                                <div align="center">
                                                                    <?php
                                                                    if (isset($religionArray[$i]['religionCode'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos(
                                                                                                strtolower($religionArray[$i]['religionCode']), strtolower($_POST['query'])
                                                                                        ) !== false
                                                                                ) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $religionArray[$i]['religionCode']
                                                                                    );
                                                                                } else {
                                                                                    echo $religionArray[$i]['religionCode'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                    if (strpos(
                                                                                                    strtolower($religionArray[$i]['religionCode']), strtolower($_POST['character'])
                                                                                            ) !== false
                                                                                    ) {
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $religionArray[$i]['religionCode']
                                                                                        );
                                                                                    } else {
                                                                                        echo $religionArray[$i]['religionCode'];
                                                                                    }
                                                            ...
                                                            <td>
                                                                <div class="pull-left">
                                                                    <?php
                                                                    if (isset($religionArray[$i]['religionDescription'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos(
                                                                                                strtolower($religionArray[$i]['religionDescription']), strtolower($_POST['query'])
                                                                                        ) !== false
                                                                                ) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $religionArray[$i]['religionDescription']
                                                                                    );
                                                                                } else {
                                                                                    echo $religionArray[$i]['religionDescription'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                    if (strpos(
                                                                                                    strtolower($religionArray[$i]['religionDescription']), strtolower($_POST['character'])
                                                                                            ) !== false
                                                                                    ) {
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $religionArray[$i]['religionDescription']
                                                                                        );
                                                                                    } else {
                                                                                        echo $religionArray[$i]['religionDescription'];
                                                                                    }
                                                            ...
                                                            <td>
                                                                <div align="center">
                                                                    <?php
                                                                    if (isset($religionArray[$i]['executeBy'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos($religionArray[$i]['staffName'], $_POST['query']) !== false) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $religionArray[$i]['staffName']
                                                                                    );
                                                                                } else {
                                                                                    echo $religionArray[$i]['staffName'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                    if (strpos(
                                                                                                    $religionArray[$i]['staffName'], $_POST['character']
                                                                                            ) !== false
                                                                                    ) {
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $religionArray[$i]['staffName']
                                                                                        );
                                                                                    } else {
                                                                                        echo $religionArray[$i]['staffName'];
                                                                                    }
                                                            ...
                                                            <?php
                                                            if (isset($religionArray[$i]['executeTime'])) {
                                                                $valueArray = $religionArray[$i]['executeTime'];
                                                            ...
                                                            <?php } ?>
                                                            <?php
                                                            if ($religionArray[$i]['isDelete']) {
                                                                $checked = "checked";
                                                            } else {
                                                                $checked = null;
                                                            }
                                                            ?>
                                                            <td><?php if($religionArray [$i]['religionCode'] !='UNBL') { ?>
                                                                <input style="display:none;" type="checkbox" name="religionId[]"
                                                                       value="<?php echo $religionArray[$i]['religionId']; ?>">
                                                                <input <?php echo $checked; ?> type="checkbox" name="isDelete[]"
                                                                                               value="<?php echo $religionArray[$i]['isDelete']; ?>">
																<?php } else { ?>&nbsp;<?php } ?>
                                                            </td>
                                                        </tr>
                                                        <?php
                        ...
                        <div class="col-xs-3 col-sm-3 col-md-3 pagination">
                            <div class="pull-right">
                                <button class="delete btn btn-warning" type="button" 
                                        ...
                                        echo $religion->getControllerPath();
                                        ?>', '<?php echo $religion->getViewPath(); ?>', '<?php echo $securityToken; ?>');">
...
if ((isset($_POST['method']) == 'new' || isset($_POST['method']) == 'read') && $_POST['type'] == 'form') {
            ...
            <div class="row">
                <div class="col-xs-12 col-sm-12 col-md-12">
                    <div class="panel panel-info">
                        ...
                        <div class="panel-body">
                            <input type="hidden" name="religionId" id="religionId" value="<?php
                            if (isset($_POST['religionId'])) {
                                echo $_POST['religionId'];
                            }
                            ?>">

                            <div class="form-group" id="religionCodeForm">
                                <label class="control-label col-xs-2 col-sm-2 col-md-2" for="religionCode"><strong><?php
                                        echo ucfirst(
                                                $leafTranslation['religionCodeLabel']
                                        );
                                        ?></strong></label>

                                <div class="control-label col-xs-4 col-sm-4 col-md-4">
                                    <div class="input-group">
                                        <input class="form-control" type="text" name="religionCode" id="religionCode"
                                               value="<?php
                                               if (isset($religionArray) && is_array($religionArray)) {
                                                   echo htmlentities($religionArray[0]['religionCode']);
                                               }
                                               ?>" maxlength="16"><span class="input-group-addon"><img
                                                src="./images/icons/document-code.png"></span>
                                    </div>
                                    <span class="help-block" id="religionCodeHelpMe"></span>
                                </div>
                            </div>
                            <div class="form-group" id="religionDescriptionForm">
                                <div class="col-xs-12 col-sm-12 col-md-12">
                                    <textarea class="form-control" name="religionDescription" id="religionDescription" rows="5"><?php
                                        if (isset($religionArray[0]['religionDescription'])) {
                                            echo htmlentities($religionArray[0]['religionDescription']);
                                        }
                                        ?></textarea> <span class="help-block" id="religionDescriptionHelpMe"></span>
                                </div>
                            </div>
                        </div>
                ...
                <script type="text/javascript">
                    $(document).ready(function() {
                        ...
                        validateMeAlphaNumeric('religionCode');
    <?php if ($_POST['method'] == "new") { ?>
                            $('#resetRecordButton')
                                    .removeClass().addClass('btn btn-default');
        <?php if ($leafAccess['leafAccessCreateValue'] == 1) { ?>
                                $('#newRecordButton1')
                                        .removeClass().addClass('btn btn-success').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $religion->getControllerPath(); ?>','<?php echo $religion->getViewPath(); ?>','<?php echo $securityToken; ?>',1)");
                                $('#newRecordButton2')
                                        .removeClass().addClass('btn dropdown-toggle btn-success');
                                $('#newRecordButton3').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $religion->getControllerPath(); ?>','<?php echo $religion->getViewPath(); ?>','<?php echo $securityToken; ?>',1)");
                                $('#newRecordButton4').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $religion->getControllerPath(); ?>','<?php echo $religion->getViewPath(); ?>','<?php echo $securityToken; ?>',2)");
                                $('#newRecordButton5').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $religion->getControllerPath(); ?>','<?php echo $religion->getViewPath(); ?>','<?php echo $securityToken; ?>',3)");
                                $('#newRecordButton6').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $religion->getControllerPath(); ?>','<?php echo $religion->getViewPath(); ?>','<?php echo $securityToken; ?>',4)");
                                $('#newRecordButton7').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $religion->getControllerPath(); ?>','<?php echo $religion->getViewPath(); ?>','<?php echo $securityToken; ?>',5)");
        <?php } else { ?>
                                $('#newRecordButton1')
        ...
        if ($_POST['religionId']) {
            ...
            <?php if ($leafAccess['leafAccessUpdateValue'] == 1) { ?>
                                    $('#updateRecordButton1')
                                            .removeClass().addClass('btn btn-info').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $religion->getControllerPath(); ?>','<?php echo $religion->getViewPath(); ?>','<?php echo $securityToken; ?>',1,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                                    $('#updateRecordButton2')
                                            .removeClass().addClass('btn dropdown-toggle btn-info');
                                    $('#updateRecordButton3').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $religion->getControllerPath(); ?>','<?php echo $religion->getViewPath(); ?>','<?php echo $securityToken; ?>',1,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                                    $('#updateRecordButton4').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $religion->getControllerPath(); ?>','<?php echo $religion->getViewPath(); ?>','<?php echo $securityToken; ?>',2,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                                    $('#updateRecordButton5').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $religion->getControllerPath(); ?>','<?php echo $religion->getViewPath(); ?>','<?php echo $securityToken; ?>',3,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
            <?php } else { ?>
                                    $('#updateRecordButton1')
                                            .removeClass().addClass(' btn btn-info disabled');
                                    $('#updateRecordButton2')
            ...
            <?php if ($leafAccess['leafAccessDeleteValue'] == 1) { ?>
                                    $('#deleteRecordButton')
                                            ...
                                            .attr('onClick', "deleteRecord(<?php echo $leafId; ?>,'<?php echo $religion->getControllerPath(); ?>','<?php echo $religion->getViewPath(); ?>','<?php echo $securityToken; ?>',<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
            <?php } else { ?>
                                    $('#deleteRecordButton')
                                            .removeClass().addClass('btn btn-danger disabled')
                                            .attr('onClick', '');
    ...
    </div></form><script type="text/javascript" src="./v3/system/common/javascript/religion.js"></script>
<hr>
