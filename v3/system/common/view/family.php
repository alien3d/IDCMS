...
require_once($newFakeDocumentRoot . "v3/system/common/controller/familyController.php");
...
$translator->setCurrentTable('family');
...
if (isset($_POST)) {
    if (isset($_POST['method'])) {
        $family = new \Core\System\Common\Family\Controller\FamilyClass();
        define('LIMIT', 10);
        ...
        if (isset($_POST ['query'])) {
            $family->setFieldQuery($_POST ['query']);
        }
        if (isset($_POST ['filter'])) {
            $family->setGridQuery($_POST ['filter']);
        }
        if (isset($_POST ['character'])) {
            $family->setCharacterQuery($_POST['character']);
        }
        if (isset($_POST ['dateRangeStart'])) {
            $family->setDateRangeStartQuery($_POST['dateRangeStart']);
            //explode the data to get day,month,year
            $start = explode('-', $_POST ['dateRangeStart']);
            $family->setStartDay($start[2]);
            $family->setStartMonth($start[1]);
            $family->setStartYear($start[0]);
        }
        if (isset($_POST ['dateRangeEnd']) && (strlen($_POST['dateRangeEnd']) > 0)) {
            $family->setDateRangeEndQuery($_POST['dateRangeEnd']);
            //explode the data to get day,month,year
            $start = explode('-', $_POST ['dateRangeEnd']);
            $family->setEndDay($start[2]);
            $family->setEndMonth($start[1]);
            $family->setEndYear($start[0]);
        }
        if (isset($_POST ['dateRangeType'])) {
            $family->setDateRangeTypeQuery($_POST['dateRangeType']);
        }
        if (isset($_POST ['dateRangeExtraType'])) {
            $family->setDateRangeExtraTypeQuery($_POST['dateRangeExtraType']);
        }
        $family->setServiceOutput('html');
        $family->setLeafId($leafId);
        $family->execute();
        if ($_POST['method'] == 'read') {
            $family->setStart($offset);
            $family->setLimit($limit); // normal system don't like paging..
            $family->setPageOutput('html');
            $familyArray = $family->read();
            if (isset($familyArray [0]['firstRecord'])) {
                $firstRecord = $familyArray [0]['firstRecord'];
            }
            if (isset($familyArray [0]['nextRecord'])) {
                $nextRecord = $familyArray [0]['nextRecord'];
            }
            if (isset($familyArray [0]['previousRecord'])) {
                $previousRecord = $familyArray [0]['previousRecord'];
            }
            if (isset($familyArray [0]['lastRecord'])) {
                $lastRecord = $familyArray [0]['lastRecord'];
                $endRecord = $familyArray [0]['lastRecord'];
            }
            $navigation = new \Core\Paging\HtmlPaging();
            $navigation->setLeafId($leafId);
            $navigation->setViewPath($family->getViewPath());
            ...
            if (isset($familyArray [0]['total'])) {
                $total = $familyArray [0]['total'];
            } else {
                $total = 0;
            }
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <form class="form-horizontal">
                                        <input type="hidden" name="familyIdPreview" id="familyIdPreview">

                                        <div class="form-group" id="familyCodeDiv">
                                            <label for="familyCodePreview"
                                                   class="control-label col-xs-4 col-sm-4 col-md-4"><?php echo $leafTranslation['familyCodeLabel']; ?></label>

                                            <div class="col-xs-8 col-sm-8 col-md-8">
                                                <input class="form-control" type="text" name="familyCodePreview"
                                                       id="familyCodePreview">
                                            </div>
                                        </div>
                                        <div class="form-group" id="familyDescriptionDiv">
                                            <label for="familyDescriptionPreview"
                                                   class="control-label col-xs-4 col-sm-4 col-md-4"><?php echo $leafTranslation['familyDescriptionLabel']; ?></label>

                                            <div class="col-xs-8 col-sm-8 col-md-8">
                                                <input class="form-control" type="text" name="familyDescriptionPreview"
                                                       id="familyDescriptionPreview">
                                            </div>
                                        </div>
                                    </form>
                                </div>
                        ...
                        <div class="col-xs-12 col-sm-12 col-md-12">
                            <div class="panel panel-default">
                                <table class="table table table-bordered table-striped table-condensed table-hover smart-form has-tickbox" id="tableData">
                                    ...
                                    <th width="100px">
                                    <div align="center"><?php echo ucwords($leafTranslation['familyCodeLabel']); ?></div>
                                    </th>
                                    <th><?php echo ucwords($leafTranslation['familyDescriptionLabel']); ?></th>
                                    <th width="100px">
                                    ...
                                    <tbody id="tableBody">
                                        ...
                                        if ($_POST['method'] == 'read' && $_POST['type'] == 'list' && $_POST['detail'] == 'body') {
                                            if (is_array($familyArray)) {
                                                $totalRecord = intval(count($familyArray));
                                                if ($totalRecord > 0) {
                                                    $counter = 0;
                                                    for ($i = 0; $i < $totalRecord; $i++) {
                                                        ...
                                                        ?>
                                                        <tr <?php
                                                        if ($familyArray[$i]['isDelete'] == 1) {
                                                            echo "class=\"danger\"";
                                                        } else {
                                                            if ($familyArray[$i]['isDraft'] == 1) {
                                                                echo "class=\"warning\"";
                                                            }
                                                        }
                                                        ?>>
                                                            ...
                                                            <td align="center">
															    <?php if($familyArray [$i]['familyCode'] !='UNBL') { ?>
                                                                <div class="btn-group" align="center">
                                                                    ...
                                                                    ?>', '<?php
                                                                    echo rawurlencode(
                                                                            $familyArray [$i]['familyCode']
                                                                    );
                                                                    ?>', '<?php
                                                                    echo rawurlencode(
                                                                            $familyArray [$i]['familyDescription']
                                                                    );
                                                                    ?>');">
                                                            ...
                                                            <td>
                                                                <div class="pull-left">
                                                                    <?php
                                                                    if (isset($familyArray[$i]['familyCode'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos(
                                                                                                strtolower($familyArray[$i]['familyCode']), strtolower($_POST['query'])
                                                                                        ) !== false
                                                                                ) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $familyArray[$i]['familyCode']
                                                                                    );
                                                                                } else {
                                                                                    echo $familyArray[$i]['familyCode'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                    if (strpos(
                                                                                                    strtolower($familyArray[$i]['familyCode']), strtolower($_POST['character'])
                                                                                            ) !== false
                                                                                    ) {
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $familyArray[$i]['familyCode']
                                                                                        );
                                                                                    } else {
                                                                                        echo $familyArray[$i]['familyCode'];
                                                                                    }
                                                            ...
                                                            <td>
                                                                <div class="pull-left">
                                                                    <?php
                                                                    if (isset($familyArray[$i]['familyDescription'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos(
                                                                                                strtolower($familyArray[$i]['familyDescription']), strtolower($_POST['query'])
                                                                                        ) !== false
                                                                                ) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $familyArray[$i]['familyDescription']
                                                                                    );
                                                                                } else {
                                                                                    echo $familyArray[$i]['familyDescription'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                    if (strpos(
                                                                                                    strtolower($familyArray[$i]['familyDescription']), strtolower($_POST['character'])
                                                                                            ) !== false
                                                                                    ) {
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $familyArray[$i]['familyDescription']
                                                                                        );
                                                                                    } else {
                                                                                        echo $familyArray[$i]['familyDescription'];
                                                                                    }
                                                            ...
                                                            <td>
                                                                <div align="center">
                                                                    <?php
                                                                    if (isset($familyArray[$i]['executeBy'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos($familyArray[$i]['staffName'], $_POST['query']) !== false) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $familyArray[$i]['staffName']
                                                                                    );
                                                                                } else {
                                                                                    echo $familyArray[$i]['staffName'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                    if (strpos(
                                                                                                    $familyArray[$i]['staffName'], $_POST['character']
                                                                                            ) !== false
                                                                                    ) {
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $familyArray[$i]['staffName']
                                                                                        );
                                                                                    } else {
                                                                                        echo $familyArray[$i]['staffName'];
                                                                                    }
                                                            ...
                                                            <?php
                                                            if (isset($familyArray[$i]['executeTime'])) {
                                                                $valueArray = $familyArray[$i]['executeTime'];
                                                            ...
                                                            <td>
															<?php if($familyArray [$i]['familyCode'] !='UNBL') { ?>
                                                                <input style="display:none;" type="checkbox" name="familyId[]"
                                                                       value="<?php echo $familyArray[$i]['familyId']; ?>">
                                                                <input <?php echo $checked; ?> type="checkbox" name="isDelete[]"
                                                                                               value="<?php echo $familyArray[$i]['isDelete']; ?>">
															<?php } else { ?>&nbsp;<?php } ?>
                                                            </td>
                                                        </tr>
                                                        <?php
                        ...
                        <div class="col-md-3 pull-right">
                            <div class="pagination">
                                <button class="delete btn btn-warning" type="button" 
                                        ...
                                        echo $family->getControllerPath();
                                        ?>', '<?php echo $family->getViewPath(); ?>', '<?php echo $securityToken; ?>');">
...
if ((isset($_POST['method']) == 'new' || isset($_POST['method']) == 'read') && $_POST['type'] == 'form') {
                ...
                <div class="col-xs-12 col-sm-12 col-md-12">
                    <div class="panel panel-info">
                        <div class="panel-body">
                            ...
                            <div class="form-group" id="familyCodeForm">
                                <label for="familyCode" class="control-label col-xs-2 col-sm-2 col-md-2"><strong><?php
                                        echo ucfirst(
                                                $leafTranslation['familyCodeLabel']
                                        );
                                        ?></strong></label>

                                <div class="control-label col-xs-4 col-sm-4 col-md-4">
                                    <div class="input-group">
                                        <input class="form-control" type="text" name="familyCode" id="familyCode" value="<?php
                                        if (isset($familyArray) && is_array($familyArray)) {
                                            if (isset($familyArray[0]['familyCode'])) {
                                                echo htmlentities($familyArray[0]['familyCode']);
                                            }
                                        }
                                        ?>" maxlength="16"><span class="input-group-addon"><img src="./images/icons/document-code.png"></span>
                                    </div>
                                    <span class="help-block" id="familyCodeHelpMe"></span>
                                </div>
                            </div>
                            <div class="form-group" id="familyDescriptionForm">
                                <div class="col-xs-12 col-sm-12 col-md-12">
                                    <textarea rows="5" class="form-control" name="familyDescription"
                                              id="familyDescription"><?php
                                                  if (isset($familyArray[0]['familyDescription'])) {
                                                      echo htmlentities($familyArray[0]['familyDescription']);
                                                  }
                                                  ?></textarea> <span class="help-block" id="familyDescriptionHelpMe"></span>
                                </div>
                            </div>
                        </div>
    ...
    <?php if ($leafAccess['leafAccessCreateValue'] == 1) { ?>
                                newRecord(<?php echo $leafId; ?>, '<?php echo $family->getControllerPath(); ?>', '<?php echo $family->getViewPath(); ?>', '<?php echo $securityToken; ?>', 1);
    <?php } ?>
                            return false;
                        }
                        // shift+s save event
    ...
    <?php if ($leafAccess['leafAccessUpdateValue'] == 1) { ?>
                                updateRecord(<?php echo $leafId; ?>, '<?php echo $family->getControllerPath(); ?>', '<?php echo $family->getViewPath(); ?>', '<?php echo $securityToken; ?>', 1, <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
    <?php } ?>
                            return false;
                        }
    ...
    <?php if ($leafAccess['leafAccessDeleteValue'] == 1) { ?>
                                deleteRecord(<?php echo $leafId; ?>, '<?php echo $family->getControllerPath(); ?>', '<?php echo $family->getViewPath(); ?>', '<?php echo $securityToken; ?>', <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
    ...
    <?php } ?>
                        ...
                        switch (e.keyCode) {
                            case 37:
                                previousRecord(<?php echo $leafId; ?>, '<?php echo $family->getControllerPath(); ?>', '<?php echo $family->getViewPath(); ?>', '<?php echo $securityToken; ?>', <?php echo $leafAccess['leafAccessUpdateValue']; ?>, <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
                                
                                return false;
                                break;
                            case 39:
                                nextRecord(<?php echo $leafId; ?>, '<?php echo $family->getControllerPath(); ?>', '<?php echo $family->getViewPath(); ?>', '<?php echo $securityToken; ?>', <?php echo $leafAccess['leafAccessUpdateValue']; ?>, <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
                    ...
                    $(document).ready(function() {
                        ...
                        validateMeNumeric('familyId');
                        validateMeAlphaNumeric('familyCode');
    <?php if ($_POST['method'] == "new") { ?>
                            $('#resetRecordButton')
                                    .removeClass().addClass('btn btn-default');
        <?php if ($leafAccess['leafAccessCreateValue'] == 1) { ?>
                                $('#newRecordButton1')
                                        .removeClass().addClass('btn btn-success').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $family->getControllerPath(); ?>','<?php echo $family->getViewPath(); ?>','<?php echo $securityToken; ?>',1)");
                                $('#newRecordButton2')
                                        .removeClass().addClass('btn dropdown-toggle btn-success');
                                $('#newRecordButton3').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $family->getControllerPath(); ?>','<?php echo $family->getViewPath(); ?>','<?php echo $securityToken; ?>',1)");
                                $('#newRecordButton4').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $family->getControllerPath(); ?>','<?php echo $family->getViewPath(); ?>','<?php echo $securityToken; ?>',2)");
                                $('#newRecordButton5').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $family->getControllerPath(); ?>','<?php echo $family->getViewPath(); ?>','<?php echo $securityToken; ?>',3)");
                                $('#newRecordButton6').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $family->getControllerPath(); ?>','<?php echo $family->getViewPath(); ?>','<?php echo $securityToken; ?>',4)");
                                $('#newRecordButton7').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $family->getControllerPath(); ?>','<?php echo $family->getViewPath(); ?>','<?php echo $securityToken; ?>',5)");
        <?php } else { ?>
                                $('#newRecordButton1')
        ...
        if ($_POST['familyId']) {
            ...
            <?php if ($leafAccess['leafAccessUpdateValue'] == 1) { ?>
                                    $('#updateRecordButton1')
                                            .removeClass().addClass('btn btn-info').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $family->getControllerPath(); ?>','<?php echo $family->getViewPath(); ?>','<?php echo $securityToken; ?>',1,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                                    $('#updateRecordButton2')
                                            .removeClass().addClass('btn dropdown-toggle btn-info');
                                    $('#updateRecordButton3').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $family->getControllerPath(); ?>','<?php echo $family->getViewPath(); ?>','<?php echo $securityToken; ?>',1,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                                    $('#updateRecordButton4').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $family->getControllerPath(); ?>','<?php echo $family->getViewPath(); ?>','<?php echo $securityToken; ?>',2,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                                    $('#updateRecordButton5').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $family->getControllerPath(); ?>','<?php echo $family->getViewPath(); ?>','<?php echo $securityToken; ?>',3,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
            <?php } else { ?>
                                    $('#updateRecordButton1')
                                            .removeClass().addClass(' btn btn-info disabled');
                                    $('#updateRecordButton2')
            ...
            <?php if ($leafAccess['leafAccessDeleteValue'] == 1) { ?>
                                    $('#deleteRecordButton')
                                            ...
                                            .attr('onClick', "deleteRecord(<?php echo $leafId; ?>,'<?php echo $family->getControllerPath(); ?>','<?php echo $family->getViewPath(); ?>','<?php echo $securityToken; ?>',<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
            <?php } else { ?>
                                    $('#deleteRecordButton')
                                            .removeClass().addClass('btn btn-danger disabled')
                                            .attr('onClick', '');
...
<script type="text/javascript" src="./v3/system/common/javascript/family.js"></script>
<hr>
