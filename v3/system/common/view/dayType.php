...
require_once($newFakeDocumentRoot . "v3/system/common/controller/dayTypeController.php");
...
$translator->setCurrentTable('dayType');
...
if (isset($_POST)) {
    if (isset($_POST['method'])) {
        $dayType = new \Core\System\Common\DayType\Controller\DayTypeClass();
        define('LIMIT', 10);
        ...
        if (isset($_POST ['query'])) {
            $dayType->setFieldQuery($_POST ['query']);
        }
        if (isset($_POST ['filter'])) {
            $dayType->setGridQuery($_POST ['filter']);
        }
        if (isset($_POST ['character'])) {
            $dayType->setCharacterQuery($_POST['character']);
        }
        if (isset($_POST ['dateRangeStart'])) {
            $dayType->setDateRangeStartQuery($_POST['dateRangeStart']);
            //explode the data to get day,month,year
            $start = explode('-', $_POST ['dateRangeStart']);
            $dayType->setStartDay($start[2]);
            $dayType->setStartMonth($start[1]);
            $dayType->setStartYear($start[0]);
        }
        if (isset($_POST ['dateRangeEnd']) && (strlen($_POST['dateRangeEnd']) > 0)) {
            $dayType->setDateRangeEndQuery($_POST['dateRangeEnd']);
            //explode the data to get day,month,year
            $start = explode('-', $_POST ['dateRangeEnd']);
            $dayType->setEndDay($start[2]);
            $dayType->setEndMonth($start[1]);
            $dayType->setEndYear($start[0]);
        }
        if (isset($_POST ['dateRangeType'])) {
            $dayType->setDateRangeTypeQuery($_POST['dateRangeType']);
        }
        if (isset($_POST ['dateRangeExtraType'])) {
            $dayType->setDateRangeExtraTypeQuery($_POST['dateRangeExtraType']);
        }
        $dayType->setServiceOutput('html');
        $dayType->setLeafId($leafId);
        $dayType->execute();
        if ($_POST['method'] == 'read') {
            $dayType->setStart($offset);
            $dayType->setLimit($limit); // normal system don't like paging..
            $dayType->setPageOutput('html');
            $dayTypeArray = $dayType->read();
            if (isset($dayTypeArray [0]['firstRecord'])) {
                $firstRecord = $dayTypeArray [0]['firstRecord'];
            }
            if (isset($dayTypeArray [0]['nextRecord'])) {
                $nextRecord = $dayTypeArray [0]['nextRecord'];
            }
            if (isset($dayTypeArray [0]['previousRecord'])) {
                $previousRecord = $dayTypeArray [0]['previousRecord'];
            }
            if (isset($dayTypeArray [0]['lastRecord'])) {
                $lastRecord = $dayTypeArray [0]['lastRecord'];
                $endRecord = $dayTypeArray [0]['lastRecord'];
            }
            $navigation = new \Core\Paging\HtmlPaging();
            $navigation->setLeafId($leafId);
            $navigation->setViewPath($dayType->getViewPath());
            ...
            if (isset($dayTypeArray [0]['total'])) {
                $total = $dayTypeArray [0]['total'];
            } else {
                $total = 0;
            }
                        ...
                        <div class="btn-group">
                            ...
                            <ul class="dropdown-menu" style="text-align:left">
                                <li>
                                    ...
                                    echo $dayType->getControllerPath();
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <form class="form-horizontal">
                                        <input type="hidden" name="dayTypeIdPreview" id="dayTypeIdPreview">

                                        <div class="form-group" id="dayTypeDescriptionDiv">
                                            <label for="dayTypeDescriptionPreview"
                                                   class="control-label col-xs-4 col-sm-4 col-md-4"><?php echo $leafTranslation['dayTypeDescriptionLabel']; ?></label>

                                            <div class="col-xs-8 col-sm-8 col-md-8">
                                                <input class="form-control" type="text" name="dayTypeDescriptionPreview"
                                                       id="dayTypeDescriptionPreview">
                                            </div>
                                        </div>
                                    </form>
                                </div>
                        ...
                        <div class="col-xs-12 col-sm-12 col-md-12">
                            <div class="panel panel-default">
                                <table class="table table table-bordered table-striped table-condensed table-hover smart-form has-tickbox" id="tableData">
                                    ...
                                    <th><?php echo ucwords($leafTranslation['dayTypeDescriptionLabel']); ?></th>
                                    <th width="100px">
                                    ...
                                    <tbody id="tableBody">
                                        ...
                                        if ($_POST['method'] == 'read' && $_POST['type'] == 'list' && $_POST['detail'] == 'body') {
                                            if (is_array($dayTypeArray)) {
                                                $totalRecord = intval(count($dayTypeArray));
                                                if ($totalRecord > 0) {
                                                    $counter = 0;
                                                    for ($i = 0; $i < $totalRecord; $i++) {
                                                        ...
                                                        ?>
                                                        <tr <?php
                                                        if ($dayTypeArray[$i]['isDelete'] == 1) {
                                                            echo "class=\"danger\"";
                                                        } else {
                                                            if ($dayTypeArray[$i]['isDraft'] == 1) {
                                                                echo "class=\"warning\"";
                                                            }
                                                        }
                                                        ?>>
                                                            ...
                                                            <td align="center">
																<?php if($dayTypeArray [$i]['dayTypeCode'] !='UNBL') { ?>
                                                                <div class="btn-group" align="center">
                                                                    ...
                                                                    ?>', '<?php
                                                                    echo rawurlencode(
                                                                            $dayTypeArray [$i]['dayTypeDescription']
                                                                    );
                                                                    ?>');">
                                                            ...
                                                            <td>
                                                                <div class="pull-left">
                                                                    <?php
                                                                    if (isset($dayTypeArray[$i]['dayTypeDescription'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos(
                                                                                                strtolower($dayTypeArray[$i]['dayTypeDescription']), strtolower($_POST['query'])
                                                                                        ) !== false
                                                                                ) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $dayTypeArray[$i]['dayTypeDescription']
                                                                                    );
                                                                                } else {
                                                                                    echo $dayTypeArray[$i]['dayTypeDescription'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen(
                                                                                    ...
                                                                                    if (strpos(
                                                                                                    strtolower($dayTypeArray[$i]['dayTypeDescription']), strtolower($_POST['character'])
                                                                                            ) !== false
                                                                                    ) {
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $dayTypeArray[$i]['dayTypeDescription']
                                                                                        );
                                                                                    } else {
                                                                                        echo $dayTypeArray[$i]['dayTypeDescription'];
                                                                                    }
                                                            ...
                                                            <td>
                                                                <div align="center">
                                                                    <?php
                                                                    if (isset($dayTypeArray[$i]['executeBy'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos(
                                                                                                $dayTypeArray[$i]['staffName'], $_POST['query']
                                                                                        ) !== false
                                                                                ) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $dayTypeArray[$i]['staffName']
                                                                                    );
                                                                                } else {
                                                                                    echo $dayTypeArray[$i]['staffName'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen(
                                                                                                $_POST['character']
                                                                                    ...
                                                                                    if (strpos(
                                                                                                    $dayTypeArray[$i]['staffName'], $_POST['character']
                                                                                            ) !== false
                                                                                    ) {
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $dayTypeArray[$i]['staffName']
                                                                                        );
                                                                                    } else {
                                                                                        echo $dayTypeArray[$i]['staffName'];
                                                                                    }
                                                            ...
                                                            <?php
                                                            if (isset($dayTypeArray[$i]['executeTime'])) {
                                                                $valueArray = $dayTypeArray[$i]['executeTime'];
                                                            ...
                                                            <td>
																<?php if($dayTypeArray [$i]['dayTypeCode'] !='UNBL') { ?>
                                                                <input style="display:none;" type="checkbox" name="dayTypeId[]"
                                                                       value="<?php echo $dayTypeArray[$i]['dayTypeId']; ?>">
                                                                <input <?php echo $checked; ?> type="checkbox" name="isDelete[]"
                                                                                               value="<?php echo $dayTypeArray[$i]['isDelete']; ?>">
																<?php } else { ?>&nbsp;<?php } ?>
                                                            </td>
                                                        </tr>
                                                        <?php
                        ...
                        <div class="col-xs-3 col-sm-3 col-md-3 pagination">
                            <div class="pull-right">
                                <button class="delete btn btn-warning" type="button" 
                                        ...
                                        echo $dayType->getControllerPath();
                                        ?>', '<?php echo $dayType->getViewPath(); ?>', '<?php echo $securityToken; ?>');">
...
if ((isset($_POST['method']) == 'new' || isset($_POST['method']) == 'read') && $_POST['type'] == 'form') {
            ...
            <div class="row">
                <div class="col-xs-12 col-sm-12 col-md-12">
                    <div class="panel panel-info">
                        ...
                        <div class="panel-body">
                            <input type="hidden" name="dayTypeId" id="dayTypeId" value="<?php
                            if (isset($_POST['dayTypeId'])) {
                                echo $_POST['dayTypeId'];
                            }
                            ?>">

                            <div class="row">
                                <div class="col-xs-12 col-sm-12 col-md-12">
                                    <div class="form-group" id="dayTypeDescriptionForm">
                                        <div class="col-xs-12 col-sm-12 col-md-12">
                                            <textarea class="form-control" rows="5" name="dayTypeDescription"
                                                      id="dayTypeDescription"><?php
                                                          if (isset($dayTypeArray[0]['dayTypeDescription'])) {
                                                              echo htmlentities($dayTypeArray[0]['dayTypeDescription']);
                                                          }
                                                          ?> </textarea> <span class="help-block" id="dayTypeDescriptionHelpMe"></span>
                                        </div>
                                    </div>
                                </div>
                            </div>
    ...
    <?php if ($leafAccess['leafAccessCreateValue'] == 1) { ?>
                            ...
                            if (e.which === 78 && e.which === 18  && e.shiftKey) {
                                ...
                                newRecord(<?php echo $leafId; ?>, '<?php echo $dayType->getControllerPath(); ?>', '<?php echo $dayType->getViewPath(); ?>', '<?php echo $securityToken; ?>', 1);
    ...
    <?php if ($leafAccess['leafAccessUpdateValue'] == 1) { ?>
                            ...
                            if (e.which === 83 && e.which === 18  && e.shiftKey) {
                                ...
                                updateRecord(<?php echo $leafId; ?>, '<?php echo $dayType->getControllerPath(); ?>', '<?php echo $dayType->getViewPath(); ?>', '<?php echo $securityToken; ?>', 1, <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
    ...
    <?php if ($leafAccess['leafAccessDeleteValue'] == 1) { ?>
                            ...
                            if (e.which === 88 && e.which === 18 && e.shiftKey) {
                                ...
                                deleteRecord(<?php echo $leafId; ?>, '<?php echo $dayType->getControllerPath(); ?>', '<?php echo $dayType->getViewPath(); ?>', '<?php echo $securityToken; ?>', <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
    ...
    <?php } ?>
                        switch (e.keyCode) {
                            case 37:
                                previousRecord(<?php echo $leafId; ?>, '<?php echo $dayType->getControllerPath(); ?>', '<?php echo $dayType->getViewPath(); ?>', '<?php echo $securityToken; ?>', <?php echo $leafAccess['leafAccessUpdateValue']; ?>, <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
                                
                                return false;
                                break;
                            case 39:
                                nextRecord(<?php echo $leafId; ?>, '<?php echo $dayType->getControllerPath(); ?>', '<?php echo $dayType->getViewPath(); ?>', '<?php echo $securityToken; ?>', <?php echo $leafAccess['leafAccessUpdateValue']; ?>, <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
                    ...
                    $(document).ready(function() {
                        ...
                        validateMeNumeric('dayTypeId');
    <?php if ($_POST['method'] == "new") { ?>
                            $('#resetRecordButton')
                                    .removeClass().addClass('btn btn-default');
        <?php if ($leafAccess['leafAccessCreateValue'] == 1) { ?>
                                $('#newRecordButton1')
                                        .removeClass().addClass('btn btn-success').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $dayType->getControllerPath(); ?>','<?php echo $dayType->getViewPath(); ?>','<?php echo $securityToken; ?>',1)");
                                $('#newRecordButton2')
                                        .removeClass().addClass('btn dropdown-toggle btn-success');
                                $('#newRecordButton3').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $dayType->getControllerPath(); ?>','<?php echo $dayType->getViewPath(); ?>','<?php echo $securityToken; ?>',1)");
                                $('#newRecordButton4').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $dayType->getControllerPath(); ?>','<?php echo $dayType->getViewPath(); ?>','<?php echo $securityToken; ?>',2)");
                                $('#newRecordButton5').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $dayType->getControllerPath(); ?>','<?php echo $dayType->getViewPath(); ?>','<?php echo $securityToken; ?>',3)");
                                $('#newRecordButton6').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $dayType->getControllerPath(); ?>','<?php echo $dayType->getViewPath(); ?>','<?php echo $securityToken; ?>',4)");
                                $('#newRecordButton7').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $dayType->getControllerPath(); ?>','<?php echo $dayType->getViewPath(); ?>','<?php echo $securityToken; ?>',5)");
        <?php } else { ?>
                                $('#newRecordButton1')
        ...
        if ($_POST['dayTypeId']) {
            ...
            <?php if ($leafAccess['leafAccessUpdateValue'] == 1) { ?>
                                    $('#updateRecordButton1')
                                            .removeClass().addClass('btn btn-info').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $dayType->getControllerPath(); ?>','<?php echo $dayType->getViewPath(); ?>','<?php echo $securityToken; ?>',1,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                                    $('#updateRecordButton2')
                                            .removeClass().addClass('btn dropdown-toggle btn-info');
                                    $('#updateRecordButton3').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $dayType->getControllerPath(); ?>','<?php echo $dayType->getViewPath(); ?>','<?php echo $securityToken; ?>',1,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                                    $('#updateRecordButton4').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $dayType->getControllerPath(); ?>','<?php echo $dayType->getViewPath(); ?>','<?php echo $securityToken; ?>',2,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                                    $('#updateRecordButton5').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $dayType->getControllerPath(); ?>','<?php echo $dayType->getViewPath(); ?>','<?php echo $securityToken; ?>',3,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
            <?php } else { ?>
                                    $('#updateRecordButton1')
                                            .removeClass().addClass(' btn btn-info disabled');
                                    $('#updateRecordButton2')
            ...
            <?php if ($leafAccess['leafAccessDeleteValue'] == 1) { ?>
                                    $('#deleteRecordButton')
                                            ...
                                            .attr('onClick', "deleteRecord(<?php echo $leafId; ?>,'<?php echo $dayType->getControllerPath(); ?>','<?php echo $dayType->getViewPath(); ?>','<?php echo $securityToken; ?>',<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
            <?php } else { ?>
                                    $('#deleteRecordButton')
                                            .removeClass().addClass('btn btn-danger disabled')
                                            .attr('onClick', '');
    ...
    </div></form><script type="text/javascript" src="./v3/system/common/javascript/dayType.js"></script>
<hr>
