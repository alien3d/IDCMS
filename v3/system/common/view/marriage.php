...
require_once($newFakeDocumentRoot . "v3/system/common/controller/marriageController.php");
...
$translator->setCurrentTable('marriage');
...
if (isset($_POST)) {
    if (isset($_POST['method'])) {
        $marriage = new \Core\System\Common\Marriage\Controller\MarriageClass();
        define('LIMIT', 10);
        ...
        if (isset($_POST ['query'])) {
            $marriage->setFieldQuery($_POST ['query']);
        }
        if (isset($_POST ['filter'])) {
            $marriage->setGridQuery($_POST ['filter']);
        }
        if (isset($_POST ['character'])) {
            $marriage->setCharacterQuery($_POST['character']);
        }
        if (isset($_POST ['dateRangeStart'])) {
            $marriage->setDateRangeStartQuery($_POST['dateRangeStart']);
            //explode the data to get day,month,year
            $start = explode('-', $_POST ['dateRangeStart']);
            $marriage->setStartDay($start[2]);
            $marriage->setStartMonth($start[1]);
            $marriage->setStartYear($start[0]);
        }
        if (isset($_POST ['dateRangeEnd']) && (strlen($_POST['dateRangeEnd']) > 0)) {
            $marriage->setDateRangeEndQuery($_POST['dateRangeEnd']);
            //explode the data to get day,month,year
            $start = explode('-', $_POST ['dateRangeEnd']);
            $marriage->setEndDay($start[2]);
            $marriage->setEndMonth($start[1]);
            $marriage->setEndYear($start[0]);
        }
        if (isset($_POST ['dateRangeType'])) {
            $marriage->setDateRangeTypeQuery($_POST['dateRangeType']);
        }
        if (isset($_POST ['dateRangeExtraType'])) {
            $marriage->setDateRangeExtraTypeQuery($_POST['dateRangeExtraType']);
        }
        $marriage->setServiceOutput('html');
        $marriage->setLeafId($leafId);
        $marriage->execute();
        if ($_POST['method'] == 'read') {
            $marriage->setStart($offset);
            $marriage->setLimit($limit); // normal system don't like paging..
            $marriage->setPageOutput('html');
            $marriageArray = $marriage->read();
            if (isset($marriageArray [0]['firstRecord'])) {
                $firstRecord = $marriageArray [0]['firstRecord'];
            }
            if (isset($marriageArray [0]['nextRecord'])) {
                $nextRecord = $marriageArray [0]['nextRecord'];
            }
            if (isset($marriageArray [0]['previousRecord'])) {
                $previousRecord = $marriageArray [0]['previousRecord'];
            }
            if (isset($marriageArray [0]['lastRecord'])) {
                $lastRecord = $marriageArray [0]['lastRecord'];
                $endRecord = $marriageArray [0]['lastRecord'];
            }
            $navigation = new \Core\Paging\HtmlPaging();
            $navigation->setLeafId($leafId);
            $navigation->setViewPath($marriage->getViewPath());
            ...
            if (isset($marriageArray [0]['total'])) {
                $total = $marriageArray [0]['total'];
            } else {
                $total = 0;
            }
                        ...
                        <div class="btn-group">
                            ...
                            <ul class="dropdown-menu" style="text-align:left">
                                <li>
                                    ...
                                    echo $marriage->getControllerPath();
                ...
                <div id="leftViewportDetail" class="col-xs-3 col-sm-3 col-md-3">
                    <div class="panel panel-default">
                        <div class="panel-body">
                                ...
                                <label for="queryWidget"></label><div class="input-group"><input type="text" class="form-control" name="queryWidget"
                                ...
                                </table><div class="input-group"><input type="text" class="form-control" name="dateRangeStart"
                                                                        id="dateRangeStart" value="<?php
                                                                        if (isset($_POST['dateRangeStart'])) {
                                ...
                                <div class="input-group"><input type="text" class="form-control" name="dateRangeEnd"
                                                                id="dateRangeEnd" value="<?php
                                                                if (isset($_POST['dateRangeEnd'])) {
                        ...
                        <div class="modal-dialog">
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                        <input type="hidden" name="marriageIdPreview" id="marriageIdPreview">

                                        <div class="form-group" id="marriageCodeDiv">
                                            <label for="marriageCodePreview"><?php echo $leafTranslation['marriageCodeLabel']; ?></label>
                                            <input type="text" class="form-control" name="marriageCodePreview" id="marriageCodePreview">
                                        </div>
                                        <div class="form-group" id="marriageDescriptionDiv">
                                            <label  for="marriageDescriptionPreview"><?php echo $leafTranslation['marriageDescriptionLabel']; ?></label>
                                            <input type="text" class="form-control" name="marriageDescriptionPreview"
                                                   id="marriageDescriptionPreview">
                                        </div>
                                </div>
                        ...
                        <div class="col-xs-12 col-sm-12 col-md-12">
                            <div class="panel panel-default">
                                <table class="table table table-bordered table-striped table-condensed table-hover smart-form has-tickbox" id="tableData">
                                    ...
                                    <th width="75px"><?php echo ucwords($leafTranslation['marriageCodeLabel']); ?></th>
                                    <th><?php echo ucwords($leafTranslation['marriageDescriptionLabel']); ?></th>
                                    <th width="100px">
                                    ...
                                    <tbody id="tableBody">
                                        ...
                                        if ($_POST['method'] == 'read' && $_POST['type'] == 'list' && $_POST['detail'] == 'body') {
                                            if (is_array($marriageArray)) {
                                                ...
                                                if ($totalRecord > 0) {
                                                    ...
                                                    for ($i = 0; $i < $totalRecord; $i++) {
                                                        ...
                                                        ?>
                                                        <tr <?php
                                                        if ($marriageArray[$i]['isDelete'] == 1) {
                                                            echo "class=\"danger\"";
                                                        } else {
                                                            if ($marriageArray[$i]['isDraft'] == 1) {
                                                                echo "class=\"warning\"";
                                                            }
                                                        }
                                                        ?>>
                                                            ...
                                                            <td align="center">
															    <?php if($marriageArray [$i]['marriageCode'] !='UNBL') { ?>
                                                                <div class="btn-group" align="center">
                                                                    ...
                                                                    ?>', '<?php
                                                                    echo rawurlencode(
                                                                            $marriageArray [$i]['marriageCode']
                                                                    );
                                                                    ?>', '<?php echo rawurlencode($marriageArray [$i]['marriageDescription']); ?>');">
                                                            ...
                                                            <td>
                                                                <div class="pull-left">
                                                                    <?php
                                                                    if (isset($marriageArray[$i]['marriageCode'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos(
                                                                                                strtolower($marriageArray[$i]['marriageCode']), strtolower($_POST['query'])
                                                                                        ) !== false
                                                                                ) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $marriageArray[$i]['marriageCode']
                                                                                    );
                                                                                } else {
                                                                                    echo $marriageArray[$i]['marriageCode'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                    if (strpos(
                                                                                                    strtolower($marriageArray[$i]['marriageCode']), strtolower($_POST['character'])
                                                                                            ) !== false
                                                                                    ) {
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $marriageArray[$i]['marriageCode']
                                                                                        );
                                                                                    } else {
                                                                                        echo $marriageArray[$i]['marriageCode'];
                                                                                    }
                                                            ...
                                                            <td>
                                                                <div class="pull-left">
                                                                    <?php
                                                                    if (isset($marriageArray[$i]['marriageDescription'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos(
                                                                                                strtolower($marriageArray[$i]['marriageDescription']), strtolower($_POST['query'])
                                                                                        ) !== false
                                                                                ) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $marriageArray[$i]['marriageDescription']
                                                                                    );
                                                                                } else {
                                                                                    echo $marriageArray[$i]['marriageDescription'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                    if (strpos(
                                                                                                    strtolower($marriageArray[$i]['marriageDescription']), strtolower($_POST['character'])
                                                                                            ) !== false
                                                                                    ) {
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $marriageArray[$i]['marriageDescription']
                                                                                        );
                                                                                    } else {
                                                                                        echo $marriageArray[$i]['marriageDescription'];
                                                                                    }
                                                            ...
                                                            <td>
                                                                <div class="pull-left">
                                                                    <?php
                                                                    if (isset($marriageArray[$i]['executeBy'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos($marriageArray[$i]['staffName'], $_POST['query']) !== false) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $marriageArray[$i]['staffName']
                                                                                    );
                                                                                } else {
                                                                                    echo $marriageArray[$i]['staffName'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                    if (strpos($marriageArray[$i]['staffName'], $_POST['character']) !== false) {
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $marriageArray[$i]['staffName']
                                                                                        );
                                                                                    } else {
                                                                                        echo $marriageArray[$i]['staffName'];
                                                                                    }
                                                            ...
                                                            <?php
                                                            if (isset($marriageArray[$i]['executeTime'])) {
                                                                $valueArray = $marriageArray[$i]['executeTime'];
                                                            ...
                                                            <td>
															<?php if($marriageArray [$i]['marriageCode'] !='UNBL') { ?>
                                                                <input style="display:none;" type="checkbox" name="marriageId[]"
                                                                       value="<?php echo $marriageArray[$i]['marriageId']; ?>">
                                                                <input <?php echo $checked; ?> type="checkbox" name="isDelete[]"
                                                                                               value="<?php echo $marriageArray[$i]['isDelete']; ?>">
																<?php } else { ?>&nbsp;<?php } ?>
                                                            </td>
                                                        </tr>
                                                        <?php
                                            ...
                                            ?>
                                            <tr>
                                                <td colspan="7" vAlign="top" align="center"><?php $marriage->exceptionMessage($t['loadFailureLabel']); ?></td>
                                            </tr>
                                            <?php
                                        }
                                        ?>
                        ...
                        <div class="col-xs-3 col-sm-3 col-md-3 pagination">
                            <div class="pull-right">
                                <button class="delete btn btn-warning" type="button" 
                                        ...
                                        echo $marriage->getControllerPath();
                                        ?>', '<?php echo $marriage->getViewPath(); ?>', '<?php echo $securityToken; ?>');">
...
if ((isset($_POST['method']) == 'new' || isset($_POST['method']) == 'read') && $_POST['type'] == 'form') {
            ...
            <div class="row">
                <div class="col-xs-12 col-sm-12 col-md-12">
                    <div class="panel panel-info">
                        ...
                        <div class="panel-body">
                            <input type="hidden" name="marriageId" id="marriageId" value="<?php
                            if (isset($_POST['marriageId'])) {
                                echo $_POST['marriageId'];
                            }
                            ?>">

                            <div class="row">
                                <div class="col-xs-12 col-sm-12 col-md-12">
                                    <div class="col-xs-6 col-sm-6 col-md-6 form-group" id="marriageCodeForm">
                                        <label class="control-label col-xs-4 col-sm-4 col-md-4" for="marriageCode"><strong><?php
                                                echo ucfirst(
                                                        $leafTranslation['marriageCodeLabel']
                                                );
                                                ?></strong></label>

                                        <div class="col-xs-8 col-sm-8 col-md-8">
                                            <input type="text" class="form-control" name="marriageCode" id="marriageCode"
                                                   value="<?php
                                                   if (isset($marriageArray) && is_array($marriageArray)) {
                                                       if (isset($marriageArray[0]['marriageCode'])) {
                                                           echo htmlentities($marriageArray[0]['marriageCode']);
                                                       }
                                                   }
                                                   ?>" maxlength="16"> <span class="help-block" id="marriageCodeHelpMe"></span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-xs-12 col-sm-12 col-md-12">
                                    <div class="col-xs-12 col-sm-12 col-md-12">
                                        <textarea class="form-control" rows="5" name="marriageDescription"
                                                  id="marriageDescription"><?php
                                                      if (isset($marriageArray[0]['marriageDescription'])) {
                                                          echo htmlentities($marriageArray[0]['marriageDescription']);
                                                      }
                                                      ?></textarea> <span class="help-block" id="marriageDescriptionHelpMe"></span>
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    <?php if ($leafAccess['leafAccessCreateValue'] == 1) { ?>
                    newRecord(<?php echo $leafId; ?>, '<?php echo $marriage->getControllerPath(); ?>', '<?php echo $marriage->getViewPath(); ?>', '<?php echo $securityToken; ?>', 1);
    <?php } ?>
                return false;
            }
            // shift+s save event
    ...
    <?php if ($leafAccess['leafAccessUpdateValue'] == 1) { ?>
                    updateRecord(<?php echo $leafId; ?>, '<?php echo $marriage->getControllerPath(); ?>', '<?php echo $marriage->getViewPath(); ?>', '<?php echo $securityToken; ?>', 1, <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
    <?php } ?>
                return false;
            }
    ...
    <?php if ($leafAccess['leafAccessDeleteValue'] == 1) { ?>
                    deleteRecord(<?php echo $leafId; ?>, '<?php echo $marriage->getControllerPath(); ?>', '<?php echo $marriage->getViewPath(); ?>', '<?php echo $securityToken; ?>', <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
    ...
    <?php } ?>
            ...
            switch (e.keyCode) {
                case 37:
                    previousRecord(<?php echo $leafId; ?>, '<?php echo $marriage->getControllerPath(); ?>', '<?php echo $marriage->getViewPath(); ?>', '<?php echo $securityToken; ?>', <?php echo $leafAccess['leafAccessUpdateValue']; ?>, <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
                    
                    return false;
                    break;
                case 39:
                    nextRecord(<?php echo $leafId; ?>, '<?php echo $marriage->getControllerPath(); ?>', '<?php echo $marriage->getViewPath(); ?>', '<?php echo $securityToken; ?>', <?php echo $leafAccess['leafAccessUpdateValue']; ?>, <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
        ...
        $(document).ready(function() {
            ...
            validateMeAlphaNumeric('marriageCode');
    <?php if ($_POST['method'] == "new") { ?>
                $('#resetRecordButton')
                        .removeClass().addClass('btn btn-default');
        <?php if ($leafAccess['leafAccessCreateValue'] == 1) { ?>
                    $('#newRecordButton1')
                            .removeClass().addClass('btn btn-success').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $marriage->getControllerPath(); ?>','<?php echo $marriage->getViewPath(); ?>','<?php echo $securityToken; ?>',1)");
                    $('#newRecordButton2')
                            .removeClass().addClass('btn dropdown-toggle btn-success');
                    $('#newRecordButton3').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $marriage->getControllerPath(); ?>','<?php echo $marriage->getViewPath(); ?>','<?php echo $securityToken; ?>',1)");
                    $('#newRecordButton4').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $marriage->getControllerPath(); ?>','<?php echo $marriage->getViewPath(); ?>','<?php echo $securityToken; ?>',2)");
                    $('#newRecordButton5').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $marriage->getControllerPath(); ?>','<?php echo $marriage->getViewPath(); ?>','<?php echo $securityToken; ?>',3)");
                    $('#newRecordButton6').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $marriage->getControllerPath(); ?>','<?php echo $marriage->getViewPath(); ?>','<?php echo $securityToken; ?>',4)");
                    $('#newRecordButton7').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $marriage->getControllerPath(); ?>','<?php echo $marriage->getViewPath(); ?>','<?php echo $securityToken; ?>',5)");
        <?php } else { ?>
                    $('#newRecordButton1')
        ...
        if ($_POST['marriageId']) {
            ...
            <?php if ($leafAccess['leafAccessUpdateValue'] == 1) { ?>
                        $('#updateRecordButton1')
                                .removeClass().addClass('btn btn-info').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $marriage->getControllerPath(); ?>','<?php echo $marriage->getViewPath(); ?>','<?php echo $securityToken; ?>',1,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                        $('#updateRecordButton2')
                                .removeClass().addClass('btn dropdown-toggle btn-info');
                        $('#updateRecordButton3').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $marriage->getControllerPath(); ?>','<?php echo $marriage->getViewPath(); ?>','<?php echo $securityToken; ?>',1,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                        $('#updateRecordButton4').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $marriage->getControllerPath(); ?>','<?php echo $marriage->getViewPath(); ?>','<?php echo $securityToken; ?>',2,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                        $('#updateRecordButton5').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $marriage->getControllerPath(); ?>','<?php echo $marriage->getViewPath(); ?>','<?php echo $securityToken; ?>',3,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
            <?php } else { ?>
                        $('#updateRecordButton1')
                                .removeClass().addClass(' btn btn-info disabled');
                        $('#updateRecordButton2')
            ...
            <?php if ($leafAccess['leafAccessDeleteValue'] == 1) { ?>
                        $('#deleteRecordButton')
                                ...
                                .attr('onClick', "deleteRecord(<?php echo $leafId; ?>,'<?php echo $marriage->getControllerPath(); ?>','<?php echo $marriage->getViewPath(); ?>','<?php echo $securityToken; ?>',<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
            <?php } else { ?>
                        $('#deleteRecordButton')
                                .removeClass().addClass('btn btn-danger disabled')
                                .attr('onClick', '');
...
<?php } ?>
<script type="text/javascript" src="./v3/system/common/javascript/marriage.js"></script>
<hr>
