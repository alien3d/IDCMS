...
require_once($newFakeDocumentRoot . "v3/system/common/controller/genderController.php");
...
$translator->setCurrentTable('gender');
...
if (isset($_POST)) {
    if (isset($_POST['method'])) {
        $gender = new \Core\System\Common\Gender\Controller\GenderClass();
        define('LIMIT', 10);
        ...
        if (isset($_POST ['query'])) {
            $gender->setFieldQuery($_POST ['query']);
        }
        if (isset($_POST ['filter'])) {
            $gender->setGridQuery($_POST ['filter']);
        }
        if (isset($_POST ['character'])) {
            $gender->setCharacterQuery($_POST['character']);
        }
        if (isset($_POST ['dateRangeStart'])) {
            $gender->setDateRangeStartQuery($_POST['dateRangeStart']);
            //explode the data to get day,month,year
            $start = explode('-', $_POST ['dateRangeStart']);
            $gender->setStartDay($start[2]);
            $gender->setStartMonth($start[1]);
            $gender->setStartYear($start[0]);
        }
        if (isset($_POST ['dateRangeEnd']) && (strlen($_POST['dateRangeEnd']) > 0)) {
            $gender->setDateRangeEndQuery($_POST['dateRangeEnd']);
            //explode the data to get day,month,year
            $start = explode('-', $_POST ['dateRangeEnd']);
            $gender->setEndDay($start[2]);
            $gender->setEndMonth($start[1]);
            $gender->setEndYear($start[0]);
        }
        if (isset($_POST ['dateRangeType'])) {
            $gender->setDateRangeTypeQuery($_POST['dateRangeType']);
        }
        if (isset($_POST ['dateRangeExtraType'])) {
            $gender->setDateRangeExtraTypeQuery($_POST['dateRangeExtraType']);
        }
        $gender->setServiceOutput('html');
        $gender->setLeafId($leafId);
        $gender->execute();
        if ($_POST['method'] == 'read') {
            $gender->setStart($offset);
            $gender->setLimit($limit); // normal system don't like paging..
            $gender->setPageOutput('html');
            $genderArray = $gender->read();
            if (isset($genderArray [0]['firstRecord'])) {
                $firstRecord = $genderArray [0]['firstRecord'];
            }
            if (isset($genderArray [0]['nextRecord'])) {
                $nextRecord = $genderArray [0]['nextRecord'];
            }
            if (isset($genderArray [0]['previousRecord'])) {
                $previousRecord = $genderArray [0]['previousRecord'];
            }
            if (isset($genderArray [0]['lastRecord'])) {
                $lastRecord = $genderArray [0]['lastRecord'];
                $endRecord = $genderArray [0]['lastRecord'];
            }
            $navigation = new \Core\Paging\HtmlPaging();
            $navigation->setLeafId($leafId);
            $navigation->setViewPath($gender->getViewPath());
            ...
            if (isset($genderArray [0]['total'])) {
                $total = $genderArray [0]['total'];
            } else {
                $total = 0;
            }
                        ...
                        <div class="panel-body">
                            <form class="form-forizontal">
                                ...
                                <div class="input-group"><input type="text" class="form-control" name="dateRangeEnd"  id="dateRangeEnd" value="<?php
                                                                ...
                                                                ?>" onClick="topPage(175);"><span class="input-group-addon"><img src="./images/icons/calendar.png" id="endDateImage"></span></div><br>
                        ...
                        <div class="modal-dialog">
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <input type="hidden" name="genderIdPreview" id="genderIdPreview">

                                    <div class="form-group" id="genderCodeDiv">
                                        <label for="genderCodePreview"
                                               class="control-label col-xs-4 col-sm-4 col-md-4"><?php echo $leafTranslation['genderCodeLabel']; ?></label>

                                        <div class="col-xs-8 col-sm-8 col-md-8">
                                            <input class="form-control" type="text" name="genderCodePreview"
                                                   id="genderCodePreview">
                                        </div>
                                    </div>
                                    <div class="form-group" id="genderIconDiv">
                                        <label for="genderIconPreview"
                                               class="control-label col-xs-4 col-sm-4 col-md-4"><?php echo $leafTranslation['genderIconLabel']; ?></label>

                                        <div class="col-xs-8 col-sm-8 col-md-8">
                                            <input class="form-control" type="text" name="genderIconPreview"
                                                   id="genderIconPreview">
                                        </div>
                                    </div>
                                    <div class="form-group" id="genderDescriptionDiv">
                                        <label for="genderDescriptionPreview"
                                               class="control-label col-xs-4 col-sm-4 col-md-4"><?php echo $leafTranslation['genderDescriptionLabel']; ?></label>

                                        <div class="col-xs-8 col-sm-8 col-md-8">
                                            <input class="form-control" type="text" name="genderDescriptionPreview"
                                                   id="genderDescriptionPreview">
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div class="col-xs-12 col-sm-12 col-md-12">
                            <div class="panel panel-default">
                                <table class="table table table-bordered table-striped table-condensed table-hover smart-form has-tickbox" id="tableData">
                                    ...
                                    <th width="100px">
                                    <div align="center"><?php echo ucwords($leafTranslation['genderCodeLabel']); ?></div>
                                    </th>
                                    <th width="125px">
                                    <div align="center"><?php echo ucwords($leafTranslation['genderIconLabel']); ?></div>
                                    </th>
                                    <th><?php echo ucwords($leafTranslation['genderDescriptionLabel']); ?></th>
                                    <th width="100px">
                                            ...
                                            if (is_array($genderArray)) {
                                                ...
                                                if ($totalRecord > 0) {
                                                    ...
                                                    for ($i = 0; $i < $totalRecord; $i++) {
                                                        ...
                                                        ?>
                                                        <tr <?php
                                                        if ($genderArray[$i]['isDelete'] == 1) {
                                                            echo "class=\"danger\"";
                                                        } else {
                                                            if ($genderArray[$i]['isDraft'] == 1) {
                                                                echo "class=\"warning\"";
                                                            }
                                                        }
                                                        ?>>
                                                            ...
                                                            <td align="center">
															   <?php if($genderArray [$i]['genderCode'] !='UNBL') { ?>
                                                                <div class="btn-group" align="center">
                                                                    ...
                                                                    ?>', '<?php
                                                                    echo rawurlencode(
                                                                            $genderArray [$i]['genderCode']
                                                                    );
                                                                    ?>', '<?php
                                                                    echo rawurlencode(
                                                                            $genderArray [$i]['genderIcon']
                                                                    );
                                                                    ?>', '<?php
                                                                    echo rawurlencode(
                                                                            $genderArray [$i]['genderDescription']
                                                                    );
                                                                    ?>');">
                                                            ...
                                                            <td>
                                                                <div align="center">
                                                                    <?php
                                                                    if (isset($genderArray[$i]['genderCode'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos(
                                                                                                strtolower($genderArray[$i]['genderCode']), strtolower($_POST['query'])
                                                                                        ) !== false
                                                                                ) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $genderArray[$i]['genderCode']
                                                                                    );
                                                                                } else {
                                                                                    echo $genderArray[$i]['genderCode'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                    if (strpos(
                                                                                                    strtolower($genderArray[$i]['genderCode']), strtolower($_POST['character'])
                                                                                            ) !== false
                                                                                    ) {
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $genderArray[$i]['genderCode']
                                                                                        );
                                                                                    } else {
                                                                                        echo $genderArray[$i]['genderCode'];
                                                                                    }
                                                            ...
                                                            <td>
                                                                <div class="pull-left">
                                                                    <?php
                                                                    if (file_exists(
                                                                                    $newFakeDocumentRoot . "images/icons/" . $genderArray[$i]['genderIcon']
                                                                            )
                                                                    ) {
                                                                        echo "<img src=\"./images/icons/" . $genderArray[$i]['genderIcon'] . "\">";
                                                                    }
                                                                    if (isset($genderArray[$i]['genderIcon'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos(
                                                                                                strtolower($genderArray[$i]['genderIcon']), strtolower($_POST['query'])
                                                                                        ) !== false
                                                                                ) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $genderArray[$i]['genderIcon']
                                                                                    );
                                                                                } else {
                                                                                    echo $genderArray[$i]['genderIcon'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                    if (strpos(
                                                                                                    strtolower($genderArray[$i]['genderIcon']), strtolower($_POST['character'])
                                                                                            ) !== false
                                                                                    ) {
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $genderArray[$i]['genderIcon']
                                                                                        );
                                                                                    } else {
                                                                                        echo $genderArray[$i]['genderIcon'];
                                                                                    }
                                                            ...
                                                            <td>
                                                                <div class="pull-left">
                                                                    <?php
                                                                    if (isset($genderArray[$i]['genderDescription'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos(
                                                                                                strtolower($genderArray[$i]['genderDescription']), strtolower($_POST['query'])
                                                                                        ) !== false
                                                                                ) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $genderArray[$i]['genderDescription']
                                                                                    );
                                                                                } else {
                                                                                    echo $genderArray[$i]['genderDescription'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                    if (strpos(
                                                                                                    strtolower($genderArray[$i]['genderDescription']), strtolower($_POST['character'])
                                                                                            ) !== false
                                                                                    ) {
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $genderArray[$i]['genderDescription']
                                                                                        );
                                                                                    } else {
                                                                                        echo $genderArray[$i]['genderDescription'];
                                                                                    }
                                                            ...
                                                            <td>
                                                                <div align="center">
                                                                    <?php
                                                                    if (isset($genderArray[$i]['executeBy'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos($genderArray[$i]['staffName'], $_POST['query']) !== false) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $genderArray[$i]['staffName']
                                                                                    );
                                                                                } else {
                                                                                    echo $genderArray[$i]['staffName'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                    if (strpos(
                                                                                                    $genderArray[$i]['staffName'], $_POST['character']
                                                                                            ) !== false
                                                                                    ) {
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $genderArray[$i]['staffName']
                                                                                        );
                                                                                    } else {
                                                                                        echo $genderArray[$i]['staffName'];
                                                                                    }
                                                            ...
                                                            <?php
                                                            if (isset($genderArray[$i]['executeTime'])) {
                                                                $valueArray = $genderArray[$i]['executeTime'];
                                                            ...
                                                            <td>
															<?php if($genderArray [$i]['genderCode'] !='UNBL') { ?>
                                                                <input style="display:none;" type="checkbox" name="genderId[]"
                                                                       value="<?php echo $genderArray[$i]['genderId']; ?>">
                                                                <input <?php echo $checked; ?> type="checkbox" name="isDelete[]"
                                                                                               value="<?php echo $genderArray[$i]['isDelete']; ?>">
																<?php } else { ?>&nbsp; <?php } ?>
                                                            </td>
                                                        </tr>
                                                        <?php
                        ...
                        <div class="col-xs-3 col-sm-3 col-md-3 pagination">
                            <div class="pull-right">
                                <button class="delete btn btn-warning" type="button" 
                                        ...
                                        echo $gender->getControllerPath();
                                        ?>', '<?php echo $gender->getViewPath(); ?>', '<?php echo $securityToken; ?>');">
...
if ((isset($_POST['method']) == 'new' || isset($_POST['method']) == 'read') && $_POST['type'] == 'form') {
            ...
            <div class="row">
                <div class="col-xs-12 col-sm-12 col-md-12">
                    <div class="panel panel-info">
                        ...
                        <div class="panel-body">
                            <input type="hidden" name="genderId" id="genderId" value="<?php
                            ...
                            <div class="row">
                                <div class="col-xs-12 col-sm-12 col-md-12">
                                    <div class="col-xs-6 col-sm-6 col-md-6 form-group" id="genderCodeForm">
                                        <label class="control-label col-xs-4 col-sm-4 col-md-4" for="genderCode"><strong><?php
                                                echo ucfirst(
                                                        $leafTranslation['genderCodeLabel']
                                                );
                                                ?></strong></label>

                                        <div class="col-xs-8 col-sm-8 col-md-8">
                                            <div class="input-group">
                                                <input type="text" class="form-control" name="genderCode" id="genderCode"
                                                       value="<?php
                                                       if (isset($genderArray) && is_array($genderArray)) {
                                                           if (isset($genderArray[0]['genderCode'])) {
                                                               echo htmlentities($genderArray[0]['genderCode']);
                                                           }
                                                       }
                                                       ?>" maxlength="16"><span class="input-group-addon"><img
                                                        src="./images/icons/document-code.png"></span>
                                            </div>
                                            <span class="help-block" id="genderCodeHelpMe"></span>
                                        </div>
                                    </div>
                                    <div class="col-xs-6 col-sm-6 col-md-6 form-group" id="genderIconForm">
                                        <label class="control-label col-xs-4 col-sm-4 col-md-4" for="genderIcon"><strong><?php
                                                echo ucfirst(
                                                        $leafTranslation['genderIconLabel']
                                                );
                                                ?></strong></label>

                                        <div class="col-xs-8 col-sm-8 col-md-8">
                                            <div class="input-group">
                                                <input type="text" class="form-control" name="genderIcon" id="genderIcon"
                                                       value="<?php
                                                       if (isset($genderArray) && is_array($genderArray)) {
                                                           if (isset($genderArray[0]['genderIcon'])) {
                                                               echo htmlentities($genderArray[0]['genderIcon']);
                                                           }
                                                       }
                                                       ?>"><span class="input-group-addon"><img
                                                        src="./images/icons/application-icon.png"></span>
                                            </div>
                                            <span class="help-block" id="genderIconHelpMe"></span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-xs-12 col-sm-12 col-md-12">
                                    <div class="form-group" id="genderDescriptionForm">
                                        <div class="col-xs-12 col-sm-12 col-md-12">
                                            <textarea class="form-control" name="genderDescription"
                                                      id="genderDescription" rows="5"> <?php
                                                          if (isset($genderArray[0]['genderDescription'])) {
                                                              echo htmlentities($genderArray[0]['genderDescription']);
                                                          }
                                                          ?></textarea> <span class="help-block" id="genderDescriptionHelpMe"></span>
                                        </div>
                                    </div>
                                </div>
                            </div>
    ...
    <?php if ($leafAccess['leafAccessCreateValue'] == 1) { ?>
                            newRecord(<?php echo $leafId; ?>, '<?php echo $gender->getControllerPath(); ?>', '<?php echo $gender->getViewPath(); ?>', '<?php echo $securityToken; ?>', 1);
    <?php } ?>
                        return false;
                    }
                    // shift+s save event
    ...
    <?php if ($leafAccess['leafAccessUpdateValue'] == 1) { ?>
                            updateRecord(<?php echo $leafId; ?>, '<?php echo $gender->getControllerPath(); ?>', '<?php echo $gender->getViewPath(); ?>', '<?php echo $securityToken; ?>', 1, <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
    <?php } ?>
                        return false;
                    }
    ...
    <?php if ($leafAccess['leafAccessDeleteValue'] == 1) { ?>
                            deleteRecord(<?php echo $leafId; ?>, '<?php echo $gender->getControllerPath(); ?>', '<?php echo $gender->getViewPath(); ?>', '<?php echo $securityToken; ?>', <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
    ...
    <?php } ?>
                    ...
                    switch (e.keyCode) {
                        case 37:
                            previousRecord(<?php echo $leafId; ?>, '<?php echo $gender->getControllerPath(); ?>', '<?php echo $gender->getViewPath(); ?>', '<?php echo $securityToken; ?>', <?php echo $leafAccess['leafAccessUpdateValue']; ?>, <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
                            
                            return false;
                            break;
                        case 39:
                            nextRecord(<?php echo $leafId; ?>, '<?php echo $gender->getControllerPath(); ?>', '<?php echo $gender->getViewPath(); ?>', '<?php echo $securityToken; ?>', <?php echo $leafAccess['leafAccessUpdateValue']; ?>, <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
                ...
                $(document).ready(function() {
                    ...
                    validateMeAlphaNumeric('genderCode');
                    validateMeAlphaNumeric('genderIcon');
    <?php if ($_POST['method'] == "new") { ?>
        ...
        <?php if ($leafAccess['leafAccessCreateValue'] == 1) { ?>
                            ...
                            $('#newRecordButton3').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $gender->getControllerPath(); ?>','<?php echo $gender->getViewPath(); ?>','<?php echo $securityToken; ?>',1)");
                            $('#newRecordButton4').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $gender->getControllerPath(); ?>','<?php echo $gender->getViewPath(); ?>','<?php echo $securityToken; ?>',2)");
                            $('#newRecordButton5').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $gender->getControllerPath(); ?>','<?php echo $gender->getViewPath(); ?>','<?php echo $securityToken; ?>',3)");
                            $('#newRecordButton6').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $gender->getControllerPath(); ?>','<?php echo $gender->getViewPath(); ?>','<?php echo $securityToken; ?>',4)");
                            $('#newRecordButton7').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $gender->getControllerPath(); ?>','<?php echo $gender->getViewPath(); ?>','<?php echo $securityToken; ?>',5)");
        <?php } else { ?>
                            $('#newRecordButton1')
        ...
        if ($_POST['genderId']) {
            ...
            <?php if ($leafAccess['leafAccessUpdateValue'] == 1) { ?>
                                $('#updateRecordButton1')
                                        .removeClass().addClass('btn btn-info').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $gender->getControllerPath(); ?>','<?php echo $gender->getViewPath(); ?>','<?php echo $securityToken; ?>',1,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                                $('#updateRecordButton2')
                                        .removeClass().addClass('btn dropdown-toggle btn-info');
                                $('#updateRecordButton3').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $gender->getControllerPath(); ?>','<?php echo $gender->getViewPath(); ?>','<?php echo $securityToken; ?>',1,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                                $('#updateRecordButton4').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $gender->getControllerPath(); ?>','<?php echo $gender->getViewPath(); ?>','<?php echo $securityToken; ?>',2,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                                $('#updateRecordButton5').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $gender->getControllerPath(); ?>','<?php echo $gender->getViewPath(); ?>','<?php echo $securityToken; ?>',3,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
            <?php } else { ?>
                                $('#updateRecordButton1')
                                        .removeClass().addClass(' btn btn-info disabled');
                                $('#updateRecordButton2')
            ...
            <?php if ($leafAccess['leafAccessDeleteValue'] == 1) { ?>
                                $('#deleteRecordButton')
                                        ...
                                        .attr('onClick', "deleteRecord(<?php echo $leafId; ?>,'<?php echo $gender->getControllerPath(); ?>','<?php echo $gender->getViewPath(); ?>','<?php echo $securityToken; ?>',<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
            <?php } else { ?>
                                $('#deleteRecordButton')
                                        .removeClass().addClass('btn btn-danger disabled')
                                        .attr('onClick', '');
...
<?php } ?>
<script type="text/javascript" src="./v3/system/common/javascript/gender.js"></script>
<hr>
