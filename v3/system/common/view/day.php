...
require_once($newFakeDocumentRoot . "v3/system/common/controller/dayController.php");
...
$translator->setCurrentTable('day');
...
if (isset($_POST)) {
    if (isset($_POST['method'])) {
        ...
        if (isset($_POST ['query'])) {
            $day->setFieldQuery($_POST ['query']);
        }
        if (isset($_POST ['filter'])) {
            $day->setGridQuery($_POST ['filter']);
        }
        if (isset($_POST ['character'])) {
            $day->setCharacterQuery($_POST['character']);
        }
        if (isset($_POST ['dateRangeStart'])) {
            $day->setDateRangeStartQuery($_POST['dateRangeStart']);
            //explode the data to get day,month,year
            $start = explode('-', $_POST ['dateRangeStart']);
            $day->setStartDay($start[2]);
            $day->setStartMonth($start[1]);
            $day->setStartYear($start[0]);
        }
        if (isset($_POST ['dateRangeEnd']) && (strlen($_POST['dateRangeEnd']) > 0)) {
            $day->setDateRangeEndQuery($_POST['dateRangeEnd']);
            //explode the data to get day,month,year
            $start = explode('-', $_POST ['dateRangeEnd']);
            $day->setEndDay($start[2]);
            $day->setEndMonth($start[1]);
            $day->setEndYear($start[0]);
        }
        if (isset($_POST ['dateRangeType'])) {
            $day->setDateRangeTypeQuery($_POST['dateRangeType']);
        }
        if (isset($_POST ['dateRangeExtraType'])) {
            $day->setDateRangeExtraTypeQuery($_POST['dateRangeExtraType']);
        }
        $day->setServiceOutput('html');
        $day->setLeafId($leafId);
        $day->execute();
        if ($_POST['method'] == 'read') {
            ...
            $dayArray = $day->read();
            if (isset($dayArray [0]['firstRecord'])) {
                $firstRecord = $dayArray [0]['firstRecord'];
            }
            if (isset($dayArray [0]['nextRecord'])) {
                $nextRecord = $dayArray [0]['nextRecord'];
            }
            if (isset($dayArray [0]['previousRecord'])) {
                $previousRecord = $dayArray [0]['previousRecord'];
            }
            if (isset($dayArray [0]['lastRecord'])) {
                $lastRecord = $dayArray [0]['lastRecord'];
                $endRecord = $dayArray [0]['lastRecord'];
            }
            $navigation = new \Core\Paging\HtmlPaging();
            $navigation->setLeafId($leafId);
            $navigation->setViewPath($day->getViewPath());
            ...
            if (isset($dayArray [0]['total'])) {
                $total = $dayArray [0]['total'];
            } else {
                $total = 0;
            }
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <form class="form-horizontal">
                                        <input type="hidden" name="dayIdPreview" id="dayIdPreview">

                                        <div class="form-group" id="dayNameDiv">
                                            <label for="dayNamePreview"
                                                   class="control-label col-xs-4 col-sm-4 col-md-4"><?php echo $leafTranslation['dayNameLabel']; ?></label>

                                            <div class="col-xs-8 col-sm-8 col-md-8">
                                                <input class="form-control" type="text" name="dayNamePreview"
                                                       id="dayNamePreview">
                                            </div>
                                        </div>
                                    </form>
                        ...
                        <div class="col-xs-12 col-sm-12 col-md-12">
                            <div class="panel panel-default">
                                <table class="table table table-bordered table-striped table-condensed table-hover smart-form has-tickbox" id="tableData">
                                    ...
                                    <th><?php echo ucwords($leafTranslation['dayNameLabel']); ?></th>
                                    <th width="100px">
                                    ...
                                    <tbody id="tableBody">
                                        ...
                                        if ($_POST['method'] == 'read' && $_POST['type'] == 'list' && $_POST['detail'] == 'body') {
                                            if (is_array($dayArray)) {
                                                $totalRecord = intval(count($dayArray));
                                                if ($totalRecord > 0) {
                                                    $counter = 0;
                                                    for ($i = 0; $i < $totalRecord; $i++) {
                                                        ...
                                                        ?>
                                                        <tr <?php
                                                        if ($dayArray[$i]['isDelete'] == 1) {
                                                            echo "class=\"danger\"";
                                                        } else {
                                                            if ($dayArray[$i]['isDraft'] == 1) {
                                                                echo "class=\"warning\"";
                                                            }
                                                        }
                                                        ?>>
                                                            ...
                                                            <td align="center">
																<?php if($dayArray [$i]['dayCode'] !='UNBL') { ?>
                                                            ...
                                                            <td>
                                                                <div class="pull-left">
                                                                    <?php
                                                                    if (isset($dayArray[$i]['dayName'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if ($_POST['query']) {
                                                                                if (strpos(
                                                                                                strtolower($dayArray[$i]['dayName']), strtolower($_POST['query'])
                                                                                        ) !== false
                                                                                ) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $dayArray[$i]['dayName']
                                                                                    );
                                                                                } else {
                                                                                    echo $dayArray[$i]['dayName'];
                                                                                }
                                                                            } else {
                                                                                if ($_POST['character']) {
                                                                                    if (strpos(
                                                                                                    strtolower($dayArray[$i]['dayName']), strtolower($_POST['character'])
                                                                                            ) !== false
                                                                                    ) {
                                                                                        echo str_replace(
                                                                                                $_POST['query'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $dayArray[$i]['dayName']
                                                                                        );
                                                                                    } else {
                                                                                        echo $dayArray[$i]['dayName'];
                                                                                    }
                                                            ...
                                                            <td>
                                                                <div align="center">
                                                                    <?php
                                                                    if (isset($dayArray[$i]['executeBy'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if ($_POST['query']) {
                                                                                if (strpos(
                                                                                                $dayArray[$i]['staffName'], $_POST['query']
                                                                                        ) !== false
                                                                                ) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $dayArray[$i]['staffName']
                                                                                    );
                                                                                } else {
                                                                                    echo $dayArray[$i]['staffName'];
                                                                                }
                                                                            } else {
                                                                                if ($_POST['character']) {
                                                                                    if (strpos(
                                                                                                    $dayArray[$i]['staffName'], $_POST['character']
                                                                                            ) !== false
                                                                                    ) {
                                                                                        echo str_replace(
                                                                                                $_POST['query'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $dayArray[$i]['staffName']
                                                                                        );
                                                                                    } else {
                                                                                        echo $dayArray[$i]['staffName'];
                                                                                    }
                                                            ...
                                                            <?php
                                                            if (isset($dayArray[$i]['executeTime'])) {
                                                                $valueArray = $dayArray[$i]['executeTime'];
                                                                if ($dateConvert->checkDateTime($valueArray)) {
                                                                    ...
                                                                    $daya = $valueDataFirst[2];
                                                                    $valueDataSecond = explode(':', $valueArraySecond);
                                                                    $hour = $valueDataSecond[0];
                                                                    $minute = $valueDataSecond[1];
                                                                    $second = $valueDataSecond[2];
                                                                    $value = date(
                                                                            $systemFormat['systemSettingDateFormat'] . " " . $systemFormat['systemSettingTimeFormat'], mktime($hour, $minute, $second, $month, $daya, $year)
                                                                    );
                                                                } else {
                                                                    $value = null;
                                                                }
                                                            ...
                                                            <td>
																	<?php if($dayArray [$i]['dayCode'] !='UNBL') { ?>
                                                                <input style="display:none;" type="checkbox" name="dayId[]"
                                                                       value="<?php echo $dayArray[$i]['dayId']; ?>">
                                                                <input <?php echo $checked; ?> type="checkbox" name="isDelete[]"
                                                                                               value="<?php echo $dayArray[$i]['isDelete']; ?>">
																<?php } else { ?>&nbsp;<?php } ?>
                                                            </td>
                                                        </tr>
                                                        <?php
                        ...
                        <div class="col-xs-3 col-sm-3 col-md-3 pagination">
                            <div class="pull-right">
                                <button class="delete btn btn-warning" type="button" 
                                        ...
                                        echo $day->getControllerPath();
                                        ?>', '<?php echo $day->getViewPath(); ?>', '<?php echo $securityToken; ?>');">
...
if ((isset($_POST['method']) == 'new' || isset($_POST['method']) == 'read') && $_POST['type'] == 'form') {
            ...
            <div class="row">
                <div class="col-xs-12 col-sm-12 col-md-12">
                    <div class="panel panel-info">
                        ...
                        <div class="panel-body">
                            <input type="hidden" name="dayId" id="dayId" value="<?php
                            ...
                            <div class="row">
                                <div class="col-xs-12 col-sm-12 col-md-12">
                                    <div class="col-xs-6 col-sm-6 col-md-6 form-group" id="dayNameForm">
                                        <label for="dayName" class="control-label col-xs-4 col-sm-4 col-md-4"><strong><?php
                                                echo ucfirst(
                                                        $leafTranslation['dayNameLabel']
                                                );
                                                ?></strong></label>

                                        <div class="col-xs-8 col-sm-8 col-md-8">
                                            <input class="form-control" type="text" name="dayName" id="dayName" value="<?php
                                            if (isset($dayArray) && is_array($dayArray)) {
                                                if (isset($dayArray[0]['dayName'])) {
                                                    echo htmlentities($dayArray[0]['dayName']);
                                                }
                                            }
                                            ?>"> <span class="help-block" id="dayNameHelpMe"></span>
                                        </div>
                                    </div>
                                </div>
                            </div>
    ...
    <?php if ($leafAccess['leafAccessCreateValue'] == 1) { ?>
                            ...
                            if (e.which === 78 && e.which === 18  && e.shiftKey) {
                                ...
                                newRecord(<?php echo $leafId; ?>, '<?php echo $day->getControllerPath(); ?>', '<?php echo $day->getViewPath(); ?>', '<?php echo $securityToken; ?>', 1);
    ...
    <?php if ($leafAccess['leafAccessUpdateValue'] == 1) { ?>
                            ...
                            if (e.which === 83 && e.which === 18  && e.shiftKey) {
                                ...
                                updateRecord(<?php echo $leafId; ?>, '<?php echo $day->getControllerPath(); ?>', '<?php echo $day->getViewPath(); ?>', '<?php echo $securityToken; ?>', 1, <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
    ...
    <?php if ($leafAccess['leafAccessDeleteValue'] == 1) { ?>
                            ...
                            if (e.which === 88 && e.which === 18 && e.shiftKey) {
                                ...
                                deleteRecord(<?php echo $leafId; ?>, '<?php echo $day->getControllerPath(); ?>', '<?php echo $day->getViewPath(); ?>', '<?php echo $securityToken; ?>', <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
    ...
    <?php } ?>
                        switch (e.keyCode) {
                            case 37:
                                previousRecord(<?php echo $leafId; ?>, '<?php echo $day->getControllerPath(); ?>', '<?php echo $day->getViewPath(); ?>', '<?php echo $securityToken; ?>', <?php echo $leafAccess['leafAccessUpdateValue']; ?>, <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
                                
                                return false;
                                break;
                            case 39:
                                nextRecord(<?php echo $leafId; ?>, '<?php echo $day->getControllerPath(); ?>', '<?php echo $day->getViewPath(); ?>', '<?php echo $securityToken; ?>', <?php echo $leafAccess['leafAccessUpdateValue']; ?>, <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
                    ...
                    $(document).ready(function() {
                        ...
                        validateMeNumeric('dayId');
                        validateMeAlphaNumeric('dayName');
    <?php if ($_POST['method'] == "new") { ?>
                            $('#resetRecordButton')
                                    .removeClass().addClass('btn btn-default');
        <?php if ($leafAccess['leafAccessCreateValue'] == 1) { ?>
                                $('#newRecordButton1')
                                        .removeClass().addClass('btn btn-success').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $day->getControllerPath(); ?>','<?php echo $day->getViewPath(); ?>','<?php echo $securityToken; ?>',1)");
                                $('#newRecordButton2')
                                        .removeClass().addClass('btn dropdown-toggle btn-success');
                                $('#newRecordButton3').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $day->getControllerPath(); ?>','<?php echo $day->getViewPath(); ?>','<?php echo $securityToken; ?>',1)");
                                $('#newRecordButton4').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $day->getControllerPath(); ?>','<?php echo $day->getViewPath(); ?>','<?php echo $securityToken; ?>',2)");
                                $('#newRecordButton5').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $day->getControllerPath(); ?>','<?php echo $day->getViewPath(); ?>','<?php echo $securityToken; ?>',3)");
                                $('#newRecordButton6').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $day->getControllerPath(); ?>','<?php echo $day->getViewPath(); ?>','<?php echo $securityToken; ?>',4)");
                                $('#newRecordButton7').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $day->getControllerPath(); ?>','<?php echo $day->getViewPath(); ?>','<?php echo $securityToken; ?>',5)");
        <?php } else { ?>
                                $('#newRecordButton1')
        ...
        if ($_POST['dayId']) {
            ...
            <?php if ($leafAccess['leafAccessUpdateValue'] == 1) { ?>
                                    $('#updateRecordButton1')
                                            .removeClass().addClass('btn btn-info').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $day->getControllerPath(); ?>','<?php echo $day->getViewPath(); ?>','<?php echo $securityToken; ?>',1,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                                    $('#updateRecordButton2')
                                            .removeClass().addClass('btn dropdown-toggle btn-info');
                                    $('#updateRecordButton3').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $day->getControllerPath(); ?>','<?php echo $day->getViewPath(); ?>','<?php echo $securityToken; ?>',1,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                                    $('#updateRecordButton4').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $day->getControllerPath(); ?>','<?php echo $day->getViewPath(); ?>','<?php echo $securityToken; ?>',2,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                                    $('#updateRecordButton5').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $day->getControllerPath(); ?>','<?php echo $day->getViewPath(); ?>','<?php echo $securityToken; ?>',3,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
            <?php } else { ?>
                                    $('#updateRecordButton1')
                                            .removeClass().addClass(' btn btn-info disabled');
                                    $('#updateRecordButton2')
            ...
            <?php if ($leafAccess['leafAccessDeleteValue'] == 1) { ?>
                                    $('#deleteRecordButton')
                                            ...
                                            .attr('onClick', "deleteRecord(<?php echo $leafId; ?>,'<?php echo $day->getControllerPath(); ?>','<?php echo $day->getViewPath(); ?>','<?php echo $securityToken; ?>',<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
            <?php } else { ?>
                                    $('#deleteRecordButton')
                                            .removeClass().addClass('btn btn-danger disabled')
                                            .attr('onClick', '');
    ...
    </div></form><script type="text/javascript" src="./v3/system/common/javascript/day.js"></script>
<hr>
