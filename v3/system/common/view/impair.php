...
require_once($newFakeDocumentRoot . "v3/system/common/controller/impairController.php");
...
$translator->setCurrentTable('impair');
...
if (isset($_POST)) {
    if (isset($_POST['method'])) {
        $impair = new \Core\System\Common\Impair\Controller\ImpairClass();
        define('LIMIT', 10);
        ...
        if (isset($_POST ['query'])) {
            $impair->setFieldQuery($_POST ['query']);
        }
        if (isset($_POST ['filter'])) {
            $impair->setGridQuery($_POST ['filter']);
        }
        if (isset($_POST ['character'])) {
            $impair->setCharacterQuery($_POST['character']);
        }
        if (isset($_POST ['dateRangeStart'])) {
            $impair->setDateRangeStartQuery($_POST['dateRangeStart']);
            //explode the data to get day,month,year
            $start = explode('-', $_POST ['dateRangeStart']);
            $impair->setStartDay($start[2]);
            $impair->setStartMonth($start[1]);
            $impair->setStartYear($start[0]);
        }
        if (isset($_POST ['dateRangeEnd']) && (strlen($_POST['dateRangeEnd']) > 0)) {
            $impair->setDateRangeEndQuery($_POST['dateRangeEnd']);
            //explode the data to get day,month,year
            $start = explode('-', $_POST ['dateRangeEnd']);
            $impair->setEndDay($start[2]);
            $impair->setEndMonth($start[1]);
            $impair->setEndYear($start[0]);
        }
        if (isset($_POST ['dateRangeType'])) {
            $impair->setDateRangeTypeQuery($_POST['dateRangeType']);
        }
        if (isset($_POST ['dateRangeExtraType'])) {
            $impair->setDateRangeExtraTypeQuery($_POST['dateRangeExtraType']);
        }
        $impair->setServiceOutput('html');
        $impair->setLeafId($leafId);
        $impair->execute();
        if ($_POST['method'] == 'read') {
            $impair->setStart($offset);
            $impair->setLimit($limit); // normal system don't like paging..
            $impair->setPageOutput('html');
            $impairArray = $impair->read();
            if (isset($impairArray [0]['firstRecord'])) {
                $firstRecord = $impairArray [0]['firstRecord'];
            }
            if (isset($impairArray [0]['nextRecord'])) {
                $nextRecord = $impairArray [0]['nextRecord'];
            }
            if (isset($impairArray [0]['previousRecord'])) {
                $previousRecord = $impairArray [0]['previousRecord'];
            }
            if (isset($impairArray [0]['lastRecord'])) {
                $lastRecord = $impairArray [0]['lastRecord'];
                $endRecord = $impairArray [0]['lastRecord'];
            }
            $navigation = new \Core\Paging\HtmlPaging();
            $navigation->setLeafId($leafId);
            $navigation->setViewPath($impair->getViewPath());
            ...
            if (isset($impairArray [0]['total'])) {
                $total = $impairArray [0]['total'];
            } else {
                $total = 0;
            }
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <form class="form-horizontal">
                                        <input type="hidden" name="impairIdPreview" id="impairIdPreview">

                                        <div class="form-group" id="impairCodeDiv">
                                            <label for="impairCodePreview"
                                                   class="control-label col-xs-4 col-sm-4 col-md-4"><?php echo $leafTranslation['impairCodeLabel']; ?></label>

                                            <div class="col-xs-8 col-sm-8 col-md-8">
                                                <input class="form-control" type="text" name="impairCodePreview"
                                                       id="impairCodePreview">
                                            </div>
                                        </div>
                                        <div class="form-group" id="impairDescriptionDiv">
                                            <label for="impairDescriptionPreview"
                                                   class="control-label col-xs-4 col-sm-4 col-md-4"><?php echo $leafTranslation['impairDescriptionLabel']; ?></label>

                                            <div class="col-xs-8 col-sm-8 col-md-8">
                                                <input class="form-control" type="text" name="impairDescriptionPreview"
                                                       id="impairDescriptionPreview">
                                            </div>
                                        </div>
                                    </form>
                                </div>
                        ...
                        <div class="col-xs-12 col-sm-12 col-md-12">
                            <div class="panel panel-default">
                                <table class="table table table-bordered table-striped table-condensed table-hover smart-form has-tickbox" id="tableData">
                                    ...
                                    <th width="100px">
                                    <div align="center"><?php echo ucwords($leafTranslation['impairCodeLabel']); ?></div>
                                    </th>
                                    <th><?php echo ucwords($leafTranslation['impairDescriptionLabel']); ?></th>
                                    <th width="100px">
                                    ...
                                    <tbody id="tableBody">
                                        ...
                                        if ($_POST['method'] == 'read' && $_POST['type'] == 'list' && $_POST['detail'] == 'body') {
                                            if (is_array($impairArray)) {
                                                $totalRecord = intval(count($impairArray));
                                                if ($totalRecord > 0) {
                                                    $counter = 0;
                                                    for ($i = 0; $i < $totalRecord; $i++) {
                                                        ...
                                                        ?>
                                                        <tr <?php
                                                        if ($impairArray[$i]['isDelete'] == 1) {
                                                            echo "class=\"danger\"";
                                                        } else {
                                                            if ($impairArray[$i]['isDraft'] == 1) {
                                                                echo "class=\"warning\"";
                                                            }
                                                        }
                                                        ?>>
                                                            ...
                                                            <td align="center">
																		<?php if($impairArray [$i]['impairCode'] !='UNBL') { ?>
                                                                <div class="btn-group" align="center">
                                                                    ...
                                                                    ?>', '<?php
                                                                    echo rawurlencode(
                                                                            $impairArray [$i]['impairCode']
                                                                    );
                                                                    ?>', '<?php
                                                                    echo rawurlencode(
                                                                            $impairArray [$i]['impairDescription']
                                                                    );
                                                                    ?>');">
                                                            ...
                                                            <td>
                                                                <div align="center">
                                                                    <?php
                                                                    if (isset($impairArray[$i]['impairCode'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos(
                                                                                                strtolower($impairArray[$i]['impairCode']), strtolower($_POST['query'])
                                                                                        ) !== false
                                                                                ) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $impairArray[$i]['impairCode']
                                                                                    );
                                                                                } else {
                                                                                    echo $impairArray[$i]['impairCode'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                    if (strpos(
                                                                                                    strtolower($impairArray[$i]['impairCode']), strtolower($_POST['character'])
                                                                                            ) !== false
                                                                                    ) {
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $impairArray[$i]['impairCode']
                                                                                        );
                                                                                    } else {
                                                                                        echo $impairArray[$i]['impairCode'];
                                                                                    }
                                                            ...
                                                            <td>
                                                                <div class="pull-left">
                                                                    <?php
                                                                    if (isset($impairArray[$i]['impairDescription'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos(
                                                                                                strtolower($impairArray[$i]['impairDescription']), strtolower($_POST['query'])
                                                                                        ) !== false
                                                                                ) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $impairArray[$i]['impairDescription']
                                                                                    );
                                                                                } else {
                                                                                    echo $impairArray[$i]['impairDescription'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                    if (strpos(
                                                                                                    strtolower($impairArray[$i]['impairDescription']), strtolower($_POST['character'])
                                                                                            ) !== false
                                                                                    ) {
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $impairArray[$i]['impairDescription']
                                                                                        );
                                                                                    } else {
                                                                                        echo $impairArray[$i]['impairDescription'];
                                                                                    }
                                                            ...
                                                            <td>
                                                                <div align="center">
                                                                    <?php
                                                                    if (isset($impairArray[$i]['executeBy'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos($impairArray[$i]['staffName'], $_POST['query']) !== false) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $impairArray[$i]['staffName']
                                                                                    );
                                                                                } else {
                                                                                    echo $impairArray[$i]['staffName'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                    if (strpos(
                                                                                                    $impairArray[$i]['staffName'], $_POST['character']
                                                                                            ) !== false
                                                                                    ) {
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $impairArray[$i]['staffName']
                                                                                        );
                                                                                    } else {
                                                                                        echo $impairArray[$i]['staffName'];
                                                                                    }
                                                            ...
                                                            <?php
                                                            if (isset($impairArray[$i]['executeTime'])) {
                                                                $valueArray = $impairArray[$i]['executeTime'];
                                                            ...
                                                            <td>
															<?php if($impairArray [$i]['impairCode'] !='UNBL') { ?>
                                                                <input style="display:none;" type="checkbox" name="impairId[]"
                                                                       value="<?php echo $impairArray[$i]['impairId']; ?>">
                                                                <input <?php echo $checked; ?> type="checkbox" name="isDelete[]"
                                                                                               value="<?php echo $impairArray[$i]['isDelete']; ?>">
														  <?php } else { ?>&nbsp; <?php } ?>
                                                            </td>
                                                        </tr>
                                                        <?php
                        ...
                        <div class="col-xs-3 col-sm-3 col-md-3 pagination">
                            <div class="pull-right">
                                <button class="delete btn btn-warning" type="button" 
                                        ...
                                        echo $impair->getControllerPath();
                                        ?>', '<?php echo $impair->getViewPath(); ?>', '<?php echo $securityToken; ?>');">
...
if ((isset($_POST['method']) == 'new' || isset($_POST['method']) == 'read') && $_POST['type'] == 'form') {
            ...
            <div class="row">
                <div class="col-xs-12 col-sm-12 col-md-12">
                    <div class="panel panel-info">
                        ...
                        <div class="panel-body">
                            <input type="hidden" name="impairId" id="impairId" value="<?php
                            if (isset($_POST['impairId'])) {
                                echo $_POST['impairId'];
                            }
                            ?>">

                            <div class="form-group" id="impairCodeForm">
                                <label for="impairCode" class="control-label col-xs-2 col-sm-2 col-md-2"><strong><?php
                                        echo ucfirst(
                                                $leafTranslation['impairCodeLabel']
                                        );
                                        ?></strong></label>

                                <div class="control-label col-xs-4 col-sm-4 col-md-4">
                                    <div class="input-group">
                                        <input class="form-control" type="text" name="impairCode" id="impairCode" value="<?php
                                        if (isset($impairArray) && is_array($impairArray)) {
                                            if (isset($impairArray[0]['impairCode'])) {
                                                echo htmlentities($impairArray[0]['impairCode']);
                                            }
                                        }
                                        ?>" maxlength="16"> <span class="input-group-addon"><img src="./images/icons/document-code.png"></span>
                                    </div>
                                    <span class="help-block" id="impairCodeHelpMe"></span>
                                </div>
                            </div>
                            <div class="form-group" id="impairDescriptionForm">
                                <div class="col-xs-12 col-sm-12 col-md-12">
                                    <textarea class="form-control" name="impairDescription" id="impairDescription" rows="5"><?php
                                        if (isset($impairArray) && is_array($impairArray)) {
                                            if (isset($impairArray[0]['impairDescription'])) {
                                                echo htmlentities($impairArray[0]['impairDescription']);
                                            }
                                        }
                                        ?></textarea> <span class="help-block" id="impairDescriptionHelpMe"></span>
                                </div>
                            </div>
                        </div>
    ...
    <?php if ($leafAccess['leafAccessCreateValue'] == 1) { ?>
                                newRecord(<?php echo $leafId; ?>, '<?php echo $impair->getControllerPath(); ?>', '<?php echo $impair->getViewPath(); ?>', '<?php echo $securityToken; ?>', 1);
    <?php } ?>
                            return false;
                        }
                        // shift+s save event
    ...
    <?php if ($leafAccess['leafAccessUpdateValue'] == 1) { ?>
                                updateRecord(<?php echo $leafId; ?>, '<?php echo $impair->getControllerPath(); ?>', '<?php echo $impair->getViewPath(); ?>', '<?php echo $securityToken; ?>', 1, <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
    <?php } ?>
                            return false;
                        }
    ...
    <?php if ($leafAccess['leafAccessDeleteValue'] == 1) { ?>
                                deleteRecord(<?php echo $leafId; ?>, '<?php echo $impair->getControllerPath(); ?>', '<?php echo $impair->getViewPath(); ?>', '<?php echo $securityToken; ?>', <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
    ...
    <?php } ?>
                        ...
                        switch (e.keyCode) {
                            case 37:
                                previousRecord(<?php echo $leafId; ?>, '<?php echo $impair->getControllerPath(); ?>', '<?php echo $impair->getViewPath(); ?>', '<?php echo $securityToken; ?>', <?php echo $leafAccess['leafAccessUpdateValue']; ?>, <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
                                
                                return false;
                                break;
                            case 39:
                                nextRecord(<?php echo $leafId; ?>, '<?php echo $impair->getControllerPath(); ?>', '<?php echo $impair->getViewPath(); ?>', '<?php echo $securityToken; ?>', <?php echo $leafAccess['leafAccessUpdateValue']; ?>, <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
                    ...
                    $(document).ready(function() {
                        ...
                        validateMeNumeric('impairId');
                        validateMeAlphaNumeric('impairCode');
    <?php if ($_POST['method'] == "new") { ?>
                            $('#resetRecordButton')
                                    .removeClass().addClass('btn btn-default');
        <?php if ($leafAccess['leafAccessCreateValue'] == 1) { ?>
                                $('#newRecordButton1')
                                        .removeClass().addClass('btn btn-success').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $impair->getControllerPath(); ?>','<?php echo $impair->getViewPath(); ?>','<?php echo $securityToken; ?>',1)");
                                $('#newRecordButton2')
                                        .removeClass().addClass('btn dropdown-toggle btn-success');
                                $('#newRecordButton3').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $impair->getControllerPath(); ?>','<?php echo $impair->getViewPath(); ?>','<?php echo $securityToken; ?>',1)");
                                $('#newRecordButton4').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $impair->getControllerPath(); ?>','<?php echo $impair->getViewPath(); ?>','<?php echo $securityToken; ?>',2)");
                                $('#newRecordButton5').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $impair->getControllerPath(); ?>','<?php echo $impair->getViewPath(); ?>','<?php echo $securityToken; ?>',3)");
                                $('#newRecordButton6').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $impair->getControllerPath(); ?>','<?php echo $impair->getViewPath(); ?>','<?php echo $securityToken; ?>',4)");
                                $('#newRecordButton7').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $impair->getControllerPath(); ?>','<?php echo $impair->getViewPath(); ?>','<?php echo $securityToken; ?>',5)");
        <?php } else { ?>
                                $('#newRecordButton1')
        ...
        if ($_POST['impairId']) {
            ...
            <?php if ($leafAccess['leafAccessUpdateValue'] == 1) { ?>
                                    $('#updateRecordButton1')
                                            .removeClass().addClass('btn btn-info').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $impair->getControllerPath(); ?>','<?php echo $impair->getViewPath(); ?>','<?php echo $securityToken; ?>',1,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                                    $('#updateRecordButton2')
                                            .removeClass().addClass('btn dropdown-toggle btn-info');
                                    $('#updateRecordButton3').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $impair->getControllerPath(); ?>','<?php echo $impair->getViewPath(); ?>','<?php echo $securityToken; ?>',1,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                                    $('#updateRecordButton4').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $impair->getControllerPath(); ?>','<?php echo $impair->getViewPath(); ?>','<?php echo $securityToken; ?>',2,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                                    $('#updateRecordButton5').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $impair->getControllerPath(); ?>','<?php echo $impair->getViewPath(); ?>','<?php echo $securityToken; ?>',3,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
            <?php } else { ?>
                                    $('#updateRecordButton1')
                                            .removeClass().addClass(' btn btn-info disabled');
                                    $('#updateRecordButton2')
            ...
            <?php if ($leafAccess['leafAccessDeleteValue'] == 1) { ?>
                                    $('#deleteRecordButton')
                                            ...
                                            .attr('onClick', "deleteRecord(<?php echo $leafId; ?>,'<?php echo $impair->getControllerPath(); ?>','<?php echo $impair->getViewPath(); ?>','<?php echo $securityToken; ?>',<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
            <?php } else { ?>
                                    $('#deleteRecordButton')
                                            .removeClass().addClass('btn btn-danger disabled')
                                            .attr('onClick', '');
    ...
    ?>
            ...
            <?php } ?>
     <script type="text/javascript" src="./v3/system/common/javascript/impair.js"></script>
<hr>
