function checkDuplicate(leafId, page, securityToken) {
    ...
    $.ajax({type: 'GET', url: page, data: {languageCode: $languageCode.val(), method: 'duplicate', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
            ...
            if (success === true) {
                ...
                $("#languageForm").removeClass().addClass("form-group has-error");
            }
            if ($infoPanel.is(':hidden')) {
                $infoPanel.show();
            }
...
function showFormUpdate(leafId, url, urlList, securityToken, languageId, updateAccess, deleteAccess) {
    sleep(500);
    $('a[rel=tooltip]').tooltip('hide');
    $.ajax({type: 'POST', url: urlList, data: {method: 'read', type: 'form', languageId: languageId, securityToken: securityToken, leafId: leafId}, beforeSend: function() {
            var smileyRoll = './images/icons/smiley-roll.png';
            var $infoPanel = $('#infoPanel');
            $infoPanel.html('').empty().html("<span class='label label-warning'><img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
            if ($infoPanel.is(':hidden')) {
...
function showModalDelete(languageId, languageCode, languageDescription, languageIcon, isGoogle, isBing, isSpeakLite, isWebServices, isImportant) {
    $('#languageIdPreview').val('').val(decodeURIComponent(languageId));
    $('#languageCodePreview').val('').val(decodeURIComponent(languageCode));
    $('#languageDescriptionPreview').val('').val(decodeURIComponent(languageDescription));
    $('#languageIconPreview').val('').val(decodeURIComponent(languageIcon));
    $('#isGooglePreview').val('').val(decodeURIComponent(isGoogle));
    $('#isBingPreview').val('').val(decodeURIComponent(isBing));
    $('#isSpeakLitePreview').val('').val(decodeURIComponent(isSpeakLite));
    $('#isWebServicesPreview').val('').val(decodeURIComponent(isWebServices));
    $('#isImportantPreview').val('').val(decodeURIComponent(isImportant));
    showMeModal('deletePreview', 1);
}
function deleteGridRecord(leafId, url, urlList, securityToken) {
    $.ajax({type: 'POST', url: url, data: {method: 'delete', output: 'json', languageId: $('#languageIdPreview').val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
            var smileyRoll = './images/icons/smiley-roll.png';
            var $infoPanel = $('#infoPanel');
            $infoPanel.html('').empty().html("<span class='label label-warning'><img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
            if ($infoPanel.is(':hidden')) {
...
function deleteGridRecordCheckbox(leafId, url, urlList, securityToken) {
    var stringText = '';
    var counter = 0;
    $('input:checkbox[name="languageId[]"]').each(function() {
        stringText = stringText + "&languageId[]=" + $(this).val();
    });
...
function reportRequest(leafId, url, securityToken, mode) {
    $.ajax({type: 'GET', url: url, data: {method: 'report', mode: mode, securityToken: securityToken, leafId: leafId}, beforeSend: function() {
            ...
            var message = data.message;
            if (success === true) {
                var path="./v3/system/common/document/" + folder + "/" + filename;
...
function newRecord(leafId, url, urlList, securityToken, type, createAccess, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        if (type === 1) {
            if ($('#languageCode').val().length === 0) {
                $('#languageCodeHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['languageCodeLabel'] + " </span>");
                $('#languageCodeForm').removeClass().addClass('form-group has-error');
                $('#languageCode').focus();
                return false;
            }
            if ($('#languageDescription').val().length === 0) {
                $('#languageDescriptionHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['languageDescriptionLabel'] + " </span>");
                $('#languageDescriptionForm').removeClass().addClass('form-group has-error');
                $('#languageDescription').focus();
                return false;
            }
            if ($('#languageIcon').val().length === 0) {
                $('#languageIconHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['languageIconLabel'] + " </span>");
                $('#languageIconForm').removeClass().addClass('form-group has-error');
                $('#languageIcon').focus();
                return false;
            }
            if ($('#isGoogle').val().length === 0) {
                $('#isGoogleHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['isGoogleLabel'] + " </span>");
                $('#isGoogleForm').removeClass().addClass('form-group has-error');
                $('#isGoogle').focus();
                return false;
            }
            if ($('#isBing').val().length === 0) {
                $('#isBingHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['isBingLabel'] + " </span>");
                $('#isBingForm').removeClass().addClass('form-group has-error');
                $('#isBing').focus();
                return false;
            }
            if ($('#isSpeakLite').val().length === 0) {
                $('#isSpeakLiteHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['isSpeakLiteLabel'] + " </span>");
                $('#isSpeakLiteForm').removeClass().addClass('form-group has-error');
                $('#isSpeakLite').focus();
                return false;
            }
            if ($('#isWebServices').val().length === 0) {
                $('#isWebServicesHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['isWebServicesLabel'] + " </span>");
                $('#isWebServicesForm').removeClass().addClass('form-group has-error');
                $('#isWebServices').focus();
                return false;
            }
            if ($('#isImportant').val().length === 0) {
                $('#isImportantHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['isImportantLabel'] + " </span>");
                $('#isImportantForm').removeClass().addClass('form-group has-error');
                $('#isImportant').focus();
                return false;
            }
            $.ajax({type: 'POST', url: url, data: {method: 'create', output: 'json', languageCode: $('#languageCode').val(), languageDescription: $('#languageDescription').val(), languageIcon: $('#languageIcon').val(), isGoogle: $('#isGoogle').val(), isBing: $('#isBing').val(), isSpeakLite: $('#isSpeakLite').val(), isWebServices: $('#isWebServices').val(), isImportant: $('#isImportant').val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    var smileyRoll = './images/icons/smiley-roll.png';
                    var $infoPanel = $('#infoPanel');
                    $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                    if ($infoPanel.is(':hidden')) {
                    ...
                    if (success === true) {
                        ...
                        $('#languageCode').val('');
                        $('#languageCode').val('');
                        $('#languageCodeHelpMe').empty().html('');
                        $('#languageDescription').val('');
                        $('#languageDescriptionForm').removeClass().addClass('form-group');
                        $('#languageDescription').val('');
                        $('#languageDescriptionHelpMe').empty().html('');
                        $('#languageIcon').val('');
                        $('#languageIcon').val('');
                        $('#languageIconHelpMe').empty().html('');
                        $('#isGoogle').val('');
                        $('#isGoogleHelpMe').empty().html('');
                        $('#isBing').val('');
                        $('#isBingHelpMe').empty().html('');
                        $('#isSpeakLite').val('');
                        $('#isSpeakLiteHelpMe').empty().html('');
                        $('#isWebServices').val('');
                        $('#isWebServicesHelpMe').empty().html('');
                        $('#isImportant').val('');
                        $('#isImportantHelpMe').empty().html('');
                    } else if (success === false) {
                        $infoPanel.html('').empty().html("<span class='label label-important'>&nbsp;" + message + "</span>");
                        if ($infoPanel.is(':hidden')) {
                            $infoPanel.show();
...
function updateRecord(leafId, url, urlList, securityToken, type, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        if (type === 1) {
            ...
            $infoPanel.html('').empty();
            $.ajax({type: 'POST', url: url, data: {method: 'save', output: 'json', languageId: $('#languageId').val(), languageCode: $('#languageCode').val(), languageDescription: $('#languageDescription').val(), languageIcon: $('#languageIcon').val(), isGoogle: $('#isGoogle').val(), isBing: $('#isBing').val(), isSpeakLite: $('#isSpeakLite').val(), isWebServices: $('#isWebServices').val(), isImportant: $('#isImportant').val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    var smileyRoll = './images/icons/smiley-roll.png';
                    var $infoPanel = $('#infoPanel');
                    $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                    if ($infoPanel.is(':hidden')) {
        ...
        if (deleteAccess === 1) {
            if (confirm(decodeURIComponent(t['deleteRecordMessageLabel']))) {
                ...
                if (!value) {
                    ...
                    $.ajax({type: 'POST', url: url, data: {method: 'delete', output: 'json', languageId: $languageId.val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                            var smileyRoll = './images/icons/smiley-roll.png';
                            var $infoPanel = $('#infoPanel');
                            $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                            if ($infoPanel.is(':hidden')) {
...
function resetRecord(leafId, url, urlList, securityToken, createAccess, updateAccess, deleteAccess) {
    ...
    $("#languageId").val('');
    $("#languageIdHelpMe").empty().html('');
    
    $("#languageCode").val('');
    $("#languageCodeHelpMe").empty().html('');
    $("#languageDescription").val('');
    $("#languageDescriptionHelpMe").empty().html('');
    $('#languageDescription').empty().val('');
    $("#languageIcon").val('');
    $("#languageIconHelpMe").empty().html('');
    $("#isGoogle").val('');
    $("#isGoogleHelpMe").empty().html('');
...
function firstRecord(leafId, url, urlList, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        $.ajax({type: 'GET', url: url, data: {method: 'dataNavigationRequest', dataNavigation: 'firstRecord', output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                ...
                if (success === true) {
                    $.ajax({type: 'POST', url: url, data: {method: 'read', languageId: firstRecord, output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                            var smileyRoll = './images/icons/smiley-roll.png';
                            $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                            if ($infoPanel.is(':hidden')) {
                                $infoPanel.show();
                            ...
                            if (success === true) {
                                $('#languageId').val(data.data.languageId);
                                $('#languageCode').val(data.data.languageCode);
                                $('#languageDescription').val(data.data.languageDescription);
                                $('#languageIcon').val(data.data.languageIcon);
                                $('#isGoogle').val(data.data.isGoogle);
                                $('#isBing').val(data.data.isBing);
                                $('#isSpeakLite').val(data.data.isSpeakLite);
                                $('#isWebServices').val(data.data.isWebServices);
                                $('#isImportant').val(data.data.isImportant);
                                if (nextRecord > 0) {
                                    $('#previousRecordButton').removeClass().addClass('btn btn-default disabled').attr('onClick', '');
...
function endRecord(leafId, url, urlList, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        $.ajax({type: 'GET', url: url, data: {method: 'dataNavigationRequest', dataNavigation: 'lastRecord', output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                ...
                var lastRecord = data.lastRecord;
                if (success === true) {
                    $.ajax({type: 'POST', url: url, data: {method: 'read', languageId: lastRecord, output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                            var smileyRoll = './images/icons/smiley-roll.png';
                            $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                            if ($infoPanel.is(':hidden')) {
                                $infoPanel.show();
...
function previousRecord(leafId, url, urlList, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($previousRecordCounter.val()) > 0 && parseFloat($previousRecordCounter.val()) < parseFloat($('#lastRecordCounter').val())) {
            $.ajax({type: 'POST', url: url, data: {method: 'read', languageId: $previousRecordCounter.val(), output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    var smileyRoll = './images/icons/smiley-roll.png';
                    var $infoPanel = $('#infoPanel');
                    $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                    if ($infoPanel.is(':hidden')) {
...
function nextRecord(leafId, url, urlList, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($nextRecordCounter.val()) <= parseFloat($('#lastRecordCounter').val())) {
            $.ajax({type: 'POST', url: url, data: {method: 'read', languageId: $nextRecordCounter.val(), output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    var smileyRoll = './images/icons/smiley-roll.png';
                    var $infoPanel = $('#infoPanel');
                    $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                    if ($infoPanel.is(':hidden')) {
