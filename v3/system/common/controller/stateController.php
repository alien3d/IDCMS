...
namespace Core\System\Common\State\Controller;

use Core\ConfigClass;
use Core\Document\Trail\DocumentTrailClass;
use Core\RecordSet\RecordSet;
use Core\shared\SharedClass;
use Core\System\Common\State\Model\StateModel;
use Core\System\Common\State\Service\StateService;
...
require_once($newFakeDocumentRoot . "v3/system/common/model/stateModel.php");
require_once($newFakeDocumentRoot . "v3/system/common/service/stateService.php");

/**
 * Class State
 * this is state controller files.
 * @name IDCMS
 * @version 2
 * @author hafizan
 * @package  Core\System\Common\State\Controller
 * @subpackage Common
...
class StateClass extends ConfigClass {
    ...
    /**
     * Model
     * @var \Core\System\Common\State\Model\StateModel
     */
    public $model;

    /**
     * Service-Business Application Process or other ajax request
     * @var \Core\System\Common\State\Service\StateService
     */
    public $service;
    ...
    function __construct() {
        ...
        $this->setViewPath("./v3/system/common/view/state.php");
        $this->setControllerPath("./v3/system/common/controller/stateController.php");
        $this->setServicePath("./v3/system/common/service/stateService.php");
    }
    ...
    public function create() {
        ...
        if ($this->getVendor() == self::MYSQL) {
            ...
            (
                 ...
                 '" . $this->model->getStateCode() . "',
                 '" . $this->model->getStateDescription() . "',
    ...
    public function read() {
        ...
        if (isset($_SESSION['isAdmin'])) {
            if ($_SESSION['isAdmin'] == 0) {
                if ($this->getVendor() == self::MYSQL) {
                    $this->setAuditFilter(
                            " `state`.`isActive` = 1  AND `state`.`companyId`='" . $this->getCompanyId() . "' "
                    );
                } else {
                    if ($this->getVendor() == self::MSSQL) {
                        $this->setAuditFilter(
                ...
                if ($_SESSION['isAdmin'] == 1) {
                    if ($this->getVendor() == self::MYSQL) {
                        $this->setAuditFilter("   `state`.`companyId`='" . $this->getCompanyId() . "'	");
                    } else {
                        if ($this->getVendor() == self::MSSQL) {
                            $this->setAuditFilter(" [state].[companyId]='" . $this->getCompanyId() . "' ");
                        } else {
	...
	JOIN	`company`
	ON		`company`.`companyId` = `state`.`companyId`
	JOIN	`country`
	ON		`country`.`countryId` = `state`.`countryId`
		  WHERE     " . $this->getAuditFilter();
            if ($this->model->getStateId(0, 'single')) {
                $sql .= " AND `state`.`" . $this->model->getPrimaryKeyName() . "`='" . $this->model->getStateId(
                                0, 'single'
                        ) . "'";
            }
            if ($this->model->getCountryId()) {
                $sql .= " AND `state`.`countryId`='" . $this->model->getCountryId() . "'";
            }
        } else {
            if ($this->getVendor() == self::MSSQL) {
	...
	ON		[country].[countryId] = [state].[countryId]
		  WHERE     " . $this->getAuditFilter();
                if ($this->model->getStateId(0, 'single')) {
                    $sql .= " AND [state].[" . $this->model->getPrimaryKeyName(
                            ) . "]		=	'" . $this->model->getStateId(0, 'single') . "'";
                }
                if ($this->model->getCountryId()) {
                    $sql .= " AND [state].[countryId]='" . $this->model->getCountryId() . "'";
                }
	...
	ON		COUNTRY.COUNTRYID = STATE.COUNTRYID
        ...
        if ($this->getCharacterQuery()) {
            if ($this->getVendor() == self::MYSQL) {
                $sql .= " AND `state`.`" . $this->model->getFilterCharacter() . "` like '" . $this->getCharacterQuery(
                        ) . "%'";
            } else {
                if ($this->getVendor() == self::MSSQL) {
                    $sql .= " AND [state].[" . $this->model->getFilterCharacter(
                            ) . "] like '" . $this->getCharacterQuery() . "%'";
                } else {
                    if ($this->getVendor() == self::ORACLE) {
                        $sql .= " AND Initcap(STATE." . strtoupper(
        ...
        if ($this->getDateRangeStartQuery()) {
            if ($this->getVendor() == self::MYSQL) {
                $sql .= $this->q->dateFilter(
                        'state', $this->model->getFilterDate(), $this->getDateRangeStartQuery(), $this->getDateRangeEndQuery(), $this->getDateRangeTypeQuery(), $this->getDateRangeExtraTypeQuery()
                );
            } else {
                if ($this->getVendor() == self::MSSQL) {
                    $sql .= $this->q->dateFilter(
        ...
        $tableArray = null;
        if ($this->getVendor() == self::MYSQL) {
            $tableArray = array('staff', 'state', 'company', 'country');
        } else {
            if ($this->getVendor() == self::MSSQL) {
                $tableArray = array('staff', 'state', 'company', 'country');
            } else {
        ...
        while (($row = $this->q->fetchAssoc()) == true) {
            ...
            if ($this->model->getStateId(0, 'single')) {
                ...
                $row['previousRecord'] = $this->previousRecord('value', $this->model->getStateId(0, 'single'));
                $row['nextRecord'] = $this->nextRecord('value', $this->model->getStateId(0, 'single'));
                            ...
                            array("[", "]"), "", json_encode(
                                    array(
                                        ...
                                        'previousRecord' => $this->previousRecord(
                                                'value', $this->model->getStateId(0, 'single')
                                        ),
                                        'nextRecord' => $this->nextRecord('value', $this->model->getStateId(0, 'single')),
    ...
    function update() {
        ...
        if ($this->getVendor() == self::MYSQL) {
           ...
           FROM 	STATE
           WHERE  	" . strtoupper($this->model->getPrimaryKeyName()) . " = '" . $this->model->getStateId(
                                    0, 'single'
                            ) . "' ";
                }
            }
        ...
        if ($total == 0) {
            ...
            if ($this->getVendor() == self::MYSQL) {
               ...
               UPDATE `state` SET
                       ...
                       `stateCode` = '" . $this->model->getStateCode() . "',
                       `stateDescription` = '" . $this->model->getStateDescription() . "',
    ...
    function excel() {
        ...
        $this->excel->getActiveSheet()->setCellValue('D3', $this->translate['stateCodeLabel']);
        $this->excel->getActiveSheet()->setCellValue('E3', $this->translate['stateDescriptionLabel']);
        ...
        while (($row = $this->q->fetchAssoc()) == true) {
            ...
            $this->excel->getActiveSheet()->setCellValue('D' . $loopRow, strip_tags($row ['stateCode']));
        ...
        switch ($this->getReportMode()) {
            case 'excel':
                ...
                $extension = '.xlsx';
                $folder = 'excel';
                $filename = "state" . rand(0, 10000000) . $extension;
...
if (isset($_POST ['method'])) {
    if (isset($_POST['output'])) {
        ...
        if ($_POST['securityToken'] != $stateObject->getSecurityToken()) {
            header('Content-Type:application/json; charset=utf-8');
        ...
        if (isset($_POST ['offset'])) {
            $stateObject->setStart($_POST ['offset']);
        }
        if (isset($_POST ['limit'])) {
            $stateObject->setLimit($_POST ['limit']);
        }
        $stateObject->setPageOutput($_POST['output']);
        $stateObject->execute();
        /*
...
if (isset($_GET ['method'])) {
    $stateObject = new StateClass ();
    if ($_GET['securityToken'] != $stateObject->getSecurityToken()) {
        header('Content-Type:application/json; charset=utf-8');
        echo json_encode(array("success" => false, "message" => "Something wrong with the system.Hola hackers"));
        exit();
    }
    ...
    if (isset($_GET ['databaseRequest'])) {
        $stateObject->setRequestDatabase($_GET ['databaseRequest']);
    }
    /*
     *  Load the dynamic value
     */
     ...
     */
    if (isset($_GET ['mode'])) {
        $stateObject->setReportMode($_GET['mode']);
    ...
    if (isset($_GET ['filter'])) {
        $stateObject->setServiceOutput('option');
        if (($_GET['filter'] == 'country')) {
            $stateObject->getCountry();
        }
    }
}
?>
