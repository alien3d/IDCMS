...
namespace Core\System\Common\Family\Controller;

use Core\ConfigClass;
use Core\Document\Trail\DocumentTrailClass;
use Core\RecordSet\RecordSet;
use Core\shared\SharedClass;
use Core\System\Common\Family\Model\FamilyModel;
use Core\System\Common\Family\Service\FamilyService;
...
require_once($newFakeDocumentRoot . "v3/system/common/model/familyModel.php");
require_once($newFakeDocumentRoot . "v3/system/common/service/familyService.php");

/**
 * Class Family
 * this is family controller files.
 * @name IDCMS
 * @version 2
 * @author hafizan
 * @package  Core\System\Common\Family\Controller
 * @subpackage Common
 * @link http://www.hafizan.com
 * @license http://www.gnu.org/copyleft/lesser.html LGPL
 */
class FamilyClass extends ConfigClass {
    ...
    /**
     * Model
     * @var \Core\System\Common\Family\Model\FamilyModel
     */
    public $model;

    /**
     * Service-Business Application Process or other ajax request
     * @var \Core\System\Common\Family\Service\FamilyService
     */
    public $service;
    ...
    function __construct() {
        ...
        $this->setViewPath("./v3/system/common/view/family.php");
        $this->setControllerPath("./v3/system/common/controller/familyController.php");
        $this->setServicePath("./v3/system/common/service/familyService.php");
    }
    ...
    public function create() {
        ...
        if ($this->getVendor() == self::MYSQL) {
            ...
            (
                 ...
                 '" . $this->getCompanyId() . "',
                 '" . $this->model->getFamilyCode() . "',
                 '" . $this->model->getFamilyDescription() . "',
    ...
    public function read() {
        ...
        if (isset($_SESSION['isAdmin'])) {
            if ($_SESSION['isAdmin'] == 0) {
                if ($this->getVendor() == self::MYSQL) {
                    $this->setAuditFilter(
                            " `family`.`isActive` = 1  AND `family`.`companyId`='" . $this->getCompanyId() . "' "
                    );
                } else {
                    if ($this->getVendor() == self::MSSQL) {
                        $this->setAuditFilter(
                ...
                if ($_SESSION['isAdmin'] == 1) {
                    if ($this->getVendor() == self::MYSQL) {
                        $this->setAuditFilter("   `family`.`companyId`='" . $this->getCompanyId() . "'	");
                    } else {
                        if ($this->getVendor() == self::MSSQL) {
                            $this->setAuditFilter(" [family].[companyId]='" . $this->getCompanyId() . "' ");
                        } else {
       ...
       SELECT                    `family`.`familyId`,
		  ...
		  FROM      `family`
		  JOIN      `staff`
		  ON        `family`.`executeBy` = `staff`.`staffId`
	JOIN	`company`
	ON		`company`.`companyId` = `family`.`companyId`
		  WHERE     " . $this->getAuditFilter();
            if ($this->model->getFamilyId(0, 'single')) {
                $sql .= " AND `family`.`" . $this->model->getPrimaryKeyName() . "`='" . $this->model->getFamilyId(
                                0, 'single'
                        ) . "'";
            }
        } else {
	...
	ON		[company].[companyId] = [family].[companyId]
		  WHERE     " . $this->getAuditFilter();
                if ($this->model->getFamilyId(0, 'single')) {
                    $sql .= " AND [family].[" . $this->model->getPrimaryKeyName(
                            ) . "]		=	'" . $this->model->getFamilyId(0, 'single') . "'";
                }
            } else {
                if ($this->getVendor() == self::ORACLE) {
		  ...
		  SELECT                    FAMILY.FAMILYID AS \"familyId\",
                    ...
                    FAMILY.COMPANYID AS \"companyId\",
                    FAMILY.FAMILYCODE AS \"familyCode\",
                    FAMILY.FAMILYDESCRIPTION AS \"familyDescription\",
                    FAMILY.ISDEFAULT AS \"isDefault\",
                    FAMILY.ISNEW AS \"isNew\",
                    FAMILY.ISDRAFT AS \"isDraft\",
                    FAMILY.ISUPDATE AS \"isUpdate\",
                    FAMILY.ISDELETE AS \"isDelete\",
                    FAMILY.ISACTIVE AS \"isActive\",
                    FAMILY.ISAPPROVED AS \"isApproved\",
                    FAMILY.ISREVIEW AS \"isReview\",
                    FAMILY.ISPOST AS \"isPost\",
                    FAMILY.EXECUTEBY AS \"executeBy\",
                    FAMILY.EXECUTETIME AS \"executeTime\",
	...
	ON		COMPANY.COMPANYID = FAMILY.COMPANYID
        ...
        if ($this->getCharacterQuery()) {
            if ($this->getVendor() == self::MYSQL) {
                $sql .= " AND `family`.`" . $this->model->getFilterCharacter() . "` like '" . $this->getCharacterQuery(
                        ) . "%'";
            } else {
                if ($this->getVendor() == self::MSSQL) {
                    $sql .= " AND [family].[" . $this->model->getFilterCharacter(
                            ) . "] like '" . $this->getCharacterQuery() . "%'";
                } else {
                    if ($this->getVendor() == self::ORACLE) {
                        $sql .= " AND Initcap(FAMILY." . strtoupper(
        ...
        if ($this->getDateRangeStartQuery()) {
            if ($this->getVendor() == self::MYSQL) {
                $sql .= $this->q->dateFilter(
                        'family', $this->model->getFilterDate(), $this->getDateRangeStartQuery(), $this->getDateRangeEndQuery(), $this->getDateRangeTypeQuery(), $this->getDateRangeExtraTypeQuery()
                );
            } else {
                if ($this->getVendor() == self::MSSQL) {
                    $sql .= $this->q->dateFilter(
        ...
        while (($row = $this->q->fetchAssoc()) == true) {
            ...
            if ($this->model->getFamilyId(0, 'single')) {
                ...
                $row['previousRecord'] = $this->previousRecord('value', $this->model->getFamilyId(0, 'single'));
                $row['nextRecord'] = $this->nextRecord('value', $this->model->getFamilyId(0, 'single'));
                            ...
                            array("[", "]"), "", json_encode(
                                    array(
                                        ...
                                        'previousRecord' => $this->previousRecord(
                                                'value', $this->model->getFamilyId(0, 'single')
                                        ),
                                        'nextRecord' => $this->nextRecord('value', $this->model->getFamilyId(0, 'single')),
    ...
    function update() {
        ...
        if ($this->getVendor() == self::MYSQL) {
           ...
           FROM 	`family`
           WHERE  	`" . $this->model->getPrimaryKeyName() . "` = '" . $this->model->getFamilyId(0, 'single') . "' ";
        } else {
            if ($this->getVendor() == self::MSSQL) {
                $sql = "
           SELECT	[" . $this->model->getPrimaryKeyName() . "]
           ...
           FROM 	FAMILY
           WHERE  	" . strtoupper($this->model->getPrimaryKeyName()) . " = '" . $this->model->getFamilyId(
                                    0, 'single'
                            ) . "' ";
                }
            }
        ...
        if ($total == 0) {
            ...
            if ($this->getVendor() == self::MYSQL) {
                $sql = "
               UPDATE `family` SET
                       `familyCode` = '" . $this->model->getFamilyCode() . "',
                       `familyDescription` = '" . $this->model->getFamilyDescription() . "',
               ...
               WHERE    `familyId`='" . $this->model->getFamilyId('0', 'single') . "'";
            } else {
                if ($this->getVendor() == self::MSSQL) {
                    $sql = "
                UPDATE [family] SET
...
/** 
    ...
    function duplicate() {
        ...
        if ($this->getVendor() == self::MYSQL) {
           ...
           SELECT  `familyCode`
           FROM    `family`
           WHERE   `familyCode` 	= 	'" . $this->model->getFamilyCode() . "'
           AND     `isActive`  =   1
           ...
           AND     [companyId] =	'" . $this->getCompanyId() . "'";
                ...
                if ($this->getVendor() == self::ORACLE) {
                    $sql = "
               SELECT  FAMILYCODE as \"familyCode\"
               FROM    FAMILY
               WHERE   FAMILYCODE	= 	'" . $this->model->getFamilyCode() . "'
               AND     ISACTIVE    =   1
               AND     COMPANYID   =   '" . $this->getCompanyId() . "'";
    ...
    function excel() {
        ...
        $this->excel->getActiveSheet()->setCellValue('C3', $this->translate['familyCodeLabel']);
        $this->excel->getActiveSheet()->setCellValue('D3', $this->translate['familyDescriptionLabel']);
        ...
        while (($row = $this->q->fetchAssoc()) == true) {
            ...
            $this->excel->getActiveSheet()->setCellValue('C' . $loopRow, strip_tags($row ['familyCode']));
            $this->excel->getActiveSheet()->setCellValue('D' . $loopRow, strip_tags($row ['familyDescription']));
        ...
        switch ($this->getReportMode()) {
            case 'excel':
                ...
                $extension = '.xlsx';
                $folder = 'excel';
                $filename = "family" . rand(0, 10000000) . $extension;
...
if (isset($_POST ['method'])) {
    if (isset($_POST['output'])) {
        $familyObject = new FamilyClass ();
        if ($_POST['securityToken'] != $familyObject->getSecurityToken()) {
            header('Content-Type:application/json; charset=utf-8');
        ...
        if (isset($_POST ['leafId'])) {
            $familyObject->setLeafId($_POST ['leafId']);
        }
        if (isset($_POST ['offset'])) {
            $familyObject->setStart($_POST ['offset']);
        }
        if (isset($_POST ['limit'])) {
            $familyObject->setLimit($_POST ['limit']);
        }
        $familyObject->setPageOutput($_POST['output']);
        $familyObject->execute();
        /*
...
if (isset($_GET ['method'])) {
    $familyObject = new FamilyClass ();
    if ($_GET['securityToken'] != $familyObject->getSecurityToken()) {
        header('Content-Type:application/json; charset=utf-8');
        echo json_encode(array("success" => false, "message" => "Something wrong with the system.Hola hackers"));
        exit();
    }
    ...
    if (isset($_GET ['leafId'])) {
        $familyObject->setLeafId($_GET ['leafId']);
    }
    /*
     *  Load the dynamic value
     */
    ...
    if ($_GET ['method'] == 'updateStatus') {
        $familyObject->updateStatus();
    }
    /*
     *  Checking Any Duplication  Key
     */
    if ($_GET['method'] == 'duplicate') {
        $familyObject->duplicate();
    }
    if ($_GET ['method'] == 'dataNavigationRequest') {
        if ($_GET ['dataNavigation'] == 'firstRecord') {
            $familyObject->firstRecord('json');
        }
        if ($_GET ['dataNavigation'] == 'previousRecord') {
            $familyObject->previousRecord('json', 0);
        }
        if ($_GET ['dataNavigation'] == 'nextRecord') {
            $familyObject->nextRecord('json', 0);
        }
        if ($_GET ['dataNavigation'] == 'lastRecord') {
            $familyObject->lastRecord('json');
        }
    }
    /*
     * Excel Reporting
     */
    if (isset($_GET ['mode'])) {
        $familyObject->setReportMode($_GET['mode']);
    ...
    if (isset($_GET ['filter'])) {
        $familyObject->setServiceOutput('option');
    }
}
?>
