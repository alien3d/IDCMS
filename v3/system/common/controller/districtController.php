...
namespace Core\System\Common\District\Controller;

use Core\ConfigClass;
use Core\Document\Trail\DocumentTrailClass;
use Core\RecordSet\RecordSet;
use Core\shared\SharedClass;
use Core\System\Common\District\Model\DistrictModel;
use Core\System\Common\District\Service\DistrictService;
...
require_once($newFakeDocumentRoot . "v3/system/common/model/districtModel.php");
require_once($newFakeDocumentRoot . "v3/system/common/service/districtService.php");

/**
 * Class District
 * this is district controller files.
 * @name IDCMS
 * @version 2
 * @author hafizan
 * @package  Core\System\Common\District\Controller
 * @subpackage Common
 * @link http://www.hafizan.com
 * @license http://www.gnu.org/copyleft/lesser.html LGPL
 */
class DistrictClass extends ConfigClass {
    ...
    /**
     * Model
     * @var \Core\System\Common\District\Model\DistrictModel
     */
    public $model;

    /**
     * Service-Business Application Process or other ajax request
     * @var \Core\System\Common\District\Service\DistrictService
     */
    public $service;
    ...
    function __construct() {
        ...
        $this->setViewPath("./v3/system/common/view/district.php");
        $this->setControllerPath("./v3/system/common/controller/districtController.php");
        $this->setServicePath("./v3/system/common/service/districtService.php");
    }
    ...
    public function create() {
        ...
        if ($this->getVendor() == self::MYSQL) {
            ...
            (
                 ...
                 '" . $this->model->getDistrictCode() . "',
                 '" . $this->model->getDistrictDescription() . "',
        ...
        $districtId = $this->q->lastInsertId();
        $this->q->commit();
        $end = microtime(true);
        $time = $end - $start;
        echo json_encode(
    ...
    public function read() {
        ...
        if (isset($_SESSION['isAdmin'])) {
            if ($_SESSION['isAdmin'] == 0) {
                if ($this->getVendor() == self::MYSQL) {
                    $this->setAuditFilter(
                            " `district`.`isActive` = 1  AND `district`.`companyId`='" . $this->getCompanyId() . "' "
                    );
                } else {
                    if ($this->getVendor() == self::MSSQL) {
                        $this->setAuditFilter(
                ...
                if ($_SESSION['isAdmin'] == 1) {
                    if ($this->getVendor() == self::MYSQL) {
                        $this->setAuditFilter("   `district`.`companyId`='" . $this->getCompanyId() . "'	");
                    } else {
                        if ($this->getVendor() == self::MSSQL) {
                            $this->setAuditFilter(" [district].[companyId]='" . $this->getCompanyId() . "' ");
                        } else {
	...
	JOIN	`company`
	ON		`company`.`companyId` = `district`.`companyId`
	JOIN	`country`
	ON		`country`.`countryId` = `district`.`countryId`
	JOIN	`state`
	ON		`state`.`stateId` = `district`.`stateId`
		  WHERE     " . $this->getAuditFilter();
            if ($this->model->getDistrictId(0, 'single')) {
                $sql .= " AND `district`.`" . $this->model->getPrimaryKeyName() . "`='" . $this->model->getDistrictId(
                                0, 'single'
                        ) . "'";
            }
            if ($this->model->getCountryId()) {
                $sql .= " AND `district`.`countryId`='" . $this->model->getCountryId() . "'";
            }
            if ($this->model->getStateId()) {
                $sql .= " AND `district`.`stateId`='" . $this->model->getStateId() . "'";
            }
        } else {
            if ($this->getVendor() == self::MSSQL) {
	...
	ON		[state].[stateId] = [district].[stateId]
		  WHERE     " . $this->getAuditFilter();
                if ($this->model->getDistrictId(0, 'single')) {
                    $sql .= " AND [district].[" . $this->model->getPrimaryKeyName(
                            ) . "]		=	'" . $this->model->getDistrictId(0, 'single') . "'";
                }
                if ($this->model->getCountryId()) {
                    $sql .= " AND [district].[countryId]='" . $this->model->getCountryId() . "'";
                }
                ...
                if ($this->getVendor() == self::ORACLE) {
                    ...
                    $sql = "
		  SELECT                    DISTRICT.DISTRICTID AS \"districtId\",
                    COMPANY.COMPANYDESCRIPTION AS  \"companyDescription\",
                    DISTRICT.COMPANYID AS \"companyId\",
                    COUNTRY.COUNTRYDESCRIPTION AS  \"countryDescription\",
                    DISTRICT.COUNTRYID AS \"countryId\",
                    STATE.STATEDESCRIPTION AS  \"stateDescription\",
                    DISTRICT.STATEID AS \"stateId\",
                    DISTRICT.DISTRICTCODE AS \"districtCode\",
                    DISTRICT.DISTRICTDESCRIPTION AS \"districtDescription\",
                    DISTRICT.ISDEFAULT AS \"isDefault\",
                    DISTRICT.ISNEW AS \"isNew\",
                    DISTRICT.ISDRAFT AS \"isDraft\",
                    DISTRICT.ISUPDATE AS \"isUpdate\",
                    DISTRICT.ISDELETE AS \"isDelete\",
                    DISTRICT.ISACTIVE AS \"isActive\",
                    DISTRICT.ISAPPROVED AS \"isApproved\",
                    DISTRICT.ISREVIEW AS \"isReview\",
	...
	ON		STATE.STATEID = DISTRICT.STATEID
        ...
        if ($this->getCharacterQuery()) {
            if ($this->getVendor() == self::MYSQL) {
                $sql .= " AND `district`.`" . $this->model->getFilterCharacter(
                ...
                if ($this->getVendor() == self::MSSQL) {
                    ...
                    if ($this->getVendor() == self::ORACLE) {
                        $sql .= " AND Initcap(DISTRICT." . strtoupper(
        ...
        if ($this->getDateRangeStartQuery()) {
            if ($this->getVendor() == self::MYSQL) {
                $sql .= $this->q->dateFilter(
                        'district', $this->model->getFilterDate(), $this->getDateRangeStartQuery(), $this->getDateRangeEndQuery(), $this->getDateRangeTypeQuery(), $this->getDateRangeExtraTypeQuery()
                );
            } else {
                if ($this->getVendor() == self::MSSQL) {
                    $sql .= $this->q->dateFilter(
        ...
        $tableArray = null;
        if ($this->getVendor() == self::MYSQL) {
            $tableArray = array('staff', 'district', 'company', 'country', 'state');
        } else {
            if ($this->getVendor() == self::MSSQL) {
                $tableArray = array('staff', 'district', 'company', 'country', 'state');
            } else {
        ...
        while (($row = $this->q->fetchAssoc()) == true) {
            ...
            if ($this->model->getDistrictId(0, 'single')) {
                ...
                $row['previousRecord'] = $this->previousRecord('value', $this->model->getDistrictId(0, 'single'));
                $row['nextRecord'] = $this->nextRecord('value', $this->model->getDistrictId(0, 'single'));
                            ...
                            array("[", "]"), "", json_encode(
                                    array(
                                        ...
                                        'previousRecord' => $this->previousRecord(
                                                'value', $this->model->getDistrictId(0, 'single')
                                        ),
                                        'nextRecord' => $this->nextRecord('value', $this->model->getDistrictId(0, 'single')),
    ...
    function update() {
        ...
        if ($this->getVendor() == self::MYSQL) {
           ...
           FROM 	`district`
           WHERE  	`" . $this->model->getPrimaryKeyName() . "` = '" . $this->model->getDistrictId(
                            0, 'single'
                    ) . "' ";
        } else {
            if ($this->getVendor() == self::MSSQL) {
           ...
           FROM 	DISTRICT
           WHERE  	" . strtoupper($this->model->getPrimaryKeyName()) . " = '" . $this->model->getDistrictId(
                                    0, 'single'
                            ) . "' ";
                }
            }
        ...
        if ($total == 0) {
            ...
            if ($this->getVendor() == self::MYSQL) {
               ...
               UPDATE `district` SET
                       ...
                       `districtCode` = '" . $this->model->getDistrictCode() . "',
                       `districtDescription` = '" . $this->model->getDistrictDescription() . "',
               ...
               WHERE    `districtId`='" . $this->model->getDistrictId('0', 'single') . "'";
            } else {
                if ($this->getVendor() == self::MSSQL) {
                    $sql = "
                UPDATE [district] SET
...
/** 
    ...
    function duplicate() {
        ...
        if ($this->getVendor() == self::MYSQL) {
           ...
           AND     [companyId] =	'" . $this->getCompanyId() . "'";
                ...
                if ($this->getVendor() == self::ORACLE) {
                    $sql = "
               SELECT  DISTRICTCODE as \"districtCode\"
               FROM    DISTRICT
               WHERE   DISTRICTCODE	= 	'" . $this->model->getDistrictCode() . "'
               AND     ISACTIVE    =   1
               AND     COMPANYID   =   '" . $this->getCompanyId() . "'";
    ...
    function excel() {
        ...
        $this->excel->getActiveSheet()->setCellValue('E3', $this->translate['districtCodeLabel']);
        $this->excel->getActiveSheet()->setCellValue('F3', $this->translate['districtDescriptionLabel']);
        ...
        while (($row = $this->q->fetchAssoc()) == true) {
            ...
            $this->excel->getActiveSheet()->setCellValue('C' . $loopRow, ($row ['countryDescription']));
            /**
              $this->excel->getActiveSheet()->setCellValue('D' . $loopRow, strip_tags($row ['stateDescription']));
              $this->excel->getActiveSheet()->setCellValue('E' . $loopRow, strip_tags($row ['districtCode']));
              $this->excel->getActiveSheet()->setCellValue('F' . $loopRow, strip_tags($row ['districtDescription']));
        ...
        switch ($this->getReportMode()) {
            case 'excel':
                ...
                $extension = '.xlsx';
                $folder = 'excel';
                $filename = "district" . rand(0, 10000000) . $extension;
...
if (isset($_POST ['method'])) {
    if (isset($_POST['output'])) {
        ...
        if ($_POST['securityToken'] != $districtObject->getSecurityToken()) {
            header('Content-Type:application/json; charset=utf-8');
        ...
        if (isset($_POST ['leafId'])) {
            $districtObject->setLeafId($_POST ['leafId']);
        }
        if (isset($_POST ['offset'])) {
            $districtObject->setStart($_POST ['offset']);
        }
        if (isset($_POST ['limit'])) {
            $districtObject->setLimit($_POST ['limit']);
        }
        $districtObject->setPageOutput($_POST['output']);
        $districtObject->execute();
        /*
         *  Crud Operation (Create Read Update Delete/Destroy)
         */
        if ($_POST ['method'] == 'create') {
            $districtObject->create();
        }
        if ($_POST ['method'] == 'save') {
            $districtObject->update();
        }
        if ($_POST ['method'] == 'read') {
            $districtObject->read();
        }
        if ($_POST ['method'] == 'delete') {
            $districtObject->delete();
        }
        if ($_POST ['method'] == 'posting') {
            //	$districtObject->posting();
        }
...
if (isset($_GET ['method'])) {
    $districtObject = new DistrictClass ();
    if ($_GET['securityToken'] != $districtObject->getSecurityToken()) {
        header('Content-Type:application/json; charset=utf-8');
        echo json_encode(array("success" => false, "message" => "Something wrong with the system.Hola hackers"));
        exit();
    }
    ...
    if (isset($_GET ['leafId'])) {
        $districtObject->setLeafId($_GET ['leafId']);
    }
    /*
     *  Load the dynamic value
     */
    ...
    if ($_GET ['method'] == 'updateStatus') {
        $districtObject->updateStatus();
    }
    /*
     *  Checking Any Duplication  Key
     */
    if ($_GET['method'] == 'duplicate') {
        $districtObject->duplicate();
    }
    if ($_GET ['method'] == 'dataNavigationRequest') {
        if ($_GET ['dataNavigation'] == 'firstRecord') {
            $districtObject->firstRecord('json');
        }
        if ($_GET ['dataNavigation'] == 'previousRecord') {
            $districtObject->previousRecord('json', 0);
        }
        if ($_GET ['dataNavigation'] == 'nextRecord') {
            $districtObject->nextRecord('json', 0);
        }
        if ($_GET ['dataNavigation'] == 'lastRecord') {
            $districtObject->lastRecord('json');
        }
    }
    /*
     * Excel Reporting
     */
    if (isset($_GET ['mode'])) {
        $districtObject->setReportMode($_GET['mode']);
        if ($_GET ['mode'] == 'excel' || $_GET ['mode'] == 'pdf' || $_GET['mode'] == 'csv' || $_GET['mode'] == 'html' || $_GET['mode'] == 'excel5' || $_GET['mode'] == 'xml'
        ) {
            echo "betul";
            $districtObject->excel();
        }
    }
    if (isset($_GET ['filter'])) {
        $districtObject->setServiceOutput('option');
        if (($_GET['filter'] == 'country')) {
            $districtObject->getCountry();
        }
        if (($_GET['filter'] == 'state')) {
            $districtObject->getState();
        }
    }
}
?>
