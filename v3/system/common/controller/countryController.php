...
use Core\System\Common\Country\Model\CountryModel;
use Core\System\Common\Country\Service\CountryService;
...
require_once($newFakeDocumentRoot . "v3/system/common/model/countryModel.php");
require_once($newFakeDocumentRoot . "v3/system/common/service/countryService.php");

/**
 * Class Country
 * this is country controller files.
 * @name IDCMS
 * @version 2
 * @author hafizan
 * @package  Core\System\Common\Country\Controller
 * @subpackage Common
...
class CountryClass extends ConfigClass {
    ...
    /**
     * Model
     * @var \Core\System\Common\Country\Model\CountryModel
     */
    public $model;

    /**
     * Service-Business Application Process or other ajax request
     * @var \Core\System\Common\Country\Service\CountryService
     */
    public $service;
    ...
    function __construct() {
        ...
        $this->setViewPath("./v3/system/common/view/country.php");
        $this->setControllerPath("./v3/system/common/controller/countryController.php");
        $this->setServicePath("./v3/system/common/service/countryService.php");
    }
    ...
    public function create() {
        ...
        if ($this->getVendor() == self::MYSQL) {
            ...
            (
                 ...
                 `countryCode`,
                 `countryCurrencyCode`,
                 `countryCurrencyCodeDescription`,
                 `localeOne`,
                 `localeTwo`,
                 `localeThree`,
                 `localeFour`,
                 ...
                 '" . $this->model->getCountryCurrencyCodeDescription() . "',
                 '" . $this->model->getLocaleOne() . "',
                 '" . $this->model->getLocaleTwo() . "',
                 '" . $this->model->getLocaleThree() . "',
                 '" . $this->model->getLocaleFour() . "',
                 '" . $this->model->getLocaleFive() . "',
                 '" . $this->model->getCountryDescription() . "',
    ...
    public function read() {
        ...
        if (isset($_SESSION['isAdmin'])) {
            if ($_SESSION['isAdmin'] == 0) {
                if ($this->getVendor() == self::MYSQL) {
                    $this->setAuditFilter(
                            " `country`.`isActive` = 1  AND `country`.`companyId`='" . $this->getCompanyId() . "' "
                    );
                } else {
                    if ($this->getVendor() == self::MSSQL) {
                        $this->setAuditFilter(
                ...
                if ($_SESSION['isAdmin'] == 1) {
                    if ($this->getVendor() == self::MYSQL) {
                        $this->setAuditFilter("   `country`.`companyId`='" . $this->getCompanyId() . "'	");
                    } else {
                        if ($this->getVendor() == self::MSSQL) {
                            $this->setAuditFilter(" [country].[companyId]='" . $this->getCompanyId() . "' ");
                        } else {
       ...
       SELECT                    `country`.`countryId`,
                    ...
                    `country`.`countryCurrencyCodeDescription`,
                    `country`.`localeOne`,
                    `country`.`localeTwo`,
                    `country`.`localeThree`,
                    `country`.`localeFour`,
	...
	JOIN	`company`
	ON		`company`.`companyId` = `country`.`companyId`
		  WHERE     " . $this->getAuditFilter();
            if ($this->model->getCountryId(0, 'single')) {
                $sql .= " AND `country`.`" . $this->model->getPrimaryKeyName() . "`='" . $this->model->getCountryId(
                                0, 'single'
                        ) . "'";
            }
        } else {
	...
	ON		[company].[companyId] = [country].[companyId]
		  WHERE     " . $this->getAuditFilter();
                if ($this->model->getCountryId(0, 'single')) {
                    $sql .= " AND [country].[" . $this->model->getPrimaryKeyName(
                            ) . "]		=	'" . $this->model->getCountryId(0, 'single') . "'";
                }
            } else {
                if ($this->getVendor() == self::ORACLE) {
		  ...
		  SELECT                    COUNTRY.COUNTRYID AS \"countryId\",
                    ...
                    COUNTRY.COMPANYID AS \"companyId\",
                    COUNTRY.COUNTRYCODE AS \"countryCode\",
                    COUNTRY.COUNTRYCURRENCYCODE AS \"countryCurrencyCode\",
                    COUNTRY.COUNTRYCURRENCYCODEDESCRIPTION AS \"countryCurrencyCodeDescription\",
                    COUNTRY.LOCALEONE AS \"localeOne\",
                    COUNTRY.LOCALETWO AS \"localeTwo\",
                    COUNTRY.LOCALETHREE AS \"localeThree\",
                    COUNTRY.LOCALEFOUR AS \"localeFour\",
                    COUNTRY.LOCALEFIVE AS \"localeFive\",
	...
	ON		COMPANY.COMPANYID = COUNTRY.COMPANYID
        ...
        if ($this->getCharacterQuery()) {
            if ($this->getVendor() == self::MYSQL) {
                $sql .= " AND `country`.`" . $this->model->getFilterCharacter() . "` like '" . $this->getCharacterQuery(
                        ) . "%'";
            } else {
                if ($this->getVendor() == self::MSSQL) {
                    $sql .= " AND [country].[" . $this->model->getFilterCharacter(
                            ) . "] like '" . $this->getCharacterQuery() . "%'";
                } else {
                    if ($this->getVendor() == self::ORACLE) {
                        $sql .= " AND Initcap(COUNTRY." . strtoupper(
        ...
        if ($this->getDateRangeStartQuery()) {
            if ($this->getVendor() == self::MYSQL) {
                $sql .= $this->q->dateFilter(
                        'country', $this->model->getFilterDate(), $this->getDateRangeStartQuery(), $this->getDateRangeEndQuery(), $this->getDateRangeTypeQuery(), $this->getDateRangeExtraTypeQuery()
                );
            } else {
                if ($this->getVendor() == self::MSSQL) {
                    $sql .= $this->q->dateFilter(
        ...
        $tableArray = null;
        if ($this->getVendor() == self::MYSQL) {
            $tableArray = array('staff', 'country');
        } else {
            if ($this->getVendor() == self::MSSQL) {
                $tableArray = array('staff', 'country');
            } else {
        ...
        while (($row = $this->q->fetchAssoc()) == true) {
            ...
            if ($this->model->getCountryId(0, 'single')) {
                ...
                $row['previousRecord'] = $this->previousRecord('value', $this->model->getCountryId(0, 'single'));
                $row['nextRecord'] = $this->nextRecord('value', $this->model->getCountryId(0, 'single'));
                            ...
                            array("[", "]"), "", json_encode(
                                    array(
                                        ...
                                        'previousRecord' => $this->previousRecord(
                                                'value', $this->model->getCountryId(0, 'single')
                                        ),
    ...
    function update() {
        ...
        if ($this->getVendor() == self::MYSQL) {
           ...
           FROM 	COUNTRY
           WHERE  	" . strtoupper($this->model->getPrimaryKeyName()) . " = '" . $this->model->getCountryId(
                                    0, 'single'
                            ) . "' ";
                }
            }
        ...
        if ($total == 0) {
            ...
            if ($this->getVendor() == self::MYSQL) {
               ...
               UPDATE `country` SET 
                       ...
                       `countryCurrencyCodeDescription` = '" . $this->model->getCountryCurrencyCodeDescription() . "',
                       `localeOne` = '" . $this->model->getLocaleOne() . "',
                       `localeTwo` = '" . $this->model->getLocaleTwo() . "',
                       `localeThree` = '" . $this->model->getLocaleThree() . "',
                       `localeFour` = '" . $this->model->getLocaleFour() . "',
                       `localeFive` = '" . $this->model->getLocaleFive() . "',
                       `countryDescription` = '" . $this->model->getCountryDescription() . "',
    ...
    function excel() {
        ...
        $this->excel->getActiveSheet()->setCellValue('C3', $this->translate['countryCodeLabel']);
        $this->excel->getActiveSheet()->setCellValue('D3', $this->translate['countryCurrencyCodeLabel']);
        $this->excel->getActiveSheet()->setCellValue('E3', $this->translate['countryCurrencyCodeDescriptionLabel']);
        $this->excel->getActiveSheet()->setCellValue('F3', $this->translate['localeOneLabel']);
        $this->excel->getActiveSheet()->setCellValue('G3', $this->translate['localeTwoLabel']);
        $this->excel->getActiveSheet()->setCellValue('H3', $this->translate['localeThreeLabel']);
        $this->excel->getActiveSheet()->setCellValue('I3', $this->translate['localeFourLabel']);
        $this->excel->getActiveSheet()->setCellValue('J3', $this->translate['localeFiveLabel']);
        $this->excel->getActiveSheet()->setCellValue('K3', $this->translate['countryDescriptionLabel']);
        ...
        while (($row = $this->q->fetchAssoc()) == true) {
            ...
            $this->excel->getActiveSheet()->setCellValue('C' . $loopRow, strip_tags($row ['countryCode']));
            $this->excel->getActiveSheet()->setCellValue('D' . $loopRow, strip_tags($row ['countryCurrencyCode']));
            $this->excel->getActiveSheet()->setCellValue(
                    'E' . $loopRow, strip_tags($row ['countryCurrencyCodeDescription'])
            );
            $this->excel->getActiveSheet()->setCellValue('F' . $loopRow, strip_tags($row ['localeOne']));
            $this->excel->getActiveSheet()->setCellValue('G' . $loopRow, strip_tags($row ['localeTwo']));
            $this->excel->getActiveSheet()->setCellValue('H' . $loopRow, strip_tags($row ['localeThree']));
            $this->excel->getActiveSheet()->setCellValue('I' . $loopRow, strip_tags($row ['localeFour']));
            $this->excel->getActiveSheet()->setCellValue('J' . $loopRow, strip_tags($row ['localeFive']));
            $this->excel->getActiveSheet()->setCellValue('K' . $loopRow, strip_tags($row ['countryDescription']));
        ...
        switch ($this->getReportMode()) {
            case 'excel':
                ...
                $extension = '.xlsx';
                $folder = 'excel';
                $filename = "country" . rand(0, 10000000) . $extension;
...
if (isset($_POST ['method'])) {
    if (isset($_POST['output'])) {
        ...
        if ($_POST['securityToken'] != $countryObject->getSecurityToken()) {
            header('Content-Type:application/json; charset=utf-8');
        ...
        if (isset($_POST ['offset'])) {
            $countryObject->setStart($_POST ['offset']);
        }
        if (isset($_POST ['limit'])) {
            $countryObject->setLimit($_POST ['limit']);
        }
        $countryObject->setPageOutput($_POST['output']);
...
if (isset($_GET ['method'])) {
    $countryObject = new CountryClass ();
    if ($_GET['securityToken'] != $countryObject->getSecurityToken()) {
        header('Content-Type:application/json; charset=utf-8');
        echo json_encode(array("success" => false, "message" => "Something wrong with the system.Hola hackers"));
        exit();
    }
     ...
     */
    if (isset($_GET ['mode'])) {
        $countryObject->setReportMode($_GET['mode']);
    ...
    if (isset($_GET ['filter'])) {
        $countryObject->setServiceOutput('option');
    }
}
?>
