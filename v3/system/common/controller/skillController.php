...
namespace Core\System\Common\Skill\Controller;

use Core\ConfigClass;
use Core\Document\Trail\DocumentTrailClass;
use Core\RecordSet\RecordSet;
use Core\shared\SharedClass;
use Core\System\Common\Skill\Model\SkillModel;
use Core\System\Common\Skill\Service\SkillService;
...
require_once($newFakeDocumentRoot . "v3/system/common/model/skillModel.php");
require_once($newFakeDocumentRoot . "v3/system/common/service/skillService.php");

/**
 * Class Skill
 * this is skill controller files.
 * @name IDCMS
 * @version 2
 * @author hafizan
 * @package  Core\System\Common\Skill\Controller
 * @subpackage Common
 * @link http://www.hafizan.com
 * @license http://www.gnu.org/copyleft/lesser.html LGPL
 */
class SkillClass extends ConfigClass {
    ...
    /**
     * Model
     * @var \Core\System\Common\Skill\Model\SkillModel
     */
    public $model;

    /**
     * Service-Business Application Process or other ajax request
     * @var \Core\System\Common\Skill\Service\SkillService
     */
    public $service;
    ...
    function __construct() {
        ...
        $this->setViewPath("./v3/system/common/view/skill.php");
        $this->setControllerPath("./v3/system/common/controller/skillController.php");
        $this->setServicePath("./v3/system/common/service/skillService.php");
    }
    ...
    public function create() {
        ...
        if ($this->getVendor() == self::MYSQL) {
            ...
            (
                 ...
                 '" . $this->getCompanyId() . "',
                 '" . $this->model->getSkillCode() . "',
                 '" . $this->model->getSkillDescription() . "',
    ...
    public function read() {
        ...
        if (isset($_SESSION['isAdmin'])) {
            if ($_SESSION['isAdmin'] == 0) {
                if ($this->getVendor() == self::MYSQL) {
                    $this->setAuditFilter(
                            " `skill`.`isActive` = 1  AND `skill`.`companyId`='" . $this->getCompanyId() . "' "
                    );
                } else {
                    if ($this->getVendor() == self::MSSQL) {
                        $this->setAuditFilter(
                ...
                if ($_SESSION['isAdmin'] == 1) {
                    if ($this->getVendor() == self::MYSQL) {
                        $this->setAuditFilter("   `skill`.`companyId`='" . $this->getCompanyId() . "'	");
                    } else {
                        if ($this->getVendor() == self::MSSQL) {
                            $this->setAuditFilter(" [skill].[companyId]='" . $this->getCompanyId() . "' ");
                        } else {
       ...
       SELECT                    `skill`.`skillId`,
		  ...
		  FROM      `skill`
		  JOIN      `staff`
		  ON        `skill`.`executeBy` = `staff`.`staffId`
	JOIN	`company`
	ON		`company`.`companyId` = `skill`.`companyId`
		  WHERE     " . $this->getAuditFilter();
            if ($this->model->getSkillId(0, 'single')) {
                $sql .= " AND `skill`.`" . $this->model->getPrimaryKeyName() . "`='" . $this->model->getSkillId(
                                0, 'single'
                        ) . "'";
            }
        } else {
	...
	ON		[company].[companyId] = [skill].[companyId]
		  WHERE     " . $this->getAuditFilter();
                if ($this->model->getSkillId(0, 'single')) {
                    $sql .= " AND [skill].[" . $this->model->getPrimaryKeyName(
                            ) . "]		=	'" . $this->model->getSkillId(0, 'single') . "'";
                }
            } else {
                if ($this->getVendor() == self::ORACLE) {

                    $sql = "
		  SELECT                    SKILL.SKILLID AS \"skillId\",
                    COMPANY.COMPANYDESCRIPTION AS  \"companyDescription\",
                    SKILL.COMPANYID AS \"companyId\",
                    SKILL.SKILLCODE AS \"skillCode\",
                    SKILL.SKILLDESCRIPTION AS \"skillDescription\",
                    SKILL.ISDEFAULT AS \"isDefault\",
                    SKILL.ISNEW AS \"isNew\",
                    SKILL.ISDRAFT AS \"isDraft\",
                    SKILL.ISUPDATE AS \"isUpdate\",
                    SKILL.ISDELETE AS \"isDelete\",
                    SKILL.ISACTIVE AS \"isActive\",
                    SKILL.ISAPPROVED AS \"isApproved\",
                    SKILL.ISREVIEW AS \"isReview\",
                    SKILL.ISPOST AS \"isPost\",
                    SKILL.EXECUTEBY AS \"executeBy\",
                    SKILL.EXECUTETIME AS \"executeTime\",
	...
	ON		COMPANY.COMPANYID = SKILL.COMPANYID
        ...
        if ($this->getCharacterQuery()) {
            if ($this->getVendor() == self::MYSQL) {
                $sql .= " AND `skill`.`" . $this->model->getFilterCharacter() . "` like '" . $this->getCharacterQuery(
                        ) . "%'";
            } else {
                if ($this->getVendor() == self::MSSQL) {
                    $sql .= " AND [skill].[" . $this->model->getFilterCharacter(
                            ) . "] like '" . $this->getCharacterQuery() . "%'";
                } else {
                    if ($this->getVendor() == self::ORACLE) {
                        $sql .= " AND Initcap(SKILL." . strtoupper(
        ...
        if ($this->getDateRangeStartQuery()) {
            if ($this->getVendor() == self::MYSQL) {
                $sql .= $this->q->dateFilter(
                        'skill', $this->model->getFilterDate(), $this->getDateRangeStartQuery(), $this->getDateRangeEndQuery(), $this->getDateRangeTypeQuery(), $this->getDateRangeExtraTypeQuery()
                );
            } else {
                if ($this->getVendor() == self::MSSQL) {
                    $sql .= $this->q->dateFilter(
        ...
        while (($row = $this->q->fetchAssoc()) == true) {
            ...
            if ($this->model->getSkillId(0, 'single')) {
                ...
                $row['previousRecord'] = $this->previousRecord('value', $this->model->getSkillId(0, 'single'));
                $row['nextRecord'] = $this->nextRecord('value', $this->model->getSkillId(0, 'single'));
                            ...
                            array("[", "]"), "", json_encode(
                                    array(
                                        ...
                                        'previousRecord' => $this->previousRecord(
                                                'value', $this->model->getSkillId(0, 'single')
                                        ),
                                        'nextRecord' => $this->nextRecord('value', $this->model->getSkillId(0, 'single')),
    ...
    function update() {
        ...
        if ($this->getVendor() == self::MYSQL) {
           ...
           FROM 	`skill`
           WHERE  	`" . $this->model->getPrimaryKeyName() . "` = '" . $this->model->getSkillId(0, 'single') . "' ";
        } else {
            if ($this->getVendor() == self::MSSQL) {
                $sql = "
           SELECT	[" . $this->model->getPrimaryKeyName() . "]
           ...
           FROM 	SKILL
           WHERE  	" . strtoupper($this->model->getPrimaryKeyName()) . " = '" . $this->model->getSkillId(
                                    0, 'single'
                            ) . "' ";
                }
            }
        ...
        if ($total == 0) {
            ...
            if ($this->getVendor() == self::MYSQL) {
                $sql = "
               UPDATE `skill` SET
                       `skillCode` = '" . $this->model->getSkillCode() . "',
                       `skillDescription` = '" . $this->model->getSkillDescription() . "',
               ...
               WHERE    `skillId`='" . $this->model->getSkillId('0', 'single') . "'";
            } else {
                if ($this->getVendor() == self::MSSQL) {
                    $sql = "
                UPDATE [skill] SET
    ...
    function duplicate() {
        ...
        if ($this->getVendor() == self::MYSQL) {
           ...
           SELECT  `skillCode`
           FROM    `skill`
           WHERE   `skillCode` 	= 	'" . $this->model->getSkillCode() . "'
           AND     `isActive`  =   1
    ...
    function excel() {
        ...
        $this->excel->getActiveSheet()->setCellValue('C3', $this->translate['skillCodeLabel']);
        $this->excel->getActiveSheet()->setCellValue('D3', $this->translate['skillDescriptionLabel']);
        ...
        while (($row = $this->q->fetchAssoc()) == true) {
            ...
            $this->excel->getActiveSheet()->setCellValue('C' . $loopRow, strip_tags($row ['skillCode']));
            $this->excel->getActiveSheet()->setCellValue('D' . $loopRow, strip_tags($row ['skillDescription']));
        ...
        switch ($this->getReportMode()) {
            case 'excel':
                ...
                $extension = '.xlsx';
                $folder = 'excel';
                $filename = "skill" . rand(0, 10000000) . $extension;
...
if (isset($_POST ['method'])) {
    if (isset($_POST['output'])) {
        $skillObject = new SkillClass ();
        if ($_POST['securityToken'] != $skillObject->getSecurityToken()) {
            header('Content-Type:application/json; charset=utf-8');
        ...
        if (isset($_POST ['leafId'])) {
            $skillObject->setLeafId($_POST ['leafId']);
        }
        if (isset($_POST ['offset'])) {
            $skillObject->setStart($_POST ['offset']);
        }
        if (isset($_POST ['leafId'])) {
            $skillObject->setLimit($_POST ['limit']);
        }
        $skillObject->setPageOutput($_POST['output']);
        $skillObject->execute();
        /*
...
if (isset($_GET ['method'])) {
    $skillObject = new SkillClass ();
    if ($_GET['securityToken'] != $skillObject->getSecurityToken()) {
        header('Content-Type:application/json; charset=utf-8');
        echo json_encode(array("success" => false, "message" => "Something wrong with the system.Hola hackers"));
        exit();
    }
    ...
    if (isset($_GET ['leafId'])) {
        $skillObject->setLeafId($_GET ['leafId']);
    }
    /*
     * Admin Only
     */
    ...
    if (isset($_GET ['databaseRequest'])) {
        $skillObject->setRequestDatabase($_GET ['databaseRequest']);
    }
    /*
     *  Load the dynamic value
     */
    ...
    if ($_GET ['method'] == 'updateStatus') {
        $skillObject->updateStatus();
    }
    /*
     *  Checking Any Duplication  Key
     */
    if ($_GET['method'] == 'duplicate') {
        $skillObject->duplicate();
    }
    if ($_GET ['method'] == 'dataNavigationRequest') {
        if ($_GET ['dataNavigation'] == 'firstRecord') {
            $skillObject->firstRecord('json');
        }
        if ($_GET ['dataNavigation'] == 'previousRecord') {
            $skillObject->previousRecord('json', 0);
        }
        if ($_GET ['dataNavigation'] == 'nextRecord') {
            $skillObject->nextRecord('json', 0);
        }
        if ($_GET ['dataNavigation'] == 'lastRecord') {
            $skillObject->lastRecord('json');
        }
    }
    /*
     * Excel Reporting
     */
    if (isset($_GET ['mode'])) {
        $skillObject->setReportMode($_GET['mode']);
    ...
    if (isset($_GET ['filter'])) {
        $skillObject->setServiceOutput('option');
    }
}
?>
