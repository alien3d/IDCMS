...
namespace Core\System\Common\Religion\Controller;

use Core\ConfigClass;
use Core\Document\Trail\DocumentTrailClass;
use Core\RecordSet\RecordSet;
use Core\shared\SharedClass;
use Core\System\Common\Religion\Model\ReligionModel;
use Core\System\Common\Religion\Service\ReligionService;
...
require_once($newFakeDocumentRoot . "v3/system/common/model/religionModel.php");
require_once($newFakeDocumentRoot . "v3/system/common/service/religionService.php");

/**
 * Class Religion
 * this is religion controller files.
 * @name IDCMS
 * @version 2
 * @author hafizan
 * @package  Core\System\Common\Religion\Controller
 * @subpackage Common
...
class ReligionClass extends ConfigClass {
    ...
    /**
     * Model
     * @var \Core\System\Common\Religion\Model\ReligionModel
     */
    public $model;

    /**
     * Service-Business Application Process or other ajax request
     * @var \Core\System\Common\Religion\Service\ReligionService
     */
    public $service;
    ...
    function __construct() {
        ...
        $this->setViewPath("./v3/system/common/view/religion.php");
        $this->setControllerPath("./v3/system/common/controller/religionController.php");
        $this->setServicePath("./v3/system/common/service/religionService.php");
    }
    ...
    public function create() {
        ...
        if ($this->getVendor() == self::MYSQL) {
            ...
            (
                 ...
                 '" . $this->getCompanyId() . "',
                 '" . $this->model->getReligionCode() . "',
                 '" . $this->model->getReligionDescription() . "',
    ...
    public function read() {
        ...
        if (isset($_SESSION['isAdmin'])) {
            if ($_SESSION['isAdmin'] == 0) {
                if ($this->getVendor() == self::MYSQL) {
                    $this->setAuditFilter(
                            " `religion`.`isActive` = 1  AND `religion`.`companyId`='" . $this->getCompanyId() . "' "
                    );
                } else {
                    if ($this->getVendor() == self::MSSQL) {
                        $this->setAuditFilter(
                ...
                if ($_SESSION['isAdmin'] == 1) {
                    if ($this->getVendor() == self::MYSQL) {
                        $this->setAuditFilter("   `religion`.`companyId`='" . $this->getCompanyId() . "'	");
                    } else {
                        if ($this->getVendor() == self::MSSQL) {
                            $this->setAuditFilter(" [religion].[companyId]='" . $this->getCompanyId() . "' ");
                        } else {
	...
	JOIN	`company`
	ON		`company`.`companyId` = `religion`.`companyId`
		  WHERE     " . $this->getAuditFilter();
            if ($this->model->getReligionId(0, 'single')) {
                $sql .= " AND `religion`.`" . $this->model->getPrimaryKeyName() . "`='" . $this->model->getReligionId(
                                0, 'single'
                        ) . "'";
            }
        } else {
	...
	ON		[company].[companyId] = [religion].[companyId]
		  WHERE     " . $this->getAuditFilter();
                if ($this->model->getReligionId(0, 'single')) {
                    $sql .= " AND [religion].[" . $this->model->getPrimaryKeyName(
		  ...
		  SELECT                    RELIGION.RELIGIONID AS \"religionId\",
                    ...
                    RELIGION.COMPANYID AS \"companyId\",
                    RELIGION.RELIGIONCODE AS \"religionCode\",
	...
	ON		COMPANY.COMPANYID = RELIGION.COMPANYID
        ...
        if ($this->getCharacterQuery()) {
            if ($this->getVendor() == self::MYSQL) {
                $sql .= " AND `religion`.`" . $this->model->getFilterCharacter(
                ...
                if ($this->getVendor() == self::MSSQL) {
                    ...
                    if ($this->getVendor() == self::ORACLE) {
                        $sql .= " AND Initcap(RELIGION." . strtoupper(
        ...
        if ($this->getDateRangeStartQuery()) {
            if ($this->getVendor() == self::MYSQL) {
                $sql .= $this->q->dateFilter(
                        'religion', $this->model->getFilterDate(), $this->getDateRangeStartQuery(), $this->getDateRangeEndQuery(), $this->getDateRangeTypeQuery(), $this->getDateRangeExtraTypeQuery()
                );
            } else {
                if ($this->getVendor() == self::MSSQL) {
                    $sql .= $this->q->dateFilter(
        ...
        $tableArray = null;
        if ($this->getVendor() == self::MYSQL) {
            $tableArray = array('staff', 'religion');
        } else {
            if ($this->getVendor() == self::MSSQL) {
                $tableArray = array('staff', 'religion');
            } else {
        ...
        while (($row = $this->q->fetchAssoc()) == true) {
            ...
            if ($this->model->getReligionId(0, 'single')) {
                ...
                $row['previousRecord'] = $this->previousRecord('value', $this->model->getReligionId(0, 'single'));
                $row['nextRecord'] = $this->nextRecord('value', $this->model->getReligionId(0, 'single'));
                            ...
                            array("[", "]"), "", json_encode(
                                    array(
                                        ...
                                        'previousRecord' => $this->previousRecord(
                                                'value', $this->model->getReligionId(0, 'single')
                                        ),
    ...
    function update() {
        ...
        if ($this->getVendor() == self::MYSQL) {
           ...
           FROM 	`religion`
           WHERE  	`" . $this->model->getPrimaryKeyName() . "` = '" . $this->model->getReligionId(
                            0, 'single'
                    ) . "' ";
        } else {
            if ($this->getVendor() == self::MSSQL) {
           ...
           FROM 	RELIGION
           WHERE  	" . strtoupper($this->model->getPrimaryKeyName()) . " = '" . $this->model->getReligionId(
                                    0, 'single'
                            ) . "' ";
                }
            }
        ...
        if ($total == 0) {
            ...
            if ($this->getVendor() == self::MYSQL) {
                $sql = "
               UPDATE `religion` SET
                       `religionCode` = '" . $this->model->getReligionCode() . "',
                       `religionDescription` = '" . $this->model->getReligionDescription() . "',
    ...
    function duplicate() {
        ...
        if ($this->getVendor() == self::MYSQL) {
           ...
           SELECT  `religionCode`
           FROM    `religion`
           WHERE   `religionCode` 	= 	'" . $this->model->getReligionCode() . "'
           AND     `isActive`  =   1
           ...
           AND     [companyId] =	'" . $this->getCompanyId() . "'";
                ...
                if ($this->getVendor() == self::ORACLE) {
                    $sql = "
               SELECT  RELIGIONCODE as \"religionCode\"
               FROM    RELIGION
               WHERE   RELIGIONCODE	= 	'" . $this->model->getReligionCode() . "'
               AND     ISACTIVE    =   1
               AND     COMPANYID   =   '" . $this->getCompanyId() . "'";
    ...
    function excel() {
        ...
        $this->excel->getActiveSheet()->setCellValue('C3', $this->translate['religionCodeLabel']);
        $this->excel->getActiveSheet()->setCellValue('D3', $this->translate['religionDescriptionLabel']);
        ...
        while (($row = $this->q->fetchAssoc()) == true) {
            ...
            $this->excel->getActiveSheet()->setCellValue('C' . $loopRow, strip_tags($row ['religionCode']));
            $this->excel->getActiveSheet()->setCellValue('D' . $loopRow, strip_tags($row ['religionDescription']));
        ...
        switch ($this->getReportMode()) {
            case 'excel':
                ...
                $extension = '.xlsx';
                $folder = 'excel';
                $filename = "religion" . rand(0, 10000000) . $extension;
...
if (isset($_POST ['method'])) {
    if (isset($_POST['output'])) {
        ...
        if ($_POST['securityToken'] != $religionObject->getSecurityToken()) {
            header('Content-Type:application/json; charset=utf-8');
        ...
        if (isset($_POST ['offset'])) {
            $religionObject->setStart($_POST ['offset']);
        }
        if (isset($_POST ['leafId'])) {
            $religionObject->setLimit($_POST ['limit']);
        }
        $religionObject->setPageOutput($_POST['output']);
...
if (isset($_GET ['method'])) {
    $religionObject = new ReligionClass ();
    if ($_GET['securityToken'] != $religionObject->getSecurityToken()) {
        header('Content-Type:application/json; charset=utf-8');
        echo json_encode(array("success" => false, "message" => "Something wrong with the system.Hola hackers"));
        exit();
    }
     ...
     */
    if (isset($_GET ['mode'])) {
        $religionObject->setReportMode($_GET['mode']);
    ...
    if (isset($_GET ['filter'])) {
        $religionObject->setServiceOutput('option');
    }
}
?>
