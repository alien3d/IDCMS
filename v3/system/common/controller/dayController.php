...
use Core\System\Common\Day\Model\DayModel;
use Core\System\Common\Day\Service\DayService;
...
require_once($newFakeDocumentRoot . "v3/system/common/model/dayModel.php");
require_once($newFakeDocumentRoot . "v3/system/common/service/dayService.php");

/**
 * Class Day
 * this is day controller files.
 * @name IDCMS
 * @version 2
 * @author hafizan
 * @package  Core\System\Common\Day\Controller
 * @subpackage Common
 * @link http://www.hafizan.com
 * @license http://www.gnu.org/copyleft/lesser.html LGPL
 */
class DayClass extends ConfigClass {
    ...
    /**
     * Model
     * @var \Core\System\Common\Day\Model\DayModel
     */
    public $model;

    /**
     * Service-Business Application Process or other ajax request
     * @var \Core\System\Common\Day\Service\DayService
     */
    public $service;
    ...
    function __construct() {
        ...
        $this->setViewPath("./v3/system/common/view/day.php");
        $this->setControllerPath("./v3/system/common/controller/dayController.php");
        $this->setServicePath("./v3/system/common/service/dayService.php");
    }
    ...
    public function read() {
        ...
        if (isset($_SESSION['isAdmin'])) {
            if ($_SESSION['isAdmin'] == 0) {
                if ($this->getVendor() == self::MYSQL) {
                    $this->setAuditFilter(
                            " `day`.`isActive` = 1  AND `day`.`companyId`='" . $this->getCompanyId() . "' "
                    );
                } else {
                    if ($this->getVendor() == self::MSSQL) {
                        $this->setAuditFilter(
                ...
                if ($_SESSION['isAdmin'] == 1) {
                    if ($this->getVendor() == self::MYSQL) {
                        $this->setAuditFilter("   `day`.`companyId`='" . $this->getCompanyId() . "'	");
                    } else {
                        if ($this->getVendor() == self::MSSQL) {
                            $this->setAuditFilter(" [day].[companyId]='" . $this->getCompanyId() . "' ");
                        } else {
	...
	JOIN	`company`
	ON		`company`.`companyId` = `day`.`companyId`
		  WHERE     " . $this->getAuditFilter();
            if ($this->model->getDayId(0, 'single')) {
                $sql .= " AND `day`.`" . $this->model->getPrimaryKeyName() . "`='" . $this->model->getDayId(
                                0, 'single'
                        ) . "'";
            }
        } else {
	...
	ON		[company].[companyId] = [day].[companyId]
		  WHERE     " . $this->getAuditFilter();
                if ($this->model->getDayId(0, 'single')) {
                    $sql .= " AND [day].[" . $this->model->getPrimaryKeyName(
                            ) . "]		=	'" . $this->model->getDayId(
                                    0, 'single'
                            ) . "'";
                }
                ...
                if ($this->getVendor() == self::ORACLE) {
                    ...
                    $sql = "
		  SELECT                    DAY.DAYID AS \"dayId\",
                    COMPANY.COMPANYDESCRIPTION AS  \"companyDescription\",
                    DAY.COMPANYID AS \"companyId\",
                    DAY.DAYNAME AS \"dayName\",
                    DAY.ISDEFAULT AS \"isDefault\",
                    DAY.ISNEW AS \"isNew\",
                    DAY.ISDRAFT AS \"isDraft\",
                    DAY.ISUPDATE AS \"isUpdate\",
                    DAY.ISDELETE AS \"isDelete\",
                    DAY.ISACTIVE AS \"isActive\",
                    DAY.ISAPPROVED AS \"isApproved\",
                    DAY.ISREVIEW AS \"isReview\",
                    DAY.ISPOST AS \"isPost\",
                    DAY.EXECUTEBY AS \"executeBy\",
                    DAY.EXECUTETIME AS \"executeTime\",
	...
	ON		COMPANY.COMPANYID = DAY.COMPANYID
        ...
        if ($this->getCharacterQuery()) {
            if ($this->getVendor() == self::MYSQL) {
                $sql .= " AND `day`.`" . $this->model->getFilterCharacter() . "` like '" . $this->getCharacterQuery(
                        ) . "%'";
            } else {
                if ($this->getVendor() == self::MSSQL) {
                    $sql .= " AND [day].[" . $this->model->getFilterCharacter() . "] like '" . $this->getCharacterQuery(
                    ...
                    if ($this->getVendor() == self::ORACLE) {
                        $sql .= " AND Initcap(DAY." . strtoupper(
        ...
        if ($this->getDateRangeStartQuery()) {
            if ($this->getVendor() == self::MYSQL) {
                $sql .= $this->q->dateFilter(
                        'day', $this->model->getFilterDate(), $this->getDateRangeStartQuery(), $this->getDateRangeEndQuery(), $this->getDateRangeTypeQuery(), $this->getDateRangeExtraTypeQuery()
                );
            } else {
                if ($this->getVendor() == self::MSSQL) {
                    $sql .= $this->q->dateFilter(
        ...
        while (($row = $this->q->fetchAssoc()) == true) {
            ...
            if ($this->model->getDayId(0, 'single')) {
                ...
                $row['previousRecord'] = $this->previousRecord('value', $this->model->getDayId(0, 'single'));
                $row['nextRecord'] = $this->nextRecord('value', $this->model->getDayId(0, 'single'));
                    ...
                    echo json_encode(
                            array(
                                ...
                                'previousRecord' => $this->recordSet->previousRecord(
                                        'value', $this->model->getDayId(0, 'single')
                                ),
    ...
    function update() {
        ...
        if ($this->getVendor() == self::MYSQL) {
           ...
           FROM 	DAY
           WHERE  	" . strtoupper($this->model->getPrimaryKeyName()) . " = '" . $this->model->getDayId(
                                    0, 'single'
                            ) . "' ";
                }
            }
   ...
   /** 
    ...
    function excel() {
        ...
        $this->excel->getActiveSheet()->setCellValue('C3', $this->translate['dayNameLabel']);
        ...
        while (($row = $this->q->fetchAssoc()) == true) {
            ...
            $this->excel->getActiveSheet()->setCellValue('C' . $loopRow, strip_tags($row ['dayName']));
        ...
        switch ($this->getReportMode()) {
            case 'excel':
                ...
                $extension = '.xlsx';
                $folder = 'excel';
                $filename = "day" . rand(0, 10000000) . $extension;
...
if (isset($_POST ['method'])) {
    if (isset($_POST['output'])) {
        ...
        if ($_POST['securityToken'] != $dayObject->getSecurityToken()) {
            header('Content-Type:application/json; charset=utf-8');
        ...
        if (isset($_POST ['offset'])) {
            $dayObject->setStart($_POST ['offset']);
        }
        if (isset($_POST ['limit'])) {
            $dayObject->setLimit($_POST ['limit']);
        }
        $dayObject->setPageOutput($_POST['output']);
        $dayObject->execute();
        /*
...
if (isset($_GET ['method'])) {
    $dayObject = new DayClass ();
    if ($_GET['securityToken'] != $dayObject->getSecurityToken()) {
        header('Content-Type:application/json; charset=utf-8');
        echo json_encode(array("success" => false, "message" => "Something wrong with the system.Hola hackers"));
        exit();
    }
     ...
     */
    if (isset($_GET ['mode'])) {
        $dayObject->setReportMode($_GET['mode']);
    ...
    if (isset($_GET ['filter'])) {
        $dayObject->setServiceOutput('option');
    }
}
?>
