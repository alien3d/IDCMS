...
namespace Core\System\Common\Salutation\Controller;

use Core\ConfigClass;
use Core\Document\Trail\DocumentTrailClass;
use Core\RecordSet\RecordSet;
use Core\shared\SharedClass;
use Core\System\Common\Salutation\Model\SalutationModel;
use Core\System\Common\Salutation\Service\SalutationService;
...
require_once($newFakeDocumentRoot . "v3/system/common/model/salutationModel.php");
require_once($newFakeDocumentRoot . "v3/system/common/service/salutationService.php");

/**
 * Class Salutation
 * this is salutation controller files.
 * @name IDCMS
 * @version 2
 * @author hafizan
 * @package  Core\System\Common\Salutation\Controller
 * @subpackage Common
...
class SalutationClass extends ConfigClass {
    ...
    /**
     * Model
     * @var \Core\System\Common\Salutation\Model\SalutationModel
     */
    public $model;

    /**
     * Service-Business Application Process or other ajax request
     * @var \Core\System\Common\Salutation\Service\SalutationService
     */
    public $service;
    ...
    function __construct() {
        ...
        $this->setViewPath("./v3/system/common/view/salutation.php");
        $this->setControllerPath("./v3/system/common/controller/salutationController.php");
        $this->setServicePath("./v3/system/common/service/salutationService.php");
    }
    ...
    function execute() {
        ...
        $this->service = new SalutationService();
    ...
    public function create() {
        ...
        if ($this->getVendor() == self::MYSQL) {
            ...
            (
                 ...
                 '" . $this->model->getSalutationDescription() . "',
    ...
    public function read() {
        ...
        if (isset($_SESSION['isAdmin'])) {
            if ($_SESSION['isAdmin'] == 0) {
                if ($this->getVendor() == self::MYSQL) {
                    $this->setAuditFilter(
                            " `salutation`.`isActive` = 1  AND `salutation`.`companyId`='" . $this->getCompanyId() . "' "
                    );
                } else {
                    if ($this->getVendor() == self::MSSQL) {
                        $this->setAuditFilter(
                ...
                if ($_SESSION['isAdmin'] == 1) {
                    if ($this->getVendor() == self::MYSQL) {
                        $this->setAuditFilter("   `salutation`.`companyId`='" . $this->getCompanyId() . "'	");
                    } else {
                        if ($this->getVendor() == self::MSSQL) {
                            $this->setAuditFilter(" [salutation].[companyId]='" . $this->getCompanyId() . "' ");
                        } else {
	...
	JOIN	`company`
	ON		`company`.`companyId` = `salutation`.`companyId`
		  WHERE     " . $this->getAuditFilter();
            if ($this->model->getSalutationId(0, 'single')) {
                $sql .= " AND `salutation`.`" . $this->model->getPrimaryKeyName(
                        ) . "`='" . $this->model->getSalutationId(0, 'single') . "'";
            }
        } else {
            if ($this->getVendor() == self::MSSQL) {
	...
	ON		[company].[companyId] = [salutation].[companyId]
		  ...
		  SELECT                    SALUTATION.SALUTATIONID AS \"salutationId\",
                    ...
                    SALUTATION.COMPANYID AS \"companyId\",
                    SALUTATION.SALUTATIONCODE AS \"salutationCode\",
                    SALUTATION.SALUTATIONDESCRIPTION AS \"salutationDescription\",
	...
	ON		COMPANY.COMPANYID = SALUTATION.COMPANYID
        ...
        if ($this->getCharacterQuery()) {
            if ($this->getVendor() == self::MYSQL) {
                $sql .= " AND `salutation`.`" . $this->model->getFilterCharacter(
                ...
                if ($this->getVendor() == self::MSSQL) {
                    ...
                    if ($this->getVendor() == self::ORACLE) {
                        $sql .= " AND Initcap(SALUTATION." . strtoupper(
        ...
        if ($this->getDateRangeStartQuery()) {
            if ($this->getVendor() == self::MYSQL) {
                $sql .= $this->q->dateFilter(
                        'salutation', $this->model->getFilterDate(), $this->getDateRangeStartQuery(), $this->getDateRangeEndQuery(), $this->getDateRangeTypeQuery(), $this->getDateRangeExtraTypeQuery()
                );
            } else {
                if ($this->getVendor() == self::MSSQL) {
                    $sql .= $this->q->dateFilter(
        ...
        $tableArray = null;
        if ($this->getVendor() == self::MYSQL) {
            $tableArray = array('staff', 'salutation');
        } else {
            if ($this->getVendor() == self::MSSQL) {
                $tableArray = array('staff', 'salutation');
            } else {
        ...
        while (($row = $this->q->fetchAssoc()) == true) {
            ...
            if ($this->model->getSalutationId(0, 'single')) {
                ...
                $row['previousRecord'] = $this->previousRecord('value', $this->model->getSalutationId(0, 'single'));
                $row['nextRecord'] = $this->nextRecord('value', $this->model->getSalutationId(0, 'single'));
                            ...
                            array("[", "]"), "", json_encode(
                                    array(
                                        ...
                                        'previousRecord' => $this->previousRecord(
                                                'value', $this->model->getSalutationId(0, 'single')
                                        ),
                                        'nextRecord' => $this->nextRecord('value', $this->model->getSalutationId(0, 'single')),
    ...
    function update() {
        ...
        if ($this->getVendor() == self::MYSQL) {
           ...
           FROM 	`salutation`
           WHERE  	`" . $this->model->getPrimaryKeyName() . "` = '" . $this->model->getSalutationId(
                            0, 'single'
                    ) . "' ";
        } else {
            if ($this->getVendor() == self::MSSQL) {
           ...
           FROM 	SALUTATION
           WHERE  	" . strtoupper($this->model->getPrimaryKeyName()) . " = '" . $this->model->getSalutationId(
                                    0, 'single'
                            ) . "' ";
                }
            }
        ...
        if ($total == 0) {
            ...
            if ($this->getVendor() == self::MYSQL) {
                $sql = "
               UPDATE `salutation` SET
                       `salutationCode` = '" . $this->model->getSalutationCode() . "',
                       `salutationDescription` = '" . $this->model->getSalutationDescription() . "',
    ...
    function duplicate() {
        ...
        if ($this->getVendor() == self::MYSQL) {
           ...
           AND     [companyId] =	'" . $this->getCompanyId() . "'";
                ...
                if ($this->getVendor() == self::ORACLE) {
                    $sql = "
               SELECT  SALUTATIONCODE as \"salutationCode\"
               FROM    SALUTATION
               WHERE   SALUTATIONCODE	= 	'" . $this->model->getSalutationCode() . "'
               AND     ISACTIVE    =   1
               AND     COMPANYID   =   '" . $this->getCompanyId() . "'";
    ...
    function excel() {
        ...
        $this->excel->getActiveSheet()->setCellValue('C3', $this->translate['salutationCodeLabel']);
        $this->excel->getActiveSheet()->setCellValue('D3', $this->translate['salutationDescriptionLabel']);
        ...
        while (($row = $this->q->fetchAssoc()) == true) {
            ...
            $this->excel->getActiveSheet()->setCellValue('C' . $loopRow, strip_tags($row ['salutationCode']));
            $this->excel->getActiveSheet()->setCellValue('D' . $loopRow, strip_tags($row ['salutationDescription']));
        ...
        switch ($this->getReportMode()) {
            case 'excel':
                ...
                $extension = '.xlsx';
                $folder = 'excel';
                $filename = "salutation" . rand(0, 10000000) . $extension;
...
if (isset($_POST ['method'])) {
    if (isset($_POST['output'])) {
        ...
        if ($_POST['securityToken'] != $salutationObject->getSecurityToken()) {
            header('Content-Type:application/json; charset=utf-8');
        ...
        if (isset($_POST ['offset'])) {
            $salutationObject->setStart($_POST ['offset']);
        }
        if (isset($_POST ['leafId'])) {
            $salutationObject->setLimit($_POST ['limit']);
        }
        $salutationObject->setPageOutput($_POST['output']);
...
if (isset($_GET ['method'])) {
    $salutationObject = new SalutationClass ();
    if ($_GET['securityToken'] != $salutationObject->getSecurityToken()) {
        header('Content-Type:application/json; charset=utf-8');
        echo json_encode(array("success" => false, "message" => "Something wrong with the system.Hola hackers"));
        exit();
    }
    ...
    if (isset($_GET ['databaseRequest'])) {
        $salutationObject->setRequestDatabase($_GET ['databaseRequest']);
    }
    /*
     *  Load the dynamic value
     */
     ...
     */
    if (isset($_GET ['mode'])) {
        $salutationObject->setReportMode($_GET['mode']);
    ...
    if (isset($_GET ['filter'])) {
        $salutationObject->setServiceOutput('option');
    }
}
?>
