...
namespace Core\System\Common\City\Controller;

use Core\ConfigClass;
use Core\Document\Trail\DocumentTrailClass;
use Core\RecordSet\RecordSet;
use Core\shared\SharedClass;
use Core\System\Common\City\Model\CityModel;
use Core\System\Common\City\Service\CityService;
...
require_once($newFakeDocumentRoot . "v3/system/common/model/cityModel.php");
require_once($newFakeDocumentRoot . "v3/system/common/service/cityService.php");

/**
 * Class City
 * this is city controller files.
 * @name IDCMS
 * @version 2
 * @author hafizan
 * @package  Core\System\Common\City\Controller
 * @subpackage Common
 * @link http://www.hafizan.com
 * @license http://www.gnu.org/copyleft/lesser.html LGPL
 */
class CityClass extends ConfigClass {
    ...
    /**
     * Model
     * @var \Core\System\Common\City\Model\CityModel
     */
    public $model;

    /**
     * Service-Business Application Process or other ajax request
     * @var \Core\System\Common\City\Service\CityService
     */
    public $service;
    ...
    function __construct() {
        ...
        $this->setViewPath("./v3/system/common/view/city.php");
        $this->setControllerPath("./v3/system/common/controller/cityController.php");
        $this->setServicePath("./v3/system/common/service/cityService.php");
    }
    ...
    public function create() {
        ...
        if ($this->getVendor() == self::MYSQL) {
            ...
            (
                 ...
                 '" . $this->model->getDistrictId() . "',
                 '" . $this->model->getCityCode() . "',
                 '" . $this->model->getCityDescription() . "',
    ...
    public function read() {
        ...
        if (isset($_SESSION['isAdmin'])) {
            if ($_SESSION['isAdmin'] == 0) {
                if ($this->getVendor() == self::MYSQL) {
                    $this->setAuditFilter(
                            " `city`.`isActive` = 1  AND `city`.`companyId`='" . $this->getCompanyId() . "' "
                    );
                } else {
                    if ($this->getVendor() == self::MSSQL) {
                        $this->setAuditFilter(
                ...
                if ($_SESSION['isAdmin'] == 1) {
                    if ($this->getVendor() == self::MYSQL) {
                        $this->setAuditFilter("   `city`.`companyId`='" . $this->getCompanyId() . "'	");
                    } else {
                        if ($this->getVendor() == self::MSSQL) {
                            $this->setAuditFilter(" [city].[companyId]='" . $this->getCompanyId() . "' ");
                        } else {
       ...
       SELECT                    `city`.`cityId`,
		  ...
		  FROM      `city`
		  JOIN      `staff`
		  ON        `city`.`executeBy` = `staff`.`staffId`
	JOIN	`company`
	ON		`company`.`companyId` = `city`.`companyId`
	JOIN	`country`
	ON		`country`.`countryId` = `city`.`countryId`
	JOIN	`state`
	ON		`state`.`stateId` = `city`.`stateId`
		  WHERE     " . $this->getAuditFilter();
            if ($this->model->getCityId(0, 'single')) {
                $sql .= " AND `city`.`" . $this->model->getPrimaryKeyName() . "`='" . $this->model->getCityId(
                                0, 'single'
                        ) . "'";
            }
            if ($this->model->getCountryId()) {
                $sql .= " AND `city`.`countryId`='" . $this->model->getCountryId() . "'";
            }
            if ($this->model->getStateId()) {
                $sql .= " AND `city`.`stateId`='" . $this->model->getStateId() . "'";
            }
        } else {
            if ($this->getVendor() == self::MSSQL) {
	...
	ON		[state].[stateId] = [city].[stateId]
		  WHERE     " . $this->getAuditFilter();
                if ($this->model->getCityId(0, 'single')) {
                    $sql .= " AND [city].[" . $this->model->getPrimaryKeyName(
                            ) . "]		=	'" . $this->model->getCityId(0, 'single') . "'";
                }
                if ($this->model->getCountryId()) {
                    $sql .= " AND [city].[countryId]='" . $this->model->getCountryId() . "'";
                }
		  ...
		  SELECT                    CITY.CITYID AS \"cityId\",
                    ...
                    CITY.DISTRICTID AS \"districtId\",
                    CITY.CITYCODE AS \"cityCode\",
                    CITY.CITYDESCRIPTION AS \"cityDescription\",
                    CITY.ISDEFAULT AS \"isDefault\",
                    CITY.ISNEW AS \"isNew\",
                    CITY.ISDRAFT AS \"isDraft\",
                    CITY.ISUPDATE AS \"isUpdate\",
                    CITY.ISDELETE AS \"isDelete\",
                    CITY.ISACTIVE AS \"isActive\",
                    CITY.ISAPPROVED AS \"isApproved\",
                    CITY.ISREVIEW AS \"isReview\",
                    CITY.ISPOST AS \"isPost\",
                    CITY.EXECUTEBY AS \"executeBy\",
                    CITY.EXECUTETIME AS \"executeTime\",
	...
	ON		STATE.STATEID = CITY.STATEID
        ...
        if ($this->getCharacterQuery()) {
            if ($this->getVendor() == self::MYSQL) {
                $sql .= " AND `city`.`" . $this->model->getFilterCharacter() . "` like '" . $this->getCharacterQuery(
                        ) . "%'";
            } else {
                if ($this->getVendor() == self::MSSQL) {
                    $sql .= " AND [city].[" . $this->model->getFilterCharacter(
                            ) . "] like '" . $this->getCharacterQuery() . "%'";
                } else {
                    if ($this->getVendor() == self::ORACLE) {
                        $sql .= " AND Initcap(CITY." . strtoupper(
        ...
        if ($this->getDateRangeStartQuery()) {
            if ($this->getVendor() == self::MYSQL) {
                $sql .= $this->q->dateFilter(
                        'city', $this->model->getFilterDate(), $this->getDateRangeStartQuery(), $this->getDateRangeEndQuery(), $this->getDateRangeTypeQuery(), $this->getDateRangeExtraTypeQuery()
                );
            } else {
                if ($this->getVendor() == self::MSSQL) {
                    $sql .= $this->q->dateFilter(
        ...
        $tableArray = null;
        if ($this->getVendor() == self::MYSQL) {
            $tableArray = array('staff', 'city', 'company', 'country', 'state');
        } else {
            if ($this->getVendor() == self::MSSQL) {
                $tableArray = array('staff', 'city', 'company', 'country', 'state');
            } else {
        ...
        while (($row = $this->q->fetchAssoc()) == true) {
            ...
            if ($this->model->getCityId(0, 'single')) {
                ...
                $row['previousRecord'] = $this->previousRecord('value', $this->model->getCityId(0, 'single'));
                $row['nextRecord'] = $this->nextRecord('value', $this->model->getCityId(0, 'single'));
                ...
                if ($this->model->getCityId(0, 'single')) {
                            ...
                            array("[", "]"), "", json_encode(
                                    array(
                                        ...
                                        'previousRecord' => $this->previousRecord(
                                                'value', $this->model->getCityId(0, 'single')
                                        ),
                                        'nextRecord' => $this->nextRecord('value', $this->model->getCityId(0, 'single')),
                    ...
                    echo json_encode(
                            array(
                                ...
                                'nextRecord' => $this->recordSet->nextRecord('value', $this->model->getCityId(0, 'single')),
    ...
    function update() {
        ...
        if ($this->getVendor() == self::MYSQL) {
           ...
           FROM 	`city`
           WHERE  	`" . $this->model->getPrimaryKeyName() . "` = '" . $this->model->getCityId(0, 'single') . "' ";
        } else {
            if ($this->getVendor() == self::MSSQL) {
                $sql = "
           SELECT	[" . $this->model->getPrimaryKeyName() . "]
           ...
           FROM 	CITY
           WHERE  	" . strtoupper($this->model->getPrimaryKeyName()) . " = '" . $this->model->getCityId(
                                    0, 'single'
                            ) . "' ";
                }
            }
        ...
        if ($total == 0) {
            ...
            if ($this->getVendor() == self::MYSQL) {
               ...
               UPDATE `city` SET 
                       ...
                       `districtId` = '" . $this->model->getDistrictId() . "',
                       `cityCode` = '" . $this->model->getCityCode() . "',
                       `cityDescription` = '" . $this->model->getCityDescription() . "',
               ...
               WHERE    `cityId`='" . $this->model->getCityId('0', 'single') . "'";
            } else {
                if ($this->getVendor() == self::MSSQL) {
                    $sql = "
                UPDATE [city] SET
  ...
  /** 
    ...
    function duplicate() {
        ...
        if ($this->getVendor() == self::MYSQL) {
           ...
           SELECT  `cityCode` 
           FROM    `city` 
           WHERE   `cityCode` 	= 	'" . $this->model->getCityCode() . "' 
           AND     `isActive`  =   1
    ...
    function excel() {
        ...
        $this->excel->getActiveSheet()->setCellValue('E3', $this->translate['districtIdLabel']);
        $this->excel->getActiveSheet()->setCellValue('F3', $this->translate['cityCodeLabel']);
        $this->excel->getActiveSheet()->setCellValue('G3', $this->translate['cityDescriptionLabel']);
        ...
        while (($row = $this->q->fetchAssoc()) == true) {
            ...
            $this->excel->getActiveSheet()->setCellValue('E' . $loopRow, strip_tags($row ['districtDescription']));
            $this->excel->getActiveSheet()->setCellValue('F' . $loopRow, strip_tags($row ['cityCode']));
            $this->excel->getActiveSheet()->setCellValue('G' . $loopRow, strip_tags($row ['cityDescription']));
        ...
        switch ($this->getReportMode()) {
            case 'excel':
                ...
                $extension = '.xlsx';
                $folder = 'excel';
                $filename = "city" . rand(0, 10000000) . $extension;
...
if (isset($_POST ['method'])) {
    if (isset($_POST['output'])) {
        $cityObject = new CityClass ();
        if ($_POST['securityToken'] != $cityObject->getSecurityToken()) {
            header('Content-Type:application/json; charset=utf-8');
        ...
        if (isset($_POST ['leafId'])) {
            $cityObject->setLeafId($_POST ['leafId']);
        }
        $cityObject->setPageOutput($_POST['output']);
        $cityObject->execute();
        /*
...
if (isset($_GET ['method'])) {
    $cityObject = new CityClass ();
    if ($_GET['securityToken'] != $cityObject->getSecurityToken()) {
        header('Content-Type:application/json; charset=utf-8');
        echo json_encode(array("success" => false, "message" => "Something wrong with the system.Hola hackers"));
        exit();
    }
    ...
    if (isset($_GET ['leafId'])) {
        $cityObject->setLeafId($_GET ['leafId']);
    }
    /*
     * Admin Only
     */
    if (isset($_GET ['isAdmin'])) {
        $cityObject->setIsAdmin($_GET ['isAdmin']);
    }
    /**
     * Database Request
     */
    if (isset($_GET ['databaseRequest'])) {
        $cityObject->setRequestDatabase($_GET ['databaseRequest']);
    }
    /*
     *  Load the dynamic value
     */
    ...
    if ($_GET ['method'] == 'updateStatus') {
        $cityObject->updateStatus();
    }
    /*
     *  Checking Any Duplication  Key
     */
    if ($_GET['method'] == 'duplicate') {
        $cityObject->duplicate();
    }
    if ($_GET ['method'] == 'dataNavigationRequest') {
        if ($_GET ['dataNavigation'] == 'firstRecord') {
            $cityObject->firstRecord('json');
        }
        if ($_GET ['dataNavigation'] == 'previousRecord') {
            $cityObject->previousRecord('json', 0);
        }
        if ($_GET ['dataNavigation'] == 'nextRecord') {
            $cityObject->nextRecord('json', 0);
        }
        if ($_GET ['dataNavigation'] == 'lastRecord') {
            $cityObject->lastRecord('json');
        }
    }
    /*
     * Excel Reporting
     */
    if (isset($_GET ['mode'])) {
        $cityObject->setReportMode($_GET['mode']);
    ...
    if (isset($_GET ['filter'])) {
        $cityObject->setServiceOutput('option');
