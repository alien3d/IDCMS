...
namespace Core\System\Common\Gender\Controller;

use Core\ConfigClass;
use Core\Document\Trail\DocumentTrailClass;
use Core\RecordSet\RecordSet;
use Core\shared\SharedClass;
use Core\System\Common\Gender\Model\GenderModel;
use Core\System\Common\Gender\Service\GenderService;
...
require_once($newFakeDocumentRoot . "v3/system/common/model/genderModel.php");
require_once($newFakeDocumentRoot . "v3/system/common/service/genderService.php");

/**
 * Class Gender
 * this is gender controller files.
 * @name IDCMS
 * @version 2
 * @author hafizan
 * @package  Core\System\Common\Gender\Controller
 * @subpackage Common
...
class GenderClass extends ConfigClass {
    ...
    /**
     * Model
     * @var \Core\System\Common\Gender\Model\GenderModel
     */
    public $model;

    /**
     * Service-Business Application Process or other ajax request
     * @var \Core\System\Common\Gender\Service\GenderService
     */
    public $service;
    ...
    function __construct() {
        ...
        $this->setViewPath("./v3/system/common/view/gender.php");
        $this->setControllerPath("./v3/system/common/controller/genderController.php");
        $this->setServicePath("./v3/system/common/service/genderService.php");
    }
    ...
    public function create() {
        ...
        if ($this->getVendor() == self::MYSQL) {
            ...
            (
                 ...
                 '" . $this->model->getGenderIcon() . "',
                 '" . $this->model->getGenderDescription() . "',
    ...
    public function read() {
        ...
        if (isset($_SESSION['isAdmin'])) {
            if ($_SESSION['isAdmin'] == 0) {
                if ($this->getVendor() == self::MYSQL) {
                    $this->setAuditFilter(
                            " `gender`.`isActive` = 1  AND `gender`.`companyId`='" . $this->getCompanyId() . "' "
                    );
                } else {
                    if ($this->getVendor() == self::MSSQL) {
                        $this->setAuditFilter(
                ...
                if ($_SESSION['isAdmin'] == 1) {
                    if ($this->getVendor() == self::MYSQL) {
                        $this->setAuditFilter("   `gender`.`companyId`='" . $this->getCompanyId() . "'	");
                    } else {
                        if ($this->getVendor() == self::MSSQL) {
                            $this->setAuditFilter(" [gender].[companyId]='" . $this->getCompanyId() . "' ");
                        } else {
	...
	JOIN	`company`
	ON		`company`.`companyId` = `gender`.`companyId`
		  WHERE     " . $this->getAuditFilter();
            if ($this->model->getGenderId(0, 'single')) {
                $sql .= " AND `gender`.`" . $this->model->getPrimaryKeyName() . "`='" . $this->model->getGenderId(
                                0, 'single'
                        ) . "'";
            }
        } else {
	...
	ON		[company].[companyId] = [gender].[companyId]
		  WHERE     " . $this->getAuditFilter();
                if ($this->model->getGenderId(0, 'single')) {
                    $sql .= " AND [gender].[" . $this->model->getPrimaryKeyName(
                            ) . "]		=	'" . $this->model->getGenderId(0, 'single') . "'";
                }
            } else {
                if ($this->getVendor() == self::ORACLE) {
		  ...
		  SELECT                    GENDER.GENDERID AS \"genderId\",
                    ...
                    GENDER.COMPANYID AS \"companyId\",
                    GENDER.GENDERCODE AS \"genderCode\",
                    GENDER.GENDERICON AS \"genderIcon\",
	...
	ON		COMPANY.COMPANYID = GENDER.COMPANYID
        ...
        if ($this->getCharacterQuery()) {
            if ($this->getVendor() == self::MYSQL) {
                $sql .= " AND `gender`.`" . $this->model->getFilterCharacter() . "` like '" . $this->getCharacterQuery(
                        ) . "%'";
            } else {
                if ($this->getVendor() == self::MSSQL) {
                    $sql .= " AND [gender].[" . $this->model->getFilterCharacter(
                            ) . "] like '" . $this->getCharacterQuery() . "%'";
                } else {
                    if ($this->getVendor() == self::ORACLE) {
                        $sql .= " AND Initcap(GENDER." . strtoupper(
        ...
        if ($this->getDateRangeStartQuery()) {
            if ($this->getVendor() == self::MYSQL) {
                $sql .= $this->q->dateFilter(
                        'gender', $this->model->getFilterDate(), $this->getDateRangeStartQuery(), $this->getDateRangeEndQuery(), $this->getDateRangeTypeQuery(), $this->getDateRangeExtraTypeQuery()
                );
            } else {
                if ($this->getVendor() == self::MSSQL) {
                    $sql .= $this->q->dateFilter(
        ...
        while (($row = $this->q->fetchAssoc()) == true) {
            ...
            if ($this->model->getGenderId(0, 'single')) {
                ...
                $row['previousRecord'] = $this->previousRecord('value', $this->model->getGenderId(0, 'single'));
                $row['nextRecord'] = $this->nextRecord('value', $this->model->getGenderId(0, 'single'));
                            ...
                            array("[", "]"), "", json_encode(
                                    array(
                                        ...
                                        'previousRecord' => $this->previousRecord(
                                                'value', $this->model->getGenderId(0, 'single')
                                        ),
                                        'nextRecord' => $this->nextRecord('value', $this->model->getGenderId(0, 'single')),
    ...
    function update() {
        ...
        if ($this->getVendor() == self::MYSQL) {
           ...
           FROM 	GENDER
           WHERE  	" . strtoupper($this->model->getPrimaryKeyName()) . " = '" . $this->model->getGenderId(
                                    0, 'single'
                            ) . "' ";
                }
            }
        ...
        if ($total == 0) {
            ...
            if ($this->getVendor() == self::MYSQL) {
                $sql = "
               UPDATE `gender` SET
                       `genderCode` = '" . $this->model->getGenderCode() . "',
                       `genderIcon` = '" . $this->model->getGenderIcon() . "',
                       `genderDescription` = '" . $this->model->getGenderDescription() . "',
   ...
   /** 
    ...
    function duplicate() {
        ...
        if ($this->getVendor() == self::MYSQL) {
           ...
           AND     [companyId] =	'" . $this->getCompanyId() . "'";
                ...
                if ($this->getVendor() == self::ORACLE) {
                    $sql = "
               SELECT  GENDERCODE as \"genderCode\"
               FROM    GENDER
               WHERE   GENDERCODE	= 	'" . $this->model->getGenderCode() . "'
               AND     ISACTIVE    =   1
               AND     COMPANYID   =   '" . $this->getCompanyId() . "'";
    ...
    function excel() {
        ...
        $this->excel->getActiveSheet()->setCellValue('C3', $this->translate['genderCodeLabel']);
        $this->excel->getActiveSheet()->setCellValue('D3', $this->translate['genderIconLabel']);
        $this->excel->getActiveSheet()->setCellValue('E3', $this->translate['genderDescriptionLabel']);
        ...
        while (($row = $this->q->fetchAssoc()) == true) {
            ...
            $this->excel->getActiveSheet()->setCellValue('C' . $loopRow, strip_tags($row ['genderCode']));
            $this->excel->getActiveSheet()->setCellValue('D' . $loopRow, strip_tags($row ['genderIcon']));
            $this->excel->getActiveSheet()->setCellValue('E' . $loopRow, strip_tags($row ['genderDescription']));
        ...
        switch ($this->getReportMode()) {
            case 'excel':
                ...
                $extension = '.xlsx';
                $folder = 'excel';
                $filename = "gender" . rand(0, 10000000) . $extension;
...
if (isset($_POST ['method'])) {
    if (isset($_POST['output'])) {
        ...
        if ($_POST['securityToken'] != $genderObject->getSecurityToken()) {
            header('Content-Type:application/json; charset=utf-8');
        ...
        if (isset($_POST ['offset'])) {
            $genderObject->setStart($_POST ['offset']);
        }
        if (isset($_POST ['leafId'])) {
            $genderObject->setLimit($_POST ['limit']);
        }
        $genderObject->setPageOutput($_POST['output']);
...
if (isset($_GET ['method'])) {
    $genderObject = new GenderClass ();
    if ($_GET['securityToken'] != $genderObject->getSecurityToken()) {
        header('Content-Type:application/json; charset=utf-8');
        echo json_encode(array("success" => false, "message" => "Something wrong with the system.Hola hackers"));
        exit();
    }
     ...
     */
    if (isset($_GET ['mode'])) {
        $genderObject->setReportMode($_GET['mode']);
    ...
    if (isset($_GET ['filter'])) {
        $genderObject->setServiceOutput('option');
    }
}
?>
