...
namespace Core\System\Common\Language\Controller;

use Core\ConfigClass;
use Core\Document\Trail\DocumentTrailClass;
use Core\RecordSet\RecordSet;
use Core\shared\SharedClass;
use Core\System\Common\Language\Model\LanguageModel;
use Core\System\Common\Language\Service\LanguageService;
...
require_once($newFakeDocumentRoot . "v3/system/common/model/languageModel.php");
require_once($newFakeDocumentRoot . "v3/system/common/service/languageService.php");

/**
 * Class Language
 * this is language controller files.
 * @name IDCMS
 * @version 2
 * @author hafizan
 * @package  Core\System\Common\Language\Controller
 * @subpackage Common
...
class LanguageClass extends ConfigClass {
    ...
    /**
     * Model
     * @var \Core\System\Common\Language\Model\LanguageModel
     */
    public $model;

    /**
     * Service-Business Application Process or other ajax request
     * @var \Core\System\Common\Language\Service\LanguageService
     */
    public $service;
    ...
    function __construct() {
        ...
        $this->setViewPath("./v3/system/common/view/language.php");
        $this->setControllerPath("./v3/system/common/controller/languageController.php");
        $this->setServicePath("./v3/system/common/service/languageService.php");
    }
    ...
    public function create() {
        ...
        if ($this->getVendor() == self::MYSQL) {
            ...
            (
                 ...
                 '" . $this->model->getLanguageDescription() . "',
                 '" . $this->model->getLanguageIcon() . "',
                 '" . $this->model->getIsGoogle() . "',
                 '" . $this->model->getIsBing() . "',
                 '" . $this->model->getIsSpeakLite() . "',
                 '" . $this->model->getIsWebServices() . "',
                 '" . $this->model->getIsDefault(0, 'single') . "',
                 '" . $this->model->getIsImportant() . "',
        ...
        $languageId = $this->q->lastInsertId();
        $this->q->commit();
        $end = microtime(true);
        $time = $end - $start;
        echo json_encode(
    ...
    public function read() {
        ...
        if (isset($_SESSION['isAdmin'])) {
            if ($_SESSION['isAdmin'] == 0) {
                if ($this->getVendor() == self::MYSQL) {
                    $this->setAuditFilter(
                            " `language`.`isActive` = 1  AND `language`.`companyId`='" . $this->getCompanyId() . "' "
                    );
                } else {
                    if ($this->getVendor() == self::MSSQL) {
                        $this->setAuditFilter(
                ...
                if ($_SESSION['isAdmin'] == 1) {
                    if ($this->getVendor() == self::MYSQL) {
                        $this->setAuditFilter("   `language`.`companyId`='" . $this->getCompanyId() . "'	");
                    } else {
                        if ($this->getVendor() == self::MSSQL) {
                            $this->setAuditFilter(" [language].[companyId]='" . $this->getCompanyId() . "' ");
                        } else {
       ...
       SELECT                    `language`.`languageId`,
                    ...
                    `language`.`isGoogle`,
                    `language`.`isBing`,
                    `language`.`isSpeakLite`,
                    `language`.`isWebServices`,
                    `language`.`isDefault`,
                    `language`.`isImportant`,
                    `language`.`isNew`,
                    `language`.`isDraft`,
                    `language`.`isUpdate`,
                    `language`.`isDelete`,
	...
	JOIN	`company`
	ON		`company`.`companyId` = `language`.`companyId`
		  WHERE     " . $this->getAuditFilter();
            if ($this->model->getLanguageId(0, 'single')) {
                $sql .= " AND `language`.`" . $this->model->getPrimaryKeyName() . "`='" . $this->model->getLanguageId(
                                0, 'single'
                        ) . "'";
            }
        } else {
	...
	ON		[company].[companyId] = [language].[companyId]
		  WHERE     " . $this->getAuditFilter();
                if ($this->model->getLanguageId(0, 'single')) {
                    $sql .= " AND [language].[" . $this->model->getPrimaryKeyName(
                            ) . "]		=	'" . $this->model->getLanguageId(0, 'single') . "'";
                }
            } else {
                if ($this->getVendor() == self::ORACLE) {
		  ...
		  SELECT                    LANGUAGE.LANGUAGEID AS \"languageId\",
                    ...
                    LANGUAGE.COMPANYID AS \"companyId\",
                    ...
                    LANGUAGE.ISGOOGLE AS \"isGoogle\",
                    LANGUAGE.ISBING AS \"isBing\",
                    LANGUAGE.ISSPEAKLITE AS \"isSpeakLite\",
                    LANGUAGE.ISWEBSERVICES AS \"isWebServices\",
                    LANGUAGE.ISDEFAULT AS \"isDefault\",
                    LANGUAGE.ISIMPORTANT AS \"isImportant\",
                    ...
                    LANGUAGE.ISREVIEW AS \"isReview\",
                    LANGUAGE.ISPOST AS \"isPost\",
	...
	ON		COMPANY.COMPANYID = LANGUAGE.COMPANYID
        ...
        if ($this->getCharacterQuery()) {
            if ($this->getVendor() == self::MYSQL) {
                $sql .= " AND `language`.`" . $this->model->getFilterCharacter(
                ...
                if ($this->getVendor() == self::MSSQL) {
                    ...
                    if ($this->getVendor() == self::ORACLE) {
                        $sql .= " AND Initcap(LANGUAGE." . strtoupper(
        ...
        if ($this->getDateRangeStartQuery()) {
            if ($this->getVendor() == self::MYSQL) {
                $sql .= $this->q->dateFilter(
                        'language', $this->model->getFilterDate(), $this->getDateRangeStartQuery(), $this->getDateRangeEndQuery(), $this->getDateRangeTypeQuery(), $this->getDateRangeExtraTypeQuery()
                );
            } else {
                if ($this->getVendor() == self::MSSQL) {
                    $sql .= $this->q->dateFilter(
        ...
        $tableArray = null;
        if ($this->getVendor() == self::MYSQL) {
            $tableArray = array('staff', 'language');
        } else {
            if ($this->getVendor() == self::MSSQL) {
                $tableArray = array('staff', 'language');
            } else {
        ...
        while (($row = $this->q->fetchAssoc()) == true) {
            ...
            if ($this->model->getLanguageId(0, 'single')) {
                ...
                $row['previousRecord'] = $this->previousRecord('value', $this->model->getLanguageId(0, 'single'));
                $row['nextRecord'] = $this->nextRecord('value', $this->model->getLanguageId(0, 'single'));
                            ...
                            array("[", "]"), "", json_encode(
                                    array(
                                        ...
                                        'previousRecord' => $this->previousRecord(
                                                'value', $this->model->getLanguageId(0, 'single')
                                        ),
                                        'nextRecord' => $this->nextRecord('value', $this->model->getLanguageId(0, 'single')),
    ...
    function update() {
        ...
        if ($this->getVendor() == self::MYSQL) {
           ...
           FROM 	`language`
           WHERE  	`" . $this->model->getPrimaryKeyName() . "` = '" . $this->model->getLanguageId(
                            0, 'single'
                    ) . "' ";
        } else {
            if ($this->getVendor() == self::MSSQL) {
           ...
           FROM 	LANGUAGE
           WHERE  	" . strtoupper($this->model->getPrimaryKeyName()) . " = '" . $this->model->getLanguageId(
                                    0, 'single'
                            ) . "' ";
                }
            }
        ...
        if ($total == 0) {
            ...
            if ($this->getVendor() == self::MYSQL) {
               ...
               UPDATE `language` SET
                       ...
                       `languageDescription` = '" . $this->model->getLanguageDescription() . "',
                       `languageIcon` = '" . $this->model->getLanguageIcon() . "',
                       `isGoogle` = '" . $this->model->getIsGoogle() . "',
                       `isBing` = '" . $this->model->getIsBing() . "',
                       `isSpeakLite` = '" . $this->model->getIsSpeakLite() . "',
                       `isWebServices` = '" . $this->model->getIsWebServices() . "',
                       `isDefault` = '" . $this->model->getIsDefault('0', 'single') . "',
                       `isImportant` = '" . $this->model->getIsImportant() . "',
...
/** 
    ...
    function excel() {
        ...
        $this->excel->getActiveSheet()->setCellValue('C3', $this->translate['languageCodeLabel']);
        $this->excel->getActiveSheet()->setCellValue('D3', $this->translate['languageDescriptionLabel']);
        $this->excel->getActiveSheet()->setCellValue('E3', $this->translate['languageIconLabel']);
        $this->excel->getActiveSheet()->setCellValue('F3', $this->translate['isGoogleLabel']);
        $this->excel->getActiveSheet()->setCellValue('G3', $this->translate['isBingLabel']);
        $this->excel->getActiveSheet()->setCellValue('H3', $this->translate['isSpeakLiteLabel']);
        $this->excel->getActiveSheet()->setCellValue('I3', $this->translate['isWebServicesLabel']);
        $this->excel->getActiveSheet()->setCellValue('J3', $this->translate['isImportantLabel']);
        ...
        while (($row = $this->q->fetchAssoc()) == true) {
            ...
            $this->excel->getActiveSheet()->setCellValue('C' . $loopRow, strip_tags($row ['languageCode']));
            $this->excel->getActiveSheet()->setCellValue('D' . $loopRow, strip_tags($row ['languageDescription']));
            $this->excel->getActiveSheet()->setCellValue('E' . $loopRow, strip_tags($row ['languageIcon']));
            $this->excel->getActiveSheet()->setCellValue('F' . $loopRow, strip_tags($row ['isGoogle']));
            $this->excel->getActiveSheet()->setCellValue('G' . $loopRow, strip_tags($row ['isBing']));
            $this->excel->getActiveSheet()->setCellValue('H' . $loopRow, strip_tags($row ['isSpeakLite']));
            $this->excel->getActiveSheet()->setCellValue('I' . $loopRow, strip_tags($row ['isWebServices']));
            $this->excel->getActiveSheet()->setCellValue('J' . $loopRow, strip_tags($row ['isImportant']));
        ...
        switch ($this->getReportMode()) {
            case 'excel':
                ...
                $extension = '.xlsx';
                $folder = 'excel';
                $filename = "language" . rand(0, 10000000) . $extension;
...
if (isset($_POST ['method'])) {
    if (isset($_POST['output'])) {
        ...
        if ($_POST['securityToken'] != $languageObject->getSecurityToken()) {
            header('Content-Type:application/json; charset=utf-8');
        ...
        if (isset($_POST ['offset'])) {
            $languageObject->setStart($_POST ['offset']);
        }
        if (isset($_POST ['leafId'])) {
            $languageObject->setLimit($_POST ['limit']);
        }
        $languageObject->setPageOutput($_POST['output']);
...
if (isset($_GET ['method'])) {
    $languageObject = new LanguageClass ();
    if ($_GET['securityToken'] != $languageObject->getSecurityToken()) {
        header('Content-Type:application/json; charset=utf-8');
        echo json_encode(array("success" => false, "message" => "Something wrong with the system.Hola hackers"));
        exit();
    }
     ...
     */
    if (isset($_GET ['mode'])) {
        $languageObject->setReportMode($_GET['mode']);
    ...
    if (isset($_GET ['filter'])) {
        $languageObject->setServiceOutput('option');
    }
}
?>
