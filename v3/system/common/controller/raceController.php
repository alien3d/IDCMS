...
use Core\System\Common\Race\Model\RaceModel;
use Core\System\Common\Race\Service\RaceService;
...
require_once($newFakeDocumentRoot . "v3/system/common/model/raceModel.php");
require_once($newFakeDocumentRoot . "v3/system/common/service/raceService.php");

/**
 * Class Race
 * this is race controller files.
 * @name IDCMS
 * @version 2
 * @author hafizan
 * @package  Core\System\Common\Race\Controller
 * @subpackage Common
 * @link http://www.hafizan.com
 * @license http://www.gnu.org/copyleft/lesser.html LGPL
 */
class RaceClass extends ConfigClass {
    ...
    /**
     * Model
     * @var \Core\System\Common\Race\Model\RaceModel
     */
    public $model;

    /**
     * Service-Business Application Process or other ajax request
     * @var \Core\System\Common\Race\Service\RaceService
     */
    public $service;
    ...
    function __construct() {
        ...
        $this->setViewPath("./v3/system/common/view/race.php");
        $this->setControllerPath("./v3/system/common/controller/raceController.php");
        $this->setServicePath("./v3/system/common/service/raceService.php");
    }
    ...
    public function read() {
        ...
        if (isset($_SESSION['isAdmin'])) {
            if ($_SESSION['isAdmin'] == 0) {
                if ($this->getVendor() == self::MYSQL) {
                    $this->setAuditFilter(
                            " `race`.`isActive` = 1  AND `race`.`companyId`='" . $this->getCompanyId() . "' "
                    );
                } else {
                    if ($this->getVendor() == self::MSSQL) {
                        $this->setAuditFilter(
                ...
                if ($_SESSION['isAdmin'] == 1) {
                    if ($this->getVendor() == self::MYSQL) {
                        $this->setAuditFilter("   `race`.`companyId`='" . $this->getCompanyId() . "'	");
                    } else {
                        if ($this->getVendor() == self::MSSQL) {
                            $this->setAuditFilter(" [race].[companyId]='" . $this->getCompanyId() . "' ");
                        } else {
	...
	JOIN	`company`
	ON		`company`.`companyId` = `race`.`companyId`
		  WHERE     " . $this->getAuditFilter();
            if ($this->model->getRaceId(0, 'single')) {
                $sql .= " AND `race`.`" . $this->model->getPrimaryKeyName() . "`='" . $this->model->getRaceId(
                                0, 'single'
                        ) . "'";
            }
        } else {
	...
	ON		[company].[companyId] = [race].[companyId]
		  WHERE     " . $this->getAuditFilter();
                if ($this->model->getRaceId(0, 'single')) {
                    $sql .= " AND [race].[" . $this->model->getPrimaryKeyName(
                            ) . "]		=	'" . $this->model->getRaceId(0, 'single') . "'";
                }
            } else {
                if ($this->getVendor() == self::ORACLE) {

                    $sql = "
		  SELECT                    RACE.RACEID AS \"raceId\",
                    COMPANY.COMPANYDESCRIPTION AS  \"companyDescription\",
                    RACE.COMPANYID AS \"companyId\",
                    RACE.RACECODE AS \"raceCode\",
                    RACE.RACEDESCRIPTION AS \"raceDescription\",
                    RACE.ISDEFAULT AS \"isDefault\",
                    RACE.ISNEW AS \"isNew\",
                    RACE.ISDRAFT AS \"isDraft\",
                    RACE.ISUPDATE AS \"isUpdate\",
                    RACE.ISDELETE AS \"isDelete\",
                    RACE.ISACTIVE AS \"isActive\",
                    RACE.ISAPPROVED AS \"isApproved\",
                    RACE.ISREVIEW AS \"isReview\",
                    RACE.ISPOST AS \"isPost\",
                    RACE.EXECUTEBY AS \"executeBy\",
                    RACE.EXECUTETIME AS \"executeTime\",
	...
	ON		COMPANY.COMPANYID = RACE.COMPANYID
        ...
        if ($this->getCharacterQuery()) {
            if ($this->getVendor() == self::MYSQL) {
                $sql .= " AND `race`.`" . $this->model->getFilterCharacter() . "` like '" . $this->getCharacterQuery(
                        ) . "%'";
            } else {
                if ($this->getVendor() == self::MSSQL) {
                    $sql .= " AND [race].[" . $this->model->getFilterCharacter(
                            ) . "] like '" . $this->getCharacterQuery() . "%'";
                } else {
                    if ($this->getVendor() == self::ORACLE) {
                        $sql .= " AND Initcap(RACE." . strtoupper(
        ...
        if ($this->getDateRangeStartQuery()) {
            if ($this->getVendor() == self::MYSQL) {
                $sql .= $this->q->dateFilter(
                        'race', $this->model->getFilterDate(), $this->getDateRangeStartQuery(), $this->getDateRangeEndQuery(), $this->getDateRangeTypeQuery(), $this->getDateRangeExtraTypeQuery()
                );
            } else {
                if ($this->getVendor() == self::MSSQL) {
                    $sql .= $this->q->dateFilter(
                            ...
                            array("[", "]"), "", json_encode(
                                    array(
                                        ...
                                        'previousRecord' => $this->previousRecord(
                                                'value', $this->model->getRaceId(0, 'single')
                                        ),
    ...
    function update() {
        ...
        if ($this->getVendor() == self::MYSQL) {
           ...
           FROM 	RACE
           WHERE  	" . strtoupper($this->model->getPrimaryKeyName()) . " = '" . $this->model->getRaceId(
                                    0, 'single'
                            ) . "' ";
                }
            }
        ...
        if ($total == 0) {
            ...
            if ($this->getVendor() == self::MYSQL) {
               ...
               UPDATE `race` SET
                       ...
                       `raceDescription` = '" . $this->model->getRaceDescription() . "',
   ...
   /** 
    ...
    function duplicate() {
        ...
        if ($this->getVendor() == self::MYSQL) {
           ...
           SELECT  `raceCode`
           FROM    `race`
           WHERE   `raceCode` 	= 	'" . $this->model->getRaceCode() . "'
           AND     `isActive`  =   1
    ...
    function excel() {
        ...
        $this->excel->getActiveSheet()->setCellValue('C3', $this->translate['raceCodeLabel']);
        $this->excel->getActiveSheet()->setCellValue('D3', $this->translate['raceDescriptionLabel']);
        ...
        while (($row = $this->q->fetchAssoc()) == true) {
            ...
            $this->excel->getActiveSheet()->setCellValue('C' . $loopRow, strip_tags($row ['raceCode']));
            $this->excel->getActiveSheet()->setCellValue('D' . $loopRow, strip_tags($row ['raceDescription']));
        ...
        switch ($this->getReportMode()) {
            case 'excel':
                ...
                $extension = '.xlsx';
                $folder = 'excel';
                $filename = "race" . rand(0, 10000000) . $extension;
...
if (isset($_POST ['method'])) {
    if (isset($_POST['output'])) {
        ...
        if ($_POST['securityToken'] != $raceObject->getSecurityToken()) {
            header('Content-Type:application/json; charset=utf-8');
        ...
        if (isset($_POST ['leafId'])) {
            $raceObject->setLeafId($_POST ['leafId']);
        }
        if (isset($_POST ['offset'])) {
            $raceObject->setStart($_POST ['offset']);
        }
        if (isset($_POST ['limit'])) {
            $raceObject->setLimit($_POST ['limit']);
        }
        $raceObject->setPageOutput($_POST['output']);
        $raceObject->execute();
        /*
...
if (isset($_GET ['method'])) {
    $raceObject = new RaceClass ();
    if ($_GET['securityToken'] != $raceObject->getSecurityToken()) {
        header('Content-Type:application/json; charset=utf-8');
        echo json_encode(array("success" => false, "message" => "Something wrong with the system.Hola hackers"));
        exit();
    }
     ...
     */
    if (isset($_GET ['mode'])) {
        $raceObject->setReportMode($_GET['mode']);
    ...
    if (isset($_GET ['filter'])) {
        $raceObject->setServiceOutput('option');
    }
}
?>
