 ...
 * @version 2
 * @author hafizan
 * @package Core\System\Common\Day\Model;
 * @subpackage Common
 * @link http://www.hafizan.com
 * @license http://www.gnu.org/copyleft/lesser.html LGPL
 */
class DayModel extends ValidationClass {
    ...
    public function execute() {
        ...
        $this->setMasterForeignKeyName('dayId');
        $this->setFilterCharacter('dayDescription');
        //$this->setFilterCharacter('dayNote');
        $this->setFilterDate('executeTime');
        /**
         * All the $_POST Environment
         */
        if (isset($_POST ['dayId'])) {
            $this->setDayId($this->strict($_POST ['dayId'], 'integer'), 0, 'single');
        }
        if (isset($_POST ['companyId'])) {
            $this->setCompanyId($this->strict($_POST ['companyId'], 'integer'));
        }
        if (isset($_POST ['dayName'])) {
            $this->setDayName($this->strict($_POST ['dayName'], 'string'));
        }
        /**
         * All the $_GET Environment
         */
        if (isset($_GET ['dayId'])) {
            $this->setDayId($this->strict($_GET ['dayId'], 'integer'), 0, 'single');
        }
        if (isset($_GET ['companyId'])) {
            $this->setCompanyId($this->strict($_GET ['companyId'], 'integer'));
        }
        if (isset($_GET ['dayName'])) {
            $this->setDayName($this->strict($_GET ['dayName'], 'string'));
        }
        if (isset($_GET ['dayId'])) {
            $this->setTotal(count($_GET ['dayId']));
            if (is_array($_GET ['dayId'])) {
                $this->dayId = array();
            }
        }
     ...
     * @return \Core\System\Common\Day\Model\DayModel
     */
    public function setDayId($value, $key, $type) {
        if ($type == 'single') {
            ...
            if ($type == 'array') {
                ...
                echo json_encode(
                        array("success" => false, "message" => "Cannot Identify Type String Or Array:setDayId?")
                );
                exit();
            }
        }
    ...
    public function getDayId($key, $type) {
        if ($type == 'single') {
            ...
            if ($type == 'array') {
                ...
                echo json_encode(
                        array("success" => false, "message" => "Cannot Identify Type String Or Array:getDayId ?")
                );
                exit();
            }
        }
