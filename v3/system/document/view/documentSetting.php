...
require_once($newFakeDocumentRoot . "v3/system/document/controller/documentSettingController.php");
...
$translator->setCurrentTable('documentSetting');
...
$documentSettingArray = array();
$documentSequenceArray = array();
$leafArray = array();
if (isset($_POST)) {
    if (isset($_POST['method'])) {
        $documentSetting = new \Core\System\Document\DocumentSetting\Controller\DocumentSettingClass();
        define('LIMIT', 10);
        ...
        if (isset($_POST ['query'])) {
            $documentSetting->setFieldQuery($_POST ['query']);
        }
        if (isset($_POST ['filter'])) {
            $documentSetting->setGridQuery($_POST ['filter']);
        }
        if (isset($_POST ['character'])) {
            $documentSetting->setCharacterQuery($_POST['character']);
        }
        if (isset($_POST ['dateRangeStart'])) {
            $documentSetting->setDateRangeStartQuery($_POST['dateRangeStart']);
            //explode the data to get day,month,year
            $start = explode('-', $_POST ['dateRangeStart']);
            $documentSetting->setStartDay($start[2]);
            $documentSetting->setStartMonth($start[1]);
            $documentSetting->setStartYear($start[0]);
        }
        if (isset($_POST ['dateRangeEnd']) && (strlen($_POST['dateRangeEnd']) > 0)) {
            $documentSetting->setDateRangeEndQuery($_POST['dateRangeEnd']);
            //explode the data to get day,month,year
            $start = explode('-', $_POST ['dateRangeEnd']);
            $documentSetting->setEndDay($start[2]);
            $documentSetting->setEndMonth($start[1]);
            $documentSetting->setEndYear($start[0]);
        }
        if (isset($_POST ['dateRangeType'])) {
            $documentSetting->setDateRangeTypeQuery($_POST['dateRangeType']);
        }
        if (isset($_POST ['dateRangeExtraType'])) {
            $documentSetting->setDateRangeExtraTypeQuery($_POST['dateRangeExtraType']);
        }
        $documentSetting->setServiceOutput('html');
        $documentSetting->setLeafId($leafId);
        $documentSetting->execute();
        $documentSequenceArray = $documentSetting->getDocumentSequence();
        $leafArray = $documentSetting->getLeaf();
        if ($_POST['method'] == 'read') {
            $documentSetting->setStart($offset);
            $documentSetting->setLimit($limit); // normal system don't like paging..
            $documentSetting->setPageOutput('html');
            $documentSettingArray = $documentSetting->read();
            if (isset($documentSettingArray [0]['firstRecord'])) {
                $firstRecord = $documentSettingArray [0]['firstRecord'];
            }
            if (isset($documentSettingArray [0]['nextRecord'])) {
                $nextRecord = $documentSettingArray [0]['nextRecord'];
            }
            if (isset($documentSettingArray [0]['previousRecord'])) {
                $previousRecord = $documentSettingArray [0]['previousRecord'];
            }
            if (isset($documentSettingArray [0]['lastRecord'])) {
                $lastRecord = $documentSettingArray [0]['lastRecord'];
                $endRecord = $documentSettingArray [0]['lastRecord'];
            }
            $navigation = new \Core\Paging\HtmlPaging();
            $navigation->setLeafId($leafId);
            $navigation->setViewPath($documentSetting->getViewPath());
            ...
            if (isset($documentSettingArray [0]['total'])) {
                $total = $documentSettingArray [0]['total'];
            } else {
                $total = 0;
            }
        ...
        <div class="row">
            <div class="pull-left btn-group col-xs-10 col-sm-10 col-md-10">
                <button title="A" class="btn btn-success btn-sm" type="button" 
                        ...
                        echo $documentSetting->getViewPath();
                    ...
                    <div class="btn-group">
                        ...
                        <ul class="dropdown-menu" style="text-align:left">
                            <li>
                                ...
                                echo $documentSetting->getControllerPath();
                        ...
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                <form class="form-horizontal">
                                    <input type="hidden" name="documentSettingIdPreview" id="documentSettingIdPreview">

                                    <div class="form-group" id="documentSequenceIdDiv">
                                        <label for="documentSequenceIdPreview"
                                               class="control-label col-xs-4 col-sm-4 col-md-4"><?php echo $leafTranslation['documentSequenceIdLabel']; ?></label>

                                        <div class="col-xs-8 col-sm-8 col-md-8">
                                            <input class="form-control" type="text" name="documentSequenceIdPreview"
                                                   id="documentSequenceIdPreview">
                                        </div>
                                    </div>
                    ...
                    <div class="col-xs-12 col-sm-12 col-md-12">
                        <div class="panel panel-default">
                            <table class="table table table-bordered table-striped table-condensed table-hover smart-form has-tickbox" id="tableData">
                                ...
                                <th width="250px"><?php echo ucwords($leafTranslation['documentSequenceIdLabel']); ?></th>
                                <th width="250px"><?php echo ucwords($leafTranslation['leafIdLabel']); ?></th>
                                ...
                                <tbody id="tableBody">
                                    ...
                                    if ($_POST['method'] == 'read' && $_POST['type'] == 'list' && $_POST['detail'] == 'body') {
                                        if (is_array($documentSettingArray)) {
                                            $totalRecord = intval(count($documentSettingArray));
                                            if ($totalRecord > 0) {
                                                $counter = 0;
                                                for ($i = 0; $i < $totalRecord; $i++) {
                                                    ...
                                                    ?>
                                                    <tr <?php
                                                    if ($documentSettingArray[$i]['isDelete'] == 1) {
                                                        echo "class=\"danger\"";
                                                    } else {
                                                        if ($documentSettingArray[$i]['isDraft'] == 1) {
                                                            echo "class=\"warning\"";
                                                        }
                                                    }
                                                    ?>>
                                                        ...
                                                        <td align="center">
                                                            <div class="btn-group" align="center">
                                                                <button type="button"  class="btn btn-info btn-sm" title="Edit"
                                                                        ...
                                                                        echo intval(
                                                                                $documentSettingArray [$i]['documentSettingId']
                                                                        );
                                                                ...
                                                                ?>', '<?php
                                                                echo rawurlencode(
                                                                        $documentSettingArray [$i]['documentSequenceDescription']
                                                                );
                                                                ?>', '<?php
                                                                echo rawurlencode(
                                                                        $documentSettingArray [$i]['leafEnglish']
                                                                );
                                                                ?>');">
                                                        ...
                                                        <td>
                                                            <div class="pull-left">
                                                                <?php
                                                                if (isset($documentSettingArray[$i]['documentSequenceDescription'])) {
                                                                    if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                        if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                            if (strpos(
                                                                                            $documentSettingArray[$i]['documentSequenceDescription'], $_POST['query']
                                                                                    ) !== false
                                                                            ) {
                                                                                echo str_replace(
                                                                                        $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $documentSettingArray[$i]['documentSequenceDescription']
                                                                                );
                                                                            } else {
                                                                                echo $documentSettingArray[$i]['documentSequenceDescription'];
                                                                            }
                                                                        } else {
                                                                            if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                if (strpos(
                                                                                                $documentSettingArray[$i]['documentSequenceDescription'], $_POST['character']
                                                                                        ) !== false
                                                                                ) {
                                                                                    echo str_replace(
                                                                                            $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $documentSettingArray[$i]['documentSequenceDescription']
                                                                                    );
                                                                                } else {
                                                                                    echo $documentSettingArray[$i]['documentSequenceDescription'];
                                                                                }
                                                        ...
                                                        <td>
                                                            <div class="pull-left">
                                                                <?php
                                                                if (isset($documentSettingArray[$i]['leafEnglish'])) {
                                                                    if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                        if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                            if (strpos(
                                                                                            $documentSettingArray[$i]['leafEnglish'], $_POST['query']
                                                                                    ) !== false
                                                                            ) {
                                                                                echo str_replace(
                                                                                        $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $documentSettingArray[$i]['leafEnglish']
                                                                                );
                                                                            } else {
                                                                                echo $documentSettingArray[$i]['leafEnglish'];
                                                                            }
                                                                        } else {
                                                                            if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                if (strpos(
                                                                                                $documentSettingArray[$i]['leafEnglish'], $_POST['character']
                                                                                        ) !== false
                                                                                ) {
                                                                                    echo str_replace(
                                                                                            $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $documentSettingArray[$i]['leafEnglish']
                                                                                    );
                                                                                } else {
                                                                                    echo $documentSettingArray[$i]['leafEnglish'];
                                                                                }
                                                        ...
                                                        <td>
                                                            <div class="pull-left">
                                                                <?php
                                                                if (isset($documentSettingArray[$i]['executeBy'])) {
                                                                    if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                        if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                            if (strpos(
                                                                                            $documentSettingArray[$i]['staffName'], $_POST['query']
                                                                                    ) !== false
                                                                            ) {
                                                                                echo str_replace(
                                                                                        $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $documentSettingArray[$i]['staffName']
                                                                                );
                                                                            } else {
                                                                                echo $documentSettingArray[$i]['staffName'];
                                                                            }
                                                                        } else {
                                                                            if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                if (strpos(
                                                                                                $documentSettingArray[$i]['staffName'], $_POST['character']
                                                                                        ) !== false
                                                                                ) {
                                                                                    echo str_replace(
                                                                                            $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $documentSettingArray[$i]['staffName']
                                                                                    );
                                                                                } else {
                                                                                    echo $documentSettingArray[$i]['staffName'];
                                                                                }
                                                        ...
                                                        <?php
                                                        if (isset($documentSettingArray[$i]['executeTime'])) {
                                                            $valueArray = $documentSettingArray[$i]['executeTime'];
                                                        ...
                                                        <?php } ?>
                                                        <?php
                                                        if ($documentSettingArray[$i]['isDelete']) {
                                                            $checked = "checked";
                                                        } else {
                                                            $checked = null;
                                                        }
                                                        ?>
                                                        <td>
                                                            <input style="display:none;" type="checkbox" name="documentSettingId[]"
                                                                   value="<?php echo $documentSettingArray[$i]['documentSettingId']; ?>">
                                                            <input <?php echo $checked; ?> type="checkbox" name="isDelete[]"
                                                                                           value="<?php echo $documentSettingArray[$i]['isDelete']; ?>">
                                                        </td>
                                                    </tr>
                                                    <?php
                                                }
                                                ...
                                                <tr>
                                                    <td colspan="7" vAlign="top" align="center"><?php
                                                        $documentSetting->exceptionMessage(
                    ...
                    <div class="col-xs-3 col-sm-3 col-md-3 pagination">
                        <div class="pull-right">
                            <button class="delete btn btn-warning" type="button" 
                                    ...
                                    echo $documentSetting->getControllerPath();
                                    ?>', '<?php echo $documentSetting->getViewPath(); ?>', '<?php echo $securityToken; ?>');">
...
if ((isset($_POST['method']) == 'new' || isset($_POST['method']) == 'read') && $_POST['type'] == 'form') {
    ...
    <form class="form-horizontal">
        ...
        <div class="row">
            <div class="col-xs-12 col-sm-12 col-md-12">
                <div class="panel panel-info">
                    ...
                    <div class="panel-body">
                        <input type="hidden" name="documentSettingId" id="documentSettingId" value="<?php
                        if (isset($_POST['documentSettingId'])) {
                            echo $_POST['documentSettingId'];
                        }
                        ?>">

                        <div class="row">
                            <div class="col-xs-12 col-sm-12 col-md-12">
                                <div class="form-group" id="documentSequenceIdForm">
                                    <label for="documentSequenceId" class="control-label col-xs-2 col-sm-2 col-md-2"><strong><?php
                                            echo ucfirst(
                                                    $leafTranslation['documentSequenceIdLabel']
                                            );
                                            ?></strong></label>

                                    <div class="col-xs-10 col-sm-10 col-md-10">
                                        <select name="documentSequenceId" id="documentSequenceId"
                                                class="chzn-select form-control"
                                                onChange="removeMeError('documentSequenceId');">
                                            <option value=""></option>
                                            <?php
                                            if (is_array($documentSequenceArray)) {
                                                ...
                                                if ($totalRecord > 0) {
                                                    $d = 1;
                                                    for ($i = 0; $i < $totalRecord; $i++) {
                                                        if ($documentSettingArray[0]['documentSequenceId'] == $documentSequenceArray[$i]['documentSequenceId']) {
                                                            $selected = "selected";
                                                        } else {
                                                            $selected = null;
                                                        }
                                                        ?>
                                                        <option
                                                            value="<?php echo $documentSequenceArray[$i]['documentSequenceId']; ?>" <?php echo $selected; ?>><?php echo $d; ?>
                                                            . <?php echo $documentSequenceArray[$i]['documentSequenceDescription']; ?></option>
                                                        <?php
                                                        $d++;
                                                    }
                                                } else {
                                                ...
                                                <option value=""><?php echo $t['notAvailableTextLabel']; ?></option>
                                            <?php } ?>
                                        </select> <span class="help-block" id="documentSequenceIdHelpMe"></span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-xs-12 col-sm-12 col-md-12">
                                <div class="form-group" id="leafIdForm">
                                    <label for="leafIdTemp" class="control-label col-xs-2 col-sm-2 col-md-2"><strong><?php
                                            echo ucfirst(
                                                    $leafTranslation['leafIdLabel']
                                            );
                                            ?></strong></label>

                                    <div class="col-xs-10 col-sm-10 col-md-10">
                                        <select name="leafIdTemp" id="leafIdTemp" class="chzn-select form-control"
                                                onChange="removeMeError('leafIdTemp');">
                                            <option value=""></option>
                                            <?php
                                            if (is_array($leafArray)) {
                                                ...
                                                if ($totalRecord > 0) {
                                                    $d = 1;
                                                    for ($i = 0; $i < $totalRecord; $i++) {
                                                        if ($documentSettingArray[0]['leafId'] == $leafArray[$i]['leafId']) {
                                                            $selected = "selected";
                                                        } else {
                                                            $selected = null;
                                                        }
                                                        ?>
                                                        <option
                                                            value="<?php echo $leafArray[$i]['leafId']; ?>" <?php echo $selected; ?>><?php echo $d; ?>
                                                            . [<?php echo $leafArray[$i]['leafEnglish']; ?>] -
                                                            [<?php echo $leafArray[$i]['leafFilename']; ?>]
                                                        </option>
                                                        <?php
                                                        $d++;
                                                    }
                                                ...
                                                <option value=""><?php echo $t['notAvailableTextLabel']; ?></option>
                                            <?php } ?>
                                        </select> <span class="help-block" id="leafIdHelpMe"></span>
                                    </div>
                                </div>
                            </div>
                        </div>
        ...
        <script type="text/javascript">
            $(document).ready(function() {
                ...
                validateMeNumeric('documentSettingId');
    ...
    <?php if ($_POST['method'] == "new") { ?>
        ...
        <?php if ($leafAccess['leafAccessCreateValue'] == 1) { ?>
                        $('#newRecordButton1').removeClass().addClass('btn btn-success').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $documentSetting->getControllerPath(); ?>','<?php echo $documentSetting->getViewPath(); ?>','<?php echo $securityToken; ?>',1)");
                        $('#newRecordButton2').removeClass().addClass('btn dropdown-toggle btn-success');
                        $('#newRecordButton3').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $documentSetting->getControllerPath(); ?>','<?php echo $documentSetting->getViewPath(); ?>','<?php echo $securityToken; ?>',1)");
                        $('#newRecordButton4').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $documentSetting->getControllerPath(); ?>','<?php echo $documentSetting->getViewPath(); ?>','<?php echo $securityToken; ?>',2)");
                        $('#newRecordButton5').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $documentSetting->getControllerPath(); ?>','<?php echo $documentSetting->getViewPath(); ?>','<?php echo $securityToken; ?>',3)");
                        $('#newRecordButton6').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $documentSetting->getControllerPath(); ?>','<?php echo $documentSetting->getViewPath(); ?>','<?php echo $securityToken; ?>',4)");
                        $('#newRecordButton7').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $documentSetting->getControllerPath(); ?>','<?php echo $documentSetting->getViewPath(); ?>','<?php echo $securityToken; ?>',5)");
        <?php } else { ?>
        ...
        if ($_POST['documentSettingId']) {
            ...
            <?php if ($leafAccess['leafAccessUpdateValue'] == 1) { ?>
                            $('#updateRecordButton1').removeClass().addClass('btn btn-info').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $documentSetting->getControllerPath(); ?>','<?php echo $documentSetting->getViewPath(); ?>','<?php echo $securityToken; ?>',1,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                            $('#updateRecordButton2').removeClass().addClass('btn dropdown-toggle btn-info');
                            $('#updateRecordButton3').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $documentSetting->getControllerPath(); ?>','<?php echo $documentSetting->getViewPath(); ?>','<?php echo $securityToken; ?>',1,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                            $('#updateRecordButton4').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $documentSetting->getControllerPath(); ?>','<?php echo $documentSetting->getViewPath(); ?>','<?php echo $securityToken; ?>',2,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                            $('#updateRecordButton5').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $documentSetting->getControllerPath(); ?>','<?php echo $documentSetting->getViewPath(); ?>','<?php echo $securityToken; ?>',3,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
            <?php } else { ?>
                            $('#updateRecordButton1').removeClass().addClass(' btn btn-info disabled').attr('onClick', '');
                            $('#updateRecordButton2').removeClass().addClass('btn dropdown-toggle btn-info disabled');
                            $('#updateRecordButton3').attr('onClick', '');
            ...
            <?php if ($leafAccess['leafAccessDeleteValue'] == 1) { ?>
                            $('#deleteRecordButton').removeClass().addClass('btn btn-danger').attr('onClick', "deleteRecord(<?php echo $leafId; ?>,'<?php echo $documentSetting->getControllerPath(); ?>','<?php echo $documentSetting->getViewPath(); ?>','<?php echo $securityToken; ?>',<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
            <?php } else { ?>
                            $('#deleteRecordButton').removeClass().addClass('btn btn-danger disabled').attr('onClick', '');
            <?php } ?>
            <?php
    ...
    <?php } ?>
    <script type="text/javascript" src="./v3/system/document/javascript/documentSetting.js"></script>
    <hr>
