function checkDuplicate(leafId, page, securityToken) {
    var $documentAttachmentCode = $("#documentAttachmentCode");
    if ($documentAttachmentCode.val().length === 0) {
    ...
    $.ajax({
        ...
        url: page,
        data: {
            documentAttachmentCode: $documentAttachmentCode.val(),
            method: 'duplicate',
        ...
        success: function(data) {
            ...
            if (success === true) {
                if (total !== 0) {
                    $("#documentAttachmentCode").val('').focus();
                    $("#documentAttachmentCodeForm").removeClass().addClass("form-group has-error");
                ...
                $("#documentAttachmentForm").removeClass().addClass("form-group has-error");
            }
            if ($infoPanel.is(':hidden')) {
                $infoPanel.show();
            }
...
function showFormUpdate(leafId, url, urlList, securityToken, documentAttachmentId, updateAccess, deleteAccess) {
    sleep(500);
...
function showModalDelete(documentAttachmentId, documentAttachmentDescription, documentAttachmentPath) {
    $('#documentAttachmentIdPreview').val('').val(decodeURIComponent(documentAttachmentId));
    $('#documentAttachmentDescriptionPreview').val('').val(decodeURIComponent(documentAttachmentDescription));
    $('#documentAttachmentPathPreview').val('').val(decodeURIComponent(documentAttachmentPath));
...
function deleteGridRecord(leafId, url, urlList, securityToken) {
    $.ajax({
        ...
        data: {
            method: 'delete',
            output: 'json',
            documentAttachmentId: $('#documentAttachmentIdPreview').val(),
            securityToken: securityToken,
            leafId: leafId
        },
        beforeSend: function() {
...
function deleteGridRecordCheckbox(leafId, url, urlList, securityToken) {
    var stringText = '';
    var counter = 0;
    $('input:checkbox[name="documentAttachmentId[]"]').each(function() {
        stringText = stringText + "&documentAttachmentId[]=" + $(this).val();
    });
...
function newRecord(leafId, url, urlList, securityToken, type, createAccess, updateAccess, deleteAccess) {
    ...
    var $documentAttachmentDescription = $('#documentAttachmentDescription');
    var $documentAttachmentPath = $('#documentAttachmentPath');
    if (css.search('disabled') > 0) {
        return false;
    } else {
        if (type === 1) {
            if ($documentAttachmentDescription.val().length === 0) {
                $('#documentAttachmentDescriptionHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['documentAttachmentDescriptionLabel'] + " </span>");
                $('#documentAttachmentDescriptionForm').removeClass().addClass('form-group has-error');
                $documentAttachmentDescription.focus();
                return false;
            }
            if ($documentAttachmentPath.val().length === 0) {
                $('#documentAttachmentPathHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['documentAttachmentPathLabel'] + " </span>");
                $('#documentAttachmentPathForm').removeClass().addClass('form-group has-error');
                $documentAttachmentPath.focus();
                return false;
            }
            $.ajax({
                type: 'POST',
                ...
                data: {
                    method: 'create',
                    output: 'json',
                    documentAttachmentDescription: $documentAttachmentDescription.val(),
                    documentAttachmentPath: $documentAttachmentPath.val(),
                    securityToken: securityToken,
                    leafId: leafId
                },
                beforeSend: function() {
                ...
                success: function(data) {
                    ...
                    if (success === true) {
                        ...
                        $documentAttachmentDescription.val('');
                        $('#documentAttachmentDescriptionForm').removeClass().addClass('form-group');
                        $('#documentAttachmentDescriptionHelpMe').html('').empty();
                        $documentAttachmentPath.val('');
                        $('#documentAttachmentPathForm').removeClass().addClass('form-group');
                        $('#documentAttachmentPathHelpMe').html('').empty();
                    } else if (success === false) {
                        $infoPanel.html('').empty().html("<span class='label label-important'>&nbsp;" + message + "</span>");
                        if ($infoPanel.is(':hidden')) {
                            $infoPanel.show();
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (success === true) {
                        ...
                        $('#documentAttachmentId').val(data.documentAttachmentId);
                        $('#newRecordButton1').removeClass().addClass('btn btn-success disabled').attr('onClick', '');
                        $('#newRecordButton2').removeClass().addClass('btn dropdown-toggle btn-success disabled');
                        $('#newRecordButton3').attr('onClick', '');
                        $('#newRecordButton4').attr('onClick', '');
...
function deleteRecord(leafId, url, urlList, securityToken, deleteAccess) {
    var $infoPanel = $('#infoPanel');
    var $documentAttachmentId = $('#documentAttachmentId');
    ...
    if (css.search('disabled') > 0) {
        ...
        if (deleteAccess === 1) {
            if (confirm(decodeURIComponent(t['deleteRecordMessageLabel']))) {
                var value = $documentAttachmentId.val();
                if (!value) {
...
function resetRecord(leafId, url, urlList, securityToken, createAccess, updateAccess, deleteAccess) {
    ...
    $("#documentAttachmentIdHelpMe").empty().html('');
    
    $("#documentAttachmentDescription").val('');
    $("#documentAttachmentDescriptionHelpMe").empty().html('');
    $('#documentAttachmentDescription').empty().val('');
    $("#documentAttachmentPath").val('');
    $("#documentAttachmentPathHelpMe").empty().html('');
    $('#documentAttachmentPath').empty().val('');
}
function postRecord() {
            ...
            success: function(data) {
                ...
                if (success === true) {
                    $.ajax({
                        ...
                        data: {
                            method: 'read',
                            documentAttachmentId: firstRecord,
                            output: 'json',
                            securityToken: securityToken,
                            leafId: leafId
                        },
                        ...
                        success: function(data) {
                            ...
                            if (success === true) {
                                ...
                                $('#documentAttachmentDescription').val(data.data.documentAttachmentDescription);
                                $('#documentAttachmentPath').val(data.data.documentAttachmentPath);
                                if (nextRecord > 0) {
                                    $('#previousRecordButton').removeClass().addClass('btn btn-default disabled').attr('onClick', '');
            ...
            success: function(data) {
                ...
                if (success === true) {
                    $.ajax({
                        ...
                        data: {
                            method: 'read',
                            documentAttachmentId: lastRecord,
                            output: 'json',
                            securityToken: securityToken,
                            leafId: leafId
                        },
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($previousRecordCounter.val()) > 0 && parseFloat($previousRecordCounter.val()) < parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    documentAttachmentId: $previousRecordCounter.val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($nextRecordCounter.val()) <= parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    documentAttachmentId: $nextRecordCounter.val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
