function getDocumentSequence(leafId, url, securityToken) {
    $.ajax({
        type: 'GET',
        url: url,
        data: {
        ...
        success: function(data) {
            ...
            if (success === false) {
                ...
                $("#documentSequenceId").html('').empty().html(data.data).trigger("chosen:updated");
...
function getLeaf(leafId, url, securityToken) {
    $.ajax({
        type: 'GET',
        url: url,
        data: {
        ...
        success: function(data) {
            ...
            if (success === false) {
                ...
                $("#leafId").html('').empty().html(data.data).trigger("chosen:updated");
                $infoPanel.html('').empty().html("<span class='label label-success'>&nbsp;<img src='" + smileyLol + "'>  " + decodeURIComponent(t['loadingCompleteTextLabel']) + "</span>").delay(5000).fadeOut();
            }
        },
        error: function(xhr) {
...
function checkDuplicate(leafId, page, securityToken) {
    var $documentSettingCode = $("#documentSettingCode");
    if ($documentSettingCode.val().length === 0) {
    ...
    $.ajax({
        ...
        url: page,
        data: {
            documentSettingCode: $documentSettingCode.val(),
            method: 'duplicate',
            securityToken: securityToken,
            leafId: leafId
        },
        ...
        success: function(data) {
            ...
            if (success === true) {
                if (total !== 0) {
                    $("#documentSettingCode").val('').focus();
                    $("#documentSettingCodeForm").removeClass().addClass("form-group has-error");
                ...
                $("#documentSettingForm").removeClass().addClass("form-group has-error");
            }
            if ($infoPanel.is(':hidden')) {
                $infoPanel.show();
            }
...
function showFormUpdate(leafId, url, urlList, securityToken, documentSettingId, updateAccess, deleteAccess) {
    sleep(500);
...
function showModalDelete(documentSettingId, documentSequenceId, leafId) {
    $('#documentSettingIdPreview').val('').val(decodeURIComponent(documentSettingId));
...
function deleteGridRecord(leafId, url, urlList, securityToken) {
    $.ajax({
        ...
        data: {
            method: 'delete',
            output: 'json',
            documentSettingId: $('#documentSettingIdPreview').val(),
            securityToken: securityToken,
            leafId: leafId
        },
        beforeSend: function() {
...
function deleteGridRecordCheckbox(leafId, url, urlList, securityToken) {
    var stringText = '';
    var counter = 0;
    $('input:checkbox[name="documentSettingId[]"]').each(function() {
        stringText = stringText + "&documentSettingId[]=" + $(this).val();
    });
...
function newRecord(leafId, url, urlList, securityToken, type, createAccess, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        if (type === 1) {
            if ($documentSequenceId.val().length === 0) {
                $('#documentSequenceIdHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['documentSequenceIdLabel'] + " </span>");
                $documentSequenceId.data('chosen').activate_action();
                return false;
            }
            if ($leafId.val().length === 0) {
                ...
                $leafId.data('chosen').activate_action();
                return false;
            }
            $.ajax({
                type: 'POST',
                ...
                success: function(data) {
                    ...
                    if (success === true) {
                        ...
                        $documentSequenceId.trigger("chosen:updated");
                        $('#documentSequenceIdHelpMe').html('').empty();
                        $leafId.val('');
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (success === true) {
                        ...
                        $('#documentSettingId').val(data.documentSettingId);
                        $('#newRecordButton1').removeClass().addClass('btn btn-success disabled').attr('onClick', '');
                        $('#newRecordButton2').removeClass().addClass('btn dropdown-toggle btn-success disabled');
                        $('#newRecordButton3').attr('onClick', '');
                        $('#newRecordButton4').attr('onClick', '');
        ...
        if (type === 1) {
            ...
            $.ajax({
                ...
                data: {
                    method: 'save',
                    output: 'json',
                    documentSettingId: $documentSettingId.val(),
                    documentSequenceId: $documentSequenceId.val(),
                    leafId: $leafId.val(),
                    securityToken: securityToken,
                    leafId : leafId
...
function deleteRecord(leafId, url, urlList, securityToken, deleteAccess) {
    var $infoPanel = $('#infoPanel');
    var $documentSettingId = $('#documentSettingId');
    ...
    if (css.search('disabled') > 0) {
        ...
        if (deleteAccess === 1) {
            if (confirm(decodeURIComponent(t['deleteRecordMessageLabel']))) {
                var value = $documentSettingId.val();
                if (!value) {
...
function resetRecord(leafId, url, urlList, securityToken, createAccess, updateAccess, deleteAccess) {
    ...
    $("#documentSettingId").val('');
    $("#documentSettingIdHelpMe").empty().html('');
                ...
                if (success === true) {
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if (success === true) {
                                $('#documentSettingId').val(data.data.documentSettingId);
                                $('#documentSequenceId').val(data.data.documentSequenceId).trigger("chosen:updated");
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($previousRecordCounter.val()) > 0 && parseFloat($previousRecordCounter.val()) < parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    documentSettingId: $previousRecordCounter.val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($nextRecordCounter.val()) <= parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    documentSettingId: $nextRecordCounter.val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
