function checkDuplicate(leafId, page, securityToken) {
    var $documentCategoryCode = $("#documentCategoryCode");
    if ($documentCategoryCode.val().length === 0) {
    ...
    $.ajax({
        ...
        url: page,
        data: {
            documentCategoryCode: $documentCategoryCode.val(),
            method: 'duplicate',
        ...
        success: function(data) {
            ...
            if (success === true) {
                if (total !== 0) {
                    $("#documentCategoryCode").val('').focus();
                    $("#documentCategoryCodeForm").removeClass().addClass("form-group has-error");
                ...
                $("#documentCategoryForm").removeClass().addClass("form-group has-error");
            }
            if ($infoPanel.is(':hidden')) {
                $infoPanel.show();
            }
...
function showFormUpdate(leafId, url, urlList, securityToken, documentCategoryId, updateAccess, deleteAccess) {
    sleep(500);
...
function showModalDelete(documentCategoryId, documentCategoryCode, documentCategoryTitle, documentCategoryDescription) {
    $('#documentCategoryIdPreview').val('').val(decodeURIComponent(documentCategoryId));
    $('#documentCategoryCodePreview').val('').val(decodeURIComponent(documentCategoryCode));
    $('#documentCategoryTitlePreview').val('').val(decodeURIComponent(documentCategoryTitle));
    $('#documentCategoryDescriptionPreview').val('').val(decodeURIComponent(documentCategoryDescription));
...
function deleteGridRecord(leafId, url, urlList, securityToken) {
    $.ajax({
        ...
        data: {
            method: 'delete',
            output: 'json',
            documentCategoryId: $('#documentCategoryIdPreview').val(),
            securityToken: securityToken,
            leafId: leafId
        },
        beforeSend: function() {
...
function deleteGridRecordCheckbox(leafId, url, urlList, securityToken) {
    var stringText = '';
    var counter = 0;
    $('input:checkbox[name="documentCategoryId[]"]').each(function() {
        stringText = stringText + "&documentCategoryId[]=" + $(this).val();
    });
...
function newRecord(leafId, url, urlList, securityToken, type, createAccess, updateAccess, deleteAccess) {
    ...
    var $documentCategoryCode = $('#documentCategoryCode');
    var $documentCategoryTitle = $('#documentCategoryTitle');
    var $documentCategoryDescription = $('#documentCategoryDescription');
    if (css.search('disabled') > 0) {
        return false;
    } else {
        if (type === 1) {
            if ($documentCategoryCode.val().length === 0) {
                $('#documentCategoryCodeHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['documentCategoryCodeLabel'] + " </span>");
                $('#documentCategoryCodeForm').removeClass().addClass('form-group has-error');
                $documentCategoryCode.focus();
                return false;
            }
            if ($documentCategoryTitle.val().length === 0) {
                $('#documentCategoryTitleHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['documentCategoryTitleLabel'] + " </span>");
                $('#documentCategoryTitleForm').removeClass().addClass('form-group has-error');
                $documentCategoryTitle.focus();
                return false;
            }
            if ($documentCategoryDescription.val().length === 0) {
                $('#documentCategoryDescriptionHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['documentCategoryDescriptionLabel'] + " </span>");
                $('#documentCategoryDescriptionForm').removeClass().addClass('form-group has-error');
                $documentCategoryDescription.focus();
                return false;
            }
            $.ajax({
                type: 'POST',
                ...
                data: {
                    ...
                    output: 'json',
                    documentCategoryCode: $documentCategoryCode.val(),
                    documentCategoryTitle: $documentCategoryTitle.val(),
                    documentCategoryDescription: $documentCategoryDescription.val(),
                    securityToken: securityToken,
                    leafId: leafId
                },
                beforeSend: function() {
                ...
                success: function(data) {
                    ...
                    if (success === true) {
                        ...
                        $('#documentCategoryCodeHelpMe').html('').empty();
                        $documentCategoryTitle.val('');
                        $('#documentCategoryTitleHelpMe').html('').empty();
                        $documentCategoryDescription.val('');
                        $('#documentCategoryDescriptionForm').removeClass().addClass('form-group');
                        $('#documentCategoryDescriptionHelpMe').html('').empty();
                    } else if (success === false) {
                        $infoPanel.html('').empty().html("<span class='label label-important'>&nbsp;" + message + "</span>");
                        if ($infoPanel.is(':hidden')) {
                            $infoPanel.show();
    ...
    if (css.search('disabled') > 0) {
        ...
        if (deleteAccess === 1) {
            if (confirm(decodeURIComponent(t['deleteRecordMessageLabel']))) {
                var value = $documentCategoryId.val();
                if (!value) {
...
function resetRecord(leafId, url, urlList, securityToken, createAccess, updateAccess, deleteAccess) {
    ...
    $("#documentCategoryCode").val('');
    $("#documentCategoryCodeHelpMe").empty().html('');
    $("#documentCategoryTitle").val('');
    $("#documentCategoryTitleHelpMe").empty().html('');
    $("#documentCategoryDescription").val('');
    $("#documentCategoryDescriptionHelpMe").empty().html('');
    $('#documentCategoryDescription').empty().val('');
}
function postRecord() {
            ...
            success: function(data) {
                ...
                if (success === true) {
                    $.ajax({
                        ...
                        data: {
                            method: 'read',
                            documentCategoryId: firstRecord,
                            output: 'json',
                            securityToken: securityToken,
                            leafId: leafId
                        },
                        ...
                        success: function(data) {
                            ...
                            if (success === true) {
                                $('#documentCategoryId').val(data.data.documentCategoryId);
                                $('#documentCategoryCode').val(data.data.documentCategoryCode);
                                $('#documentCategoryTitle').val(data.data.documentCategoryTitle);
                                $('#documentCategoryDescription').val(data.data.documentCategoryDescription);
                                if (nextRecord > 0) {
                                    $('#previousRecordButton').removeClass().addClass('btn btn-default disabled').attr('onClick', '');
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($previousRecordCounter.val()) > 0 && parseFloat($previousRecordCounter.val()) < parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    documentCategoryId: $previousRecordCounter.val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($nextRecordCounter.val()) <= parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    documentCategoryId: $nextRecordCounter.val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
