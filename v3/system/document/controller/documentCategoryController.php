...
namespace Core\System\Document\DocumentCategory\Controller;

use Core\ConfigClass;
use Core\System\Document\DocumentCategory\Model\DocumentCategoryModel;
use Core\System\Document\DocumentCategory\Service\DocumentCategoryService;
...
require_once($newFakeDocumentRoot . "v3/system/document/model/documentCategoryModel.php");
require_once($newFakeDocumentRoot . "v3/system/document/service/documentCategoryService.php");

/**
 * Class DocumentCategory
 * this is documentCategory controller files.
 * @name IDCMS
 * @version 2
 * @author hafizan
 * @package  Core\System\Document\DocumentCategory\Controller
 * @subpackage Document
...
class DocumentCategoryClass extends ConfigClass {
    ...
    /**
     * Model
     * @var \Core\System\Document\DocumentCategory\Model\DocumentCategoryModel
     */
    public $model;

    /**
     * Service-Business Application Process or other ajax request
     * @var \Core\System\Document\DocumentCategory\Service\DocumentCategoryService
     */
    public $service;
    ...
    function __construct() {
        ...
        $this->setViewPath("./v3/system/document/view/documentCategory.php");
        $this->setControllerPath("./v3/system/document/controller/documentCategoryController.php");
        $this->setServicePath("./v3/system/document/service/documentCategoryService.php");
    }
    ...
    public function create() {
        ...
        if ($this->getVendor() == self::MYSQL) {
            ...
            (
                 ...
                 '" . $this->model->getDocumentCategoryDescription() . "',
        ...
        $documentCategoryId = $this->q->lastInsertId();
        $this->q->commit();
        $end = microtime(true);
        $time = $end - $start;
        echo json_encode(
    ...
    public function read() {
        ...
        if (isset($_SESSION['isAdmin'])) {
            if ($_SESSION['isAdmin'] == 0) {
                if ($this->getVendor() == self::MYSQL) {
                    $this->setAuditFilter(
                            " `documentcategory`.`isActive` = 1  AND `documentcategory`.`companyId`='" . $this->getCompanyId(
                            ) . "' "
                    );
                } else {
                    if ($this->getVendor() == self::MSSQL) {
                ...
                if ($_SESSION['isAdmin'] == 1) {
                    if ($this->getVendor() == self::MYSQL) {
                        $this->setAuditFilter("   `documentcategory`.`companyId`='" . $this->getCompanyId() . "'	");
                    } else {
                        if ($this->getVendor() == self::MSSQL) {
                            $this->setAuditFilter(" [documentCategory].[companyId]='" . $this->getCompanyId() . "' ");
                        } else {
	...
	JOIN	`company`
	ON		`company`.`companyId` = `documentcategory`.`companyId`
		  WHERE     " . $this->getAuditFilter();
            if ($this->model->getDocumentCategoryId(0, 'single')) {
                $sql .= " AND `documentcategory`.`" . $this->model->getPrimaryKeyName(
	...
	ON		[company].[companyId] = [documentCategory].[companyId]
		  ...
		  SELECT                    DOCUMENTCATEGORY.DOCUMENTCATEGORYID AS \"documentCategoryId\",
                    ...
                    DOCUMENTCATEGORY.COMPANYID AS \"companyId\",
	...
	ON		COMPANY.COMPANYID = DOCUMENTCATEGORY.COMPANYID
         WHERE     " . $this->getAuditFilter();
                    if ($this->model->getDocumentCategoryId(0, 'single')) {
                        $sql .= " AND DOCUMENTCATEGORY. " . strtoupper(
        ...
        if ($this->getCharacterQuery()) {
            if ($this->getVendor() == self::MYSQL) {
                $sql .= " AND `documentcategory`.`" . $this->model->getFilterCharacter(
                ...
                if ($this->getVendor() == self::MSSQL) {
                    ...
                    if ($this->getVendor() == self::ORACLE) {
                        $sql .= " AND Initcap(DOCUMENTCATEGORY." . strtoupper(
        ...
        if ($this->getDateRangeStartQuery()) {
            if ($this->getVendor() == self::MYSQL) {
                $sql .= $this->q->dateFilter(
                        'documentcategory', $this->model->getFilterDate(), $this->getDateRangeStartQuery(), $this->getDateRangeEndQuery(), $this->getDateRangeTypeQuery(), $this->getDateRangeExtraTypeQuery()
                );
            } else {
                if ($this->getVendor() == self::MSSQL) {
                    $sql .= $this->q->dateFilter(
        ...
        $tableArray = null;
        if ($this->getVendor() == self::MYSQL) {
            $tableArray = array('staff', 'documentcategory');
        } else {
            if ($this->getVendor() == self::MSSQL) {
                $tableArray = array('staff', 'documentcategory');
            } else {
        ...
        while (($row = $this->q->fetchAssoc()) == true) {
            ...
            if ($this->model->getDocumentCategoryId(0, 'single')) {
                ...
                $row['previousRecord'] = $this->previousRecord(
                        'value', $this->model->getDocumentCategoryId(0, 'single')
                );
                $row['nextRecord'] = $this->nextRecord('value', $this->model->getDocumentCategoryId(0, 'single'));
    ...
    function update() {
        ...
        if ($this->getVendor() == self::MYSQL) {
           ...
           FROM 	`documentCategory`
           WHERE  	`" . $this->model->getPrimaryKeyName() . "` = '" . $this->model->getDocumentCategoryId(
                            0, 'single'
                    ) . "' ";
           ...
           FROM 	DOCUMENTCATEGORY
           WHERE  	" . strtoupper($this->model->getPrimaryKeyName()) . " = '" . $this->model->getDocumentCategoryId(
                                0, 'single'
                        ) . "' ";
            }
        }
        ...
        if ($total == 0) {
            ...
            if ($this->getVendor() == self::MYSQL) {
                $sql = "
               UPDATE `documentcategory` SET
                       `documentCategoryCode` = '" . $this->model->getDocumentCategoryCode() . "',
                       `documentCategoryTitle` = '" . $this->model->getDocumentCategoryTitle() . "',
                       `documentCategoryDescription` = '" . $this->model->getDocumentCategoryDescription() . "',
    ...
    function duplicate() {
        ...
        if ($this->getVendor() == self::MYSQL) {
           ...
           AND     [companyId] =	'" . $this->getCompanyId() . "'";
            ...
            $sql = "
               SELECT  DOCUMENTCATEGORYCODE as \"documentCategoryCode\"
               FROM    DOCUMENTCATEGORY
               WHERE   DOCUMENTCATEGORYCODE	= 	'" . $this->model->getDocumentCategoryCode() . "'
               AND     ISACTIVE    =   1
               AND     COMPANYID   =   '" . $this->getCompanyId() . "'";
    ...
    function excel() {
        ...
        $this->excel->getActiveSheet()->setCellValue('C3', $this->translate['documentCategoryCodeLabel']);
        $this->excel->getActiveSheet()->setCellValue('D3', $this->translate['documentCategoryTitleLabel']);
        $this->excel->getActiveSheet()->setCellValue('E3', $this->translate['documentCategoryDescriptionLabel']);
        ...
        while (($row = $this->q->fetchAssoc()) == true) {
            ...
            $this->excel->getActiveSheet()->setCellValue('C' . $loopRow, strip_tags($row ['documentCategoryCode']));
            $this->excel->getActiveSheet()->setCellValue('D' . $loopRow, strip_tags($row ['documentCategoryTitle']));
            $this->excel->getActiveSheet()->setCellValue(
                    'E' . $loopRow, strip_tags($row ['documentCategoryDescription'])
            );
        ...
        switch ($this->getReportMode()) {
            case 'excel':
                ...
                $extension = '.xlsx';
                $folder = 'excel';
                $filename = "documentCategory" . rand(0, 10000000) . $extension;
...
if (isset($_POST ['method'])) {
    if (isset($_POST['output'])) {
        ...
        if ($_POST['securityToken'] != $documentCategoryObject->getSecurityToken()) {
            header('Content-Type:application/json; charset=utf-8');
        ...
        if (isset($_POST ['offset'])) {
            $documentCategoryObject->setStart($_POST ['offset']);
        }
        if (isset($_POST ['limit'])) {
            $documentCategoryObject->setLimit($_POST ['limit']);
        }
        $documentCategoryObject->setPageOutput($_POST['output']);
        ...
        if ($_POST ['method'] == 'posting') {
            //	$documentCategoryObject->posting();
        }
...
if (isset($_GET ['method'])) {
    $documentCategoryObject = new DocumentCategoryClass ();
    if ($_GET['securityToken'] != $documentCategoryObject->getSecurityToken()) {
        header('Content-Type:application/json; charset=utf-8');
        echo json_encode(array("success" => false, "message" => "Something wrong with the system.Hola hackers"));
        exit();
    }
    ...
    if ($_GET ['method'] == 'dataNavigationRequest') {
        if ($_GET ['dataNavigation'] == 'firstRecord') {
            $documentCategoryObject->firstRecord('json');
        }
        if ($_GET ['dataNavigation'] == 'previousRecord') {
            $documentCategoryObject->previousRecord('json', 0);
        }
        if ($_GET ['dataNavigation'] == 'nextRecord') {
            $documentCategoryObject->nextRecord('json', 0);
        }
        if ($_GET ['dataNavigation'] == 'lastRecord') {
            $documentCategoryObject->lastRecord('json');
        }
    }
    /*
     * Excel Reporting
     */
    if (isset($_GET ['mode'])) {
        $documentCategoryObject->setReportMode($_GET['mode']);
    ...
    if (isset($_GET ['filter'])) {
        $documentCategoryObject->setServiceOutput('option');
    }
}
?>
