...
namespace Core\System\Document\Document\Controller;
...
/**
 * Class Document
 * this is document controller files.
 * @name IDCMS
 * @version 2
 * @author hafizan
 * @package  Core\System\Document\Document\Controller
 * @subpackage Document
...
class DocumentClass extends ConfigClass {
    ...
    function __construct() {
        ...
        $this->setViewPath("./v3/system/document/view/document.php");
        $this->setControllerPath("./v3/system/document/controller/documentController.php");
    }
    ...
    public function read() {
        ...
        if (isset($_SESSION['isAdmin'])) {
            if ($_SESSION['isAdmin'] == 0) {
                if ($this->getVendor() == self::MYSQL) {
                    $this->setAuditFilter(
                            " `document`.`isActive` = 1  AND `document`.`companyId`='" . $this->getCompanyId() . "' "
                    );
                } else {
                    if ($this->getVendor() == self::MSSQL) {
                        $this->setAuditFilter(
                ...
                if ($_SESSION['isAdmin'] == 1) {
                    if ($this->getVendor() == self::MYSQL) {
                        $this->setAuditFilter("   `document`.`companyId`='" . $this->getCompanyId() . "'	");
                    } else {
                        if ($this->getVendor() == self::MSSQL) {
                            $this->setAuditFilter(" [document].[companyId]='" . $this->getCompanyId() . "' ");
                        } else {
       ...
       SELECT                    `document`.`documentId`,
		  ...
		  WHERE     " . $this->getAuditFilter();
            if ($this->model->getDocumentId(0, 'single')) {
                $sql .= " AND `document`.`" . $this->model->getPrimaryKeyName() . "`='" . $this->model->getDocumentId(
                                0, 'single'
                        ) . "'";
            }
        } else {
		  ...
		  WHERE     " . $this->getAuditFilter();
                if ($this->model->getDocumentId(0, 'single')) {
                    $sql .= " AND [document].[" . $this->model->getPrimaryKeyName(
		  ...
		  SELECT                    DOCUMENT.DOCUMENTID AS \"documentId\",
                    ...
                    DOCUMENT.DOCUMENTCATEGORYID AS \"documentCategoryId\",
                    DOCUMENT.APPLICATIONID AS \"applicationId\",
                    DOCUMENT.MODULEID AS \"moduleId\",
                    DOCUMENT.FOLDERID AS \"folderId\",
                    DOCUMENT.LEAFID AS \"leafId\",
                    DOCUMENT.DOCUMENTTITLE AS \"documentTitle\",
                    DOCUMENT.DOCUMENTDESCRIPTION AS \"documentDescription\",
                    DOCUMENT.DOCUMENTPATH AS \"documentPath\",
        ...
        if ($this->getCharacterQuery()) {
            if ($this->getVendor() == self::MYSQL) {
                $sql .= " AND `document`.`" . $this->model->getFilterCharacter(
                ...
                if ($this->getVendor() == self::MSSQL) {
                    ...
                    if ($this->getVendor() == self::ORACLE) {
                        $sql .= " AND Initcap(DOCUMENT." . strtoupper(
        ...
        if ($this->getDateRangeStartQuery()) {
            if ($this->getVendor() == self::MYSQL) {
                $sql .= $this->q->dateFilter(
                        'document', $this->model->getFilterDate(), $this->getDateRangeStartQuery(), $this->getDateRangeEndQuery(), $this->getDateRangeTypeQuery(), $this->getDateRangeExtraTypeQuery()
                );
            } else {
                if ($this->getVendor() == self::MSSQL) {
                    $sql .= $this->q->dateFilter(
        ...
        $tableArray = null;
        if ($this->getVendor() == self::MYSQL) {
            $tableArray = array('staff', 'document',);
        } else {
            if ($this->getVendor() == self::MSSQL) {
                $tableArray = array('staff', 'document',);
            } else {
        ...
        while (($row = $this->q->fetchAssoc()) == true) {
            ...
            if ($this->model->getDocumentId(0, 'single')) {
                ...
                $row['previousRecord'] = $this->previousRecord('value', $this->model->getDocumentId(0, 'single'));
                $row['nextRecord'] = $this->nextRecord('value', $this->model->getDocumentId(0, 'single'));
                            ...
                            array("[", "]"), "", json_encode(
                                    array(
                                        ...
                                        'previousRecord' => $this->previousRecord(
                                                'value', $this->model->getDocumentId(0, 'single')
                                        ),
                                        'nextRecord' => $this->nextRecord('value', $this->model->getDocumentId(0, 'single')),
    ...
    function update() {
        ...
        if ($this->getVendor() == self::MYSQL) {
           ...
           FROM 	`document`
           WHERE  	`" . $this->model->getPrimaryKeyName() . "` = '" . $this->model->getDocumentId(
                            0, 'single'
                    ) . "' ";
        } else {
            if ($this->getVendor() == self::MSSQL) {
           ...
           FROM 	DOCUMENT
           WHERE  	" . strtoupper($this->model->getPrimaryKeyName()) . " = '" . $this->model->getDocumentId(
                                    0, 'single'
                            ) . "' ";
                }
            }
        ...
        if ($total == 0) {
            ...
            if ($this->getVendor() == self::MYSQL) {
               ...
               UPDATE `document` SET
                       ...
                       `documentDescription` = '" . $this->model->getDocumentDescription() . "',
  ...
  /** 
    ...
    function excel() {
        ...
        $this->excel->getActiveSheet()->setCellValue('C3', $this->translate['documentCategoryIdLabel']);
        $this->excel->getActiveSheet()->setCellValue('D3', $this->translate['applicationIdLabel']);
        $this->excel->getActiveSheet()->setCellValue('E3', $this->translate['moduleIdLabel']);
        $this->excel->getActiveSheet()->setCellValue('F3', $this->translate['folderIdLabel']);
        $this->excel->getActiveSheet()->setCellValue('G3', $this->translate['leafIdLabel']);
        $this->excel->getActiveSheet()->setCellValue('H3', $this->translate['documentTitleLabel']);
        $this->excel->getActiveSheet()->setCellValue('I3', $this->translate['documentDescriptionLabel']);
        $this->excel->getActiveSheet()->setCellValue('J3', $this->translate['documentPathLabel']);
        $this->excel->getActiveSheet()->setCellValue('K3', $this->translate['documentFilenameLabel']);
        ...
        while (($row = $this->q->fetchAssoc()) == true) {
            ...
            $this->excel->getActiveSheet()->setCellValue(
                    'C' . $loopRow, strip_tags($row ['documentCategoryDescription'])
            );
            $this->excel->getActiveSheet()->setCellValue('D' . $loopRow, strip_tags($row ['applicationDescription']));
            $this->excel->getActiveSheet()->setCellValue('E' . $loopRow, strip_tags($row ['moduleDescription']));
            $this->excel->getActiveSheet()->setCellValue('F' . $loopRow, strip_tags($row ['folderDescription']));
            $this->excel->getActiveSheet()->setCellValue('G' . $loopRow, strip_tags($row ['leafDescription']));
            $this->excel->getActiveSheet()->setCellValue('H' . $loopRow, strip_tags($row ['documentTitle']));
            $this->excel->getActiveSheet()->setCellValue('I' . $loopRow, strip_tags($row ['documentDescription']));
            $this->excel->getActiveSheet()->setCellValue('J' . $loopRow, strip_tags($row ['documentPath']));
            $this->excel->getActiveSheet()->setCellValue('K' . $loopRow, strip_tags($row ['documentFilename']));
        ...
        switch ($this->getReportMode()) {
            case 'excel':
                ...
                $extension = '.xlsx';
                $folder = 'excel';
                $filename = "document" . rand(0, 10000000) . $extension;
...
if (isset($_POST ['method'])) {
    if (isset($_POST['output'])) {
        ...
        if ($_POST['securityToken'] != $documentObject->getSecurityToken()) {
            header('Content-Type:application/json; charset=utf-8');
        ...
        if (isset($_POST ['offset'])) {
            $documentObject->setStart($_POST ['offset']);
        }
        if (isset($_POST ['limit'])) {
            $documentObject->setLimit($_POST ['limit']);
        }
        $documentObject->setPageOutput($_POST['output']);
...
if (isset($_GET ['method'])) {
    $documentObject = new DocumentClass ();
    if ($_GET['securityToken'] != $documentObject->getSecurityToken()) {
        header('Content-Type:application/json; charset=utf-8');
        echo json_encode(array("success" => false, "message" => "Something wrong with the system.Hola hackers"));
        exit();
    }
     ...
     */
    if (isset($_GET ['mode'])) {
        $documentObject->setReportMode($_GET['mode']);
