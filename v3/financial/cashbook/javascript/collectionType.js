function checkDuplicate(leafId, page, securityToken) {
    ...
    if ($collectionTypeCode.val().length === 0) {
    ...
    $.ajax({
        ...
        url: page,
        data: {
            collectionTypeCode: $collectionTypeCode.val(),
            method: 'duplicate',
        ...
        success: function(data) {
            ...
            if (success === true) {
                if (total !== 0) {
                    $("#collectionTypeCode").val('').focus();
                    $("#collectionTypeCodeForm").removeClass().addClass("form-group has-error");
                ...
                $("#collectionTypeForm").removeClass().addClass("form-group has-error");
            }
            if ($infoPanel.is(':hidden')) {
                $infoPanel.show();
            }
...
function showFormUpdate(leafId, url, urlList, securityToken, collectionTypeId, updateAccess, deleteAccess) {
    sleep(500);
...
function showModalDelete(collectionTypeId, collectionTypeCode, collectionTypeDescription) {
    $('#collectionTypeIdPreview').val('').val(decodeURIComponent(collectionTypeId));
    $('#collectionTypeCodePreview').val('').val(decodeURIComponent(collectionTypeCode));
    $('#collectionTypeDescriptionPreview').val('').val(decodeURIComponent(collectionTypeDescription));
...
function deleteGridRecord(leafId, url, urlList, securityToken) {
    $.ajax({
        ...
        data: {
            method: 'delete',
            output: 'json',
            collectionTypeId: $('#collectionTypeIdPreview').val(),
            securityToken: securityToken,
            leafId: leafId
        },
        beforeSend: function() {
...
function deleteGridRecordCheckbox(leafId, url, urlList, securityToken) {
    var stringText = '';
    var counter = 0;
    $('input:checkbox[name="collectionTypeId[]"]').each(function() {
        stringText = stringText + "&collectionTypeId[]=" + $(this).val();
    });
...
function newRecord(leafId, url, urlList, securityToken, type, createAccess, updateAccess, deleteAccess) {
    ...
    var $collectionTypeCode = $('#collectionTypeCode');
    var $collectionTypeDescription = $('#collectionTypeDescription');
    if (css.search('disabled') > 0) {
        return false;
    } else {
        if (type === 1) {
            if ($collectionTypeCode.val().length === 0) {
                $('#collectionTypeCodeHelpMe').html('').empty().html("<span class='label label-danger'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['collectionTypeCodeLabel'] + " </span>");
                $('#collectionTypeCodeForm').removeClass().addClass('form-group has-error');
                $collectionTypeCode.focus();
                return false;
            }
            if ($collectionTypeDescription.val().length === 0) {
                $('#collectionTypeDescriptionHelpMe').html('').empty().html("<span class='label label-danger'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['collectionTypeDescriptionLabel'] + " </span>");
                $('#collectionTypeDescriptionForm').removeClass().addClass('form-group has-error');
                $collectionTypeDescription.focus();
                return false;
            }
            $.ajax({
                type: 'POST',
                ...
                data: {
                    ...
                    output: 'json',
                    collectionTypeCode: $collectionTypeCode.val(),
                    collectionTypeDescription: $collectionTypeDescription.val(),
                    securityToken: securityToken,
                    leafId: leafId
                },
                beforeSend: function() {
                ...
                success: function(data) {
                    ...
                    if (success === true) {
                        ...
                        $collectionTypeCode.val('');
                        $('#collectionTypeCodeHelpMe').html('').empty();
                        $collectionTypeDescription.val('');
                        $('#collectionTypeDescriptionForm').removeClass().addClass('form-group');
                        $('#collectionTypeDescriptionHelpMe').html('').empty();
                    } else if (success === false) {
                        $infoPanel.html('').empty().html("<span class='label label-danger'>&nbsp;" + message + "</span>");
                        if ($infoPanel.is(':hidden')) {
                            $infoPanel.show();
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (success === true) {
                        ...
                        $('#collectionTypeId').val(data.collectionTypeId);
                        $('#newRecordButton1').removeClass().addClass('btn btn-success disabled').attr('onClick', '');
                        $('#newRecordButton2').removeClass().addClass('btn dropdown-toggle btn-success disabled');
                        $('#newRecordButton3').attr('onClick', '');
                        $('#newRecordButton4').attr('onClick', '');
    ...
    if (css.search('disabled') > 0) {
        ...
        if (deleteAccess === 1) {
            if (confirm(decodeURIComponent(t['deleteRecordMessageLabel']))) {
                var value = $collectionTypeId.val();
                if (!value) {
...
function resetRecord(leafId, url, urlList, securityToken, createAccess, updateAccess, deleteAccess) {
    ...
    $("#collectionTypeCode").val('');
    $("#collectionTypeCodeHelpMe").empty().html('');
    $("#collectionTypeDescription").val('');
    $("#collectionTypeDescriptionHelpMe").empty().html('');
    $('#collectionTypeDescription').empty().val('');
}
function postRecord() {
                ...
                if (success === true) {
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if (success === true) {
                                $('#collectionTypeId').val(data.data.collectionTypeId);
                                $('#collectionTypeCode').val(data.data.collectionTypeCode);
                                $('#collectionTypeDescription').val(data.data.collectionTypeDescription);
                                if (nextRecord > 0) {
                                    $('#previousRecordButton').removeClass().addClass('btn btn-default disabled').attr('onClick', '');
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($previousRecordCounter.val()) > 0 && parseFloat($previousRecordCounter.val()) < parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    collectionTypeId: $previousRecordCounter.val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($nextRecordCounter.val()) <= parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    collectionTypeId: $nextRecordCounter.val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
