function checkDuplicate(leafId, page, securityToken) {
    ...
    if ($paymentTypeCode.val().length === 0) {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if (success === true) {
                if (total !== 0) {
                    $("#paymentTypeCode").val('').focus();
                    $("#paymentTypeCodeForm").removeClass().addClass("form-group has-error");
                ...
                $("#paymentTypeForm").removeClass().addClass("form-group has-error");
            }
            if ($infoPanel.is(':hidden')) {
                $infoPanel.show();
            }
...
function showFormUpdate(leafId, url, urlList, securityToken, paymentTypeId, updateAccess, deleteAccess) {
    sleep(500);
...
function showModalDelete(paymentTypeId, paymentTypeCode, paymentTypeDescription, isCollection, isPaymentVoucher) {
    $('#paymentTypeIdPreview').val('').val(decodeURIComponent(paymentTypeId));
    $('#paymentTypeCodePreview').val('').val(decodeURIComponent(paymentTypeCode));
    $('#paymentTypeDescriptionPreview').val('').val(decodeURIComponent(paymentTypeDescription));
...
function deleteGridRecord(leafId, url, urlList, securityToken) {
    $.ajax({
        ...
        data: {
            method: 'delete',
            output: 'json',
            paymentTypeId: $('#paymentTypeIdPreview').val(),
            securityToken: securityToken,
            leafId: leafId
        },
        beforeSend: function() {
...
function deleteGridRecordCheckbox(leafId, url, urlList, securityToken) {
    var stringText = '';
    var counter = 0;
    $('input:checkbox[name="paymentTypeId[]"]').each(function() {
        stringText = stringText + "&paymentTypeId[]=" + $(this).val();
    });
...
function newRecord(leafId, url, urlList, securityToken, type, createAccess, updateAccess, deleteAccess) {
    ...
    var $paymentTypeCode = $('#paymentTypeCode');
    var $paymentTypeDescription = $('#paymentTypeDescription');
    ...
    if (css.search('disabled') > 0) {
        ...
        if (type === 1) {
            if ($paymentTypeCode.val().length === 0) {
                $('#paymentTypeCodeHelpMe').html('').empty().html("<span class='label label-danger'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['paymentTypeCodeLabel'] + " </span>");
                $('#paymentTypeCodeForm').removeClass().addClass('form-group has-error');
                $paymentTypeCode.focus();
                return false;
            }
            if ($paymentTypeDescription.val().length === 0) {
                $('#paymentTypeDescriptionHelpMe').html('').empty().html("<span class='label label-danger'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['paymentTypeDescriptionLabel'] + " </span>");
                $('#paymentTypeDescriptionForm').removeClass().addClass('form-group has-error');
                $paymentTypeDescription.focus();
                return false;
            }
            if ($isCollection.val().length === 0) {
                $('#isCollectionHelpMe').html('').empty().html("<span class='label label-danger'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['isCollectionLabel'] + " </span>");
                $('#isCollectionForm').removeClass().addClass('form-group has-error');
                $isCollection.focus();
                return false;
            }
            if ($isPaymentVoucher.val().length === 0) {
                $('#isPaymentVoucherHelpMe').html('').empty().html("<span class='label label-danger'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['isPaymentVoucherLabel'] + " </span>");
                $('#isPaymentVoucherForm').removeClass().addClass('form-group has-error');
                $isPaymentVoucher.focus();
                return false;
            }
            $.ajax({
                type: 'POST',
                ...
                data: {
                    ...
                    output: 'json',
                    paymentTypeCode: $paymentTypeCode.val(),
                    paymentTypeDescription: $paymentTypeDescription.val(),
                ...
                success: function(data) {
                    ...
                    if (success === true) {
                        ...
                        $paymentTypeCode.val('');
                        $('#paymentTypeCodeHelpMe').html('').empty();
                        $paymentTypeDescription.val('');
                        $('#paymentTypeDescriptionForm').removeClass().addClass('form-group');
                        $('#paymentTypeDescriptionHelpMe').html('').empty();
                        $isCollection.val('');
...
function resetRecord(leafId, url, urlList, securityToken, createAccess, updateAccess, deleteAccess) {
    ...
    $("#paymentTypeCode").val('');
    $("#paymentTypeCodeHelpMe").empty().html('');
    $("#paymentTypeDescription").val('');
    $("#paymentTypeDescriptionHelpMe").empty().html('');
    $('#paymentTypeDescription').empty().val('');
    $("#isCollection").val('');
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if (success === true) {
                                ...
                                $('#paymentTypeCode').val(data.data.paymentTypeCode);
                                $('#paymentTypeDescription').val(data.data.paymentTypeDescription);
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($previousRecordCounter.val()) > 0 && parseFloat($previousRecordCounter.val()) < parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    paymentTypeId: $previousRecordCounter.val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($nextRecordCounter.val()) <= parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    paymentTypeId: $nextRecordCounter.val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
