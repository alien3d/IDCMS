function getCollection(leafId, url, securityToken) {
...
function getCollectionCancellationType(leafId, url, securityToken) {
    // un hide button search
    // unlimited for searching because  lazy paging.
    $.ajax({
        type: 'GET',
        ...
        data: {
            ...
            securityToken: securityToken,
            leafId: leafId,
            filter: 'collectionCancellationType'
        },
        beforeSend: function() {
            // this is where we append a loading image
            $('#infoPanel').empty();
        ...
        success: function(data) {
            ...
            if (data.success == false) {
                ...
                $('#infoPanel').empty();
                $('#infoPanel').html('');
                $("#collectionCancellationTypeId").empty();
                $("#collectionCancellationTypeId").html('');
                $("#collectionCancellationTypeId").html(data.data);
                $("#collectionCancellationTypeId").trigger("chosen:updated");
                $('#infoPanel').html("<span class='label label-success'>&nbsp;<img src='./images/icons/smiley-lol.png'>  " + decodeURIComponent(t['loadingCompleteTextLabel']) + "</spanm>").delay(5000).fadeOut();
            }
        },
        error: function(xhr) {
...
function checkDuplicate(leafId, page, securityToken) {
    if ($("#collectionCancellationCode").val().length == 0) {
    ...
    $.ajax({
        ...
        url: page,
        data: {
            collectionCancellationCode: $("#collectionCancellationCode").val(),
            method: 'duplicate',
            securityToken: securityToken,
            leafId: leafId
        },
        ...
        success: function(data) {
            ...
            if (data.success == true) {
                if (data.total != 0) {
                    $("#collectionCancellationCode").empty();
                    $("#collectionCancellationCode").val('');
                    $("#collectionCancellationCode").focus();
                    $("#collectionCancellationCodeForm").removeClass();
                    $("#collectionCancellationCodeForm").addClass("col-md-12 form-group has-error");
                    $('#infoPanel').empty();
                    $('#infoPanel').html('');
                ...
                $("#collectionCancellationForm").removeClass();
                $("#collectionCancellationForm").addClass("col-md-12 form-group has-error");
            }
        },
        error: function(xhr) {
            $('#infoError').empty();
...
function showFormUpdate(leafId, url, urlList, securityToken, collectionCancellationId, updateAccess, deleteAccess) {
    sleep(500);
    // unlimited for searching because  lazy paging.
    $.ajax({
        type: 'POST',
...
function showModalDelete(collectionCancellationId, collectionId, collectionCancellationTypeId, documentNumber, collectionCancellationDate, collectionCancellationDescription) {
    // clear first old record if exist
    $('#collectionCancellationIdPreview').val('');
    $('#collectionCancellationIdPreview').val(decodeURIComponent(collectionCancellationId));

    $('#collectionIdPreview').val('');
    $('#collectionIdPreview').val(decodeURIComponent(collectionId));

    $('#collectionCancellationTypeIdPreview').val('');
    $('#collectionCancellationTypeIdPreview').val(decodeURIComponent(collectionCancellationTypeId));

    $('#documentNumberPreview').val('');
    $('#documentNumberPreview').val(decodeURIComponent(documentNumber));

    $('#collectionCancellationDatePreview').val('');
    $('#collectionCancellationDatePreview').val(decodeURIComponent(collectionCancellationDate));

    $('#collectionCancellationDescriptionPreview').val('');
    $('#collectionCancellationDescriptionPreview').val(decodeURIComponent(collectionCancellationDescription));
...
function deleteGridRecord(leafId, url, urlList, securityToken) {
    $.ajax({
        ...
        data: {
            method: 'delete',
            output: 'json',
            collectionCancellationId: $('#collectionCancellationIdPreview').val(),
            securityToken: securityToken,
            leafId: leafId
        },
        beforeSend: function() {
...
function deleteGridRecordCheckbox(leafId, url, urlList, securityToken) {
    var stringText = '';
    var counter = 0;
    $('input:checkbox[name="collectionCancellationId[]"]').each(function() {
        stringText = stringText + "&collectionCancellationId[]=" + $(this).val();
    });
    ...
    if (css.search('disabled') > 0) {
        ...
        if (type == 1) {
            ...
            if ($('#collectionId').val().length == 0) {
                $('#collectionIdHelpMe').empty();
                $('#collectionIdHelpMe').html("<span class='label label-danger'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['collectionIdLabel'] + " </span>");
                $('#collectionId').data('chosen').activate_action();
                return false;
            }
            if ($('#collectionCancellationTypeId').val().length == 0) {
                $('#collectionCancellationTypeIdHelpMe').empty();
                $('#collectionCancellationTypeIdHelpMe').html("<span class='label label-danger'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['collectionCancellationTypeIdLabel'] + " </span>");
                $('#collectionCancellationTypeId').data('chosen').activate_action();
                return false;
            }
            if ($('#collectionCancellationDate').val().length == 0) {
                $('#collectionCancellationDateHelpMe').empty();
                $('#collectionCancellationDateHelpMe').html("<span class='label label-danger'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['collectionCancellationDateLabel'] + " </span>");
                $('#collectionCancellationDateForm').addClass('form-group has-error');
                $('#collectionCancellationDate').focus();
                return false;
            }
            if ($('#collectionCancellationDescription').val().length == 0) {
                $('#collectionCancellationDescriptionHelpMe').empty();
                $('#collectionCancellationDescriptionHelpMe').html("<span class='label label-danger'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['collectionCancellationDescriptionLabel'] + " </span>");
                $('#collectionCancellationDescriptionForm').addClass('form-group has-error');
                $('#collectionCancellationDescription').focus();
                return false;
            }
            $.ajax({
                type: 'POST',
                ...
                data: {
                    ...
                    collectionCancellationTypeId: $('#collectionCancellationTypeId').val(),
                    collectionCancellationDate: $('#collectionCancellationDate').val(),
                    collectionCancellationDescription: $('#collectionCancellationDescription').val(),
                    securityToken: securityToken,
                    leafId: leafId
                },
                beforeSend: function() {
                ...
                success: function(data) {
                    ...
                    if (data.success == true) {
                        ...
                        $('#collectionId').trigger("chosen:updated");
                        $('#collectionIdHelpMe').empty();
                        $('#collectionIdHelpMe').html('');
                        $('#collectionCancellationTypeId').val('');
                        $('#collectionCancellationTypeId').trigger("chosen:updated");
                        $('#collectionCancellationTypeIdHelpMe').empty();
                        $('#collectionCancellationTypeIdHelpMe').html('');
                        $('#documentNumber').val('');
                        $('#documentNumber').val('');
                        $('#documentNumberHelpMe').empty();
                        $('#documentNumberHelpMe').html('');
                        $('#collectionCancellationDate').val('');
                        $('#collectionCancellationDateHelpMe').empty();
                        $('#collectionCancellationDateHelpMe').html('');
                        $('#collectionCancellationDescription').val('');
                        $('#collectionCancellationDescriptionForm').removeClass().addClass('col-md-12 form-group');
                        $('#collectionCancellationDescription').val('');
                        $('#collectionCancellationDescriptionHelpMe').empty();
                        $('#collectionCancellationDescriptionHelpMe').html('');
                    } else if (data.success == false) {
                        $('#infoPanel').empty();
                        $('#infoPanel').html("<span class='label label-danger'>&nbsp;" + data.message + "</span>");
                        if ($('#infoPanel').is(':hidden')) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (data.success == true) {
                        ...
                        $('#collectionCancellationId').val(data.collectionCancellationId);
                        $('#newRecordButton1').removeClass();
                        $('#newRecordButton2').removeClass();
                        $('#newRecordButton3').removeClass();
                        $('#newRecordButton4').removeClass();
        ...
        if (type == 1) {
            ...
            $.ajax({
                ...
                data: {
                    method: 'save',
                    output: 'json',
                    collectionCancellationId: $('#collectionCancellationId').val(),
                    collectionId: $('#collectionId').val(),
                    collectionCancellationTypeId: $('#collectionCancellationTypeId').val(),
                    collectionCancellationDate: $('#collectionCancellationDate').val(),
                    collectionCancellationDescription: $('#collectionCancellationDescription').val(),
    ...
    if (css.search('disabled') > 0) {
        ...
        if (deleteAccess == 1) {
            if (confirm(decodeURIComponent(t['deleteRecordMessageLabel']))) {
                var value = $('#collectionCancellationId').val();
                if (!value) {
                    $('#infoPanel').empty();
                    $('#infoPanel').html('');
...
function resetRecord(leafId, url, urlList, securityToken, createAccess, updateAccess, deleteAccess) {
    ...
    $("#collectionCancellationId").val('');
    $("#collectionCancellationIdHelpMe").empty();
    $("#collectionCancellationIdHelpMe").html('');
    ...
    $("#collectionCancellationDescriptionHelpMe").empty();
    $("#collectionCancellationDescriptionHelpMe").html('');
    $('#collectionCancellationDescription').empty();
    $('#collectionCancellationDescription').val('');
}
            ...
            success: function(data) {
                ...
                if (data.success == true) {
                    $.ajax({
                        ...
                        data: {
                            method: 'read',
                            collectionCancellationId: data.firstRecord,
                            output: 'json',
                            securityToken: securityToken,
                            leafId: leafId
                        },
                        ...
                        success: function(data) {
                            ...
                            if (data.success == true) {
                                // resetting field value
                                $('#collectionCancellationId').val(data.data.collectionCancellationId);
                                $('#collectionId').val(data.data.collectionId);
                                $('#collectionId').trigger("chosen:updated");
                                $('#collectionCancellationTypeId').val(data.data.collectionCancellationTypeId);
                                $('#collectionCancellationTypeId').trigger("chosen:updated");
                                $('#documentNumber').val(data.data.documentNumber);
                                var x = data.data.collectionCancellationDate;
                                x = x.split("-");
                                var output = x[2] + "-" + x[1] + "-" + x[0];
                                output = output.toString();
                                $('#collectionCancellationDate').val(output);
                                $('#collectionCancellationDescription').val(data.data.collectionCancellationDescription);
            ...
            success: function(data) {
                ...
                if (data.success == true) {
                    $.ajax({
                        ...
                        data: {
                            method: 'read',
                            collectionCancellationId: data.lastRecord,
                            output: 'json',
                            securityToken: securityToken,
                            leafId: leafId
                        },
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($('#previousRecordCounter').val()) > 0 && parseFloat($('#previousRecordCounter').val()) < parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    collectionCancellationId: $('#previousRecordCounter').val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($('#nextRecordCounter').val()) <= parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    collectionCancellationId: $('#nextRecordCounter').val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
