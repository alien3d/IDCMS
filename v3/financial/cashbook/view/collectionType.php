...
require_once($newFakeDocumentRoot . "v3/financial/cashbook/controller/collectionTypeController.php");
...
$translator->setCurrentTable('collectionType');
...
if (isset($_POST)) {
    if (isset($_POST['method'])) {
        $collectionType = new \Core\Financial\Cashbook\CollectionType\Controller\CollectionTypeClass();
        define('LIMIT', 10);
        ...
        if (isset($_POST ['query'])) {
            $collectionType->setFieldQuery($_POST ['query']);
        }
        if (isset($_POST ['filter'])) {
            $collectionType->setGridQuery($_POST ['filter']);
        }
        if (isset($_POST ['character'])) {
            $collectionType->setCharacterQuery($_POST['character']);
        }
        if (isset($_POST ['dateRangeStart'])) {
            $collectionType->setDateRangeStartQuery($_POST['dateRangeStart']);
            //explode the data to get day,month,year
            $start = explode('-', $_POST ['dateRangeStart']);
            $collectionType->setStartDay($start[2]);
            $collectionType->setStartMonth($start[1]);
            $collectionType->setStartYear($start[0]);
        }
        if (isset($_POST ['dateRangeEnd']) && (strlen($_POST['dateRangeEnd']) > 0)) {
            $collectionType->setDateRangeEndQuery($_POST['dateRangeEnd']);
            //explode the data to get day,month,year
            $start = explode('-', $_POST ['dateRangeEnd']);
            $collectionType->setEndDay($start[2]);
            $collectionType->setEndMonth($start[1]);
            $collectionType->setEndYear($start[0]);
        }
        if (isset($_POST ['dateRangeType'])) {
            $collectionType->setDateRangeTypeQuery($_POST['dateRangeType']);
        }
        if (isset($_POST ['dateRangeExtraType'])) {
            $collectionType->setDateRangeExtraTypeQuery($_POST['dateRangeExtraType']);
        }
        $collectionType->setServiceOutput('html');
        $collectionType->setLeafId($leafId);
        $collectionType->execute();
        if ($_POST['method'] == 'read') {
            $collectionType->setStart($offset);
            $collectionType->setLimit($limit); // normal system don't like paging..
            $collectionType->setPageOutput('html');
            $collectionTypeArray = $collectionType->read();
            if (isset($collectionTypeArray [0]['firstRecord'])) {
                $firstRecord = $collectionTypeArray [0]['firstRecord'];
            }
            if (isset($collectionTypeArray [0]['nextRecord'])) {
                $nextRecord = $collectionTypeArray [0]['nextRecord'];
            }
            if (isset($collectionTypeArray [0]['previousRecord'])) {
                $previousRecord = $collectionTypeArray [0]['previousRecord'];
            }
            if (isset($collectionTypeArray [0]['lastRecord'])) {
                $lastRecord = $collectionTypeArray [0]['lastRecord'];
                $endRecord = $collectionTypeArray [0]['lastRecord'];
            }
            $navigation = new \Core\Paging\HtmlPaging();
            $navigation->setLeafId($leafId);
            $navigation->setViewPath($collectionType->getViewPath());
            ...
            if (isset($collectionTypeArray [0]['total'])) {
                $total = $collectionTypeArray [0]['total'];
            } else {
                $total = 0;
            }
            ...
            <div class="row">
                <div class="pull-left btn-group col-xs-10 col-sm-10 col-md-10">
                    <button title="A" class="btn btn-success btn-sm" type="button" 
                            ...
                            echo $collectionType->getViewPath();
                        ...
                        <div class="btn-group">
                            ...
                            <ul class="dropdown-menu" style="text-align:left">
                                <li>
                                    ...
                                    echo $collectionType->getControllerPath();
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <form class="form-horizontal">
                                        <input type="hidden" name="collectionTypeIdPreview" id="collectionTypeIdPreview">

                                        <div class="form-group" id="collectionTypeCodeDiv">
                                            <label for="collectionTypeCodePreview"
                                                   class="control-label col-xs-4 col-sm-4 col-md-4"><?php echo $leafTranslation['collectionTypeCodeLabel']; ?></label>

                                            <div class="col-xs-8 col-sm-8 col-md-8">
                                                <input type="text" class="form-control" name="collectionTypeCodePreview"
                                                       id="collectionTypeCodePreview">
                                            </div>
                                        </div>
                                        <div class="form-group" id="collectionTypeDescriptionDiv">
                                            <label for="collectionTypeDescriptionPreview"
                                                   class="control-label col-xs-4 col-sm-4 col-md-4"><?php echo $leafTranslation['collectionTypeDescriptionLabel']; ?></label>

                                            <div class="col-xs-8 col-sm-8 col-md-8">
                                                <input type="text" class="form-control"
                                                       name="collectionTypeDescriptionPreview"
                                                       id="collectionTypeDescriptionPreview">
                                            </div>
                                        </div>
                                    </form>
                                </div>
                        ...
                        <div class="col-xs-12 col-sm-12 col-md-12">
                            <div class="panel panel-default">
                                <table class="table table table-bordered table-striped table-condensed table-hover smart-form has-tickbox" id="tableData">
                                    ...
                                    <th width="100px">
                                    <div align="center"><?php echo ucwords($leafTranslation['collectionTypeCodeLabel']); ?></div>
                                    </th>
                                    <th><?php echo ucwords($leafTranslation['collectionTypeDescriptionLabel']); ?></th>
                                    <th width="100px">
                                            ...
                                            if (is_array($collectionTypeArray)) {
                                                ...
                                                if ($totalRecord > 0) {
                                                    ...
                                                    for ($i = 0; $i < $totalRecord; $i++) {
                                                        ...
                                                        ?>
                                                        <tr <?php
                                                        if ($collectionTypeArray[$i]['isDelete'] == 1) {
                                                            echo "class=\"danger\"";
                                                        } else {
                                                            if ($collectionTypeArray[$i]['isDraft'] == 1) {
                                                                echo "class=\"warning\"";
                                                            }
                                                        }
                                                        ?>>
                                                            ...
                                                            <td align="center">
                                                                <?php if ($collectionTypeArray [$i]['collectionTypeCode'] != 'UNBL') { ?>
                                                                    <div class="btn-group" align="center">
                                                                        <button type="button"  class="btn btn-info btn-sm" title="Edit"
                                                                                ...
                                                                                echo intval(
                                                                                        $collectionTypeArray [$i]['collectionTypeId']
                                                                                );
                                                                        ...
                                                                        ?>', '<?php
                                                                        echo rawurlencode(
                                                                                $collectionTypeArray [$i]['collectionTypeCode']
                                                                        );
                                                                        ?>', '<?php
                                                                        echo rawurlencode(
                                                                                $collectionTypeArray [$i]['collectionTypeDescription']
                                                                        );
                                                                        ?>');">
                                                            ...
                                                            <td>
                                                                <div align="center">
                                                                    <?php
                                                                    if (isset($collectionTypeArray[$i]['collectionTypeCode'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos(
                                                                                                strtolower($collectionTypeArray[$i]['collectionTypeCode']), strtolower($_POST['query'])
                                                                                        ) !== false
                                                                                ) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $collectionTypeArray[$i]['collectionTypeCode']
                                                                                    );
                                                                                } else {
                                                                                    echo $collectionTypeArray[$i]['collectionTypeCode'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                    if (strpos(
                                                                                                    strtolower($collectionTypeArray[$i]['collectionTypeCode']), strtolower($_POST['character'])
                                                                                            ) !== false
                                                                                    ) {
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $collectionTypeArray[$i]['collectionTypeCode']
                                                                                        );
                                                                                    } else {
                                                                                        echo $collectionTypeArray[$i]['collectionTypeCode'];
                                                                                    }
                                                            ...
                                                            <td>
                                                                <div class="pull-left">
                                                                    <?php
                                                                    if (isset($collectionTypeArray[$i]['collectionTypeDescription'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos(
                                                                                                strtolower($collectionTypeArray[$i]['collectionTypeDescription']), strtolower($_POST['query'])
                                                                                        ) !== false
                                                                                ) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $collectionTypeArray[$i]['collectionTypeDescription']
                                                                                    );
                                                                                } else {
                                                                                    echo $collectionTypeArray[$i]['collectionTypeDescription'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                    if (strpos(
                                                                                        ...
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $collectionTypeArray[$i]['collectionTypeDescription']
                                                                                        );
                                                                                    } else {
                                                                                        echo $collectionTypeArray[$i]['collectionTypeDescription'];
                                                                                    }
                                                            ...
                                                            <td>
                                                                <div align="center">
                                                                    <?php
                                                                    if (isset($collectionTypeArray[$i]['executeBy'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos(
                                                                                                $collectionTypeArray[$i]['staffName'], $_POST['query']
                                                                                        ) !== false
                                                                                ) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $collectionTypeArray[$i]['staffName']
                                                                                    );
                                                                                } else {
                                                                                    echo $collectionTypeArray[$i]['staffName'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                    if (strpos(
                                                                                                    $collectionTypeArray[$i]['staffName'], $_POST['character']
                                                                                            ) !== false
                                                                                    ) {
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $collectionTypeArray[$i]['staffName']
                                                                                        );
                                                                                    } else {
                                                                                        echo $collectionTypeArray[$i]['staffName'];
                                                                                    }
                                                            ...
                                                            <?php
                                                            if (isset($collectionTypeArray[$i]['executeTime'])) {
                                                                $valueArray = $collectionTypeArray[$i]['executeTime'];
                                                            ...
                                                            <?php } ?>
                                                            <?php
                                                            if ($collectionTypeArray[$i]['isDelete']) {
                                                                $checked = "checked";
                                                            } else {
                                                                $checked = null;
                                                            }
                                                            ...
                                                            <td>
                                                                <?php if ($collectionTypeArray [$i]['collectionTypeCode'] != 'UNBL') { ?>
                                                                    <input style="display:none;" type="checkbox" name="collectionTypeId[]"
                                                                           value="<?php echo $collectionTypeArray[$i]['collectionTypeId']; ?>">
                                                                    <input <?php echo $checked; ?> type="checkbox" name="isDelete[]"
                                                                                                   value="<?php echo $collectionTypeArray[$i]['isDelete']; ?>">
                                                                                               <?php } ?>
                                                            </td>
                                                        </tr>
                                                        <?php
                                                    ...
                                                    <tr>
                                                        <td colspan="7" vAlign="top" align="center"><?php
                                                            $collectionType->exceptionMessage(
                        ...
                        <div class="col-xs-3 col-sm-3 col-md-3 pagination">
                            <div class="pull-right">
                                <button class="delete btn btn-warning" type="button" 
                                        ...
                                        echo $collectionType->getControllerPath();
                                        ?>', '<?php echo $collectionType->getViewPath(); ?>', '<?php echo $securityToken; ?>');">
...
if ((isset($_POST['method']) == 'new' || isset($_POST['method']) == 'read') && $_POST['type'] == 'form') {
            ...
            <div class="row">
                <div class="col-xs-12 col-sm-12 col-md-12">
                    <div class="panel panel-info">
                        ...
                        <div class="panel-body">
                            <input type="hidden" name="collectionTypeId" id="collectionTypeId" value="<?php
                            if (isset($_POST['collectionTypeId'])) {
                                echo $_POST['collectionTypeId'];
                            }
                            ?>">

                            <div class="row">
                                <div class="col-xs-12 col-sm-12 col-md-12">
                                    <div class="col-xs-6 col-sm-6 col-md-6 form-group" id="collectionTypeCodeForm">
                                        <label for="collectionTypeCode" class="control-label col-xs-2 col-sm-2 col-md-2"><strong><?php
                                                echo ucfirst(
                                                        $leafTranslation['collectionTypeCodeLabel']
                                                );
                                                ?></strong></label>

                                        <div class="control-label col-xs-4 col-sm-4 col-md-4">
                                            <input type="text" class="form-control" name="collectionTypeCode"
                                                   id="collectionTypeCode" value="<?php
                                                   if (isset($collectionTypeArray) && is_array($collectionTypeArray)) {
                                                       if (isset($collectionTypeArray[0]['collectionTypeCode'])) {
                                                           echo htmlentities($collectionTypeArray[0]['collectionTypeCode']);
                                                       }
                                                   }
                                                   ?>" maxlength="16"> <span class="help-block" id="collectionTypeCodeHelpMe"></span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-xs-12 col-sm-12 col-md-12">
                                    <div class="form-group" id="collectionTypeDescriptionForm">
                                        <div class="col-xs-12 col-sm-12 col-md-12">
                                            <textarea rows="5" class="form-control" name="collectionTypeDescription"
                                                      id="collectionTypeDescription"><?php
                                                          if (isset($collectionTypeArray[0]['collectionTypeDescription'])) {
                                                              echo htmlentities($collectionTypeArray[0]['collectionTypeDescription']);
                                                          }
                                                          ?></textarea> <span class="help-block" id="collectionTypeDescriptionHelpMe"></span>
                                        </div>
                                    </div>
                                </div>
                            </div>
    ...
    <script type="text/javascript">
        $(document).keypress(function(e) {
            switch (e.keyCode) {
                case 37:
                    previousRecord(<?php echo $leafId; ?>, '<?php echo $collectionType->getControllerPath(); ?>', '<?php echo $collectionType->getViewPath(); ?>', '<?php echo $securityToken; ?>', <?php echo $leafAccess['leafAccessUpdateValue']; ?>, <?php echo $leafAccess['leafAccessDeleteValue']; ?>);

                    return false;
                    break;
                case 39:
                    nextRecord(<?php echo $leafId; ?>, '<?php echo $collectionType->getControllerPath(); ?>', '<?php echo $collectionType->getViewPath(); ?>', '<?php echo $securityToken; ?>', <?php echo $leafAccess['leafAccessUpdateValue']; ?>, <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
        ...
        $(document).ready(function() {
            ...
            validateMeAlphaNumeric('collectionTypeCode');
    <?php if ($_POST['method'] == "new") { ?>
                $('#resetRecordButton').removeClass().addClass('btn btn-default');
        <?php if ($leafAccess['leafAccessCreateValue'] == 1) { ?>
                    $('#newRecordButton1').removeClass().addClass('btn btn-success').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $collectionType->getControllerPath(); ?>','<?php echo $collectionType->getViewPath(); ?>','<?php echo $securityToken; ?>',1)");
                    $('#newRecordButton2').removeClass().addClass('btn dropdown-toggle btn-success');
                    $('#newRecordButton3').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $collectionType->getControllerPath(); ?>','<?php echo $collectionType->getViewPath(); ?>','<?php echo $securityToken; ?>',1)");
                    $('#newRecordButton4').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $collectionType->getControllerPath(); ?>','<?php echo $collectionType->getViewPath(); ?>','<?php echo $securityToken; ?>',2)");
                    $('#newRecordButton5').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $collectionType->getControllerPath(); ?>','<?php echo $collectionType->getViewPath(); ?>','<?php echo $securityToken; ?>',3)");
                    $('#newRecordButton6').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $collectionType->getControllerPath(); ?>','<?php echo $collectionType->getViewPath(); ?>','<?php echo $securityToken; ?>',4)");
                    $('#newRecordButton7').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $collectionType->getControllerPath(); ?>','<?php echo $collectionType->getViewPath(); ?>','<?php echo $securityToken; ?>',5)");
        <?php } else { ?>
        ...
        if ($_POST['collectionTypeId']) {
            ...
            <?php if ($leafAccess['leafAccessUpdateValue'] == 1) { ?>
                        $('#updateRecordButton1').removeClass().addClass('btn btn-info').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $collectionType->getControllerPath(); ?>','<?php echo $collectionType->getViewPath(); ?>','<?php echo $securityToken; ?>',1,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                        $('#updateRecordButton2').removeClass().addClass('btn dropdown-toggle btn-info');
                        $('#updateRecordButton3').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $collectionType->getControllerPath(); ?>','<?php echo $collectionType->getViewPath(); ?>','<?php echo $securityToken; ?>',1,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                        $('#updateRecordButton4').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $collectionType->getControllerPath(); ?>','<?php echo $collectionType->getViewPath(); ?>','<?php echo $securityToken; ?>',2,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                        $('#updateRecordButton5').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $collectionType->getControllerPath(); ?>','<?php echo $collectionType->getViewPath(); ?>','<?php echo $securityToken; ?>',3,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
            <?php } else { ?>
                        $('#updateRecordButton1').removeClass().addClass(' btn btn-info disabled').attr('onClick', '');
                        $('#updateRecordButton2').removeClass().addClass('btn dropdown-toggle btn-info disabled');
                        $('#updateRecordButton3').attr('onClick', '');
            ...
            <?php if ($leafAccess['leafAccessDeleteValue'] == 1) { ?>
                        $('#deleteRecordButton').removeClass().addClass('btn btn-danger').attr('onClick', "deleteRecord(<?php echo $leafId; ?>,'<?php echo $collectionType->getControllerPath(); ?>','<?php echo $collectionType->getViewPath(); ?>','<?php echo $securityToken; ?>',<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
            <?php } else { ?>
                        $('#deleteRecordButton').removeClass().addClass('btn btn-danger disabled').attr('onClick', '');
            <?php } ?>
            <?php
...
<?php } ?>
<script type="text/javascript" src="./v3/financial/cashbook/javascript/collectionType.js"></script>
<hr>
