...
require_once($newFakeDocumentRoot . "v3/financial/cashbook/controller/paymentTypeController.php");
...
$translator->setCurrentTable('paymentType');
...
if (isset($_POST)) {
    if (isset($_POST['method'])) {
        $paymentType = new \Core\Financial\Cashbook\PaymentType\Controller\PaymentTypeClass();
        define('LIMIT', 10);
        ...
        if (isset($_POST ['query'])) {
            $paymentType->setFieldQuery($_POST ['query']);
        }
        if (isset($_POST ['filter'])) {
            $paymentType->setGridQuery($_POST ['filter']);
        }
        if (isset($_POST ['character'])) {
            $paymentType->setCharacterQuery($_POST['character']);
        }
        if (isset($_POST ['dateRangeStart'])) {
            $paymentType->setDateRangeStartQuery($_POST['dateRangeStart']);
            //explode the data to get day,month,year
            $start = explode('-', $_POST ['dateRangeStart']);
            $paymentType->setStartDay($start[2]);
            $paymentType->setStartMonth($start[1]);
            $paymentType->setStartYear($start[0]);
        }
        if (isset($_POST ['dateRangeEnd']) && (strlen($_POST['dateRangeEnd']) > 0)) {
            $paymentType->setDateRangeEndQuery($_POST['dateRangeEnd']);
            //explode the data to get day,month,year
            $start = explode('-', $_POST ['dateRangeEnd']);
            $paymentType->setEndDay($start[2]);
            $paymentType->setEndMonth($start[1]);
            $paymentType->setEndYear($start[0]);
        }
        if (isset($_POST ['dateRangeType'])) {
            $paymentType->setDateRangeTypeQuery($_POST['dateRangeType']);
        }
        if (isset($_POST ['dateRangeExtraType'])) {
            $paymentType->setDateRangeExtraTypeQuery($_POST['dateRangeExtraType']);
        }
        $paymentType->setServiceOutput('html');
        $paymentType->setLeafId($leafId);
        $paymentType->execute();
        if ($_POST['method'] == 'read') {
            $paymentType->setStart($offset);
            $paymentType->setLimit($limit); // normal system don't like paging..
            $paymentType->setPageOutput('html');
            $paymentTypeArray = $paymentType->read();
            if (isset($paymentTypeArray [0]['firstRecord'])) {
                $firstRecord = $paymentTypeArray [0]['firstRecord'];
            }
            if (isset($paymentTypeArray [0]['nextRecord'])) {
                $nextRecord = $paymentTypeArray [0]['nextRecord'];
            }
            if (isset($paymentTypeArray [0]['previousRecord'])) {
                $previousRecord = $paymentTypeArray [0]['previousRecord'];
            }
            if (isset($paymentTypeArray [0]['lastRecord'])) {
                $lastRecord = $paymentTypeArray [0]['lastRecord'];
                $endRecord = $paymentTypeArray [0]['lastRecord'];
            }
            $navigation = new \Core\Paging\HtmlPaging();
            $navigation->setLeafId($leafId);
            $navigation->setViewPath($paymentType->getViewPath());
            ...
            if (isset($paymentTypeArray [0]['total'])) {
                $total = $paymentTypeArray [0]['total'];
            } else {
                $total = 0;
            }
                        ...
                        <div class="btn-group">
                            ...
                            <ul class="dropdown-menu" style="text-align:left">
                                <li>
                                    ...
                                    echo $paymentType->getControllerPath();
                        ...
                        <div class="modal-dialog">
                            <div class="modal-content">
                                <div class="modal-header">
                                    ...
                                    <h4 class="modal-title"><strong><?php echo $t['deleteRecordMessageLabel']; ?></strong></h4>
                                </div>
                                <div class="modal-body">
                                    <form class="form-horizontal">
                                        <input type="hidden" name="paymentTypeIdPreview" id="paymentTypeIdPreview">

                                        <div class="form-group" id="paymentTypeCodeDiv">
                                            <label for="paymentTypeCodePreview"
                                                   class="control-label col-xs-4 col-sm-4 col-md-4"><?php echo $leafTranslation['paymentTypeCodeLabel']; ?></label>

                                            <div class="col-xs-8 col-sm-8 col-md-8">
                                                <input type="text" class="form-control" name="paymentTypeCodePreview"
                                                       id="paymentTypeCodePreview">
                                            </div>
                                        </div>
                                        <div class="form-group" id="paymentTypeDescriptionDiv">
                                            <label for="paymentTypeDescriptionPreview"
                                                   class="control-label col-xs-4 col-sm-4 col-md-4"><?php echo $leafTranslation['paymentTypeDescriptionLabel']; ?></label>

                                            <div class="col-xs-8 col-sm-8 col-md-8">
                                                <input type="text" class="form-control"
                                                       name="paymentTypeDescriptionPreview" id="paymentTypeDescriptionPreview">
                                            </div>
                                        </div>
                                    </form>
                                </div>
                        ...
                        <div class="col-xs-12 col-sm-12 col-md-12">
                            <div class="panel panel-default">
                                <table class="table table table-bordered table-striped table-condensed table-hover smart-form has-tickbox" id="tableData">
                                    ...
                                    <th width="100px">
                                    <div align="center"><?php echo ucwords($leafTranslation['paymentTypeCodeLabel']); ?></div>
                                    </th>
                                    <th><?php echo ucwords($leafTranslation['paymentTypeDescriptionLabel']); ?></th>
                                    <th width="100px">
                                            ...
                                            if (is_array($paymentTypeArray)) {
                                                ...
                                                if ($totalRecord > 0) {
                                                    ...
                                                    for ($i = 0; $i < $totalRecord; $i++) {
                                                        ...
                                                        ?>
                                                        <tr <?php
                                                        if ($paymentTypeArray[$i]['isDelete'] == 1) {
                                                            echo "class=\"danger\"";
                                                        } else {
                                                            if ($paymentTypeArray[$i]['isDraft'] == 1) {
                                                                echo "class=\"warning\"";
                                                            }
                                                        }
                                                        ?>>
                                                            ...
                                                            <td align="center">
                                                                <?php if ($paymentTypeArray [$i]['paymentTypeCode'] != 'UNBL') { ?>
                                                                    <div class="btn-group" align="center">
                                                                        <button type="button"  class="btn btn-info btn-sm" title="Edit"
                                                                                ...
                                                                                echo intval(
                                                                                        $paymentTypeArray [$i]['paymentTypeId']
                                                                                );
                                                                        ...
                                                                        ?>', '<?php
                                                                        echo rawurlencode(
                                                                                $paymentTypeArray [$i]['paymentTypeCode']
                                                                        );
                                                                        ?>', '<?php
                                                                        echo rawurlencode(
                                                                                $paymentTypeArray [$i]['paymentTypeDescription']
                                                                        );
                                                                        ?>');">
                                                            ...
                                                            <td>
                                                                <div align="center">
                                                                    <?php
                                                                    if (isset($paymentTypeArray[$i]['paymentTypeCode'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos(
                                                                                                strtolower($paymentTypeArray[$i]['paymentTypeCode']), strtolower($_POST['query'])
                                                                                        ) !== false
                                                                                ) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $paymentTypeArray[$i]['paymentTypeCode']
                                                                                    );
                                                                                } else {
                                                                                    echo $paymentTypeArray[$i]['paymentTypeCode'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                    if (strpos(
                                                                                                    strtolower($paymentTypeArray[$i]['paymentTypeCode']), strtolower($_POST['character'])
                                                                                            ) !== false
                                                                                    ) {
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $paymentTypeArray[$i]['paymentTypeCode']
                                                                                        );
                                                                                    } else {
                                                                                        echo $paymentTypeArray[$i]['paymentTypeCode'];
                                                                                    }
                                                            ...
                                                            <td>
                                                                <div class="pull-left">
                                                                    <?php
                                                                    if (isset($paymentTypeArray[$i]['paymentTypeDescription'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos(
                                                                                                strtolower($paymentTypeArray[$i]['paymentTypeDescription']), strtolower($_POST['query'])
                                                                                        ) !== false
                                                                                ) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $paymentTypeArray[$i]['paymentTypeDescription']
                                                                                    );
                                                                                } else {
                                                                                    echo $paymentTypeArray[$i]['paymentTypeDescription'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                    if (strpos(
                                                                                                    strtolower($paymentTypeArray[$i]['paymentTypeDescription']), strtolower($_POST['character'])
                                                                                            ) !== false
                                                                                    ) {
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $paymentTypeArray[$i]['paymentTypeDescription']
                                                                                        );
                                                                                    } else {
                                                                                        echo $paymentTypeArray[$i]['paymentTypeDescription'];
                                                                                    }
                                                            ...
                                                            <td>
                                                                <div align="center">
                                                                    <?php
                                                                    if (isset($paymentTypeArray[$i]['executeBy'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos($paymentTypeArray[$i]['staffName'], $_POST['query']) !== false) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $paymentTypeArray[$i]['staffName']
                                                                                    );
                                                                                } else {
                                                                                    echo $paymentTypeArray[$i]['staffName'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                    if (strpos(
                                                                                                    $paymentTypeArray[$i]['staffName'], $_POST['character']
                                                                                            ) !== false
                                                                                    ) {
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $paymentTypeArray[$i]['staffName']
                                                                                        );
                                                                                    } else {
                                                                                        echo $paymentTypeArray[$i]['staffName'];
                                                                                    }
                                                            ...
                                                            <?php
                                                            if (isset($paymentTypeArray[$i]['executeTime'])) {
                                                                $valueArray = $paymentTypeArray[$i]['executeTime'];
                                                            ...
                                                            <?php } ?>
                                                            <?php
                                                            if ($paymentTypeArray[$i]['isDelete']) {
                                                                $checked = "checked";
                                                            } else {
                                                                $checked = null;
                                                            }
                                                            ...
                                                            <td>
                                                                <?php if ($paymentTypeArray [$i]['paymentTypeCode'] != 'UNBL') { ?>
                                                                    <input style="display:none;" type="checkbox" name="paymentTypeId[]"
                                                                           value="<?php echo $paymentTypeArray[$i]['paymentTypeId']; ?>">
                                                                    <input <?php echo $checked; ?> type="checkbox" name="isDelete[]"
                                                                                                   value="<?php echo $paymentTypeArray[$i]['isDelete']; ?>">
                                                                                               <?php } ?>
                                                            </td>
                                                        </tr>
                                                        <?php
                        ...
                        <div class="col-xs-3 col-sm-3 col-md-3 pagination">
                            <div class="pull-right">
                                <button class="delete btn btn-warning" type="button" 
                                        ...
                                        echo $paymentType->getControllerPath();
                                        ?>', '<?php echo $paymentType->getViewPath(); ?>', '<?php echo $securityToken; ?>');">
...
if ((isset($_POST['method']) == 'new' || isset($_POST['method']) == 'read') && $_POST['type'] == 'form') {
            ...
            <div class="row">
                <div class="col-xs-12 col-sm-12 col-md-12">
                    <div class="panel panel-info">
                        ...
                        <div class="panel-body">
                            <input type="hidden" name="paymentTypeId" id="paymentTypeId" value="<?php
                            if (isset($_POST['paymentTypeId'])) {
                                echo $_POST['paymentTypeId'];
                            }
                            ?>">

                            <div class="form-group" id="paymentTypeCodeForm">
                                <label for="paymentTypeCode" class="control-label col-xs-2 col-sm-2 col-md-2"><strong><?php
                                        echo ucfirst(
                                                $leafTranslation['paymentTypeCodeLabel']
                                        );
                                        ?></strong></label>

                                <div class="control-label col-xs-4 col-sm-4 col-md-4">
                                    <input class="form-control" type="text" name="paymentTypeCode" id="paymentTypeCode"
                                           value="<?php
                                           if (isset($paymentTypeArray) && is_array($paymentTypeArray)) {
                                               if (isset($paymentTypeArray[0]['paymentTypeCode'])) {
                                                   echo htmlentities($paymentTypeArray[0]['paymentTypeCode']);
                                               }
                                           }
                                           ?>" maxlength="16"> <span class="help-block" id="paymentTypeCodeHelpMe"></span>
                                </div>
                            </div>
                            <div class="form-group" id="paymentTypeDescriptionForm">
                                <div class="col-xs-12 col-sm-12 col-md-12">
                                    <textarea class="form-control" rows="5" name="paymentTypeDescription"
                                              id="paymentTypeDescription"><?php
                                                  if (isset($paymentTypeArray[0]['paymentTypeDescription'])) {
                                                      echo htmlentities($paymentTypeArray[0]['paymentTypeDescription']);
                                                  }
                                                  ?></textarea> <span class="help-block" id="paymentTypeDescriptionHelpMe"></span>
                                </div>
                            </div>
                        </div>
    ...
    <script type="text/javascript">
        $(document).keypress(function(e) {
            switch (e.keyCode) {
                case 37:
                    previousRecord(<?php echo $leafId; ?>, '<?php echo $paymentType->getControllerPath(); ?>', '<?php echo $paymentType->getViewPath(); ?>', '<?php echo $securityToken; ?>', <?php echo $leafAccess['leafAccessUpdateValue']; ?>, <?php echo $leafAccess['leafAccessDeleteValue']; ?>);

                    return false;
                    break;
                case 39:
                    nextRecord(<?php echo $leafId; ?>, '<?php echo $paymentType->getControllerPath(); ?>', '<?php echo $paymentType->getViewPath(); ?>', '<?php echo $securityToken; ?>', <?php echo $leafAccess['leafAccessUpdateValue']; ?>, <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
        ...
        $(document).ready(function() {
            ...
            validateMeAlphaNumeric('paymentTypeCode');
    <?php if ($_POST['method'] == "new") { ?>
                $('#resetRecordButton').removeClass().addClass('btn btn-default');
        <?php if ($leafAccess['leafAccessCreateValue'] == 1) { ?>
                    $('#newRecordButton1').removeClass().addClass('btn btn-success').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $paymentType->getControllerPath(); ?>','<?php echo $paymentType->getViewPath(); ?>','<?php echo $securityToken; ?>',1)");
                    $('#newRecordButton2').removeClass().addClass('btn dropdown-toggle btn-success');
                    $('#newRecordButton3').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $paymentType->getControllerPath(); ?>','<?php echo $paymentType->getViewPath(); ?>','<?php echo $securityToken; ?>',1)");
                    $('#newRecordButton4').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $paymentType->getControllerPath(); ?>','<?php echo $paymentType->getViewPath(); ?>','<?php echo $securityToken; ?>',2)");
                    $('#newRecordButton5').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $paymentType->getControllerPath(); ?>','<?php echo $paymentType->getViewPath(); ?>','<?php echo $securityToken; ?>',3)");
                    $('#newRecordButton6').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $paymentType->getControllerPath(); ?>','<?php echo $paymentType->getViewPath(); ?>','<?php echo $securityToken; ?>',4)");
                    $('#newRecordButton7').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $paymentType->getControllerPath(); ?>','<?php echo $paymentType->getViewPath(); ?>','<?php echo $securityToken; ?>',5)");
        <?php } else { ?>
        ...
        if ($_POST['paymentTypeId']) {
            ...
            <?php if ($leafAccess['leafAccessUpdateValue'] == 1) { ?>
                        $('#updateRecordButton1').removeClass().addClass('btn btn-info').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $paymentType->getControllerPath(); ?>','<?php echo $paymentType->getViewPath(); ?>','<?php echo $securityToken; ?>',1,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                        $('#updateRecordButton2').removeClass().addClass('btn dropdown-toggle btn-info');
                        $('#updateRecordButton3').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $paymentType->getControllerPath(); ?>','<?php echo $paymentType->getViewPath(); ?>','<?php echo $securityToken; ?>',1,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                        $('#updateRecordButton4').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $paymentType->getControllerPath(); ?>','<?php echo $paymentType->getViewPath(); ?>','<?php echo $securityToken; ?>',2,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                        $('#updateRecordButton5').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $paymentType->getControllerPath(); ?>','<?php echo $paymentType->getViewPath(); ?>','<?php echo $securityToken; ?>',3,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
            <?php } else { ?>
                        $('#updateRecordButton1').removeClass().addClass(' btn btn-info disabled').attr('onClick', '');
                        $('#updateRecordButton2').removeClass().addClass('btn dropdown-toggle btn-info disabled');
                        $('#updateRecordButton3').attr('onClick', '');
            ...
            <?php if ($leafAccess['leafAccessDeleteValue'] == 1) { ?>
                        $('#deleteRecordButton').removeClass().addClass('btn btn-danger').attr('onClick', "deleteRecord(<?php echo $leafId; ?>,'<?php echo $paymentType->getControllerPath(); ?>','<?php echo $paymentType->getViewPath(); ?>','<?php echo $securityToken; ?>',<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
            <?php } else { ?>
                        $('#deleteRecordButton').removeClass().addClass('btn btn-danger disabled').attr('onClick', '');
            <?php } ?>
            <?php
...
<?php } ?>
<script type="text/javascript" src="./v3/financial/cashbook/javascript/paymentType.js"></script>
<hr>
