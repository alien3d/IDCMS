...
function checkDuplicate(leafId, page, securityToken) {
    var $productDimensionCode = $("#productDimensionCode");
    if ($productDimensionCode.val().length === 0) {
        alert(t['oddTextLabel']);
        return false;
    }
    $.ajax({type: 'GET', url: page, data: {productDimensionCode: $productDimensionCode.val(), method: 'duplicate', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
            var smileyRoll = './images/icons/smiley-roll.png';
            var $infoPanel = $('#infoPanel');
            $infoPanel.html('').empty().html("<span class='label label-warning'><img src='" + smileyRoll + "'>&nbsp;" + decodeURIComponent(t['loadingTextLabel']) + "....</span>");
            if ($infoPanel.is(':hidden')) {
            ...
            if (success === true) {
                if (total !== 0) {
                    $("#productDimensionCode").val('').focus();
                    $("#productDimensionCodeForm").removeClass().addClass("form-group has-error");
                ...
                $("#productDimensionForm").removeClass().addClass("form-group has-error");
            }
            if ($infoPanel.is(':hidden')) {
                $infoPanel.show();
            }
...
function showFormUpdate(leafId, url, urlList, securityToken, productDimensionId, updateAccess, deleteAccess) {
    sleep(500);
    $('a[rel=tooltip]').tooltip('hide');
    $.ajax({type: 'POST', url: urlList, data: {method: 'read', type: 'form', productDimensionId: productDimensionId, securityToken: securityToken, leafId: leafId}, beforeSend: function() {
            var smileyRoll = './images/icons/smiley-roll.png';
            var $infoPanel = $('#infoPanel');
            $infoPanel.html('').empty().html("<span class='label label-warning'><img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
            if ($infoPanel.is(':hidden')) {
...
function showModalDelete(productDimensionId, unitOfMeasurementId, productId, productDimensionPackSize, productDimensionHeight, productDimensionWeight, productDimensionDepth, productDimensionWidth, productDimensionCubicTotal) {
    $('#productDimensionIdPreview').val('').val(decodeURIComponent(productDimensionId));
    ...
    $('#productDimensionPackSizePreview').val('').val(decodeURIComponent(productDimensionPackSize));
    $('#productDimensionHeightPreview').val('').val(decodeURIComponent(productDimensionHeight));
    $('#productDimensionWeightPreview').val('').val(decodeURIComponent(productDimensionWeight));
    $('#productDimensionDepthPreview').val('').val(decodeURIComponent(productDimensionDepth));
    $('#productDimensionWidthPreview').val('').val(decodeURIComponent(productDimensionWidth));
    $('#productDimensionCubicTotalPreview').val('').val(decodeURIComponent(productDimensionCubicTotal));
    showMeModal('deletePreview', 1);
}
function deleteGridRecord(leafId, url, urlList, securityToken) {
    $.ajax({type: 'POST', url: url, data: {method: 'delete', output: 'json', productDimensionId: $('#productDimensionIdPreview').val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
            var smileyRoll = './images/icons/smiley-roll.png';
            var $infoPanel = $('#infoPanel');
            $infoPanel.html('').empty().html("<span class='label label-warning'><img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
            if ($infoPanel.is(':hidden')) {
...
function deleteGridRecordCheckbox(leafId, url, urlList, securityToken) {
    var stringText = '';
    var counter = 0;
    $('input:checkbox[name="productDimensionId[]"]').each(function() {
        stringText = stringText + "&productDimensionId[]=" + $(this).val();
    });
...
function newRecord(leafId, url, urlList, securityToken, type, createAccess, updateAccess, deleteAccess) {
    ...
    var $productDimensionPackSize = $('#productDimensionPackSize');
    var $productDimensionHeight = $('#productDimensionHeight');
    var $productDimensionWeight = $('#productDimensionWeight');
    var $productDimensionDepth = $('#productDimensionDepth');
    var $productDimensionWidth = $('#productDimensionWidth');
    var $productDimensionCubicTotal = $('#productDimensionCubicTotal');
    if (css.search('disabled') > 0) {
        return false;
    } else {
        if (type === 1) {
            ...
            if ($productId.val().length === 0) {
                $('#productIdHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['productIdLabel'] + " </span>");
                $('#productIdForm').removeClass().addClass('form-group has-error');
                $productId.focus();
                return false;
            }
            if ($productDimensionPackSize.val().length === 0) {
                $('#productDimensionPackSizeHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['productDimensionPackSizeLabel'] + " </span>");
                $('#productDimensionPackSizeForm').removeClass().addClass('form-group has-error');
                $productDimensionPackSize.focus();
                return false;
            }
            if ($productDimensionHeight.val().length === 0) {
                $('#productDimensionHeightHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['productDimensionHeightLabel'] + " </span>");
                $('#productDimensionHeightForm').removeClass().addClass('form-group has-error');
                $productDimensionHeight.focus();
                return false;
            }
            if ($productDimensionWeight.val().length === 0) {
                $('#productDimensionWeightHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['productDimensionWeightLabel'] + " </span>");
                $('#productDimensionWeightForm').removeClass().addClass('form-group has-error');
                $productDimensionWeight.focus();
                return false;
            }
            if ($productDimensionDepth.val().length === 0) {
                $('#productDimensionDepthHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['productDimensionDepthLabel'] + " </span>");
                $('#productDimensionDepthForm').removeClass().addClass('form-group has-error');
                $productDimensionDepth.focus();
                return false;
            }
            if ($productDimensionWidth.val().length === 0) {
                $('#productDimensionWidthHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['productDimensionWidthLabel'] + " </span>");
                $('#productDimensionWidthForm').removeClass().addClass('form-group has-error');
                $productDimensionWidth.focus();
                return false;
            }
            if ($productDimensionCubicTotal.val().length === 0) {
                $('#productDimensionCubicTotalHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['productDimensionCubicTotalLabel'] + " </span>");
                $('#productDimensionCubicTotalForm').removeClass().addClass('form-group has-error');
                $productDimensionCubicTotal.focus();
                return false;
            }
            $.ajax({type: 'POST', url: url, data: {method: 'create', output: 'json', unitOfMeasurementId: $unitOfMeasurementId.val(), productId: $productId.val(), productDimensionPackSize: $productDimensionPackSize.val(), productDimensionHeight: $productDimensionHeight.val(), productDimensionWeight: $productDimensionWeight.val(), productDimensionDepth: $productDimensionDepth.val(), productDimensionWidth: $productDimensionWidth.val(), productDimensionCubicTotal: $productDimensionCubicTotal.val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    var smileyRoll = './images/icons/smiley-roll.png';
                    var $infoPanel = $('#infoPanel');
                    $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                    if ($infoPanel.is(':hidden')) {
                    ...
                    if (success === true) {
                        ...
                        $productId.val('');
                        $('#productIdHelpMe').html('').empty();
                        $productDimensionPackSize.val('');
                        $('#productDimensionPackSizeHelpMe').html('').empty();
                        $productDimensionHeight.val('');
                        $('#productDimensionHeightHelpMe').html('').empty();
                        $productDimensionWeight.val('');
                        $('#productDimensionWeightHelpMe').html('').empty();
                        $productDimensionDepth.val('');
                        $('#productDimensionDepthHelpMe').html('').empty();
                        $productDimensionWidth.val('');
                        $('#productDimensionWidthHelpMe').html('').empty();
                        $productDimensionCubicTotal.val('');
                        $('#productDimensionCubicTotalHelpMe').html('').empty();
                    } else if (success === false) {
                        $infoPanel.html('').empty().html("<span class='label label-important'>&nbsp;" + message + "</span>");
                        if ($infoPanel.is(':hidden')) {
                            $infoPanel.show();
            ...
            $.ajax({type: 'POST', url: url, data: {method: 'create', output: 'json', unitOfMeasurementId: $unitOfMeasurementId.val(), productId: $productId.val(), productDimensionPackSize: $productDimensionPackSize.val(), productDimensionHeight: $productDimensionHeight.val(), productDimensionWeight: $productDimensionWeight.val(), productDimensionDepth: $productDimensionDepth.val(), productDimensionWidth: $productDimensionWidth.val(), productDimensionCubicTotal: $productDimensionCubicTotal.val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    ...
                    if (success === true) {
                        ...
                        $('#productDimensionId').val(data.productDimensionId);
                        $('#newRecordButton1').removeClass().addClass('btn btn-success disabled');
                        $('#newRecordButton2').removeClass().addClass('btn dropdown-toggle btn-success disabled');
                        $('#newRecordButton3').attr('onClick', '');
                        $('#newRecordButton4').attr('onClick', '');
...
function updateRecord(leafId, url, urlList, securityToken, type, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        if (type === 1) {
            ...
            $infoPanel.html('').empty();
            $.ajax({type: 'POST', url: url, data: {method: 'save', output: 'json', productDimensionId: $productDimensionId.val(), unitOfMeasurementId: $unitOfMeasurementId.val(), productId: $productId.val(), productDimensionPackSize: $productDimensionPackSize.val(), productDimensionHeight: $productDimensionHeight.val(), productDimensionWeight: $productDimensionWeight.val(), productDimensionDepth: $productDimensionDepth.val(), productDimensionWidth: $productDimensionWidth.val(), productDimensionCubicTotal: $productDimensionCubicTotal.val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    var smileyRoll = './images/icons/smiley-roll.png';
                    var $infoPanel = $('#infoPanel');
                    $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                    if ($infoPanel.is(':hidden')) {
...
function deleteRecord(leafId, url, urlList, securityToken, deleteAccess) {
    var $infoPanel = $('#infoPanel');
    var $productDimensionId = $('#productDimensionId');
    ...
    if (css.search('disabled') > 0) {
        ...
        if (deleteAccess === 1) {
            if (confirm(decodeURIComponent(t['deleteRecordMessageLabel']))) {
                var value = $productDimensionId.val();
                if (!value) {
                    ...
                    $.ajax({type: 'POST', url: url, data: {method: 'delete', output: 'json', productDimensionId: $productDimensionId.val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                            var smileyRoll = './images/icons/smiley-roll.png';
                            var $infoPanel = $('#infoPanel');
                            $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                            if ($infoPanel.is(':hidden')) {
...
function resetRecord(leafId, url, urlList, securityToken, createAccess, updateAccess, deleteAccess) {
    ...
    $("#productDimensionIdHelpMe").empty().html('');
    
    $("#unitOfMeasurementId").val('');
    $("#unitOfMeasurementIdHelpMe").empty().html('');
    $('#unitOfMeasurementId').trigger("chosen:updated");
    $("#productId").val('');
    $("#productIdHelpMe").empty().html('');
    $("#productDimensionPackSize").val('');
    $("#productDimensionPackSizeHelpMe").empty().html('');
    $("#productDimensionHeight").val('');
    $("#productDimensionHeightHelpMe").empty().html('');
    $("#productDimensionWeight").val('');
    $("#productDimensionWeightHelpMe").empty().html('');
    $("#productDimensionDepth").val('');
    $("#productDimensionDepthHelpMe").empty().html('');
    $("#productDimensionWidth").val('');
    $("#productDimensionWidthHelpMe").empty().html('');
    $("#productDimensionCubicTotal").val('');
    $("#productDimensionCubicTotalHelpMe").empty().html('');
}
function postRecord() {
...
function firstRecord(leafId, url, urlList, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        $.ajax({type: 'GET', url: url, data: {method: 'dataNavigationRequest', dataNavigation: 'firstRecord', output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                ...
                if (success === true) {
                    $.ajax({type: 'POST', url: url, data: {method: 'read', productDimensionId: firstRecord, output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                            var smileyRoll = './images/icons/smiley-roll.png';
                            $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                            if ($infoPanel.is(':hidden')) {
                                $infoPanel.show();
                            ...
                            if (success === true) {
                                $('#productDimensionId').val(data.data.productDimensionId);
                                $('#unitOfMeasurementId').val(data.data.unitOfMeasurementId).trigger("chosen:updated");
                                $('#productId').val(data.data.productId);
                                $('#productDimensionPackSize').val(data.data.productDimensionPackSize);
                                $('#productDimensionHeight').val(data.data.productDimensionHeight);
                                $('#productDimensionWeight').val(data.data.productDimensionWeight);
                                $('#productDimensionDepth').val(data.data.productDimensionDepth);
                                $('#productDimensionWidth').val(data.data.productDimensionWidth);
                                $('#productDimensionCubicTotal').val(data.data.productDimensionCubicTotal);
                                if (nextRecord > 0) {
                                    $('#previousRecordButton').removeClass().addClass('btn btn-default disabled').attr('onClick', '');
...
function endRecord(leafId, url, urlList, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        $.ajax({type: 'GET', url: url, data: {method: 'dataNavigationRequest', dataNavigation: 'lastRecord', output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                ...
                if (success === true) {
                    $.ajax({type: 'POST', url: url, data: {method: 'read', productDimensionId: lastRecord, output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                            var smileyRoll = './images/icons/smiley-roll.png';
                            $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                            if ($infoPanel.is(':hidden')) {
                                $infoPanel.show();
...
function previousRecord(leafId, url, urlList, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($previousRecordCounter.val()) > 0 && parseFloat($previousRecordCounter.val()) < parseFloat($('#lastRecordCounter').val())) {
            $.ajax({type: 'POST', url: url, data: {method: 'read', productDimensionId: $previousRecordCounter.val(), output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    var smileyRoll = './images/icons/smiley-roll.png';
                    var $infoPanel = $('#infoPanel');
                    $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                    if ($infoPanel.is(':hidden')) {
...
function nextRecord(leafId, url, urlList, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($nextRecordCounter.val()) <= parseFloat($('#lastRecordCounter').val())) {
            $.ajax({type: 'POST', url: url, data: {method: 'read', productDimensionId: $nextRecordCounter.val(), output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    var smileyRoll = './images/icons/smiley-roll.png';
                    var $infoPanel = $('#infoPanel');
                    $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                    if ($infoPanel.is(':hidden')) {
