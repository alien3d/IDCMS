...
function checkDuplicate(leafId, page, securityToken) {
    var $productResourcesBillOfMaterialCode = $("#productResourcesBillOfMaterialCode");
    if ($productResourcesBillOfMaterialCode.val().length === 0) {
        alert(t['oddTextLabel']);
        return false;
    }
    $.ajax({type: 'GET', url: page, data: {productResourcesBillOfMaterialCode: $productResourcesBillOfMaterialCode.val(), method: 'duplicate', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
            var smileyRoll = './images/icons/smiley-roll.png';
            var $infoPanel = $('#infoPanel');
            $infoPanel.html('').empty().html("<span class='label label-warning'><img src='" + smileyRoll + "'>&nbsp;" + decodeURIComponent(t['loadingTextLabel']) + "....</span>");
            if ($infoPanel.is(':hidden')) {
            ...
            if (success === true) {
                if (total !== 0) {
                    $("#productResourcesBillOfMaterialCode").val('').focus();
                    $("#productResourcesBillOfMaterialCodeForm").removeClass().addClass("form-group has-error");
                ...
                $("#productResourcesBillOfMaterialForm").removeClass().addClass("form-group has-error");
            }
            if ($infoPanel.is(':hidden')) {
                $infoPanel.show();
            }
...
function showFormUpdate(leafId, url, urlList, securityToken, productResourcesBillOfMaterialId, updateAccess, deleteAccess) {
    sleep(500);
    $('a[rel=tooltip]').tooltip('hide');
    $.ajax({type: 'POST', url: urlList, data: {method: 'read', type: 'form', productResourcesBillOfMaterialId: productResourcesBillOfMaterialId, securityToken: securityToken, leafId: leafId}, beforeSend: function() {
            var smileyRoll = './images/icons/smiley-roll.png';
            var $infoPanel = $('#infoPanel');
            $infoPanel.html('').empty().html("<span class='label label-warning'><img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
            if ($infoPanel.is(':hidden')) {
...
function showModalDelete(productResourcesBillOfMaterialId, productResourcesId, productResourcesTypeId, productId, productResourcesBillOfMaterialCost, productResourcesBillOfMaterialWaste) {
    $('#productResourcesBillOfMaterialIdPreview').val('').val(decodeURIComponent(productResourcesBillOfMaterialId));
    ...
    $('#productResourcesBillOfMaterialCostPreview').val('').val(decodeURIComponent(productResourcesBillOfMaterialCost));
    $('#productResourcesBillOfMaterialWastePreview').val('').val(decodeURIComponent(productResourcesBillOfMaterialWaste));
    showMeModal('deletePreview', 1);
}
function deleteGridRecord(leafId, url, urlList, securityToken) {
    $.ajax({type: 'POST', url: url, data: {method: 'delete', output: 'json', productResourcesBillOfMaterialId: $('#productResourcesBillOfMaterialIdPreview').val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
            var smileyRoll = './images/icons/smiley-roll.png';
            var $infoPanel = $('#infoPanel');
            $infoPanel.html('').empty().html("<span class='label label-warning'><img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
            if ($infoPanel.is(':hidden')) {
...
function deleteGridRecordCheckbox(leafId, url, urlList, securityToken) {
    var stringText = '';
    var counter = 0;
    $('input:checkbox[name="productResourcesBillOfMaterialId[]"]').each(function() {
        stringText = stringText + "&productResourcesBillOfMaterialId[]=" + $(this).val();
    });
...
function newRecord(leafId, url, urlList, securityToken, type, createAccess, updateAccess, deleteAccess) {
    ...
    var $productResourcesBillOfMaterialCost = $('#productResourcesBillOfMaterialCost');
    var $productResourcesBillOfMaterialWaste = $('#productResourcesBillOfMaterialWaste');
    if (css.search('disabled') > 0) {
        return false;
    } else {
        if (type === 1) {
            ...
            if ($productResourcesBillOfMaterialCost.val().length === 0) {
                $('#productResourcesBillOfMaterialCostHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['productResourcesBillOfMaterialCostLabel'] + " </span>");
                $('#productResourcesBillOfMaterialCostForm').removeClass().addClass('form-group has-error');
                $productResourcesBillOfMaterialCost.focus();
                return false;
            }
            if ($productResourcesBillOfMaterialWaste.val().length === 0) {
                $('#productResourcesBillOfMaterialWasteHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['productResourcesBillOfMaterialWasteLabel'] + " </span>");
                $('#productResourcesBillOfMaterialWasteForm').removeClass().addClass('form-group has-error');
                $productResourcesBillOfMaterialWaste.focus();
                return false;
            }
            $.ajax({type: 'POST', url: url, data: {method: 'create', output: 'json', productResourcesId: $productResourcesId.val(), productResourcesTypeId: $productResourcesTypeId.val(), productId: $productId.val(), productResourcesBillOfMaterialCost: $productResourcesBillOfMaterialCost.val(), productResourcesBillOfMaterialWaste: $productResourcesBillOfMaterialWaste.val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    var smileyRoll = './images/icons/smiley-roll.png';
                    var $infoPanel = $('#infoPanel');
                    $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                    if ($infoPanel.is(':hidden')) {
                    ...
                    if (success === true) {
                        ...
                        $productResourcesBillOfMaterialCost.val('');
                        $('#productResourcesBillOfMaterialCostHelpMe').html('').empty();
                        $productResourcesBillOfMaterialWaste.val('');
                        $('#productResourcesBillOfMaterialWasteHelpMe').html('').empty();
                    } else if (success === false) {
                        $infoPanel.html('').empty().html("<span class='label label-important'>&nbsp;" + message + "</span>");
                        if ($infoPanel.is(':hidden')) {
                            $infoPanel.show();
            ...
            $.ajax({type: 'POST', url: url, data: {method: 'create', output: 'json', productResourcesId: $productResourcesId.val(), productResourcesTypeId: $productResourcesTypeId.val(), productId: $productId.val(), productResourcesBillOfMaterialCost: $productResourcesBillOfMaterialCost.val(), productResourcesBillOfMaterialWaste: $productResourcesBillOfMaterialWaste.val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    ...
                    if (success === true) {
                        ...
                        $('#productResourcesBillOfMaterialId').val(data.productResourcesBillOfMaterialId);
                        $('#newRecordButton1').removeClass().addClass('btn btn-success disabled');
                        $('#newRecordButton2').removeClass().addClass('btn dropdown-toggle btn-success disabled');
                        $('#newRecordButton3').attr('onClick', '');
                        $('#newRecordButton4').attr('onClick', '');
...
function updateRecord(leafId, url, urlList, securityToken, type, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        if (type === 1) {
            ...
            $infoPanel.html('').empty();
            $.ajax({type: 'POST', url: url, data: {method: 'save', output: 'json', productResourcesBillOfMaterialId: $productResourcesBillOfMaterialId.val(), productResourcesId: $productResourcesId.val(), productResourcesTypeId: $productResourcesTypeId.val(), productId: $productId.val(), productResourcesBillOfMaterialCost: $productResourcesBillOfMaterialCost.val(), productResourcesBillOfMaterialWaste: $productResourcesBillOfMaterialWaste.val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    var smileyRoll = './images/icons/smiley-roll.png';
                    var $infoPanel = $('#infoPanel');
                    $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                    if ($infoPanel.is(':hidden')) {
...
function deleteRecord(leafId, url, urlList, securityToken, deleteAccess) {
    var $infoPanel = $('#infoPanel');
    var $productResourcesBillOfMaterialId = $('#productResourcesBillOfMaterialId');
    ...
    if (css.search('disabled') > 0) {
        ...
        if (deleteAccess === 1) {
            if (confirm(decodeURIComponent(t['deleteRecordMessageLabel']))) {
                var value = $productResourcesBillOfMaterialId.val();
                if (!value) {
                    ...
                    $.ajax({type: 'POST', url: url, data: {method: 'delete', output: 'json', productResourcesBillOfMaterialId: $productResourcesBillOfMaterialId.val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                            var smileyRoll = './images/icons/smiley-roll.png';
                            var $infoPanel = $('#infoPanel');
                            $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                            if ($infoPanel.is(':hidden')) {
...
function resetRecord(leafId, url, urlList, securityToken, createAccess, updateAccess, deleteAccess) {
    ...
    $("#productResourcesBillOfMaterialId").val('');
    $("#productResourcesBillOfMaterialIdHelpMe").empty().html('');
    ...
    $("#productResourcesBillOfMaterialCost").val('');
    $("#productResourcesBillOfMaterialCostHelpMe").empty().html('');
    $("#productResourcesBillOfMaterialWaste").val('');
    $("#productResourcesBillOfMaterialWasteHelpMe").empty().html('');
}
function postRecord() {
...
function firstRecord(leafId, url, urlList, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        $.ajax({type: 'GET', url: url, data: {method: 'dataNavigationRequest', dataNavigation: 'firstRecord', output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                ...
                if (success === true) {
                    $.ajax({type: 'POST', url: url, data: {method: 'read', productResourcesBillOfMaterialId: firstRecord, output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                            var smileyRoll = './images/icons/smiley-roll.png';
                            $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                            if ($infoPanel.is(':hidden')) {
                                $infoPanel.show();
                            ...
                            if (success === true) {
                                $('#productResourcesBillOfMaterialId').val(data.data.productResourcesBillOfMaterialId);
                                ...
                                $('#productResourcesBillOfMaterialCost').val(data.data.productResourcesBillOfMaterialCost);
                                $('#productResourcesBillOfMaterialWaste').val(data.data.productResourcesBillOfMaterialWaste);
                                if (nextRecord > 0) {
                                    $('#previousRecordButton').removeClass().addClass('btn btn-info disabled').attr('onClick', '');
...
function endRecord(leafId, url, urlList, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        $.ajax({type: 'GET', url: url, data: {method: 'dataNavigationRequest', dataNavigation: 'lastRecord', output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                ...
                if (success === true) {
                    $.ajax({type: 'POST', url: url, data: {method: 'read', productResourcesBillOfMaterialId: lastRecord, output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                            var smileyRoll = './images/icons/smiley-roll.png';
                            $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                            if ($infoPanel.is(':hidden')) {
                                $infoPanel.show();
...
function previousRecord(leafId, url, urlList, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($previousRecordCounter.val()) > 0 && parseFloat($previousRecordCounter.val()) < parseFloat($('#lastRecordCounter').val())) {
            $.ajax({type: 'POST', url: url, data: {method: 'read', productResourcesBillOfMaterialId: $previousRecordCounter.val(), output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    var smileyRoll = './images/icons/smiley-roll.png';
                    var $infoPanel = $('#infoPanel');
                    $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                    if ($infoPanel.is(':hidden')) {
...
function nextRecord(leafId, url, urlList, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($nextRecordCounter.val()) <= parseFloat($('#lastRecordCounter').val())) {
            $.ajax({type: 'POST', url: url, data: {method: 'read', productResourcesBillOfMaterialId: $nextRecordCounter.val(), output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    var smileyRoll = './images/icons/smiley-roll.png';
                    var $infoPanel = $('#infoPanel');
                    $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                    if ($infoPanel.is(':hidden')) {
