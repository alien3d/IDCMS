function getProduct(leafId, url, securityToken) {
    $.ajax({type: 'GET', url: url, data: {offset: 0, limit: 99999, method: 'read', type: 'filter', securityToken: securityToken, leafId: leafId, filter: 'product'}, beforeSend: function() {
            ...
            if (success === false) {
                ...
                $("#productId").html('').empty().html(data.data).trigger("chosen:updated");
...
function getState(leafId, url, securityToken) {
    $.ajax({type: 'GET', url: url, data: {offset: 0, limit: 99999, method: 'read', type: 'filter', securityToken: securityToken, leafId: leafId, filter: 'state'}, beforeSend: function() {
            var smileyRoll = './images/icons/smiley-roll.png';
            var $infoPanel = $('#infoPanel');
            $infoPanel.html('').empty().html("<span class='label label-warning'><img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
            if ($infoPanel.is(':hidden')) {
...
function checkDuplicate(leafId, page, securityToken) {
    var $productSellingPriceCode = $("#productSellingPriceCode");
    if ($productSellingPriceCode.val().length === 0) {
        alert(t['oddTextLabel']);
        return false;
    }
    $.ajax({type: 'GET', url: page, data: {productSellingPriceCode: $productSellingPriceCode.val(), method: 'duplicate', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
            var smileyRoll = './images/icons/smiley-roll.png';
            var $infoPanel = $('#infoPanel');
            $infoPanel.html('').empty().html("<span class='label label-warning'><img src='" + smileyRoll + "'>&nbsp;" + decodeURIComponent(t['loadingTextLabel']) + "....</span>");
            if ($infoPanel.is(':hidden')) {
            ...
            if (success === true) {
                if (total !== 0) {
                    $("#productSellingPriceCode").val('').focus();
                    $("#productSellingPriceCodeForm").removeClass().addClass("form-group has-error");
...
function showFormUpdate(leafId, url, urlList, securityToken, productSellingPriceId, updateAccess, deleteAccess) {
    sleep(500);
    $('a[rel=tooltip]').tooltip('hide');
    $.ajax({type: 'POST', url: urlList, data: {method: 'read', type: 'form', productSellingPriceId: productSellingPriceId, securityToken: securityToken, leafId: leafId}, beforeSend: function() {
            var smileyRoll = './images/icons/smiley-roll.png';
            var $infoPanel = $('#infoPanel');
            $infoPanel.html('').empty().html("<span class='label label-warning'><img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
            if ($infoPanel.is(':hidden')) {
...
function showModalDelete(productSellingPriceId, productId, countryId, stateId, unitOfMeasurementId, productSellingPriceQuantity, productSellingPricePrice, productSellingPriceStartDate, productSellingPriceEndDate) {
    $('#productSellingPriceIdPreview').val('').val(decodeURIComponent(productSellingPriceId));
    ...
    $('#productSellingPriceQuantityPreview').val('').val(decodeURIComponent(productSellingPriceQuantity));
    $('#productSellingPricePricePreview').val('').val(decodeURIComponent(productSellingPricePrice));
    $('#productSellingPriceStartDatePreview').val('').val(decodeURIComponent(productSellingPriceStartDate));
    $('#productSellingPriceEndDatePreview').val('').val(decodeURIComponent(productSellingPriceEndDate));
    showMeModal('deletePreview', 1);
}
function deleteGridRecord(leafId, url, urlList, securityToken) {
    $.ajax({type: 'POST', url: url, data: {method: 'delete', output: 'json', productSellingPriceId: $('#productSellingPriceIdPreview').val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
            var smileyRoll = './images/icons/smiley-roll.png';
            var $infoPanel = $('#infoPanel');
            $infoPanel.html('').empty().html("<span class='label label-warning'><img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
            if ($infoPanel.is(':hidden')) {
...
function deleteGridRecordCheckbox(leafId, url, urlList, securityToken) {
    var stringText = '';
    var counter = 0;
    $('input:checkbox[name="productSellingPriceId[]"]').each(function() {
        stringText = stringText + "&productSellingPriceId[]=" + $(this).val();
    });
...
function newRecord(leafId, url, urlList, securityToken, type, createAccess, updateAccess, deleteAccess) {
    ...
    var $productSellingPriceQuantity = $('#productSellingPriceQuantity');
    var $productSellingPricePrice = $('#productSellingPricePrice');
    var $productSellingPriceStartDate = $('#productSellingPriceStartDate');
    var $productSellingPriceEndDate = $('#productSellingPriceEndDate');
    if (css.search('disabled') > 0) {
        return false;
    } else {
        if (type === 1) {
            ...
            if ($productSellingPriceQuantity.val().length === 0) {
                $('#productSellingPriceQuantityHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['productSellingPriceQuantityLabel'] + " </span>");
                $('#productSellingPriceQuantityForm').removeClass().addClass('form-group has-error');
                $productSellingPriceQuantity.focus();
                return false;
            }
            if ($productSellingPricePrice.val().length === 0) {
                $('#productSellingPricePriceHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['productSellingPricePriceLabel'] + " </span>");
                $('#productSellingPricePriceForm').removeClass().addClass('form-group has-error');
                $productSellingPricePrice.focus();
                return false;
            }
            if ($productSellingPriceStartDate.val().length === 0) {
                $('#productSellingPriceStartDateHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['productSellingPriceStartDateLabel'] + " </span>");
                $('#productSellingPriceStartDateForm').removeClass().addClass('form-group has-error');
                $productSellingPriceStartDate.focus();
                return false;
            }
            if ($productSellingPriceEndDate.val().length === 0) {
                $('#productSellingPriceEndDateHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['productSellingPriceEndDateLabel'] + " </span>");
                $('#productSellingPriceEndDateForm').removeClass().addClass('form-group has-error');
                $productSellingPriceEndDate.focus();
                return false;
            }
            $.ajax({type: 'POST', url: url, data: {method: 'create', output: 'json', productId: $productId.val(), countryId: $countryId.val(), stateId: $stateId.val(), unitOfMeasurementId: $unitOfMeasurementId.val(), productSellingPriceQuantity: $productSellingPriceQuantity.val(), productSellingPricePrice: $productSellingPricePrice.val(), productSellingPriceStartDate: $productSellingPriceStartDate.val(), productSellingPriceEndDate: $productSellingPriceEndDate.val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    var smileyRoll = './images/icons/smiley-roll.png';
                    var $infoPanel = $('#infoPanel');
                    $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                    if ($infoPanel.is(':hidden')) {
                    ...
                    if (success === true) {
                        ...
                        $productSellingPriceQuantity.val('');
                        $('#productSellingPriceQuantityHelpMe').html('').empty();
                        $productSellingPricePrice.val('');
                        $('#productSellingPricePriceHelpMe').html('').empty();
                        $productSellingPriceStartDate.val('');
                        $('#productSellingPriceStartDateHelpMe').html('').empty();
                        $productSellingPriceEndDate.val('');
                        $('#productSellingPriceEndDateHelpMe').html('').empty();
                    } else if (success === false) {
                        $infoPanel.html('').empty().html("<span class='label label-important'>&nbsp;" + message + "</span>");
                        if ($infoPanel.is(':hidden')) {
                            $infoPanel.show();
            ...
            $.ajax({type: 'POST', url: url, data: {method: 'create', output: 'json', productId: $productId.val(), countryId: $countryId.val(), stateId: $stateId.val(), unitOfMeasurementId: $unitOfMeasurementId.val(), productSellingPriceQuantity: $productSellingPriceQuantity.val(), productSellingPricePrice: $productSellingPricePrice.val(), productSellingPriceStartDate: $productSellingPriceStartDate.val(), productSellingPriceEndDate: $productSellingPriceEndDate.val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    ...
                    if (success === true) {
                        ...
                        $('#productSellingPriceId').val(data.productSellingPriceId);
                        $('#newRecordButton1').removeClass().addClass('btn btn-success disabled');
                        $('#newRecordButton2').removeClass().addClass('btn dropdown-toggle btn-success disabled');
                        $('#newRecordButton3').attr('onClick', '');
                        $('#newRecordButton4').attr('onClick', '');
...
function updateRecord(leafId, url, urlList, securityToken, type, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        if (type === 1) {
            ...
            $infoPanel.html('').empty();
            $.ajax({type: 'POST', url: url, data: {method: 'save', output: 'json', productSellingPriceId: $productSellingPriceId.val(), productId: $productId.val(), countryId: $countryId.val(), stateId: $stateId.val(), unitOfMeasurementId: $unitOfMeasurementId.val(), productSellingPriceQuantity: $productSellingPriceQuantity.val(), productSellingPricePrice: $productSellingPricePrice.val(), productSellingPriceStartDate: $productSellingPriceStartDate.val(), productSellingPriceEndDate: $productSellingPriceEndDate.val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    var smileyRoll = './images/icons/smiley-roll.png';
                    var $infoPanel = $('#infoPanel');
                    $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                    if ($infoPanel.is(':hidden')) {
...
function deleteRecord(leafId, url, urlList, securityToken, deleteAccess) {
    var $infoPanel = $('#infoPanel');
    var $productSellingPriceId = $('#productSellingPriceId');
    ...
    if (css.search('disabled') > 0) {
        ...
        if (deleteAccess === 1) {
            if (confirm(decodeURIComponent(t['deleteRecordMessageLabel']))) {
                var value = $productSellingPriceId.val();
                if (!value) {
                    ...
                    $.ajax({type: 'POST', url: url, data: {method: 'delete', output: 'json', productSellingPriceId: $productSellingPriceId.val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                            var smileyRoll = './images/icons/smiley-roll.png';
                            var $infoPanel = $('#infoPanel');
                            $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                            if ($infoPanel.is(':hidden')) {
...
function resetRecord(leafId, url, urlList, securityToken, createAccess, updateAccess, deleteAccess) {
    ...
    $("#productSellingPriceId").val('');
    $("#productSellingPriceIdHelpMe").empty().html('');
    ...
    $('#unitOfMeasurementId').trigger("chosen:updated");
    $("#productSellingPriceQuantity").val('');
    $("#productSellingPriceQuantityHelpMe").empty().html('');
    $("#productSellingPricePrice").val('');
    $("#productSellingPricePriceHelpMe").empty().html('');
    $("#productSellingPriceStartDate").val('');
    $("#productSellingPriceStartDateHelpMe").empty().html('');
    $("#productSellingPriceEndDate").val('');
    $("#productSellingPriceEndDateHelpMe").empty().html('');
}
function postRecord() {
...
function firstRecord(leafId, url, urlList, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        $.ajax({type: 'GET', url: url, data: {method: 'dataNavigationRequest', dataNavigation: 'firstRecord', output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                ...
                if (success === true) {
                    $.ajax({type: 'POST', url: url, data: {method: 'read', productSellingPriceId: firstRecord, output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                            var smileyRoll = './images/icons/smiley-roll.png';
                            $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                            if ($infoPanel.is(':hidden')) {
                                $infoPanel.show();
                            ...
                            if (success === true) {
                                $('#productSellingPriceId').val(data.data.productSellingPriceId);
                                ...
                                $('#productSellingPriceQuantity').val(data.data.productSellingPriceQuantity);
                                $('#productSellingPricePrice').val(data.data.productSellingPricePrice);
                                x = data.data.productSellingPriceStartDate;
                                x = x.split("-");
                                output = x[2] + "-" + x[1] + "-" + x[0];
                                output = output.toString();
                                $('#productSellingPriceStartDate').val(output);
                                x = data.data.productSellingPriceEndDate;
                                x = x.split("-");
                                output = x[2] + "-" + x[1] + "-" + x[0];
                                output = output.toString();
                                $('#productSellingPriceEndDate').val(output);
                                if (nextRecord > 0) {
                                    $('#previousRecordButton').removeClass().addClass('btn btn-default disabled').attr('onClick', '');
...
function endRecord(leafId, url, urlList, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        $.ajax({type: 'GET', url: url, data: {method: 'dataNavigationRequest', dataNavigation: 'lastRecord', output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                ...
                if (success === true) {
                    $.ajax({type: 'POST', url: url, data: {method: 'read', productSellingPriceId: lastRecord, output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                            var smileyRoll = './images/icons/smiley-roll.png';
                            $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                            if ($infoPanel.is(':hidden')) {
                                $infoPanel.show();
...
function previousRecord(leafId, url, urlList, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($previousRecordCounter.val()) > 0 && parseFloat($previousRecordCounter.val()) < parseFloat($('#lastRecordCounter').val())) {
            $.ajax({type: 'POST', url: url, data: {method: 'read', productSellingPriceId: $previousRecordCounter.val(), output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    var smileyRoll = './images/icons/smiley-roll.png';
                    var $infoPanel = $('#infoPanel');
                    $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                    if ($infoPanel.is(':hidden')) {
...
function nextRecord(leafId, url, urlList, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($nextRecordCounter.val()) <= parseFloat($('#lastRecordCounter').val())) {
            $.ajax({type: 'POST', url: url, data: {method: 'read', productSellingPriceId: $nextRecordCounter.val(), output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    var smileyRoll = './images/icons/smiley-roll.png';
                    var $infoPanel = $('#infoPanel');
                    $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                    if ($infoPanel.is(':hidden')) {
