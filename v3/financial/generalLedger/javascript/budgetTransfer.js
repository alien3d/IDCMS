function getBudgetTransferFrom(leafId, url, securityToken) {
    $.ajax({type: 'GET', url: url, data: {offset: 0, limit: 99999, method: 'read', type: 'filter', securityToken: securityToken, leafId: leafId, filter: 'budgetTransferFrom'}, beforeSend: function() {
            ...
            if (success === false) {
                ...
                $("#budgetTransferFrom").html('').empty().html(data.data).trigger("chosen:updated");
...
function getBudgetTransferTo(leafId, url, securityToken) {
    $.ajax({type: 'GET', url: url, data: {offset: 0, limit: 99999, method: 'read', type: 'filter', securityToken: securityToken, leafId: leafId, filter: 'budgetTransferTo'}, beforeSend: function() {
            var smileyRoll = './images/icons/smiley-roll.png';
            var $infoPanel = $('#infoPanel');
            $infoPanel.html('').empty().html("<span class='label label-warning'><img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
            if ($infoPanel.is(':hidden')) {
            ...
            if (success === false) {
                ...
                $("#budgetTransferTo").html('').empty().html(data.data).trigger("chosen:updated");
                $infoPanel.html('').empty().html("<span class='label label-success'>&nbsp;<img src='" + smileyLol + "'>  " + decodeURIComponent(t['loadingCompleteTextLabel']) + "</span>").delay(5000).fadeOut();
            }
        }, error: function(xhr) {
            var smileyRollSweat = './images/icons/smiley-roll-sweat.png';
...
function checkDuplicate(leafId, page, securityToken) {
    var $budgetTransferCode = $("#budgetTransferCode");
    if ($budgetTransferCode.val().length === 0) {
        alert(t['oddTextLabel']);
        return false;
    }
    $.ajax({type: 'GET', url: page, data: {budgetTransferCode: $budgetTransferCode.val(), method: 'duplicate', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
            var smileyRoll = './images/icons/smiley-roll.png';
            var $infoPanel = $('#infoPanel');
            $infoPanel.html('').empty().html("<span class='label label-warning'><img src='" + smileyRoll + "'>&nbsp;" + decodeURIComponent(t['loadingTextLabel']) + "....</span>");
            if ($infoPanel.is(':hidden')) {
            ...
            if (success === true) {
                if (total !== 0) {
                    $("#budgetTransferCode").val('').focus();
                    $("#budgetTransferCodeForm").removeClass().addClass("form-group has-error");
                ...
                $("#budgetTransferForm").removeClass().addClass("form-group has-error");
            }
            if ($infoPanel.is(':hidden')) {
                $infoPanel.show();
            }
...
function showFormUpdate(leafId, url, urlList, securityToken, budgetTransferId, updateAccess, deleteAccess) {
    sleep(500);
    $('a[rel=tooltip]').tooltip('hide');
    $.ajax({type: 'POST', url: urlList, data: {method: 'read', type: 'form', budgetTransferId: budgetTransferId, securityToken: securityToken, leafId: leafId}, beforeSend: function() {
            var smileyRoll = './images/icons/smiley-roll.png';
            var $infoPanel = $('#infoPanel');
            $infoPanel.html('').empty().html("<span class='label label-warning'><img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
            if ($infoPanel.is(':hidden')) {
...
function showModalDelete(budgetTransferId, budgetTransferFrom, budgetTransferTo, financeYearId, financePeriodRangeId, documentNumber, budgetTransferDate, budgetTransferAmount, budgetTransferComment) {
    $('#budgetTransferIdPreview').val('').val(decodeURIComponent(budgetTransferId));
    $('#budgetTransferFromPreview').val('').val(decodeURIComponent(budgetTransferFrom));
    $('#budgetTransferToPreview').val('').val(decodeURIComponent(budgetTransferTo));
    ...
    $('#budgetTransferDatePreview').val('').val(decodeURIComponent(budgetTransferDate));
    $('#budgetTransferAmountPreview').val('').val(decodeURIComponent(budgetTransferAmount));
    $('#budgetTransferCommentPreview').val('').val(decodeURIComponent(budgetTransferComment));
    showMeModal('deletePreview', 1);
}
function deleteGridRecord(leafId, url, urlList, securityToken) {
    $.ajax({type: 'POST', url: url, data: {method: 'delete', output: 'json', budgetTransferId: $('#budgetTransferIdPreview').val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
            var smileyRoll = './images/icons/smiley-roll.png';
            var $infoPanel = $('#infoPanel');
            $infoPanel.html('').empty().html("<span class='label label-warning'><img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
            if ($infoPanel.is(':hidden')) {
...
function deleteGridRecordCheckbox(leafId, url, urlList, securityToken) {
    var stringText = '';
    var counter = 0;
    $('input:checkbox[name="budgetTransferId[]"]').each(function() {
        stringText = stringText + "&budgetTransferId[]=" + $(this).val();
    });
...
function newRecord(leafId, url, urlList, securityToken, type, createAccess, updateAccess, deleteAccess) {
    ...
    var $budgetTransferFrom = $('#budgetTransferFrom');
    var $budgetTransferTo = $('#budgetTransferTo');
    var $financeYearId = $('#financeYearId');
    var $financePeriodRangeId = $('#financePeriodRangeId');
    var $budgetTransferDate = $('#budgetTransferDate');
    var $budgetTransferAmount = $('#budgetTransferAmount');
    var $budgetTransferComment = $('#budgetTransferComment');
    if (css.search('disabled') > 0) {
        return false;
    } else {
        if (type === 1) {
            if ($budgetTransferFrom.val().length === 0) {
                $('#budgetTransferFromHelpMe').html('').empty().html("<span class='label label-danger'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['budgetTransferFromLabel'] + " </span>");
                $budgetTransferFrom.data('chosen').activate_action();
                return false;
            }
            if ($budgetTransferTo.val().length === 0) {
                $('#budgetTransferToHelpMe').html('').empty().html("<span class='label label-danger'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['budgetTransferToLabel'] + " </span>");
                $budgetTransferTo.data('chosen').activate_action();
                return false;
            }
            ...
            if ($financePeriodRangeId.val().length === 0) {
                $('#financePeriodRangeIdHelpMe').html('').empty().html("<span class='label label-danger'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['financePeriodRangeIdLabel'] + " </span>");
                $financePeriodRangeId.data('chosen').activate_action();
                return false;
            }
            if ($budgetTransferDate.val().length === 0) {
                $('#budgetTransferDateHelpMe').html('').empty().html("<span class='label label-danger'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['budgetTransferDateLabel'] + " </span>");
                $('#budgetTransferDateForm').removeClass().addClass('form-group has-error');
                $budgetTransferDate.focus();
                return false;
            }
            if ($budgetTransferAmount.val().length === 0) {
                $('#budgetTransferAmountHelpMe').html('').empty().html("<span class='label label-danger'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['budgetTransferAmountLabel'] + " </span>");
                $('#budgetTransferAmountForm').removeClass().addClass('form-group has-error');
                $budgetTransferAmount.focus();
                return false;
            }
            if ($budgetTransferComment.val().length === 0) {
                $('#budgetTransferCommentHelpMe').html('').empty().html("<span class='label label-danger'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['budgetTransferCommentLabel'] + " </span>");
                $('#budgetTransferCommentForm').removeClass().addClass('form-group has-error');
                $budgetTransferComment.focus();
                return false;
            }
            $.ajax({type: 'POST', url: url, data: {method: 'create', output: 'json', budgetTransferFrom: $budgetTransferFrom.val(), budgetTransferTo: $budgetTransferTo.val(), financeYearId: $financeYearId.val(), financePeriodRangeId: $financePeriodRangeId.val(), budgetTransferDate: $budgetTransferDate.val(), budgetTransferAmount: $budgetTransferAmount.val(), budgetTransferComment: $budgetTransferComment.val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    var smileyRoll = './images/icons/smiley-roll.png';
                    var $infoPanel = $('#infoPanel');
                    $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                    if ($infoPanel.is(':hidden')) {
                    ...
                    if (success === true) {
                        ...
                        $budgetTransferFrom.val('');
                        $budgetTransferFrom.trigger("chosen:updated");
                        $('#budgetTransferFromHelpMe').html('').empty();
                        $budgetTransferTo.val('');
                        $budgetTransferTo.trigger("chosen:updated");
                        $('#budgetTransferToHelpMe').html('').empty();
                        $financeYearId.val('');
                        $financeYearId.trigger("chosen:updated");
                        $('#financeYearIdHelpMe').html('').empty();
                        $financePeriodRangeId.val('');
                        $financePeriodRangeId.trigger("chosen:updated");
                        $('#financePeriodRangeIdHelpMe').html('').empty();
                        $("documentNumber").val('');
                        $('#documentNumberHelpMe').html('').empty();
                        $budgetTransferDate.val('');
                        $('#budgetTransferDateHelpMe').html('').empty();
                        $budgetTransferAmount.val('');
                        $('#budgetTransferAmountHelpMe').html('').empty();
                        $budgetTransferComment.val('');
                        $('#budgetTransferCommentForm').removeClass().addClass('form-group');
                        $('#budgetTransferCommentHelpMe').html('').empty();
                    } else if (success === false) {
                        $infoPanel.html('').empty().html("<span class='label label-danger'>&nbsp;" + message + "</span>");
                        if ($infoPanel.is(':hidden')) {
                            $infoPanel.show();
            ...
            $.ajax({type: 'POST', url: url, data: {method: 'create', output: 'json', budgetTransferFrom: $budgetTransferFrom.val(), budgetTransferTo: $budgetTransferTo.val(), financeYearId: $financeYearId.val(), financePeriodRangeId: $financePeriodRangeId.val(), budgetTransferDate: $budgetTransferDate.val(), budgetTransferAmount: $budgetTransferAmount.val(), budgetTransferComment: $budgetTransferComment.val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    ...
                    if (success === true) {
                        ...
                        $('#budgetTransferId').val(data.budgetTransferId);
                        $('#newRecordButton1').removeClass().addClass('btn btn-success disabled');
                        $('#newRecordButton2').removeClass().addClass('btn dropdown-toggle btn-success disabled');
                        $('#newRecordButton3').attr('onClick', '');
                        $('#newRecordButton4').attr('onClick', '');
...
function updateRecord(leafId, url, urlList, securityToken, type, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        if (type === 1) {
            ...
            $infoPanel.html('').empty();
            $.ajax({type: 'POST', url: url, data: {method: 'save', output: 'json', budgetTransferId: $budgetTransferId.val(), budgetTransferFrom: $budgetTransferFrom.val(), budgetTransferTo: $budgetTransferTo.val(), financeYearId: $financeYearId.val(), financePeriodRangeId: $financePeriodRangeId.val(), budgetTransferDate: $budgetTransferDate.val(), budgetTransferAmount: $budgetTransferAmount.val(), budgetTransferComment: $budgetTransferComment.val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    var smileyRoll = './images/icons/smiley-roll.png';
                    var $infoPanel = $('#infoPanel');
                    $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                    if ($infoPanel.is(':hidden')) {
...
function deleteRecord(leafId, url, urlList, securityToken, deleteAccess) {
    var $infoPanel = $('#infoPanel');
    var $budgetTransferId = $('#budgetTransferId');
    ...
    if (css.search('disabled') > 0) {
        ...
        if (deleteAccess === 1) {
            if (confirm(decodeURIComponent(t['deleteRecordMessageLabel']))) {
                var value = $budgetTransferId.val();
                if (!value) {
                    ...
                    $.ajax({type: 'POST', url: url, data: {method: 'delete', output: 'json', budgetTransferId: $budgetTransferId.val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                            var smileyRoll = './images/icons/smiley-roll.png';
                            var $infoPanel = $('#infoPanel');
                            $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                            if ($infoPanel.is(':hidden')) {
...
function resetRecord(leafId, url, urlList, securityToken, createAccess, updateAccess, deleteAccess) {
    ...
    $("#budgetTransferId").val('');
    $("#budgetTransferIdHelpMe").empty().html('');

    $("#budgetTransferFrom").val('');
    $("#budgetTransferFromHelpMe").empty().html('');
    $('#budgetTransferFrom').trigger("chosen:updated");
    $("#budgetTransferTo").val('');
    $("#budgetTransferToHelpMe").empty().html('');
    $('#budgetTransferTo').trigger("chosen:updated");
    $("#financeYearId").val('');
    ...
    $("#budgetTransferDate").val('');
    $("#budgetTransferDateHelpMe").empty().html('');
    $("#budgetTransferAmount").val('');
    $("#budgetTransferAmountHelpMe").empty().html('');
    $("#budgetTransferComment").val('');
    $("#budgetTransferCommentHelpMe").empty().html('');
    $('#budgetTransferComment').empty().val('');
}
function postRecord() {
...
function firstRecord(leafId, url, urlList, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        $.ajax({type: 'GET', url: url, data: {method: 'dataNavigationRequest', dataNavigation: 'firstRecord', output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                ...
                if (success === true) {
                    $.ajax({type: 'POST', url: url, data: {method: 'read', budgetTransferId: firstRecord, output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                            var smileyRoll = './images/icons/smiley-roll.png';
                            $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                            if ($infoPanel.is(':hidden')) {
                                $infoPanel.show();
                            ...
                            if (success === true) {
                                $('#budgetTransferId').val(data.data.budgetTransferId);
                                $('#budgetTransferFrom').val(data.data.budgetTransferFrom).trigger("chosen:updated");
                                $('#budgetTransferTo').val(data.data.budgetTransferTo).trigger("chosen:updated");
                                $('#financeYearId').val(data.data.financeYearId).trigger("chosen:updated");
                                $('#financePeriodRangeId').val(data.data.financePeriodRangeId).trigger("chosen:updated");
                                $('#documentNumber').val(data.data.documentNumber);
                                x = data.data.budgetTransferDate;
                                x = x.split("-");
                                output = x[2] + "-" + x[1] + "-" + x[0];
                                output = output.toString();
                                $('#budgetTransferDate').val(output);
                                $('#budgetTransferAmount').val(data.data.budgetTransferAmount);
                                $('#budgetTransferComment').val(data.data.budgetTransferComment);
                                if (nextRecord > 0) {
                                    $('#previousRecordButton').removeClass().addClass('btn btn-info disabled').attr('onClick', '');
...
function endRecord(leafId, url, urlList, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        $.ajax({type: 'GET', url: url, data: {method: 'dataNavigationRequest', dataNavigation: 'lastRecord', output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                ...
                if (success === true) {
                    $.ajax({type: 'POST', url: url, data: {method: 'read', budgetTransferId: lastRecord, output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                            var smileyRoll = './images/icons/smiley-roll.png';
                            $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                            if ($infoPanel.is(':hidden')) {
                                $infoPanel.show();
...
function previousRecord(leafId, url, urlList, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($previousRecordCounter.val()) > 0 && parseFloat($previousRecordCounter.val()) < parseFloat($('#lastRecordCounter').val())) {
            $.ajax({type: 'POST', url: url, data: {method: 'read', budgetTransferId: $previousRecordCounter.val(), output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    var smileyRoll = './images/icons/smiley-roll.png';
                    var $infoPanel = $('#infoPanel');
                    $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                    if ($infoPanel.is(':hidden')) {
...
function nextRecord(leafId, url, urlList, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($nextRecordCounter.val()) <= parseFloat($('#lastRecordCounter').val())) {
            $.ajax({type: 'POST', url: url, data: {method: 'read', budgetTransferId: $nextRecordCounter.val(), output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    var smileyRoll = './images/icons/smiley-roll.png';
                    var $infoPanel = $('#infoPanel');
                    $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                    if ($infoPanel.is(':hidden')) {
