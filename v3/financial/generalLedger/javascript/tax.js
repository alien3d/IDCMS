function getTaxCategory(leafId, url, securityToken) {
    $.ajax({
        type: 'GET',
        url: url,
        data: {
...
function getTaxType(leafId, url, securityToken) {
    $.ajax({
        type: 'GET',
        url: url,
        data: {
...
function showFormUpdate(leafId, url, urlList, securityToken, taxId, updateAccess, deleteAccess) {
    sleep(500);
...
function showModalDelete(taxId, taxCategoryId, taxTypeId, chartOfAccountId, taxName, taxCode, taxRate, taxStartDate, taxEndDate, taxDescription) {
    // clear first old record if exist
    $('#taxIdPreview').val('').val(decodeURIComponent(taxId));
    $('#taxCategoryIdPreview').val('').val(decodeURIComponent(taxCategoryId));
    $('#taxTypeIdPreview').val('').val(decodeURIComponent(taxTypeId));
    $('#chartOfAccountIdPreview').val('').val(decodeURIComponent(chartOfAccountId));
    $('#taxNamePreview').val('').val(decodeURIComponent(taxName));
    $('#taxCodePreview').val('').val(decodeURIComponent(taxCode));
    $('#taxRatePreview').val('').val(decodeURIComponent(taxRate));
    $('#taxStartDatePreview').val('').val(decodeURIComponent(taxStartDate));
    $('#taxEndDatePreview').val('').val(decodeURIComponent(taxEndDate));
    $('#taxDescriptionPreview').val('').val(decodeURIComponent(taxDescription));
...
function deleteGridRecordCheckbox(leafId, url, urlList, securityToken) {
    var stringText = '';
    var counter = 0;
    $('input:checkbox[name="taxId[]"]').each(function() {
        stringText = stringText + "&taxId[]=" + $(this).val();
    });
...
function newRecord(leafId, url, urlList, securityToken, type, createAccess, updateAccess, deleteAccess) {
    ...
    var $taxTypeId = $('#taxTypeId');
    var $chartOfAccountId = $('#chartOfAccountId');
    var $taxName = $('#taxName');
    var $taxCode = $('#taxCode');
    var $taxRate = $('#taxRate');
    var $taxStartDate = $('#taxStartDate');
    var $taxEndDate = $('#taxEndDate');
    var $taxDescription = $('#taxDescription');
    if (css.search('disabled') > 0) {
        return false;
    } else {
        if (type === 1) {
            if ($taxCategoryId.val().length === 0) {
                $('#taxCategoryIdHelpMe')
                        .html('').empty()
                        .html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['taxCategoryIdLabel'] + " </span>");
                $taxCategoryId.data('chosen').activate_action();
                return false;
            }
            if ($taxTypeId.val().length === 0) {
                $('#taxTypeIdHelpMe')
                        .html('').empty()
                        .html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['taxTypeIdLabel'] + " </span>");
                $taxTypeId.data('chosen').activate_action();
                return false;
            }
            if ($chartOfAccountId.val().length === 0) {
                $('#chartOfAccountIdHelpMe')
                        .html('').empty()
                        .html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['chartOfAccountIdLabel'] + " </span>");
            ...
            if ($taxName.val().length === 0) {
                $('#taxNameHelpMe')
                        .html('').empty()
                        .html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['taxNameLabel'] + " </span>");
                $('#taxNameForm')
                        .removeClass().addClass('col-xs-6 col-sm-6 col-md-6 col-lg-6 form-group has-error');
                $taxName.focus();
                return false;
            }
            if ($taxCode.val().length === 0) {
                $('#taxCodeHelpMe')
                        .html('').empty()
                        .html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['taxCodeLabel'] + " </span>");
                $('#taxCodeForm')
                        .removeClass().addClass('col-xs-6 col-sm-6 col-md-6 col-lg-6 form-group has-error');
                $taxCode.focus();
                return false;
            }
            if ($taxRate.val().length === 0) {
                $('#taxRateHelpMe')
                        .html('').empty()
                        .html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['taxRateLabel'] + " </span>");
                $('#taxRateForm')
                        .removeClass().addClass('col-xs-6 col-sm-6 col-md-6 col-lg-6 form-group has-error');
                $taxRate.focus();
                return false;
            }
            if ($taxStartDate.val().length === 0) {
                $('#taxStartDateHelpMe')
                        .html('').empty()
                        .html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['taxStartDateLabel'] + " </span>");
                $('#taxStartDateForm')
                        .removeClass().addClass('col-xs-6 col-sm-6 col-md-6 col-lg-6 form-group has-error');
                $taxStartDate.focus();
                return false;
            }
            if ($taxEndDate.val().length === 0) {
                $('#taxEndDateHelpMe')
                        .html('').empty()
                        .html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['taxEndDateLabel'] + " </span>");
                $('#taxEndDateForm')
                        .removeClass().addClass('col-xs-6 col-sm-6 col-md-6 col-lg-6 form-group has-error');
                $taxEndDate.focus();
                return false;
            }
            if ($taxDescription.val().length === 0) {
                $('#taxDescriptionHelpMe')
                        .html('').empty()
                        .html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['taxDescriptionLabel'] + " </span>");
                $('#taxDescriptionForm')
                        .removeClass().addClass('col-xs-6 col-sm-6 col-md-6 col-lg-6 form-group has-error');
            ...
            $.ajax({
                ...
                data: {
                    method: 'create',
                    output: 'json',
                    taxCategoryId: $taxCategoryId.val(),
                    taxTypeId: $taxTypeId.val(),
                    chartOfAccountId: $chartOfAccountId.val(),
                    taxName: $taxName.val(),
                    taxCode: $taxCode.val(),
                    taxRate: $taxRate.val(),
                    taxStartDate: $taxStartDate.val(),
                    taxEndDate: $taxEndDate.val(),
                ...
                success: function(data) {
                    ...
                    if (success === true) {
                        ...
                        $taxCategoryId.val('');
                        $taxCategoryId.trigger("chosen:updated");
                        $('#taxCategoryIdHelpMe')
                                .html('').empty();
                        $taxTypeId.val('');
                        $taxTypeId.trigger("chosen:updated");
                        $('#taxTypeIdHelpMe')
                                .html('').empty();
        ...
        if (type === 1) {
            ...
            if ($taxName.val().length === 0) {
                $('#taxNameHelpMe')
                        .html('').empty()
                        .html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['taxNameLabel'] + " </span>");
                $('#taxNameForm').removeClass().addClass('col-xs-6 col-sm-6 col-md-6 col-lg-6 form-group has-error');
                $taxName.focus();
                return false;
            }
            if ($taxCode.val().length === 0) {
                $('#taxCodeHelpMe')
                        .html('').empty()
                        .html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['taxCodeLabel'] + " </span>");
                $('#taxCodeForm').removeClass().addClass('col-xs-6 col-sm-6 col-md-6 col-lg-6 form-group has-error');
                $taxCode.focus();
                return false;
            }
            if ($taxRate.val().length === 0) {
                $('#taxRateHelpMe')
                        .html('').empty()
                        .html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['taxRateLabel'] + " </span>");
                $('#taxRateForm').removeClass().addClass('col-xs-6 col-sm-6 col-md-6 col-lg-6 form-group has-error');
                $taxRate.focus();
                return false;
            }
            if ($taxStartDate.val().length === 0) {
                $('#taxStartDateHelpMe')
                        .html('').empty()
                        .html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['taxStartDateLabel'] + " </span>");
                $('#taxStartDateForm').removeClass().addClass('col-xs-6 col-sm-6 col-md-6 col-lg-6 form-group has-error');
                $taxStartDate.focus();
                return false;
            }
            if ($taxEndDate.val().length === 0) {
                $('#taxEndDateHelpMe')
                        .html('').empty()
                        .html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['taxEndDateLabel'] + " </span>");
                $('#taxEndDateForm').removeClass().addClass('col-xs-6 col-sm-6 col-md-6 col-lg-6 form-group has-error');
                $taxEndDate.focus();
                return false;
            }
            if ($taxDescription.val().length === 0) {
                $('#taxDescriptionHelpMe')
                        .html('').empty()
                        .html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['taxDescriptionLabel'] + " </span>");
                $('#taxDescriptionForm').removeClass().addClass('col-xs-6 col-sm-6 col-md-6 col-lg-6 form-group has-error');
                $taxDescription.focus();
                return false;
            }
            if ($infoPanel.is(':hidden')) {
                ...
                if (success === true) {
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if (success === true) {
                                $('#taxId').val(data.data.taxId);
                                $('#taxCategoryId').val(data.data.taxCategoryId).trigger("chosen:updated");
                                $('#taxTypeId').val(data.data.taxTypeId).trigger("chosen:updated");
                                $('#chartOfAccountId').val(data.data.chartOfAccountId).trigger("chosen:updated");
                                $('#taxName').val(data.data.taxName);
                                $('#taxCode').val(data.data.taxCode);
                                $('#taxRate').val(data.data.taxRate);
                                x = data.data.taxStartDate;
                                x = x.split("-");
                                output = x[2] + "-" + x[1] + "-" + x[0];
                                output = output.toString();
                                $('#taxStartDate').val(output);
                                x = data.data.taxEndDate;
                                x = x.split("-");
                                output = x[2] + "-" + x[1] + "-" + x[0];
                                output = output.toString();
                                $('#taxEndDate').val(output);
                                $('#taxDescription').val(data.data.taxDescription);
                                if (nextRecord > 0) {
                                    $('#previousRecordButton')
                                            .removeClass().addClass('btn btn-default disabled')
                                            .attr('onClick', '');
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($previousRecordCounter.val()) > 0 && parseFloat($previousRecordCounter.val()) < parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    taxId: $previousRecordCounter.val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
