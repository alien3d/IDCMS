function checkDuplicate(leafId, page, securityToken) {
    var $budgetTypeCode = $("#budgetTypeCode");
    if ($budgetTypeCode.val().length === 0) {
        alert(t['oddTextLabel']);
        return false;
    }
    $.ajax({type: 'GET', url: page, data: {budgetTypeCode: $budgetTypeCode.val(), method: 'duplicate', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
            ...
            if (success === true) {
                if (total !== 0) {
                    $("#budgetTypeCode").val('').focus();
                    $("#budgetTypeCodeForm").removeClass().addClass("form-group has-error");
                ...
                $("#budgetTypeForm").removeClass().addClass("form-group has-error");
            }
            if ($infoPanel.is(':hidden')) {
                $infoPanel.show();
            }
...
function showFormUpdate(leafId, url, urlList, securityToken, budgetTypeId, updateAccess, deleteAccess) {
    sleep(500);
    $('a[rel=tooltip]').tooltip('hide');
    $.ajax({type: 'POST', url: urlList, data: {method: 'read', type: 'form', budgetTypeId: budgetTypeId, securityToken: securityToken, leafId: leafId}, beforeSend: function() {
            var smileyRoll = './images/icons/smiley-roll.png';
            var $infoPanel = $('#infoPanel');
            $infoPanel.html('').empty().html("<span class='label label-warning'><img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
            if ($infoPanel.is(':hidden')) {
...
function showModalDelete(budgetTypeId, budgetTypeCode, budgetTypeDescription) {
    $('#budgetTypeIdPreview').val('').val(decodeURIComponent(budgetTypeId));
    $('#budgetTypeCodePreview').val('').val(decodeURIComponent(budgetTypeCode));
    $('#budgetTypeDescriptionPreview').val('').val(decodeURIComponent(budgetTypeDescription));
    showMeModal('deletePreview', 1);
}
function deleteGridRecord(leafId, url, urlList, securityToken) {
    $.ajax({type: 'POST', url: url, data: {method: 'delete', output: 'json', budgetTypeId: $('#budgetTypeIdPreview').val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
            var smileyRoll = './images/icons/smiley-roll.png';
            var $infoPanel = $('#infoPanel');
            $infoPanel.html('').empty().html("<span class='label label-warning'><img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
            if ($infoPanel.is(':hidden')) {
...
function deleteGridRecordCheckbox(leafId, url, urlList, securityToken) {
    var stringText = '';
    var counter = 0;
    $('input:checkbox[name="budgetTypeId[]"]').each(function() {
        stringText = stringText + "&budgetTypeId[]=" + $(this).val();
    });
...
function newRecord(leafId, url, urlList, securityToken, type, createAccess, updateAccess, deleteAccess) {
    ...
    var $budgetTypeCode = $('#budgetTypeCode');
    var $budgetTypeDescription = $('#budgetTypeDescription');
    if (css.search('disabled') > 0) {
        return false;
    } else {
        if (type === 1) {
            if ($budgetTypeCode.val().length === 0) {
                $('#budgetTypeCodeHelpMe').html('').empty().html("<span class='label label-danger'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['budgetTypeCodeLabel'] + " </span>");
                $('#budgetTypeCodeForm').removeClass().addClass('form-group has-error');
                $budgetTypeCode.focus();
                return false;
            }
            if ($budgetTypeDescription.val().length === 0) {
                $('#budgetTypeDescriptionHelpMe').html('').empty().html("<span class='label label-danger'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['budgetTypeDescriptionLabel'] + " </span>");
                $('#budgetTypeDescriptionForm').removeClass().addClass('form-group has-error');
                $budgetTypeDescription.focus();
                return false;
            }
            $.ajax({type: 'POST', url: url, data: {method: 'create', output: 'json', budgetTypeCode: $budgetTypeCode.val(), budgetTypeDescription: $budgetTypeDescription.val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    var smileyRoll = './images/icons/smiley-roll.png';
                    var $infoPanel = $('#infoPanel');
                    $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                    if ($infoPanel.is(':hidden')) {
                    ...
                    if (success === true) {
                        ...
                        $budgetTypeCode.val('');
                        $('#budgetTypeCodeHelpMe').html('').empty();
                        $budgetTypeDescription.val('');
                        $('#budgetTypeDescriptionForm').removeClass().addClass('form-group');
                        $('#budgetTypeDescriptionHelpMe').html('').empty();
                    } else if (success === false) {
                        $infoPanel.html('').empty().html("<span class='label label-danger'>&nbsp;" + message + "</span>");
                        if ($infoPanel.is(':hidden')) {
                            $infoPanel.show();
            ...
            $.ajax({type: 'POST', url: url, data: {method: 'create', output: 'json', budgetTypeCode: $budgetTypeCode.val(), budgetTypeDescription: $budgetTypeDescription.val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    ...
                    if (success === true) {
                        ...
                        $('#budgetTypeId').val(data.budgetTypeId);
                        $('#newRecordButton1').removeClass().addClass('btn btn-success disabled').attr('onClick', '');
                        $('#newRecordButton2').removeClass().addClass('btn dropdown-toggle btn-success disabled');
                        $('#newRecordButton3').attr('onClick', '');
                        $('#newRecordButton4').attr('onClick', '');
...
function updateRecord(leafId, url, urlList, securityToken, type, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        if (type === 1) {
            ...
            $infoPanel.html('').empty();
            $.ajax({type: 'POST', url: url, data: {method: 'save', output: 'json', budgetTypeId: $budgetTypeId.val(), budgetTypeCode: $budgetTypeCode.val(), budgetTypeDescription: $budgetTypeDescription.val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    var smileyRoll = './images/icons/smiley-roll.png';
                    var $infoPanel = $('#infoPanel');
                    $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                    if ($infoPanel.is(':hidden')) {
        ...
        if (deleteAccess === 1) {
            if (confirm(decodeURIComponent(t['deleteRecordMessageLabel']))) {
                ...
                if (!value) {
                    ...
                    $.ajax({type: 'POST', url: url, data: {method: 'delete', output: 'json', budgetTypeId: $budgetTypeId.val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                            var smileyRoll = './images/icons/smiley-roll.png';
                            var $infoPanel = $('#infoPanel');
                            $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                            if ($infoPanel.is(':hidden')) {
...
function resetRecord(leafId, url, urlList, securityToken, createAccess, updateAccess, deleteAccess) {
    ...
    $("#budgetTypeId").val('');
    $("#budgetTypeIdHelpMe").empty().html('');

    $("#budgetTypeCode").val('');
    $("#budgetTypeCodeHelpMe").empty().html('');
    $("#budgetTypeDescription").val('');
    $("#budgetTypeDescriptionHelpMe").empty().html('');
    $('#budgetTypeDescription').empty().val('');
}
function postRecord() {
...
function firstRecord(leafId, url, urlList, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        $.ajax({type: 'GET', url: url, data: {method: 'dataNavigationRequest', dataNavigation: 'firstRecord', output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                ...
                if (success === true) {
                    $.ajax({type: 'POST', url: url, data: {method: 'read', budgetTypeId: firstRecord, output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                            var smileyRoll = './images/icons/smiley-roll.png';
                            $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                            if ($infoPanel.is(':hidden')) {
                                $infoPanel.show();
                            ...
                            if (success === true) {
                                $('#budgetTypeId').val(data.data.budgetTypeId);
                                $('#budgetTypeCode').val(data.data.budgetTypeCode);
                                $('#budgetTypeDescription').val(data.data.budgetTypeDescription);
                                if (nextRecord > 0) {
                                    $('#previousRecordButton').removeClass().addClass('btn btn-default disabled').attr('onClick', '');
...
function endRecord(leafId, url, urlList, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        $.ajax({type: 'GET', url: url, data: {method: 'dataNavigationRequest', dataNavigation: 'lastRecord', output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                ...
                if (success === true) {
                    $.ajax({type: 'POST', url: url, data: {method: 'read', budgetTypeId: lastRecord, output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                            var smileyRoll = './images/icons/smiley-roll.png';
                            $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                            if ($infoPanel.is(':hidden')) {
                                $infoPanel.show();
...
function previousRecord(leafId, url, urlList, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($previousRecordCounter.val()) > 0 && parseFloat($previousRecordCounter.val()) < parseFloat($('#lastRecordCounter').val())) {
            $.ajax({type: 'POST', url: url, data: {method: 'read', budgetTypeId: $previousRecordCounter.val(), output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    var smileyRoll = './images/icons/smiley-roll.png';
                    var $infoPanel = $('#infoPanel');
                    $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                    if ($infoPanel.is(':hidden')) {
...
function nextRecord(leafId, url, urlList, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($nextRecordCounter.val()) <= parseFloat($('#lastRecordCounter').val())) {
            $.ajax({type: 'POST', url: url, data: {method: 'read', budgetTypeId: $nextRecordCounter.val(), output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    var smileyRoll = './images/icons/smiley-roll.png';
                    var $infoPanel = $('#infoPanel');
                    $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                    if ($infoPanel.is(':hidden')) {
