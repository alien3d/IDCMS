function getChartOfAccountSegmentType(leafId, url, securityToken) {
    $.ajax({type: 'GET', url: url, data: {offset: 0, limit: 99999, method: 'read', type: 'filter', securityToken: securityToken, leafId: leafId, filter: 'chartOfAccountSegmentType'}, beforeSend: function() {
            ...
            if (success === false) {
                ...
                $("#chartOfAccountSegmentTypeId").html('').empty().html(data.data).trigger("chosen:updated");
...
function checkDuplicate(leafId, page, securityToken) {
    var $chartOfAccountSegmentCode = $("#chartOfAccountSegmentCode");
    if ($chartOfAccountSegmentCode.val().length === 0) {
        alert(t['oddTextLabel']);
        return false;
    }
    $.ajax({type: 'GET', url: page, data: {chartOfAccountSegmentCode: $chartOfAccountSegmentCode.val(), method: 'duplicate', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
            var smileyRoll = './images/icons/smiley-roll.png';
            var $infoPanel = $('#infoPanel');
            $infoPanel.html('').empty().html("<span class='label label-warning'><img src='" + smileyRoll + "'>&nbsp;" + decodeURIComponent(t['loadingTextLabel']) + "....</span>");
            if ($infoPanel.is(':hidden')) {
            ...
            if (success === true) {
                if (total !== 0) {
                    $("#chartOfAccountSegmentCode").val('').focus();
                    $("#chartOfAccountSegmentCodeForm").removeClass().addClass("form-group has-error");
                ...
                $("#chartOfAccountSegmentForm").removeClass().addClass("form-group has-error");
            }
            if ($infoPanel.is(':hidden')) {
                $infoPanel.show();
            }
...
function showFormUpdate(leafId, url, urlList, securityToken, chartOfAccountSegmentId, updateAccess, deleteAccess) {
    sleep(500);
    $('a[rel=tooltip]').tooltip('hide');
    $.ajax({type: 'POST', url: urlList, data: {method: 'read', type: 'form', chartOfAccountSegmentId: chartOfAccountSegmentId, securityToken: securityToken, leafId: leafId}, beforeSend: function() {
            var smileyRoll = './images/icons/smiley-roll.png';
            var $infoPanel = $('#infoPanel');
            $infoPanel.html('').empty().html("<span class='label label-warning'><img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
            if ($infoPanel.is(':hidden')) {
...
function showModalDelete(chartOfAccountSegmentId, chartOfAccountSegmentTypeId, chartOfAccountSegmentNumber, chartOfAccountSegmentLength, chartOfAccountSegmentTitle, chartOfAccountSegmentDescription) {
    $('#chartOfAccountSegmentIdPreview').val('').val(decodeURIComponent(chartOfAccountSegmentId));
    $('#chartOfAccountSegmentTypeIdPreview').val('').val(decodeURIComponent(chartOfAccountSegmentTypeId));
    $('#chartOfAccountSegmentNumberPreview').val('').val(decodeURIComponent(chartOfAccountSegmentNumber));
    $('#chartOfAccountSegmentLengthPreview').val('').val(decodeURIComponent(chartOfAccountSegmentLength));
    $('#chartOfAccountSegmentTitlePreview').val('').val(decodeURIComponent(chartOfAccountSegmentTitle));
    $('#chartOfAccountSegmentDescriptionPreview').val('').val(decodeURIComponent(chartOfAccountSegmentDescription));
    showMeModal('deletePreview', 1);
}
function deleteGridRecord(leafId, url, urlList, securityToken) {
    $.ajax({type: 'POST', url: url, data: {method: 'delete', output: 'json', chartOfAccountSegmentId: $('#chartOfAccountSegmentIdPreview').val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
            var smileyRoll = './images/icons/smiley-roll.png';
            var $infoPanel = $('#infoPanel');
            $infoPanel.html('').empty().html("<span class='label label-warning'><img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
            if ($infoPanel.is(':hidden')) {
...
function deleteGridRecordCheckbox(leafId, url, urlList, securityToken) {
    var stringText = '';
    var counter = 0;
    $('input:checkbox[name="chartOfAccountSegmentId[]"]').each(function() {
        stringText = stringText + "&chartOfAccountSegmentId[]=" + $(this).val();
    });
...
function newRecord(leafId, url, urlList, securityToken, type, createAccess, updateAccess, deleteAccess) {
    ...
    var $chartOfAccountSegmentTypeId = $('#chartOfAccountSegmentTypeId');
    var $chartOfAccountSegmentNumber = $('#chartOfAccountSegmentNumber');
    var $chartOfAccountSegmentLength = $('#chartOfAccountSegmentLength');
    var $chartOfAccountSegmentTitle = $('#chartOfAccountSegmentTitle');
    var $chartOfAccountSegmentDescription = $('#chartOfAccountSegmentDescription');
    if (css.search('disabled') > 0) {
        return false;
    } else {
        if (type === 1) {
            if ($chartOfAccountSegmentTypeId.val().length === 0) {
                $('#chartOfAccountSegmentTypeIdHelpMe').html('').empty().html("<span class='label label-danger'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['chartOfAccountSegmentTypeIdLabel'] + " </span>");
                $chartOfAccountSegmentTypeId.data('chosen').activate_action();
                return false;
            }
            if ($chartOfAccountSegmentNumber.val().length === 0) {
                $('#chartOfAccountSegmentNumberHelpMe').html('').empty().html("<span class='label label-danger'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['chartOfAccountSegmentNumberLabel'] + " </span>");
                $('#chartOfAccountSegmentNumberForm').removeClass().addClass('form-group has-error');
                $chartOfAccountSegmentNumber.focus();
                return false;
            }
            if ($chartOfAccountSegmentLength.val().length === 0) {
                $('#chartOfAccountSegmentLengthHelpMe').html('').empty().html("<span class='label label-danger'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['chartOfAccountSegmentLengthLabel'] + " </span>");
                $('#chartOfAccountSegmentLengthForm').removeClass().addClass('form-group has-error');
                $chartOfAccountSegmentLength.focus();
                return false;
            }
            if ($chartOfAccountSegmentTitle.val().length === 0) {
                $('#chartOfAccountSegmentTitleHelpMe').html('').empty().html("<span class='label label-danger'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['chartOfAccountSegmentTitleLabel'] + " </span>");
                $('#chartOfAccountSegmentTitleForm').removeClass().addClass('form-group has-error');
                $chartOfAccountSegmentTitle.focus();
                return false;
            }
            if ($chartOfAccountSegmentDescription.val().length === 0) {
                $('#chartOfAccountSegmentDescriptionHelpMe').html('').empty().html("<span class='label label-danger'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['chartOfAccountSegmentDescriptionLabel'] + " </span>");
                $('#chartOfAccountSegmentDescriptionForm').removeClass().addClass('form-group has-error');
                $chartOfAccountSegmentDescription.focus();
                return false;
            }
            $.ajax({type: 'POST', url: url, data: {method: 'create', output: 'json', chartOfAccountSegmentTypeId: $chartOfAccountSegmentTypeId.val(), chartOfAccountSegmentNumber: $chartOfAccountSegmentNumber.val(), chartOfAccountSegmentLength: $chartOfAccountSegmentLength.val(), chartOfAccountSegmentTitle: $chartOfAccountSegmentTitle.val(), chartOfAccountSegmentDescription: $chartOfAccountSegmentDescription.val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    var smileyRoll = './images/icons/smiley-roll.png';
                    var $infoPanel = $('#infoPanel');
                    $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                    if ($infoPanel.is(':hidden')) {
                    ...
                    if (success === true) {
                        ...
                        $chartOfAccountSegmentTypeId.val('');
                        $chartOfAccountSegmentTypeId.trigger("chosen:updated");
                        $('#chartOfAccountSegmentTypeIdHelpMe').html('').empty();
                        $chartOfAccountSegmentNumber.val('');
                        $('#chartOfAccountSegmentNumberHelpMe').html('').empty();
                        $chartOfAccountSegmentLength.val('');
                        $('#chartOfAccountSegmentLengthHelpMe').html('').empty();
                        $chartOfAccountSegmentTitle.val('');
                        $('#chartOfAccountSegmentTitleHelpMe').html('').empty();
                        $chartOfAccountSegmentDescription.val('');
                        $('#chartOfAccountSegmentDescriptionForm').removeClass().addClass('form-group');
                        $('#chartOfAccountSegmentDescriptionHelpMe').html('').empty();
                    } else if (success === false) {
                        $infoPanel.html('').empty().html("<span class='label label-danger'>&nbsp;" + message + "</span>");
                        if ($infoPanel.is(':hidden')) {
                            $infoPanel.show();
            ...
            $.ajax({type: 'POST', url: url, data: {method: 'create', output: 'json', chartOfAccountSegmentTypeId: $chartOfAccountSegmentTypeId.val(), chartOfAccountSegmentNumber: $chartOfAccountSegmentNumber.val(), chartOfAccountSegmentLength: $chartOfAccountSegmentLength.val(), chartOfAccountSegmentTitle: $chartOfAccountSegmentTitle.val(), chartOfAccountSegmentDescription: $chartOfAccountSegmentDescription.val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    ...
                    if (success === true) {
                        ...
                        $('#chartOfAccountSegmentId').val(data.chartOfAccountSegmentId);
                        $('#newRecordButton1').removeClass().addClass('btn btn-success disabled').attr('onClick', '');
                        $('#newRecordButton2').removeClass().addClass('btn dropdown-toggle btn-success disabled');
                        $('#newRecordButton3').attr('onClick', '');
                        $('#newRecordButton4').attr('onClick', '');
...
function updateRecord(leafId, url, urlList, securityToken, type, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        if (type === 1) {
            ...
            $infoPanel.html('').empty();
            $.ajax({type: 'POST', url: url, data: {method: 'save', output: 'json', chartOfAccountSegmentId: $chartOfAccountSegmentId.val(), chartOfAccountSegmentTypeId: $chartOfAccountSegmentTypeId.val(), chartOfAccountSegmentNumber: $chartOfAccountSegmentNumber.val(), chartOfAccountSegmentLength: $chartOfAccountSegmentLength.val(), chartOfAccountSegmentTitle: $chartOfAccountSegmentTitle.val(), chartOfAccountSegmentDescription: $chartOfAccountSegmentDescription.val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    var smileyRoll = './images/icons/smiley-roll.png';
                    var $infoPanel = $('#infoPanel');
                    $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                    if ($infoPanel.is(':hidden')) {
...
function deleteRecord(leafId, url, urlList, securityToken, deleteAccess) {
    var $infoPanel = $('#infoPanel');
    var $chartOfAccountSegmentId = $('#chartOfAccountSegmentId');
    ...
    if (css.search('disabled') > 0) {
        ...
        if (deleteAccess === 1) {
            if (confirm(decodeURIComponent(t['deleteRecordMessageLabel']))) {
                var value = $chartOfAccountSegmentId.val();
                if (!value) {
                    ...
                    $.ajax({type: 'POST', url: url, data: {method: 'delete', output: 'json', chartOfAccountSegmentId: $chartOfAccountSegmentId.val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                            var smileyRoll = './images/icons/smiley-roll.png';
                            var $infoPanel = $('#infoPanel');
                            $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                            if ($infoPanel.is(':hidden')) {
...
function resetRecord(leafId, url, urlList, securityToken, createAccess, updateAccess, deleteAccess) {
    ...
    $("#chartOfAccountSegmentId").val('');
    $("#chartOfAccountSegmentIdHelpMe").empty().html('');

    $("#chartOfAccountSegmentTypeId").val('');
    $("#chartOfAccountSegmentTypeIdHelpMe").empty().html('');
    $('#chartOfAccountSegmentTypeId').trigger("chosen:updated");
    $("#chartOfAccountSegmentNumber").val('');
    $("#chartOfAccountSegmentNumberHelpMe").empty().html('');
    $("#chartOfAccountSegmentLength").val('');
    $("#chartOfAccountSegmentLengthHelpMe").empty().html('');
    $("#chartOfAccountSegmentTitle").val('');
    $("#chartOfAccountSegmentTitleHelpMe").empty().html('');
    $("#chartOfAccountSegmentDescription").val('');
    $("#chartOfAccountSegmentDescriptionHelpMe").empty().html('');
    $('#chartOfAccountSegmentDescription').empty().val('');
}
function postRecord() {
...
function firstRecord(leafId, url, urlList, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        $.ajax({type: 'GET', url: url, data: {method: 'dataNavigationRequest', dataNavigation: 'firstRecord', output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                ...
                if (success === true) {
                    $.ajax({type: 'POST', url: url, data: {method: 'read', chartOfAccountSegmentId: firstRecord, output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                            var smileyRoll = './images/icons/smiley-roll.png';
                            $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                            if ($infoPanel.is(':hidden')) {
                                $infoPanel.show();
                            ...
                            if (success === true) {
                                $('#chartOfAccountSegmentId').val(data.data.chartOfAccountSegmentId);
                                $('#chartOfAccountSegmentTypeId').val(data.data.chartOfAccountSegmentTypeId).trigger("chosen:updated");
                                $('#chartOfAccountSegmentNumber').val(data.data.chartOfAccountSegmentNumber);
                                $('#chartOfAccountSegmentLength').val(data.data.chartOfAccountSegmentLength);
                                $('#chartOfAccountSegmentTitle').val(data.data.chartOfAccountSegmentTitle);
                                $('#chartOfAccountSegmentDescription').val(data.data.chartOfAccountSegmentDescription);
                                if (nextRecord > 0) {
                                    $('#previousRecordButton').removeClass().addClass('btn btn-default disabled').attr('onClick', '');
...
function endRecord(leafId, url, urlList, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        $.ajax({type: 'GET', url: url, data: {method: 'dataNavigationRequest', dataNavigation: 'lastRecord', output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                ...
                if (success === true) {
                    $.ajax({type: 'POST', url: url, data: {method: 'read', chartOfAccountSegmentId: lastRecord, output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                            var smileyRoll = './images/icons/smiley-roll.png';
                            $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                            if ($infoPanel.is(':hidden')) {
                                $infoPanel.show();
...
function previousRecord(leafId, url, urlList, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($previousRecordCounter.val()) > 0 && parseFloat($previousRecordCounter.val()) < parseFloat($('#lastRecordCounter').val())) {
            $.ajax({type: 'POST', url: url, data: {method: 'read', chartOfAccountSegmentId: $previousRecordCounter.val(), output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    var smileyRoll = './images/icons/smiley-roll.png';
                    var $infoPanel = $('#infoPanel');
                    $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                    if ($infoPanel.is(':hidden')) {
...
function nextRecord(leafId, url, urlList, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($nextRecordCounter.val()) <= parseFloat($('#lastRecordCounter').val())) {
            $.ajax({type: 'POST', url: url, data: {method: 'read', chartOfAccountSegmentId: $nextRecordCounter.val(), output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    var smileyRoll = './images/icons/smiley-roll.png';
                    var $infoPanel = $('#infoPanel');
                    $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                    if ($infoPanel.is(':hidden')) {
