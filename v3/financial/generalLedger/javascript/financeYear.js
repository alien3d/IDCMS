function checkDuplicate(leafId, page, securityToken) {
    var $financeYearCode = $("#financeYearCode");
    if ($financeYearCode.val().length === 0) {
    ...
    $.ajax({
        ...
        url: page,
        data: {
            financeYearCode: $financeYearCode.val(),
            method: 'duplicate',
        ...
        success: function(data) {
            ...
            if (success === true) {
                if (total !== 0) {
                    $("#financeYearCode").val('').focus();
                    $("#financeYearCodeForm").removeClass().addClass("form-group has-error");
...
function showFormUpdate(leafId, url, urlList, securityToken, financeYearId, updateAccess, deleteAccess) {
    sleep(500);
...
function showModalDelete(financeYearId, financeYearYear, isClose) {
...
function deleteGridRecord(leafId, url, urlList, securityToken) {
    $.ajax({
        ...
        data: {
            method: 'delete',
            output: 'json',
            financeYearId: $('#financeYearIdPreview').val(),
            securityToken: securityToken,
            leafId: leafId
        },
        beforeSend: function() {
...
function deleteGridRecordCheckbox(leafId, url, urlList, securityToken) {
    var stringText = '';
    var counter = 0;
    $('input:checkbox[name="financeYearId[]"]').each(function() {
        stringText = stringText + "&financeYearId[]=" + $(this).val();
    });
...
function newRecord(leafId, url, urlList, securityToken, type, createAccess, updateAccess, deleteAccess) {
    ...
    var $financeYearYear = $('#financeYearYear');
    var $isClose = $('#isClose').is(":checked") ? 1 : 0;
    if (css.search('disabled') > 0) {
        return false;
    } else {
        if (type === 1) {
            if ($financeYearYear.val().length === 0) {
                $('#financeYearYearHelpMe').html('').empty().html("<span class='label label-danger'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['financeYearYearLabel'] + " </span>");
                $('#financeYearYearForm').removeClass().addClass('form-group has-error');
                $financeYearYear.focus();
                return false;
            }
            $.ajax({
                type: 'POST',
                ...
                data: {
                    method: 'create',
                    output: 'json',
                    financeYearYear: $financeYearYear.val(),
                    isClose: $isClose,
                    securityToken: securityToken,
                    leafId: leafId
                },
                ...
                success: function(data) {
                    ...
                    if (success === true) {
                        ...
                        $financeYearYear.val('');
                        $('#financeYearYearHelpMe').html('').empty();
                        $('#isClose').val('');
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (success === true) {
                        ...
                        $('#financeYearId').val(data.financeYearId);
                        $('#newRecordButton1').removeClass().addClass('btn btn-success disabled');
                        $('#newRecordButton2').removeClass().addClass('btn dropdown-toggle btn-success disabled');
                        $('#newRecordButton3').attr('onClick', '');
                        $('#newRecordButton4').attr('onClick', '');
                ...
                if (success === true) {
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if (success === true) {
                                $('#financeYearId').val(data.data.financeYearId);
                                $('#financeYearYear').val(data.data.financeYearYear);
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($previousRecordCounter.val()) > 0 && parseFloat($previousRecordCounter.val()) < parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    financeYearId: $previousRecordCounter.val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($nextRecordCounter.val()) <= parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    financeYearId: $nextRecordCounter.val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
