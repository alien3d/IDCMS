function checkDuplicate(leafId, page, securityToken) {
    var $taxCategoryCode = $("#taxCategoryCode");
    if ($taxCategoryCode.val().length === 0) {
    ...
    $.ajax({
        ...
        url: page,
        data: {
            taxCategoryCode: $taxCategoryCode.val(),
            method: 'duplicate',
        ...
        success: function(data) {
            ...
            if (success === true) {
                if (total !== 0) {
                    $("#taxCategoryCode").val('').focus();
                    $("#taxCategoryCodeForm").removeClass().addClass("form-group has-error");
                ...
                $("#taxCategoryForm").removeClass().addClass("form-group has-error");
            }
            if ($infoPanel.is(':hidden')) {
                $infoPanel.show();
            }
...
function showFormUpdate(leafId, url, urlList, securityToken, taxCategoryId, updateAccess, deleteAccess) {
    sleep(500);
...
function showModalDelete(taxCategoryId, taxCategoryCode, taxCategoryDescription) {
    $('#taxCategoryIdPreview').val('').val(decodeURIComponent(taxCategoryId));
    $('#taxCategoryCodePreview').val('').val(decodeURIComponent(taxCategoryCode));
    $('#taxCategoryDescriptionPreview').val('').val(decodeURIComponent(taxCategoryDescription));
...
function deleteGridRecord(leafId, url, urlList, securityToken) {
    $.ajax({
        ...
        data: {
            method: 'delete',
            output: 'json',
            taxCategoryId: $('#taxCategoryIdPreview').val(),
            securityToken: securityToken,
            leafId: leafId
        },
        beforeSend: function() {
...
function deleteGridRecordCheckbox(leafId, url, urlList, securityToken) {
    var stringText = '';
    var counter = 0;
    $('input:checkbox[name="taxCategoryId[]"]').each(function() {
        stringText = stringText + "&taxCategoryId[]=" + $(this).val();
    });
...
function newRecord(leafId, url, urlList, securityToken, type, createAccess, updateAccess, deleteAccess) {
    ...
    var $taxCategoryCode = $('#taxCategoryCode');
    var $taxCategoryDescription = $('#taxCategoryDescription');
    if (css.search('disabled') > 0) {
        return false;
    } else {
        if (type === 1) {
            if ($taxCategoryCode.val().length === 0) {
                $('#taxCategoryCodeHelpMe').html('').empty().html("<span class='label label-danger'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['taxCategoryCodeLabel'] + " </span>");

                $('#taxCategoryCodeForm').removeClass().addClass('col-xs-6 col-sm-6 col-md-6 col-lg-6 form-group has-error');
            ...
            $.ajax({
                ...
                data: {
                    ...
                    output: 'json',
                    taxCategoryCode: $taxCategoryCode.val(),
                    taxCategoryDescription: $taxCategoryDescription.val(),
                    securityToken: securityToken,
                    leafId: leafId
                },
                beforeSend: function() {
                ...
                success: function(data) {
                    ...
                    if (success === true) {
                        ...
                        $taxCategoryCode.val('');
                        $('#taxCategoryCodeHelpMe').html('').empty();
                        $taxCategoryDescription.val('');
                        $('#taxCategoryDescriptionHelpMe').html('').empty();
                    } else if (success === false) {
                        $infoPanel.html('').empty().html("<span class='label label-danger'>&nbsp;" + message + "</span>");
                        if ($infoPanel.is(':hidden')) {
                            $infoPanel.show();
...
function resetRecord(leafId, url, urlList, securityToken, createAccess, updateAccess, deleteAccess) {
    ...
    $("#taxCategoryId").val('');
    $("#taxCategoryIdHelpMe").empty().html('');
    $("#taxCategoryCode").val('');
    $("#taxCategoryCodeHelpMe").empty().html('');
    $("#taxCategoryDescription").val('');
    $("#taxCategoryDescriptionHelpMe").empty().html('');
}
function postRecord() {
                ...
                if (success === true) {
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if (success === true) {
                                $('#taxCategoryId').val(data.data.taxCategoryId);
                                $('#taxCategoryCode').val(data.data.taxCategoryCode);
                                $('#taxCategoryDescription').val(data.data.taxCategoryDescription);
                                if (nextRecord > 0) {
                                    $('#previousRecordButton').removeClass().addClass('btn btn-default disabled').attr('onClick', '');
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($previousRecordCounter.val()) > 0 && parseFloat($previousRecordCounter.val()) < parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    taxCategoryId: $previousRecordCounter.val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($nextRecordCounter.val()) <= parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    taxCategoryId: $nextRecordCounter.val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
