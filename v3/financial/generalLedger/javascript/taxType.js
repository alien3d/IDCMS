function checkDuplicate(leafId, page, securityToken) {
    var $taxTypeCode = $("#taxTypeCode");
    if ($taxTypeCode.val().length === 0) {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if (success === true) {
                if (total !== 0) {
                    $("#taxTypeCode").val('').focus();
                    $("#taxTypeCodeForm").removeClass().addClass("form-group has-error");
                ...
                $("#taxTypeForm").removeClass().addClass("form-group has-error");
            }
            if ($infoPanel.is(':hidden')) {
                $infoPanel.show();
            }
...
function showFormUpdate(leafId, url, urlList, securityToken, taxTypeId, updateAccess, deleteAccess) {
    sleep(500);
...
function showModalDelete(taxTypeId, taxTypeCode, taxTypeDescription) {
    $('#taxTypeIdPreview').val('').val(decodeURIComponent(taxTypeId));
    $('#taxTypeCodePreview').val('').val(decodeURIComponent(taxTypeCode));
    $('#taxTypeDescriptionPreview').val('').val(decodeURIComponent(taxTypeDescription));
...
function deleteGridRecord(leafId, url, urlList, securityToken) {
    $.ajax({
        ...
        data: {
            method: 'delete',
            output: 'json',
            taxTypeId: $('#taxTypeIdPreview').val(),
            securityToken: securityToken,
            leafId: leafId
        },
        beforeSend: function() {
...
function deleteGridRecordCheckbox(leafId, url, urlList, securityToken) {
    var stringText = '';
    var counter = 0;
    $('input:checkbox[name="taxTypeId[]"]').each(function() {
        stringText = stringText + "&taxTypeId[]=" + $(this).val();
    });
...
function newRecord(leafId, url, urlList, securityToken, type, createAccess, updateAccess, deleteAccess) {
    ...
    var $taxTypeCode = $('#taxTypeCode');
    var $taxTypeDescription = $('#taxTypeDescription');
    if (css.search('disabled') > 0) {
        return false;
    } else {
        if (type === 1) {
            if ($taxTypeCode.val().length === 0) {
                $('#taxTypeCodeHelpMe').html('').empty().html("<span class='label label-danger'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['taxTypeCodeLabel'] + " </span>");
                $('#taxTypeCodeForm').removeClass().addClass('form-group has-error');
                $taxTypeCode.focus();
                return false;
            }
            $.ajax({
                ...
                data: {
                    ...
                    output: 'json',
                    taxTypeCode: $taxTypeCode.val(),
                    taxTypeDescription: $taxTypeDescription.val(),
                    securityToken: securityToken,
                    leafId: leafId
                },
                beforeSend: function() {
                ...
                success: function(data) {
                    ...
                    if (success === true) {
                        ...
                        $taxTypeCode.val('');
                        $('#taxTypeCodeHelpMe').html('').empty();
                        $taxTypeDescription.val('');
                        $('#taxTypeDescriptionHelpMe').html('').empty();
                    } else if (success === false) {
                        $infoPanel.html('').empty().html("<span class='label label-danger'>&nbsp;" + message + "</span>");
                        if ($infoPanel.is(':hidden')) {
                            $infoPanel.show();
...
function resetRecord(leafId, url, urlList, securityToken, createAccess, updateAccess, deleteAccess) {
    ...
    $("#taxTypeId").val('');
    $("#taxTypeIdHelpMe").empty().html('');
    $("#taxTypeCode").val('');
    $("#taxTypeCodeHelpMe").empty().html('');
    $("#taxTypeDescription").val('');
    $("#taxTypeDescriptionHelpMe").empty().html('');
}
function postRecord() {
                ...
                if (success === true) {
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if (success === true) {
                                $('#taxTypeId').val(data.data.taxTypeId);
                                $('#taxTypeCode').val(data.data.taxTypeCode);
                                $('#taxTypeDescription').val(data.data.taxTypeDescription);
                                if (nextRecord > 0) {
                                    $('#previousRecordButton').removeClass().addClass('btn btn-default disabled').attr('onClick', '');
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($previousRecordCounter.val()) > 0 && parseFloat($previousRecordCounter.val()) < parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    taxTypeId: $previousRecordCounter.val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($nextRecordCounter.val()) <= parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    taxTypeId: $nextRecordCounter.val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
