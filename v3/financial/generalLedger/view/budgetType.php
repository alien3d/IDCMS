...
require_once($newFakeDocumentRoot . "v3/financial/generalLedger/controller/budgetTypeController.php");
...
$translator->setCurrentTable('budgetType');
...
$budgetTypeArray = array();
if (isset($_POST)) {
    if (isset($_POST['method'])) {
        $budgetType = new \Core\Financial\GeneralLedger\BudgetType\Controller\BudgetTypeClass();
        define('LIMIT', 10);
        ...
        if (isset($_POST ['query'])) {
            $budgetType->setFieldQuery($_POST ['query']);
        }
        if (isset($_POST ['filter'])) {
            $budgetType->setGridQuery($_POST ['filter']);
        }
        if (isset($_POST ['character'])) {
            $budgetType->setCharacterQuery($_POST['character']);
        }
        if (isset($_POST ['dateRangeStart'])) {
            $budgetType->setDateRangeStartQuery($_POST['dateRangeStart']);
            //explode the data to get day,month,year
            $start = explode('-', $_POST ['dateRangeStart']);
            $budgetType->setStartDay($start[2]);
            $budgetType->setStartMonth($start[1]);
            $budgetType->setStartYear($start[0]);
        }
        if (isset($_POST ['dateRangeEnd']) && (strlen($_POST['dateRangeEnd']) > 0)) {
            $budgetType->setDateRangeEndQuery($_POST['dateRangeEnd']);
            //explode the data to get day,month,year
            $start = explode('-', $_POST ['dateRangeEnd']);
            $budgetType->setEndDay($start[2]);
            $budgetType->setEndMonth($start[1]);
            $budgetType->setEndYear($start[0]);
        }
        if (isset($_POST ['dateRangeType'])) {
            $budgetType->setDateRangeTypeQuery($_POST['dateRangeType']);
        }
        if (isset($_POST ['dateRangeExtraType'])) {
            $budgetType->setDateRangeExtraTypeQuery($_POST['dateRangeExtraType']);
        }
        $budgetType->setServiceOutput('html');
        $budgetType->setLeafId($leafId);
        $budgetType->execute();
        if ($_POST['method'] == 'read') {
            $budgetType->setStart($offset);
            $budgetType->setLimit($limit); // normal system don't like paging..
            $budgetType->setPageOutput('html');
            $budgetTypeArray = $budgetType->read();
            if (isset($budgetTypeArray [0]['firstRecord'])) {
                $firstRecord = $budgetTypeArray [0]['firstRecord'];
            }
            if (isset($budgetTypeArray [0]['nextRecord'])) {
                $nextRecord = $budgetTypeArray [0]['nextRecord'];
            }
            if (isset($budgetTypeArray [0]['previousRecord'])) {
                $previousRecord = $budgetTypeArray [0]['previousRecord'];
            }
            if (isset($budgetTypeArray [0]['lastRecord'])) {
                $lastRecord = $budgetTypeArray [0]['lastRecord'];
                $endRecord = $budgetTypeArray [0]['lastRecord'];
            }
            $navigation = new \Core\Paging\HtmlPaging();
            $navigation->setLeafId($leafId);
            $navigation->setViewPath($budgetType->getViewPath());
            ...
            if (isset($budgetTypeArray [0]['total'])) {
                $total = $budgetTypeArray [0]['total'];
            } else {
                $total = 0;
            }
                        ...
                        <div class="btn-group">
                            ...
                            <ul class="dropdown-menu" style="text-align:left">
                                <li>
                                    ...
                                    echo $budgetType->getControllerPath();
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <form class="form-horizontal">
                                        <input type="hidden" name="budgetTypeIdPreview" id="budgetTypeIdPreview">

                                        <div class="form-group" id="budgetTypeCodeDiv">
                                            <label for="budgetTypeCodePreview"
                                                   class="control-label col-xs-4 col-sm-4 col-md-4"><?php echo $leafTranslation['budgetTypeCodeLabel']; ?></label>

                                            <div class="col-xs-8 col-sm-8 col-md-8">
                                                <input class="form-control" type="text" name="budgetTypeCodePreview"
                                                       id="budgetTypeCodePreview">
                                            </div>
                                        </div>
                                        <div class="form-group" id="budgetTypeDescriptionDiv">
                                            <label for="budgetTypeDescriptionPreview"
                                                   class="control-label col-xs-4 col-sm-4 col-md-4"><?php echo $leafTranslation['budgetTypeDescriptionLabel']; ?></label>

                                            <div class="col-xs-8 col-sm-8 col-md-8">
                                                <input class="form-control" type="text" name="budgetTypeDescriptionPreview"
                                                       id="budgetTypeDescriptionPreview">
                                            </div>
                                        </div>
                                    </form>
                                </div>
                        ...
                        <div class="col-xs-12 col-sm-12 col-md-12">
                            <div class="panel panel-default">
                                <table class="table table table-bordered table-striped table-condensed table-hover smart-form has-tickbox" id="tableData">
                                    ...
                                    <th width="100px">
                                    <div align="center"><?php echo ucwords($leafTranslation['budgetTypeCodeLabel']); ?></div>
                                    </th>
                                    <th><?php echo ucwords($leafTranslation['budgetTypeDescriptionLabel']); ?></th>
                                    <th width="100px">
                                    ...
                                    <tbody id="tableBody">
                                        ...
                                        if ($_POST['method'] == 'read' && $_POST['type'] == 'list' && $_POST['detail'] == 'body') {
                                            if (is_array($budgetTypeArray)) {
                                                $totalRecord = intval(count($budgetTypeArray));
                                                if ($totalRecord > 0) {
                                                    $counter = 0;
                                                    for ($i = 0; $i < $totalRecord; $i++) {
                                                        ...
                                                        ?>
                                                        <tr <?php
                                                        if ($budgetTypeArray[$i]['isDelete'] == 1) {
                                                            echo "class=\"danger\"";
                                                        } else {
                                                            if ($budgetTypeArray[$i]['isDraft'] == 1) {
                                                                echo "class=\"warning\"";
                                                            }
                                                        }
                                                        ?>>
                                                            ...
                                                            <td align="center">
                                                                <?php if ($budgetTypeArray [$i]['budgetTypeCode'] != 'UNBL') { ?>
                                                                    <div class="btn-group" align="center">
                                                                        <button type="button"  class="btn btn-info btn-sm" title="Edit"
                                                                                ...
                                                                                echo intval(
                                                                                        $budgetTypeArray [$i]['budgetTypeId']
                                                                                );
                                                                        ...
                                                                        <button type="button"  class="btn btn-danger btn-sm" title="Delete"
                                                                                ...
                                                                                ?>', '<?php
                                                                                echo rawurlencode(
                                                                                        $budgetTypeArray [$i]['budgetTypeCode']
                                                                                );
                                                                                ?>', '<?php
                                                                                echo rawurlencode(
                                                                                        $budgetTypeArray [$i]['budgetTypeDescription']
                                                                                );
                                                                                ?>');">
                                                            ...
                                                            <td>
                                                                <div align="center">
                                                                    <?php
                                                                    if (isset($budgetTypeArray[$i]['budgetTypeCode'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos(
                                                                                                strtolower($budgetTypeArray[$i]['budgetTypeCode']), strtolower($_POST['query'])
                                                                                        ) !== false
                                                                                ) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $budgetTypeArray[$i]['budgetTypeCode']
                                                                                    );
                                                                                } else {
                                                                                    echo $budgetTypeArray[$i]['budgetTypeCode'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                    if (strpos(
                                                                                                    strtolower($budgetTypeArray[$i]['budgetTypeCode']), strtolower($_POST['character'])
                                                                                            ) !== false
                                                                                    ) {
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $budgetTypeArray[$i]['budgetTypeCode']
                                                                                        );
                                                                                    } else {
                                                                                        echo $budgetTypeArray[$i]['budgetTypeCode'];
                                                                                    }
                                                            ...
                                                            <td>
                                                                <div class="pull-left">
                                                                    <?php
                                                                    if (isset($budgetTypeArray[$i]['budgetTypeDescription'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos(
                                                                                                strtolower($budgetTypeArray[$i]['budgetTypeDescription']), strtolower($_POST['query'])
                                                                                        ) !== false
                                                                                ) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $budgetTypeArray[$i]['budgetTypeDescription']
                                                                                    );
                                                                                } else {
                                                                                    echo $budgetTypeArray[$i]['budgetTypeDescription'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                    if (strpos(
                                                                                                    strtolower($budgetTypeArray[$i]['budgetTypeDescription']), strtolower($_POST['character'])
                                                                                            ) !== false
                                                                                    ) {
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $budgetTypeArray[$i]['budgetTypeDescription']
                                                                                        );
                                                                                    } else {
                                                                                        echo $budgetTypeArray[$i]['budgetTypeDescription'];
                                                                                    }
                                                            ...
                                                            <td>
                                                                <div align="center">
                                                                    <?php
                                                                    if (isset($budgetTypeArray[$i]['executeBy'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos($budgetTypeArray[$i]['staffName'], $_POST['query']) !== false) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $budgetTypeArray[$i]['staffName']
                                                                                    );
                                                                                } else {
                                                                                    echo $budgetTypeArray[$i]['staffName'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                    if (strpos(
                                                                                                    $budgetTypeArray[$i]['staffName'], $_POST['character']
                                                                                            ) !== false
                                                                                    ) {
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $budgetTypeArray[$i]['staffName']
                                                                                        );
                                                                                    } else {
                                                                                        echo $budgetTypeArray[$i]['staffName'];
                                                                                    }
                                                            ...
                                                            <?php
                                                            if (isset($budgetTypeArray[$i]['executeTime'])) {
                                                                $valueArray = $budgetTypeArray[$i]['executeTime'];
                                                            ...
                                                            <?php
                                                            if ($budgetTypeArray[$i]['isDelete']) {
                                                                $checked = "checked";
                                                            } else {
                                                                $checked = null;
                                                            }
                                                            ?>
                                                            <td><?php if ($budgetTypeArray [$i]['budgetTypeCode'] != 'UNBL') { ?>
                                                                    <input style="display:none;" type="checkbox" name="budgetTypeId[]"
                                                                           value="<?php echo $budgetTypeArray[$i]['budgetTypeId']; ?>">
                                                                    <input <?php echo $checked; ?> type="checkbox" name="isDelete[]"
                                                                                                   value="<?php echo $budgetTypeArray[$i]['isDelete']; ?>">
                                                                <?php } ?></td>
                                                        </tr>
                                                        <?php
                                                    }
                        ...
                        <div class="col-xs-3 col-sm-3 col-md-3 pagination">
                            <div class="pull-right">
                                <button class="delete btn btn-warning" type="button" 
                                        ...
                                        echo $budgetType->getControllerPath();
                                        ?>', '<?php echo $budgetType->getViewPath(); ?>', '<?php echo $securityToken; ?>');">
...
if ((isset($_POST['method']) == 'new' || isset($_POST['method']) == 'read') && $_POST['type'] == 'form') {
            ...
            <div class="row">
                <div class="col-xs-12 col-sm-12 col-md-12">
                    <div class="panel panel-info">
                        ...
                        <div class="panel-body">
                            <input type="hidden" name="budgetTypeId" id="budgetTypeId" value="<?php
                            if (isset($_POST['budgetTypeId'])) {
                                echo $_POST['budgetTypeId'];
                            }
                            ?>">

                            <div class="row">
                                <div class="col-xs-12 col-sm-12 col-md-12">
                                    <div class="col-xs-6 col-sm-6 col-md-6 form-group" id="budgetTypeCodeForm">
                                        <label for="budgetTypeCode" class="control-label col-xs-4 col-sm-4 col-md-4"><strong><?php
                                                echo ucfirst(
                                                        $leafTranslation['budgetTypeCodeLabel']
                                                );
                                                ?></strong></label>

                                        <div class="col-xs-8 col-sm-8 col-md-8">
                                            <div class="input-group">
                                                <input type="text" class="form-control" name="budgetTypeCode"
                                                       id="budgetTypeCode" value="<?php
                                                       if (isset($budgetTypeArray) && is_array($budgetTypeArray)) {
                                                           if (isset($budgetTypeArray[0]['budgetTypeCode'])) {
                                                               echo htmlentities($budgetTypeArray[0]['budgetTypeCode']);
                                                           }
                                                       }
                                                       ?>" maxlength="16">
                                                <span class="input-group-addon"><img src="./images/icons/document-code.png"></span>
                                            </div>
                                            <span class="help-block" id="budgetTypeCodeHelpMe"></span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-xs-12 col-sm-12 col-md-12">
                                    <div class="form-group" id="budgetTypeDescriptionForm">
                                        <div class="col-xs-12 col-sm-12 col-md-12">
                                            <textarea rows="5" class="form-control" name="budgetTypeDescription"
                                                      id="budgetTypeDescription"><?php
                                                          if (isset($budgetTypeArray[0]['budgetTypeDescription'])) {
                                                              echo htmlentities($budgetTypeArray[0]['budgetTypeDescription']);
                                                          }
                                                          ?></textarea> <span class="help-block" id="budgetTypeDescriptionHelpMe"></span>
                                        </div>
                                    </div>
                                </div>
                            </div>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            validateMeNumeric('budgetTypeId');
            validateMeAlphaNumeric('budgetTypeCode');
    <?php if ($_POST['method'] == "new") { ?>
                $('#resetRecordButton').removeClass().addClass('btn btn-default');
        <?php if ($leafAccess['leafAccessCreateValue'] == 1) { ?>
                    $('#newRecordButton1').removeClass().addClass('btn btn-success').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $budgetType->getControllerPath(); ?>','<?php echo $budgetType->getViewPath(); ?>','<?php echo $securityToken; ?>',1)");
                    $('#newRecordButton2').removeClass().addClass('btn dropdown-toggle btn-success');
                    $('#newRecordButton3').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $budgetType->getControllerPath(); ?>','<?php echo $budgetType->getViewPath(); ?>','<?php echo $securityToken; ?>',1)");
                    $('#newRecordButton4').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $budgetType->getControllerPath(); ?>','<?php echo $budgetType->getViewPath(); ?>','<?php echo $securityToken; ?>',2)");
                    $('#newRecordButton5').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $budgetType->getControllerPath(); ?>','<?php echo $budgetType->getViewPath(); ?>','<?php echo $securityToken; ?>',3)");
                    $('#newRecordButton6').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $budgetType->getControllerPath(); ?>','<?php echo $budgetType->getViewPath(); ?>','<?php echo $securityToken; ?>',4)");
                    $('#newRecordButton7').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $budgetType->getControllerPath(); ?>','<?php echo $budgetType->getViewPath(); ?>','<?php echo $securityToken; ?>',5)");
        <?php } else { ?>
        ...
        if ($_POST['budgetTypeId']) {
            ...
            <?php if ($leafAccess['leafAccessUpdateValue'] == 1) { ?>
                        $('#updateRecordButton1').removeClass().addClass('btn btn-info').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $budgetType->getControllerPath(); ?>','<?php echo $budgetType->getViewPath(); ?>','<?php echo $securityToken; ?>',1,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                        $('#updateRecordButton2').removeClass().addClass('btn dropdown-toggle btn-info');
                        $('#updateRecordButton3').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $budgetType->getControllerPath(); ?>','<?php echo $budgetType->getViewPath(); ?>','<?php echo $securityToken; ?>',1,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                        $('#updateRecordButton4').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $budgetType->getControllerPath(); ?>','<?php echo $budgetType->getViewPath(); ?>','<?php echo $securityToken; ?>',2,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                        $('#updateRecordButton5').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $budgetType->getControllerPath(); ?>','<?php echo $budgetType->getViewPath(); ?>','<?php echo $securityToken; ?>',3,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
            <?php } else { ?>
                        $('#updateRecordButton1').removeClass().addClass(' btn btn-info disabled').attr('onClick', '');
                        $('#updateRecordButton2').removeClass().addClass('btn dropdown-toggle btn-info disabled');
                        $('#updateRecordButton3').attr('onClick', '');
            ...
            <?php if ($leafAccess['leafAccessDeleteValue'] == 1) { ?>
                        $('#deleteRecordButton').removeClass().addClass('btn btn-danger').attr('onClick', "deleteRecord(<?php echo $leafId; ?>,'<?php echo $budgetType->getControllerPath(); ?>','<?php echo $budgetType->getViewPath(); ?>','<?php echo $securityToken; ?>',<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
            <?php } else { ?>
                        $('#deleteRecordButton').removeClass().addClass('btn btn-danger disabled').attr('onClick', '');
...
<?php } ?>
<script type="text/javascript" src="./v3/financial/generalLedger/javascript/budgetType.js"></script>
<hr>
