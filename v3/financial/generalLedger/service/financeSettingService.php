...
namespace Core\Financial\GeneralLedger\FinanceSetting\Service;
 ...
 * @version 2
 * @author hafizan
 * @package Core\Financial\GeneralLedger\FinanceSetting\Service
 * @subpackage GeneralLedger
 * @link http://www.hafizan.com
 * @license http://www.gnu.org/copyleft/lesser.html LGPL
 */
class FinanceSettingService extends ConfigClass {
        ...
        if ($result) {
            ...
            while (($row = $this->q->fetchArray($result)) == true) {
                if ($this->getServiceOutput() == 'option') {
                    $str .= "<option value='" . $row['countryId'] . "'>" . $d . ". " . $row['countryCurrencyCode'] . " " . $row['countryDescription'] . "</option>";
    ...
    /**
     * Return group concat country currency code.. Current Limit 5 language per country
     * @param type $countryId
     */
    function getConcatCountryCurrencyCode($countryId) {
        //initialize dummy value.. no content header.pure html
        $sql = null;
        $countryCurrenyCode = array();
        if ($this->getVendor() == self::MYSQL) {
            $sql = "
            SELECT      `localeOne`,
                        `localeTwo`,
        ...
        if ($result) {
            $row = $this->q->fetchArray($result);
            $countryCurrenyCode[] = $row['localeOne'];
            $countryCurrenyCode[] = $row['localeTwo'];
            $countryCurrenyCode[] = $row['localeThree'];
            $countryCurrenyCode[] = $row['localeFour'];
            $countryCurrenyCode[] = $row['localeFive'];
        }
        return $countryCurrenyCode;
    }
