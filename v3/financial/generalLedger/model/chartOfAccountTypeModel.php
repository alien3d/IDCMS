...
namespace Core\Financial\GeneralLedger\ChartOfAccountType\Model;
 ...
 * This is Chart Of Account Type model file.This is to ensure strict setting enable for all variable enter to database
 *
 * @name IDCMS .
 * @version 2
 * @author hafizan
 * @package Core\Financial\GeneralLedger\ChartOfAccountType\Model;
 * @subpackage GeneralLedger
 * @link http://www.hafizan.com
 * @license http://www.gnu.org/copyleft/lesser.html LGPL
 */
class ChartOfAccountTypeModel extends ValidationClass {
    ...
    public function execute() {
         ...
         * */
        $this->setTableName('chartOfAccountType');
        $this->setPrimaryKeyName('chartOfAccountTypeId');
        $this->setMasterForeignKeyName('chartOfAccountTypeId');
        $this->setFilterCharacter('chartOfAccountTypeDescription');
        //$this->setFilterCharacter('chartOfAccountTypeNote');
        $this->setFilterDate('executeTime');
        /**
         * All the $_POST Environment
         */
        if (isset($_POST ['chartOfAccountTypeId'])) {
            $this->setChartOfAccountTypeId($this->strict($_POST ['chartOfAccountTypeId'], 'integer'), 0, 'single');
        }
        ...
        if (isset($_GET ['chartOfAccountTypeId'])) {
            $this->setChartOfAccountTypeId($this->strict($_GET ['chartOfAccountTypeId'], 'integer'), 0, 'single');
        }
        ...
        if (isset($_GET ['chartOfAccountTypeId'])) {
            $this->setTotal(count($_GET ['chartOfAccountTypeId']));
            if (is_array($_GET ['chartOfAccountTypeId'])) {
                $this->chartOfAccountTypeId = array();
            }
        }
        ...
        for ($i = 0; $i < $this->getTotal(); $i++) {
            if (isset($_GET ['chartOfAccountTypeId'])) {
                $this->setChartOfAccountTypeId(
                        $this->strict($_GET ['chartOfAccountTypeId'] [$i], 'numeric'), $i, 'array'
                );
            }
            if (isset($_GET ['isDefault'])) {
            ...
            $primaryKeyAll .= $this->getChartOfAccountTypeId($i, 'array') . ",";
        }
     ...
     */
    public function getChartOfAccountTypeId($key, $type) {
        if ($type == 'single') {
            return $this->chartOfAccountTypeId;
        } else {
            if ($type == 'array') {
                return $this->chartOfAccountTypeId [$key];
            } else {
                echo json_encode(
                        array(
                            "success" => false,
                            "message" => "Cannot Identify Type String Or Array:getChartOfAccountTypeId ?"
                        )
                );
                exit();
            }
     ...
     * @param array|int $key List Of Primary Key.
     * @param array|int|string $type  List Of Type.0 As 'single' 1 As 'array'
     * @return \Core\Financial\GeneralLedger\ChartOfAccountType\Model\ChartOfAccountTypeModel
     */
    public function setChartOfAccountTypeId($value, $key, $type) {
        if ($type == 'single') {
            $this->chartOfAccountTypeId = $value;
            return $this;
        } else {
            if ($type == 'array') {
                $this->chartOfAccountTypeId[$key] = $value;
                return $this;
            } else {
                echo json_encode(
                        array(
                            "success" => false,
                            "message" => "Cannot Identify Type String Or Array:setChartOfAccountTypeId?"
                        )
                );
                exit();
            }
    ...
    /**
     * To Set Chart Of Account Category
