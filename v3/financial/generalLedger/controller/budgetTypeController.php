...
namespace Core\Financial\GeneralLedger\BudgetType\Controller;

use Core\ConfigClass;
use Core\Document\Trail\DocumentTrailClass;
use Core\Financial\GeneralLedger\BudgetType\Model\BudgetTypeModel;
use Core\Financial\GeneralLedger\BudgetType\Service\BudgetTypeService;
...
require_once($newFakeDocumentRoot . "v3/financial/generalLedger/model/budgetTypeModel.php");
require_once($newFakeDocumentRoot . "v3/financial/generalLedger/service/budgetTypeService.php");

/**
 * Class BudgetType
 * this is budgetType controller files.
 * @name IDCMS
 * @version 2
 * @author hafizan
 * @package  Core\Financial\GeneralLedger\BudgetType\Controller
...
class BudgetTypeClass extends ConfigClass {
    ...
    /**
     * Model
     * @var \Core\Financial\GeneralLedger\BudgetType\Model\BudgetTypeModel
     */
    public $model;
     ...
     * @var \Core\Financial\GeneralLedger\BudgetType\Service\BudgetTypeService
     */
    public $service;
    ...
    function __construct() {
        ...
        $this->setViewPath("./v3/financial/generalLedger/view/budgetType.php");
        $this->setControllerPath("./v3/financial/generalLedger/controller/budgetTypeController.php");
        $this->setServicePath("./v3/financial/generalLedger/service/budgetTypeService.php");
    }
    ...
    function execute() {
        ...
        $this->service = new BudgetTypeService();
    ...
    public function create() {
        ...
        if ($this->getVendor() == self::MYSQL) {
            ...
            (
                 ...
                 '" . $this->model->getBudgetTypeDescription() . "',
    ...
    public function read() {
        ...
        if (isset($_SESSION['isAdmin'])) {
            if ($_SESSION['isAdmin'] == 0) {
                if ($this->getVendor() == self::MYSQL) {
                    $this->setAuditFilter(
                            " `budgettype`.`isActive` = 1  AND `budgettype`.`companyId`='" . $this->getCompanyId() . "' "
                    );
                } else {
                    if ($this->getVendor() == self::MSSQL) {
                        $this->setAuditFilter(
                ...
                if ($_SESSION['isAdmin'] == 1) {
                    if ($this->getVendor() == self::MYSQL) {
                        $this->setAuditFilter("   `budgettype`.`companyId`='" . $this->getCompanyId() . "'	");
                    } else {
                        if ($this->getVendor() == self::MSSQL) {
                            $this->setAuditFilter("   [budgetType].[companyId]='" . $this->getCompanyId() . "' ");
                        } else {
       ...
       SELECT                    `budgettype`.`budgetTypeId`,
                    ...
                    `budgettype`.`companyId`,
                    `budgettype`.`budgetTypeCode`,
                    `budgettype`.`budgetTypeDescription`,
	...
	JOIN	`company`
	ON		`company`.`companyId` = `budgettype`.`companyId`
		  WHERE     " . $this->getAuditFilter();
            if ($this->model->getBudgetTypeId(0, 'single')) {
                $sql .= " AND `budgettype`.`" . $this->model->getPrimaryKeyName(
                        ) . "`='" . $this->model->getBudgetTypeId(0, 'single') . "'";
            }
        } else {
            if ($this->getVendor() == self::MSSQL) {
	...
	ON		[company].[companyId] =   [budgetType].[companyId]
		  WHERE     " . $this->getAuditFilter();
                ...
                if ($this->getVendor() == self::ORACLE) {
                    ...
                    $sql = "
		  SELECT                    BUDGETTYPE.BUDGETTYPEID AS \"budgetTypeId\",
                    COMPANY.COMPANYDESCRIPTION AS  \"companyDescription\",
                    BUDGETTYPE.COMPANYID AS \"companyId\",
                    BUDGETTYPE.BUDGETTYPECODE AS \"budgetTypeCode\",
                    BUDGETTYPE.BUDGETTYPEDESCRIPTION AS \"budgetTypeDescription\",
                    BUDGETTYPE.ISDEFAULT AS \"isDefault\",
                    BUDGETTYPE.ISNEW AS \"isNew\",
                    BUDGETTYPE.ISDRAFT AS \"isDraft\",
                    BUDGETTYPE.ISUPDATE AS \"isUpdate\",
                    BUDGETTYPE.ISDELETE AS \"isDelete\",
                    BUDGETTYPE.ISACTIVE AS \"isActive\",
                    BUDGETTYPE.ISAPPROVED AS \"isApproved\",
                    BUDGETTYPE.ISREVIEW AS \"isReview\",
                    BUDGETTYPE.ISPOST AS \"isPost\",
                    BUDGETTYPE.EXECUTEBY AS \"executeBy\",
                    BUDGETTYPE.EXECUTETIME AS \"executeTime\",
	...
	ON		COMPANY.COMPANYID = BUDGETTYPE.COMPANYID
        ...
        if ($this->getCharacterQuery()) {
            if ($this->getVendor() == self::MYSQL) {
                $sql .= " AND `budgettype`.`" . $this->model->getFilterCharacter(
                ...
                if ($this->getVendor() == self::MSSQL) {
                    ...
                    if ($this->getVendor() == self::ORACLE) {
                        $sql .= " AND Initcap(BUDGETTYPE." . strtoupper(
        ...
        if ($this->getDateRangeStartQuery()) {
            if ($this->getVendor() == self::MYSQL) {
                $sql .= $this->q->dateFilter(
                        'budgettype', $this->model->getFilterDate(), $this->getDateRangeStartQuery(), $this->getDateRangeEndQuery(), $this->getDateRangeTypeQuery(), $this->getDateRangeExtraTypeQuery()
                );
            } else {
                if ($this->getVendor() == self::MSSQL) {
                    $sql .= $this->q->dateFilter(
        ...
        $tableArray = null;
        if ($this->getVendor() == self::MYSQL) {
            $tableArray = array('staff', 'budgettype');
        } else {
            if ($this->getVendor() == self::MSSQL) {
                $tableArray = array('staff', 'budgettype');
            } else {
        ...
        while (($row = $this->q->fetchAssoc()) == true) {
            ...
            if ($this->model->getBudgetTypeId(0, 'single')) {
                ...
                $row['previousRecord'] = $this->previousRecord('value', $this->model->getBudgetTypeId(0, 'single'));
                $row['nextRecord'] = $this->nextRecord('value', $this->model->getBudgetTypeId(0, 'single'));
                            ...
                            array("[", "]"), "", json_encode(
                                    array(
                                        ...
                                        'previousRecord' => $this->previousRecord(
                                                'value', $this->model->getBudgetTypeId(0, 'single')
                                        ),
                                        'nextRecord' => $this->nextRecord('value', $this->model->getBudgetTypeId(0, 'single')),
    ...
    function update() {
        ...
        if ($this->getVendor() == self::MYSQL) {
           ...
           FROM 	`budgettype`
           WHERE  	`" . $this->model->getPrimaryKeyName() . "` = '" . $this->model->getBudgetTypeId(
                            0, 'single'
                    ) . "' ";
        } else {
            if ($this->getVendor() == self::MSSQL) {
           ...
           FROM 	BUDGETTYPE
           WHERE  	" . strtoupper($this->model->getPrimaryKeyName()) . " = '" . $this->model->getBudgetTypeId(
                                    0, 'single'
                            ) . "' ";
                }
            }
        ...
        if ($total == 0) {
            ...
            if ($this->getVendor() == self::MYSQL) {
                $sql = "
               UPDATE `budgettype` SET
                       `budgetTypeCode` = '" . $this->model->getBudgetTypeCode() . "',
                       `budgetTypeDescription` = '" . $this->model->getBudgetTypeDescription() . "',
...
/** 
    ...
    function duplicate() {
        ...
        if ($this->getVendor() == self::MYSQL) {
            $sql = "
           SELECT  `budgetTypeCode`
           FROM    `budgettype`
           WHERE   `budgetTypeCode` 	= 	'" . $this->model->getBudgetTypeCode() . "'
           AND     `isActive`  =   1
           AND     `companyId` =   '" . $this->getCompanyId() . "'";
        } else {
            if ($this->getVendor() == self::MSSQL) {
           ...
           AND     [companyId] =	'" . $this->getCompanyId() . "'";
                ...
                if ($this->getVendor() == self::ORACLE) {
                    $sql = "
               SELECT  BUDGETTYPECODE as \"budgetTypeCode\"
               FROM    BUDGETTYPE
               WHERE   BUDGETTYPECODE	= 	'" . $this->model->getBudgetTypeCode() . "'
               AND     ISACTIVE    =   1
               AND     COMPANYID   =   '" . $this->getCompanyId() . "'";
    ...
    function excel() {
        ...
        $this->excel->getActiveSheet()->setCellValue('C3', $this->translate['budgetTypeCodeLabel']);
        $this->excel->getActiveSheet()->setCellValue('D3', $this->translate['budgetTypeDescriptionLabel']);
        ...
        while (($row = $this->q->fetchAssoc()) == true) {
            ...
            $this->excel->getActiveSheet()->setCellValue('C' . $loopRow, strip_tags($row ['budgetTypeCode']));
            $this->excel->getActiveSheet()->setCellValue('D' . $loopRow, strip_tags($row ['budgetTypeDescription']));
        ...
        switch ($this->getReportMode()) {
            case 'excel':
                ...
                $extension = '.xlsx';
                $folder = 'excel';
                $filename = "budgetType" . rand(0, 10000000) . $extension;
...
if (isset($_POST ['method'])) {
    if (isset($_POST['output'])) {
        ...
        if ($_POST['securityToken'] != $budgetTypeObject->getSecurityToken()) {
            header('Content-Type:application/json; charset=utf-8');
        ...
        if (isset($_POST ['offset'])) {
            $budgetTypeObject->setStart($_POST ['offset']);
        }
        if (isset($_POST ['limit'])) {
            $budgetTypeObject->setLimit($_POST ['limit']);
        }
        $budgetTypeObject->setPageOutput($_POST['output']);
...
if (isset($_GET ['method'])) {
    $budgetTypeObject = new BudgetTypeClass ();
    if ($_GET['securityToken'] != $budgetTypeObject->getSecurityToken()) {
        header('Content-Type:application/json; charset=utf-8');
        echo json_encode(array("success" => false, "message" => "Something wrong with the system.Hola hackers"));
        exit();
    }
     ...
     */
    if (isset($_GET ['mode'])) {
        $budgetTypeObject->setReportMode($_GET['mode']);
    ...
    if (isset($_GET ['filter'])) {
        $budgetTypeObject->setServiceOutput('option');
    }
}
?>
