...
function checkDuplicate(leafId, page, securityToken) {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if (success === true) {
                if (total !== 0) {
                    $("#businessPartnerPriceListCode").val('').focus();
                    $("#businessPartnerPriceListCodeForm").removeClass().addClass("form-group has-error");
                ...
                $("#businessPartnerPriceListForm").removeClass().addClass("form-group has-error");
            }
            if ($infoPanel.is(':hidden')) {
                $infoPanel.show();
            }
    ...
    if (css.search('disabled') > 0) {
        ...
        if (type === 1) {
            ...
            if ($businessPartnerPriceListAmount.val().length === 0) {
                $('#businessPartnerPriceListAmountHelpMe').html('').empty().html("<span class='label label-danger'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['businessPartnerPriceListAmountLabel'] + " </span>");
                $('#businessPartnerPriceListAmountForm').removeClass().addClass('col-xs-6 col-sm-6 col-md-6 form-group has-error');
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (success === true) {
                        ...
                        $('#businessPartnerPriceListProductCodeHelpMe').html('').empty();
                        $businessPartnerPriceListValidStartDate.val('');
                        $('#businessPartnerPriceListValidStartDateHelpMe').html('').empty();
                        $businessPartnerPriceListValidEndDate.val('');
                        $('#businessPartnerPriceListValidEndDateHelpMe').html('').empty();
                        $businessPartnerPriceListDescription.val('');
                        $('#businessPartnerPriceListDescriptionHelpMe').html('').empty();
                        $businessPartnerPriceListAmount.val('');
                        $('#businessPartnerPriceListAmountHelpMe').html('').empty();
                    } else if (success === false) {
                        $infoPanel.html('').empty().html("<span class='label label-danger'>&nbsp;" + message + "</span>");
                        if ($infoPanel.is(':hidden')) {
                            $infoPanel.show();
...
function resetRecord(leafId, url, urlList, securityToken, createAccess, updateAccess, deleteAccess) {
    ...
    $("#businessPartnerPriceListIdHelpMe").empty().html('');
    $("#businessPartnerId").val('');
    ...
    $('#productId').trigger("chosen:updated");
    $("#businessPartnerPriceListProductCode").val('');
    $("#businessPartnerPriceListProductCodeHelpMe").empty().html('');
    $("#businessPartnerPriceListValidStartDate").val('');
    $("#businessPartnerPriceListValidStartDateHelpMe").empty().html('');
    $("#businessPartnerPriceListValidEndDate").val('');
    $("#businessPartnerPriceListValidEndDateHelpMe").empty().html('');
    $("#businessPartnerPriceListDescription").val('');
    $("#businessPartnerPriceListDescriptionHelpMe").empty().html('');
    $("#businessPartnerPriceListAmount").val('');
    $("#businessPartnerPriceListAmountHelpMe").empty().html('');
}
function postRecord() {
