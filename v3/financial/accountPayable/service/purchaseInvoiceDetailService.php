...
namespace Core\Financial\AccountPayable\PurchaseInvoiceDetail\Service;
 ...
 * @version 2
 * @author hafizan
 * @package Core\Financial\AccountPayable\PurchaseInvoiceDetail\Service
 * @subpackage AccountPayable
 * @link http://www.hafizan.com
 * @license http://www.gnu.org/copyleft/lesser.html LGPL
 */
class PurchaseInvoiceDetailService extends ConfigClass {
    ...
    /**
     * Return PurchaseInvoiceProject Default Value
     * @return int
     */
   ...
   /**
     ...
     * @param int $purchaseInvoiceId Main Table
     * @param string $type 1->debit,2->credit
     * @return double $total
     */
    public function getTotalPurchaseInvoiceAmount($purchaseInvoiceId, $type) {
        $total = 0;
        $sql = null;
        if ($this->getVendor() == self::MYSQL) {
            $sql = "
            SELECT      SUM(`purchaseInvoiceDetailAmount`) AS `total`
            FROM        `puchaseinvoicedetail`

            WHERE       `puchaseinvoicedetail`.`companyId`                         =   '" . $this->getCompanyId() . "'
            AND         `puchaseinvoicedetail`.`purchaseInvoiceId`                 IN   (" . $purchaseInvoiceId . ")
            AND         `puchaseinvoicedetail`.`isActive`                          =   1";
            if ($type == 1) {
            ...
            // $sql .= "
            //  GROUP BY    `puchaseinvoicedetail`.`purchaseInvoiceDetailAmount`
            //  ";
        } else if ($this->getVendor() == self::MSSQL) {
            $sql .= "
            ...
            AND         [purchaseInvoiceDetail].[purchaseInvoiceId]                         IN   (" . $purchaseInvoiceId . ")
            AND         [purchaseInvoiceDetail].[isActive]                          =   1";
            if ($type == 1) {
                $sql .= "  AND [purchaseInvoiceDetailAmount] >0 ";
            } else {
                $sql .= "  AND [purchaseInvoiceDetailAmount] < 0 ";
            }
            $sql .= "
            GROUP BY    [purchaseInvoiceDetail].[purchaseInvoiceDetailAmount]";
        } else if ($this->getVendor() == self::ORACLE) {
            $sql = "
            SELECT      SUM(PURCHASEINVOICEDETAILAMOUNT) AS \"total\"
            FROM        PURCHASEINVOICEDETAIL
     ...
     * @return array
     */
    public function getTotalPurchaseInvoiceDetail($purchaseInvoiceId) {
        header('Content-Type:application/json; charset=utf-8');
        $totalDebit = $this->getTotalPurchaseInvoiceAmount($purchaseInvoiceId, 1);
        $totalCredit = $this->getTotalPurchaseInvoiceAmount($purchaseInvoiceId, 2);
        $trialBalance = $this->getCheckTrialBalance($purchaseInvoiceId);
        return array(
            "success" => true,
     ...
     */
    private function getCheckTrialBalance($purchaseInvoiceId) {
        $trialBalance = 0;
        $sql = null;
        ...
        if ($this->getVendor() == self::MYSQL) {
            ...
            SELECT (
                (
                    SELECT      SUM(`purchaseInvoiceDetailAmount`)
                    FROM        `purchaseinvoicedetail`
                    ...
                    AND         `purchaseinvoicedetail`.`paymentVoucherId`                         	IN   (" . $purchaseInvoiceId . ")
                    AND         `purchaseinvoicedetail`.`isActive`                          	=   1
            ...
            SELECT (
                (
                    ...
                    WHERE       PURCHASEINVOICEDETAIL.COMPANYID                    	=   '" . $this->getCompanyId() . "'
                    AND         PURCHASEINVOICEDETAIL.JOURNALID                     	IN   (" . $purchaseInvoiceId . ")
                    AND         PURCHASEINVOICEDETAIL.ISACTIVE                      	=   1
                    AND         CHARTOFACCOUNTCATEGORY.CHARTOFACCOUNTCATEGORYCODE 	=   '" . self::INCOME . "'
                    GROUP BY    PURCHASEINVOICEDETAIL.PURCHASEINVOICEDETAILAMOUNT
                )
                -
                 ...
                 (
                    ...
                    AND         PURCHASEINVOICEDETAIL.ISACTIVE                     	=   1
                    AND         CHARTOFACCOUNTCATEGORY.CHARTOFACCOUNTCATEGORYCODE	=	'" . self::EXPENSES . "'
                    GROUP BY    PURCHASEINVOICEDETAIL.PAYMENTVOUCHERDETAILAMOUNT
                )
            ) as [trialBalance]
