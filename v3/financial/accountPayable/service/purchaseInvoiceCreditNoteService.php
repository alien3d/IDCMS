...
namespace Core\Financial\AccountPayable\PurchaseInvoiceCreditNote\Service;
...
/**
 * Class PurchaseInvoiceCreditNoteService
 * Contain extra processing function / method.
 * @name IDCMS
 * @version 2
 * @author hafizan
 * @package Core\Financial\AccountPayable\PurchaseInvoiceCreditNote\Service
 * @subpackage AccountPayable
 * @link http://www.hafizan.com
 * @link http://en.wikipedia.org/wiki/Credit_note
 * @license http://www.gnu.org/copyleft/lesser.html LGPL
 */
class PurchaseInvoiceCreditNoteService extends ConfigClass {
    ...
    /**
     * Upload Purchase Invoice Credit Note Attachment
     * @var int
     */
    private $sizeLimit;
     ...
     * @param int $purchaseInvoiceCreditNoteId Purchase Invoice Credit Note Primary Key
     * @param int $leafId Leaf Primary key
     * @param string $leafName Leaf Name
     */
    public function setPosting($purchaseInvoiceCreditNoteId, $leafId, $leafName) {
        $sql = null;
        if ($this->getVendor() == self::MYSQL) {
            $sql = "
            SELECT  *
            FROM    `purchaseinvoicecreditnote`
            WHERE   `purchaseInvoiceCreditNoteId` IN (" . $purchaseInvoiceCreditNoteId . ")
            AND     `isPost`    =   0
            AND     `companyId` =   '" . $this->getCompanyId() . "'";
        } else if ($this->getVendor() == self::MSSQL) {
            $sql = "
        ...
        if ($result) {
            while (($row = $this->q->fetchArray($result)) == true) {
                ...
                $this->setPurchaseInvoiceStatusTracking(
                        $purchaseInvoiceCreditNoteId, $purchaseInvoiceId, $this->getPurchaseInvoiceStatusId(self::TRANSFER_TO_GENERAL_LEDGER)
                );
            }
        }
        ...
        if ($this->getVendor() == self::MYSQL) {
            ...
            USING   (`companyId`,`purchaseInvoiceCreditNoteId`)
            WHERE   `purchaseinvoicecreditnote`.`purchaseInvoiceCreditNoteId` IN (" . $purchaseInvoiceCreditNoteId . ")
            ORDER BY `purchaseInvoiceCreditNoteId";
        } else if ($this->getVendor() == self::MSSQL) {
            $sql = "
            SELECT  *
            FROM    [purchaseInvoiceDetail]
            JOIN    [purchaseInvoiceCreditNote]
            ON      [purchaseInvoiceDetail].[companyId]         =   [purchaseInvoiceCreditNote].[companyId]
            AND     [purchaseInvoiceDetail].[purchaseInvoiceCreditNoteId] =   [purchaseInvoiceCreditNote].[purchaseInvoiceCreditNoteId]
            WHERE   [purchaseInvoiceCreditNoteId] IN (" . $purchaseInvoiceCreditNoteId . ")";
        } else if ($this->getVendor() == self::ORACLE) {
            $sql = "
            SELECT  *
            FROM    PURCHASEINVOICECREDITNOTENDETAIL
            JOIN    PURCHASEINVOICECREDITNOTE
            ON      PURCHASEINVOICECREDITNOTEDETAIL.COMPANYID         =   PURCHASEINVOICECREDITNOTE.COMPANYID
            AND     PURCHASEINVOICECREDITNOTEDETAIL.PURCHASEINVOICECREDITNOTEID =   PURCHASEINVOICECREDITNOTE.PURCHASEINVOICECREDITNOTEID
            WHERE   PURCHASEINVOICECREDITNOTE.PURCHASEINVOICECREDITNOTEID IN (" . $purchaseInvoiceCreditNoteId . ")";
        }
        try {
            $result = $this->q->fast($sql);
        ...
        if ($result) {
            while (($row = $this->q->fetchArray($result)) == true) {
                ...
                $documentDate = $row['purchaseInvoiceCreditNoteDate'];
                $localAmount = $row['purchaseInvoiceCreditNoteDetailAmount'];
                $description = $row['purchaseInvoiceCreditNoteDescription'];
                $module = 'AP';
                $tableName = 'purchaseInvoiceCreditNote';
                $tableNameDetail = 'purchaseInvoiceCreditNoteDetail';
                $tableNameId = 'purchaseInvoiceCreditNoteId';
                $tableNameDetailId = 'purchaseInvoiceCreditNoteDetailId';
                $referenceTableNameId = $row['purchaseInvoiceCreditNoteId'];
                $referenceTableNameDetailId = $row['purchaseInvoiceCreditNoteDetailId'];
                $purchaseInvoiceId = $row['purchaseInvoiceId'];
                $this->ledgerService->setPurchaseInvoiceLedger(
                    $businessPartnerId, $chartOfAccountId,$documentNumber, $documentDate, $localAmount, $description, $leafId, $purchaseInvoiceCreditNoteId, $purchaseInvoiceId, $purchaseInvoiceCreditNoteLedgerId = null
                );
        ...
        $this->setPurchaseInvoiceCreditNotePosted($purchaseInvoiceCreditNoteId);
    }

    /**
     * Set Purchase Invoice Tracking
     * @param int $purchaseInvoiceCreditNoteId Credit Note Primary Key
     * @param int $purchaseInvoiceId Purchase Invoice Primary Key
     * @param int $purchaseInvoiceStatusId Status
     */
    public function setPurchaseInvoiceStatusTracking($purchaseInvoiceCreditNoteId, $purchaseInvoiceId, $purchaseInvoiceStatusId) {
        $sql = null;
        ...
        if ($this->getVendor() == self::MYSQL) {
            ...
            FROM   `purchaseinvoicecreditnote`
            WHERE  `collectionId` ='" . $purchaseInvoiceCreditNoteId . "'
            ";
        } else if ($this->getVendor() == self::MSSQL) {
            $sql = "
            ...
            FROM   [purchaseInvoiceCreditNote]
            WHERE  [purchaseInvoiceCreditNoteId] ='" . $purchaseInvoiceCreditNoteId . "'
            ";
        } else if ($this->getVendor() == self::ORACLE) {
            $sql = "
     ...
     * @param string $purchaseInvoiceStatusCode Code
     * @return int
     */
    ...
    /**
     * Update Purchase Invoice Credit Note Posted Flag
     * @param int $purchaseInvoiceCreditNoteId Purchase Invoice Credit Note Primary Key
     * @return void
     */
    private function setPurchaseInvoiceCreditNotePosted($purchaseInvoiceCreditNoteId) {
        $sql = null;
        if ($this->getVendor() == self::MYSQL) {
            $sql = "
    ...
    /**
     * Upload Purchase Invoice Credit Note Attachment before submitting the form.
     * @throws \Exception
     * @return void
     */
...
// to pass data through iframe you will need to encode all html tags
     ...
     * @throws \Exception
     */
    function transferAttachment($staffId, $purchaseInvoiceCreditNoteId) {
        $sql = null;
        if ($this->getVendor() == self::MYSQL) {
            $sql = "
			SELECT      * 
			FROM        `purchaseinvoicecreditnotetemp` 
			WHERE       `isNew`=1
			AND         `staffId`='" . $staffId . "'
        ...
        if ($result) {
            ...
            while (($row = $this->q->fetchArray($result)) == TRUE) {
                ...
                $documentPath = $this->getFakeDocumentRoot() . "v3/financial/accountPayable/attachment/" . $row['purchaseInvoiceCreditNoteTempName'];
                //  insert into document attachment/
                if ($this->getVendor() == self::MYSQL) {
                    $sql = "
				INSERT INTO `document`(
									...
									'" . $this->getLeafId() . "',
									'" . $row['purchaseInvoiceCreditNoteTempName'] . "',
									'" . $row['purchaseInvoiceCreditNoteTempName'] . "',
                ...
                if ($this->getVendor() == self::MYSQL) {
                    $sql = "
				INSERT INTO `purchaseinvoicecreditnoteattachment`(
                    ...
                    $sql = "
				INSERT INTO [purchaseInvoiceCreditNoteAttachment](
									[purchaseInvoiceCreditNoteAttachmentId],
									[companyId], 
									[purchaseInvoiceCreditNoteId], 
        ...
        if ($this->getVendor() == self::MYSQL) {
            $sql = "
			UPDATE `purchaseinvoicecreditnotetemp`
			SET    `isNew`    = '0'
