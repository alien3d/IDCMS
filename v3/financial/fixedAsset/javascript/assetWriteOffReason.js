function checkDuplicate(leafId, page, securityToken) {
    var $assetWriteOffReasonCode = $("#assetWriteOffReasonCode");
    if ($assetWriteOffReasonCode.val().length === 0) {
        alert(t['oddTextLabel']);
        return false;
    }
    $.ajax({type: 'GET', url: page, data: {assetWriteOffReasonCode: $assetWriteOffReasonCode.val(), method: 'duplicate', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
            ...
            if (success === true) {
                if (total !== 0) {
                    $("#assetWriteOffReasonCode").val('').focus();
                    $("#assetWriteOffReasonCodeForm").removeClass().addClass("form-group has-error");
                ...
                $("#assetWriteOffReasonForm").removeClass().addClass("form-group has-error");
            }
            if ($infoPanel.is(':hidden')) {
                $infoPanel.show();
            }
...
function showFormUpdate(leafId, url, urlList, securityToken, assetWriteOffReasonId, updateAccess, deleteAccess) {
    sleep(500);
    $('a[rel=tooltip]').tooltip('hide');
    $.ajax({type: 'POST', url: urlList, data: {method: 'read', type: 'form', assetWriteOffReasonId: assetWriteOffReasonId, securityToken: securityToken, leafId: leafId}, beforeSend: function() {
            var smileyRoll = './images/icons/smiley-roll.png';
            var $infoPanel = $('#infoPanel');
            $infoPanel.html('').empty().html("<span class='label label-warning'><img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
            if ($infoPanel.is(':hidden')) {
...
function showModalDelete(assetWriteOffReasonId, assetWriteOffReasonCode, assetWriteOffReasonDescription) {
    $('#assetWriteOffReasonIdPreview').val('').val(decodeURIComponent(assetWriteOffReasonId));
    $('#assetWriteOffReasonCodePreview').val('').val(decodeURIComponent(assetWriteOffReasonCode));
    $('#assetWriteOffReasonDescriptionPreview').val('').val(decodeURIComponent(assetWriteOffReasonDescription));
    showMeModal('deletePreview', 1);
}
function deleteGridRecord(leafId, url, urlList, securityToken) {
    $.ajax({type: 'POST', url: url, data: {method: 'delete', output: 'json', assetWriteOffReasonId: $('#assetWriteOffReasonIdPreview').val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
            var smileyRoll = './images/icons/smiley-roll.png';
            var $infoPanel = $('#infoPanel');
            $infoPanel.html('').empty().html("<span class='label label-warning'><img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
            if ($infoPanel.is(':hidden')) {
...
function deleteGridRecordCheckbox(leafId, url, urlList, securityToken) {
    var stringText = '';
    var counter = 0;
    $('input:checkbox[name="assetWriteOffReasonId[]"]').each(function() {
        stringText = stringText + "&assetWriteOffReasonId[]=" + $(this).val();
    });
...
function newRecord(leafId, url, urlList, securityToken, type, createAccess, updateAccess, deleteAccess) {
    ...
    var $assetWriteOffReasonCode = $('#assetWriteOffReasonCode');
    var $assetWriteOffReasonDescription = $('#assetWriteOffReasonDescription');
    if (css.search('disabled') > 0) {
        return false;
    } else {
        if (type === 1) {
            if ($assetWriteOffReasonCode.val().length === 0) {
                $('#assetWriteOffReasonCodeHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['assetWriteOffReasonCodeLabel'] + " </span>");
                $('#assetWriteOffReasonCodeForm').removeClass().addClass('form-group has-error');
                $assetWriteOffReasonCode.focus();
                return false;
            }
            if ($assetWriteOffReasonDescription.val().length === 0) {
                $('#assetWriteOffReasonDescriptionHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['assetWriteOffReasonDescriptionLabel'] + " </span>");
                $('#assetWriteOffReasonDescriptionForm').removeClass().addClass('form-group has-error');
                $assetWriteOffReasonDescription.focus();
                return false;
            }
            $.ajax({type: 'POST', url: url, data: {method: 'create', output: 'json', assetWriteOffReasonCode: $assetWriteOffReasonCode.val(), assetWriteOffReasonDescription: $assetWriteOffReasonDescription.val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    var smileyRoll = './images/icons/smiley-roll.png';
                    var $infoPanel = $('#infoPanel');
                    $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                    if ($infoPanel.is(':hidden')) {
                    ...
                    if (success === true) {
                        ...
                        $assetWriteOffReasonCode.val('');
                        $('#assetWriteOffReasonCodeHelpMe').html('').empty();
                        $assetWriteOffReasonDescription.val('');
                        $('#assetWriteOffReasonDescriptionForm').removeClass().addClass('form-group');
                        $('#assetWriteOffReasonDescriptionHelpMe').html('').empty();
                    } else if (success === false) {
                        $infoPanel.html('').empty().html("<span class='label label-important'>&nbsp;" + message + "</span>");
                        if ($infoPanel.is(':hidden')) {
                            $infoPanel.show();
            ...
            $.ajax({type: 'POST', url: url, data: {method: 'create', output: 'json', assetWriteOffReasonCode: $assetWriteOffReasonCode.val(), assetWriteOffReasonDescription: $assetWriteOffReasonDescription.val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    ...
                    if (success === true) {
                        ...
                        $('#assetWriteOffReasonId').val(data.assetWriteOffReasonId);
                        $('#newRecordButton1').removeClass().addClass('btn btn-success disabled').attr('onClick', '');
                        $('#newRecordButton2').removeClass().addClass('btn dropdown-toggle btn-success disabled');
                        $('#newRecordButton3').attr('onClick', '');
                        $('#newRecordButton4').attr('onClick', '');
...
function updateRecord(leafId, url, urlList, securityToken, type, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        if (type === 1) {
            ...
            $infoPanel.html('').empty();
            $.ajax({type: 'POST', url: url, data: {method: 'save', output: 'json', assetWriteOffReasonId: $assetWriteOffReasonId.val(), assetWriteOffReasonCode: $assetWriteOffReasonCode.val(), assetWriteOffReasonDescription: $assetWriteOffReasonDescription.val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    var smileyRoll = './images/icons/smiley-roll.png';
                    var $infoPanel = $('#infoPanel');
                    $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                    if ($infoPanel.is(':hidden')) {
    ...
    if (css.search('disabled') > 0) {
        ...
        if (deleteAccess === 1) {
            if (confirm(decodeURIComponent(t['deleteRecordMessageLabel']))) {
                var value = $assetWriteOffReasonId.val();
                if (!value) {
                    ...
                    $.ajax({type: 'POST', url: url, data: {method: 'delete', output: 'json', assetWriteOffReasonId: $assetWriteOffReasonId.val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                            var smileyRoll = './images/icons/smiley-roll.png';
                            var $infoPanel = $('#infoPanel');
                            $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                            if ($infoPanel.is(':hidden')) {
...
function resetRecord(leafId, url, urlList, securityToken, createAccess, updateAccess, deleteAccess) {
    ...
    $("#assetWriteOffReasonCode").val('');
    $("#assetWriteOffReasonCodeHelpMe").empty().html('');
    $("#assetWriteOffReasonDescription").val('');
    $("#assetWriteOffReasonDescriptionHelpMe").empty().html('');
    $('#assetWriteOffReasonDescription').empty().val('');
}
function postRecord() {
...
function firstRecord(leafId, url, urlList, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        $.ajax({type: 'GET', url: url, data: {method: 'dataNavigationRequest', dataNavigation: 'firstRecord', output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                ...
                if (success === true) {
                    $.ajax({type: 'POST', url: url, data: {method: 'read', assetWriteOffReasonId: firstRecord, output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                            var smileyRoll = './images/icons/smiley-roll.png';
                            $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                            if ($infoPanel.is(':hidden')) {
                                $infoPanel.show();
                            ...
                            if (success === true) {
                                $('#assetWriteOffReasonId').val(data.data.assetWriteOffReasonId);
                                $('#assetWriteOffReasonCode').val(data.data.assetWriteOffReasonCode);
                                $('#assetWriteOffReasonDescription').val(data.data.assetWriteOffReasonDescription);
                                if (nextRecord > 0) {
                                    $('#previousRecordButton').removeClass().addClass('btn btn-default disabled').attr('onClick', '');
...
function endRecord(leafId, url, urlList, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        $.ajax({type: 'GET', url: url, data: {method: 'dataNavigationRequest', dataNavigation: 'lastRecord', output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                ...
                if (success === true) {
                    $.ajax({type: 'POST', url: url, data: {method: 'read', assetWriteOffReasonId: lastRecord, output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                            var smileyRoll = './images/icons/smiley-roll.png';
                            $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                            if ($infoPanel.is(':hidden')) {
                                $infoPanel.show();
...
function previousRecord(leafId, url, urlList, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($previousRecordCounter.val()) > 0 && parseFloat($previousRecordCounter.val()) < parseFloat($('#lastRecordCounter').val())) {
            $.ajax({type: 'POST', url: url, data: {method: 'read', assetWriteOffReasonId: $previousRecordCounter.val(), output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    var smileyRoll = './images/icons/smiley-roll.png';
                    var $infoPanel = $('#infoPanel');
                    $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                    if ($infoPanel.is(':hidden')) {
...
function nextRecord(leafId, url, urlList, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($nextRecordCounter.val()) <= parseFloat($('#lastRecordCounter').val())) {
            $.ajax({type: 'POST', url: url, data: {method: 'read', assetWriteOffReasonId: $nextRecordCounter.val(), output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    var smileyRoll = './images/icons/smiley-roll.png';
                    var $infoPanel = $('#infoPanel');
                    $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                    if ($infoPanel.is(':hidden')) {
