...
function checkDuplicate(leafId, page, securityToken) {
    ...
    if ($itemTypeCode.val().length === 0) {
        alert(t['oddTextLabel']);
        return false;
    }
    $.ajax({type: 'GET', url: page, data: {itemTypeCode: $itemTypeCode.val(), method: 'duplicate', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
            var smileyRoll = './images/icons/smiley-roll.png';
            var $infoPanel = $('#infoPanel');
            $infoPanel.html('').empty().html("<span class='label label-warning'><img src='" + smileyRoll + "'>&nbsp;" + decodeURIComponent(t['loadingTextLabel']) + "....</span>");
            if ($infoPanel.is(':hidden')) {
            ...
            if (success === true) {
                if (total !== 0) {
                    $("#itemTypeCode").val('').focus();
                    $("#itemTypeCodeForm").removeClass().addClass("form-group has-error");
                ...
                $("#itemTypeForm").removeClass().addClass("form-group has-error");
            }
            if ($infoPanel.is(':hidden')) {
                $infoPanel.show();
            }
...
function showFormUpdate(leafId, url, urlList, securityToken, itemTypeId, updateAccess, deleteAccess) {
    sleep(500);
    $('a[rel=tooltip]').tooltip('hide');
    $.ajax({type: 'POST', url: urlList, data: {method: 'read', type: 'form', itemTypeId: itemTypeId, securityToken: securityToken, leafId: leafId}, beforeSend: function() {
            var smileyRoll = './images/icons/smiley-roll.png';
            var $infoPanel = $('#infoPanel');
            $infoPanel.html('').empty().html("<span class='label label-warning'><img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
            if ($infoPanel.is(':hidden')) {
...
function showModalDelete(itemTypeId, itemCategoryId, itemTypeCode, itemTypeDepreciationRate, itemTypeLife, itemTypeMinimumReOrder, itemTypeDescription, isDepreciate, isFixedAsset, isInventory, isSalesItem) {
    $('#itemTypeIdPreview').val('').val(decodeURIComponent(itemTypeId));
    $('#itemCategoryIdPreview').val('').val(decodeURIComponent(itemCategoryId));
    $('#itemTypeCodePreview').val('').val(decodeURIComponent(itemTypeCode));
    $('#itemTypeDepreciationRatePreview').val('').val(decodeURIComponent(itemTypeDepreciationRate));
    $('#itemTypeLifePreview').val('').val(decodeURIComponent(itemTypeLife));
    $('#itemTypeMinimumReOrderPreview').val('').val(decodeURIComponent(itemTypeMinimumReOrder));
    $('#itemTypeDescriptionPreview').val('').val(decodeURIComponent(itemTypeDescription));
    $('#isDepreciatePreview').val('').val(decodeURIComponent(isDepreciate));
    $('#isFixedAssetPreview').val('').val(decodeURIComponent(isFixedAsset));
    $('#isInventoryPreview').val('').val(decodeURIComponent(isInventory));
    $('#isSalesItemPreview').val('').val(decodeURIComponent(isSalesItem));
    showMeModal('deletePreview', 1);
}
function deleteGridRecord(leafId, url, urlList, securityToken) {
    $.ajax({type: 'POST', url: url, data: {method: 'delete', output: 'json', itemTypeId: $('#itemTypeIdPreview').val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
            var smileyRoll = './images/icons/smiley-roll.png';
            var $infoPanel = $('#infoPanel');
            $infoPanel.html('').empty().html("<span class='label label-warning'><img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
            if ($infoPanel.is(':hidden')) {
...
function deleteGridRecordCheckbox(leafId, url, urlList, securityToken) {
    var stringText = '';
    var counter = 0;
    $('input:checkbox[name="itemTypeId[]"]').each(function() {
        stringText = stringText + "&itemTypeId[]=" + $(this).val();
    });
...
function newRecord(leafId, url, urlList, securityToken, type, createAccess, updateAccess, deleteAccess) {
    ...
    var $itemTypeCode = $('#itemTypeCode');
    var $itemTypeDepreciationRate = $('#itemTypeDepreciationRate');
    var $itemTypeLife = $('#itemTypeLife');
    var $itemTypeMinimumReOrder = $('#itemTypeMinimumReOrder');
    var $itemTypeDescription = $('#itemTypeDescription');
    var $isDepreciate = $('#isDepreciate');
    var $isFixedAsset = $('#isFixedAsset');
    var $isInventory = $('#isInventory');
    var $isSalesItem = $('#isSalesItem');
    if (css.search('disabled') > 0) {
        return false;
    } else {
        if (type === 1) {
            ...
            if ($itemTypeCode.val().length === 0) {
                $('#itemTypeCodeHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['itemTypeCodeLabel'] + " </span>");
                $('#itemTypeCodeForm').removeClass().addClass('form-group has-error');
                $itemTypeCode.focus();
                return false;
            }
            if ($itemTypeDepreciationRate.val().length === 0) {
                $('#itemTypeDepreciationRateHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['itemTypeDepreciationRateLabel'] + " </span>");
                $('#itemTypeDepreciationRateForm').removeClass().addClass('form-group has-error');
                $itemTypeDepreciationRate.focus();
                return false;
            }
            if ($itemTypeLife.val().length === 0) {
                $('#itemTypeLifeHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['itemTypeLifeLabel'] + " </span>");
                $('#itemTypeLifeForm').removeClass().addClass('form-group has-error');
                $itemTypeLife.focus();
                return false;
            }
            if ($itemTypeMinimumReOrder.val().length === 0) {
                $('#itemTypeMinimumReOrderHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['itemTypeMinimumReOrderLabel'] + " </span>");
                $('#itemTypeMinimumReOrderForm').removeClass().addClass('form-group has-error');
                $itemTypeMinimumReOrder.focus();
                return false;
            }
            if ($itemTypeDescription.val().length === 0) {
                $('#itemTypeDescriptionHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['itemTypeDescriptionLabel'] + " </span>");
                $('#itemTypeDescriptionForm').removeClass().addClass('form-group has-error');
                $itemTypeDescription.focus();
                return false;
            }
            ...
            if ($isFixedAsset.val().length === 0) {
                $('#isFixedAssetHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['isFixedAssetLabel'] + " </span>");
                $('#isFixedAssetForm').removeClass().addClass('form-group has-error');
                $isFixedAsset.focus();
                return false;
            }
            if ($isInventory.val().length === 0) {
                $('#isInventoryHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['isInventoryLabel'] + " </span>");
                $('#isInventoryForm').removeClass().addClass('form-group has-error');
                $isInventory.focus();
                return false;
            }
            if ($isSalesItem.val().length === 0) {
                $('#isSalesItemHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['isSalesItemLabel'] + " </span>");
                $('#isSalesItemForm').removeClass().addClass('form-group has-error');
                $isSalesItem.focus();
                return false;
            }
            $.ajax({type: 'POST', url: url, data: {method: 'create', output: 'json', itemCategoryId: $itemCategoryId.val(), itemTypeCode: $itemTypeCode.val(), itemTypeDepreciationRate: $itemTypeDepreciationRate.val(), itemTypeLife: $itemTypeLife.val(), itemTypeMinimumReOrder: $itemTypeMinimumReOrder.val(), itemTypeDescription: $itemTypeDescription.val(), isDepreciate: $isDepreciate.val(), isFixedAsset: $isFixedAsset.val(), isInventory: $isInventory.val(), isSalesItem: $isSalesItem.val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    var smileyRoll = './images/icons/smiley-roll.png';
                    var $infoPanel = $('#infoPanel');
                    $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                    if ($infoPanel.is(':hidden')) {
                    ...
                    if (success === true) {
                        ...
                        $itemTypeCode.val('');
                        $('#itemTypeCodeHelpMe').html('').empty();
                        $itemTypeDepreciationRate.val('');
                        $('#itemTypeDepreciationRateHelpMe').html('').empty();
                        $itemTypeLife.val('');
                        $('#itemTypeLifeHelpMe').html('').empty();
                        $itemTypeMinimumReOrder.val('');
                        $('#itemTypeMinimumReOrderHelpMe').html('').empty();
                        $itemTypeDescription.val('');
                        $('#itemTypeDescriptionForm').removeClass().addClass('form-group');
                        $('#itemTypeDescriptionHelpMe').html('').empty();
                        $isDepreciate.val('');
                        $('#isDepreciateHelpMe').html('').empty();
                        $isFixedAsset.val('');
                        $('#isFixedAssetHelpMe').html('').empty();
                        $isInventory.val('');
                        $('#isInventoryHelpMe').html('').empty();
                        $isSalesItem.val('');
                        $('#isSalesItemHelpMe').html('').empty();
                    } else if (success === false) {
                        $infoPanel.html('').empty().html("<span class='label label-important'>&nbsp;" + message + "</span>");
                        if ($infoPanel.is(':hidden')) {
                            $infoPanel.show();
            ...
            $.ajax({type: 'POST', url: url, data: {method: 'create', output: 'json', itemCategoryId: $itemCategoryId.val(), itemTypeCode: $itemTypeCode.val(), itemTypeDepreciationRate: $itemTypeDepreciationRate.val(), itemTypeLife: $itemTypeLife.val(), itemTypeMinimumReOrder: $itemTypeMinimumReOrder.val(), itemTypeDescription: $itemTypeDescription.val(), isDepreciate: $isDepreciate.val(), isFixedAsset: $isFixedAsset.val(), isInventory: $isInventory.val(), isSalesItem: $isSalesItem.val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    ...
                    if (success === true) {
                        ...
                        $('#itemTypeId').val(data.itemTypeId);
                        $('#newRecordButton1').removeClass().addClass('btn btn-success disabled').attr('onClick', '');
                        $('#newRecordButton2').removeClass().addClass('btn dropdown-toggle btn-success disabled');
                        $('#newRecordButton3').attr('onClick', '');
                        $('#newRecordButton4').attr('onClick', '');
...
function updateRecord(leafId, url, urlList, securityToken, type, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        if (type === 1) {
            ...
            $infoPanel.html('').empty();
            $.ajax({type: 'POST', url: url, data: {method: 'save', output: 'json', itemTypeId: $itemTypeId.val(), itemCategoryId: $itemCategoryId.val(), itemTypeCode: $itemTypeCode.val(), itemTypeDepreciationRate: $itemTypeDepreciationRate.val(), itemTypeLife: $itemTypeLife.val(), itemTypeMinimumReOrder: $itemTypeMinimumReOrder.val(), itemTypeDescription: $itemTypeDescription.val(), isDepreciate: $isDepreciate.val(), isFixedAsset: $isFixedAsset.val(), isInventory: $isInventory.val(), isSalesItem: $isSalesItem.val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    var smileyRoll = './images/icons/smiley-roll.png';
                    var $infoPanel = $('#infoPanel');
                    $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                    if ($infoPanel.is(':hidden')) {
        ...
        if (deleteAccess === 1) {
            if (confirm(decodeURIComponent(t['deleteRecordMessageLabel']))) {
                ...
                if (!value) {
                    ...
                    $.ajax({type: 'POST', url: url, data: {method: 'delete', output: 'json', itemTypeId: $itemTypeId.val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                            var smileyRoll = './images/icons/smiley-roll.png';
                            var $infoPanel = $('#infoPanel');
                            $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                            if ($infoPanel.is(':hidden')) {
...
function resetRecord(leafId, url, urlList, securityToken, createAccess, updateAccess, deleteAccess) {
    ...
    $('#itemCategoryId').trigger("chosen:updated");
    $("#itemTypeCode").val('');
    $("#itemTypeCodeHelpMe").empty().html('');
    $("#itemTypeDepreciationRate").val('');
    $("#itemTypeDepreciationRateHelpMe").empty().html('');
    $("#itemTypeLife").val('');
    $("#itemTypeLifeHelpMe").empty().html('');
    $("#itemTypeMinimumReOrder").val('');
    $("#itemTypeMinimumReOrderHelpMe").empty().html('');
    $("#itemTypeDescription").val('');
    $("#itemTypeDescriptionHelpMe").empty().html('');
    $('#itemTypeDescription').empty().val('');
    $("#isDepreciate").val('');
    $("#isDepreciateHelpMe").empty().html('');
    $("#isFixedAsset").val('');
    $("#isFixedAssetHelpMe").empty().html('');
    $("#isInventory").val('');
    $("#isInventoryHelpMe").empty().html('');
    $("#isSalesItem").val('');
    $("#isSalesItemHelpMe").empty().html('');
}
function postRecord() {
...
function firstRecord(leafId, url, urlList, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        $.ajax({type: 'GET', url: url, data: {method: 'dataNavigationRequest', dataNavigation: 'firstRecord', output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                ...
                if (success === true) {
                    $.ajax({type: 'POST', url: url, data: {method: 'read', itemTypeId: firstRecord, output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                            var smileyRoll = './images/icons/smiley-roll.png';
                            $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                            if ($infoPanel.is(':hidden')) {
                                $infoPanel.show();
                            ...
                            if (success === true) {
                                $('#itemTypeId').val(data.data.itemTypeId);
                                $('#itemCategoryId').val(data.data.itemCategoryId).trigger("chosen:updated");
                                $('#itemTypeCode').val(data.data.itemTypeCode);
                                $('#itemTypeDepreciationRate').val(data.data.itemTypeDepreciationRate);
                                $('#itemTypeLife').val(data.data.itemTypeLife);
                                $('#itemTypeMinimumReOrder').val(data.data.itemTypeMinimumReOrder);
                                $('#itemTypeDescription').val(data.data.itemTypeDescription);
                                $('#isDepreciate').val(data.data.isDepreciate);
                                $('#isFixedAsset').val(data.data.isFixedAsset);
                                $('#isInventory').val(data.data.isInventory);
                                $('#isSalesItem').val(data.data.isSalesItem);
                                if (nextRecord > 0) {
                                    $('#previousRecordButton').removeClass().addClass('btn btn-default disabled').attr('onClick', '');
...
function endRecord(leafId, url, urlList, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        $.ajax({type: 'GET', url: url, data: {method: 'dataNavigationRequest', dataNavigation: 'lastRecord', output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                ...
                if (success === true) {
                    $.ajax({type: 'POST', url: url, data: {method: 'read', itemTypeId: lastRecord, output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                            var smileyRoll = './images/icons/smiley-roll.png';
                            $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                            if ($infoPanel.is(':hidden')) {
                                $infoPanel.show();
...
function previousRecord(leafId, url, urlList, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($previousRecordCounter.val()) > 0 && parseFloat($previousRecordCounter.val()) < parseFloat($('#lastRecordCounter').val())) {
            $.ajax({type: 'POST', url: url, data: {method: 'read', itemTypeId: $previousRecordCounter.val(), output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    var smileyRoll = './images/icons/smiley-roll.png';
                    var $infoPanel = $('#infoPanel');
                    $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                    if ($infoPanel.is(':hidden')) {
...
function nextRecord(leafId, url, urlList, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($nextRecordCounter.val()) <= parseFloat($('#lastRecordCounter').val())) {
            $.ajax({type: 'POST', url: url, data: {method: 'read', itemTypeId: $nextRecordCounter.val(), output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    var smileyRoll = './images/icons/smiley-roll.png';
                    var $infoPanel = $('#infoPanel');
                    $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                    if ($infoPanel.is(':hidden')) {
