...
function getAssetParts(leafId, url, securityToken) {
    $.ajax({type: 'GET', url: url, data: {offset: 0, limit: 99999, method: 'read', type: 'filter', securityToken: securityToken, leafId: leafId, filter: 'assetParts'}, beforeSend: function() {
            var smileyRoll = './images/icons/smiley-roll.png';
            var $infoPanel = $('#infoPanel');
            $infoPanel.html('').empty().html("<span class='label label-warning'><img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
            if ($infoPanel.is(':hidden')) {
            ...
            if (success === false) {
                ...
                $("#assetPartsId").html('').empty().html(data.data).trigger("chosen:updated");
                $infoPanel.html('').empty().html("<span class='label label-success'>&nbsp;<img src='" + smileyLol + "'>  " + decodeURIComponent(t['loadingCompleteTextLabel']) + "</span>").delay(5000).fadeOut();
            }
        }, error: function(xhr) {
            var smileyRollSweat = './images/icons/smiley-roll-sweat.png';
...
function checkDuplicate(leafId, page, securityToken) {
    var $assetPartsFaultCode = $("#assetPartsFaultCode");
    if ($assetPartsFaultCode.val().length === 0) {
        alert(t['oddTextLabel']);
        return false;
    }
    $.ajax({type: 'GET', url: page, data: {assetPartsFaultCode: $assetPartsFaultCode.val(), method: 'duplicate', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
            var smileyRoll = './images/icons/smiley-roll.png';
            var $infoPanel = $('#infoPanel');
            $infoPanel.html('').empty().html("<span class='label label-warning'><img src='" + smileyRoll + "'>&nbsp;" + decodeURIComponent(t['loadingTextLabel']) + "....</span>");
            if ($infoPanel.is(':hidden')) {
            ...
            if (success === true) {
                if (total !== 0) {
                    $("#assetPartsFaultCode").val('').focus();
                    $("#assetPartsFaultCodeForm").removeClass().addClass("form-group has-error");
                ...
                $("#assetPartsFaultForm").removeClass().addClass("form-group has-error");
            }
            if ($infoPanel.is(':hidden')) {
                $infoPanel.show();
            }
...
function showFormUpdate(leafId, url, urlList, securityToken, assetPartsFaultId, updateAccess, deleteAccess) {
    sleep(500);
    $('a[rel=tooltip]').tooltip('hide');
    $.ajax({type: 'POST', url: urlList, data: {method: 'read', type: 'form', assetPartsFaultId: assetPartsFaultId, securityToken: securityToken, leafId: leafId}, beforeSend: function() {
            var smileyRoll = './images/icons/smiley-roll.png';
            var $infoPanel = $('#infoPanel');
            $infoPanel.html('').empty().html("<span class='label label-warning'><img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
            if ($infoPanel.is(':hidden')) {
...
function showModalDelete(assetPartsFaultId, assetId, assetPartsId, assetMaintenanceCodeId, documentNumber, assetPartsFaultDate, assetPartsFaultDescription) {
    $('#assetPartsFaultIdPreview').val('').val(decodeURIComponent(assetPartsFaultId));
    $('#assetIdPreview').val('').val(decodeURIComponent(assetId));
    $('#assetPartsIdPreview').val('').val(decodeURIComponent(assetPartsId));
    ...
    $('#assetPartsFaultDatePreview').val('').val(decodeURIComponent(assetPartsFaultDate));
    $('#assetPartsFaultDescriptionPreview').val('').val(decodeURIComponent(assetPartsFaultDescription));
    showMeModal('deletePreview', 1);
}
function deleteGridRecord(leafId, url, urlList, securityToken) {
    $.ajax({type: 'POST', url: url, data: {method: 'delete', output: 'json', assetPartsFaultId: $('#assetPartsFaultIdPreview').val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
            var smileyRoll = './images/icons/smiley-roll.png';
            var $infoPanel = $('#infoPanel');
            $infoPanel.html('').empty().html("<span class='label label-warning'><img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
            if ($infoPanel.is(':hidden')) {
...
function deleteGridRecordCheckbox(leafId, url, urlList, securityToken) {
    var stringText = '';
    var counter = 0;
    $('input:checkbox[name="assetPartsFaultId[]"]').each(function() {
        stringText = stringText + "&assetPartsFaultId[]=" + $(this).val();
    });
...
function newRecord(leafId, url, urlList, securityToken, type, createAccess, updateAccess, deleteAccess) {
    ...
    var $assetId = $('#assetId');
    var $assetPartsId = $('#assetPartsId');
    var $assetMaintenanceCodeId = $('#assetMaintenanceCodeId');
    var $assetPartsFaultDate = $('#assetPartsFaultDate');
    var $assetPartsFaultDescription = $('#assetPartsFaultDescription');
    if (css.search('disabled') > 0) {
        return false;
    } else {
        if (type === 1) {
            ...
            if ($assetPartsId.val().length === 0) {
                $('#assetPartsIdHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['assetPartsIdLabel'] + " </span>");
                $assetPartsId.data('chosen').activate_action();
                return false;
            }
            ...
            if ($assetPartsFaultDate.val().length === 0) {
                $('#assetPartsFaultDateHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['assetPartsFaultDateLabel'] + " </span>");
                $('#assetPartsFaultDateForm').removeClass().addClass('form-group has-error');
                $assetPartsFaultDate.focus();
                return false;
            }
            if ($assetPartsFaultDescription.val().length === 0) {
                $('#assetPartsFaultDescriptionHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['assetPartsFaultDescriptionLabel'] + " </span>");
                $('#assetPartsFaultDescriptionForm').removeClass().addClass('form-group has-error');
                $assetPartsFaultDescription.focus();
                return false;
            }
            $.ajax({type: 'POST', url: url, data: {method: 'create', output: 'json', assetId: $assetId.val(), assetPartsId: $assetPartsId.val(), assetMaintenanceCodeId: $assetMaintenanceCodeId.val(),  assetPartsFaultDate: $assetPartsFaultDate.val(), assetPartsFaultDescription: $assetPartsFaultDescription.val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    var smileyRoll = './images/icons/smiley-roll.png';
                    var $infoPanel = $('#infoPanel');
                    $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                    if ($infoPanel.is(':hidden')) {
                    ...
                    if (success === true) {
                        ...
                        $assetPartsId.val('');
                        $assetPartsId.trigger("chosen:updated");
                        $('#assetPartsIdHelpMe').html('').empty();
                        ...
                        $assetPartsFaultDate.val('');
                        $('#assetPartsFaultDateHelpMe').html('').empty();
                        $assetPartsFaultDescription.val('');
                        $('#assetPartsFaultDescriptionForm').removeClass().addClass('form-group');
                        $('#assetPartsFaultDescriptionHelpMe').html('').empty();
                    } else if (success === false) {
                        $infoPanel.html('').empty().html("<span class='label label-important'>&nbsp;" + message + "</span>");
                        if ($infoPanel.is(':hidden')) {
                            $infoPanel.show();
            ...
            $.ajax({type: 'POST', url: url, data: {method: 'create', output: 'json', assetId: $assetId.val(), assetPartsId: $assetPartsId.val(), assetMaintenanceCodeId: $assetMaintenanceCodeId.val(),  assetPartsFaultDate: $assetPartsFaultDate.val(), assetPartsFaultDescription: $assetPartsFaultDescription.val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    ...
                    if (success === true) {
                        ...
                        $('#assetPartsFaultId').val(data.assetPartsFaultId);
                        $('#newRecordButton1').removeClass().addClass('btn btn-success disabled').attr('onClick', '');
                        $('#newRecordButton2').removeClass().addClass('btn dropdown-toggle btn-success disabled');
                        $('#newRecordButton3').attr('onClick', '');
                        $('#newRecordButton4').attr('onClick', '');
...
function updateRecord(leafId, url, urlList, securityToken, type, deleteAccess) {
	...
	var $assetPartsFaultId = $('#assetPartsFaultId');
    var $assetId = $('#assetId');
    var $assetPartsId = $('#assetPartsId');
    var $assetMaintenanceCodeId = $('#assetMaintenanceCodeId');
    var $assetPartsFaultDate = $('#assetPartsFaultDate');
    ...
    if (css.search('disabled') > 0) {
        ...
        if (type === 1) {
            ...
            $infoPanel.html('').empty();
            $.ajax({type: 'POST', url: url, data: {method: 'save', output: 'json', assetPartsFaultId: $assetPartsFaultId.val(), assetId: $assetId.val(), assetPartsId: $assetPartsId.val(), assetMaintenanceCodeId: $assetMaintenanceCodeId.val(),  assetPartsFaultDate: $assetPartsFaultDate.val(), assetPartsFaultDescription: $assetPartsFaultDescription.val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    var smileyRoll = './images/icons/smiley-roll.png';
                    var $infoPanel = $('#infoPanel');
                    $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                    if ($infoPanel.is(':hidden')) {
    ...
    if (css.search('disabled') > 0) {
        ...
        if (deleteAccess === 1) {
            if (confirm(decodeURIComponent(t['deleteRecordMessageLabel']))) {
                var value = $assetPartsFaultId.val();
                if (!value) {
                    ...
                    $.ajax({type: 'POST', url: url, data: {method: 'delete', output: 'json', assetPartsFaultId: $assetPartsFaultId.val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                            var smileyRoll = './images/icons/smiley-roll.png';
                            var $infoPanel = $('#infoPanel');
                            $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                            if ($infoPanel.is(':hidden')) {
...
function resetRecord(leafId, url, urlList, securityToken, createAccess, updateAccess, deleteAccess) {
    ...
    $("#assetPartsFaultId").val('');
    $("#assetPartsFaultIdHelpMe").empty().html('');
    
    $("#assetId").val('');
    $("#assetIdHelpMe").empty().html('');
    $('#assetId').trigger("chosen:updated");
    $("#assetPartsId").val('');
    $("#assetPartsIdHelpMe").empty().html('');
    $('#assetPartsId').trigger("chosen:updated");
    $("#assetMaintenanceCodeId").val('');
    ...
    $("#assetPartsFaultDate").val('');
    $("#assetPartsFaultDateHelpMe").empty().html('');
    $("#assetPartsFaultDescription").val('');
    $("#assetPartsFaultDescriptionHelpMe").empty().html('');
    $('#assetPartsFaultDescription').empty().val('');
}
function postRecord() {
...
function firstRecord(leafId, url, urlList, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        $.ajax({type: 'GET', url: url, data: {method: 'dataNavigationRequest', dataNavigation: 'firstRecord', output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                ...
                if (success === true) {
                    $.ajax({type: 'POST', url: url, data: {method: 'read', assetPartsFaultId: firstRecord, output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                            var smileyRoll = './images/icons/smiley-roll.png';
                            $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                            if ($infoPanel.is(':hidden')) {
                                $infoPanel.show();
                            ...
                            if (success === true) {
                                $('#assetPartsFaultId').val(data.data.assetPartsFaultId);
                                $('#assetId').val(data.data.assetId).trigger("chosen:updated");
                                $('#assetPartsId').val(data.data.assetPartsId).trigger("chosen:updated");
                                ...
                                x = data.data.assetPartsFaultDate;
                                x = x.split("-");
                                output = x[2] + "-" + x[1] + "-" + x[0];
                                output = output.toString();
                                $('#assetPartsFaultDate').val(output);
                                $('#assetPartsFaultDescription').val(data.data.assetPartsFaultDescription);
                                if (nextRecord > 0) {
                                    $('#previousRecordButton').removeClass().addClass('btn btn-default disabled').attr('onClick', '');
...
function endRecord(leafId, url, urlList, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        $.ajax({type: 'GET', url: url, data: {method: 'dataNavigationRequest', dataNavigation: 'lastRecord', output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                ...
                if (success === true) {
                    $.ajax({type: 'POST', url: url, data: {method: 'read', assetPartsFaultId: lastRecord, output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                            var smileyRoll = './images/icons/smiley-roll.png';
                            $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                            if ($infoPanel.is(':hidden')) {
                                $infoPanel.show();
...
function previousRecord(leafId, url, urlList, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($previousRecordCounter.val()) > 0 && parseFloat($previousRecordCounter.val()) < parseFloat($('#lastRecordCounter').val())) {
            $.ajax({type: 'POST', url: url, data: {method: 'read', assetPartsFaultId: $previousRecordCounter.val(), output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    var smileyRoll = './images/icons/smiley-roll.png';
                    var $infoPanel = $('#infoPanel');
                    $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                    if ($infoPanel.is(':hidden')) {
...
function nextRecord(leafId, url, urlList, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($nextRecordCounter.val()) <= parseFloat($('#lastRecordCounter').val())) {
            $.ajax({type: 'POST', url: url, data: {method: 'read', assetPartsFaultId: $nextRecordCounter.val(), output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    var smileyRoll = './images/icons/smiley-roll.png';
                    var $infoPanel = $('#infoPanel');
                    $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                    if ($infoPanel.is(':hidden')) {
