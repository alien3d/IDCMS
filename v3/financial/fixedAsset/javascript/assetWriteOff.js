...
function getAssetWriteOffReason(leafId, url, securityToken) {
    $.ajax({type: 'GET', url: url, data: {offset: 0, limit: 99999, method: 'read', type: 'filter', securityToken: securityToken, leafId: leafId, filter: 'assetWriteOffReason'}, beforeSend: function() {
            var smileyRoll = './images/icons/smiley-roll.png';
            var $infoPanel = $('#infoPanel');
            $infoPanel.html('').empty().html("<span class='label label-warning'><img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
            if ($infoPanel.is(':hidden')) {
            ...
            if (success === false) {
                ...
                $("#assetWriteOffReasonId").html('').empty().html(data.data).trigger("chosen:updated");
                $infoPanel.html('').empty().html("<span class='label label-success'>&nbsp;<img src='" + smileyLol + "'>  " + decodeURIComponent(t['loadingCompleteTextLabel']) + "</span>").delay(5000).fadeOut();
            }
        }, error: function(xhr) {
            var smileyRollSweat = './images/icons/smiley-roll-sweat.png';
...
function checkDuplicate(leafId, page, securityToken) {
    var $assetWriteOffCode = $("#assetWriteOffCode");
    if ($assetWriteOffCode.val().length === 0) {
        alert(t['oddTextLabel']);
        return false;
    }
    $.ajax({type: 'GET', url: page, data: {assetWriteOffCode: $assetWriteOffCode.val(), method: 'duplicate', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
            var smileyRoll = './images/icons/smiley-roll.png';
            var $infoPanel = $('#infoPanel');
            $infoPanel.html('').empty().html("<span class='label label-warning'><img src='" + smileyRoll + "'>&nbsp;" + decodeURIComponent(t['loadingTextLabel']) + "....</span>");
            if ($infoPanel.is(':hidden')) {
            ...
            if (success === true) {
                if (total !== 0) {
                    $("#assetWriteOffCode").val('').focus();
                    $("#assetWriteOffCodeForm").removeClass().addClass("form-group has-error");
                ...
                $("#assetWriteOffForm").removeClass().addClass("form-group has-error");
            }
            if ($infoPanel.is(':hidden')) {
                $infoPanel.show();
            }
...
function showFormUpdate(leafId, url, urlList, securityToken, assetWriteOffId, updateAccess, deleteAccess) {
    sleep(500);
    $('a[rel=tooltip]').tooltip('hide');
    $.ajax({type: 'POST', url: urlList, data: {method: 'read', type: 'form', assetWriteOffId: assetWriteOffId, securityToken: securityToken, leafId: leafId}, beforeSend: function() {
            var smileyRoll = './images/icons/smiley-roll.png';
            var $infoPanel = $('#infoPanel');
            $infoPanel.html('').empty().html("<span class='label label-warning'><img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
            if ($infoPanel.is(':hidden')) {
...
function showModalDelete(assetWriteOffId, itemCategoryId, itemTypeId, assetId, assetWriteOffReasonId, documentNumber, referenceNumber, journalNumber, assetWriteOffDate, assetPrice, yearToDate, currentNetBookValue, assetWriteOffAmount, assetWriteOffDescription) {
    $('#assetWriteOffIdPreview').val('').val(decodeURIComponent(assetWriteOffId));
    ...
    $('#assetWriteOffReasonIdPreview').val('').val(decodeURIComponent(assetWriteOffReasonId));
    ...
    $('#assetWriteOffDatePreview').val('').val(decodeURIComponent(assetWriteOffDate));
    ...
    $('#assetWriteOffAmountPreview').val('').val(decodeURIComponent(assetWriteOffAmount));
    $('#assetWriteOffDescriptionPreview').val('').val(decodeURIComponent(assetWriteOffDescription));
    showMeModal('deletePreview', 1);
}
function deleteGridRecord(leafId, url, urlList, securityToken) {
    $.ajax({type: 'POST', url: url, data: {method: 'delete', output: 'json', assetWriteOffId: $('#assetWriteOffIdPreview').val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
            var smileyRoll = './images/icons/smiley-roll.png';
            var $infoPanel = $('#infoPanel');
            $infoPanel.html('').empty().html("<span class='label label-warning'><img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
            if ($infoPanel.is(':hidden')) {
...
function deleteGridRecordCheckbox(leafId, url, urlList, securityToken) {
    var stringText = '';
    var counter = 0;
    $('input:checkbox[name="assetWriteOffId[]"]').each(function() {
        stringText = stringText + "&assetWriteOffId[]=" + $(this).val();
    });
...
function newRecord(leafId, url, urlList, securityToken, type, createAccess, updateAccess, deleteAccess) {
    ...
    var $assetId = $('#assetId');
    var $assetWriteOffReasonId = $('#assetWriteOffReasonId');
    
    var $referenceNumber = $('#referenceNumber');
    var $journalNumber = $('#journalNumber');
    var $assetWriteOffDate = $('#assetWriteOffDate');
    var $assetPrice = $('#assetPrice');
    var $yearToDate = $('#yearToDate');
    var $currentNetBookValue = $('#currentNetBookValue');
    var $assetWriteOffAmount = $('#assetWriteOffAmount');
    var $assetWriteOffDescription = $('#assetWriteOffDescription');
    if (css.search('disabled') > 0) {
        return false;
    } else {
        if (type === 1) {
            ...
            if ($assetWriteOffReasonId.val().length === 0) {
                $('#assetWriteOffReasonIdHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['assetWriteOffReasonIdLabel'] + " </span>");
                $assetWriteOffReasonId.data('chosen').activate_action();
                return false;
            }
            ...
            if ($assetWriteOffDate.val().length === 0) {
                $('#assetWriteOffDateHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['assetWriteOffDateLabel'] + " </span>");
                $('#assetWriteOffDateForm').removeClass().addClass('form-group has-error');
                $assetWriteOffDate.focus();
                return false;
            }
            ...
            if ($assetWriteOffAmount.val().length === 0) {
                $('#assetWriteOffAmountHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['assetWriteOffAmountLabel'] + " </span>");
                $('#assetWriteOffAmountForm').removeClass().addClass('form-group has-error');
                $assetWriteOffAmount.focus();
                return false;
            }
            if ($assetWriteOffDescription.val().length === 0) {
                $('#assetWriteOffDescriptionHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['assetWriteOffDescriptionLabel'] + " </span>");
                $('#assetWriteOffDescriptionForm').removeClass().addClass('form-group has-error');
                $assetWriteOffDescription.focus();
                return false;
            }
            $.ajax({type: 'POST', url: url, data: {method: 'create', output: 'json', itemCategoryId: $itemCategoryId.val(), itemTypeId: $itemTypeId.val(), assetId: $assetId.val(), assetWriteOffReasonId: $assetWriteOffReasonId.val(),  referenceNumber: $referenceNumber.val(), journalNumber: $journalNumber.val(), assetWriteOffDate: $assetWriteOffDate.val(), assetPrice: $assetPrice.val(), yearToDate: $yearToDate.val(), currentNetBookValue: $currentNetBookValue.val(), assetWriteOffAmount: $assetWriteOffAmount.val(), assetWriteOffDescription: $assetWriteOffDescription.val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    var smileyRoll = './images/icons/smiley-roll.png';
                    var $infoPanel = $('#infoPanel');
                    $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                    if ($infoPanel.is(':hidden')) {
                    ...
                    if (success === true) {
                        ...
                        $assetWriteOffReasonId.val('');
                        $assetWriteOffReasonId.trigger("chosen:updated");
                        $('#assetWriteOffReasonIdHelpMe').html('').empty();
                        $("documentNumber").val('');
                        ...
                        $assetWriteOffDate.val('');
                        $('#assetWriteOffDateHelpMe').html('').empty();
                        $assetPrice.val('');
                        ...
                        $assetWriteOffAmount.val('');
                        $('#assetWriteOffAmountHelpMe').html('').empty();
                        $assetWriteOffDescription.val('');
                        $('#assetWriteOffDescriptionForm').removeClass().addClass('form-group');
                        $('#assetWriteOffDescriptionHelpMe').html('').empty();
                    } else if (success === false) {
                        $infoPanel.html('').empty().html("<span class='label label-important'>&nbsp;" + message + "</span>");
                        if ($infoPanel.is(':hidden')) {
                            $infoPanel.show();
            ...
            $.ajax({type: 'POST', url: url, data: {method: 'create', output: 'json', itemCategoryId: $itemCategoryId.val(), itemTypeId: $itemTypeId.val(), assetId: $assetId.val(), assetWriteOffReasonId: $assetWriteOffReasonId.val(),  referenceNumber: $referenceNumber.val(), journalNumber: $journalNumber.val(), assetWriteOffDate: $assetWriteOffDate.val(), assetPrice: $assetPrice.val(), yearToDate: $yearToDate.val(), currentNetBookValue: $currentNetBookValue.val(), assetWriteOffAmount: $assetWriteOffAmount.val(), assetWriteOffDescription: $assetWriteOffDescription.val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    ...
                    if (success === true) {
                        ...
                        $('#assetWriteOffId').val(data.assetWriteOffId);
                        $('#newRecordButton1').removeClass().addClass('btn btn-success disabled').attr('onClick', '');
                        $('#newRecordButton2').removeClass().addClass('btn dropdown-toggle btn-success disabled');
                        $('#newRecordButton3').attr('onClick', '');
                        $('#newRecordButton4').attr('onClick', '');
...
function updateRecord(leafId, url, urlList, securityToken, type, deleteAccess) {
	...
	var $assetWriteOffId = $('#assetWriteOffId');
    var $itemCategoryId = $('#itemCategoryId');
    var $itemTypeId = $('#itemTypeId');
    var $assetId = $('#assetId');
    var $assetWriteOffReasonId = $('#assetWriteOffReasonId');
    ...
    if (css.search('disabled') > 0) {
        ...
        if (type === 1) {
            ...
            $infoPanel.html('').empty();
            $.ajax({type: 'POST', url: url, data: {method: 'save', output: 'json', assetWriteOffId: $assetWriteOffId.val(), itemCategoryId: $itemCategoryId.val(), itemTypeId: $itemTypeId.val(), assetId: $assetId.val(), assetWriteOffReasonId: $assetWriteOffReasonId.val(),  referenceNumber: $referenceNumber.val(), journalNumber: $journalNumber.val(), assetWriteOffDate: $assetWriteOffDate.val(), assetPrice: $assetPrice.val(), yearToDate: $yearToDate.val(), currentNetBookValue: $currentNetBookValue.val(), assetWriteOffAmount: $assetWriteOffAmount.val(), assetWriteOffDescription: $assetWriteOffDescription.val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    var smileyRoll = './images/icons/smiley-roll.png';
                    var $infoPanel = $('#infoPanel');
                    $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                    if ($infoPanel.is(':hidden')) {
        ...
        if (deleteAccess === 1) {
            if (confirm(decodeURIComponent(t['deleteRecordMessageLabel']))) {
                ...
                if (!value) {
                    ...
                    $.ajax({type: 'POST', url: url, data: {method: 'delete', output: 'json', assetWriteOffId: $assetWriteOffId.val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                            var smileyRoll = './images/icons/smiley-roll.png';
                            var $infoPanel = $('#infoPanel');
                            $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                            if ($infoPanel.is(':hidden')) {
...
function resetRecord(leafId, url, urlList, securityToken, createAccess, updateAccess, deleteAccess) {
    ...
    $("#assetWriteOffId").val('');
    $("#assetWriteOffIdHelpMe").empty().html('');
    ...
    $('#assetId').trigger("chosen:updated");
    $("#assetWriteOffReasonId").val('');
    $("#assetWriteOffReasonIdHelpMe").empty().html('');
    $('#assetWriteOffReasonId').trigger("chosen:updated");
    $("#documentNumber").val('');
    ...
    $("#assetWriteOffDate").val('');
    $("#assetWriteOffDateHelpMe").empty().html('');
    $("#assetPrice").val('');
    ...
    $("#assetWriteOffAmount").val('');
    $("#assetWriteOffAmountHelpMe").empty().html('');
    $("#assetWriteOffDescription").val('');
    $("#assetWriteOffDescriptionHelpMe").empty().html('');
    $('#assetWriteOffDescription').empty().val('');
}
function postRecord() {
...
function firstRecord(leafId, url, urlList, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        $.ajax({type: 'GET', url: url, data: {method: 'dataNavigationRequest', dataNavigation: 'firstRecord', output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                ...
                if (success === true) {
                    $.ajax({type: 'POST', url: url, data: {method: 'read', assetWriteOffId: firstRecord, output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                            var smileyRoll = './images/icons/smiley-roll.png';
                            $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                            if ($infoPanel.is(':hidden')) {
                                $infoPanel.show();
                            ...
                            if (success === true) {
                                $('#assetWriteOffId').val(data.data.assetWriteOffId);
                                ...
                                $('#assetWriteOffReasonId').val(data.data.assetWriteOffReasonId).trigger("chosen:updated");
                                ...
                                x = data.data.assetWriteOffDate;
                                x = x.split("-");
                                output = x[2] + "-" + x[1] + "-" + x[0];
                                output = output.toString();
                                $('#assetWriteOffDate').val(output);
                                ...
                                $('#assetWriteOffAmount').val(data.data.assetWriteOffAmount);
                                $('#assetWriteOffDescription').val(data.data.assetWriteOffDescription);
                                if (nextRecord > 0) {
                                    $('#previousRecordButton').removeClass().addClass('btn btn-default disabled').attr('onClick', '');
...
function endRecord(leafId, url, urlList, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        $.ajax({type: 'GET', url: url, data: {method: 'dataNavigationRequest', dataNavigation: 'lastRecord', output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                ...
                if (success === true) {
                    $.ajax({type: 'POST', url: url, data: {method: 'read', assetWriteOffId: lastRecord, output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                            var smileyRoll = './images/icons/smiley-roll.png';
                            $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                            if ($infoPanel.is(':hidden')) {
                                $infoPanel.show();
...
function previousRecord(leafId, url, urlList, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($previousRecordCounter.val()) > 0 && parseFloat($previousRecordCounter.val()) < parseFloat($('#lastRecordCounter').val())) {
            $.ajax({type: 'POST', url: url, data: {method: 'read', assetWriteOffId: $previousRecordCounter.val(), output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    var smileyRoll = './images/icons/smiley-roll.png';
                    var $infoPanel = $('#infoPanel');
                    $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                    if ($infoPanel.is(':hidden')) {
...
function nextRecord(leafId, url, urlList, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($nextRecordCounter.val()) <= parseFloat($('#lastRecordCounter').val())) {
            $.ajax({type: 'POST', url: url, data: {method: 'read', assetWriteOffId: $nextRecordCounter.val(), output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    var smileyRoll = './images/icons/smiley-roll.png';
                    var $infoPanel = $('#infoPanel');
                    $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                    if ($infoPanel.is(':hidden')) {
