...
function checkDuplicate(leafId, page, securityToken) {
    var $assetMaintenanceContractCode = $("#assetMaintenanceContractCode");
    if ($assetMaintenanceContractCode.val().length === 0) {
        alert(t['oddTextLabel']);
        return false;
    }
    $.ajax({type: 'GET', url: page, data: {assetMaintenanceContractCode: $assetMaintenanceContractCode.val(), method: 'duplicate', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
            var smileyRoll = './images/icons/smiley-roll.png';
            var $infoPanel = $('#infoPanel');
            $infoPanel.html('').empty().html("<span class='label label-warning'><img src='" + smileyRoll + "'>&nbsp;" + decodeURIComponent(t['loadingTextLabel']) + "....</span>");
            if ($infoPanel.is(':hidden')) {
            ...
            if (success === true) {
                if (total !== 0) {
                    $("#assetMaintenanceContractCode").val('').focus();
                    $("#assetMaintenanceContractCodeForm").removeClass().addClass("form-group has-error");
                ...
                $("#assetMaintenanceContractForm").removeClass().addClass("form-group has-error");
            }
            if ($infoPanel.is(':hidden')) {
                $infoPanel.show();
            }
...
function showFormUpdate(leafId, url, urlList, securityToken, assetMaintenanceContractId, updateAccess, deleteAccess) {
    sleep(500);
    $('a[rel=tooltip]').tooltip('hide');
    $.ajax({type: 'POST', url: urlList, data: {method: 'read', type: 'form', assetMaintenanceContractId: assetMaintenanceContractId, securityToken: securityToken, leafId: leafId}, beforeSend: function() {
            var smileyRoll = './images/icons/smiley-roll.png';
            var $infoPanel = $('#infoPanel');
            $infoPanel.html('').empty().html("<span class='label label-warning'><img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
            if ($infoPanel.is(':hidden')) {
...
function showModalDelete(assetMaintenanceContractId, businessPartnerId, assetId, assetMaintenanceContractStartDate, assetMaintenanceContractEndDate, assetMaintenanceContractDescription) {
    $('#assetMaintenanceContractIdPreview').val('').val(decodeURIComponent(assetMaintenanceContractId));
    $('#businessPartnerIdPreview').val('').val(decodeURIComponent(businessPartnerId));
    $('#assetIdPreview').val('').val(decodeURIComponent(assetId));
    $('#assetMaintenanceContractStartDatePreview').val('').val(decodeURIComponent(assetMaintenanceContractStartDate));
    $('#assetMaintenanceContractEndDatePreview').val('').val(decodeURIComponent(assetMaintenanceContractEndDate));
    $('#assetMaintenanceContractDescriptionPreview').val('').val(decodeURIComponent(assetMaintenanceContractDescription));
    showMeModal('deletePreview', 1);
}
function deleteGridRecord(leafId, url, urlList, securityToken) {
    $.ajax({type: 'POST', url: url, data: {method: 'delete', output: 'json', assetMaintenanceContractId: $('#assetMaintenanceContractIdPreview').val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
            var smileyRoll = './images/icons/smiley-roll.png';
            var $infoPanel = $('#infoPanel');
            $infoPanel.html('').empty().html("<span class='label label-warning'><img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
            if ($infoPanel.is(':hidden')) {
...
function deleteGridRecordCheckbox(leafId, url, urlList, securityToken) {
    var stringText = '';
    var counter = 0;
    $('input:checkbox[name="assetMaintenanceContractId[]"]').each(function() {
        stringText = stringText + "&assetMaintenanceContractId[]=" + $(this).val();
    });
...
function newRecord(leafId, url, urlList, securityToken, type, createAccess, updateAccess, deleteAccess) {
    ...
    var $assetId = $('#assetId');
    var $assetMaintenanceContractStartDate = $('#assetMaintenanceContractStartDate');
    var $assetMaintenanceContractEndDate = $('#assetMaintenanceContractEndDate');
    var $assetMaintenanceContractDescription = $('#assetMaintenanceContractDescription');
    if (css.search('disabled') > 0) {
        return false;
    } else {
        if (type === 1) {
            ...
            if ($assetMaintenanceContractStartDate.val().length === 0) {
                $('#assetMaintenanceContractStartDateHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['assetMaintenanceContractStartDateLabel'] + " </span>");
                $('#assetMaintenanceContractStartDateForm').removeClass().addClass('form-group has-error');
                $assetMaintenanceContractStartDate.focus();
                return false;
            }
            if ($assetMaintenanceContractEndDate.val().length === 0) {
                $('#assetMaintenanceContractEndDateHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['assetMaintenanceContractEndDateLabel'] + " </span>");
                $('#assetMaintenanceContractEndDateForm').removeClass().addClass('form-group has-error');
                $assetMaintenanceContractEndDate.focus();
                return false;
            }
            if ($assetMaintenanceContractDescription.val().length === 0) {
                $('#assetMaintenanceContractDescriptionHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['assetMaintenanceContractDescriptionLabel'] + " </span>");
                $('#assetMaintenanceContractDescriptionForm').removeClass().addClass('form-group has-error');
                $assetMaintenanceContractDescription.focus();
                return false;
            }
            $.ajax({type: 'POST', url: url, data: {method: 'create', output: 'json', businessPartnerId: $businessPartnerId.val(), assetId: $assetId.val(), assetMaintenanceContractStartDate: $assetMaintenanceContractStartDate.val(), assetMaintenanceContractEndDate: $assetMaintenanceContractEndDate.val(), assetMaintenanceContractDescription: $assetMaintenanceContractDescription.val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    var smileyRoll = './images/icons/smiley-roll.png';
                    var $infoPanel = $('#infoPanel');
                    $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                    if ($infoPanel.is(':hidden')) {
                    ...
                    if (success === true) {
                        ...
                        $assetMaintenanceContractStartDate.val('');
                        $('#assetMaintenanceContractStartDateHelpMe').html('').empty();
                        $assetMaintenanceContractEndDate.val('');
                        $('#assetMaintenanceContractEndDateHelpMe').html('').empty();
                        $assetMaintenanceContractDescription.val('');
                        $('#assetMaintenanceContractDescriptionForm').removeClass().addClass('form-group');
                        $('#assetMaintenanceContractDescriptionHelpMe').html('').empty();
                    } else if (success === false) {
                        $infoPanel.html('').empty().html("<span class='label label-important'>&nbsp;" + message + "</span>");
                        if ($infoPanel.is(':hidden')) {
                            $infoPanel.show();
            ...
            $.ajax({type: 'POST', url: url, data: {method: 'create', output: 'json', businessPartnerId: $businessPartnerId.val(), assetId: $assetId.val(), assetMaintenanceContractStartDate: $assetMaintenanceContractStartDate.val(), assetMaintenanceContractEndDate: $assetMaintenanceContractEndDate.val(), assetMaintenanceContractDescription: $assetMaintenanceContractDescription.val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    ...
                    if (success === true) {
                        ...
                        $('#assetMaintenanceContractId').val(data.assetMaintenanceContractId);
                        $('#newRecordButton1').removeClass().addClass('btn btn-success disabled').attr('onClick', '');
                        $('#newRecordButton2').removeClass().addClass('btn dropdown-toggle btn-success disabled');
                        $('#newRecordButton3').attr('onClick', '');
                        $('#newRecordButton4').attr('onClick', '');
...
function updateRecord(leafId, url, urlList, securityToken, type, deleteAccess) {
	...
	var $assetMaintenanceContractId = $('#assetMaintenanceContractId');
    var $businessPartnerId = $('#businessPartnerId');
    var $assetId = $('#assetId');
    var $assetMaintenanceContractStartDate = $('#assetMaintenanceContractStartDate');
    var $assetMaintenanceContractEndDate = $('#assetMaintenanceContractEndDate');
    ...
    if (css.search('disabled') > 0) {
        ...
        if (type === 1) {
            ...
            $infoPanel.html('').empty();
            $.ajax({type: 'POST', url: url, data: {method: 'save', output: 'json', assetMaintenanceContractId: $assetMaintenanceContractId.val(), businessPartnerId: $businessPartnerId.val(), assetId: $assetId.val(), assetMaintenanceContractStartDate: $assetMaintenanceContractStartDate.val(), assetMaintenanceContractEndDate: $assetMaintenanceContractEndDate.val(), assetMaintenanceContractDescription: $assetMaintenanceContractDescription.val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    var smileyRoll = './images/icons/smiley-roll.png';
                    var $infoPanel = $('#infoPanel');
                    $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                    if ($infoPanel.is(':hidden')) {
    ...
    if (css.search('disabled') > 0) {
        ...
        if (deleteAccess === 1) {
            if (confirm(decodeURIComponent(t['deleteRecordMessageLabel']))) {
                var value = $assetMaintenanceContractId.val();
                if (!value) {
                    ...
                    $.ajax({type: 'POST', url: url, data: {method: 'delete', output: 'json', assetMaintenanceContractId: $assetMaintenanceContractId.val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                            var smileyRoll = './images/icons/smiley-roll.png';
                            var $infoPanel = $('#infoPanel');
                            $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                            if ($infoPanel.is(':hidden')) {
...
function resetRecord(leafId, url, urlList, securityToken, createAccess, updateAccess, deleteAccess) {
    ...
    $("#assetMaintenanceContractId").val('');
    $("#assetMaintenanceContractIdHelpMe").empty().html('');
    ...
    $('#assetId').trigger("chosen:updated");
    $("#assetMaintenanceContractStartDate").val('');
    $("#assetMaintenanceContractStartDateHelpMe").empty().html('');
    $("#assetMaintenanceContractEndDate").val('');
    $("#assetMaintenanceContractEndDateHelpMe").empty().html('');
    $("#assetMaintenanceContractDescription").val('');
    $("#assetMaintenanceContractDescriptionHelpMe").empty().html('');
    $('#assetMaintenanceContractDescription').empty().val('');
}
function postRecord() {
...
function firstRecord(leafId, url, urlList, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        $.ajax({type: 'GET', url: url, data: {method: 'dataNavigationRequest', dataNavigation: 'firstRecord', output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                ...
                if (success === true) {
                    $.ajax({type: 'POST', url: url, data: {method: 'read', assetMaintenanceContractId: firstRecord, output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                            var smileyRoll = './images/icons/smiley-roll.png';
                            $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                            if ($infoPanel.is(':hidden')) {
                                $infoPanel.show();
                            ...
                            if (success === true) {
                                $('#assetMaintenanceContractId').val(data.data.assetMaintenanceContractId);
                                ...
                                x = data.data.assetMaintenanceContractStartDate;
                                x = x.split("-");
                                output = x[2] + "-" + x[1] + "-" + x[0];
                                output = output.toString();
                                $('#assetMaintenanceContractStartDate').val(output);
                                x = data.data.assetMaintenanceContractEndDate;
                                x = x.split("-");
                                output = x[2] + "-" + x[1] + "-" + x[0];
                                output = output.toString();
                                $('#assetMaintenanceContractEndDate').val(output);
                                $('#assetMaintenanceContractDescription').val(data.data.assetMaintenanceContractDescription);
                                if (nextRecord > 0) {
                                    $('#previousRecordButton').removeClass().addClass('btn btn-default disabled').attr('onClick', '');
...
function endRecord(leafId, url, urlList, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        $.ajax({type: 'GET', url: url, data: {method: 'dataNavigationRequest', dataNavigation: 'lastRecord', output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                ...
                if (success === true) {
                    $.ajax({type: 'POST', url: url, data: {method: 'read', assetMaintenanceContractId: lastRecord, output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                            var smileyRoll = './images/icons/smiley-roll.png';
                            $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                            if ($infoPanel.is(':hidden')) {
                                $infoPanel.show();
...
function previousRecord(leafId, url, urlList, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($previousRecordCounter.val()) > 0 && parseFloat($previousRecordCounter.val()) < parseFloat($('#lastRecordCounter').val())) {
            $.ajax({type: 'POST', url: url, data: {method: 'read', assetMaintenanceContractId: $previousRecordCounter.val(), output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    var smileyRoll = './images/icons/smiley-roll.png';
                    var $infoPanel = $('#infoPanel');
                    $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                    if ($infoPanel.is(':hidden')) {
...
function nextRecord(leafId, url, urlList, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($nextRecordCounter.val()) <= parseFloat($('#lastRecordCounter').val())) {
            $.ajax({type: 'POST', url: url, data: {method: 'read', assetMaintenanceContractId: $nextRecordCounter.val(), output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    var smileyRoll = './images/icons/smiley-roll.png';
                    var $infoPanel = $('#infoPanel');
                    $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                    if ($infoPanel.is(':hidden')) {
