...
function checkDuplicate(leafId, page, securityToken) {
    var $assetLoanCode = $("#assetLoanCode");
    if ($assetLoanCode.val().length === 0) {
        alert(t['oddTextLabel']);
        return false;
    }
    $.ajax({type: 'GET', url: page, data: {assetLoanCode: $assetLoanCode.val(), method: 'duplicate', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
            var smileyRoll = './images/icons/smiley-roll.png';
            var $infoPanel = $('#infoPanel');
            $infoPanel.html('').empty().html("<span class='label label-warning'><img src='" + smileyRoll + "'>&nbsp;" + decodeURIComponent(t['loadingTextLabel']) + "....</span>");
            if ($infoPanel.is(':hidden')) {
            ...
            if (success === true) {
                if (total !== 0) {
                    $("#assetLoanCode").val('').focus();
                    $("#assetLoanCodeForm").removeClass().addClass("form-group has-error");
                ...
                $("#assetLoanForm").removeClass().addClass("form-group has-error");
            }
            if ($infoPanel.is(':hidden')) {
                $infoPanel.show();
            }
...
function showFormUpdate(leafId, url, urlList, securityToken, assetLoanId, updateAccess, deleteAccess) {
    sleep(500);
    $('a[rel=tooltip]').tooltip('hide');
    $.ajax({type: 'POST', url: urlList, data: {method: 'read', type: 'form', assetLoanId: assetLoanId, securityToken: securityToken, leafId: leafId}, beforeSend: function() {
            var smileyRoll = './images/icons/smiley-roll.png';
            var $infoPanel = $('#infoPanel');
            $infoPanel.html('').empty().html("<span class='label label-warning'><img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
            if ($infoPanel.is(':hidden')) {
...
function showModalDelete(assetLoanId, branchId, warehouseId, departmentId, assetId, employeeId, documentNumber, assetLoanDateStart, assetLoanDateEnd, assetLoanDescription) {
    $('#assetLoanIdPreview').val('').val(decodeURIComponent(assetLoanId));
    ...
    $('#assetLoanDateStartPreview').val('').val(decodeURIComponent(assetLoanDateStart));
    $('#assetLoanDateEndPreview').val('').val(decodeURIComponent(assetLoanDateEnd));
    $('#assetLoanDescriptionPreview').val('').val(decodeURIComponent(assetLoanDescription));
    showMeModal('deletePreview', 1);
}
function deleteGridRecord(leafId, url, urlList, securityToken) {
    $.ajax({type: 'POST', url: url, data: {method: 'delete', output: 'json', assetLoanId: $('#assetLoanIdPreview').val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
            var smileyRoll = './images/icons/smiley-roll.png';
            var $infoPanel = $('#infoPanel');
            $infoPanel.html('').empty().html("<span class='label label-warning'><img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
            if ($infoPanel.is(':hidden')) {
...
function deleteGridRecordCheckbox(leafId, url, urlList, securityToken) {
    var stringText = '';
    var counter = 0;
    $('input:checkbox[name="assetLoanId[]"]').each(function() {
        stringText = stringText + "&assetLoanId[]=" + $(this).val();
    });
...
function newRecord(leafId, url, urlList, securityToken, type, createAccess, updateAccess, deleteAccess) {
    ...
    var $assetLoanDateStart = $('#assetLoanDateStart');
    var $assetLoanDateEnd = $('#assetLoanDateEnd');
    var $assetLoanDescription = $('#assetLoanDescription');
    if (css.search('disabled') > 0) {
        return false;
    } else {
        if (type === 1) {
            ...
            if ($assetLoanDateStart.val().length === 0) {
                $('#assetLoanDateStartHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['assetLoanDateStartLabel'] + " </span>");
                $('#assetLoanDateStartForm').removeClass().addClass('form-group has-error');
                $assetLoanDateStart.focus();
                return false;
            }
            if ($assetLoanDateEnd.val().length === 0) {
                $('#assetLoanDateEndHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['assetLoanDateEndLabel'] + " </span>");
                $('#assetLoanDateEndForm').removeClass().addClass('form-group has-error');
                $assetLoanDateEnd.focus();
                return false;
            }
            if ($assetLoanDescription.val().length === 0) {
                $('#assetLoanDescriptionHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['assetLoanDescriptionLabel'] + " </span>");
                $('#assetLoanDescriptionForm').removeClass().addClass('form-group has-error');
                $assetLoanDescription.focus();
                return false;
            }
            $.ajax({type: 'POST', url: url, data: {method: 'create', output: 'json', branchId: $branchId.val(), warehouseId: $warehouseId.val(), departmentId: $departmentId.val(), assetId: $assetId.val(), employeeId: $employeeId.val(),  assetLoanDateStart: $assetLoanDateStart.val(), assetLoanDateEnd: $assetLoanDateEnd.val(), assetLoanDescription: $assetLoanDescription.val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    var smileyRoll = './images/icons/smiley-roll.png';
                    var $infoPanel = $('#infoPanel');
                    $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                    if ($infoPanel.is(':hidden')) {
                    ...
                    if (success === true) {
                        ...
                        $assetLoanDateStart.val('');
                        $('#assetLoanDateStartHelpMe').html('').empty();
                        $assetLoanDateEnd.val('');
                        $('#assetLoanDateEndHelpMe').html('').empty();
                        $assetLoanDescription.val('');
                        $('#assetLoanDescriptionForm').removeClass().addClass('form-group');
                        $('#assetLoanDescriptionHelpMe').html('').empty();
                    } else if (success === false) {
                        $infoPanel.html('').empty().html("<span class='label label-important'>&nbsp;" + message + "</span>");
                        if ($infoPanel.is(':hidden')) {
                            $infoPanel.show();
            ...
            $.ajax({type: 'POST', url: url, data: {method: 'create', output: 'json', branchId: $branchId.val(), warehouseId: $warehouseId.val(), departmentId: $departmentId.val(), assetId: $assetId.val(), employeeId: $employeeId.val(),  assetLoanDateStart: $assetLoanDateStart.val(), assetLoanDateEnd: $assetLoanDateEnd.val(), assetLoanDescription: $assetLoanDescription.val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    ...
                    if (success === true) {
                        ...
                        $('#assetLoanId').val(data.assetLoanId);
                        $('#newRecordButton1').removeClass().addClass('btn btn-success disabled').attr('onClick', '');
                        $('#newRecordButton2').removeClass().addClass('btn dropdown-toggle btn-success disabled');
                        $('#newRecordButton3').attr('onClick', '');
                        $('#newRecordButton4').attr('onClick', '');
...
function updateRecord(leafId, url, urlList, securityToken, type, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        if (type === 1) {
            ...
            $infoPanel.html('').empty();
            $.ajax({type: 'POST', url: url, data: {method: 'save', output: 'json', assetLoanId: $assetLoanId.val(), branchId: $branchId.val(), warehouseId: $warehouseId.val(), departmentId: $departmentId.val(), assetId: $assetId.val(), employeeId: $employeeId.val(),  assetLoanDateStart: $assetLoanDateStart.val(), assetLoanDateEnd: $assetLoanDateEnd.val(), assetLoanDescription: $assetLoanDescription.val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    var smileyRoll = './images/icons/smiley-roll.png';
                    var $infoPanel = $('#infoPanel');
                    $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                    if ($infoPanel.is(':hidden')) {
        ...
        if (deleteAccess === 1) {
            if (confirm(decodeURIComponent(t['deleteRecordMessageLabel']))) {
                ...
                if (!value) {
                    ...
                    $.ajax({type: 'POST', url: url, data: {method: 'delete', output: 'json', assetLoanId: $assetLoanId.val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                            var smileyRoll = './images/icons/smiley-roll.png';
                            var $infoPanel = $('#infoPanel');
                            $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                            if ($infoPanel.is(':hidden')) {
...
function resetRecord(leafId, url, urlList, securityToken, createAccess, updateAccess, deleteAccess) {
    ...
    $("#assetLoanDateStart").val('');
    $("#assetLoanDateStartHelpMe").empty().html('');
    $("#assetLoanDateEnd").val('');
    $("#assetLoanDateEndHelpMe").empty().html('');
    $("#assetLoanDescription").val('');
    $("#assetLoanDescriptionHelpMe").empty().html('');
    $('#assetLoanDescription').empty().val('');
}
function postRecord() {
...
function firstRecord(leafId, url, urlList, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        $.ajax({type: 'GET', url: url, data: {method: 'dataNavigationRequest', dataNavigation: 'firstRecord', output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                ...
                if (success === true) {
                    $.ajax({type: 'POST', url: url, data: {method: 'read', assetLoanId: firstRecord, output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                            var smileyRoll = './images/icons/smiley-roll.png';
                            $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                            if ($infoPanel.is(':hidden')) {
                                $infoPanel.show();
                            ...
                            if (success === true) {
                                $('#assetLoanId').val(data.data.assetLoanId);
                                ...
                                x = data.data.assetLoanDateStart;
                                x = x.split("-");
                                output = x[2] + "-" + x[1] + "-" + x[0];
                                output = output.toString();
                                $('#assetLoanDateStart').val(output);
                                x = data.data.assetLoanDateEnd;
                                x = x.split("-");
                                output = x[2] + "-" + x[1] + "-" + x[0];
                                output = output.toString();
                                $('#assetLoanDateEnd').val(output);
                                $('#assetLoanDescription').val(data.data.assetLoanDescription);
                                if (nextRecord > 0) {
                                    $('#previousRecordButton').removeClass().addClass('btn btn-default disabled').attr('onClick', '');
...
function endRecord(leafId, url, urlList, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        $.ajax({type: 'GET', url: url, data: {method: 'dataNavigationRequest', dataNavigation: 'lastRecord', output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                ...
                if (success === true) {
                    $.ajax({type: 'POST', url: url, data: {method: 'read', assetLoanId: lastRecord, output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                            var smileyRoll = './images/icons/smiley-roll.png';
                            $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                            if ($infoPanel.is(':hidden')) {
                                $infoPanel.show();
...
function previousRecord(leafId, url, urlList, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($previousRecordCounter.val()) > 0 && parseFloat($previousRecordCounter.val()) < parseFloat($('#lastRecordCounter').val())) {
            $.ajax({type: 'POST', url: url, data: {method: 'read', assetLoanId: $previousRecordCounter.val(), output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    var smileyRoll = './images/icons/smiley-roll.png';
                    var $infoPanel = $('#infoPanel');
                    $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                    if ($infoPanel.is(':hidden')) {
...
function nextRecord(leafId, url, urlList, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($nextRecordCounter.val()) <= parseFloat($('#lastRecordCounter').val())) {
            $.ajax({type: 'POST', url: url, data: {method: 'read', assetLoanId: $nextRecordCounter.val(), output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    var smileyRoll = './images/icons/smiley-roll.png';
                    var $infoPanel = $('#infoPanel');
                    $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                    if ($infoPanel.is(':hidden')) {
