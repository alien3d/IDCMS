...
namespace Core\Financial\FixedAsset\AssetDisposal\Service;
 ...
 * @version 2
 * @author hafizan
 * @package Core\Financial\FixedAsset\AssetDisposal\Service
 * @subpackage FixedAsset
 * @link http://www.hafizan.com
 * @license http://www.gnu.org/copyleft/lesser.html LGPL
 */
class AssetDisposalService extends ConfigClass {
    ...
    public function getAssetDisposalReason() {
        ...
        if ($this->getVendor() == self::MYSQL) {
         ...
         ORDER BY    [isDefault]";
                ...
                if ($this->getVendor() == self::ORACLE) {
                    $sql = "
         SELECT      ASSETDISPOSALREASONID AS \"assetDisposalReasonId\",
                     ASSETDISPOSALREASONDESCRIPTION AS \"assetDisposalReasonDescription\"
         FROM        ASSETDISPOSALREASON
         WHERE       ISACTIVE    =   1
         AND         COMPANYID   =   '" . $this->getCompanyId() . "'
         ORDER BY    ISDEFAULT";
        ...
        if ($result) {
            ...
            while (($row = $this->q->fetchArray($result)) == true) {
                if ($this->getServiceOutput() == 'option') {
                    $str .= "<option value='" . $row['assetDisposalReasonId'] . "'>" . $d . ". " . $row['assetDisposalReasonDescription'] . "</option>";
                } else {
                    if ($this->getServiceOutput() == 'html') {
                        $items[] = $row;
                    }
    ...
    function setDisposal($rangeId, $leafId, $leafName) {
        ...
        if ($this->getVendor() == self::MYSQL) {
			...
			UPDATE	ASSETDISPOSAL
			SET			ISPOST= 1
			WHERE		DISPOSALID	IN	 (" . $rangeId . ")";
        }

        try {
            $result = $this->q->update($sql);
        } catch (\Exception $e) {
            echo json_encode(array("success" => false, "message" => $e->getMessage()));
            exit();
        }
        if ($this->getVendor() == self::MYSQL) {
			...
			FROM	[assetDisposal]
			JOIN		[asset]
			ON		[assetDisposal].[companyId] = [asset].[companyId]
			AND		[assetDisposal].[itemCategoryId] = [asset].[itemCategoryId]
			AND		[assetDisposal].[itemTypeId] = [asset].[itemTypeId]
			AND		[assetDisposal].[assetId] = [asset].[assetId]
			WHERE	[assetDisposalId] IN (" . $rangeId . ")";
        } else if ($this->getVendor() == self::ORACLE) {
            $sql = "
			SELECT  ASSET.BUSINESSPARTNERID AS \"businessPartnerId\",
						...
						ASSETDISPOSALDESCRIPTION	 AS \"assetWriteOffDescription\"
			FROM	ASSETDISPOSAL
			JOIN		ASSET
			ON		ASSETDISPOSAL.COMPANYID = ASSET.COMPANYID
			AND		ASSETDISPOSAL.ITEMCATEGORYID= ASSET.ITEMCATEGORYID
        ...
        while (($rowAsset = $this->q->fetchArray($result)) == TRUE) {
            ...
            $documentDate = $rowAsset['assetDisposalDate'];
            $description = $rowAsset['assetDisposalDescription'];
            $module = 'AS';
                $tableName = 'assetDisposal';
                $tableNameDetail = 'assetDisposalDetail';
                $tableNameId = 'assetDisposalId';
                $tableNameDetailId = 'assetDisposalDetailId';
                $referenceTableNameId = $row['assetDisposalId'];
                $referenceTableNameDetailId = $row['assetDisposalDetailId'];

            /*             * ********************************************************************************
             *
             *    Disposal of Fixed Assets Journal Entry – Sold at a profit
             *    Account                                               Debit                        Credit
             * Fixed Assets                                                                              90,000
             * Accumulated Depreciation                            60,000
             * Cash                                                        45,000
             * Disposal of Fixed Assets Account                15,000
             * Disposal of Fixed Assets Account                                              15,000
             * Total                                                          105,000                    105,000
             * ********************************************************************************* */
            //  45k > 30k (depreciation) so gain and loss(profit) 15k
            if ($rowAsset['assetDisposalSalesValue'] >= $rowAsset['assetNetBookValue']) {
                $chartOfAccountId = $assetSpecificationCostAccounts;
                $localAmount = $rowAsset['assetPrice'] * -1;
                ...
                $chartOfAccountId = $assetSpecificationGainAndLossAccounts;
                $localAmount = $rowAsset['assetDisposalGainAndLoss'];
                $this->ledgerService->setGeneralLedger($leafId, $leafName, $businessPartnerId, $chartOfAccountId, $documentNumber, $journalNumber, $documentDate, $localAmount, $description, $module, $tableName, $tableNameDetail, $tableNameId, $tableNameDetailId, $referenceTableNameId, $referenceTableNameDetailId);

                $chartOfAccountId = $assetSpecificationClearingAccounts;
                $localAmount = $rowAsset['assetDisposalGainAndLoss'];
                $this->ledgerService->setGeneralLedger($leafId, $leafName, $businessPartnerId, $chartOfAccountId, $documentNumber, $journalNumber, $documentDate, $localAmount, $description, $module, $tableName, $tableNameDetail, $tableNameId, $tableNameDetailId, $referenceTableNameId, $referenceTableNameDetailId);


                $chartOfAccountId = $assetSpecificationClearingAccounts;
                $localAmount = $rowAsset['assetDisposalGainAndLoss'] * -1;
                $this->ledgerService->setGeneralLedger($leafId, $leafName, $businessPartnerId, $chartOfAccountId, $documentNumber, $journalNumber, $documentDate, $localAmount, $description, $module, $tableName, $tableNameDetail, $tableNameId, $tableNameDetailId, $referenceTableNameId, $referenceTableNameDetailId);
            }

            /*             * *********************************************************************************
             *                                                                                                                                              *
             *    Disposal of Fixed Assets Journal Entry – Sold at a loss Account
             *  Account                                                Debit                        Credit
             *    Fixed Assets                                                                                90,000
             *    Accumulated Depreciation                            60,000
             *    Cash                                                        20,000
             *    Disposal of Fixed Assets Account                10,000
             *    Total                                                        90,000                        90,000
             * ********************************************************************************* */
            // sales 20k but net book value worth 30k. so gain and loss (loss) is 10k
            if ($rowAsset['assetDisposalSalesValue'] < $rowAsset['assetNetBookValue']) {
                $chartOfAccountId = $assetSpecificationCostAccounts;
                $localAmount = $rowAsset['assetPrice'] * -1;
                $this->ledgerService->setGeneralLedger($leafId, $leafName, $businessPartnerId, $chartOfAccountId, $documentNumber, $journalNumber, $documentDate, $localAmount, $description, $module, $tableName, $tableNameDetail, $tableNameId, $tableNameDetailId, $referenceTableNameId, $referenceTableNameDetailId);
    ...
    /**
     * Return AssetDisposalReason Default Value
     * @return int
     */
    public function getAssetDisposalReasonDefaultValue() {
        //initialize dummy value.. no content header.pure html
        $sql = null;
        $assetDisposalReasonId = null;
        if ($this->getVendor() == self::MYSQL) {
            $sql = "
         SELECT      `assetDisposalReasonId`
         FROM        	`assetdisposalreason`
         ...
         LIMIT 1";
            ...
            if ($this->getVendor() == self::MSSQL) {
                $sql = "
         SELECT      TOP 1 [assetDisposalReasonId],
         FROM        [assetDisposalReason]
         WHERE       [isActive]  =   1
         AND         [companyId] =   '" . $this->getCompanyId() . "'
         AND    	  [isDefault] =   1";
        ...
        if ($result) {
            $row = $this->q->fetchArray($result);
            $assetDisposalReasonId = $row['assetDisposalReasonId'];
        }
        return $assetDisposalReasonId;
    }
