...
namespace Core\Financial\FixedAsset\AssetPartsFault\Service;
...
/**
 * Class AssetPartsFaultService
 * Contain extra processing function / method.
 * @name IDCMS
 * @version 2
 * @author hafizan
 * @package Core\Financial\FixedAsset\AssetPartsFault\Service
 * @subpackage FixedAsset
 * @link http://www.hafizan.com
 * @license http://www.gnu.org/copyleft/lesser.html LGPL
 */
class AssetPartsFaultService extends ConfigClass {
    ...
    public function getAssetParts() {
        ...
        if ($this->getVendor() == self::MYSQL) {
         ...
         ORDER BY    [isDefault]";
                ...
                if ($this->getVendor() == self::ORACLE) {
                    $sql = "
         SELECT      ASSETPARTSID AS \"assetPartsId\",
                     ASSETPARTSDESCRIPTION AS \"assetPartsDescription\"
         FROM        ASSETPARTS
         WHERE       ISACTIVE    =   1
         AND         COMPANYID   =   '" . $this->getCompanyId() . "'
         ORDER BY    ISDEFAULT";
        ...
        if ($result) {
            ...
            while (($row = $this->q->fetchArray($result)) == true) {
                if ($this->getServiceOutput() == 'option') {
                    $str .= "<option value='" . $row['assetPartsId'] . "'>" . $d . ". " . $row['assetPartsDescription'] . "</option>";
                } else {
                    if ($this->getServiceOutput() == 'html') {
                        $items[] = $row;
                    }
    ...
    /**
     * Return AssetParts Default Value
     * @return int
     */
    public function getAssetPartsDefaultValue() {
        //initialize dummy value.. no content header.pure html
        $sql = null;
        $assetPartsId = null;
        if ($this->getVendor() == self::MYSQL) {
