...
require_once($newFakeDocumentRoot . "v3/financial/fixedAsset/controller/assetLoanController.php");
...
$translator->setCurrentTable('assetLoan');
...
$assetLoanArray = array();
$assetArray = array();
$employeeArray = array();
if (isset($_POST)) {
    if (isset($_POST['method'])) {
        $assetLoan = new \Core\Financial\FixedAsset\AssetLoan\Controller\AssetLoanClass();
        define('LIMIT', 10);
        ...
        if (isset($_POST ['query'])) {
            $assetLoan->setFieldQuery($_POST ['query']);
        }
        if (isset($_POST ['filter'])) {
            $assetLoan->setGridQuery($_POST ['filter']);
        }
        if (isset($_POST ['character'])) {
            $assetLoan->setCharacterQuery($_POST['character']);
        }
        if (isset($_POST ['dateRangeStart'])) {
            $assetLoan->setDateRangeStartQuery($_POST['dateRangeStart']);
            //explode the data to get day,month,year
            $start = explode('-', $_POST ['dateRangeStart']);
            $assetLoan->setStartDay($start[2]);
            $assetLoan->setStartMonth($start[1]);
            $assetLoan->setStartYear($start[0]);
        }
        if (isset($_POST ['dateRangeEnd']) && (strlen($_POST['dateRangeEnd']) > 0)) {
            $assetLoan->setDateRangeEndQuery($_POST['dateRangeEnd']);
            //explode the data to get day,month,year
            $start = explode('-', $_POST ['dateRangeEnd']);
            $assetLoan->setEndDay($start[2]);
            $assetLoan->setEndMonth($start[1]);
            $assetLoan->setEndYear($start[0]);
        }
        if (isset($_POST ['dateRangeType'])) {
            $assetLoan->setDateRangeTypeQuery($_POST['dateRangeType']);
        }
        if (isset($_POST ['dateRangeExtraType'])) {
            $assetLoan->setDateRangeExtraTypeQuery($_POST['dateRangeExtraType']);
        }
        $assetLoan->setServiceOutput('html');
        $assetLoan->setLeafId($leafId);
        $assetLoan->execute();
        $assetArray = $assetLoan->getAsset();
        $employeeArray = $assetLoan->getEmployee();
        if ($_POST['method'] == 'read') {
            $assetLoan->setStart($offset);
            $assetLoan->setLimit($limit); // normal system don't like paging..
            $assetLoan->setPageOutput('html');
            $assetLoanArray = $assetLoan->read();
            if (isset($assetLoanArray [0]['firstRecord'])) {
                $firstRecord = $assetLoanArray [0]['firstRecord'];
            }
            if (isset($assetLoanArray [0]['nextRecord'])) {
                $nextRecord = $assetLoanArray [0]['nextRecord'];
            }
            if (isset($assetLoanArray [0]['previousRecord'])) {
                $previousRecord = $assetLoanArray [0]['previousRecord'];
            }
            if (isset($assetLoanArray [0]['lastRecord'])) {
                $lastRecord = $assetLoanArray [0]['lastRecord'];
                $endRecord = $assetLoanArray [0]['lastRecord'];
            }
            $navigation = new \Core\Paging\HtmlPaging();
            $navigation->setLeafId($leafId);
            $navigation->setViewPath($assetLoan->getViewPath());
            ...
            if (isset($assetLoanArray [0]['total'])) {
                $total = $assetLoanArray [0]['total'];
            } else {
                $total = 0;
            }
                        ...
                        <div class="btn-group">
                            ...
                            <ul class="dropdown-menu" style="text-align:left">
                                <li>
                                    ...
                                    echo $assetLoan->getControllerPath();
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <form class="form-horizontal">
                                        <input type="hidden" name="assetLoanIdPreview" id="assetLoanIdPreview">
                                        ...
                                        <div class="form-group" id="assetLoanDateStartDiv">
                                            <label for="assetLoanDateStartPreview"
                                                   class="control-label col-xs-4 col-sm-4 col-md-4"><?php echo $leafTranslation['assetLoanDateStartLabel']; ?></label>

                                            <div class="col-xs-8 col-sm-8 col-md-8">
                                                <input class="form-control" type="text" name="assetLoanDateStartPreview"
                                                       id="assetLoanDateStartPreview">
                                            </div>
                                        </div>
                                        <div class="form-group" id="assetLoanDateEndDiv">
                                            <label for="assetLoanDateEndPreview"
                                                   class="control-label col-xs-4 col-sm-4 col-md-4"><?php echo $leafTranslation['assetLoanDateEndLabel']; ?></label>

                                            <div class="col-xs-8 col-sm-8 col-md-8">
                                                <input class="form-control" type="text" name="assetLoanDateEndPreview"
                                                       id="assetLoanDateEndPreview">
                                            </div>
                                        </div>
                                        <div class="form-group" id="assetLoanDescriptionDiv">
                                            <label for="assetLoanDescriptionPreview"
                                                   class="control-label col-xs-4 col-sm-4 col-md-4"><?php echo $leafTranslation['assetLoanDescriptionLabel']; ?></label>

                                            <div class="col-xs-8 col-sm-8 col-md-8">
                                                <input class="form-control" type="text" name="assetLoanDescriptionPreview"
                                                       id="assetLoanDescriptionPreview">
                                            </div>
                                        </div>
                                    </form>
                                </div>
                        ...
                        <div class="col-xs-12 col-sm-12 col-md-12">
                            <div class="panel panel-default">
                                <table class="table table table-bordered table-striped table-condensed table-hover smart-form has-tickbox" id="tableData">
                                    ...
                                    <th width="100px">
                                    <div align="center"><?php echo ucwords($leafTranslation['employeeIdLabel']); ?></div>
                                    </th>
                                    <th width="50px">
                                    <div align="center"><?php echo ucwords($leafTranslation['assetLoanDateStartLabel']); ?></div>
                                    </th>
                                    <th width="50px">
                                    <div align="center"><?php echo ucwords($leafTranslation['assetLoanDateEndLabel']); ?></div>
                                    </th>
                                    <th width="100px">
                                    ...
                                    <tbody id="tableBody">
                                        ...
                                        if ($_POST['method'] == 'read' && $_POST['type'] == 'list' && $_POST['detail'] == 'body') {
                                            if (is_array($assetLoanArray)) {
                                                $totalRecord = intval(count($assetLoanArray));
                                                if ($totalRecord > 0) {
                                                    $counter = 0;
                                                    for ($i = 0; $i < $totalRecord; $i++) {
                                                        ...
                                                        ?>
                                                        <tr <?php
                                                        if ($assetLoanArray[$i]['isDelete'] == 1) {
                                                            echo "class=\"danger\"";
                                                        } else {
                                                            if ($assetLoanArray[$i]['isDraft'] == 1) {
                                                                echo "class=\"warning\"";
                                                            }
                                                        }
                                                        ?>>
                                                            ...
                                                            <td align="center">
                                                                <div class="btn-group" align="center">
                                                                    ...
                                                                    <button type="button"  class="btn btn-danger btn-sm" title="Delete"
                                                                            ...
                                                                            ?>', '<?php
                                                                            echo rawurlencode(
                                                                                    $assetLoanArray [$i]['assetDescription']
                                                                            );
                                                                            ?>', '<?php
                                                                            echo rawurlencode(
                                                                                    $assetLoanArray [$i]['employeeDescription']
                                                                            );
                                                                            ?>', '<?php
                                                                            echo rawurlencode(
                                                                                    $assetLoanArray [$i]['documentNumber']
                                                                            );
                                                                            ?>', '<?php
                                                                            echo rawurlencode(
                                                                                    $assetLoanArray [$i]['assetLoanDateStart']
                                                                            );
                                                                            ?>', '<?php
                                                                            echo rawurlencode(
                                                                                    $assetLoanArray [$i]['assetLoanDateEnd']
                                                                            );
                                                                            ?>', '<?php
                                                                            echo rawurlencode(
                                                                                    $assetLoanArray [$i]['assetLoanDescription']
                                                                            );
                                                                            ?>');">
                                                            ...
                                                            <td>
                                                                <div class="pull-left">
                                                                    <?php
                                                                    if (isset($assetLoanArray[$i]['documentNumber'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos(
                                                                                                strtolower($assetLoanArray[$i]['documentNumber']), strtolower($_POST['query'])
                                                                                        ) !== false
                                                                                ) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $assetLoanArray[$i]['documentNumber']
                                                                                    );
                                                                                } else {
                                                                                    echo $assetLoanArray[$i]['documentNumber'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                    if (strpos(
                                                                                                    strtolower($assetLoanArray[$i]['documentNumber']), strtolower($_POST['character'])
                                                                                            ) !== false
                                                                                    ) {
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $assetLoanArray[$i]['documentNumber']
                                                                                        );
                                                                                    } else {
                                                                                        echo $assetLoanArray[$i]['documentNumber'];
                                                                                    }
                                                            ...
                                                            <td>
                                                                <div class="pull-left">
                                                                    <?php
                                                                    if (isset($assetLoanArray[$i]['assetDescription'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos(
                                                                                                $assetLoanArray[$i]['assetDescription'], $_POST['query']
                                                                                        ) !== false
                                                                                ) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $assetLoanArray[$i]['assetDescription']
                                                                                    );
                                                                                } else {
                                                                                    echo $assetLoanArray[$i]['assetDescription'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                    if (strpos(
                                                                                                    $assetLoanArray[$i]['assetDescription'], $_POST['character']
                                                                                            ) !== false
                                                                                    ) {
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $assetLoanArray[$i]['assetDescription']
                                                                                        );
                                                                                    } else {
                                                                                        echo $assetLoanArray[$i]['assetDescription'];
                                                                                    }
                                                            ...
                                                            <td>
                                                                <div class="pull-left">
                                                                    <?php
                                                                    if (isset($assetLoanArray[$i]['employeeDescription'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos(
                                                                                                $assetLoanArray[$i]['employeeDescription'], $_POST['query']
                                                                                        ) !== false
                                                                                ) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $assetLoanArray[$i]['employeeDescription']
                                                                                    );
                                                                                } else {
                                                                                    echo $assetLoanArray[$i]['employeeDescription'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                    if (strpos(
                                                                                                    $assetLoanArray[$i]['employeeDescription'], $_POST['character']
                                                                                            ) !== false
                                                                                    ) {
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $assetLoanArray[$i]['employeeDescription']
                                                                                        );
                                                                                    } else {
                                                                                        echo $assetLoanArray[$i]['employeeDescription'];
                                                                                    }
                                                            ...
                                                            <?php
                                                            if (isset($assetLoanArray[$i]['assetLoanDateStart'])) {
                                                                $valueArray = $assetLoanArray[$i]['assetLoanDateStart'];
                                                            ...
                                                            <?php
                                                            if (isset($assetLoanArray[$i]['assetLoanDateEnd'])) {
                                                                $valueArray = $assetLoanArray[$i]['assetLoanDateEnd'];
                                                                if ($dateConvert->checkDate($valueArray)) {
                                                                    $valueData = explode('-', $valueArray);
                                                                    $year = $valueData[0];
                                                                    $month = $valueData[1];
                                                            ...
                                                            <td>
                                                                <div class="pull-left">
                                                                    <?php
                                                                    if (isset($assetLoanArray[$i]['assetLoanDescription'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos(
                                                                                                strtolower($assetLoanArray[$i]['assetLoanDescription']), strtolower($_POST['query'])
                                                                                        ) !== false
                                                                                ) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $assetLoanArray[$i]['assetLoanDescription']
                                                                                    );
                                                                                } else {
                                                                                    echo $assetLoanArray[$i]['assetLoanDescription'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                    if (strpos(
                                                                                                    strtolower($assetLoanArray[$i]['assetLoanDescription']), strtolower($_POST['character'])
                                                                                            ) !== false
                                                                                    ) {
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $assetLoanArray[$i]['assetLoanDescription']
                                                                                        );
                                                                                    } else {
                                                                                        echo $assetLoanArray[$i]['assetLoanDescription'];
                                                                                    }
                                                            ...
                                                            <td>
                                                                <div class="pull-left">
                                                                    <?php
                                                                    if (isset($assetLoanArray[$i]['executeBy'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos($assetLoanArray[$i]['staffName'], $_POST['query']) !== false) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $assetLoanArray[$i]['staffName']
                                                                                    );
                                                                                } else {
                                                                                    echo $assetLoanArray[$i]['staffName'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                    if (strpos(
                                                                                                    $assetLoanArray[$i]['staffName'], $_POST['character']
                                                                                            ) !== false
                                                                                    ) {
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $assetLoanArray[$i]['staffName']
                                                                                        );
                                                                                    } else {
                                                                                        echo $assetLoanArray[$i]['staffName'];
                                                                                    }
                                                            ...
                                                            <?php
                                                            if (isset($assetLoanArray[$i]['executeTime'])) {
                                                                $valueArray = $assetLoanArray[$i]['executeTime'];
                                                            ...
                                                            ?>
                                                            <td>
                                                                <input style="display:none;" type="checkbox" name="assetLoanId[]"
                                                                       value="<?php echo $assetLoanArray[$i]['assetLoanId']; ?>">
                                                                <input <?php echo $checked; ?> type="checkbox" name="isDelete[]"
                                                                                               value="<?php echo $assetLoanArray[$i]['isDelete']; ?>">
                                                            </td>
                                                        </tr>
                                                        <?php
                                                    }
                        ...
                        <div class="col-xs-3 col-sm-3 col-md-3 pagination">
                            <div class="pull-right">
                                <button class="delete btn btn-warning" type="button" 
                                        ...
                                        echo $assetLoan->getControllerPath();
                                        ?>', '<?php echo $assetLoan->getViewPath(); ?>', '<?php echo $securityToken; ?>');">
...
if ((isset($_POST['method']) == 'new' || isset($_POST['method']) == 'read') && $_POST['type'] == 'form') {
        ...
        <div id="content" style="opacity: 1;">
            <div class="row">
                <div class="col-xs-12 col-sm-12 col-md-12">
                    <div class="panel panel-info">
                        ...
                        <div class="panel-body">
                            <input type="hidden" name="assetLoanId" id="assetLoanId" value="<?php
                            if (isset($_POST['assetLoanId'])) {
                                echo $_POST['assetLoanId'];
                            }
                            ?>">
                                            ...
                                            <select name="assetId" id="assetId" class="chzn-select form-control">
                                                ...
                                                if (is_array($assetArray)) {
                                                    ...
                                                    if ($totalRecord > 0) {
                                                        $d = 1;
                                                        for ($i = 0; $i < $totalRecord; $i++) {
                                                            if (isset($assetLoanArray[0]['assetId'])) {
                                                                if ($assetLoanArray[0]['assetId'] == $assetArray[$i]['assetId']) {
                                                                    $selected = "selected";
                                                                } else {
                                                                    $selected = null;
                                                                }
                                        ...
                                        <div class="col-xs-8 col-sm-8 col-md-8">
                                            <div class="input-group">
                                                ...
                                                ?>
                                                       ...
                                                       ?>" value="<?php
                                                       if (isset($assetLoanArray) && is_array($assetLoanArray)) {
                                                           if (isset($assetLoanArray[0]['documentNumber'])) {
                                                               echo htmlentities($assetLoanArray[0]['documentNumber']);
                                                           }
                                                       }
                                            ...
                                            <select name="employeeId" id="employeeId" class="chzn-select form-control">
                                                ...
                                                if (is_array($employeeArray)) {
                                                    ...
                                                    if ($totalRecord > 0) {
                                                        $d = 1;
                                                        for ($i = 0; $i < $totalRecord; $i++) {
                                                            if (isset($assetLoanArray[0]['employeeId'])) {
                                                                if ($assetLoanArray[0]['employeeId'] == $employeeArray[$i]['employeeId']) {
                                                                    $selected = "selected";
                                                                } else {
                                                                    $selected = null;
                                                                }
                            ...
                            <div class="row">
                                <div class="col-xs-12 col-sm-12 col-md-12">
                                    <?php
                                    if (isset($assetLoanArray) && is_array($assetLoanArray)) {
                                        if (isset($assetLoanArray[0]['assetLoanDateStart'])) {
                                            $valueArray = $assetLoanArray[0]['assetLoanDateStart'];
                                    ...
                                    ?>
                                    <div class="col-xs-6 col-sm-6 col-md-6 form-group" id="assetLoanDateStartForm">
                                        <label for="assetLoanDateStart" class="control-label col-xs-4 col-sm-4 col-md-4"><strong><?php
                                                echo ucfirst(
                                                        $leafTranslation['assetLoanDateStartLabel']
                                                );
                                                ?></strong></label>

                                        <div class="col-xs-8 col-sm-8 col-md-8">
                                            <div class="input-group">
                                                <input type="text" class="form-control" name="assetLoanDateStart" id="assetLoanDateStart"
                                                       value="<?php
                                                       if (isset($value)) {
                                                           echo $value;
                                                       }
                                                       ?>">
                                                <span class="input-group-addon"><img src="./images/icons/calendar.png" id="assetLoanDateStartImage"></span>
                                            </div>
                                            <span class="help-block" id="assetLoanDateStartHelpMe"></span>
                                        </div>
                                    </div>
                                    <?php
                                    if (isset($assetLoanArray) && is_array($assetLoanArray)) {
                                        if (isset($assetLoanArray[0]['assetLoanDateEnd'])) {
                                            $valueArray = $assetLoanArray[0]['assetLoanDateEnd'];
                                            if ($dateConvert->checkDate($valueArray)) {
                                                $valueData = explode('-', $valueArray);
                                                $year = $valueData[0];
                                                $month = $valueData[1];
                                    ...
                                    ?>
                                    <div class="col-xs-6 col-sm-6 col-md-6 form-group" id="assetLoanDateEndForm">
                                        <label for="assetLoanDateEnd" class="control-label col-xs-4 col-sm-4 col-md-4"><strong><?php
                                                echo ucfirst(
                                                        $leafTranslation['assetLoanDateEndLabel']
                                                );
                                                ?></strong></label>

                                        <div class="col-xs-8 col-sm-8 col-md-8">
                                            <div class="input-group">
                                                <input type="text" class="form-control" name="assetLoanDateEnd" id="assetLoanDateEnd"
                                                       value="<?php
                                                       if (isset($value)) {
                                                           echo $value;
                                                       }
                                                ...
                                                <span class="input-group-addon"><img src="./images/icons/calendar.png"
                                                                                     id="assetLoanDateEndImage"></span>
                                            </div>
                                            <span class="help-block" id="assetLoanDateEndHelpMe"></span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-xs-12 col-sm-12 col-md-12">
                                    <div class="form-group" id="assetLoanDescriptionForm">
                                        <div class="col-xs-12 col-sm-12 col-md-12">
                                            <textarea class="form-control" rows="5" name="assetLoanDescription" id="assetLoanDescription"><?php
                                                if (isset($assetLoanArray[0]['assetLoanDescription'])) {

                                                    echo htmlentities($assetLoanArray[0]['assetLoanDescription']);
                                                }
                                                ?></textarea> <span class="help-block" id="assetLoanDescriptionHelpMe"></span>
                                        </div>
                                    </div>
                                </div>
                            </div>
            ...
            <script type="text/javascript">
                ...
                $(document).ready(function() {
                    ...
                    validateMeNumeric('assetLoanId');
                    validateMeNumeric('assetId');
                    validateMeNumeric('employeeId');
                    
                    $('#assetLoanDateStart').datepicker({
                        format: "<?php echo $systemFormat['systemSettingDateFormat']; ?>"
                    }).on('changeDate', function() {
                        $(this).datepicker('hide');
                    });
                    $('#assetLoanDateEnd').datepicker({
                        format: "<?php echo $systemFormat['systemSettingDateFormat']; ?>"
                    }).on('changeDate', function() {
                        $(this).datepicker('hide');
                    });
    <?php if ($_POST['method'] == "new") { ?>
        ...
        <?php if ($leafAccess['leafAccessCreateValue'] == 1) { ?>
                            $('#newRecordButton1').removeClass().addClass('btn btn-success').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $assetLoan->getControllerPath(); ?>','<?php echo $assetLoan->getViewPath(); ?>','<?php echo $securityToken; ?>',1)");
                            $('#newRecordButton2').removeClass().addClass('btn dropdown-toggle btn-success');
                            $('#newRecordButton3').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $assetLoan->getControllerPath(); ?>','<?php echo $assetLoan->getViewPath(); ?>','<?php echo $securityToken; ?>',1)");
                            $('#newRecordButton4').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $assetLoan->getControllerPath(); ?>','<?php echo $assetLoan->getViewPath(); ?>','<?php echo $securityToken; ?>',2)");
                            $('#newRecordButton5').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $assetLoan->getControllerPath(); ?>','<?php echo $assetLoan->getViewPath(); ?>','<?php echo $securityToken; ?>',3)");
                            $('#newRecordButton6').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $assetLoan->getControllerPath(); ?>','<?php echo $assetLoan->getViewPath(); ?>','<?php echo $securityToken; ?>',4)");
                            $('#newRecordButton7').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $assetLoan->getControllerPath(); ?>','<?php echo $assetLoan->getViewPath(); ?>','<?php echo $securityToken; ?>',5)");
        <?php } else { ?>
                            $('#newRecordButton1')
        ...
        if ($_POST['assetLoanId']) {
            ...
            <?php if ($leafAccess['leafAccessUpdateValue'] == 1) { ?>
                                $('#updateRecordButton1').removeClass().addClass('btn btn-info').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $assetLoan->getControllerPath(); ?>','<?php echo $assetLoan->getViewPath(); ?>','<?php echo $securityToken; ?>',1,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                                $('#updateRecordButton2').removeClass().addClass('btn dropdown-toggle btn-info');
                                $('#updateRecordButton3').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $assetLoan->getControllerPath(); ?>','<?php echo $assetLoan->getViewPath(); ?>','<?php echo $securityToken; ?>',1,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                                $('#updateRecordButton4').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $assetLoan->getControllerPath(); ?>','<?php echo $assetLoan->getViewPath(); ?>','<?php echo $securityToken; ?>',2,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                                $('#updateRecordButton5').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $assetLoan->getControllerPath(); ?>','<?php echo $assetLoan->getViewPath(); ?>','<?php echo $securityToken; ?>',3,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
            <?php } else { ?>
            ...
            <?php if ($leafAccess['leafAccessDeleteValue'] == 1) { ?>
                                $('#deleteRecordButton')
                                        ...
                                        .attr('onClick', "deleteRecord(<?php echo $leafId; ?>,'<?php echo $assetLoan->getControllerPath(); ?>','<?php echo $assetLoan->getViewPath(); ?>','<?php echo $securityToken; ?>',<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
            <?php } else { ?>
                                $('#deleteRecordButton')
                                        .removeClass().addClass('btn btn-danger disabled')
                                        .attr('onClick', '');
    ...
    <?php } ?>
    <script type="text/javascript" src="./v3/financial/fixedAsset/javascript/assetLoan.js"></script>
    <hr>
