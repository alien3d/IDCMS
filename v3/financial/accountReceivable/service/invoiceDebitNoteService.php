...
namespace Core\Financial\AccountReceivable\InvoiceDebitNote\Service;
...
/**
 * Class InvoiceDebitNoteService
 * Contain extra processing function / method.
 * @name IDCMS
 * @version 2
 * @author hafizan
 * @package Core\Financial\AccountReceivable\InvoiceDebitNote\Service
 * @subpackage Account Receivable
 * @link http://www.idcmsapp.com
 * @link http://en.wikipedia.org/wiki/Debit_note WikiPedia Debit Note
 * @license http://www.gnu.org/copyleft/lesser.html LGPL
 */
class InvoiceDebitNoteService extends ConfigClass {
    ...
    /**
     * Upload Invoice Debit Note Attachment
     * @var int
     */
    private $sizeLimit;
    ...
    /**
     * Post Invoice Debit Note To General Ledger
     * @param int $invoiceDebitNoteId Invoice Debit Note Primry Key
     * @param int $leafId Leaf Primary Key
     * @param string $leafName Leaf Name
     * 
     */
    public function setPosting($invoiceDebitNoteId, $leafId, $leafName) {
        $sql = null;
        if ($this->getVendor() == self::MYSQL) {
            $sql = "
            SELECT  *
            ...
            SELECT  *
            FROM    [invoiceDebitNote]
            WHERE   [invoiceId] IN (" . $invoiceDebitNoteId . ")
			AND		[isActive]= 	1
            AND     [isPost] =0
            AND     [companyId] =   '" . $this->getCompanyId() . "'";
        } else if ($this->getVendor() == self::ORACLE) {
        ...
        $this->setInvoiceDebitNotePosted($invoiceDebitNoteId);
    }

    /**
     * Set Invoice Tracking
     * @param int $invoiceDebitNoteId Debit Note Primary Key
     * @param int $invoiceId Invoice Primary Key
     * @param int $invoiceStatusId Status Primary Key
     */
    public function setInvoiceStatusTracking($invoiceDebitNoteId, $invoiceId, $invoiceStatusId) {
        $sql = null;
        ...
        if ($this->getVendor() == self::MYSQL) {
            ...
            FROM   `invoicedebitnote`
            WHERE  `invoiceDebitNoteId` ='" . $invoiceDebitNoteId . "'
            ";
        } else if ($this->getVendor() == self::MSSQL) {
            $sql = "
    ...
    /**
     * Update Invoice Debit Note Posted Flag
     * @param int $invoiceDebitNoteId Invoice Debit Note Primary Key
     */
    private function setInvoiceDebitNotePosted($invoiceDebitNoteId) {
        $sql = null;
        if ($this->getVendor() == self::MYSQL) {
            $sql = "
    ...
    /**
     * Upload Invoice Debit Note Attachment before submitting the form.
     * @throws \Exception
     * @return void
     */
    function setInvoiceDebitNoteAttachment() {
        header('Content-Type:application/json; charset=utf-8');
        if ($this->getVendor() == self::MYSQL) {
            $sql = "SET NAMES utf8";
            $this->q->fast($sql);
...
// to pass data through iframe you will need to encode all html tags
        if ($this->getVendor() == self::MYSQL) {
            $sql = "
        INSERT INTO `invoicedebitnotetemp`(
             `companyId`,
             `staffId`,
             `leafId`,
             `invoiceDebitNoteTempName`, 
             `isNew`, 
             `executeBy`, 
             `executeTime`
        ) VALUES (
     ...
     * @throws \Exception
     */
    function transferAttachment($staffId, $invoiceDebitNoteId) {
        $sql = null;
        if ($this->getVendor() == self::MYSQL) {
            $sql = "
			SELECT      * 
        ...
        if ($result) {
            ...
            while (($row = $this->q->fetchArray($result)) == TRUE) {
                ...
                $documentPath = $this->getFakeDocumentRoot() . "v3/financial/accountReceivable/attachment/" . $row['invoiceDebitNoteTempName'];
                //  insert into document attachment/
                if ($this->getVendor() == self::MYSQL) {
                    $sql = "
