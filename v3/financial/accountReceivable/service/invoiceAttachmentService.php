<?php namespace Core\Financial\AccountReceivable\InvoiceAttachment\Service; 
 ...
 * @version 2 
 * @author hafizan 
 * @package Core\Financial\AccountReceivable\InvoiceAttachment\Service
 * @subpackage AccountReceivable 
 * @link http://www.hafizan.com 
 * @license http://www.gnu.org/copyleft/lesser.html LGPL 
 */ 
class InvoiceAttachmentService extends ConfigClass { 
	/** 
	...
	public function getDocumentCategory() { 
     ...
     if($this->getVendor()==self::MYSQL) { 
         ...
         SELECT      DOCUMENTCATEGORYID AS \"documentCategoryId\",
                     DOCUMENTCATEGORYDESCRIPTION AS \"documentCategoryDescription\"
         FROM        DOCUMENTCATEGORY  
         WHERE       ISACTIVE    =   1
         AND         COMPANYID   =   '".$this->getCompanyId()."'
         ORDER BY    ISDEFAULT"; 
     ...
     if($result) { 
         ...
         while(($row = $this->q->fetchArray($result))==TRUE) { 
             if($this->getServiceOutput()=='option'){
                  $str.="<option value='".$row['documentCategoryId']."'>".$d.". ".$row['documentCategoryDescription']."</option>";
             } else if ($this->getServiceOutput()=='html')  { 
                 $items[] = $row; 
             }
			  $d++;
  ...
  /**
   * Return DocumentCategory Default Value
   * @return int
   * @throws \Exception
	*/
	public function getDocumentCategoryDefaultValue() { 
     //initialize dummy value.. no content header.pure html  
     $sql=null; 
     $str=null; 
     ...
     if($this->getVendor()==self::MYSQL) { 
         ...
         $sql =" 
         SELECT      TOP 1 [documentCategoryId],
         FROM        [documentCategory]
         WHERE       [isActive]  =   1
         AND         [companyId] =   '".$this->getCompanyId()."'
         AND    	  [isDefault] =   1"; 
