...
namespace Core\Financial\AccountReceivable\EmployeeWorkOrder\Service;
...
/**
 * Class EmployeeWorkOrderService
 * Contain extra processing function / method.
 * @name IDCMS
 * @version 2
 * @author hafizan
 * @package Core\Financial\AccountReceivable\EmployeeWorkOrder\Service
 * @subpackage AccountReceivable
 * @link http://www.hafizan.com
 * @license http://www.gnu.org/copyleft/lesser.html LGPL
 */
class EmployeeWorkOrderService extends ConfigClass {
    ...
    public function getShift() {
        ...
        if ($this->getVendor() == self::MYSQL) {
         ...
         SELECT      SHIFTID AS \"shiftId\",
                     SHIFTDESCRIPTION AS \"shiftDescription\"
         FROM        SHIFT  
         WHERE       ISACTIVE    =   1
         AND         COMPANYID   =   '" . $this->getCompanyId() . "'
         ORDER BY    ISDEFAULT";
        ...
        if ($result) {
            ...
            while (($row = $this->q->fetchArray($result)) == true) {
                if ($this->getServiceOutput() == 'option') {
                    $str .= "<option value='" . $row['shiftId'] . "'>" . $d . ". " . $row['shiftDescription'] . "</option>";
                } else if ($this->getServiceOutput() == 'html') {
                    $items[] = $row;
                }
                $d++;
    ...
    /**
     * Return Shift Default Value
     * @return int
     */
    public function getShiftDefaultValue() {
        //initialize dummy value.. no content header.pure html
        $sql = null;
        $shiftId = null;
        if ($this->getVendor() == self::MYSQL) {
    ...
    public function getMilestone() {
        ...
        if ($this->getVendor() == self::MYSQL) {
         ...
         ORDER BY    [isDefault]";
            ...
            $sql = "
         SELECT      MILESTONEID AS \"milestoneId\",
        ...
        if ($result) {
            ...
            while (($row = $this->q->fetchArray($result)) == true) {
                if ($this->getServiceOutput() == 'option') {
                    $str .= "<option value='" . $row['milestoneId'] . "'>" . $d . ". " . $row['milestoneDescription'] . "</option>";
                } else if ($this->getServiceOutput() == 'html') {
                    $items[] = $row;
                }
                $d++;
    ...
    /**
     * Return Milestone Default Value
     * @return int
     */
    public function getMilestoneDefaultValue() {
        //initialize dummy value.. no content header.pure html
        $sql = null;
        $milestoneId = null;
        if ($this->getVendor() == self::MYSQL) {
