...
namespace Core\Financial\AccountReceivable\InvoiceRecurring\Service;
 ...
 * @version 2
 * @author hafizan
 * @package Core\Financial\AccountReceivable\InvoiceRecurring\Service
 * @subpackage AccountReceivable
 * @link http://www.hafizan.com
 * @license http://www.gnu.org/copyleft/lesser.html LGPL
 */
class InvoiceRecurringService extends ConfigClass {
        ...
        if ($this->getVendor() == self::MYSQL) {
         ...
         ORDER BY    [isDefault]";
                ...
                if ($this->getVendor() == self::ORACLE) {
                    $sql = "
         SELECT      INVOICERECURRINGTYPEID AS \"invoiceRecurringTypeId\",
                     INVOICERECURRINGTYPEDESCRIPTION AS \"invoiceRecurringTypeDescription\"
         FROM        INVOICERECURRINGTYPE
         WHERE       ISACTIVE    =   1
         AND         COMPANYID   =   '" . $this->getCompanyId() . "'
         ORDER BY    ISDEFAULT";
        ...
        if ($result) {
            ...
            while (($row = $this->q->fetchArray($result)) == true) {
                if ($this->getServiceOutput() == 'option') {
                    $str .= "<option value='" . $row['invoiceRecurringTypeId'] . "'>" . $d . ". " . $row['invoiceRecurringTypeDescription'] . "</option>";
                } else {
    ...
    /**
     * Return InvoiceRecurringType Default Value
     * @return int
     */
    public function getInvoiceRecurringTypeDefaultValue() {
        //initialize dummy value.. no content header.pure html
        $sql = null;
        $invoiceRecurringTypeId = null;
        if ($this->getVendor() == self::MYSQL) {
            $sql = "
         SELECT      `invoiceRecurringTypeId`
         FROM        	`invoicerecurringtype`
         ...
         LIMIT 1";
            ...
            if ($this->getVendor() == self::MSSQL) {
                $sql = "
         SELECT      TOP 1 [invoiceRecurringTypeId],
         FROM        [invoiceRecurringType]
         WHERE       [isActive]  =   1
         AND         [companyId] =   '" . $this->getCompanyId() . "'
         AND    	  [isDefault] =   1";
        ...
        if ($result) {
            ...
            $invoiceRecurringTypeId = $row['invoiceRecurringTypeId'];
        }
        return $invoiceRecurringTypeId;
    }
