...
namespace Core\Financial\AccountReceivable\InvoiceCreditNoteDetail\Service;
...
/**
 * Class InvoiceCreditNoteDetailService
 * Contain extra processing function / method.
 * @name IDCMS
 * @version 2
 * @author hafizan
 * @package Core\Financial\AccountReceivable\InvoiceCreditNoteDetail\Service
 * @subpackage AccountReceivable
 * @link http://www.hafizan.com
 * @license http://www.gnu.org/copyleft/lesser.html LGPL
 */
class InvoiceCreditNoteDetailService extends ConfigClass {
    ...
    public function getInvoiceCreditNote() {
        ...
        if ($this->getVendor() == self::MYSQL) {
         ...
         ORDER BY    [isDefault]";
            ...
            $sql = "
         SELECT      INVOICECREDITNOTEID AS \"invoiceCreditNoteId\",
                     INVOICECREDITNOTEDESCRIPTION AS \"invoiceCreditNoteDescription\"
         FROM        INVOICECREDITNOTE
         WHERE       ISACTIVE    =   1
         AND         COMPANYID   =   '" . $this->getCompanyId() . "'
         ORDER BY    ISDEFAULT";
        ...
        if ($result) {
            ...
            while (($row = $this->q->fetchArray($result)) == TRUE) {
                if ($this->getServiceOutput() == 'option') {
                    $str.="<option value='" . $row['invoiceCreditNoteId'] . "'>" . $d . ". " . $row['invoiceCreditNoteDescription'] . "</option>";
                } else if ($this->getServiceOutput() == 'html') {
                    $items[] = $row;
                }
                $d++;
    ...
    /**
     * Return InvoiceCreditNote Default Value
     * @return int
     */
    public function getInvoiceCreditNoteDefaultValue() {
        //initialize dummy value.. no content header.pure html
        $sql = null;
        ...
        if ($this->getVendor() == self::MYSQL) {
         ...
         LIMIT 1";
            ...
            $sql = "
         SELECT      TOP 1 [invoiceCreditNoteId],
         FROM        [invoiceCreditNote]
         WHERE       [isActive]  =   1
         AND         [companyId] =   '" . $this->getCompanyId() . "'
         AND    	  [isDefault] =   1";
     ...
     * @param int $invoiceCreditNoteId Main Table
     * @param string $type 1->debit,2->credit
     * @return double $total
     */
    public function getTotalInvoiceCreditNoteAmount($invoiceCreditNoteId, $type) {
        $total = 0;
        $sql = null;
        if ($this->getVendor() == self::MYSQL) {
            $sql = "
            ...
            WHERE       `invoicecreditnotedetail`.`companyId`                         =   '" . $this->getCompanyId() . "'
            AND         `invoicecreditnotedetail`.`invoiceCreditNoteId`                 IN   (" . $invoiceCreditNoteId . ")
            AND         `invoicecreditnotedetail`.`isActive`                          =   1";
            if ($type == 1) {
            ...
            // $sql .= "
            //  GROUP BY    `invoicecreditnotedetail`.`invoiceCreditNoteDetailAmount`
            //  ";
        } else if ($this->getVendor() == self::MSSQL) {
            $sql .= "
            SELECT      SUM([invoiceCreditNoteDetailAmount]) AS [total]
            ...
            WHERE       [invoiceCreditNoteDetail].[companyId]                         =   '" . $this->getCompanyId() . "'
            AND         [invoiceCreditNoteDetail].[purchaseInvoiceId]                         IN   (" . $invoiceCreditNoteId . ")
            AND         [invoiceCreditNoteDetail].[isActive]                          =   1";
            if ($type == 1) {
                $sql .= "  AND [INVOICECREDITNOTEDETAILAmount] >0 ";
            } else {
     ...
     * @param int $invoiceCreditNoteId
     * @return array
     */
    public function getTotalInvoiceCreditNoteDetail($invoiceCreditNoteId) {
        header('Content-Type:application/json; charset=utf-8');
        $totalDebit = $this->getTotalInvoiceCreditNoteAmount($invoiceCreditNoteId, 1);
        $totalCredit = $this->getTotalInvoiceCreditNoteAmount($invoiceCreditNoteId, 2);
        $trialBalance = $this->getCheckTrialBalance($invoiceCreditNoteId);
        return array(
            "success" => true,
     ...
     */
    private function getCheckTrialBalance($invoiceCreditNoteId) {
        $trialBalance = 0;
        $sql = null;
        ...
        if ($this->getVendor() == self::MYSQL) {
            ...
            SELECT (
                (
                    SELECT      SUM(`invoiceCreditNoteDetailAmount`)
                    FROM        `invoicecreditnotedetail`
            ...
            SELECT (
                (
                    ...
                    JOIN        [chartOfAccount]
                    ON          [invoiceCreditNoteDetail].[companyId]                         =   [chartOfAccount].[companyId]
                    AND         [invoiceCreditNoteDetail].[chartOfAccountId]                  =   [chartOfAccount].[chartOfAccountId]

                    JOIN        [chartOfAccountCategory]
                    ON          [invoiceCreditNoteDetail].[companyId]                         =   [chartOfAccountCategory][companyId]
