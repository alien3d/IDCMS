function checkDuplicate(leafId, page, securityToken) {
    var $invoiceRecurringTypeCode = $("#invoiceRecurringTypeCode");
    if ($invoiceRecurringTypeCode.val().length === 0) {
    ...
    $.ajax({
        ...
        url: page,
        data: {
            invoiceRecurringTypeCode: $invoiceRecurringTypeCode.val(),
            method: 'duplicate',
        ...
        success: function(data) {
            ...
            if (success === true) {
                if (total !== 0) {
                    $("#invoiceRecurringTypeCode").val('').focus();
                    $("#invoiceRecurringTypeCodeForm").removeClass().addClass("form-group has-error");
                ...
                $("#invoiceRecurringTypeForm").removeClass().addClass("form-group has-error");
            }
            if ($infoPanel.is(':hidden')) {
                $infoPanel.show();
            }
...
function showFormUpdate(leafId, url, urlList, securityToken, invoiceRecurringTypeId, updateAccess, deleteAccess) {
    sleep(500);
...
function showModalDelete(invoiceRecurringTypeId, invoiceRecurringTypeCode, invoiceRecurringTypeDescription) {
    $('#invoiceRecurringTypeIdPreview').val('').val(decodeURIComponent(invoiceRecurringTypeId));
    $('#invoiceRecurringTypeCodePreview').val('').val(decodeURIComponent(invoiceRecurringTypeCode));
    $('#invoiceRecurringTypeDescriptionPreview').val('').val(decodeURIComponent(invoiceRecurringTypeDescription));
...
function deleteGridRecord(leafId, url, urlList, securityToken) {
    $.ajax({
        ...
        data: {
            method: 'delete',
            output: 'json',
            invoiceRecurringTypeId: $('#invoiceRecurringTypeIdPreview').val(),
            securityToken: securityToken,
            leafId: leafId
        },
        beforeSend: function() {
...
function deleteGridRecordCheckbox(leafId, url, urlList, securityToken) {
    var stringText = '';
    var counter = 0;
    $('input:checkbox[name="invoiceRecurringTypeId[]"]').each(function() {
        stringText = stringText + "&invoiceRecurringTypeId[]=" + $(this).val();
    });
...
function newRecord(leafId, url, urlList, securityToken, type, createAccess, updateAccess, deleteAccess) {
    ...
    var $invoiceRecurringTypeCode = $('#invoiceRecurringTypeCode');
    var $invoiceRecurringTypeDescription = $('#invoiceRecurringTypeDescription');
    if (css.search('disabled') > 0) {
        return false;
    } else {
        if (type === 1) {
            if ($invoiceRecurringTypeCode.val().length === 0) {
                $('#invoiceRecurringTypeCodeHelpMe').html('').empty().html("<span class='label label-danger'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['invoiceRecurringTypeCodeLabel'] + " </span>");
                $('#invoiceRecurringTypeCodeForm').removeClass().addClass('form-group has-error');
                $invoiceRecurringTypeCode.focus();
                return false;
            }
            $.ajax({
                type: 'POST',
                ...
                data: {
                    ...
                    output: 'json',
                    invoiceRecurringTypeCode: $invoiceRecurringTypeCode.val(),
                    invoiceRecurringTypeDescription: $invoiceRecurringTypeDescription.val(),
                    securityToken: securityToken,
                    leafId: leafId
                },
                beforeSend: function() {
                ...
                success: function(data) {
                    ...
                    if (success === true) {
                        ...
                        $invoiceRecurringTypeCode.val('');
                        $('#invoiceRecurringTypeCodeHelpMe').html('').empty();
                        $invoiceRecurringTypeDescription.val('');
                        $('#invoiceRecurringTypeDescriptionHelpMe').html('').empty();
                    } else if (success === false) {
                        $infoPanel.html('').empty().html("<span class='label label-danger'>&nbsp;" + message + "</span>");
                        if ($infoPanel.is(':hidden')) {
                            $infoPanel.show();
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (success === true) {
                        ...
                        $('#invoiceRecurringTypeId').val(data.invoiceRecurringTypeId);
                        $('#newRecordButton1').removeClass().addClass('btn btn-success disabled');
                        $('#newRecordButton2').removeClass().addClass('btn dropdown-toggle btn-success disabled');
                        $('#newRecordButton3').attr('onClick', '');
                        $('#newRecordButton4').attr('onClick', '');
...
function updateRecord(leafId, url, urlList, securityToken, type, deleteAccess) {
    ...
    var $invoiceRecurringTypeId = $('#invoiceRecurringTypeId');
    var $invoiceRecurringTypeCode = $('#invoiceRecurringTypeCode');
    var $invoiceRecurringTypeDescription = $('#invoiceRecurringTypeDescription');
    if (css.search('disabled') > 0) {
        return false;
        ...
        if (type === 1) {
            ...
            $.ajax({
                ...
                data: {
                    method: 'save',
                    output: 'json',
                    invoiceRecurringTypeId: $invoiceRecurringTypeId.val(),
                    invoiceRecurringTypeCode: $invoiceRecurringTypeCode.val(),
                    invoiceRecurringTypeDescription: $invoiceRecurringTypeDescription.val(),
                    securityToken: securityToken,
                    leafId: leafId
    ...
    if (css.search('disabled') > 0) {
        ...
        if (deleteAccess === 1) {
            if (confirm(decodeURIComponent(t['deleteRecordMessageLabel']))) {
                var value = $invoiceRecurringTypeId.val();
                if (!value) {
...
function resetRecord(leafId, url, urlList, securityToken, createAccess) {
    ...
    $("#invoiceRecurringTypeIdHelpMe").empty().html('');
    $("#invoiceRecurringTypeCode").val('');
    $("#invoiceRecurringTypeCodeHelpMe").empty().html('');
    $("#invoiceRecurringTypeDescription").val('');
    $("#invoiceRecurringTypeDescriptionHelpMe").empty().html('');
}
function postRecord() {
            ...
            success: function(data) {
                ...
                if (success === true) {
                    $.ajax({
                        ...
                        data: {
                            method: 'read',
                            invoiceRecurringTypeId: firstRecord,
                            output: 'json',
                            securityToken: securityToken,
                            leafId: leafId
                        },
                        ...
                        success: function(data) {
                            ...
                            if (success === true) {
                                ...
                                $('#invoiceRecurringTypeCode').val(data.data.invoiceRecurringTypeCode);
                                $('#invoiceRecurringTypeDescription').val(data.data.invoiceRecurringTypeDescription);
                                if (nextRecord > 0) {
                                    $('#previousRecordButton').removeClass().addClass('btn btn-default disabled').attr('onClick', '');
            ...
            success: function(data) {
                ...
                if (success === true) {
                    $.ajax({
                        ...
                        data: {
                            method: 'read',
                            invoiceRecurringTypeId: lastRecord,
                            output: 'json',
                            securityToken: securityToken,
                            leafId: leafId
                        },
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($previousRecordCounter.val()) > 0 && parseFloat($previousRecordCounter.val()) < parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    invoiceRecurringTypeId: $previousRecordCounter.val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($nextRecordCounter.val()) <= parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    invoiceRecurringTypeId: $nextRecordCounter.val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
