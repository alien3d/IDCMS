...
function checkDuplicate(leafId, page, securityToken) {
    var $invoiceFollowUpCode = $("#invoiceFollowUpCode");
    if ($invoiceFollowUpCode.val().length === 0) {
    ...
    $.ajax({
        ...
        url: page,
        data: {
            invoiceFollowUpCode: $invoiceFollowUpCode.val(),
            method: 'duplicate',
            securityToken: securityToken,
            leafId: leafId
        },
...
function showFormUpdate(leafId, url, urlList, securityToken, invoiceFollowUpId, updateAccess, deleteAccess) {
    sleep(500);
...
function showModalDelete(invoiceFollowUpId, invoiceId, followUpId, documentNumber, invoiceFollowUpDate, invoiceFollowUpDescription) {
    // clear first old record if exist
    $('#invoiceFollowUpIdPreview').val('').val(decodeURIComponent(invoiceFollowUpId));
    ...
    $('#invoiceFollowUpDatePreview').val('').val(decodeURIComponent(invoiceFollowUpDate));
    $('#invoiceFollowUpDescriptionPreview').val('').val(decodeURIComponent(invoiceFollowUpDescription));
...
function deleteGridRecord(leafId, url, urlList, securityToken) {
    $.ajax({
        ...
        data: {
            method: 'delete',
            output: 'json',
            invoiceFollowUpId: $('#invoiceFollowUpIdPreview').val(),
            securityToken: securityToken,
            leafId: leafId
        },
        beforeSend: function() {
...
function deleteGridRecordCheckbox(leafId, url, urlList, securityToken) {
    var stringText = '';
    var counter = 0;
    $('input:checkbox[name="invoiceFollowUpId[]"]').each(function() {
        stringText = stringText + "&invoiceFollowUpId[]=" + $(this).val();
    });
...
function newRecord(leafId, url, urlList, securityToken, type, createAccess, updateAccess, deleteAccess) {
    ...
    var $invoiceFollowUpDate = $('#invoiceFollowUpDate');
    var $invoiceFollowUpDescription = $('#invoiceFollowUpDescription');
    if (css.search('disabled') > 0) {
        return false;
    } else {
        if (type === 1) {
            ...
            if ($invoiceFollowUpDate.val().length === 0) {
                $('#invoiceFollowUpDateHelpMe')
                        .html('').empty()
                        .html("<span class='label label-danger'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['invoiceFollowUpDateLabel'] + " </span>");
                $('#invoiceFollowUpDateForm')
                        .removeClass().addClass('form-group has-error');
                $invoiceFollowUpDate.focus();
                return false;
            }
            if ($invoiceFollowUpDescription.val().length === 0) {
                $('#invoiceFollowUpDescriptionHelpMe')
                        .html('').empty()
                        .html("<span class='label label-danger'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['invoiceFollowUpDescriptionLabel'] + " </span>");
                $('#invoiceFollowUpDescriptionForm')
                        .removeClass().addClass('form-group has-error');
                $invoiceFollowUpDescription.focus();
                return false;
            }
            $.ajax({
                type: 'POST',
                ...
                data: {
                    ...
                    invoiceFollowUpDate: $invoiceFollowUpDate.val(),
                    invoiceFollowUpDescription: $invoiceFollowUpDescription.val(),
                    securityToken: securityToken,
                    leafId: leafId
                },
                beforeSend: function() {
                ...
                success: function(data) {
                    ...
                    if (success === true) {
                        ...
                        $('#documentNumberHelpMe')
                                .html('').empty();
                        $invoiceFollowUpDate.val('');
                        $('#invoiceFollowUpDateHelpMe')
                                .html('').empty();
                        $invoiceFollowUpDescription.val('');
                        $('#invoiceFollowUpDescriptionHelpMe')
                                .html('').empty();
                    } else if (success === false) {
                        $infoPanel
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (success === true) {
                        ...
                        $('#invoiceFollowUpId').val(data.invoiceFollowUpId);
                        $('#newRecordButton1')
                                .removeClass().addClass('btn btn-success disabled');
                        $('#newRecordButton2')
                                .removeClass().addClass('btn dropdown-toggle btn-success disabled');
...
function updateRecord(leafId, url, urlList, securityToken, type, deleteAccess) {
    ...
    var $invoiceFollowUpId = $('#invoiceFollowUpId');
    var $invoiceId = $('#invoiceId');
    var $followUpId = $('#followUpId');
        ...
        if (type === 1) {
            ...
            $.ajax({
                ...
                data: {
                    method: 'save',
                    output: 'json',
                    invoiceFollowUpId: $invoiceFollowUpId.val(),
                    invoiceId: $invoiceId.val(),
                    followUpId: $followUpId.val(),
                    invoiceFollowUpDate: $invoiceFollowUpDate.val(),
                    invoiceFollowUpDescription: $invoiceFollowUpDescription.val(),
            ...
            if ($invoiceFollowUpDate.val().length === 0) {
                $('#invoiceFollowUpDateHelpMe')
                        .html('').empty()
                        .html("<span class='label label-danger'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['invoiceFollowUpDateLabel'] + " </span>");
                $('#invoiceFollowUpDateForm').removeClass().addClass('form-group has-error');
                $invoiceFollowUpDate.focus();
                return false;
            }
            if ($invoiceFollowUpDescription.val().length === 0) {
                $('#invoiceFollowUpDescriptionHelpMe')
                        .html('').empty()
                        .html("<span class='label label-danger'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['invoiceFollowUpDescriptionLabel'] + " </span>");
                $('#invoiceFollowUpDescriptionForm').removeClass().addClass('form-group has-error');
                $invoiceFollowUpDescription.focus();
                return false;
            }
            if ($infoPanel.is(':hidden')) {
    ...
    if (css.search('disabled') > 0) {
        ...
        if (deleteAccess === 1) {
            if (confirm(decodeURIComponent(t['deleteRecordMessageLabel']))) {
                var value = $invoiceFollowUpId.val();
                if (!value) {
                    $infoPanel
                            .html('').empty()
...
function resetRecord(leafId, url, urlList, urlPurchaseInvoiceCreditNoteDetail, securityToken, createAccess, updateAccess, deleteAccess) {
    ...
    $("#invoiceFollowUpId").val('');
    $("#invoiceFollowUpIdHelpMe")
            .empty().html('');
                ...
                if (success === true) {
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if (success === true) {
                                $('#invoiceFollowUpId').val(data.data.invoiceFollowUpId);
                                ...
                                x = data.data.invoiceFollowUpDate;
                                x = x.split("-");
                                output = x[2] + "-" + x[1] + "-" + x[0];
                                output = output.toString();
                                $('#invoiceFollowUpDate').val(output);
                                $('#invoiceFollowUpDescription').val(data.data.invoiceFollowUpDescription);
                                if (nextRecord > 0) {
                                    $('#previousRecordButton')
                                            .removeClass().addClass('btn btn-default disabled')
                                            .attr('onClick', '');
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($previousRecordCounter.val()) > 0 && parseFloat($previousRecordCounter.val()) < parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    invoiceFollowUpId: $previousRecordCounter.val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($nextRecordCounter.val()) <= parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    invoiceFollowUpId: $nextRecordCounter.val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
