function getInvoiceCategory(leafId, url, securityToken) {
    $.ajax({type: 'GET', url: url, data: {offset: 0, limit: 99999, method: 'read', type: 'filter', securityToken: securityToken, leafId: leafId, filter: 'invoiceCategory'}, beforeSend: function() {
            $('#infoPanel').empty();
            $('#infoPanel').html('');
            ...
            if (data.success == false) {
                ...
                $('#infoPanel').empty();
                $('#infoPanel').html('');
                $("#invoiceCategory").trigger("chosen:updated");
...
function getLateInterest(leafId, url, securityToken) {
    $.ajax({type: 'GET', url: url, data: {offset: 0, limit: 99999, method: 'read', type: 'filter', securityToken: securityToken, leafId: leafId, filter: 'lateInterest'}, beforeSend: function() {
            $('#infoPanel').empty();
            $('#infoPanel').html('');
            $('#infoPanel').html("<span class='label label-warning'><img src='./images/icons/smiley-roll.png'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
            if ($('#infoPanel').is(':hidden')) {
            ...
            if (data.success == false) {
                ...
                $('#infoPanel').empty();
                $('#infoPanel').html('');
                $("#lateInterest").trigger("chosen:updated");
                $('#infoPanel').html("<div class='alert alert-success  col-md-12'> <img src='./images/icons/smiley-lol.png'> " + decodeURIComponent(t['loadingCompleteTextLabel']) + "</div>");
            }
        }, error: function(xhr) {
            $('#infoError').empty();
...
function checkDuplicate(leafId, page, securityToken) {
    if ($("#invoiceTypeCode").val().length == 0) {
        alert(t['oddTextLabel']);
        return false;
    }
    $.ajax({type: 'GET', url: page, data: {invoiceTypeCode: $("#invoiceTypeCode").val(), method: 'duplicate', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
            $('#infoPanel').empty();
            $('#infoPanel').html('');
            $('#infoPanel').html("<span class='label label-warning'><img src='./images/icons/smiley-roll.png'>&nbsp;" + decodeURIComponent(t['loadingTextLabel']) + "....</span>");
            if ($('#infoPanel').is(':hidden')) {
            ...
            if (data.success == true) {
                if (data.total != 0) {
                    $("#invoiceTypeCode").empty();
                    $("#invoiceTypeCode").val('');
                    $("#invoiceTypeCode").focus();
                    $("#invoiceTypeCodeForm").removeClass();
                    $("#invoiceTypeCodeForm").addClass("col-md-12 form-group has-error");
                    $('#infoPanel').empty();
                    $('#infoPanel').html('');
                ...
                $("#invoiceTypeForm").removeClass();
                $("#invoiceTypeForm").addClass("col-md-12 form-group has-error");
            }
        }, error: function(xhr) {
            $('#infoError').empty();
            $('#infoError').html('');
...
function showFormUpdate(leafId, url, urlList, securityToken, invoiceTypeId, updateAccess, deleteAccess) {
    sleep(500);
    $.ajax({type: 'POST', url: urlList, data: {method: 'read', type: 'form', invoiceTypeId: invoiceTypeId, securityToken: securityToken, leafId: leafId}, beforeSend: function() {
            $('#infoPanel').empty();
            $('#infoPanel').html("<span class='label label-warning'><img src='./images/icons/smiley-roll.png'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
            if ($('#infoPanel').is(':hidden')) {
                $('#infoPanel').show();
...
function showModalDelete(invoiceTypeId, invoiceCategoryId, lateInterestId, invoiceTypeCode, invoiceTypeCreditLimit, invoiceTypeInterestRate, invoiceTypeMinimumDeposit, invoiceTypeDescription) {
    $('#invoiceTypeIdPreview').val('');
    $('#invoiceTypeIdPreview').val(decodeURIComponent(invoiceTypeId));
    $('#invoiceCategoryIdPreview').val('');
    $('#invoiceCategoryIdPreview').val(decodeURIComponent(invoiceCategoryId));
    $('#lateInterestIdPreview').val('');
    $('#lateInterestIdPreview').val(decodeURIComponent(lateInterestId));
    $('#invoiceTypeCodePreview').val('');
    $('#invoiceTypeCodePreview').val(decodeURIComponent(invoiceTypeCode));
    $('#invoiceTypeCreditLimitPreview').val('');
    $('#invoiceTypeCreditLimitPreview').val(decodeURIComponent(invoiceTypeCreditLimit));
    $('#invoiceTypeInterestRatePreview').val('');
    $('#invoiceTypeInterestRatePreview').val(decodeURIComponent(invoiceTypeInterestRate));
    $('#invoiceTypeMinimumDepositPreview').val('');
    $('#invoiceTypeMinimumDepositPreview').val(decodeURIComponent(invoiceTypeMinimumDeposit));
    $('#invoiceTypeDescriptionPreview').val('');
    $('#invoiceTypeDescriptionPreview').val(decodeURIComponent(invoiceTypeDescription));
    showMeModal('deletePreview', 1);
}
function deleteGridRecord(leafId, url, urlList, securityToken) {
    $.ajax({type: 'POST', url: url, data: {method: 'delete', output: 'json', invoiceTypeId: $('#invoiceTypeIdPreview').val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
            $('#infoPanel').empty();
            $('#infoPanel').html('');
            $('#infoPanel').html("<span class='label label-warning'><img src='./images/icons/smiley-roll.png'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
            if ($('#infoPanel').is(':hidden')) {
...
function deleteGridRecordCheckbox(leafId, url, urlList, securityToken) {
    var stringText = '';
    var counter = 0;
    $('input:checkbox[name="invoiceTypeId[]"]').each(function() {
        stringText = stringText + "&invoiceTypeId[]=" + $(this).val();
    });
...
function newRecord(leafId, url, urlList, securityToken, type, createAccess, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        $('#infoPanel').html('');
        if (type == 1) {
            if ($('#invoiceCategoryId').val().length == 0) {
                $('#invoiceCategoryIdHelpMe').empty();
                $('#invoiceCategoryIdHelpMe').html("<span class='label label-danger'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['invoiceCategoryIdLabel'] + " </span>");
                $('#invoiceCategoryId').data('chosen').activate_action();
                return false;
            }
            if ($('#lateInterestId').val().length == 0) {
                $('#lateInterestIdHelpMe').empty();
                $('#lateInterestIdHelpMe').html("<span class='label label-danger'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['lateInterestIdLabel'] + " </span>");
                $('#lateInterestId').data('chosen').activate_action();
                return false;
            }
            if ($('#invoiceTypeCode').val().length == 0) {
                $('#invoiceTypeCodeHelpMe').empty();
                $('#invoiceTypeCodeHelpMe').html("<span class='label label-danger'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['invoiceTypeCodeLabel'] + " </span>");
                $('#invoiceTypeCodeForm').addClass('form-group has-error');
                $('#invoiceTypeCode').focus();
                return false;
            }
            if ($('#invoiceTypeCreditLimit').val().length == 0) {
                $('#invoiceTypeCreditLimitHelpMe').empty();
                $('#invoiceTypeCreditLimitHelpMe').html("<span class='label label-danger'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['invoiceTypeCreditLimitLabel'] + " </span>");
                $('#invoiceTypeCreditLimitForm').addClass('form-group has-error');
                $('#invoiceTypeCreditLimit').focus();
                return false;
            }
            if ($('#invoiceTypeInterestRate').val().length == 0) {
                $('#invoiceTypeInterestRateHelpMe').empty();
                $('#invoiceTypeInterestRateHelpMe').html("<span class='label label-danger'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['invoiceTypeInterestRateLabel'] + " </span>");
                $('#invoiceTypeInterestRateForm').addClass('form-group has-error');
                $('#invoiceTypeInterestRate').focus();
                return false;
            }
            if ($('#invoiceTypeMinimumDeposit').val().length == 0) {
                $('#invoiceTypeMinimumDepositHelpMe').empty();
                $('#invoiceTypeMinimumDepositHelpMe').html("<span class='label label-danger'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['invoiceTypeMinimumDepositLabel'] + " </span>");
                $('#invoiceTypeMinimumDepositForm').addClass('form-group has-error');
                $('#invoiceTypeMinimumDeposit').focus();
                return false;
            }
            if ($('#invoiceTypeDescription').val().length == 0) {
                $('#invoiceTypeDescriptionHelpMe').empty();
                $('#invoiceTypeDescriptionHelpMe').html("<span class='label label-danger'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['invoiceTypeDescriptionLabel'] + " </span>");
                $('#invoiceTypeDescriptionForm').addClass('form-group has-error');
                $('#invoiceTypeDescription').focus();
                return false;
            }
            $.ajax({type: 'POST', url: url, data: {method: 'create', output: 'json', invoiceCategoryId: $('#invoiceCategoryId').val(), lateInterestId: $('#lateInterestId').val(), invoiceTypeCode: $('#invoiceTypeCode').val(), invoiceTypeCreditLimit: $('#invoiceTypeCreditLimit').val(), invoiceTypeInterestRate: $('#invoiceTypeInterestRate').val(), invoiceTypeMinimumDeposit: $('#invoiceTypeMinimumDeposit').val(), invoiceTypeDescription: $('#invoiceTypeDescription').val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    $('#infoPanel').empty();
                    $('#infoPanel').html('');
                    $('#infoPanel').html("<span class='label label-warning'>&nbsp;<img src='./images/icons/smiley-roll.png'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                    if ($('#infoPanel').is(':hidden')) {
                    ...
                    if (data.success == true) {
                        ...
                        $('#invoiceCategoryId').val('');
                        $('#invoiceCategoryIdHelpMe').empty();
                        $('#invoiceCategoryIdHelpMe').html('');
                        $('#lateInterestId').val('');
                        $('#lateInterestIdHelpMe').empty();
                        $('#lateInterestIdHelpMe').html('');
                        $('#invoiceTypeCode').val('');
                        $('#invoiceTypeCode').val('');
                        $('#invoiceTypeCodeHelpMe').empty();
                        $('#invoiceTypeCodeHelpMe').html('');
                        $('#invoiceTypeCreditLimit').val('');
                        $('#invoiceTypeCreditLimitHelpMe').empty();
                        $('#invoiceTypeCreditLimitHelpMe').html('');
                        $('#invoiceTypeInterestRate').val('');
                        $('#invoiceTypeInterestRateHelpMe').empty();
                        $('#invoiceTypeInterestRateHelpMe').html('');
                        $('#invoiceTypeMinimumDeposit').val('');
                        $('#invoiceTypeMinimumDepositHelpMe').empty();
                        $('#invoiceTypeMinimumDepositHelpMe').html('');
                        $('#invoiceTypeDescription').val('');
                        $('#invoiceTypeDescriptionForm').removeClass().addClass('col-md-12 form-group');
                        $('#invoiceTypeDescription').val('');
                        $('#invoiceTypeDescriptionHelpMe').empty();
                        $('#invoiceTypeDescriptionHelpMe').html('');
                    } else if (data.success == false) {
                        $('#infoPanel').empty();
                        $('#infoPanel').html("<span class='label label-danger'>&nbsp;" + data.message + "</span>");
                        if ($('#infoPanel').is(':hidden')) {
            ...
            $.ajax({type: 'POST', url: url, data: {method: 'create', output: 'json', invoiceCategoryId: $('#invoiceCategoryId').val(), lateInterestId: $('#lateInterestId').val(), invoiceTypeCode: $('#invoiceTypeCode').val(), invoiceTypeCreditLimit: $('#invoiceTypeCreditLimit').val(), invoiceTypeInterestRate: $('#invoiceTypeInterestRate').val(), invoiceTypeMinimumDeposit: $('#invoiceTypeMinimumDeposit').val(), invoiceTypeDescription: $('#invoiceTypeDescription').val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    ...
                    if (data.success == true) {
                        ...
                        $('#invoiceTypeId').val(data.invoiceTypeId);
                        $('#newRecordButton1').removeClass();
                        $('#newRecordButton2').removeClass();
                        $('#newRecordButton3').removeClass();
                        $('#newRecordButton4').removeClass();
...
function updateRecord(leafId, url, urlList, securityToken, type, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        if (type == 1) {
            ...
            $('#infoPanel').empty();
            $('#infoPanel').html('');
            $.ajax({type: 'POST', url: url, data: {method: 'save', output: 'json', invoiceTypeId: $('#invoiceTypeId').val(), invoiceCategoryId: $('#invoiceCategoryId').val(), lateInterestId: $('#lateInterestId').val(), invoiceTypeCode: $('#invoiceTypeCode').val(), invoiceTypeCreditLimit: $('#invoiceTypeCreditLimit').val(), invoiceTypeInterestRate: $('#invoiceTypeInterestRate').val(), invoiceTypeMinimumDeposit: $('#invoiceTypeMinimumDeposit').val(), invoiceTypeDescription: $('#invoiceTypeDescription').val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    $('#infoPanel').empty();
                    $('#infoPanel').html('');
                    $('#infoPanel').html("<span class='label label-warning'>&nbsp;<img src='./images/icons/smiley-roll.png'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                    if ($('#infoPanel').is(':hidden')) {
        ...
        if (deleteAccess == 1) {
            if (confirm(decodeURIComponent(t['deleteRecordMessageLabel']))) {
                ...
                if (!value) {
                    ...
                    $.ajax({type: 'POST', url: url, data: {method: 'delete', output: 'json', invoiceTypeId: $('#invoiceTypeId').val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                            $('#infoPanel').empty();
                            $('#infoPanel').html('');
                            $('#infoPanel').html("<span class='label label-warning'>&nbsp;<img src='./images/icons/smiley-roll.png'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                            if ($('#infoPanel').is(':hidden')) {
...
function resetRecord(leafId, url, urlList, securityToken, createAccess, updateAccess, deleteAccess) {
    ...
    $('#invoiceCategoryId').trigger("chosen:updated");
    $('#lateInterestId').trigger("chosen:updated");
    $("#invoiceTypeCode").val('');
    $("#invoiceTypeCreditLimit").val('');
    $("#invoiceTypeInterestRate").val('');
    $("#invoiceTypeMinimumDeposit").val('');
    $("#invoiceTypeDescription").val('');
    $('#invoiceTypeDescription').empty();
    $('#invoiceTypeDescription').val('');
    $(".chzn-select").chosen();
}
...
function firstRecord(leafId, url, urlList, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        $.ajax({type: 'GET', url: url, data: {method: 'dataNavigationRequest', dataNavigation: 'firstRecord', output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                ...
                if (data.success == true) {
                    $.ajax({type: 'POST', url: url, data: {method: 'read', invoiceTypeId: data.firstRecord, output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                            $('#infoPanel').empty();
                            $('#infoPanel').html('');
                            $('#infoPanel').html("<span class='label label-warning'>&nbsp;<img src='./images/icons/smiley-roll.png'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                            if ($('#infoPanel').is(':hidden')) {
                            ...
                            if (data.success == true) {
                                $('#invoiceTypeId').val(data.data.invoiceTypeId);
                                $('#invoiceCategoryId').val(data.data.invoiceCategoryId);
                                $('#invoiceCategoryId').trigger("chosen:updated");
                                $('#lateInterestId').val(data.data.lateInterestId);
                                $('#lateInterestId').trigger("chosen:updated");
                                $('#invoiceTypeCode').val(data.data.invoiceTypeCode);
                                $('#invoiceTypeCode').val(data.data.invoiceTypeCode);
                                $('#invoiceTypeCreditLimit').val(data.data.invoiceTypeCreditLimit);
                                $('#invoiceTypeInterestRate').val(data.data.invoiceTypeInterestRate);
                                $('#invoiceTypeMinimumDeposit').val(data.data.invoiceTypeMinimumDeposit);
                                $('#invoiceTypeDescription').val(data.data.invoiceTypeDescription);
                                $('#invoiceTypeDescription').val(data.data.invoiceTypeDescription);
...
function endRecord(leafId, url, urlList, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        $.ajax({type: 'GET', url: url, data: {method: 'dataNavigationRequest', dataNavigation: 'lastRecord', output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                ...
                if (data.success == true) {
                    $.ajax({type: 'POST', url: url, data: {method: 'read', invoiceTypeId: data.lastRecord, output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                            $('#infoPanel').empty();
                            $('#infoPanel').html('');
                            $('#infoPanel').html("<span class='label label-warning'>&nbsp;<img src='./images/icons/smiley-roll.png'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                            if ($('#infoPanel').is(':hidden')) {
...
function previousRecord(leafId, url, urlList, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($('#previousRecordCounter').val()) > 0 && parseFloat($('#previousRecordCounter').val()) < parseFloat($('#lastRecordCounter').val())) {
            $.ajax({type: 'POST', url: url, data: {method: 'read', invoiceTypeId: $('#previousRecordCounter').val(), output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    $('#infoPanel').empty();
                    $('#infoPanel').html('');
                    $('#infoPanel').html("<span class='label label-warning'>&nbsp;<img src='./images/icons/smiley-roll.png'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                    if ($('#infoPanel').is(':hidden')) {
...
function nextRecord(leafId, url, urlList, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($('#nextRecordCounter').val()) <= parseFloat($('#lastRecordCounter').val())) {
            $.ajax({type: 'POST', url: url, data: {method: 'read', invoiceTypeId: $('#nextRecordCounter').val(), output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    $('#infoPanel').empty();
                    $('#infoPanel').html('');
                    $('#infoPanel').html("<span class='label label-warning'>&nbsp;<img src='./images/icons/smiley-roll.png'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                    if ($('#infoPanel').is(':hidden')) {
