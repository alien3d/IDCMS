function getDiscountType(leafId, url, securityToken) {
    $.ajax({type: 'GET', url: url, data: {offset: 0, limit: 99999, method: 'read', type: 'filter', securityToken: securityToken, leafId: leafId, filter: 'discountType'}, beforeSend: function() {
            $('#infoPanel').empty();
            $('#infoPanel').html('');
...
function checkDuplicate(leafId, page, securityToken) {
    ...
    $.ajax({type: 'GET', url: page, data: {discountCode: $("#discountCode").val(), method: 'duplicate', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
            $('#infoPanel').empty();
            $('#infoPanel').html('');
            $('#infoPanel').html("<span class='label label-warning'><img src='./images/icons/smiley-roll.png'>&nbsp;" + decodeURIComponent(t['loadingTextLabel']) + "....</span>");
            if ($('#infoPanel').is(':hidden')) {
            ...
            if (data.success == true) {
                if (data.total != 0) {
                    ...
                    $("#discountCode").val('');
                    $("#discountCode").focus();
                    $("#discountCodeForm").removeClass();
                    $("#discountCodeForm").addClass("col-md-12 form-group has-error");
                    $('#infoPanel').empty();
                    $('#infoPanel').html('');
                ...
                $("#discountForm").removeClass();
                $("#discountForm").addClass("col-md-12 form-group has-error");
            }
        }, error: function(xhr) {
            $('#infoError').empty();
            $('#infoError').html('');
...
function showFormUpdate(leafId, url, urlList, securityToken, discountId, updateAccess, deleteAccess) {
    sleep(500);
    $.ajax({type: 'POST', url: urlList, data: {method: 'read', type: 'form', discountId: discountId, securityToken: securityToken, leafId: leafId}, beforeSend: function() {
            $('#infoPanel').empty();
            $('#infoPanel').html("<span class='label label-warning'><img src='./images/icons/smiley-roll.png'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
            if ($('#infoPanel').is(':hidden')) {
                $('#infoPanel').show();
...
function showModalDelete(discountId, discountTypeId, discountCode, discountStartDate, discountEndDate, discountDescription, discountAmount) {
    $('#discountIdPreview').val('');
    $('#discountIdPreview').val(decodeURIComponent(discountId));
    $('#discountTypeIdPreview').val('');
    $('#discountTypeIdPreview').val(decodeURIComponent(discountTypeId));
    $('#discountCodePreview').val('');
    $('#discountCodePreview').val(decodeURIComponent(discountCode));
    $('#discountStartDatePreview').val('');
    $('#discountStartDatePreview').val(decodeURIComponent(discountStartDate));
    $('#discountEndDatePreview').val('');
    $('#discountEndDatePreview').val(decodeURIComponent(discountEndDate));
    $('#discountDescriptionPreview').val('');
    $('#discountDescriptionPreview').val(decodeURIComponent(discountDescription));
    $('#discountAmountPreview').val('');
    $('#discountAmountPreview').val(decodeURIComponent(discountAmount));
    showMeModal('deletePreview', 1);
}
function deleteGridRecord(leafId, url, urlList, securityToken) {
    $.ajax({type: 'POST', url: url, data: {method: 'delete', output: 'json', discountId: $('#discountIdPreview').val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
            $('#infoPanel').empty();
            $('#infoPanel').html('');
            $('#infoPanel').html("<span class='label label-warning'><img src='./images/icons/smiley-roll.png'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
            if ($('#infoPanel').is(':hidden')) {
...
function deleteGridRecordCheckbox(leafId, url, urlList, securityToken) {
    var stringText = '';
    var counter = 0;
    $('input:checkbox[name="discountId[]"]').each(function() {
        stringText = stringText + "&discountId[]=" + $(this).val();
    });
...
function newRecord(leafId, url, urlList, securityToken, type, createAccess, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        $('#infoPanel').html('');
        if (type == 1) {
            if ($('#discountTypeId').val().length == 0) {
                $('#discountTypeIdHelpMe').empty();
                $('#discountTypeIdHelpMe').html("<span class='label label-danger'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['discountTypeIdLabel'] + " </span>");
                $('#discountTypeId').data('chosen').activate_action();
                return false;
            }
            if ($('#discountCode').val().length == 0) {
                $('#discountCodeHelpMe').empty();
                $('#discountCodeHelpMe').html("<span class='label label-danger'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['discountCodeLabel'] + " </span>");
                $('#discountCodeForm').addClass('form-group has-error');
                $('#discountCode').focus();
                return false;
            }
            if ($('#discountStartDate').val().length == 0) {
                $('#discountStartDateHelpMe').empty();
                $('#discountStartDateHelpMe').html("<span class='label label-danger'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['discountStartDateLabel'] + " </span>");
                $('#discountStartDateForm').addClass('form-group has-error');
                $('#discountStartDate').focus();
                return false;
            }
            if ($('#discountEndDate').val().length == 0) {
                $('#discountEndDateHelpMe').empty();
                $('#discountEndDateHelpMe').html("<span class='label label-danger'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['discountEndDateLabel'] + " </span>");
                $('#discountEndDateForm').addClass('form-group has-error');
                $('#discountEndDate').focus();
                return false;
            }
            if ($('#discountDescription').val().length == 0) {
                $('#discountDescriptionHelpMe').empty();
                $('#discountDescriptionHelpMe').html("<span class='label label-danger'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['discountDescriptionLabel'] + " </span>");
                $('#discountDescriptionForm').addClass('form-group has-error');
                $('#discountDescription').focus();
                return false;
            }
            if ($('#discountAmount').val().length == 0) {
                $('#discountAmountHelpMe').empty();
                $('#discountAmountHelpMe').html("<span class='label label-danger'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['discountAmountLabel'] + " </span>");
                $('#discountAmountForm').addClass('form-group has-error');
                $('#discountAmount').focus();
                return false;
            }
            $.ajax({type: 'POST', url: url, data: {method: 'create', output: 'json', discountTypeId: $('#discountTypeId').val(), discountCode: $('#discountCode').val(), discountStartDate: $('#discountStartDate').val(), discountEndDate: $('#discountEndDate').val(), discountDescription: $('#discountDescription').val(), discountAmount: $('#discountAmount').val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    $('#infoPanel').empty();
                    $('#infoPanel').html('');
                    $('#infoPanel').html("<span class='label label-warning'>&nbsp;<img src='./images/icons/smiley-roll.png'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                    if ($('#infoPanel').is(':hidden')) {
                    ...
                    if (data.success == true) {
                        ...
                        $('#discountTypeId').val('');
                        $('#discountTypeIdHelpMe').empty();
                        $('#discountTypeIdHelpMe').html('');
                        $('#discountCode').val('');
                        $('#discountCode').val('');
                        $('#discountCodeHelpMe').empty();
                        $('#discountCodeHelpMe').html('');
                        $('#discountStartDate').val('');
                        $('#discountStartDateHelpMe').empty();
                        $('#discountStartDateHelpMe').html('');
                        $('#discountEndDate').val('');
                        $('#discountEndDateHelpMe').empty();
                        $('#discountEndDateHelpMe').html('');
                        $('#discountDescription').val('');
                        $('#discountDescriptionForm').removeClass().addClass('col-md-12 form-group');
                        $('#discountDescription').val('');
                        $('#discountDescriptionHelpMe').empty();
                        $('#discountDescriptionHelpMe').html('');
                        $('#discountAmount').val('');
                        $('#discountAmountHelpMe').empty();
                        $('#discountAmountHelpMe').html('');
                    } else if (data.success == false) {
                        $('#infoPanel').empty();
                        $('#infoPanel').html("<span class='label label-danger'>&nbsp;" + data.message + "</span>");
                        if ($('#infoPanel').is(':hidden')) {
...
function updateRecord(leafId, url, urlList, securityToken, type, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        if (type == 1) {
            ...
            $('#infoPanel').empty();
            $('#infoPanel').html('');
            $.ajax({type: 'POST', url: url, data: {method: 'save', output: 'json', discountId: $('#discountId').val(), discountTypeId: $('#discountTypeId').val(), discountCode: $('#discountCode').val(), discountStartDate: $('#discountStartDate').val(), discountEndDate: $('#discountEndDate').val(), discountDescription: $('#discountDescription').val(), discountAmount: $('#discountAmount').val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    $('#infoPanel').empty();
                    $('#infoPanel').html('');
                    $('#infoPanel').html("<span class='label label-warning'>&nbsp;<img src='./images/icons/smiley-roll.png'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                    if ($('#infoPanel').is(':hidden')) {
        ...
        if (deleteAccess == 1) {
            if (confirm(decodeURIComponent(t['deleteRecordMessageLabel']))) {
                ...
                if (!value) {
                    ...
                    $.ajax({type: 'POST', url: url, data: {method: 'delete', output: 'json', discountId: $('#discountId').val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                            $('#infoPanel').empty();
                            $('#infoPanel').html('');
                            $('#infoPanel').html("<span class='label label-warning'>&nbsp;<img src='./images/icons/smiley-roll.png'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                            if ($('#infoPanel').is(':hidden')) {
...
function resetRecord(leafId, url, urlList, securityToken, createAccess, updateAccess, deleteAccess) {
    ...
    $('#discountTypeId').trigger("chosen:updated");
    $("#discountCode").val('');
    $("#discountStartDate").val('');
    $("#discountEndDate").val('');
    $("#discountDescription").val('');
...
function firstRecord(leafId, url, urlList, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        $.ajax({type: 'GET', url: url, data: {method: 'dataNavigationRequest', dataNavigation: 'firstRecord', output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                ...
                if (data.success == true) {
                    $.ajax({type: 'POST', url: url, data: {method: 'read', discountId: data.firstRecord, output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                            $('#infoPanel').empty();
                            $('#infoPanel').html('');
                            $('#infoPanel').html("<span class='label label-warning'>&nbsp;<img src='./images/icons/smiley-roll.png'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                            if ($('#infoPanel').is(':hidden')) {
                            ...
                            if (data.success == true) {
                                $('#discountId').val(data.data.discountId);
                                $('#discountTypeId').val(data.data.discountTypeId);
                                $('#discountTypeId').trigger("chosen:updated");
                                $('#discountCode').val(data.data.discountCode);
                                $('#discountCode').val(data.data.discountCode);
                                $('#discountStartDate').val(data.data.discountStartDate);
                                $('#discountEndDate').val(data.data.discountEndDate);
                                $('#discountDescription').val(data.data.discountDescription);
                                $('#discountDescription').val(data.data.discountDescription);
                                $('#discountAmount').val(data.data.discountAmount);
...
function endRecord(leafId, url, urlList, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        $.ajax({type: 'GET', url: url, data: {method: 'dataNavigationRequest', dataNavigation: 'lastRecord', output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                ...
                if (data.success == true) {
                    $.ajax({type: 'POST', url: url, data: {method: 'read', discountId: data.lastRecord, output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                            $('#infoPanel').empty();
                            $('#infoPanel').html('');
                            $('#infoPanel').html("<span class='label label-warning'>&nbsp;<img src='./images/icons/smiley-roll.png'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                            if ($('#infoPanel').is(':hidden')) {
...
function previousRecord(leafId, url, urlList, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($('#previousRecordCounter').val()) > 0 && parseFloat($('#previousRecordCounter').val()) < parseFloat($('#lastRecordCounter').val())) {
            $.ajax({type: 'POST', url: url, data: {method: 'read', discountId: $('#previousRecordCounter').val(), output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    $('#infoPanel').empty();
                    $('#infoPanel').html('');
                    $('#infoPanel').html("<span class='label label-warning'>&nbsp;<img src='./images/icons/smiley-roll.png'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                    if ($('#infoPanel').is(':hidden')) {
...
function nextRecord(leafId, url, urlList, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($('#nextRecordCounter').val()) <= parseFloat($('#lastRecordCounter').val())) {
            $.ajax({type: 'POST', url: url, data: {method: 'read', discountId: $('#nextRecordCounter').val(), output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    $('#infoPanel').empty();
                    $('#infoPanel').html('');
                    $('#infoPanel').html("<span class='label label-warning'>&nbsp;<img src='./images/icons/smiley-roll.png'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                    if ($('#infoPanel').is(':hidden')) {
