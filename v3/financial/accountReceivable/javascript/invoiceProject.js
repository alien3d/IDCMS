function checkDuplicate(leafId, page, securityToken) {
    var $invoiceProjectCode = $("#invoiceProjectCode");
    if ($invoiceProjectCode.val().length === 0) {
        alert(t['oddTextLabel']);
        return false;
    }
    $.ajax({type: 'GET', url: page, data: {invoiceProjectCode: $invoiceProjectCode.val(), method: 'duplicate', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
            ...
            if (success === true) {
                if (total !== 0) {
                    $("#invoiceProjectCode").val('').focus();
                    $("#invoiceProjectCodeForm").removeClass().addClass("form-group has-error");
                ...
                $("#invoiceProjectForm").removeClass().addClass("form-group has-error");
            }
            if ($infoPanel.is(':hidden')) {
                $infoPanel.show();
            }
...
function showFormUpdate(leafId, url, urlList, securityToken, invoiceProjectId, updateAccess, deleteAccess) {
    sleep(500);
    $('a[rel=tooltip]').tooltip('hide');
    $.ajax({type: 'POST', url: urlList, data: {method: 'read', type: 'form', invoiceProjectId: invoiceProjectId, securityToken: securityToken, leafId: leafId}, beforeSend: function() {
            var smileyRoll = './images/icons/smiley-roll.png';
            var $infoPanel = $('#infoPanel');
            $infoPanel.html('').empty().html("<span class='label label-warning'><img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
            if ($infoPanel.is(':hidden')) {
...
function showModalDelete(invoiceProjectId, documentNumber, referenceNumber, invoiceProjectTitle, invoiceProjectStartDate, invoiceProjectEndDate, invoiceProjectCurrentStage, invoiceProjectTotalStage, invoiceProjectValue, invoiceProjectClaim, invoiceProjectBalance, invoiceProjectRetentionValue, invoiceProjectRetentionPercent, invoiceProjectDescription) {
    ...
    $('#invoiceProjectTitlePreview').val('').val(decodeURIComponent(invoiceProjectTitle));
    $('#invoiceProjectStartDatePreview').val('').val(decodeURIComponent(invoiceProjectStartDate));
    $('#invoiceProjectEndDatePreview').val('').val(decodeURIComponent(invoiceProjectEndDate));
    $('#invoiceProjectCurrentStagePreview').val('').val(decodeURIComponent(invoiceProjectCurrentStage));
    $('#invoiceProjectTotalStagePreview').val('').val(decodeURIComponent(invoiceProjectTotalStage));
    $('#invoiceProjectValuePreview').val('').val(decodeURIComponent(invoiceProjectValue));
    $('#invoiceProjectClaimPreview').val('').val(decodeURIComponent(invoiceProjectClaim));
    $('#invoiceProjectBalancePreview').val('').val(decodeURIComponent(invoiceProjectBalance));
    $('#invoiceProjectRetentionValuePreview').val('').val(decodeURIComponent(invoiceProjectRetentionValue));
    $('#invoiceProjectRetentionPercentPreview').val('').val(decodeURIComponent(invoiceProjectRetentionPercent));
    $('#invoiceProjectDescriptionPreview').val('').val(decodeURIComponent(invoiceProjectDescription));
    showMeModal('deletePreview', 1);
}
function deleteGridRecord(leafId, url, urlList, securityToken) {
    $.ajax({type: 'POST', url: url, data: {method: 'delete', output: 'json', invoiceProjectId: $('#invoiceProjectIdPreview').val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
            var smileyRoll = './images/icons/smiley-roll.png';
            var $infoPanel = $('#infoPanel');
            $infoPanel.html('').empty().html("<span class='label label-warning'><img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
            if ($infoPanel.is(':hidden')) {
...
function deleteGridRecordCheckbox(leafId, url, urlList, securityToken) {
    var stringText = '';
    var counter = 0;
    $('input:checkbox[name="invoiceProjectId[]"]').each(function() {
        stringText = stringText + "&invoiceProjectId[]=" + $(this).val();
    });
...
function newRecord(leafId, url, urlList, securityToken, type, createAccess, updateAccess, deleteAccess) {
    ...
    var $invoiceProjectTitle = $('#invoiceProjectTitle');
    var $invoiceProjectStartDate = $('#invoiceProjectStartDate');
    var $invoiceProjectEndDate = $('#invoiceProjectEndDate');
    var $invoiceProjectCurrentStage = $('#invoiceProjectCurrentStage');
    var $invoiceProjectTotalStage = $('#invoiceProjectTotalStage');
    var $invoiceProjectValue = $('#invoiceProjectValue');
    var $invoiceProjectClaim = $('#invoiceProjectClaim');
    var $invoiceProjectBalance = $('#invoiceProjectBalance');
    var $invoiceProjectRetentionValue = $('#invoiceProjectRetentionValue');
    var $invoiceProjectRetentionPercent = $('#invoiceProjectRetentionPercent');
    var $invoiceProjectDescription = $('#invoiceProjectDescription');
    if (css.search('disabled') > 0) {
        return false;
    } else {
        if (type === 1) {

            if ($invoiceProjectTitle.val().length === 0) {
                $('#invoiceProjectTitleHelpMe').html('').empty().html("<span class='label label-danger'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['invoiceProjectTitleLabel'] + " </span>");
                $('#invoiceProjectTitleForm').removeClass().addClass('form-group has-error');
                $invoiceProjectTitle.focus();
                return false;
            }
            if ($invoiceProjectValue.val().length === 0) {
                $('#invoiceProjectValueHelpMe').html('').empty().html("<span class='label label-danger'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['invoiceProjectValueLabel'] + " </span>");
                $('#invoiceProjectValueForm').removeClass().addClass('form-group has-error');
                $invoiceProjectValue.focus();
                return false;
            }
            $.ajax({type: 'POST', url: url, data: {method: 'create', output: 'json', referenceNumber: $referenceNumber.val(), invoiceProjectTitle: $invoiceProjectTitle.val(), invoiceProjectStartDate: $invoiceProjectStartDate.val(), invoiceProjectEndDate: $invoiceProjectEndDate.val(), invoiceProjectCurrentStage: $invoiceProjectCurrentStage.val(), invoiceProjectTotalStage: $invoiceProjectTotalStage.val(), invoiceProjectValue: $invoiceProjectValue.val(), invoiceProjectClaim: $invoiceProjectClaim.val(), invoiceProjectBalance: $invoiceProjectBalance.val(), invoiceProjectRetentionValue: $invoiceProjectRetentionValue.val(), invoiceProjectRetentionPercent: $invoiceProjectRetentionPercent.val(), invoiceProjectDescription: $invoiceProjectDescription.val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    var smileyRoll = './images/icons/smiley-roll.png';
                    var $infoPanel = $('#infoPanel');
                    $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                    if ($infoPanel.is(':hidden')) {
                    ...
                    if (success === true) {
                        ...
                        $invoiceProjectTitle.val('');
                        $('#invoiceProjectTitleHelpMe').html('').empty();
                        $invoiceProjectStartDate.val('');
                        $('#invoiceProjectStartDateHelpMe').html('').empty();
                        $invoiceProjectEndDate.val('');
                        $('#invoiceProjectEndDateHelpMe').html('').empty();
                        $invoiceProjectCurrentStage.val('');
                        $('#invoiceProjectCurrentStageHelpMe').html('').empty();
                        $invoiceProjectTotalStage.val('');
                        $('#invoiceProjectTotalStageHelpMe').html('').empty();
                        $invoiceProjectValue.val('');
                        $('#invoiceProjectValueHelpMe').html('').empty();
                        $invoiceProjectClaim.val('');
                        $('#invoiceProjectClaimHelpMe').html('').empty();
                        $invoiceProjectBalance.val('');
                        $('#invoiceProjectBalanceHelpMe').html('').empty();
                        $invoiceProjectRetentionValue.val('');
                        $('#invoiceProjectRetentionValueHelpMe').html('').empty();
                        $invoiceProjectRetentionPercent.val('');
                        $('#invoiceProjectRetentionPercentHelpMe').html('').empty();
                        $invoiceProjectDescription.val('');
                        $('#invoiceProjectDescriptionHelpMe').html('').empty();
                    } else if (success === false) {
                        $infoPanel.html('').empty().html("<span class='label label-danger'>&nbsp;" + message + "</span>");
                        if ($infoPanel.is(':hidden')) {
                            $infoPanel.show();
            ...
            $.ajax({type: 'POST', url: url, data: {method: 'create', output: 'json', referenceNumber: $referenceNumber.val(), invoiceProjectTitle: $invoiceProjectTitle.val(), invoiceProjectStartDate: $invoiceProjectStartDate.val(), invoiceProjectEndDate: $invoiceProjectEndDate.val(), invoiceProjectCurrentStage: $invoiceProjectCurrentStage.val(), invoiceProjectTotalStage: $invoiceProjectTotalStage.val(), invoiceProjectValue: $invoiceProjectValue.val(), invoiceProjectClaim: $invoiceProjectClaim.val(), invoiceProjectBalance: $invoiceProjectBalance.val(), invoiceProjectRetentionValue: $invoiceProjectRetentionValue.val(), invoiceProjectRetentionPercent: $invoiceProjectRetentionPercent.val(), invoiceProjectDescription: $invoiceProjectDescription.val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    ...
                    if (success === true) {
                        ...
                        $('#invoiceProjectId').val(data.invoiceProjectId);
                        $('#newRecordButton1').removeClass().addClass('btn btn-success disabled').attr('onClick', '');
                        $('#newRecordButton2').removeClass().addClass('btn dropdown-toggle btn-success disabled');
                        $('#newRecordButton3').attr('onClick', '');
                        $('#newRecordButton4').attr('onClick', '');
...
function updateRecord(leafId, url, urlList, securityToken, type, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        if (type === 1) {
            ...
            $infoPanel.html('').empty();
            $.ajax({type: 'POST', url: url, data: {method: 'save', output: 'json', invoiceProjectId: $invoiceProjectId.val(), referenceNumber: $referenceNumber.val(), invoiceProjectTitle: $invoiceProjectTitle.val(), invoiceProjectStartDate: $invoiceProjectStartDate.val(), invoiceProjectEndDate: $invoiceProjectEndDate.val(), invoiceProjectCurrentStage: $invoiceProjectCurrentStage.val(), invoiceProjectTotalStage: $invoiceProjectTotalStage.val(), invoiceProjectValue: $invoiceProjectValue.val(), invoiceProjectClaim: $invoiceProjectClaim.val(), invoiceProjectBalance: $invoiceProjectBalance.val(), invoiceProjectRetentionValue: $invoiceProjectRetentionValue.val(), invoiceProjectRetentionPercent: $invoiceProjectRetentionPercent.val(), invoiceProjectDescription: $invoiceProjectDescription.val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    var smileyRoll = './images/icons/smiley-roll.png';
                    var $infoPanel = $('#infoPanel');
                    $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                    if ($infoPanel.is(':hidden')) {
    ...
    if (css.search('disabled') > 0) {
        ...
        if (deleteAccess === 1) {
            if (confirm(decodeURIComponent(t['deleteRecordMessageLabel']))) {
                var value = $invoiceProjectId.val();
                if (!value) {
                    ...
                    $.ajax({type: 'POST', url: url, data: {method: 'delete', output: 'json', invoiceProjectId: $invoiceProjectId.val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                            var smileyRoll = './images/icons/smiley-roll.png';
                            var $infoPanel = $('#infoPanel');
                            $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                            if ($infoPanel.is(':hidden')) {
...
function resetRecord(leafId, url, urlList, securityToken, createAccess, updateAccess, deleteAccess) {
    ...
    $("#invoiceProjectIdHelpMe").empty().html('');
    ...
    $("#invoiceProjectTitle").val('');
    $("#invoiceProjectTitleHelpMe").empty().html('');
    $("#invoiceProjectStartDate").val('');
    $("#invoiceProjectStartDateHelpMe").empty().html('');
    $("#invoiceProjectEndDate").val('');
    $("#invoiceProjectEndDateHelpMe").empty().html('');
    $("#invoiceProjectCurrentStage").val('');
    $("#invoiceProjectCurrentStageHelpMe").empty().html('');
    $("#invoiceProjectTotalStage").val('');
    $("#invoiceProjectTotalStageHelpMe").empty().html('');
    $("#invoiceProjectValue").val('');
    $("#invoiceProjectValueHelpMe").empty().html('');
    $("#invoiceProjectClaim").val('');
    $("#invoiceProjectClaimHelpMe").empty().html('');
    $("#invoiceProjectBalance").val('');
    $("#invoiceProjectBalanceHelpMe").empty().html('');
    $("#invoiceProjectRetentionValue").val('');
    $("#invoiceProjectRetentionValueHelpMe").empty().html('');
    $("#invoiceProjectRetentionPercent").val('');
    $("#invoiceProjectRetentionPercentHelpMe").empty().html('');
    $("#invoiceProjectDescription").val('');
    $("#invoiceProjectDescriptionHelpMe").empty().html('');
}
function postRecord() {
...
function firstRecord(leafId, url, urlList, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        $.ajax({type: 'GET', url: url, data: {method: 'dataNavigationRequest', dataNavigation: 'firstRecord', output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                ...
                if (success === true) {
                    $.ajax({type: 'POST', url: url, data: {method: 'read', invoiceProjectId: firstRecord, output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                            var smileyRoll = './images/icons/smiley-roll.png';
                            $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                            if ($infoPanel.is(':hidden')) {
                                $infoPanel.show();
                            ...
                            if (success === true) {
                                $('#invoiceProjectId').val(data.data.invoiceProjectId);
                                $('#documentNumber').val(data.data.documentNumber);
                                $('#referenceNumber').val(data.data.referenceNumber);
                                $('#invoiceProjectTitle').val(data.data.invoiceProjectTitle);
                                x = data.data.invoiceProjectStartDate;
                                x = x.split("-");
                                output = x[2] + "-" + x[1] + "-" + x[0];
                                output = output.toString();
                                $('#invoiceProjectStartDate').val(output);
                                x = data.data.invoiceProjectEndDate;
                                x = x.split("-");
                                output = x[2] + "-" + x[1] + "-" + x[0];
                                output = output.toString();
                                $('#invoiceProjectEndDate').val(output);
                                $('#invoiceProjectCurrentStage').val(data.data.invoiceProjectCurrentStage);
                                $('#invoiceProjectTotalStage').val(data.data.invoiceProjectTotalStage);
                                $('#invoiceProjectValue').val(data.data.invoiceProjectValue);
                                $('#invoiceProjectClaim').val(data.data.invoiceProjectClaim);
                                $('#invoiceProjectBalance').val(data.data.invoiceProjectBalance);
                                $('#invoiceProjectRetentionValue').val(data.data.invoiceProjectRetentionValue);
                                $('#invoiceProjectRetentionPercent').val(data.data.invoiceProjectRetentionPercent);
                                $('#invoiceProjectDescription').val(data.data.invoiceProjectDescription);
                                if (nextRecord > 0) {
                                    $('#previousRecordButton').removeClass().addClass('btn btn-default disabled').attr('onClick', '');
...
function endRecord(leafId, url, urlList, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        $.ajax({type: 'GET', url: url, data: {method: 'dataNavigationRequest', dataNavigation: 'lastRecord', output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                ...
                if (success === true) {
                    $.ajax({type: 'POST', url: url, data: {method: 'read', invoiceProjectId: lastRecord, output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                            var smileyRoll = './images/icons/smiley-roll.png';
                            $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                            if ($infoPanel.is(':hidden')) {
                                $infoPanel.show();
...
function previousRecord(leafId, url, urlList, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($previousRecordCounter.val()) > 0 && parseFloat($previousRecordCounter.val()) < parseFloat($('#lastRecordCounter').val())) {
            $.ajax({type: 'POST', url: url, data: {method: 'read', invoiceProjectId: $previousRecordCounter.val(), output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    var smileyRoll = './images/icons/smiley-roll.png';
                    var $infoPanel = $('#infoPanel');
                    $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                    if ($infoPanel.is(':hidden')) {
...
function nextRecord(leafId, url, urlList, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($nextRecordCounter.val()) <= parseFloat($('#lastRecordCounter').val())) {
            $.ajax({type: 'POST', url: url, data: {method: 'read', invoiceProjectId: $nextRecordCounter.val(), output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    var smileyRoll = './images/icons/smiley-roll.png';
                    var $infoPanel = $('#infoPanel');
                    $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                    if ($infoPanel.is(':hidden')) {
