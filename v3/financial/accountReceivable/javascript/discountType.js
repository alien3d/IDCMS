function checkDuplicate(leafId, page, securityToken) {
    if ($("#discountTypeCode").val().length == 0) {
        alert(t['oddTextLabel']);
        return false;
    }
    $.ajax({type: 'GET', url: page, data: {discountTypeCode: $("#discountTypeCode").val(), method: 'duplicate', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
            $('#infoPanel').empty();
            $('#infoPanel').html('');
            ...
            if (data.success == true) {
                if (data.total != 0) {
                    $("#discountTypeCode").empty();
                    $("#discountTypeCode").val('');
                    $("#discountTypeCode").focus();
                    $("#discountTypeCodeForm").removeClass();
                    $("#discountTypeCodeForm").addClass("col-md-12 form-group has-error");
                    $('#infoPanel').empty();
                    $('#infoPanel').html('');
                ...
                $("#discountTypeForm").removeClass();
                $("#discountTypeForm").addClass("col-md-12 form-group has-error");
            }
        }, error: function(xhr) {
            $('#infoError').empty();
            $('#infoError').html('');
...
function showFormUpdate(leafId, url, urlList, securityToken, discountTypeId, updateAccess, deleteAccess) {
    sleep(500);
    $.ajax({type: 'POST', url: urlList, data: {method: 'read', type: 'form', discountTypeId: discountTypeId, securityToken: securityToken, leafId: leafId}, beforeSend: function() {
            $('#infoPanel').empty();
            $('#infoPanel').html("<span class='label label-warning'><img src='./images/icons/smiley-roll.png'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
            if ($('#infoPanel').is(':hidden')) {
                $('#infoPanel').show();
...
function showModalDelete(discountTypeId, discountTypeCode, discountTypeDescription) {
    $('#discountTypeIdPreview').val('');
    $('#discountTypeIdPreview').val(decodeURIComponent(discountTypeId));
    $('#discountTypeCodePreview').val('');
    $('#discountTypeCodePreview').val(decodeURIComponent(discountTypeCode));
    $('#discountTypeDescriptionPreview').val('');
    $('#discountTypeDescriptionPreview').val(decodeURIComponent(discountTypeDescription));
    showMeModal('deletePreview', 1);
}
function deleteGridRecord(leafId, url, urlList, securityToken) {
    $.ajax({type: 'POST', url: url, data: {method: 'delete', output: 'json', discountTypeId: $('#discountTypeIdPreview').val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
            $('#infoPanel').empty();
            $('#infoPanel').html('');
            $('#infoPanel').html("<span class='label label-warning'><img src='./images/icons/smiley-roll.png'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
            if ($('#infoPanel').is(':hidden')) {
...
function deleteGridRecordCheckbox(leafId, url, urlList, securityToken) {
    var stringText = '';
    var counter = 0;
    $('input:checkbox[name="discountTypeId[]"]').each(function() {
        stringText = stringText + "&discountTypeId[]=" + $(this).val();
    });
...
function newRecord(leafId, url, urlList, securityToken, type, createAccess, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        if (type == 1) {
            if ($('#discountTypeCode').val().length == 0) {
                $('#discountTypeCodeHelpMe').empty();
                $('#discountTypeCodeHelpMe').html("<span class='label label-danger'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['discountTypeCodeLabel'] + " </span>");
                $('#discountTypeCodeForm').addClass('form-group has-error');
                $('#discountTypeCode').focus();
                return false;
            }
            if ($('#discountTypeDescription').val().length == 0) {
                $('#discountTypeDescriptionHelpMe').empty();
                $('#discountTypeDescriptionHelpMe').html("<span class='label label-danger'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['discountTypeDescriptionLabel'] + " </span>");
                $('#discountTypeDescriptionForm').addClass('form-group has-error');
                $('#discountTypeDescription').focus();
                return false;
            }
            $.ajax({type: 'POST', url: url, data: {method: 'create', output: 'json', discountTypeCode: $('#discountTypeCode').val(), discountTypeDescription: $('#discountTypeDescription').val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    $('#infoPanel').empty();
                    $('#infoPanel').html('');
                    $('#infoPanel').html("<span class='label label-warning'>&nbsp;<img src='./images/icons/smiley-roll.png'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                    if ($('#infoPanel').is(':hidden')) {
                    ...
                    if (data.success == true) {
                        ...
                        $('#discountTypeCode').val('');
                        $('#discountTypeCodeHelpMe').empty();
                        $('#discountTypeCodeHelpMe').html('');
                        $('#discountTypeDescription').val('');
                        $('#discountTypeDescriptionForm').removeClass().addClass('col-md-12 form-group');
                        $('#discountTypeDescription').val('');
                        $('#discountTypeDescriptionHelpMe').empty();
                        $('#discountTypeDescriptionHelpMe').html('');
                    } else if (data.success == false) {
                        $('#infoPanel').empty();
                        $('#infoPanel').html("<span class='label label-danger'>&nbsp;" + data.message + "</span>");
                        if ($('#infoPanel').is(':hidden')) {
...
function updateRecord(leafId, url, urlList, securityToken, type, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        if (type == 1) {
            ...
            $('#infoPanel').empty();
            $('#infoPanel').html('');
            $.ajax({type: 'POST', url: url, data: {method: 'save', output: 'json', discountTypeId: $('#discountTypeId').val(), discountTypeCode: $('#discountTypeCode').val(), discountTypeDescription: $('#discountTypeDescription').val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    $('#infoPanel').empty();
                    $('#infoPanel').html('');
                    $('#infoPanel').html("<span class='label label-warning'>&nbsp;<img src='./images/icons/smiley-roll.png'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                    if ($('#infoPanel').is(':hidden')) {
        ...
        if (deleteAccess == 1) {
            if (confirm(decodeURIComponent(t['deleteRecordMessageLabel']))) {
                ...
                if (!value) {
                    ...
                    $.ajax({type: 'POST', url: url, data: {method: 'delete', output: 'json', discountTypeId: $('#discountTypeId').val(), securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                            $('#infoPanel').empty();
                            $('#infoPanel').html('');
                            $('#infoPanel').html("<span class='label label-warning'>&nbsp;<img src='./images/icons/smiley-roll.png'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                            if ($('#infoPanel').is(':hidden')) {
...
function firstRecord(leafId, url, urlList, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        $.ajax({type: 'GET', url: url, data: {method: 'dataNavigationRequest', dataNavigation: 'firstRecord', output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                ...
                if (data.success == true) {
                    $.ajax({type: 'POST', url: url, data: {method: 'read', discountTypeId: data.firstRecord, output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                            $('#infoPanel').empty();
                            $('#infoPanel').html('');
                            $('#infoPanel').html("<span class='label label-warning'>&nbsp;<img src='./images/icons/smiley-roll.png'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                            if ($('#infoPanel').is(':hidden')) {
                            ...
                            if (data.success == true) {
                                ...
                                $('#discountTypeCode').val(data.data.discountTypeCode);
                                $('#discountTypeCode').val(data.data.discountTypeCode);
                                $('#discountTypeDescription').val(data.data.discountTypeDescription);
                                $('#discountTypeDescription').val(data.data.discountTypeDescription);
...
function endRecord(leafId, url, urlList, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        $.ajax({type: 'GET', url: url, data: {method: 'dataNavigationRequest', dataNavigation: 'lastRecord', output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                ...
                if (data.success == true) {
                    $.ajax({type: 'POST', url: url, data: {method: 'read', discountTypeId: data.lastRecord, output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                            $('#infoPanel').empty();
                            $('#infoPanel').html('');
                            $('#infoPanel').html("<span class='label label-warning'>&nbsp;<img src='./images/icons/smiley-roll.png'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                            if ($('#infoPanel').is(':hidden')) {
...
function previousRecord(leafId, url, urlList, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($('#previousRecordCounter').val()) > 0 && parseFloat($('#previousRecordCounter').val()) < parseFloat($('#lastRecordCounter').val())) {
            $.ajax({type: 'POST', url: url, data: {method: 'read', discountTypeId: $('#previousRecordCounter').val(), output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    $('#infoPanel').empty();
                    $('#infoPanel').html('');
                    $('#infoPanel').html("<span class='label label-warning'>&nbsp;<img src='./images/icons/smiley-roll.png'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                    if ($('#infoPanel').is(':hidden')) {
...
function nextRecord(leafId, url, urlList, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($('#nextRecordCounter').val()) <= parseFloat($('#lastRecordCounter').val())) {
            $.ajax({type: 'POST', url: url, data: {method: 'read', discountTypeId: $('#nextRecordCounter').val(), output: 'json', securityToken: securityToken, leafId: leafId}, beforeSend: function() {
                    $('#infoPanel').empty();
                    $('#infoPanel').html('');
                    $('#infoPanel').html("<span class='label label-warning'>&nbsp;<img src='./images/icons/smiley-roll.png'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
                    if ($('#infoPanel').is(':hidden')) {
