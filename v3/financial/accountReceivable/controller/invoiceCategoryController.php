...
namespace Core\Financial\AccountReceivable\InvoiceCategory\Controller;

use Core\ConfigClass;
use Core\Document\Trail\DocumentTrailClass;
use Core\Financial\AccountReceivable\InvoiceCategory\Model\InvoiceCategoryModel;
use Core\Financial\AccountReceivable\InvoiceCategory\Service\InvoiceCategoryService;
...
require_once($newFakeDocumentRoot . "v3/financial/accountReceivable/model/invoiceCategoryModel.php");
require_once($newFakeDocumentRoot . "v3/financial/accountReceivable/service/invoiceCategoryService.php");

/**
 * Class InvoiceCategory
 * this is invoiceCategory controller files.
 * @name IDCMS
 * @version 2
 * @author hafizan
 * @package  Core\Financial\AccountReceivable\InvoiceCategory\Controller
...
class InvoiceCategoryClass extends ConfigClass {
    ...
    /**
     * Model
     * @var \Core\Financial\AccountReceivable\InvoiceCategory\Model\InvoiceCategoryModel
     */
    public $model;
     ...
     * @var \Core\Financial\AccountReceivable\InvoiceCategory\Service\InvoiceCategoryService
     */
    public $service;
    ...
    function __construct() {
        ...
        $this->setViewPath("./v3/financial/accountReceivable/view/invoiceCategory.php");
        $this->setControllerPath("./v3/financial/accountReceivable/controller/invoiceCategoryController.php");
        $this->setServicePath("./v3/financial/accountReceivable/service/invoiceCategoryService.php");
    }
    ...
    function execute() {
        ...
        $this->service = new InvoiceCategoryService();
    ...
    public function create() {
        ...
        if ($this->getVendor() == self::MYSQL) {
            ...
            (
                 ...
                 '" . $this->model->getInvoiceCategoryDescription() . "',
        ...
        $invoiceCategoryId = $this->q->lastInsertId("invoiceCategory");
        $this->q->commit();
        $end = microtime(true);
        $time = $end - $start;
        echo json_encode(
    ...
    public function read() {
        ...
        if (isset($_SESSION['isAdmin'])) {
            if ($_SESSION['isAdmin'] == 0) {
                if ($this->getVendor() == self::MYSQL) {
                    $this->setAuditFilter(
                            " `invoicecategory`.`isActive` = 1  AND `invoicecategory`.`companyId`='" . $this->getCompanyId(
                            ) . "' "
                    );
                } else {
                    if ($this->getVendor() == self::MSSQL) {
                ...
                if ($_SESSION['isAdmin'] == 1) {
                    if ($this->getVendor() == self::MYSQL) {
                        $this->setAuditFilter("   `invoicecategory`.`companyId`='" . $this->getCompanyId() . "'	");
                    } else {
                        if ($this->getVendor() == self::MSSQL) {
                            $this->setAuditFilter(" [invoiceCategory].[companyId]='" . $this->getCompanyId() . "' ");
                        } else {
	...
	JOIN	`company`
	ON		`company`.`companyId` = `invoicecategory`.`companyId`
		  WHERE     " . $this->getAuditFilter();
            if ($this->model->getInvoiceCategoryId(0, 'single')) {
                $sql .= " AND `invoicecategory`.`" . $this->model->getPrimaryKeyName(
                        ) . "`='" . $this->model->getInvoiceCategoryId(0, 'single') . "'";
            }
        } else {
            if ($this->getVendor() == self::MSSQL) {
	...
	ON		[company].[companyId] = [invoiceCategory].[companyId]
		  ...
		  SELECT                    INVOICECATEGORY.INVOICECATEGORYID AS \"invoiceCategoryId\",
                    ...
                    INVOICECATEGORY.COMPANYID AS \"companyId\",
	...
	ON		COMPANY.COMPANYID = INVOICECATEGORY.COMPANYID
         WHERE     " . $this->getAuditFilter();
                    if ($this->model->getInvoiceCategoryId(0, 'single')) {
                        $sql .= " AND INVOICECATEGORY. " . strtoupper(
        ...
        if ($this->getCharacterQuery()) {
            if ($this->getVendor() == self::MYSQL) {
                $sql .= " AND `invoicecategory`.`" . $this->model->getFilterCharacter(
                ...
                if ($this->getVendor() == self::MSSQL) {
                    ...
                    if ($this->getVendor() == self::ORACLE) {
                        $sql .= " AND Initcap(INVOICECATEGORY." . strtoupper(
        ...
        if ($this->getDateRangeStartQuery()) {
            if ($this->getVendor() == self::MYSQL) {
                $sql .= $this->q->dateFilter(
                        'invoicecategory', $this->model->getFilterDate(), $this->getDateRangeStartQuery(), $this->getDateRangeEndQuery(), $this->getDateRangeTypeQuery(), $this->getDateRangeExtraTypeQuery()
                );
            } else {
                if ($this->getVendor() == self::MSSQL) {
                    $sql .= $this->q->dateFilter(
        ...
        if ($this->getVendor() == self::MYSQL) {
            $filterArray = array(
                "`invoicecategory`.`invoiceCategoryId`",
                "`staff`.`staffPassword`"
            );
        } else {
            if ($this->getVendor() == self::MSSQL) {
        ...
        $tableArray = null;
        if ($this->getVendor() == self::MYSQL) {
            $tableArray = array('staff', 'invoicecategory');
        } else {
            if ($this->getVendor() == self::MSSQL) {
                $tableArray = array('staff', 'invoicecategory');
            } else {
        ...
        while (($row = $this->q->fetchAssoc()) == true) {
            ...
            if ($this->model->getInvoiceCategoryId(0, 'single')) {
                ...
                $row['previousRecord'] = $this->previousRecord(
                        'value', $this->model->getInvoiceCategoryId(0, 'single')
                );
                $row['nextRecord'] = $this->nextRecord('value', $this->model->getInvoiceCategoryId(0, 'single'));
    ...
    function update() {
        ...
        if ($this->getVendor() == self::MYSQL) {
           ...
           FROM 	`invoicecategory`
           WHERE  	`" . $this->model->getPrimaryKeyName() . "` = '" . $this->model->getInvoiceCategoryId(
                            0, 'single'
                    ) . "' ";
        } else {
            if ($this->getVendor() == self::MSSQL) {
           ...
           FROM 	INVOICECATEGORY
           WHERE  	" . strtoupper($this->model->getPrimaryKeyName()) . " = '" . $this->model->getInvoiceCategoryId(
                                    0, 'single'
                            ) . "' ";
                }
            }
        ...
        if ($total == 0) {
            ...
            if ($this->getVendor() == self::MYSQL) {
                $sql = "
               UPDATE `invoicecategory` SET
                       `invoiceCategoryCode` = '" . $this->model->getInvoiceCategoryCode() . "',
                       `invoiceCategoryDescription` = '" . $this->model->getInvoiceCategoryDescription() . "',
 ...
 /** 
    ...
    function duplicate() {
        ...
        if ($this->getVendor() == self::MYSQL) {
            $sql = "
           SELECT  `invoiceCategoryCode`
           FROM    `invoicecategory`
           WHERE   `invoiceCategoryCode` 	= 	'" . $this->model->getInvoiceCategoryCode() . "'
           AND     `isActive`  =   1
           AND     `companyId` =   '" . $this->getCompanyId() . "'";
        } else {
            if ($this->getVendor() == self::MSSQL) {
           ...
           AND     [companyId] =	'" . $this->getCompanyId() . "'";
                ...
                if ($this->getVendor() == self::ORACLE) {
                    $sql = "
               SELECT  INVOICECATEGORYCODE as \"invoiceCategoryCode\"
               FROM    INVOICECATEGORY
               WHERE   INVOICECATEGORYCODE	= 	'" . $this->model->getInvoiceCategoryCode() . "'
               AND     ISACTIVE    =   1
               AND     COMPANYID   =   '" . $this->getCompanyId() . "'";
    ...
    function excel() {
        ...
        $this->excel->getActiveSheet()->setCellValue('C3', $this->translate['invoiceCategoryCodeLabel']);
        $this->excel->getActiveSheet()->setCellValue('D3', $this->translate['invoiceCategoryDescriptionLabel']);
        ...
        while (($row = $this->q->fetchAssoc()) == true) {
            ...
            $this->excel->getActiveSheet()->setCellValue('C' . $loopRow, strip_tags($row ['invoiceCategoryCode']));
            $this->excel->getActiveSheet()->setCellValue(
                    'D' . $loopRow, strip_tags($row ['invoiceCategoryDescription'])
            );
        ...
        switch ($this->getReportMode()) {
            case 'excel':
                ...
                $extension = '.xlsx';
                $folder = 'excel';
                $filename = "invoiceCategory" . rand(0, 10000000) . $extension;
...
if (isset($_POST ['method'])) {
    if (isset($_POST['output'])) {
        ...
        if ($_POST['securityToken'] != $invoiceCategoryObject->getSecurityToken()) {
            header('Content-Type:application/json; charset=utf-8');
        ...
        if (isset($_POST ['offset'])) {
            $invoiceCategoryObject->setStart($_POST ['offset']);
        }
        if (isset($_POST ['limit'])) {
            $invoiceCategoryObject->setLimit($_POST ['limit']);
        }
        $invoiceCategoryObject->setPageOutput($_POST['output']);
...
if (isset($_GET ['method'])) {
    $invoiceCategoryObject = new InvoiceCategoryClass ();
    if ($_GET['securityToken'] != $invoiceCategoryObject->getSecurityToken()) {
        header('Content-Type:application/json; charset=utf-8');
        echo json_encode(array("success" => false, "message" => "Something wrong with the system.Hola hackers"));
        exit();
    }
     ...
     */
    if (isset($_GET ['mode'])) {
        $invoiceCategoryObject->setReportMode($_GET['mode']);
    ...
    if (isset($_GET ['filter'])) {
        $invoiceCategoryObject->setServiceOutput('option');
    }
}
?>
