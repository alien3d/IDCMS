...
namespace Core\Financial\AccountReceivable\InvoiceType\Controller;

use Core\ConfigClass;
use Core\Document\Trail\DocumentTrailClass;
use Core\Financial\AccountReceivable\InvoiceType\Model\InvoiceTypeModel;
use Core\Financial\AccountReceivable\InvoiceType\Service\InvoiceTypeService;
...
require_once($newFakeDocumentRoot . "v3/financial/accountReceivable/model/invoiceTypeModel.php");
require_once($newFakeDocumentRoot . "v3/financial/accountReceivable/service/invoiceTypeService.php");

/**
 * Class InvoiceType
 * this is invoiceType controller files.
 * @name IDCMS
 * @version 2
 * @author hafizan
 * @package  Core\Financial\AccountReceivable\InvoiceType\Controller
...
class InvoiceTypeClass extends ConfigClass {
    ...
    /**
     * Model
     * @var \Core\Financial\AccountReceivable\InvoiceType\Model\InvoiceTypeModel
     */
    public $model;
     ...
     * @var \Core\Financial\AccountReceivable\InvoiceType\Service\InvoiceTypeService
     */
    public $service;
    ...
    function __construct() {
        ...
        $this->setViewPath("./v3/financial/accountReceivable/view/invoiceType.php");
        $this->setControllerPath("./v3/financial/accountReceivable/controller/invoiceTypeController.php");
        $this->setServicePath("./v3/financial/accountReceivable/service/invoiceTypeService.php");
    }
    ...
    function execute() {
        ...
        $this->setAudit(1);
        $this->setLog(1);
        $this->model = new InvoiceTypeModel();
        ...
        $this->service = new InvoiceTypeService();
    ...
    public function create() {
        ...
        if ($this->getVendor() == self::MYSQL) {
            ...
            (
                 ...
                 `invoiceTypeCreditLimit`,
                 `invoiceTypeInterestRate`,
                 `invoiceTypeMinimumDeposit`,
                 `isDefault`,
                 `isNew`,
                 `isDraft`,
                 `isUpdate`,
                 ...
                 '" . $this->model->getInvoiceTypeCode() . "',
                 '" . $this->model->getInvoiceTypeDescription() . "',
                 '" . $this->model->getInvoiceTypeCreditLimit() . "',
                 '" . $this->model->getInvoiceTypeInterestRate() . "',
                 '" . $this->model->getInvoiceTypeMinimumDeposit() . "',
        ...
        $invoiceTypeId = $this->q->lastInsertId("invoiceType");
        $this->q->commit();
        $end = microtime(true);
        $time = $end - $start;
        echo json_encode(
    ...
    public function read() {
        ...
        if (isset($_SESSION['isAdmin'])) {
            if ($_SESSION['isAdmin'] == 0) {
                if ($this->getVendor() == self::MYSQL) {
                    $this->setAuditFilter(
                            " `invoicetype`.`isActive` = 1  AND `invoicetype`.`companyId`='" . $this->getCompanyId() . "' "
                    );
                } else {
                    if ($this->getVendor() == self::MSSQL) {
                        $this->setAuditFilter(
                ...
                if ($_SESSION['isAdmin'] == 1) {
                    if ($this->getVendor() == self::MYSQL) {
                        $this->setAuditFilter("   `invoicetype`.`companyId`='" . $this->getCompanyId() . "'	");
                    } else {
                        if ($this->getVendor() == self::MSSQL) {
                            $this->setAuditFilter(" [invoiceType].[companyId]='" . $this->getCompanyId() . "' ");
                        } else {
	...
	JOIN	`company`
	ON		`company`.`companyId` = `invoicetype`.`companyId`
	JOIN	`invoicecategory`
	ON		`invoicecategory`.`invoiceCategoryId` = `invoicetype`.`invoiceCategoryId`
	JOIN	`lateinterest`
	ON		`lateinterest`.`lateInterestId` = `invoicetype`.`lateInterestId`
		  WHERE     " . $this->getAuditFilter();
            if ($this->model->getInvoiceTypeId(0, 'single')) {
                $sql .= " AND `invoicetype`.`" . $this->model->getPrimaryKeyName(
                        ) . "`='" . $this->model->getInvoiceTypeId(0, 'single') . "'";
            }
            if ($this->model->getInvoiceCategoryId()) {
                $sql .= " AND `invoicetype`.`invoiceCategoryId`='" . $this->model->getInvoiceCategoryId() . "'";
            }
            if ($this->model->getLateInterestId()) {
                $sql .= " AND `invoicetype`.`lateInterestId`='" . $this->model->getLateInterestId() . "'";
            }
        } else {
            if ($this->getVendor() == self::MSSQL) {
	...
	ON		[lateInterest].[lateInterestId] = [invoiceType].[lateInterestId]
		  ...
		  SELECT                    INVOICETYPE.INVOICETYPEID AS \"invoiceTypeId\",
                    ...
                    INVOICETYPE.COMPANYID AS \"companyId\",
                    INVOICECATEGORY.INVOICECATEGORYDESCRIPTION AS  \"invoiceCategoryDescription\",
                    INVOICETYPE.INVOICECATEGORYID AS \"invoiceCategoryId\",
                    LATEINTEREST.LATEINTERESTDESCRIPTION AS  \"lateInterestDescription\",
                    INVOICETYPE.LATEINTERESTID AS \"lateInterestId\",
                    INVOICETYPE.INVOICETYPECODE AS \"invoiceTypeCode\",
                    INVOICETYPE.INVOICETYPEDESCRIPTION AS \"invoiceTypeDescription\",
                    INVOICETYPE.INVOICETYPECREDITLIMIT AS \"invoiceTypeCreditLimit\",
	...
	ON		LATEINTEREST.LATEINTERESTID = INVOICETYPE.LATEINTERESTID
         WHERE     " . $this->getAuditFilter();
                    if ($this->model->getInvoiceTypeId(0, 'single')) {
                        $sql .= " AND INVOICETYPE. " . strtoupper(
        ...
        if ($this->getCharacterQuery()) {
            if ($this->getVendor() == self::MYSQL) {
                $sql .= " AND `invoicetype`.`" . $this->model->getFilterCharacter(
                ...
                if ($this->getVendor() == self::MSSQL) {
                    ...
                    if ($this->getVendor() == self::ORACLE) {
                        $sql .= " AND Initcap(INVOICETYPE." . strtoupper(
        ...
        if ($this->getDateRangeStartQuery()) {
            if ($this->getVendor() == self::MYSQL) {
                $sql .= $this->q->dateFilter(
                        'invoicetype', $this->model->getFilterDate(), $this->getDateRangeStartQuery(), $this->getDateRangeEndQuery(), $this->getDateRangeTypeQuery(), $this->getDateRangeExtraTypeQuery()
                );
            } else {
                if ($this->getVendor() == self::MSSQL) {
                    $sql .= $this->q->dateFilter(
        ...
        $tableArray = null;
        if ($this->getVendor() == self::MYSQL) {
            $tableArray = array('staff', 'invoicetype', 'company', 'invoicecategory', 'lateinterest');
        } else {
            if ($this->getVendor() == self::MSSQL) {
                $tableArray = array('staff', 'invoicetype', 'company', 'invoicecategory', 'lateinterest');
            } else {
        ...
        while (($row = $this->q->fetchAssoc()) == true) {
            ...
            if ($this->model->getInvoiceTypeId(0, 'single')) {
                ...
                $row['previousRecord'] = $this->previousRecord('value', $this->model->getInvoiceTypeId(0, 'single'));
                $row['nextRecord'] = $this->nextRecord('value', $this->model->getInvoiceTypeId(0, 'single'));
                            ...
                            array("[", "]"), "", json_encode(
                                    array(
                                        ...
                                        'previousRecord' => $this->previousRecord(
                                                'value', $this->model->getInvoiceTypeId(0, 'single')
                                        ),
                                        'nextRecord' => $this->nextRecord('value', $this->model->getInvoiceTypeId(0, 'single')),
    ...
    function update() {
        ...
        if ($this->getVendor() == self::MYSQL) {
           ...
           FROM 	`invoicetype`
           WHERE  	`" . $this->model->getPrimaryKeyName() . "` = '" . $this->model->getInvoiceTypeId(
                            0, 'single'
                    ) . "' ";
        } else {
            if ($this->getVendor() == self::MSSQL) {
           ...
           FROM 	INVOICETYPE
           WHERE  	" . strtoupper($this->model->getPrimaryKeyName()) . " = '" . $this->model->getInvoiceTypeId(
                                    0, 'single'
                            ) . "' ";
                }
            }
        ...
        if ($total == 0) {
            ...
            if ($this->getVendor() == self::MYSQL) {
                $sql = "
               UPDATE `invoicetype` SET
                       `invoiceCategoryId` = '" . $this->model->getInvoiceCategoryId() . "',
                       `lateInterestId` = '" . $this->model->getLateInterestId() . "',
                       `invoiceTypeCode` = '" . $this->model->getInvoiceTypeCode() . "',
                       `invoiceTypeDescription` = '" . $this->model->getInvoiceTypeDescription() . "',
                       `invoiceTypeCreditLimit` = '" . $this->model->getInvoiceTypeCreditLimit() . "',
                       `invoiceTypeInterestRate` = '" . $this->model->getInvoiceTypeInterestRate() . "',
                       `invoiceTypeMinimumDeposit` = '" . $this->model->getInvoiceTypeMinimumDeposit() . "',
               ...
               WHERE    `invoiceTypeId`='" . $this->model->getInvoiceTypeId('0', 'single') . "'";
            } else {
                if ($this->getVendor() == self::MSSQL) {
                    $sql = "
                UPDATE [invoiceType] SET
 ...
 /** 
    ...
    function duplicate() {
        ...
        if ($this->getVendor() == self::MYSQL) {
           ...
           FROM    `invoicetype`
           WHERE   `invoiceTypeCode` 	= 	'" . $this->model->getInvoiceTypeCode() . "'
           AND     `isActive`  =   1
           AND     `companyId` =   '" . $this->getCompanyId() . "'";
        } else {
            if ($this->getVendor() == self::MSSQL) {
           ...
           AND     [companyId] =	'" . $this->getCompanyId() . "'";
                ...
                if ($this->getVendor() == self::ORACLE) {
                    $sql = "
               SELECT  INVOICETYPECODE as \"invoiceTypeCode\"
               FROM    INVOICETYPE
               WHERE   INVOICETYPECODE	= 	'" . $this->model->getInvoiceTypeCode() . "'
               AND     ISACTIVE    =   1
               AND     COMPANYID   =   '" . $this->getCompanyId() . "'";
    ...
    function excel() {
        ...
        $this->excel->getActiveSheet()->setCellValue('D3', $this->translate['lateInterestIdLabel']);
        $this->excel->getActiveSheet()->setCellValue('E3', $this->translate['invoiceTypeCodeLabel']);
        $this->excel->getActiveSheet()->setCellValue('F3', $this->translate['invoiceTypeDescriptionLabel']);
        $this->excel->getActiveSheet()->setCellValue('G3', $this->translate['invoiceTypeCreditLimitLabel']);
        $this->excel->getActiveSheet()->setCellValue('H3', $this->translate['invoiceTypeInterestRateLabel']);
        $this->excel->getActiveSheet()->setCellValue('I3', $this->translate['invoiceTypeMinimumDepositLabel']);
        ...
        while (($row = $this->q->fetchAssoc()) == true) {
            ...
            $this->excel->getActiveSheet()->setCellValue('C' . $loopRow, strip_tags($row ['invoiceCategoryDesc']));
            $this->excel->getActiveSheet()->setCellValue('D' . $loopRow, strip_tags($row ['lateInterestDesc']));
            $this->excel->getActiveSheet()->setCellValue('E' . $loopRow, strip_tags($row ['invoiceTypeCode']));
            $this->excel->getActiveSheet()->setCellValue('F' . $loopRow, strip_tags($row ['invoiceTypeDescription']));
            ...
            $this->excel->getActiveSheet()->setCellValue('G' . $loopRow, strip_tags($row ['invoiceTypeCreditLimit']));
            $this->excel->getActiveSheet()->getStyle()->getNumberFormat('H' . $loopRow)->setFormatCode(
                    \PHPExcel_Style_NumberFormat::FORMAT_NUMBER_COMMA_SEPARATED1
            );
            $this->excel->getActiveSheet()->setCellValue('H' . $loopRow, strip_tags($row ['invoiceTypeInterestRate']));
            $this->excel->getActiveSheet()->getStyle()->getNumberFormat('I' . $loopRow)->setFormatCode(
                    \PHPExcel_Style_NumberFormat::FORMAT_NUMBER_COMMA_SEPARATED1
            );
            $this->excel->getActiveSheet()->setCellValue(
                    'I' . $loopRow, strip_tags($row ['invoiceTypeMinimumDeposit'])
            );
        ...
        switch ($this->getReportMode()) {
            case 'excel':
                ...
                $extension = '.xlsx';
                $folder = 'excel';
                $filename = "invoiceType" . rand(0, 10000000) . $extension;
...
if (isset($_POST ['method'])) {
    if (isset($_POST['output'])) {
        $invoiceTypeObject = new InvoiceTypeClass ();
        if ($_POST['securityToken'] != $invoiceTypeObject->getSecurityToken()) {
            header('Content-Type:application/json; charset=utf-8');
        ...
        if (isset($_POST ['leafId'])) {
            $invoiceTypeObject->setLeafId($_POST ['leafId']);
        }
        if (isset($_POST ['offset'])) {
            $invoiceTypeObject->setStart($_POST ['offset']);
        }
        if (isset($_POST ['limit'])) {
            $invoiceTypeObject->setLimit($_POST ['limit']);
        }
        $invoiceTypeObject->setPageOutput($_POST['output']);
        $invoiceTypeObject->execute();
        /*
         *  Crud Operation (Create Read Update Delete/Destroy)
         */
        if ($_POST ['method'] == 'create') {
            $invoiceTypeObject->create();
        }
        if ($_POST ['method'] == 'save') {
            $invoiceTypeObject->update();
        }
        if ($_POST ['method'] == 'read') {
            $invoiceTypeObject->read();
        }
        if ($_POST ['method'] == 'delete') {
            $invoiceTypeObject->delete();
        }
        if ($_POST ['method'] == 'posting') {
            //	$invoiceTypeObject->posting();
        }
...
if (isset($_GET ['method'])) {
    $invoiceTypeObject = new InvoiceTypeClass ();
    if ($_GET['securityToken'] != $invoiceTypeObject->getSecurityToken()) {
        header('Content-Type:application/json; charset=utf-8');
        echo json_encode(array("success" => false, "message" => "Something wrong with the system.Hola hackers"));
        exit();
    }
    ...
    if (isset($_GET ['leafId'])) {
        $invoiceTypeObject->setLeafId($_GET ['leafId']);
    }
    /*
     *  Load the dynamic value
     */
    ...
    if ($_GET ['method'] == 'updateStatus') {
        $invoiceTypeObject->updateStatus();
    }
    /*
     *  Checking Any Duplication  Key
     */
    if ($_GET['method'] == 'duplicate') {
        $invoiceTypeObject->duplicate();
    }
    if ($_GET ['method'] == 'dataNavigationRequest') {
        if ($_GET ['dataNavigation'] == 'firstRecord') {
            $invoiceTypeObject->firstRecord('json');
        }
        if ($_GET ['dataNavigation'] == 'previousRecord') {
            $invoiceTypeObject->previousRecord('json', 0);
        }
        if ($_GET ['dataNavigation'] == 'nextRecord') {
            $invoiceTypeObject->nextRecord('json', 0);
        }
        if ($_GET ['dataNavigation'] == 'lastRecord') {
            $invoiceTypeObject->lastRecord('json');
        }
    }
    /*
     * Excel Reporting
     */
    if (isset($_GET ['mode'])) {
        $invoiceTypeObject->setReportMode($_GET['mode']);
        if ($_GET ['mode'] == 'excel' || $_GET ['mode'] == 'pdf' || $_GET['mode'] == 'csv' || $_GET['mode'] == 'html' || $_GET['mode'] == 'excel5' || $_GET['mode'] == 'xml'
        ) {
            $invoiceTypeObject->excel();
        }
    }
    if (isset($_GET ['filter'])) {
        $invoiceTypeObject->setServiceOutput('option');
        if (($_GET['filter'] == 'invoiceCategory')) {
            $invoiceTypeObject->getInvoiceCategory();
        }
        if (($_GET['filter'] == 'lateInterest')) {
            $invoiceTypeObject->getLateInterest();
        }
    }
}
?>
