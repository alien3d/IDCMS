...
require_once($newFakeDocumentRoot . "v3/financial/accountReceivable/controller/invoiceRecurringTypeController.php");
...
if (isset($_POST)) {
    if (isset($_POST['method'])) {
        $invoiceRecurringType = new \Core\Financial\AccountReceivable\InvoiceRecurringType\Controller\InvoiceRecurringTypeClass( );
        define('LIMIT', 10);
        ...
        if (isset($_POST ['query'])) {
            $invoiceRecurringType->setFieldQuery($_POST ['query']);
        }
        if (isset($_POST ['filter'])) {
            $invoiceRecurringType->setGridQuery($_POST ['filter']);
        }
        if (isset($_POST ['character'])) {
            $invoiceRecurringType->setCharacterQuery($_POST['character']);
        }
        if (isset($_POST ['dateRangeStart'])) {
            $invoiceRecurringType->setDateRangeStartQuery($_POST['dateRangeStart']);
            //explode the data to get day,month,year
            $start = explode('-', $_POST ['dateRangeStart']);
            $invoiceRecurringType->setStartDay($start[2]);
            $invoiceRecurringType->setStartMonth($start[1]);
            $invoiceRecurringType->setStartYear($start[0]);
        }
        if (isset($_POST ['dateRangeEnd']) && (strlen($_POST['dateRangeEnd']) > 0)) {
            $invoiceRecurringType->setDateRangeEndQuery($_POST['dateRangeEnd']);
            //explode the data to get day,month,year
            $start = explode('-', $_POST ['dateRangeEnd']);
            $invoiceRecurringType->setEndDay($start[2]);
            $invoiceRecurringType->setEndMonth($start[1]);
            $invoiceRecurringType->setEndYear($start[0]);
        }
        if (isset($_POST ['dateRangeType'])) {
            $invoiceRecurringType->setDateRangeTypeQuery($_POST['dateRangeType']);
        }
        if (isset($_POST ['dateRangeExtraType'])) {
            $invoiceRecurringType->setDateRangeExtraTypeQuery($_POST['dateRangeExtraType']);
        }
        $invoiceRecurringType->setServiceOutput('html');
        $invoiceRecurringType->setLeafId($leafId);
        $invoiceRecurringType->execute();
        if ($_POST['method'] == 'read') {
            $invoiceRecurringType->setStart($offset);
            $invoiceRecurringType->setLimit($limit); // normal system don't like paging..
            $invoiceRecurringType->setPageOutput('html');
            $invoiceRecurringTypeArray = $invoiceRecurringType->read();
            if (isset($invoiceRecurringTypeArray [0]['firstRecord'])) {
                $firstRecord = $invoiceRecurringTypeArray [0]['firstRecord'];
            }
            if (isset($invoiceRecurringTypeArray [0]['nextRecord'])) {
                $nextRecord = $invoiceRecurringTypeArray [0]['nextRecord'];
            }
            if (isset($invoiceRecurringTypeArray [0]['previousRecord'])) {
                $previousRecord = $invoiceRecurringTypeArray [0]['previousRecord'];
            }
            if (isset($invoiceRecurringTypeArray [0]['lastRecord'])) {
                $lastRecord = $invoiceRecurringTypeArray [0]['lastRecord'];
                $endRecord = $invoiceRecurringTypeArray [0]['lastRecord'];
            }
            $navigation = new \Core\Paging\HtmlPaging();
            $navigation->setLeafId($leafId);
            $navigation->setViewPath($invoiceRecurringType->getViewPath());
            ...
            if (isset($invoiceRecurringTypeArray [0]['total'])) {
                $total = $invoiceRecurringTypeArray [0]['total'];
            } else {
                $total = 0;
            }
...
if (isset($_POST['method']) && isset($_POST['type'])) {
            ...
            <div class="row">
                <div class="pull-left btn-group col-xs-10 col-sm-10 col-md-10">
                    <button title="A" class="btn btn-success btn-sm" type="button" 
                            ...
                            echo $invoiceRecurringType->getViewPath();
                        ...
                        <div class="btn-group">
                            ...
                            <ul class="dropdown-menu" style="text-align:left">
                                <li> <a href="javascript:void(0)" onclick="reportRequest(<?php echo $leafId; ?>, '<?php
                                    echo $invoiceRecurringType->getControllerPath();
                ...
                <div class="control-label col-md-1">
                    <div class="pull-right">
                        <button class="delete btn btn-warning btn-sm" type="button" 
                                ...
                                echo $invoiceRecurringType->getControllerPath();
                                ?>', '<?php echo $invoiceRecurringType->getViewPath(); ?>', '<?php echo $securityToken; ?>');"> <i class="glyphicon glyphicon-white glyphicon-trash"></i> </button>
                    </div>
                </div>
            </div>
            <div class="row">
                                            ...
                                            <div class="modal-content">
                                                ...
                                                <div class="modal-body">
                                                    <form class="form-horizontal">
                                                        <input type="hidden" name="invoiceRecurringTypeIdPreview"
                                                               id="invoiceRecurringTypeIdPreview">
                                                        <div class="form-group" id="invoiceRecurringTypeCodeDiv">
                                                            <label for="invoiceRecurringTypeCodePreview"
                                                                   class="control-label col-xs-4 col-sm-4 col-md-4"><?php echo $leafTranslation['invoiceRecurringTypeCodeLabel']; ?></label>
                                                            <div class="col-xs-8 col-sm-8 col-md-8">
                                                                <input type="text" class="form-control"
                                                                       name="invoiceRecurringTypeCodePreview" id="invoiceRecurringTypeCodePreview">
                                                            </div>
                                                        </div>
                                                        <div class="form-group" id="invoiceRecurringTypeDescriptionDiv">
                                                            <label for="invoiceRecurringTypeDescriptionPreview"
                                                                   class="control-label col-xs-4 col-sm-4 col-md-4"><?php echo $leafTranslation['invoiceRecurringTypeDescriptionLabel']; ?></label>
                                                            <div class="col-xs-8 col-sm-8 col-md-8">
                                                                <input type="text" class="form-control"
                                                                       name="invoiceRecurringTypeDescriptionPreview"
                                                                       id="invoiceRecurringTypeDescriptionPreview">
                                                            </div>
                                                        </div>
                                                    </form>
                                                </div>
                                        ...
                                        <div class="col-xs-12 col-sm-12 col-md-12">
                                            <div class="panel panel-default">
                                                <table class="table table table-bordered table-striped table-condensed table-hover smart-form has-tickbox" id="tableData">
                                                    ...
                                                    <th width="100px"> <div align="center"><?php echo ucwords($leafTranslation['invoiceRecurringTypeCodeLabel']); ?></div>
                                                    </th>
                                                    <th><?php echo ucwords($leafTranslation['invoiceRecurringTypeDescriptionLabel']); ?></th>
                                                    <th width="100px"><?php echo ucwords($leafTranslation['executeByLabel']); ?></th>
                                                    <th width="175px" align="center"> <div align="center"><?php echo ucwords($leafTranslation['executeTimeLabel']); ?></div>
                                                    </th>
                                                            ...
                                                            if (is_array($invoiceRecurringTypeArray)) {
                                                                ...
                                                                if ($totalRecord > 0) {
                                                                    ...
                                                                    for ($i = 0; $i < $totalRecord; $i++) {
                                                                        ...
                                                                        ?>
                                                                        <tr <?php
                                                                        if ($invoiceRecurringTypeArray[$i]['isDelete'] == 1) {
                                                                            echo "class=\"danger\"";
                                                                        } else {
                                                                            if ($invoiceRecurringTypeArray[$i]['isDraft'] == 1) {
                                                                                echo "class=\"warning\"";
                                                                            }
                                                                        }
                                                                        ?>>
                                                                            <td align="center"><div align="center"><?php echo($counter + $offset); ?></div></td>
                                                                            <td><?php if ($invoiceRecurringTypeArray [$i]['invoiceRecurringTypeCode'] != 'UNBL') { ?><div class="btn-group">
                                                                                        <button type="button"  class="btn btn-info btn-sm" title="Edit"
                                                                                                ...
                                                                                                echo intval(
                                                                                                        $invoiceRecurringTypeArray [$i]['invoiceRecurringTypeId']
                                                                                                );
                                                                                        ...
                                                                                        <button type="button"  class="btn btn-danger btn-sm" title="Delete"
                                                                                                ...
                                                                                                ?>', '<?php
                                                                                                echo rawurlencode(
                                                                                                        $invoiceRecurringTypeArray [$i]['invoiceRecurringTypeCode']
                                                                                                );
                                                                                                ?>', '<?php
                                                                                                echo rawurlencode(
                                                                                                        $invoiceRecurringTypeArray [$i]['invoiceRecurringTypeDescription']
                                                                                                );
                                                                                                ?>');"> <i class="glyphicon glyphicon-trash glyphicon-white"></i></button>
                                                                                    </div><?php } ?></td>
                                                                            <td><div align="center">
                                                                                    <?php
                                                                                    if (isset($invoiceRecurringTypeArray[$i]['invoiceRecurringTypeCode'])) {
                                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                                if (strpos(
                                                                                                                strtolower($invoiceRecurringTypeArray[$i]['invoiceRecurringTypeCode']), strtolower($_POST['query'])
                                                                                                        ) !== false
                                                                                                ) {
                                                                                                    echo str_replace(
                                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $invoiceRecurringTypeArray[$i]['invoiceRecurringTypeCode']
                                                                                                    );
                                                                                                } else {
                                                                                                    echo $invoiceRecurringTypeArray[$i]['invoiceRecurringTypeCode'];
                                                                                                }
                                                                                            } else {
                                                                                                if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                                    if (strpos(
                                                                                                        ...
                                                                                                        echo str_replace(
                                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $invoiceRecurringTypeArray[$i]['invoiceRecurringTypeCode']
                                                                                                        );
                                                                                                    } else {
                                                                                                        echo $invoiceRecurringTypeArray[$i]['invoiceRecurringTypeCode'];
                                                                                                    }
                                                                            ...
                                                                            <td><div class="pull-left">
                                                                                    <?php
                                                                                    if (isset($invoiceRecurringTypeArray[$i]['invoiceRecurringTypeDescription'])) {
                                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                                if (strpos(
                                                                                                                strtolower(
                                                                                                    ...
                                                                                                    echo str_replace(
                                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $invoiceRecurringTypeArray[$i]['invoiceRecurringTypeDescription']
                                                                                                    );
                                                                                                } else {
                                                                                                    echo $invoiceRecurringTypeArray[$i]['invoiceRecurringTypeDescription'];
                                                                                                }
                                                                                            } else {
                                                                                                if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                                    if (strpos(
                                                                                                        ...
                                                                                                        echo str_replace(
                                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $invoiceRecurringTypeArray[$i]['invoiceRecurringTypeDescription']
                                                                                                        );
                                                                                                    } else {
                                                                                                        echo $invoiceRecurringTypeArray[$i]['invoiceRecurringTypeDescription'];
                                                                                                    }
                                                                            ...
                                                                            <td><div align="center">
                                                                                    <?php
                                                                                    if (isset($invoiceRecurringTypeArray[$i]['executeBy'])) {
                                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                                if (strpos(
                                                                                                                $invoiceRecurringTypeArray[$i]['staffName'], $_POST['query']
                                                                                                        ) !== false
                                                                                                ) {
                                                                                                    echo str_replace(
                                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $invoiceRecurringTypeArray[$i]['staffName']
                                                                                                    );
                                                                                                } else {
                                                                                                    echo $invoiceRecurringTypeArray[$i]['staffName'];
                                                                                                }
                                                                                            } else {
                                                                                                if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                                    if (strpos(
                                                                                                                    $invoiceRecurringTypeArray[$i]['staffName'], $_POST['character']
                                                                                                            ) !== false
                                                                                                    ) {
                                                                                                        echo str_replace(
                                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $invoiceRecurringTypeArray[$i]['staffName']
                                                                                                        );
                                                                                                    } else {
                                                                                                        echo $invoiceRecurringTypeArray[$i]['staffName'];
                                                                                                    }
                                                                                ...
                                                                                <?php } ?></td>
                                                                            <?php
                                                                            if (isset($invoiceRecurringTypeArray[$i]['executeTime'])) {
                                                                                $valueArray = $invoiceRecurringTypeArray[$i]['executeTime'];
                                                                            ...
                                                                            <?php
                                                                            if ($invoiceRecurringTypeArray[$i]['isDelete']) {
                                                                                $checked = "checked";
                                                                            } else {
                                                                                $checked = null;
                                                                            }
                                                                            ?>
                                                                            <td><?php if ($invoiceRecurringTypeArray [$i]['invoiceRecurringTypeCode'] != 'UNBL') { ?><input style="display:none;" type="checkbox" name="invoiceRecurringTypeId[]"
                                                                                                                                                                                value="<?php echo $invoiceRecurringTypeArray[$i]['invoiceRecurringTypeId']; ?>">
                                                                                    <input <?php echo $checked; ?> type="checkbox" name="isDelete[]"
                                                                                                                   value="<?php echo $invoiceRecurringTypeArray[$i]['isDelete']; ?>"><?php } ?></td>
                                                                        </tr>
                                                                        <?php
                                                                    }
                                                                } else {
                                                                    ...
                                                                    <tr>
                                                                        <td colspan="7" valign="top" align="center"><?php
                                                                            $invoiceRecurringType->exceptionMessage(
                        ...
                        <div class="row">
                            <div class="col-xs-12 col-sm-12 col-md-12">
                                <div class="panel panel-info">
                                    ...
                                    <div class="panel-body">
                                        <input type="hidden" name="invoiceRecurringTypeId" id="invoiceRecurringTypeId" value="<?php
                                        if (isset($_POST['invoiceRecurringTypeId'])) {
                                            echo $_POST['invoiceRecurringTypeId'];
                                        }
                                        ?>">
                                        <div class="row">
                                            <div class="col-xs-12 col-sm-12 col-md-12">
                                                <div class="form-group" id="invoiceRecurringTypeCodeForm">
                                                    <label for="invoiceRecurringTypeCode"
                                                           class="control-label col-xs-4 col-sm-4 col-md-4"><strong>
                                                                   <?php
                                                                   echo ucfirst(
                                                                           $leafTranslation['invoiceRecurringTypeCodeLabel']
                                                                   );
                                                                   ?>
                                                        </strong></label>
                                                    <div class="col-xs-8 col-sm-8 col-md-8">
                                                        <input class="form-control" type="text" name="invoiceRecurringTypeCode"
                                                               id="invoiceRecurringTypeCode" value="<?php
                                                               if (isset($invoiceRecurringTypeArray) && is_array($invoiceRecurringTypeArray)) {
                                                                   echo htmlentities($invoiceRecurringTypeArray[0]['invoiceRecurringTypeCode']);
                                                               }
                                                               ?>" maxlength="16">
                                                        <span class="help-block" id="invoiceRecurringTypeCodeHelpMe"></span> </div>
                                                </div>
                                                <div class="form-group" id="invoiceRecurringTypeDescriptionForm">
                                                    <div class="col-xs-12 col-sm-12 col-md-12">
                                                        <textarea class="form-control" name="invoiceRecurringTypeDescription"
                                                                  id="invoiceRecurringTypeDescription"><?php
                                                                      if (isset($invoiceRecurringTypeArray[0]['invoiceRecurringTypeDescription'])) {
                                                                          echo htmlentities($invoiceRecurringTypeArray[0]['invoiceRecurringTypeDescription']);
                                                                      }
                                                                      ?></textarea>
                                                        <span class="help-block" id="invoiceRecurringTypeDescriptionHelpMe"></span> </div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <script type="text/javascript">
                                $(document).ready(function() {
                                    ...
                                    validateMeAlphaNumeric('invoiceRecurringTypeCode');
    <?php if ($_POST['method'] == "new") { ?>
        ...
        <?php if ($leafAccess['leafAccessCreateValue'] == 1) { ?>
                                            ...
                                            $('#newRecordButton1').removeClass().addClass('btn btn-success').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $invoiceRecurringType->getControllerPath(); ?>','<?php echo $invoiceRecurringType->getViewPath(); ?>','<?php echo $securityToken; ?>',1)");
                                            $('#newRecordButton2').removeClass().addClass('btn dropdown-toggle btn-success');


                                            $('#newRecordButton3').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $invoiceRecurringType->getControllerPath(); ?>','<?php echo $invoiceRecurringType->getViewPath(); ?>','<?php echo $securityToken; ?>',1)");
                                            $('#newRecordButton4').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $invoiceRecurringType->getControllerPath(); ?>','<?php echo $invoiceRecurringType->getViewPath(); ?>','<?php echo $securityToken; ?>',2)");
                                            $('#newRecordButton5').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $invoiceRecurringType->getControllerPath(); ?>','<?php echo $invoiceRecurringType->getViewPath(); ?>','<?php echo $securityToken; ?>',3)");
                                            $('#newRecordButton6').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $invoiceRecurringType->getControllerPath(); ?>','<?php echo $invoiceRecurringType->getViewPath(); ?>','<?php echo $securityToken; ?>',4)");
                                            $('#newRecordButton7').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $invoiceRecurringType->getControllerPath(); ?>','<?php echo $invoiceRecurringType->getViewPath(); ?>','<?php echo $securityToken; ?>',5)");
        <?php } else { ?>
        ...
        if ($_POST['invoiceRecurringTypeId']) {
            ?>
            ...
            <?php if ($leafAccess['leafAccessUpdateValue'] == 1) { ?>
                                                $('#updateRecordButton1').removeClass().addClass('btn btn-info').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $invoiceRecurringType->getControllerPath(); ?>','<?php echo $invoiceRecurringType->getViewPath(); ?>','<?php echo $securityToken; ?>',1,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                                                $('#updateRecordButton2').removeClass().addClass('btn dropdown-toggle btn-info');
                                                $('#updateRecordButton3').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $invoiceRecurringType->getControllerPath(); ?>','<?php echo $invoiceRecurringType->getViewPath(); ?>','<?php echo $securityToken; ?>',1,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                                                $('#updateRecordButton4').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $invoiceRecurringType->getControllerPath(); ?>','<?php echo $invoiceRecurringType->getViewPath(); ?>','<?php echo $securityToken; ?>',2,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                                                $('#updateRecordButton5').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $invoiceRecurringType->getControllerPath(); ?>','<?php echo $invoiceRecurringType->getViewPath(); ?>','<?php echo $securityToken; ?>',3,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
            <?php } else { ?>
                                                $('#updateRecordButton1').removeClass().addClass(' btn btn-info disabled').attr('onClick', '');
                                                $('#updateRecordButton2').removeClass().addClass('btn dropdown-toggle btn-info disabled');
            ...
            <?php if ($leafAccess['leafAccessDeleteValue'] == 1) { ?>
                                                ...
                                                $('#deleteRecordButton')
                                                removeClass().addClass('btn btn-danger').attr('onClick', "deleteRecord(<?php echo $leafId; ?>,'<?php echo $invoiceRecurringType->getControllerPath(); ?>','<?php echo $invoiceRecurringType->getViewPath(); ?>','<?php echo $securityToken; ?>',<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
            <?php } else { ?>
    ...
    ?>
            ...
            <script type="text/javascript" src="./v3/financial/accountReceivable/javascript/invoiceRecurringType.js"></script>
            <hr>
            <footer>
                <p>IDCMS 2012/2013</p>
            </footer>