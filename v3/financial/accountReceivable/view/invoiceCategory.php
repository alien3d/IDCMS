...
require_once($newFakeDocumentRoot . "v3/financial/accountReceivable/controller/invoiceCategoryController.php");
...
$translator->setCurrentTable('invoiceCategory');
...
if (isset($_POST)) {
    if (isset($_POST['method'])) {
        $invoiceCategory = new \Core\Financial\AccountReceivable\InvoiceCategory\Controller\InvoiceCategoryClass();
        define('LIMIT', 10);
        ...
        if (isset($_POST ['query'])) {
            $invoiceCategory->setFieldQuery($_POST ['query']);
        }
        if (isset($_POST ['filter'])) {
            $invoiceCategory->setGridQuery($_POST ['filter']);
        }
        if (isset($_POST ['character'])) {
            $invoiceCategory->setCharacterQuery($_POST['character']);
        }
        if (isset($_POST ['dateRangeStart'])) {
            $invoiceCategory->setDateRangeStartQuery($_POST['dateRangeStart']);
            //explode the data to get day,month,year
            $start = explode('-', $_POST ['dateRangeStart']);
            $invoiceCategory->setStartDay($start[2]);
            $invoiceCategory->setStartMonth($start[1]);
            $invoiceCategory->setStartYear($start[0]);
        }
        if (isset($_POST ['dateRangeEnd']) && (strlen($_POST['dateRangeEnd']) > 0)) {
            $invoiceCategory->setDateRangeEndQuery($_POST['dateRangeEnd']);
            //explode the data to get day,month,year
            $start = explode('-', $_POST ['dateRangeEnd']);
            $invoiceCategory->setEndDay($start[2]);
            $invoiceCategory->setEndMonth($start[1]);
            $invoiceCategory->setEndYear($start[0]);
        }
        if (isset($_POST ['dateRangeType'])) {
            $invoiceCategory->setDateRangeTypeQuery($_POST['dateRangeType']);
        }
        if (isset($_POST ['dateRangeExtraType'])) {
            $invoiceCategory->setDateRangeExtraTypeQuery($_POST['dateRangeExtraType']);
        }
        $invoiceCategory->setServiceOutput('html');
        $invoiceCategory->setLeafId($leafId);
        $invoiceCategory->execute();
        if ($_POST['method'] == 'read') {
            $invoiceCategory->setStart($offset);
            $invoiceCategory->setLimit($limit); // normal system don't like paging..
            $invoiceCategory->setPageOutput('html');
            $invoiceCategoryArray = $invoiceCategory->read();
            if (isset($invoiceCategoryArray [0]['firstRecord'])) {
                $firstRecord = $invoiceCategoryArray [0]['firstRecord'];
            }
            if (isset($invoiceCategoryArray [0]['nextRecord'])) {
                $nextRecord = $invoiceCategoryArray [0]['nextRecord'];
            }
            if (isset($invoiceCategoryArray [0]['previousRecord'])) {
                $previousRecord = $invoiceCategoryArray [0]['previousRecord'];
            }
            if (isset($invoiceCategoryArray [0]['lastRecord'])) {
                $lastRecord = $invoiceCategoryArray [0]['lastRecord'];
                $endRecord = $invoiceCategoryArray [0]['lastRecord'];
            }
            $navigation = new \Core\Paging\HtmlPaging();
            $navigation->setLeafId($leafId);
            $navigation->setViewPath($invoiceCategory->getViewPath());
            ...
            if (isset($invoiceCategoryArray [0]['total'])) {
                $total = $invoiceCategoryArray [0]['total'];
            } else {
                $total = 0;
            }
            ...
            <div class="row">
                <div class="pull-left btn-group col-xs-10 col-sm-10 col-md-10">
                    <button title="A" class="btn btn-success btn-sm" type="button" 
                            ...
                            echo $invoiceCategory->getViewPath();
                        ...
                        <div class="btn-group">
                            ...
                            <ul class="dropdown-menu" style="text-align:left">
                                <li> <a href="javascript:void(0)" onclick="reportRequest(<?php echo $leafId; ?>, '<?php
                                    echo $invoiceCategory->getControllerPath();
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <form class="form-horizontal">
                                        <input type="hidden" name="invoiceCategoryIdPreview" id="invoiceCategoryIdPreview">
                                        <div class="form-group" id="invoiceCategoryCodeDiv">
                                            <label for="invoiceCategoryCodePreview"
                                                   class="control-label col-xs-4 col-sm-4 col-md-4"><?php echo $leafTranslation['invoiceCategoryCodeLabel']; ?></label>
                                            <div class="col-xs-8 col-sm-8 col-md-8">
                                                <input class="form-control" type="text" name="invoiceCategoryCodePreview"
                                                       id="invoiceCategoryCodePreview">
                                            </div>
                                        </div>
                                        <div class="form-group" id="invoiceCategoryDescriptionDiv">
                                            <label for="invoiceCategoryDescriptionPreview"
                                                   class="control-label col-xs-4 col-sm-4 col-md-4"><?php echo $leafTranslation['invoiceCategoryDescriptionLabel']; ?></label>
                                            <div class="col-xs-8 col-sm-8 col-md-8">
                                                <input class="form-control" type="text"
                                                       name="invoiceCategoryDescriptionPreview"
                                                       id="invoiceCategoryDescriptionPreview">
                                            </div>
                                        </div>
                                    </form>
                                </div>
                        ...
                        <div class="col-xs-12 col-sm-12 col-md-12">
                            <div class="panel panel-default">
                                <table class="table table table-bordered table-striped table-condensed table-hover smart-form has-tickbox" id="tableData">
                                    ...
                                    <th width="100px"> <div align="center"><?php echo ucwords($leafTranslation['invoiceCategoryCodeLabel']); ?></div>
                                    </th>
                                    <th><?php echo ucwords($leafTranslation['invoiceCategoryDescriptionLabel']); ?></th>
                                            ...
                                            if (is_array($invoiceCategoryArray)) {
                                                ...
                                                if ($totalRecord > 0) {
                                                    ...
                                                    for ($i = 0; $i < $totalRecord; $i++) {
                                                        ...
                                                        ?>
                                                        <tr <?php
                                                        if ($invoiceCategoryArray[$i]['isDelete'] == 1) {
                                                            echo "class=\"danger\"";
                                                        } else {
                                                            if ($invoiceCategoryArray[$i]['isDraft'] == 1) {
                                                                echo "class=\"warning\"";
                                                            }
                                                        }
                                                        ?>>
                                                            <td><div align="center"><?php echo($counter + $offset); ?></div></td>
                                                            <td align="center"><div class="btn-group" align="center">
                                                                    <button type="button"  class="btn btn-info btn-sm" title="Edit"
                                                                            ...
                                                                            echo intval(
                                                                                    $invoiceCategoryArray [$i]['invoiceCategoryId']
                                                                            );
                                                                    ...
                                                                    <button type="button"  class="btn btn-danger btn-sm" title="Delete"
                                                                            ...
                                                                            ?>', '<?php
                                                                            echo rawurlencode(
                                                                                    $invoiceCategoryArray [$i]['invoiceCategoryCode']
                                                                            );
                                                                            ?>', '<?php
                                                                            echo rawurlencode(
                                                                                    $invoiceCategoryArray [$i]['invoiceCategoryDescription']
                                                                            );
                                                                            ?>');"> <i class="glyphicon glyphicon-trash glyphicon-white"></i></button>
                                                                </div></td>
                                                            <td><div align="center">
                                                                    <?php
                                                                    if (isset($invoiceCategoryArray[$i]['invoiceCategoryCode'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query'])) {
                                                                                if (strpos(
                                                                                                strtolower($invoiceCategoryArray[$i]['invoiceCategoryCode']), strtolower($_POST['query'])
                                                                                        ) !== false
                                                                                ) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $invoiceCategoryArray[$i]['invoiceCategoryCode']
                                                                                    );
                                                                                } else {
                                                                                    echo $invoiceCategoryArray[$i]['invoiceCategoryCode'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character'])) {
                                                                                    if (strpos(
                                                                                                    strtolower($invoiceCategoryArray[$i]['invoiceCategoryCode']), strtolower($_POST['character'])
                                                                                            ) !== false
                                                                                    ) {
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $invoiceCategoryArray[$i]['invoiceCategoryCode']
                                                                                        );
                                                                                    } else {
                                                                                        echo $invoiceCategoryArray[$i]['invoiceCategoryCode'];
                                                                                    }
                                                            ...
                                                            <td><div class="pull-left">
                                                                    <?php
                                                                    if (isset($invoiceCategoryArray[$i]['invoiceCategoryDescription'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query'])) {
                                                                                if (strpos(
                                                                                                strtolower($invoiceCategoryArray[$i]['invoiceCategoryDescription']), strtolower($_POST['query'])
                                                                                        ) !== false
                                                                                ) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $invoiceCategoryArray[$i]['invoiceCategoryDescription']
                                                                                    );
                                                                                } else {
                                                                                    echo $invoiceCategoryArray[$i]['invoiceCategoryDescription'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character'])) {
                                                                                    if (strpos(
                                                                                        ...
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $invoiceCategoryArray[$i]['invoiceCategoryDescription']
                                                                                        );
                                                                                    } else {
                                                                                        echo $invoiceCategoryArray[$i]['invoiceCategoryDescription'];
                                                                                    }
                                                            ...
                                                            <td><div align="center">
                                                                    <?php
                                                                    if (isset($invoiceCategoryArray[$i]['executeBy'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query'])) {
                                                                                if (strpos(
                                                                                                $invoiceCategoryArray[$i]['staffName'], $_POST['query']
                                                                                        ) !== false
                                                                                ) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $invoiceCategoryArray[$i]['staffName']
                                                                                    );
                                                                                } else {
                                                                                    echo $invoiceCategoryArray[$i]['staffName'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                    if (strpos(
                                                                                                    $invoiceCategoryArray[$i]['staffName'], $_POST['character']
                                                                                            ) !== false
                                                                                    ) {
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $invoiceCategoryArray[$i]['staffName']
                                                                                        );
                                                                                    } else {
                                                                                        echo $invoiceCategoryArray[$i]['staffName'];
                                                                                    }
                                                                ...
                                                                <?php } ?></td>
                                                            <?php
                                                            if (isset($invoiceCategoryArray[$i]['executeTime'])) {
                                                                $valueArray = $invoiceCategoryArray[$i]['executeTime'];
                                                            ...
                                                            <?php
                                                            if ($invoiceCategoryArray[$i]['isDelete']) {
                                                                $checked = "checked";
                                                            } else {
                                                                $checked = null;
                                                            }
                                                            ?>
                                                            <td><input style="display:none;" type="checkbox" name="invoiceCategoryId[]"
                                                                       value="<?php echo $invoiceCategoryArray[$i]['invoiceCategoryId']; ?>">
                                                                <input <?php echo $checked; ?> type="checkbox" name="isDelete[]"
                                                                                               value="<?php echo $invoiceCategoryArray[$i]['isDelete']; ?>"></td>
                                                        </tr>
                                                        <?php
                                                    }
                                                } else {
                                                    ...
                                                    <tr>
                                                        <td colspan="7" valign="top" align="center"><?php
                                                            $invoiceCategory->exceptionMessage(
                        ...
                        <div class="col-xs-3 col-sm-3 col-md-3 pagination">
                            <div class="pull-right">
                                <button class="delete btn btn-warning" type="button" 
                                        ...
                                        echo $invoiceCategory->getControllerPath();
                                        ?>', '<?php echo $invoiceCategory->getViewPath(); ?>', '<?php echo $securityToken; ?>');"> <i class="glyphicon glyphicon-white glyphicon-trash"></i></button>
                            </div>
                        </div>
                    </div>
                    <script type="text/javascript">
...
if ((isset($_POST['method']) == 'new' || isset($_POST['method']) == 'read') && $_POST['type'] == 'form') {
            ...
            <div class="row">
                <div class="col-xs-12 col-sm-12 col-md-12">
                    <div class="panel panel-info">
                        ...
                        <div class="panel-body">
                            <input type="hidden" name="invoiceCategoryId" id="invoiceCategoryId" value="<?php
                            if (isset($_POST['invoiceCategoryId'])) {
                                echo $_POST['invoiceCategoryId'];
                            }
                            ?>">
                            <div class="row">
                                <div class="col-xs-12 col-sm-12 col-md-12">
                                    <div class="form-group" id="invoiceCategoryCodeForm">
                                        <label for="invoiceCategoryCode"
                                               class="control-label col-xs-2 col-sm-2 col-md-2"><strong>
                                                       <?php
                                                       echo ucfirst(
                                                               $leafTranslation['invoiceCategoryCodeLabel']
                                                       );
                                                       ?>
                                            </strong></label>
                                        <div class="control-label col-xs-4 col-sm-4 col-md-4">
                                            <div class="input-group">
                                                <input type="text" class="form-control" name="invoiceCategoryCode"
                                                       id="invoiceCategoryCode" value="<?php
                                                       if (isset($invoiceCategoryArray) && is_array($invoiceCategoryArray)) {
                                                           if (isset($invoiceCategoryArray[0]['invoiceCategoryCode'])) {
                                                               echo htmlentities($invoiceCategoryArray[0]['invoiceCategoryCode']);
                                                           }
                                                       }
                                                       ?>" maxlength="16">
                                                <span class="input-group-addon"><img src="./images/icons/document-code.png"></span> </div>
                                            <span class="help-block" id="invoiceCategoryCodeHelpMe"></span></div>
                                    </div>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-xs-12 col-sm-12 col-md-12">
                                    <div class="form-group" id="invoiceCategoryDescriptionForm">
                                        <div class="col-xs-12 col-sm-12 col-md-12">
                                            <textarea rows="5" class="form-control" name="invoiceCategoryDescription"
                                                      id="invoiceCategoryDescription"><?php
                                                          if (isset($invoiceCategoryArray[0]['invoiceCategoryDescription'])) {
                                                              echo htmlentities($invoiceCategoryArray[0]['invoiceCategoryDescription']);
                                                          }
                                                          ?></textarea>
                                            <span class="help-block" id="invoiceCategoryDescriptionHelpMe"></span></div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="panel-footer" align="center">
                            <div class="btn-group" align="left"> <a id="newRecordButton1" href="javascript:void(0)" class="btn btn-success disabled"><i
                                ...
                                <ul class="dropdown-menu" style="text-align:left">
                                    <li> <a id="newRecordButton3" href="javascript:void(0)"><i class="glyphicon glyphicon-plus"></i> <?php echo $t['newContinueButtonLabel']; ?> </a></li>
                                    <li> <a id="newRecordButton4" href="javascript:void(0)"><i class="glyphicon glyphicon-edit"></i> <?php echo $t['newUpdateButtonLabel']; ?> </a></li>
                                    ...
                                    <li> <a id="newRecordButton7" href="javascript:void(0)"><i class="glyphicon glyphicon-list"></i> <?php echo $t['newListingButtonLabel']; ?> </a></li>
                                </ul>
                            </div>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            validateMeAlphaNumeric('invoiceCategoryCode');
    <?php if ($_POST['method'] == "new") { ?>
        ...
        <?php if ($leafAccess['leafAccessCreateValue'] == 1) { ?>
                    ...
                    $('#newRecordButton1').removeClass().addClass('btn btn-success').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $invoiceCategory->getControllerPath(); ?>','<?php echo $invoiceCategory->getViewPath(); ?>','<?php echo $securityToken; ?>',1)");
                    $('#newRecordButton2').removeClass().addClass('btn dropdown-toggle btn-success');


                    $('#newRecordButton3').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $invoiceCategory->getControllerPath(); ?>','<?php echo $invoiceCategory->getViewPath(); ?>','<?php echo $securityToken; ?>',1)");
                    $('#newRecordButton4').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $invoiceCategory->getControllerPath(); ?>','<?php echo $invoiceCategory->getViewPath(); ?>','<?php echo $securityToken; ?>',2)");
                    $('#newRecordButton5').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $invoiceCategory->getControllerPath(); ?>','<?php echo $invoiceCategory->getViewPath(); ?>','<?php echo $securityToken; ?>',3)");
                    $('#newRecordButton6').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $invoiceCategory->getControllerPath(); ?>','<?php echo $invoiceCategory->getViewPath(); ?>','<?php echo $securityToken; ?>',4)");
                    $('#newRecordButton7').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $invoiceCategory->getControllerPath(); ?>','<?php echo $invoiceCategory->getViewPath(); ?>','<?php echo $securityToken; ?>',5)");
        <?php } else { ?>
        ...
        if ($_POST['invoiceCategoryId']) {
            ...
            <?php if ($leafAccess['leafAccessUpdateValue'] == 1) { ?>
                        $('#updateRecordButton1').removeClass().addClass('btn btn-info').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $invoiceCategory->getControllerPath(); ?>','<?php echo $invoiceCategory->getViewPath(); ?>','<?php echo $securityToken; ?>',1,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                        $('#updateRecordButton2').removeClass().addClass('btn dropdown-toggle btn-info');
                        $('#updateRecordButton3').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $invoiceCategory->getControllerPath(); ?>','<?php echo $invoiceCategory->getViewPath(); ?>','<?php echo $securityToken; ?>',1,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                        $('#updateRecordButton4').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $invoiceCategory->getControllerPath(); ?>','<?php echo $invoiceCategory->getViewPath(); ?>','<?php echo $securityToken; ?>',2,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                        $('#updateRecordButton5').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $invoiceCategory->getControllerPath(); ?>','<?php echo $invoiceCategory->getViewPath(); ?>','<?php echo $securityToken; ?>',3,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
            <?php } else { ?>
                        $('#updateRecordButton1').removeClass().addClass('btn btn-info disabled').attr('onClick', '');
                        $('#updateRecordButton2').removeClass().addClass('btn dropdown-toggle btn-info disabled');
            ...
            <?php if ($leafAccess['leafAccessDeleteValue'] == 1) { ?>
                        ...
                        $('#deleteRecordButton').removeClass().addClass('btn btn-danger').attr('onClick', "deleteRecord(<?php echo $leafId; ?>,'<?php echo $invoiceCategory->getControllerPath(); ?>','<?php echo $invoiceCategory->getViewPath(); ?>','<?php echo $securityToken; ?>',<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
            <?php } else { ?>
...
<?php } ?>
<script type="text/javascript" src="./v3/financial/accountReceivable/javascript/invoiceCategory.js"></script>
<hr>
<footer>
    <p>IDCMS 2012/2013</p>
</footer>
