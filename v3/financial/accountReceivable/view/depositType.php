...
require_once($newFakeDocumentRoot . "v3/financial/accountReceivable/controller/depositTypeController.php");
...
$translator->setCurrentTable('depositType');
...
$depositTypeArray = array();
if (isset($_POST)) {
    if (isset($_POST['method'])) {
        $depositType = new \Core\Financial\AccountReceivable\DepositType\Controller\DepositTypeClass();
        define('LIMIT', 10);
        ...
        if (isset($_POST ['query'])) {
            $depositType->setFieldQuery($_POST ['query']);
        }
        if (isset($_POST ['filter'])) {
            $depositType->setGridQuery($_POST ['filter']);
        }
        if (isset($_POST ['character'])) {
            $depositType->setCharacterQuery($_POST['character']);
        }
        if (isset($_POST ['dateRangeStart'])) {
            $depositType->setDateRangeStartQuery($_POST['dateRangeStart']);
            //explode the data to get day,month,year
            $start = explode('-', $_POST ['dateRangeStart']);
            $depositType->setStartDay($start[2]);
            $depositType->setStartMonth($start[1]);
            $depositType->setStartYear($start[0]);
        }
        if (isset($_POST ['dateRangeEnd']) && (strlen($_POST['dateRangeEnd']) > 0)) {
            $depositType->setDateRangeEndQuery($_POST['dateRangeEnd']);
            //explode the data to get day,month,year
            $start = explode('-', $_POST ['dateRangeEnd']);
            $depositType->setEndDay($start[2]);
            $depositType->setEndMonth($start[1]);
            $depositType->setEndYear($start[0]);
        }
        if (isset($_POST ['dateRangeType'])) {
            $depositType->setDateRangeTypeQuery($_POST['dateRangeType']);
        }
        if (isset($_POST ['dateRangeExtraType'])) {
            $depositType->setDateRangeExtraTypeQuery($_POST['dateRangeExtraType']);
        }
        $depositType->setServiceOutput('html');
        $depositType->setLeafId($leafId);
        $depositType->execute();
        if ($_POST['method'] == 'read') {
            $depositType->setStart($offset);
            $depositType->setLimit($limit); // normal system don't like paging..
            $depositType->setPageOutput('html');
            $depositTypeArray = $depositType->read();
            if (isset($depositTypeArray [0]['firstRecord'])) {
                $firstRecord = $depositTypeArray [0]['firstRecord'];
            }
            if (isset($depositTypeArray [0]['nextRecord'])) {
                $nextRecord = $depositTypeArray [0]['nextRecord'];
            }
            if (isset($depositTypeArray [0]['previousRecord'])) {
                $previousRecord = $depositTypeArray [0]['previousRecord'];
            }
            if (isset($depositTypeArray [0]['lastRecord'])) {
                $lastRecord = $depositTypeArray [0]['lastRecord'];
                $endRecord = $depositTypeArray [0]['lastRecord'];
            }
            $navigation = new \Core\Paging\HtmlPaging();
            $navigation->setLeafId($leafId);
            $navigation->setViewPath($depositType->getViewPath());
            ...
            if (isset($depositTypeArray [0]['total'])) {
                $total = $depositTypeArray [0]['total'];
            } else {
                $total = 0;
            }
                        ...
                        <div class="btn-group">
                            ...
                            <ul class="dropdown-menu" style="text-align:left">
                                <li> <a href="javascript:void(0)" onclick="reportRequest(<?php echo $leafId; ?>, '<?php
                                    echo $depositType->getControllerPath();
                        ...
                        <div class="modal-dialog">
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                        <input type="hidden" name="depositTypeIdPreview" id="depositTypeIdPreview">
                                        <div class="form-group" id="depositTypeCodeDiv">
                                            <label class="control-label col-xs-4 col-sm-4 col-md-4 col-lg-4" for="depositTypeCodePreview"><?php echo $leafTranslation['depositTypeCodeLabel']; ?></label>
                                            <div class=" col-xs-8 col-sm-8 col-md-8 col-lg-8">
                                                <input class="form-control" type="text" name="depositTypeCodePreview" id="depositTypeCodePreview">
                                            </div>					</div>					<div class="form-group" id="depositTypeDescriptionDiv">
                                            <label class="control-label col-xs-4 col-sm-4 col-md-4 col-lg-4" for="depositTypeDescriptionPreview"><?php echo $leafTranslation['depositTypeDescriptionLabel']; ?></label>
                                            <div class=" col-xs-8 col-sm-8 col-md-8 col-lg-8">
                                                <input class="form-control" type="text" name="depositTypeDescriptionPreview" id="depositTypeDescriptionPreview">
                                            </div>					</div>     		</div> 
                                <div class="modal-footer"> 
                                    <button type="button"  class="btn btn-danger" onClick="deleteGridRecord('<?php echo $leafId; ?>', '<?php echo $depositType->getControllerPath(); ?>', '<?php echo $depositType->getViewPath(); ?>', '<?php echo $securityToken; ?>')" value="<?php echo $t['deleteButtonLabel']; ?>"><?php echo $t['deleteButtonLabel']; ?></button>
                        ...
                        <div class="col-xs-12 col-sm-12 col-md-12">
                            <div class="panel panel-default">
                                <table class="table table table-bordered table-striped table-condensed table-hover smart-form has-tickbox" id="tableData">
                                    ...
                                    <th width="75px"> <div align="center"><?php echo ucwords($leafTranslation['depositTypeCodeLabel']); ?></div>
                                    </th>
                                    <th><?php echo ucwords($leafTranslation['depositTypeDescriptionLabel']); ?></th>
                                    <th width="150px"> <div align="center"><?php echo ucwords($leafTranslation['executeByLabel']); ?></div>
                                    </th>
                                    ...
                                    <tbody id="tableBody">
                                        ...
                                        if ($_POST['method'] == 'read' && $_POST['type'] == 'list' && $_POST['detail'] == 'body') {
                                            if (is_array($depositTypeArray)) {
                                                $totalRecord = intval(count($depositTypeArray));
                                                if ($totalRecord > 0) {
                                                    $counter = 0;
                                                    for ($i = 0; $i < $totalRecord; $i++) {
                                                        ...
                                                        ?>
                                                        <tr <?php
                                                        if ($depositTypeArray[$i]['isDelete'] == 1) {
                                                            echo "class=\"danger\"";
                                                        } else {
                                                            if ($depositTypeArray[$i]['isDraft'] == 1) {
                                                                echo "class=\"warning\"";
                                                            }
                                                        }
                                                        ?>>
                                                            <td align="center"><div align="center"><?php echo($counter + $offset); ?></div></td>
                                                            <td align="center"><?php if ($depositTypeArray [$i]['depositTypeCode'] != 'UNBL') { ?><div class="btn-group" align="center">
                                                                        <button type="button"  class="btn btn-info btn-sm" title="Edit"
                                                                                ...
                                                                                echo intval(
                                                                                        $depositTypeArray [$i]['depositTypeId']
                                                                                );
                                                                        ...
                                                                        <button type="button"  class="btn btn-danger btn-sm" title="Delete"
                                                                                ...
                                                                                ?>', '<?php
                                                                                echo rawurlencode(
                                                                                        $depositTypeArray [$i]['depositTypeCode']
                                                                                );
                                                                                ?>', '<?php
                                                                                echo rawurlencode(
                                                                                        $depositTypeArray [$i]['depositTypeDescription']
                                                                                );
                                                                                ?>');"> <i class="glyphicon glyphicon-trash glyphicon-white"></i></button>
                                                                    </div><?php } ?></td>
                                                            <td><div align="center">
                                                                    <?php
                                                                    if (isset($depositTypeArray[$i]['depositTypeCode'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos(
                                                                                                strtolower($depositTypeArray[$i]['depositTypeCode']), strtolower($_POST['query'])
                                                                                        ) !== false
                                                                                ) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $depositTypeArray[$i]['depositTypeCode']
                                                                                    );
                                                                                } else {
                                                                                    echo $depositTypeArray[$i]['depositTypeCode'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                    if (strpos(
                                                                                                    strtolower($depositTypeArray[$i]['depositTypeCode']), strtolower($_POST['character'])
                                                                                            ) !== false
                                                                                    ) {
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $depositTypeArray[$i]['depositTypeCode']
                                                                                        );
                                                                                    } else {
                                                                                        echo $depositTypeArray[$i]['depositTypeCode'];
                                                                                    }
                                                            ...
                                                            <td><div class="pull-left">
                                                                    <?php
                                                                    if (isset($depositTypeArray[$i]['depositTypeDescription'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos(
                                                                                                strtolower($depositTypeArray[$i]['depositTypeDescription']), strtolower($_POST['query'])
                                                                                        ) !== false
                                                                                ) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $depositTypeArray[$i]['depositTypeDescription']
                                                                                    );
                                                                                } else {
                                                                                    echo $depositTypeArray[$i]['depositTypeDescription'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                    if (strpos(
                                                                                                    strtolower($depositTypeArray[$i]['depositTypeDescription']), strtolower($_POST['character'])
                                                                                            ) !== false
                                                                                    ) {
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $depositTypeArray[$i]['depositTypeDescription']
                                                                                        );
                                                                                    } else {
                                                                                        echo $depositTypeArray[$i]['depositTypeDescription'];
                                                                                    }
                                                            ...
                                                            <td><div align="center">
                                                                    <?php
                                                                    if (isset($depositTypeArray[$i]['executeBy'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos(
                                                                                                $depositTypeArray[$i]['staffName'], $_POST['query']
                                                                                        ) !== false
                                                                                ) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $depositTypeArray[$i]['staffName']
                                                                                    );
                                                                                } else {
                                                                                    echo $depositTypeArray[$i]['staffName'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                    if (strpos(
                                                                                                    $depositTypeArray[$i]['staffName'], $_POST['character']
                                                                                            ) !== false
                                                                                    ) {
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $depositTypeArray[$i]['staffName']
                                                                                        );
                                                                                    } else {
                                                                                        echo $depositTypeArray[$i]['staffName'];
                                                                                    }
                                                                ...
                                                                <?php } ?></td>
                                                            <?php
                                                            if (isset($depositTypeArray[$i]['executeTime'])) {
                                                                $valueArray = $depositTypeArray[$i]['executeTime'];
                                                            ...
                                                            <?php
                                                            if ($depositTypeArray[$i]['isDelete']) {
                                                                $checked = "checked";
                                                            } else {
                                                                $checked = null;
                                                            }
                                                            ?>
                                                            <td><?php if ($depositTypeArray [$i]['depositTypeCode'] != 'UNBL') { ?><input style="display:none;" type="checkbox" name="depositTypeId[]"
                                                                                                                                              value="<?php echo $depositTypeArray[$i]['depositTypeId']; ?>">
                                                                    <input <?php echo $checked; ?> type="checkbox" name="isDelete[]"
                                                                                                   value="<?php echo $depositTypeArray[$i]['isDelete']; ?>"><?php } ?></td>
                                                        </tr>
                                                        <?php
                                                    }
                                                } else {
                        ...
                        <div class="col-xs-3 col-sm-3 col-md-3 pagination">
                            <div class="pull-right">
                                <button class="delete btn btn-warning" type="button" 
                                        ...
                                        echo $depositType->getControllerPath();
                                        ?>', '<?php echo $depositType->getViewPath(); ?>', '<?php echo $securityToken; ?>');"> <i class="glyphicon glyphicon-white glyphicon-trash"></i></button>
                            </div>
                        </div>
                    </div>
                </div>
...
if ((isset($_POST['method']) == 'new' || isset($_POST['method']) == 'read') && $_POST['type'] == 'form') {
            ...
            <div class="row">
                <div class="col-xs-12 col-sm-12 col-md-12">
                    <div class="panel panel-info">
                        ...
                        <div class="panel-body">
                            <input type="hidden" name="depositTypeId" id="depositTypeId" value="<?php
                            if (isset($_POST['depositTypeId'])) {
                                echo $_POST['depositTypeId'];
                            }
                            ?>">
                            <div class="row">
                                <div class="col-xs-12 col-sm-12 col-md-12">
                                    <div class="form-group" id="depositTypeCodeForm">
                                        <label for="depositTypeCode"
                                               class="control-label col-xs-2 col-sm-2 col-md-2"><strong>
                                                       <?php
                                                       echo ucfirst(
                                                               $leafTranslation['depositTypeCodeLabel']
                                                       );
                                                       ?>
                                            </strong></label>
                                        <div class="col-xs-4 col-sm-4 col-md-4">
                                            <input class="form-control" type="text" name="depositTypeCode"
                                                   id="depositTypeCode" value="<?php
                                                   if (isset($depositTypeArray) && is_array($depositTypeArray)) {
                                                       if (isset($depositTypeArray[0]['depositTypeCode'])) {
                                                           echo htmlentities($depositTypeArray[0]['depositTypeCode']);
                                                       }
                                                   }
                                                   ?>" maxlength="16">
                                            <span class="help-block" id="depositTypeCodeHelpMe"></span> </div>
                                    </div>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-xs-12 col-sm-12 col-md-12">
                                    <div class="form-group" id="depositTypeDescriptionForm">
                                        <div class="col-xs-12 col-sm-12 col-md-12">
                                            <textarea class="form-control" name="depositTypeDescription" rows="5"
                                                      id="depositTypeDescription"><?php
                                                          if (isset($depositTypeArray[0]['depositTypeDescription'])) {
                                                              echo htmlentities($depositTypeArray[0]['depositTypeDescription']);
                                                          }
                                                          ?></textarea>
                                            <span class="help-block" id="depositTypeDescriptionHelpMe"></span> </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="panel-footer" align="center">
                            <?php if ($depositTypeArray [$i]['depositTypeCode'] != 'UNBL') { ?>
                                <div align="center">
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            validateMeNumeric('depositTypeId');
            validateMeAlphaNumeric('depositTypeCode');
    <?php if ($_POST['method'] == "new") { ?>
        ...
        <?php if ($leafAccess['leafAccessCreateValue'] == 1) { ?>
                    ...
                    $('#newRecordButton1').removeClass().addClass('btn btn-success').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $depositType->getControllerPath(); ?>','<?php echo $depositType->getViewPath(); ?>','<?php echo $securityToken; ?>',1)");
                    $('#newRecordButton2').removeClass().addClass('btn dropdown-toggle btn-success');


                    $('#newRecordButton3').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $depositType->getControllerPath(); ?>','<?php echo $depositType->getViewPath(); ?>','<?php echo $securityToken; ?>',1)");
                    $('#newRecordButton4').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $depositType->getControllerPath(); ?>','<?php echo $depositType->getViewPath(); ?>','<?php echo $securityToken; ?>',2)");
                    $('#newRecordButton5').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $depositType->getControllerPath(); ?>','<?php echo $depositType->getViewPath(); ?>','<?php echo $securityToken; ?>',3)");
                    $('#newRecordButton6').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $depositType->getControllerPath(); ?>','<?php echo $depositType->getViewPath(); ?>','<?php echo $securityToken; ?>',4)");
                    $('#newRecordButton7').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $depositType->getControllerPath(); ?>','<?php echo $depositType->getViewPath(); ?>','<?php echo $securityToken; ?>',5)");
        <?php } else { ?>
        ...
        if ($_POST['depositTypeId']) {
            ...
            <?php if ($leafAccess['leafAccessUpdateValue'] == 1) { ?>
                        $('#updateRecordButton1').removeClass().addClass('btn btn-info').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $depositType->getControllerPath(); ?>','<?php echo $depositType->getViewPath(); ?>','<?php echo $securityToken; ?>',1,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                        $('#updateRecordButton2').removeClass().addClass('btn dropdown-toggle btn-info');
                        $('#updateRecordButton3').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $depositType->getControllerPath(); ?>','<?php echo $depositType->getViewPath(); ?>','<?php echo $securityToken; ?>',1,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                        $('#updateRecordButton4').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $depositType->getControllerPath(); ?>','<?php echo $depositType->getViewPath(); ?>','<?php echo $securityToken; ?>',2,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                        $('#updateRecordButton5').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $depositType->getControllerPath(); ?>','<?php echo $depositType->getViewPath(); ?>','<?php echo $securityToken; ?>',3,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
            <?php } else { ?>
                        $('#updateRecordButton1').removeClass().addClass(' btn btn-info disabled').attr('onClick', '');
                        $('#updateRecordButton2').removeClass().addClass('btn dropdown-toggle btn-info disabled');
            ...
            <?php if ($leafAccess['leafAccessDeleteValue'] == 1) { ?>
                        ...
                        $('#deleteRecordButton').removeClass().addClass('btn btn-danger').attr('onClick', "deleteRecord(<?php echo $leafId; ?>,'<?php echo $depositType->getControllerPath(); ?>','<?php echo $depositType->getViewPath(); ?>','<?php echo $securityToken; ?>',<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
            <?php } else { ?>
...
<?php } ?>
<script type="text/javascript" src="./v3/financial/accountReceivable/javascript/depositType.js"></script>
<hr>
<footer>
    <p>IDCMS 2012/2013</p>
</footer>
