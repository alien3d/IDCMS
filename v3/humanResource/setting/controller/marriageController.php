...
namespace Core\HumanResource\Setting\Marriage\Controller;

use Core\ConfigClass;
use Core\Document\Trail\DocumentTrailClass;
use Core\HumanResource\Setting\Marriage\Model\MarriageModel;
use Core\HumanResource\Setting\Marriage\Service\MarriageService;
...
require_once($newFakeDocumentRoot . "v3/humanResource/setting/model/marriageModel.php");
require_once($newFakeDocumentRoot . "v3/humanResource/setting/service/marriageService.php");

/**
 * Class Marriage
 * this is marriage controller files.
 * @name IDCMS
 * @version 2
 * @author hafizan
 * @package  Core\HumanResource\Setting\Marriage\Controller
 * @subpackage Setting
 * @link http://www.hafizan.com
 * @license http://www.gnu.org/copyleft/lesser.html LGPL
 */
class MarriageClass extends ConfigClass {
    ...
    /**
     * Model
     * @var \Core\HumanResource\Setting\Marriage\Model\MarriageModel
     */
    public $model;
     ...
     * @var \Core\HumanResource\Setting\Marriage\Service\MarriageService
     */
    public $service;
    ...
    function __construct() {
        ...
        $this->setViewPath("./v3/humanResource/setting/view/marriage.php");
        $this->setControllerPath("./v3/humanResource/setting/controller/marriageController.php");
        $this->setServicePath("./v3/humanResource/setting/service/marriageService.php");
    }
    ...
    function execute() {
        ...
        $this->service = new MarriageService();
    ...
    public function create() {
        ...
        if ($this->getVendor() == self::MYSQL) {
            ...
            (
                 ...
                 '" . $this->model->getMarriageDescription() . "',
    ...
    public function read() {
        ...
        if (isset($_SESSION['isAdmin'])) {
            if ($_SESSION['isAdmin'] == 0) {
                if ($this->getVendor() == self::MYSQL) {
                    $this->setAuditFilter(
                            " `marriage`.`isActive` = 1  AND `marriage`.`companyId`='" . $this->getCompanyId() . "' "
                    );
                } else {
                    if ($this->getVendor() == self::MSSQL) {
                        $this->setAuditFilter(
                ...
                if ($_SESSION['isAdmin'] == 1) {
                    if ($this->getVendor() == self::MYSQL) {
                        $this->setAuditFilter("   `marriage`.`companyId`='" . $this->getCompanyId() . "'	");
                    } else {
                        if ($this->getVendor() == self::MSSQL) {
                            $this->setAuditFilter(" [marriage].[companyId]='" . $this->getCompanyId() . "' ");
                        } else {
	...
	JOIN	`company`
	ON		`company`.`companyId` = `marriage`.`companyId`
		  WHERE     " . $this->getAuditFilter();
            if ($this->model->getMarriageId(0, 'single')) {
                $sql .= " AND `marriage`.`" . $this->model->getPrimaryKeyName() . "`='" . $this->model->getMarriageId(
                                0, 'single'
                        ) . "'";
            }
        } else {
	...
	ON		[company].[companyId] = [marriage].[companyId]
		  WHERE     " . $this->getAuditFilter();
                if ($this->model->getMarriageId(0, 'single')) {
                    $sql .= " AND [marriage].[" . $this->model->getPrimaryKeyName(
                            ) . "]		=	'" . $this->model->getMarriageId(0, 'single') . "'";
                }
            } else {
                if ($this->getVendor() == self::ORACLE) {
		  ...
		  SELECT                    MARRIAGE.MARRIAGEID AS \"marriageId\",
                    ...
                    MARRIAGE.COMPANYID AS \"companyId\",
	...
	ON		COMPANY.COMPANYID = MARRIAGE.COMPANYID
        ...
        if ($this->getCharacterQuery()) {
            if ($this->getVendor() == self::MYSQL) {
                $sql .= " AND `marriage`.`" . $this->model->getFilterCharacter(
                ...
                if ($this->getVendor() == self::MSSQL) {
                    ...
                    if ($this->getVendor() == self::ORACLE) {
                        $sql .= " AND Initcap(MARRIAGE." . strtoupper(
        ...
        if ($this->getDateRangeStartQuery()) {
            if ($this->getVendor() == self::MYSQL) {
                $sql .= $this->q->dateFilter(
                        'marriage', $this->model->getFilterDate(), $this->getDateRangeStartQuery(), $this->getDateRangeEndQuery(), $this->getDateRangeTypeQuery(), $this->getDateRangeExtraTypeQuery()
                );
            } else {
                if ($this->getVendor() == self::MSSQL) {
                    $sql .= $this->q->dateFilter(
        ...
        $tableArray = null;
        if ($this->getVendor() == self::MYSQL) {
            $tableArray = array('staff', 'marriage');
        } else {
            if ($this->getVendor() == self::MSSQL) {
                $tableArray = array('staff', 'marriage');
            } else {
        ...
        while (($row = $this->q->fetchAssoc()) == true) {
            ...
            if ($this->model->getMarriageId(0, 'single')) {
                ...
                $row['previousRecord'] = $this->previousRecord('value', $this->model->getMarriageId(0, 'single'));
                $row['nextRecord'] = $this->nextRecord('value', $this->model->getMarriageId(0, 'single'));
                            ...
                            array("[", "]"), "", json_encode(
                                    array(
                                        ...
                                        'previousRecord' => $this->previousRecord(
                                                'value', $this->model->getMarriageId(0, 'single')
                                        ),
                                        'nextRecord' => $this->nextRecord('value', $this->model->getMarriageId(0, 'single')),
    ...
    function update() {
        ...
        if ($this->getVendor() == self::MYSQL) {
           ...
           FROM 	`marriage`
           WHERE  	`" . $this->model->getPrimaryKeyName() . "` = '" . $this->model->getMarriageId(
                            0, 'single'
                    ) . "' ";
        } else {
            if ($this->getVendor() == self::MSSQL) {
           ...
           FROM 	MARRIAGE
           WHERE  	" . strtoupper($this->model->getPrimaryKeyName()) . " = '" . $this->model->getMarriageId(
                                    0, 'single'
                            ) . "' ";
                }
            }
        ...
        if ($total == 0) {
            ...
            if ($this->getVendor() == self::MYSQL) {
                $sql = "
               UPDATE `marriage` SET
                       `marriageCode` = '" . $this->model->getMarriageCode() . "',
                       `marriageDescription` = '" . $this->model->getMarriageDescription() . "',
   ...
   /** 
    ...
    function duplicate() {
        ...
        if ($this->getVendor() == self::MYSQL) {
           ...
           AND     [companyId] =	'" . $this->getCompanyId() . "'";
                ...
                if ($this->getVendor() == self::ORACLE) {
                    $sql = "
               SELECT  MARRIAGECODE as \"marriageCode\"
               FROM    MARRIAGE
               WHERE   MARRIAGECODE	= 	'" . $this->model->getMarriageCode() . "'
               AND     ISACTIVE    =   1
               AND     COMPANYID   =   '" . $this->getCompanyId() . "'";
    ...
    function excel() {
        ...
        $this->excel->getActiveSheet()->setCellValue('C3', $this->translate['marriageCodeLabel']);
        $this->excel->getActiveSheet()->setCellValue('D3', $this->translate['marriageDescriptionLabel']);
        ...
        while (($row = $this->q->fetchAssoc()) == true) {
            ...
            $this->excel->getActiveSheet()->setCellValue('C' . $loopRow, strip_tags($row ['marriageCode']));
            $this->excel->getActiveSheet()->setCellValue('D' . $loopRow, strip_tags($row ['marriageDescription']));
        ...
        switch ($this->getReportMode()) {
            case 'excel':
                ...
                $extension = '.xlsx';
                $folder = 'excel';
                $filename = "marriage" . rand(0, 10000000) . $extension;
...
if (isset($_POST ['method'])) {
    if (isset($_POST['output'])) {
        ...
        if ($_POST['securityToken'] != $marriageObject->getSecurityToken()) {
            header('Content-Type:application/json; charset=utf-8');
        ...
        $marriageObject->setPageOutput($_POST['output']);
...
if (isset($_GET ['method'])) {
    $marriageObject = new MarriageClass ();
    if ($_GET['securityToken'] != $marriageObject->getSecurityToken()) {
        header('Content-Type:application/json; charset=utf-8');
        echo json_encode(array("success" => false, "message" => "Something wrong with the system.Hola hackers"));
        exit();
    }
    ...
    if (isset($_GET ['databaseRequest'])) {
        $marriageObject->setRequestDatabase($_GET ['databaseRequest']);
    }
    /*
     *  Load the dynamic value
     */
     ...
     */
    if (isset($_GET ['mode'])) {
        $marriageObject->setReportMode($_GET['mode']);
    ...
    if (isset($_GET ['filter'])) {
        $marriageObject->setServiceOutput('option');
    }
}
?>
