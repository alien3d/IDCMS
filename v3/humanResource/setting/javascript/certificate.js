function checkDuplicate(leafId, page, securityToken) {
    var $certificateCode = $("#certificateCode");
    if ($certificateCode.val().length === 0) {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if (success === true) {
                if (total !== 0) {
                    $("#certificateCode").val('').focus();
                    $("#certificateCodeForm").removeClass().addClass("form-group has-error");
                ...
                $("#certificateForm").removeClass().addClass("form-group has-error");
            }
            if ($infoPanel.is(':hidden')) {
                $infoPanel.show();
            }
...
function showFormUpdate(leafId, url, urlList, securityToken, certificateId, updateAccess, deleteAccess) {
    sleep(500);
...
function showModalDelete(certificateId, certificateCode, certificateDescription) {
    $('#certificateIdPreview').val('').val(decodeURIComponent(certificateId));
    $('#certificateCodePreview').val('').val(decodeURIComponent(certificateCode));
    $('#certificateDescriptionPreview').val('').val(decodeURIComponent(certificateDescription));
...
function deleteGridRecord(leafId, url, urlList, securityToken) {
    $.ajax({
        ...
        data: {
            method: 'delete',
            output: 'json',
            certificateId: $('#certificateIdPreview').val(),
            securityToken: securityToken,
            leafId: leafId
        },
        beforeSend: function() {
...
function deleteGridRecordCheckbox(leafId, url, urlList, securityToken) {
    var stringText = '';
    var counter = 0;
    $('input:checkbox[name="certificateId[]"]').each(function() {
        stringText = stringText + "&certificateId[]=" + $(this).val();
    });
...
function newRecord(leafId, url, urlList, securityToken, type, createAccess, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        if (type === 1) {
            if ($certificateCode.val().length === 0) {
                $('#certificateCodeHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['certificateCodeLabel'] + " </span>");
                $('#certificateCodeForm').removeClass().addClass('form-group has-error');
                $certificateCode.focus();
                return false;
            }
            if ($certificateDescription.val().length === 0) {
                $('#certificateDescriptionHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['certificateDescriptionLabel'] + " </span>");
                $('#certificateDescriptionForm').removeClass().addClass('form-group has-error');
            ...
            $.ajax({
                ...
                data: {
                    ...
                    output: 'json',
                    certificateCode: $certificateCode.val(),
                    certificateDescription: $certificateDescription.val(),
                    securityToken: securityToken,
                    leafId: leafId
                },
                beforeSend: function() {
                ...
                success: function(data) {
                    ...
                    if (success === true) {
                        ...
                        $certificateCode.val('');
                        $('#certificateCodeHelpMe').html('').empty();
                        $certificateDescription.val('');
                        $('#certificateDescriptionForm').removeClass().addClass('form-group');
                        $('#certificateDescriptionHelpMe').html('').empty();
                    } else if (success === false) {
                        $infoPanel.html('').empty().html("<span class='label label-important'>&nbsp;" + message + "</span>");
                        if ($infoPanel.is(':hidden')) {
                            $infoPanel.show();
...
function resetRecord(leafId, url, urlList, securityToken, createAccess, updateAccess, deleteAccess) {
    ...
    $("#certificateId").val('');
    $("#certificateIdHelpMe").empty().html('');
    
    $("#certificateCode").val('');
    $("#certificateCodeHelpMe").empty().html('');
    $("#certificateDescription").val('');
    $("#certificateDescriptionHelpMe").empty().html('');
    $('#certificateDescription').empty().val('');
}
function postRecord() {
                ...
                if (success === true) {
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if (success === true) {
                                $('#certificateId').val(data.data.certificateId);
                                $('#certificateCode').val(data.data.certificateCode);
                                $('#certificateDescription').val(data.data.certificateDescription);
                                if (nextRecord > 0) {
                                    $('#previousRecordButton').removeClass().addClass('btn btn-default disabled').attr('onClick', '');
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($previousRecordCounter.val()) > 0 && parseFloat($previousRecordCounter.val()) < parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    certificateId: $previousRecordCounter.val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($nextRecordCounter.val()) <= parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    certificateId: $nextRecordCounter.val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
