function checkDuplicate(leafId, page, securityToken) {
    var $membershipCode = $("#membershipCode");
    if ($membershipCode.val().length === 0) {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if (success === true) {
                if (total !== 0) {
                    $("#membershipCode").val('').focus();
                    $("#membershipCodeForm").removeClass().addClass("form-group has-error");
                ...
                $("#membershipForm").removeClass().addClass("form-group has-error");
            }
            if ($infoPanel.is(':hidden')) {
                $infoPanel.show();
            }
...
function showFormUpdate(leafId, url, urlList, securityToken, membershipId, updateAccess, deleteAccess) {
    sleep(500);
...
function showModalDelete(membershipId, membershipTitle, membershipCode, membershipDescription) {
    $('#membershipIdPreview').val('').val(decodeURIComponent(membershipId));
    $('#membershipTitlePreview').val('').val(decodeURIComponent(membershipTitle));
    $('#membershipCodePreview').val('').val(decodeURIComponent(membershipCode));
    $('#membershipDescriptionPreview').val('').val(decodeURIComponent(membershipDescription));
...
function deleteGridRecord(leafId, url, urlList, securityToken) {
    $.ajax({
        ...
        data: {
            method: 'delete',
            output: 'json',
            membershipId: $('#membershipIdPreview').val(),
            securityToken: securityToken,
            leafId: leafId
        },
        beforeSend: function() {
...
function deleteGridRecordCheckbox(leafId, url, urlList, securityToken) {
    var stringText = '';
    var counter = 0;
    $('input:checkbox[name="membershipId[]"]').each(function() {
        stringText = stringText + "&membershipId[]=" + $(this).val();
    });
...
function newRecord(leafId, url, urlList, securityToken, type, createAccess, updateAccess, deleteAccess) {
    ...
    var $membershipTitle = $('#membershipTitle');
    var $membershipCode = $('#membershipCode');
    var $membershipDescription = $('#membershipDescription');
    if (css.search('disabled') > 0) {
        return false;
    } else {
        if (type === 1) {
            if ($membershipTitle.val().length === 0) {
                $('#membershipTitleHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['membershipTitleLabel'] + " </span>");
                $('#membershipTitleForm').removeClass().addClass('form-group has-error');
                $membershipTitle.focus();
                return false;
            }
            if ($membershipCode.val().length === 0) {
                $('#membershipCodeHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['membershipCodeLabel'] + " </span>");
                $('#membershipCodeForm').removeClass().addClass('form-group has-error');
                $membershipCode.focus();
                return false;
            }
            if ($membershipDescription.val().length === 0) {
                $('#membershipDescriptionHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['membershipDescriptionLabel'] + " </span>");
                $('#membershipDescriptionForm').removeClass().addClass('form-group has-error');
                $membershipDescription.focus();
                return false;
            }
            $.ajax({
                type: 'POST',
                ...
                data: {
                    method: 'create',
                    output: 'json',
                    membershipTitle: $membershipTitle.val(),
                    membershipCode: $membershipCode.val(),
                    membershipDescription: $membershipDescription.val(),
                    securityToken: securityToken,
                    leafId: leafId
                },
                beforeSend: function() {
                ...
                success: function(data) {
                    ...
                    if (success === true) {
                        ...
                        $membershipTitle.val('');
                        $('#membershipTitleHelpMe').html('').empty();
                        $membershipCode.val('');
                        $('#membershipCodeHelpMe').html('').empty();
                        $membershipDescription.val('');
                        $('#membershipDescriptionForm').removeClass().addClass('form-group');
                        $('#membershipDescriptionHelpMe').html('').empty();
                    } else if (success === false) {
                        $infoPanel.html('').empty().html("<span class='label label-important'>&nbsp;" + message + "</span>");
                        if ($infoPanel.is(':hidden')) {
                            $infoPanel.show();
...
function resetRecord(leafId, url, urlList, securityToken, createAccess, updateAccess, deleteAccess) {
    ...
    $("#membershipId").val('');
    $("#membershipIdHelpMe").empty().html('');
    
    $("#membershipTitle").val('');
    $("#membershipTitleHelpMe").empty().html('');
    $("#membershipCode").val('');
    $("#membershipCodeHelpMe").empty().html('');
    $("#membershipDescription").val('');
    $("#membershipDescriptionHelpMe").empty().html('');
    $('#membershipDescription').empty().val('');
}
function postRecord() {
                ...
                if (success === true) {
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if (success === true) {
                                $('#membershipId').val(data.data.membershipId);
                                $('#membershipTitle').val(data.data.membershipTitle);
                                $('#membershipCode').val(data.data.membershipCode);
                                $('#membershipDescription').val(data.data.membershipDescription);
                                if (nextRecord > 0) {
                                    $('#previousRecordButton').removeClass().addClass('btn btn-default disabled').attr('onClick', '');
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($previousRecordCounter.val()) > 0 && parseFloat($previousRecordCounter.val()) < parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    membershipId: $previousRecordCounter.val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($nextRecordCounter.val()) <= parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    membershipId: $nextRecordCounter.val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
