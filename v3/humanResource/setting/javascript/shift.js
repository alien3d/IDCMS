    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if (success === true) {
                if (total !== 0) {
                    $("#shiftCode").val('').focus();
                    $("#shiftCodeForm").removeClass().addClass("form-group has-error");
                ...
                $("#shiftForm").removeClass().addClass("form-group has-error");
            }
            if ($infoPanel.is(':hidden')) {
                $infoPanel.show();
            }
...
function showFormUpdate(leafId, url, urlList, securityToken, shiftId, updateAccess, deleteAccess) {
    sleep(500);
...
function showModalDelete(shiftId, shiftName, shiftCode, shiftStartTime, shiftEndTime, shiftDescription) {
    $('#shiftIdPreview').val('').val(decodeURIComponent(shiftId));
    $('#shiftNamePreview').val('').val(decodeURIComponent(shiftName));
    $('#shiftCodePreview').val('').val(decodeURIComponent(shiftCode));
    $('#shiftStartTimePreview').val('').val(decodeURIComponent(shiftStartTime));
    $('#shiftEndTimePreview').val('').val(decodeURIComponent(shiftEndTime));
    $('#shiftDescriptionPreview').val('').val(decodeURIComponent(shiftDescription));
...
function deleteGridRecordCheckbox(leafId, url, urlList, securityToken) {
    var stringText = '';
    var counter = 0;
    $('input:checkbox[name="shiftId[]"]').each(function() {
        stringText = stringText + "&shiftId[]=" + $(this).val();
    });
...
function newRecord(leafId, url, urlList, securityToken, type, createAccess, updateAccess, deleteAccess) {
    ...
    var $shiftName = $('#shiftName');
    var $shiftCode = $('#shiftCode');
    var $shiftStartTime = $('#shiftStartTime');
    var $shiftEndTime = $('#shiftEndTime');
    var $shiftDescription = $('#shiftDescription');
    if (css.search('disabled') > 0) {
        return false;
    } else {
        if (type === 1) {
            if ($shiftName.val().length === 0) {
                $('#shiftNameHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['shiftNameLabel'] + " </span>");
                $('#shiftNameForm').removeClass().addClass('form-group has-error');
                $shiftName.focus();
                return false;
            }
            if ($shiftCode.val().length === 0) {
                $('#shiftCodeHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['shiftCodeLabel'] + " </span>");
                $('#shiftCodeForm').removeClass().addClass('form-group has-error');
                $shiftCode.focus();
                return false;
            }
            if ($shiftStartTime.val().length === 0) {
                $('#shiftStartTimeHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['shiftStartTimeLabel'] + " </span>");
                $('#shiftStartTimeForm').removeClass().addClass('form-group has-error');
                $shiftStartTime.focus();
                return false;
            }
            if ($shiftEndTime.val().length === 0) {
                $('#shiftEndTimeHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['shiftEndTimeLabel'] + " </span>");
                $('#shiftEndTimeForm').removeClass().addClass('form-group has-error');
                $shiftEndTime.focus();
                return false;
            }
            if ($shiftDescription.val().length === 0) {
                $('#shiftDescriptionHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['shiftDescriptionLabel'] + " </span>");
                $('#shiftDescriptionForm').removeClass().addClass('form-group has-error');
                $shiftDescription.focus();
                return false;
            }
            $.ajax({
                type: 'POST',
                ...
                data: {
                    ...
                    shiftCode: $shiftCode.val(),
                    shiftStartTime: $shiftStartTime.val(),
                    shiftEndTime: $shiftEndTime.val(),
                ...
                success: function(data) {
                    ...
                    if (success === true) {
                        ...
                        $shiftName.val('');
                        $('#shiftNameHelpMe').html('').empty();
                        $shiftCode.val('');
                        $('#shiftCodeHelpMe').html('').empty();
                        $shiftStartTime.val('');
                        $('#shiftStartTimeHelpMe').html('').empty();
                        $shiftEndTime.val('');
                        $('#shiftEndTimeHelpMe').html('').empty();
                        $shiftDescription.val('');
                        $('#shiftDescriptionForm').removeClass().addClass('form-group');
                        $('#shiftDescriptionHelpMe').html('').empty();
                    } else if (success === false) {
                        $infoPanel.html('').empty().html("<span class='label label-important'>&nbsp;" + message + "</span>");
                        if ($infoPanel.is(':hidden')) {
                            $infoPanel.show();
...
function resetRecord(leafId, url, urlList, securityToken, createAccess, updateAccess, deleteAccess) {
    ...
    $("#shiftName").val('');
    $("#shiftNameHelpMe").empty().html('');
    $("#shiftCode").val('');
    $("#shiftCodeHelpMe").empty().html('');
    $("#shiftStartTime").val('');
    $("#shiftStartTimeHelpMe").empty().html('');
    $("#shiftEndTime").val('');
    $("#shiftEndTimeHelpMe").empty().html('');
    $("#shiftDescription").val('');
    $("#shiftDescriptionHelpMe").empty().html('');
    $('#shiftDescription').empty().val('');
}
function postRecord() {
                ...
                if (success === true) {
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if (success === true) {
                                $('#shiftId').val(data.data.shiftId);
                                $('#shiftName').val(data.data.shiftName);
                                $('#shiftCode').val(data.data.shiftCode);
                                $('#shiftStartTime').val(data.data.shiftStartTime);
                                $('#shiftEndTime').val(data.data.shiftEndTime);
                                $('#shiftDescription').val(data.data.shiftDescription);
                                if (nextRecord > 0) {
                                    $('#previousRecordButton').removeClass().addClass('btn btn-default disabled').attr('onClick', '');
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($previousRecordCounter.val()) > 0 && parseFloat($previousRecordCounter.val()) < parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    shiftId: $previousRecordCounter.val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
