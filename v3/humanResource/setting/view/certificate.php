...
require_once($newFakeDocumentRoot . "v3/humanResource/setting/controller/certificateController.php");
...
$translator->setCurrentTable('certificate');
...
if (isset($_POST)) {
    if (isset($_POST['method'])) {
        $certificate = new \Core\HumanResource\Setting\Certificate\Controller\CertificateClass();
        define('LIMIT', 10);
        ...
        if (isset($_POST ['query'])) {
            $certificate->setFieldQuery($_POST ['query']);
        }
        if (isset($_POST ['filter'])) {
            $certificate->setGridQuery($_POST ['filter']);
        }
        if (isset($_POST ['character'])) {
            $certificate->setCharacterQuery($_POST['character']);
        }
        if (isset($_POST ['dateRangeStart'])) {
            $certificate->setDateRangeStartQuery($_POST['dateRangeStart']);
            //explode the data to get day,month,year
            $start = explode('-', $_POST ['dateRangeStart']);
            $certificate->setStartDay($start[2]);
            $certificate->setStartMonth($start[1]);
            $certificate->setStartYear($start[0]);
        }
        if (isset($_POST ['dateRangeEnd']) && (strlen($_POST['dateRangeEnd']) > 0)) {
            $certificate->setDateRangeEndQuery($_POST['dateRangeEnd']);
            //explode the data to get day,month,year
            $start = explode('-', $_POST ['dateRangeEnd']);
            $certificate->setEndDay($start[2]);
            $certificate->setEndMonth($start[1]);
            $certificate->setEndYear($start[0]);
        }
        if (isset($_POST ['dateRangeType'])) {
            $certificate->setDateRangeTypeQuery($_POST['dateRangeType']);
        }
        if (isset($_POST ['dateRangeExtraType'])) {
            $certificate->setDateRangeExtraTypeQuery($_POST['dateRangeExtraType']);
        }
        $certificate->setServiceOutput('html');
        $certificate->setLeafId($leafId);
        $certificate->execute();
        if ($_POST['method'] == 'read') {
            $certificate->setStart($offset);
            $certificate->setLimit($limit); // normal system don't like paging..
            $certificate->setPageOutput('html');
            $certificateArray = $certificate->read();
            if (isset($certificateArray [0]['firstRecord'])) {
                $firstRecord = $certificateArray [0]['firstRecord'];
            }
            if (isset($certificateArray [0]['nextRecord'])) {
                $nextRecord = $certificateArray [0]['nextRecord'];
            }
            if (isset($certificateArray [0]['previousRecord'])) {
                $previousRecord = $certificateArray [0]['previousRecord'];
            }
            if (isset($certificateArray [0]['lastRecord'])) {
                $lastRecord = $certificateArray [0]['lastRecord'];
                $endRecord = $certificateArray [0]['lastRecord'];
            }
            $navigation = new \Core\Paging\HtmlPaging();
            $navigation->setLeafId($leafId);
            $navigation->setViewPath($certificate->getViewPath());
            ...
            if (isset($certificateArray [0]['total'])) {
                $total = $certificateArray [0]['total'];
            } else {
                $total = 0;
            }
                        ...
                        <div class="btn-group">
                            ...
                            <ul class="dropdown-menu" style="text-align:left">
                                <li>
                                    ...
                                    echo $certificate->getControllerPath();
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <form class="form-horizontal">
                                        <input type="hidden" name="certificateIdPreview" id="certificateIdPreview">

                                        <div class="form-group" id="certificateCodeDiv">
                                            <label for="certificateCodePreview"
                                                   class="control-label col-xs-4 col-sm-4 col-md-4"><?php echo $leafTranslation['certificateCodeLabel']; ?></label>

                                            <div class="col-xs-8 col-sm-8 col-md-8">
                                                <input class="form-control" type="text" name="certificateCodePreview"
                                                       id="certificateCodePreview">
                                            </div>
                                        </div>
                                        <div class="form-group" id="certificateDescriptionDiv">
                                            <label for="certificateDescriptionPreview"
                                                   class="control-label col-xs-4 col-sm-4 col-md-4"><?php echo $leafTranslation['certificateDescriptionLabel']; ?></label>

                                            <div class="col-xs-8 col-sm-8 col-md-8">
                                                <input class="form-control" type="text"
                                                       name="certificateDescriptionPreview" id="certificateDescriptionPreview">
                                            </div>
                                        </div>
                                    </form>
                                </div>
                        ...
                        <div class="col-xs-12 col-sm-12 col-md-12">
                            <div class="panel panel-default">
                                <table class="table table table-bordered table-striped table-condensed table-hover smart-form has-tickbox" id="tableData">
                                    ...
                                    <th width="75px">
                                    <div align="center"><?php echo ucwords($leafTranslation['certificateCodeLabel']); ?></div>
                                    </th>
                                    <th><?php echo ucwords($leafTranslation['certificateDescriptionLabel']); ?></th>
                                    ...
                                    <tbody id="tableBody">
                                        ...
                                        if ($_POST['method'] == 'read' && $_POST['type'] == 'list' && $_POST['detail'] == 'body') {
                                            if (is_array($certificateArray)) {
                                                $totalRecord = intval(count($certificateArray));
                                                if ($totalRecord > 0) {
                                                    $counter = 0;
                                                    for ($i = 0; $i < $totalRecord; $i++) {
                                                        ...
                                                        ?>
                                                        <tr <?php
                                                        if ($certificateArray[$i]['isDelete'] == 1) {
                                                            echo "class=\"danger\"";
                                                        } else {
                                                            if ($certificateArray[$i]['isDraft'] == 1) {
                                                                echo "class=\"warning\"";
                                                            }
                                                        }
                                                        ?>>
                                                            ...
                                                            <td align="center">
                                                                <div class="btn-group" align="center">
                                                                    <button type="button"  class="btn btn-info btn-sm" title="Edit"
                                                                            ...
                                                                            echo intval(
                                                                                    $certificateArray [$i]['certificateId']
                                                                            );
                                                                    ...
                                                                    <button type="button"  class="btn btn-danger btn-sm" title="Delete"
                                                                            ...
                                                                            ?>', '<?php
                                                                            echo rawurlencode(
                                                                                    $certificateArray [$i]['certificateCode']
                                                                            );
                                                                            ?>', '<?php
                                                                            echo rawurlencode(
                                                                                    $certificateArray [$i]['certificateDescription']
                                                                            );
                                                                            ?>');">
                                                            ...
                                                            <td>
                                                                <div align="center">
                                                                    <?php
                                                                    if (isset($certificateArray[$i]['certificateCode'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos(
                                                                                                strtolower($certificateArray[$i]['certificateCode']), strtolower($_POST['query'])
                                                                                        ) !== false
                                                                                ) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $certificateArray[$i]['certificateCode']
                                                                                    );
                                                                                } else {
                                                                                    echo $certificateArray[$i]['certificateCode'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                    if (strpos(
                                                                                                    strtolower($certificateArray[$i]['certificateCode']), strtolower($_POST['character'])
                                                                                            ) !== false
                                                                                    ) {
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $certificateArray[$i]['certificateCode']
                                                                                        );
                                                                                    } else {
                                                                                        echo $certificateArray[$i]['certificateCode'];
                                                                                    }
                                                            ...
                                                            <td>
                                                                <div class="pull-left">
                                                                    <?php
                                                                    if (isset($certificateArray[$i]['certificateDescription'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos(
                                                                                                strtolower($certificateArray[$i]['certificateDescription']), strtolower($_POST['query'])
                                                                                        ) !== false
                                                                                ) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $certificateArray[$i]['certificateDescription']
                                                                                    );
                                                                                } else {
                                                                                    echo $certificateArray[$i]['certificateDescription'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                    if (strpos(
                                                                                                    strtolower($certificateArray[$i]['certificateDescription']), strtolower($_POST['character'])
                                                                                            ) !== false
                                                                                    ) {
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $certificateArray[$i]['certificateDescription']
                                                                                        );
                                                                                    } else {
                                                                                        echo $certificateArray[$i]['certificateDescription'];
                                                                                    }
                                                            ...
                                                            <td>
                                                                <div class="pull-left">
                                                                    <?php
                                                                    if (isset($certificateArray[$i]['executeBy'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos($certificateArray[$i]['staffName'], $_POST['query']) !== false) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $certificateArray[$i]['staffName']
                                                                                    );
                                                                                } else {
                                                                                    echo $certificateArray[$i]['staffName'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                    if (strpos(
                                                                                                    $certificateArray[$i]['staffName'], $_POST['character']
                                                                                            ) !== false
                                                                                    ) {
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $certificateArray[$i]['staffName']
                                                                                        );
                                                                                    } else {
                                                                                        echo $certificateArray[$i]['staffName'];
                                                                                    }
                                                            ...
                                                            <?php
                                                            if (isset($certificateArray[$i]['executeTime'])) {
                                                                $valueArray = $certificateArray[$i]['executeTime'];
                                                            ...
                                                            <?php } ?>
                                                            <?php
                                                            if ($certificateArray[$i]['isDelete']) {
                                                                $checked = "checked";
                                                            } else {
                                                                $checked = null;
                                                            }
                                                            ?>
                                                            <td vAlign="top">
                                                                <input style="display:none;" type="checkbox" name="certificateId[]"
                                                                       value="<?php echo $certificateArray[$i]['certificateId']; ?>">
                                                                <input <?php echo $checked; ?> type="checkbox" name="isDelete[]"
                                                                                               value="<?php echo $certificateArray[$i]['isDelete']; ?>">
                                                            </td>
                                                        </tr>
                                                        <?php
                                                    }
                        ...
                        <div class="col-xs-3 col-sm-3 col-md-3 pagination">
                            <div class="pull-right">
                                <button class="delete btn btn-warning btn-sm" type="button" 
                                        ...
                                        echo $certificate->getControllerPath();
                                        ?>', '<?php echo $certificate->getViewPath(); ?>', '<?php echo $securityToken; ?>');">
...
if ((isset($_POST['method']) == 'new' || isset($_POST['method']) == 'read') && $_POST['type'] == 'form') {
            ...
            <div class="row">
                <div class="col-xs-12 col-sm-12 col-md-12">
                    <div class="panel panel-info">
                        ...
                        <div class="panel-body">
                            <input type="hidden" name="certificateId" id="certificateId" value="<?php
                            if (isset($_POST['certificateId'])) {
                                echo $_POST['certificateId'];
                            }
                            ?>">

                            <div class="form-group" id="certificateCodeForm">
                                <label for="certificateCode" class="control-label col-xs-2 col-sm-2 col-md-2"><strong><?php
                                        echo ucfirst(
                                                $leafTranslation['certificateCodeLabel']
                                        );
                                        ?></strong></label>

                                <div class="col-xs-10 col-sm-10 col-md-10">
                                    <div class="input-group">
                                        <input type="text" class="form-control" name="certificateCode" id="certificateCode"
                                               onKeyUp="removeMeError('certificateCode');" value="<?php
                                               if (isset($certificateArray) && is_array($certificateArray)) {
                                                   if (isset($certificateArray[0]['certificateCode'])) {
                                                       echo htmlentities($certificateArray[0]['certificateCode']);
                                                   }
                                               }
                                               ?>" maxlength="16"> <span class="input-group-addon"><img
                                                src="./images/icons/document-code.png"></span>
                                    </div>
                                    <span class="help-block" id="certificateCodeHelpMe"></span>
                                </div>
                            </div>
                            <div class="form-group" id="certificateDescriptionForm">
                                <div class="col-xs-12 col-sm-12 col-md-12">
                                    <textarea name="certificateDescription" id="certificateDescription" rows="5"
                                              class="form-control"><?php
                                                  if (isset($certificateArray[0]['certificateDescription'])) {
                                                      echo htmlentities($certificateArray[0]['certificateDescription']);
                                                  }
                                                  ?></textarea> <span class="help-block" id="certificateDescriptionHelpMe"></span>
                                </div>
                            </div>
                        </div>
    ...
    <?php if ($leafAccess['leafAccessCreateValue'] == 1) { ?>
                            newRecord(<?php echo $leafId; ?>, '<?php echo $certificate->getControllerPath(); ?>', '<?php echo $certificate->getViewPath(); ?>', '<?php echo $securityToken; ?>', 1);
    <?php } ?>
                        return false;
                    }
                    // shift+s save event
    ...
    <?php if ($leafAccess['leafAccessUpdateValue'] == 1) { ?>
                            updateRecord(<?php echo $leafId; ?>, '<?php echo $certificate->getControllerPath(); ?>', '<?php echo $certificate->getViewPath(); ?>', '<?php echo $securityToken; ?>', 1, <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
    <?php } ?>
                        return false;
                    }
    ...
    <?php if ($leafAccess['leafAccessDeleteValue'] == 1) { ?>
                            deleteRecord(<?php echo $leafId; ?>, '<?php echo $certificate->getControllerPath(); ?>', '<?php echo $certificate->getViewPath(); ?>', '<?php echo $securityToken; ?>', <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
    ...
    <?php } ?>
                    ...
                    switch (e.keyCode) {
                        case 37:
                            previousRecord(<?php echo $leafId; ?>, '<?php echo $certificate->getControllerPath(); ?>', '<?php echo $certificate->getViewPath(); ?>', '<?php echo $securityToken; ?>', <?php echo $leafAccess['leafAccessUpdateValue']; ?>, <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
                            
                            return false;
                            break;
                        case 39:
                            nextRecord(<?php echo $leafId; ?>, '<?php echo $certificate->getControllerPath(); ?>', '<?php echo $certificate->getViewPath(); ?>', '<?php echo $securityToken; ?>', <?php echo $leafAccess['leafAccessUpdateValue']; ?>, <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
                ...
                $(document).ready(function() {
                    ...
                    validateMeNumeric('certificateId');
                    validateMeAlphaNumeric('certificateCode');
    <?php if ($_POST['method'] == "new") { ?>
                        $('#resetRecordButton').removeClass().addClass('btn btn-default');
        <?php if ($leafAccess['leafAccessCreateValue'] == 1) { ?>
                            $('#newRecordButton1').removeClass().addClass('btn btn-success').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $certificate->getControllerPath(); ?>','<?php echo $certificate->getViewPath(); ?>','<?php echo $securityToken; ?>',1)");
                            $('#newRecordButton2').removeClass().addClass('btn dropdown-toggle btn-success');
                            $('#newRecordButton3').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $certificate->getControllerPath(); ?>','<?php echo $certificate->getViewPath(); ?>','<?php echo $securityToken; ?>',1)");
                            $('#newRecordButton4').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $certificate->getControllerPath(); ?>','<?php echo $certificate->getViewPath(); ?>','<?php echo $securityToken; ?>',2)");
                            $('#newRecordButton5').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $certificate->getControllerPath(); ?>','<?php echo $certificate->getViewPath(); ?>','<?php echo $securityToken; ?>',3)");
                            $('#newRecordButton6').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $certificate->getControllerPath(); ?>','<?php echo $certificate->getViewPath(); ?>','<?php echo $securityToken; ?>',4)");
                            $('#newRecordButton7').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $certificate->getControllerPath(); ?>','<?php echo $certificate->getViewPath(); ?>','<?php echo $securityToken; ?>',5)");
        <?php } else { ?>
        ...
        if ($_POST['certificateId']) {
            ...
            <?php if ($leafAccess['leafAccessUpdateValue'] == 1) { ?>
                                $('#updateRecordButton1').removeClass().addClass('btn btn-info').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $certificate->getControllerPath(); ?>','<?php echo $certificate->getViewPath(); ?>','<?php echo $securityToken; ?>',1,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                                $('#updateRecordButton2').removeClass().addClass('btn dropdown-toggle btn-info');
                                $('#updateRecordButton3').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $certificate->getControllerPath(); ?>','<?php echo $certificate->getViewPath(); ?>','<?php echo $securityToken; ?>',1,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                                $('#updateRecordButton4').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $certificate->getControllerPath(); ?>','<?php echo $certificate->getViewPath(); ?>','<?php echo $securityToken; ?>',2,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                                $('#updateRecordButton5').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $certificate->getControllerPath(); ?>','<?php echo $certificate->getViewPath(); ?>','<?php echo $securityToken; ?>',3,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
            <?php } else { ?>
                                $('#updateRecordButton1').removeClass().addClass(' btn btn-info disabled').attr('onClick', '');
                                $('#updateRecordButton2').removeClass().addClass('btn dropdown-toggle btn-info disabled');
                                $('#updateRecordButton3').attr('onClick', '');
            ...
            <?php } ?>
            <?php if ($leafAccess['leafAccessDeleteValue'] == 1) { ?>
                                $('#deleteRecordButton').removeClass().addClass('btn btn-danger').attr('onClick', "deleteRecord(<?php echo $leafId; ?>,'<?php echo $certificate->getControllerPath(); ?>','<?php echo $certificate->getViewPath(); ?>','<?php echo $securityToken; ?>',<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
            <?php } else { ?>
                                $('#deleteRecordButton').removeClass().addClass('btn btn-danger disabled').attr('onClick', '');
            <?php } ?>
            <?php
...
<?php } ?>
<script type="text/javascript" src="./v3/humanResource/setting/javascript/certificate.js"></script>
<hr>
