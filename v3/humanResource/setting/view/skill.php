...
require_once($newFakeDocumentRoot . "v3/humanResource/setting/controller/skillController.php");
...
$translator->setCurrentTable('skill');
...
if (isset($_POST)) {
    if (isset($_POST['method'])) {
        $skill = new \Core\HumanResource\Setting\Skill\Controller\SkillClass();
        define('LIMIT', 10);
        ...
        if (isset($_POST ['query'])) {
            $skill->setFieldQuery($_POST ['query']);
        }
        if (isset($_POST ['filter'])) {
            $skill->setGridQuery($_POST ['filter']);
        }
        if (isset($_POST ['character'])) {
            $skill->setCharacterQuery($_POST['character']);
        }
        if (isset($_POST ['dateRangeStart'])) {
            $skill->setDateRangeStartQuery($_POST['dateRangeStart']);
            //explode the data to get day,month,year
            $start = explode('-', $_POST ['dateRangeStart']);
            $skill->setStartDay($start[2]);
            $skill->setStartMonth($start[1]);
            $skill->setStartYear($start[0]);
        }
        if (isset($_POST ['dateRangeEnd']) && (strlen($_POST['dateRangeEnd']) > 0)) {
            $skill->setDateRangeEndQuery($_POST['dateRangeEnd']);
            //explode the data to get day,month,year
            $start = explode('-', $_POST ['dateRangeEnd']);
            $skill->setEndDay($start[2]);
            $skill->setEndMonth($start[1]);
            $skill->setEndYear($start[0]);
        }
        if (isset($_POST ['dateRangeType'])) {
            $skill->setDateRangeTypeQuery($_POST['dateRangeType']);
        }
        if (isset($_POST ['dateRangeExtraType'])) {
            $skill->setDateRangeExtraTypeQuery($_POST['dateRangeExtraType']);
        }
        $skill->setServiceOutput('html');
        $skill->setLeafId($leafId);
        $skill->execute();
        if ($_POST['method'] == 'read') {
            $skill->setStart($offset);
            $skill->setLimit($limit); // normal system don't like paging..
            $skill->setPageOutput('html');
            $skillArray = $skill->read();
            if (isset($skillArray [0]['firstRecord'])) {
                $firstRecord = $skillArray [0]['firstRecord'];
            }
            if (isset($skillArray [0]['nextRecord'])) {
                $nextRecord = $skillArray [0]['nextRecord'];
            }
            if (isset($skillArray [0]['previousRecord'])) {
                $previousRecord = $skillArray [0]['previousRecord'];
            }
            if (isset($skillArray [0]['lastRecord'])) {
                $lastRecord = $skillArray [0]['lastRecord'];
                $endRecord = $skillArray [0]['lastRecord'];
            }
            $navigation = new \Core\Paging\HtmlPaging();
            $navigation->setLeafId($leafId);
            $navigation->setViewPath($skill->getViewPath());
            ...
            if (isset($skillArray [0]['total'])) {
                $total = $skillArray [0]['total'];
            } else {
                $total = 0;
            }
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <form class="form-horizontal">
                                        <input type="hidden" name="skillIdPreview" id="skillIdPreview">

                                        <div class="form-group" id="skillCodeDiv">
                                            <label for="skillCodePreview"
                                                   class="control-label col-xs-4 col-sm-4 col-md-4"><?php echo $leafTranslation['skillCodeLabel']; ?></label>

                                            <div class="col-xs-8 col-sm-8 col-md-8">
                                                <input class="form-control" type="text" name="skillCodePreview"
                                                       id="skillCodePreview">
                                            </div>
                                        </div>
                                        <div class="form-group" id="skillDescriptionDiv">
                                            <label for="skillDescriptionPreview"
                                                   class="control-label col-xs-4 col-sm-4 col-md-4"><?php echo $leafTranslation['skillDescriptionLabel']; ?></label>

                                            <div class="col-xs-8 col-sm-8 col-md-8">
                                                <input class="form-control" type="text" name="skillDescriptionPreview"
                                                       id="skillDescriptionPreview">
                                            </div>
                                        </div>
                                    </form>
                                </div>
                        ...
                        <div class="col-xs-12 col-sm-12 col-md-12">
                            <div class="panel panel-default">
                                <table class="table table table-bordered table-striped table-condensed table-hover smart-form has-tickbox" id="tableData">
                                    ...
                                    <th width="75px">
                                    <div align="center"><?php echo ucwords($leafTranslation['skillCodeLabel']); ?></div>
                                    </th>
                                    <th><?php echo ucwords($leafTranslation['skillDescriptionLabel']); ?></th>
                                    <th width="100px">
                                    ...
                                    <tbody id="tableBody">
                                        ...
                                        if ($_POST['method'] == 'read' && $_POST['type'] == 'list' && $_POST['detail'] == 'body') {
                                            if (is_array($skillArray)) {
                                                $totalRecord = intval(count($skillArray));
                                                if ($totalRecord > 0) {
                                                    $counter = 0;
                                                    for ($i = 0; $i < $totalRecord; $i++) {
                                                        ...
                                                        ?>
                                                        <tr <?php
                                                        if ($skillArray[$i]['isDelete'] == 1) {
                                                            echo "class=\"danger\"";
                                                        } else {
                                                            if ($skillArray[$i]['isDraft'] == 1) {
                                                                echo "class=\"warning\"";
                                                            }
                                                        }
                                                        ?>>
                                                            ...
                                                            <td align="center">
                                                                <div class="btn-group" align="center">
                                                                    ...
                                                                    <button type="button"  class="btn btn-danger btn-sm" title="Delete"
                                                                            ...
                                                                            ?>', '<?php
                                                                            echo rawurlencode(
                                                                                    $skillArray [$i]['skillCode']
                                                                            );
                                                                            ?>', '<?php
                                                                            echo rawurlencode(
                                                                                    $skillArray [$i]['skillDescription']
                                                                            );
                                                                            ?>');">
                                                            ...
                                                            <td>
                                                                <div align="center">
                                                                    <?php
                                                                    if (isset($skillArray[$i]['skillCode'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos(
                                                                                                strtolower($skillArray[$i]['skillCode']), strtolower($_POST['query'])
                                                                                        ) !== false
                                                                                ) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $skillArray[$i]['skillCode']
                                                                                    );
                                                                                } else {
                                                                                    echo $skillArray[$i]['skillCode'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                    if (strpos(
                                                                                                    strtolower($skillArray[$i]['skillCode']), strtolower($_POST['character'])
                                                                                            ) !== false
                                                                                    ) {
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $skillArray[$i]['skillCode']
                                                                                        );
                                                                                    } else {
                                                                                        echo $skillArray[$i]['skillCode'];
                                                                                    }
                                                            ...
                                                            <td>
                                                                <div class="pull-left">
                                                                    <?php
                                                                    if (isset($skillArray[$i]['skillDescription'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos(
                                                                                                strtolower($skillArray[$i]['skillDescription']), strtolower($_POST['query'])
                                                                                        ) !== false
                                                                                ) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $skillArray[$i]['skillDescription']
                                                                                    );
                                                                                } else {
                                                                                    echo $skillArray[$i]['skillDescription'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                    if (strpos(
                                                                                                    strtolower($skillArray[$i]['skillDescription']), strtolower($_POST['character'])
                                                                                            ) !== false
                                                                                    ) {
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $skillArray[$i]['skillDescription']
                                                                                        );
                                                                                    } else {
                                                                                        echo $skillArray[$i]['skillDescription'];
                                                                                    }
                                                            ...
                                                            <td>
                                                                <div align="center">
                                                                    <?php
                                                                    if (isset($skillArray[$i]['executeBy'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos($skillArray[$i]['staffName'], $_POST['query']) !== false) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $skillArray[$i]['staffName']
                                                                                    );
                                                                                } else {
                                                                                    echo $skillArray[$i]['staffName'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                    if (strpos(
                                                                                                    $skillArray[$i]['staffName'], $_POST['character']
                                                                                            ) !== false
                                                                                    ) {
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $skillArray[$i]['staffName']
                                                                                        );
                                                                                    } else {
                                                                                        echo $skillArray[$i]['staffName'];
                                                                                    }
                                                            ...
                                                            <?php
                                                            if (isset($skillArray[$i]['executeTime'])) {
                                                                $valueArray = $skillArray[$i]['executeTime'];
                                                            ...
                                                            ?>
                                                            <td>
                                                                <input style="display:none;" type="checkbox" name="skillId[]"
                                                                       value="<?php echo $skillArray[$i]['skillId']; ?>">
                                                                <input <?php echo $checked; ?> type="checkbox" name="isDelete[]"
                                                                                               value="<?php echo $skillArray[$i]['isDelete']; ?>">
                                                            </td>
                                                        </tr>
                                                        <?php
                                                    }
                        ...
                        <div class="col-xs-3 col-sm-3 col-md-3 pagination">
                            <div class="pull-right">
                                <button class="delete btn btn-warning" type="button" 
                                        ...
                                        echo $skill->getControllerPath();
                                        ?>', '<?php echo $skill->getViewPath(); ?>', '<?php echo $securityToken; ?>');">
...
if ((isset($_POST['method']) == 'new' || isset($_POST['method']) == 'read') && $_POST['type'] == 'form') {
            ...
            <div class="row">
                <div class="col-xs-12 col-sm-12 col-md-12">
                    <div class="panel panel-info">
                        ...
                        <div class="panel-body">
                            <input type="hidden" name="skillId" id="skillId" value="<?php
                            if (isset($_POST['skillId'])) {
                                echo $_POST['skillId'];
                            }
                            ?>">

                            <div class="form-group" id="skillCodeForm">
                                <label for="skillCode" class="control-label col-xs-2 col-sm-2 col-md-2"><strong><?php
                                        echo ucfirst(
                                                $leafTranslation['skillCodeLabel']
                                        );
                                        ?></strong></label>

                                <div class="control-label col-xs-4 col-sm-4 col-md-4">
                                    <div class="input-group">
                                        <input class="form-control" type="text" name="skillCode" id="skillCode"
                                               onKeyUp="removeMeError('skillCode');" value="<?php
                                               if (isset($skillArray) && is_array($skillArray)) {
                                                   if (isset($skillArray[0]['skillCode'])) {
                                                       echo htmlentities($skillArray[0]['skillCode']);
                                                   }
                                               }
                                               ?>" maxlength="16"> <span class="input-group-addon"><img src="./images/icons/document-code.png"></span>
                                    </div>
                                    <span class="help-block" id="skillCodeHelpMe"></span>
                                </div>
                            </div>
                            <div class="form-group" id="skillDescriptionForm">
                                <div class="col-xs-12 col-sm-12 col-md-12">
                                    <textarea class="form-control" rows="5" name="skillDescription" id="skillDescription"><?php
                                        if (isset($skillArray[0]['skillDescription'])) {
                                            echo htmlentities($skillArray[0]['skillDescription']);
                                        }
                                        ?></textarea> <span class="help-block" id="skillDescriptionHelpMe"></span>
                                </div>
                            </div>
                        </div>
    ...
    <?php if ($leafAccess['leafAccessCreateValue'] == 1) { ?>
                                newRecord(<?php echo $leafId; ?>, '<?php echo $skill->getControllerPath(); ?>', '<?php echo $skill->getViewPath(); ?>', '<?php echo $securityToken; ?>', 1);
    <?php } ?>
                            return false;
                        }
                        // shift+s save event
    ...
    <?php if ($leafAccess['leafAccessUpdateValue'] == 1) { ?>
                                updateRecord(<?php echo $leafId; ?>, '<?php echo $skill->getControllerPath(); ?>', '<?php echo $skill->getViewPath(); ?>', '<?php echo $securityToken; ?>', 1, <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
    <?php } ?>
                            return false;
                        }
    ...
    <?php if ($leafAccess['leafAccessDeleteValue'] == 1) { ?>
                                deleteRecord(<?php echo $leafId; ?>, '<?php echo $skill->getControllerPath(); ?>', '<?php echo $skill->getViewPath(); ?>', '<?php echo $securityToken; ?>', <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
    ...
    <?php } ?>
                        ...
                        switch (e.keyCode) {
                            case 37:
                                previousRecord(<?php echo $leafId; ?>, '<?php echo $skill->getControllerPath(); ?>', '<?php echo $skill->getViewPath(); ?>', '<?php echo $securityToken; ?>', <?php echo $leafAccess['leafAccessUpdateValue']; ?>, <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
                                
                                return false;
                                break;
                            case 39:
                                nextRecord(<?php echo $leafId; ?>, '<?php echo $skill->getControllerPath(); ?>', '<?php echo $skill->getViewPath(); ?>', '<?php echo $securityToken; ?>', <?php echo $leafAccess['leafAccessUpdateValue']; ?>, <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
                    ...
                    $(document).ready(function() {
                        ...
                        validateMeNumeric('skillId');
                        validateMeAlphaNumeric('skillCode');
    <?php if ($_POST['method'] == "new") { ?>
                            $('#resetRecordButton').removeClass().addClass('btn btn-default');
        <?php if ($leafAccess['leafAccessCreateValue'] == 1) { ?>
                                $('#newRecordButton1').removeClass().addClass('btn btn-success').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $skill->getControllerPath(); ?>','<?php echo $skill->getViewPath(); ?>','<?php echo $securityToken; ?>',1)");
                                $('#newRecordButton2').removeClass().addClass('btn dropdown-toggle btn-success');
                                $('#newRecordButton3').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $skill->getControllerPath(); ?>','<?php echo $skill->getViewPath(); ?>','<?php echo $securityToken; ?>',1)");
                                $('#newRecordButton4').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $skill->getControllerPath(); ?>','<?php echo $skill->getViewPath(); ?>','<?php echo $securityToken; ?>',2)");
                                $('#newRecordButton5').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $skill->getControllerPath(); ?>','<?php echo $skill->getViewPath(); ?>','<?php echo $securityToken; ?>',3)");
                                $('#newRecordButton6').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $skill->getControllerPath(); ?>','<?php echo $skill->getViewPath(); ?>','<?php echo $securityToken; ?>',4)");
                                $('#newRecordButton7').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $skill->getControllerPath(); ?>','<?php echo $skill->getViewPath(); ?>','<?php echo $securityToken; ?>',5)");
        <?php } else { ?>
        ...
        if ($_POST['skillId']) {
            ...
            <?php if ($leafAccess['leafAccessUpdateValue'] == 1) { ?>
                                    $('#updateRecordButton1').removeClass().addClass('btn btn-info').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $skill->getControllerPath(); ?>','<?php echo $skill->getViewPath(); ?>','<?php echo $securityToken; ?>',1,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                                    $('#updateRecordButton2').removeClass().addClass('btn dropdown-toggle btn-info');
                                    $('#updateRecordButton3').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $skill->getControllerPath(); ?>','<?php echo $skill->getViewPath(); ?>','<?php echo $securityToken; ?>',1,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                                    $('#updateRecordButton4').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $skill->getControllerPath(); ?>','<?php echo $skill->getViewPath(); ?>','<?php echo $securityToken; ?>',2,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                                    $('#updateRecordButton5').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $skill->getControllerPath(); ?>','<?php echo $skill->getViewPath(); ?>','<?php echo $securityToken; ?>',3,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
            <?php } else { ?>
                                    $('#updateRecordButton1').removeClass().addClass(' btn btn-info disabled').attr('onClick', '');
                                    $('#updateRecordButton2').removeClass().addClass('btn dropdown-toggle btn-info disabled');
                                    $('#updateRecordButton3').attr('onClick', '');
            ...
            <?php } ?>
            <?php if ($leafAccess['leafAccessDeleteValue'] == 1) { ?>
                                    $('#deleteRecordButton').removeClass().addClass('btn btn-danger').attr('onClick', "deleteRecord(<?php echo $leafId; ?>,'<?php echo $skill->getControllerPath(); ?>','<?php echo $skill->getViewPath(); ?>','<?php echo $securityToken; ?>',<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
            <?php } else { ?>
                                    $('#deleteRecordButton').removeClass().addClass('btn btn-danger disabled').attr('onClick', '');
            <?php } ?>
            <?php
...
<?php } ?>
<script type="text/javascript" src="./v3/humanResource/setting/javascript/skill.js"></script>
<hr>
