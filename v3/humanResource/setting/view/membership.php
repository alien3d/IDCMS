...
require_once($newFakeDocumentRoot . "v3/humanResource/setting/controller/membershipController.php");
...
$translator->setCurrentTable('membership');
...
if (isset($_POST)) {
    if (isset($_POST['method'])) {
        $membership = new \Core\HumanResource\Setting\Membership\Controller\MembershipClass();
        define('LIMIT', 10);
        ...
        if (isset($_POST ['query'])) {
            $membership->setFieldQuery($_POST ['query']);
        }
        if (isset($_POST ['filter'])) {
            $membership->setGridQuery($_POST ['filter']);
        }
        if (isset($_POST ['character'])) {
            $membership->setCharacterQuery($_POST['character']);
        }
        if (isset($_POST ['dateRangeStart'])) {
            $membership->setDateRangeStartQuery($_POST['dateRangeStart']);
            //explode the data to get day,month,year
            $start = explode('-', $_POST ['dateRangeStart']);
            $membership->setStartDay($start[2]);
            $membership->setStartMonth($start[1]);
            $membership->setStartYear($start[0]);
        }
        if (isset($_POST ['dateRangeEnd']) && (strlen($_POST['dateRangeEnd']) > 0)) {
            $membership->setDateRangeEndQuery($_POST['dateRangeEnd']);
            //explode the data to get day,month,year
            $start = explode('-', $_POST ['dateRangeEnd']);
            $membership->setEndDay($start[2]);
            $membership->setEndMonth($start[1]);
            $membership->setEndYear($start[0]);
        }
        if (isset($_POST ['dateRangeType'])) {
            $membership->setDateRangeTypeQuery($_POST['dateRangeType']);
        }
        if (isset($_POST ['dateRangeExtraType'])) {
            $membership->setDateRangeExtraTypeQuery($_POST['dateRangeExtraType']);
        }
        $membership->setServiceOutput('html');
        $membership->setLeafId($leafId);
        $membership->execute();
        if ($_POST['method'] == 'read') {
            $membership->setStart($offset);
            $membership->setLimit($limit); // normal system don't like paging..
            $membership->setPageOutput('html');
            $membershipArray = $membership->read();
            if (isset($membershipArray [0]['firstRecord'])) {
                $firstRecord = $membershipArray [0]['firstRecord'];
            }
            if (isset($membershipArray [0]['nextRecord'])) {
                $nextRecord = $membershipArray [0]['nextRecord'];
            }
            if (isset($membershipArray [0]['previousRecord'])) {
                $previousRecord = $membershipArray [0]['previousRecord'];
            }
            if (isset($membershipArray [0]['lastRecord'])) {
                $lastRecord = $membershipArray [0]['lastRecord'];
                $endRecord = $membershipArray [0]['lastRecord'];
            }
            $navigation = new \Core\Paging\HtmlPaging();
            $navigation->setLeafId($leafId);
            $navigation->setViewPath($membership->getViewPath());
            ...
            if (isset($membershipArray [0]['total'])) {
                $total = $membershipArray [0]['total'];
            } else {
                $total = 0;
            }
                        ...
                        <div class="btn-group">
                            ...
                            <ul class="dropdown-menu" style="text-align:left">
                                <li>
                                    ...
                                    echo $membership->getControllerPath();
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <form class="form-horizontal">
                                        <input type="hidden" name="membershipIdPreview" id="membershipIdPreview">

                                        <div class="form-group" id="membershipTitleDiv">
                                            <label for="membershipTitlePreview"
                                                   class="control-label col-xs-4 col-sm-4 col-md-4"><?php echo $leafTranslation['membershipTitleLabel']; ?></label>

                                            <div class="col-xs-8 col-sm-8 col-md-8">
                                                <input class="form-control" type="text" name="membershipTitlePreview"
                                                       id="membershipTitlePreview">
                                            </div>
                                        </div>
                                        <div class="form-group" id="membershipCodeDiv">
                                            <label for="membershipCodePreview"
                                                   class="control-label col-xs-4 col-sm-4 col-md-4"><?php echo $leafTranslation['membershipCodeLabel']; ?></label>

                                            <div class="col-xs-8 col-sm-8 col-md-8">
                                                <input class="form-control" type="text" name="membershipCodePreview"
                                                       id="membershipCodePreview">
                                            </div>
                                        </div>
                                        <div class="form-group" id="membershipDescriptionDiv">
                                            <label for="membershipDescriptionPreview"
                                                   class="control-label col-xs-4 col-sm-4 col-md-4"><?php echo $leafTranslation['membershipDescriptionLabel']; ?></label>

                                            <div class="col-xs-8 col-sm-8 col-md-8">
                                                <input class="form-control" type="text" name="membershipDescriptionPreview"
                                                       id="membershipDescriptionPreview">
                                            </div>
                                        </div>
                                    </form>
                                </div>
                        ...
                        <div class="col-xs-12 col-sm-12 col-md-12">
                            <div class="panel panel-default">
                                <table class="table table table-bordered table-striped table-condensed table-hover smart-form has-tickbox" id="tableData">
                                    ...
                                    <th width="75px"><?php echo ucwords($leafTranslation['membershipTitleLabel']); ?></th>
                                    <th width="75px">
                                    <div align="center"><?php echo ucwords($leafTranslation['membershipCodeLabel']); ?></div>
                                    </th>
                                    <th><?php echo ucwords($leafTranslation['membershipDescriptionLabel']); ?></th>
                                    ...
                                    <tbody id="tableBody">
                                        ...
                                        if ($_POST['method'] == 'read' && $_POST['type'] == 'list' && $_POST['detail'] == 'body') {
                                            if (is_array($membershipArray)) {
                                                $totalRecord = intval(count($membershipArray));
                                                if ($totalRecord > 0) {
                                                    $counter = 0;
                                                    for ($i = 0; $i < $totalRecord; $i++) {
                                                        ...
                                                        ?>
                                                        <tr <?php
                                                        if ($membershipArray[$i]['isDelete'] == 1) {
                                                            echo "class=\"danger\"";
                                                        } else {
                                                            if ($membershipArray[$i]['isDraft'] == 1) {
                                                                echo "class=\"warning\"";
                                                            }
                                                        }
                                                        ?>>
                                                            ...
                                                            <td align="center">
                                                                <div class="btn-group" align="center">
                                                                    <button type="button"  class="btn btn-info btn-sm" title="Edit"
                                                                            ...
                                                                            echo intval(
                                                                                    $membershipArray [$i]['membershipId']
                                                                            );
                                                                    ...
                                                                    <button type="button"  class="btn btn-danger btn-sm" title="Delete"
                                                                            ...
                                                                            ?>', '<?php
                                                                            echo rawurlencode(
                                                                                    $membershipArray [$i]['membershipTitle']
                                                                            );
                                                                            ?>', '<?php
                                                                            echo rawurlencode(
                                                                                    $membershipArray [$i]['membershipCode']
                                                                            );
                                                                            ?>', '<?php
                                                                            echo rawurlencode(
                                                                                    $membershipArray [$i]['membershipDescription']
                                                                            );
                                                                            ?>');">
                                                            ...
                                                            <td>
                                                                <div class="pull-left">
                                                                    <?php
                                                                    if (isset($membershipArray[$i]['membershipTitle'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos(
                                                                                                strtolower($membershipArray[$i]['membershipTitle']), strtolower($_POST['query'])
                                                                                        ) !== false
                                                                                ) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $membershipArray[$i]['membershipTitle']
                                                                                    );
                                                                                } else {
                                                                                    echo $membershipArray[$i]['membershipTitle'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                    if (strpos(
                                                                                                    strtolower($membershipArray[$i]['membershipTitle']), strtolower($_POST['character'])
                                                                                            ) !== false
                                                                                    ) {
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $membershipArray[$i]['membershipTitle']
                                                                                        );
                                                                                    } else {
                                                                                        echo $membershipArray[$i]['membershipTitle'];
                                                                                    }
                                                            ...
                                                            <td>
                                                                <div align="center">
                                                                    <?php
                                                                    if (isset($membershipArray[$i]['membershipCode'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos(
                                                                                                strtolower($membershipArray[$i]['membershipCode']), strtolower($_POST['query'])
                                                                                        ) !== false
                                                                                ) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $membershipArray[$i]['membershipCode']
                                                                                    );
                                                                                } else {
                                                                                    echo $membershipArray[$i]['membershipCode'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                    if (strpos(
                                                                                                    strtolower($membershipArray[$i]['membershipCode']), strtolower($_POST['character'])
                                                                                            ) !== false
                                                                                    ) {
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $membershipArray[$i]['membershipCode']
                                                                                        );
                                                                                    } else {
                                                                                        echo $membershipArray[$i]['membershipCode'];
                                                                                    }
                                                            ...
                                                            <td>
                                                                <div class="pull-left">
                                                                    <?php
                                                                    if (isset($membershipArray[$i]['membershipDescription'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos(
                                                                                                strtolower($membershipArray[$i]['membershipDescription']), strtolower($_POST['query'])
                                                                                        ) !== false
                                                                                ) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $membershipArray[$i]['membershipDescription']
                                                                                    );
                                                                                } else {
                                                                                    echo $membershipArray[$i]['membershipDescription'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                    if (strpos(
                                                                                                    strtolower($membershipArray[$i]['membershipDescription']), strtolower($_POST['character'])
                                                                                            ) !== false
                                                                                    ) {
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $membershipArray[$i]['membershipDescription']
                                                                                        );
                                                                                    } else {
                                                                                        echo $membershipArray[$i]['membershipDescription'];
                                                                                    }
                                                            ...
                                                            <td>
                                                                <div class="pull-left">
                                                                    <?php
                                                                    if (isset($membershipArray[$i]['executeBy'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos($membershipArray[$i]['staffName'], $_POST['query']) !== false) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $membershipArray[$i]['staffName']
                                                                                    );
                                                                                } else {
                                                                                    echo $membershipArray[$i]['staffName'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                    if (strpos(
                                                                                                    $membershipArray[$i]['staffName'], $_POST['character']
                                                                                            ) !== false
                                                                                    ) {
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $membershipArray[$i]['staffName']
                                                                                        );
                                                                                    } else {
                                                                                        echo $membershipArray[$i]['staffName'];
                                                                                    }
                                                            ...
                                                            <?php
                                                            if (isset($membershipArray[$i]['executeTime'])) {
                                                                $valueArray = $membershipArray[$i]['executeTime'];
                                                            ...
                                                            ?>
                                                            <td>
                                                                <input style="display:none;" type="checkbox" name="membershipId[]"
                                                                       value="<?php echo $membershipArray[$i]['membershipId']; ?>">
                                                                <input <?php echo $checked; ?> type="checkbox" name="isDelete[]"
                                                                                               value="<?php echo $membershipArray[$i]['isDelete']; ?>">
                                                            </td>
                                                        </tr>
                                                        <?php
                                                    }
                        ...
                        <div class="col-xs-3 col-sm-3 col-md-3 pagination">
                            <div class="pull-right">
                                <button class="delete btn btn-warning" type="button" 
                                        ...
                                        echo $membership->getControllerPath();
                                        ?>', '<?php echo $membership->getViewPath(); ?>', '<?php echo $securityToken; ?>');">
...
if ((isset($_POST['method']) == 'new' || isset($_POST['method']) == 'read') && $_POST['type'] == 'form') {
            ...
            <div class="row">
                <div class="col-xs-12 col-sm-12 col-md-12">
                    <div class="panel panel-info">
                        ...
                        <div class="panel-body">
                            <input type="hidden" name="membershipId" id="membershipId" value="<?php
                            if (isset($_POST['membershipId'])) {
                                echo $_POST['membershipId'];
                            }
                            ?>">

                            <div class="row">
                                <div class="col-xs-12 col-sm-12 col-md-12">
                                    <div class="col-xs-6 col-sm-6 col-md-6 form-group" id="membershipTitleForm">
                                        <label for="membershipTitle" class="control-label col-xs-4 col-sm-4 col-md-4"><strong><?php
                                                echo ucfirst(
                                                        $leafTranslation['membershipTitleLabel']
                                                );
                                                ?></strong></label>

                                        <div class="col-xs-8 col-sm-8 col-md-8">
                                            <div class="input-group">
                                                <input type="text" class="form-control" name="membershipTitle"
                                                       id="membershipTitle" onKeyUp="removeMeError('membershipTitle');" value="<?php
                                                       if (isset($membershipArray) && is_array($membershipArray)) {
                                                           echo htmlentities($membershipArray[0]['membershipTitle']);
                                                       }
                                                       ?>"> <span class="input-group-addon"><img
                                                        src="./images/icons/document-code.png"></span>
                                            </div>
                                            <span class="help-block" id="membershipTitleHelpMe"></span>
                                        </div>
                                    </div>
                                    <div class="col-xs-6 col-sm-6 col-md-6 form-group" id="membershipCodeForm">
                                        <label for="membershipCode" class="control-label col-xs-4 col-sm-4 col-md-4"><strong><?php
                                                echo ucfirst(
                                                        $leafTranslation['membershipCodeLabel']
                                                );
                                                ?></strong></label>

                                        <div class="col-xs-8 col-sm-8 col-md-8">
                                            <input type="text" class="form-control" name="membershipCode"
                                                   id="membershipCode" onKeyUp="removeMeError('membershipCode');" value="<?php
                                                   if (isset($membershipArray) && is_array($membershipArray)) {
                                                       if (isset($membershipArray[0]['membershipCode'])) {
                                                           echo htmlentities($membershipArray[0]['membershipCode']);
                                                       }
                                                   }
                                                   ?>" maxlength="16"> <span class="help-block" id="membershipCodeHelpMe"></span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-xs-12 col-sm-12 col-md-12">
                                    <div class="form-group" id="membershipDescriptionForm">
                                        <div class="col-xs-12 col-sm-12 col-md-12">
                                            <textarea class="form-control" name="membershipDescription"
                                                      id="membershipDescription"><?php
                                                          if (isset($membershipArray[0]['membershipDescription'])) {
                                                              echo htmlentities($membershipArray[0]['membershipDescription']);
                                                          }
                                                          ?></textarea> <span class="help-block" id="membershipDescriptionHelpMe"></span>
                                        </div>
                                    </div>
                                </div>
                            </div>
    ...
    <?php if ($leafAccess['leafAccessCreateValue'] == 1) { ?>
                            newRecord(<?php echo $leafId; ?>, '<?php echo $membership->getControllerPath(); ?>', '<?php echo $membership->getViewPath(); ?>', '<?php echo $securityToken; ?>', 1);
    <?php } ?>
                        return false;
                    }
                    // shift+s save event
    ...
    <?php if ($leafAccess['leafAccessUpdateValue'] == 1) { ?>
                            updateRecord(<?php echo $leafId; ?>, '<?php echo $membership->getControllerPath(); ?>', '<?php echo $membership->getViewPath(); ?>', '<?php echo $securityToken; ?>', 1, <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
    <?php } ?>
                        return false;
                    }
    ...
    <?php if ($leafAccess['leafAccessDeleteValue'] == 1) { ?>
                            deleteRecord(<?php echo $leafId; ?>, '<?php echo $membership->getControllerPath(); ?>', '<?php echo $membership->getViewPath(); ?>', '<?php echo $securityToken; ?>', <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
    ...
    <?php } ?>
                    ...
                    switch (e.keyCode) {
                        case 37:
                            previousRecord(<?php echo $leafId; ?>, '<?php echo $membership->getControllerPath(); ?>', '<?php echo $membership->getViewPath(); ?>', '<?php echo $securityToken; ?>', <?php echo $leafAccess['leafAccessUpdateValue']; ?>, <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
                            
                            return false;
                            break;
                        case 39:
                            nextRecord(<?php echo $leafId; ?>, '<?php echo $membership->getControllerPath(); ?>', '<?php echo $membership->getViewPath(); ?>', '<?php echo $securityToken; ?>', <?php echo $leafAccess['leafAccessUpdateValue']; ?>, <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
                ...
                $(document).ready(function() {
                    ...
                    validateMeNumeric('membershipId');
                    validateMeAlphaNumeric('membershipTitle');
                    validateMeAlphaNumeric('membershipCode');
    <?php if ($_POST['method'] == "new") { ?>
                        $('#resetRecordButton').removeClass().addClass('btn btn-default');
        <?php if ($leafAccess['leafAccessCreateValue'] == 1) { ?>
                            $('#newRecordButton1').removeClass().addClass('btn btn-success').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $membership->getControllerPath(); ?>','<?php echo $membership->getViewPath(); ?>','<?php echo $securityToken; ?>',1)");
                            $('#newRecordButton2').removeClass().addClass('btn dropdown-toggle btn-success');
                            $('#newRecordButton3').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $membership->getControllerPath(); ?>','<?php echo $membership->getViewPath(); ?>','<?php echo $securityToken; ?>',1)");
                            $('#newRecordButton4').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $membership->getControllerPath(); ?>','<?php echo $membership->getViewPath(); ?>','<?php echo $securityToken; ?>',2)");
                            $('#newRecordButton5').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $membership->getControllerPath(); ?>','<?php echo $membership->getViewPath(); ?>','<?php echo $securityToken; ?>',3)");
                            $('#newRecordButton6').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $membership->getControllerPath(); ?>','<?php echo $membership->getViewPath(); ?>','<?php echo $securityToken; ?>',4)");
                            $('#newRecordButton7').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $membership->getControllerPath(); ?>','<?php echo $membership->getViewPath(); ?>','<?php echo $securityToken; ?>',5)");
        <?php } else { ?>
        ...
        if ($_POST['membershipId']) {
            ...
            <?php if ($leafAccess['leafAccessUpdateValue'] == 1) { ?>
                                $('#updateRecordButton1').removeClass().addClass('btn btn-info').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $membership->getControllerPath(); ?>','<?php echo $membership->getViewPath(); ?>','<?php echo $securityToken; ?>',1,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                                $('#updateRecordButton2').removeClass().addClass('btn dropdown-toggle btn-info');
                                $('#updateRecordButton3').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $membership->getControllerPath(); ?>','<?php echo $membership->getViewPath(); ?>','<?php echo $securityToken; ?>',1,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                                $('#updateRecordButton4').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $membership->getControllerPath(); ?>','<?php echo $membership->getViewPath(); ?>','<?php echo $securityToken; ?>',2,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                                $('#updateRecordButton5').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $membership->getControllerPath(); ?>','<?php echo $membership->getViewPath(); ?>','<?php echo $securityToken; ?>',3,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
            <?php } else { ?>
                                $('#updateRecordButton1').removeClass().addClass(' btn btn-info disabled').attr('onClick', '');
                                $('#updateRecordButton2').removeClass().addClass('btn dropdown-toggle btn-info disabled');
                                $('#updateRecordButton3').attr('onClick', '');
            ...
            <?php } ?>
            <?php if ($leafAccess['leafAccessDeleteValue'] == 1) { ?>
                                $('#deleteRecordButton').removeClass().addClass('btn btn-danger').attr('onClick', "deleteRecord(<?php echo $leafId; ?>,'<?php echo $membership->getControllerPath(); ?>','<?php echo $membership->getViewPath(); ?>','<?php echo $securityToken; ?>',<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
            <?php } else { ?>
                                $('#deleteRecordButton').removeClass().addClass('btn btn-danger disabled').attr('onClick', '');
            <?php } ?>
            <?php
...
<?php } ?>
<script type="text/javascript" src="./v3/humanResource/setting/javascript/membership.js"></script>
<hr>
