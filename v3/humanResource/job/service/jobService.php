...
namespace Core\HumanResource\Job\Job\Service;
 ...
 * @version 2
 * @author hafizan
 * @package Core\HumanResource\Job\Job\Service
 * @subpackage Job
 * @link http://www.hafizan.com
 * @license http://www.gnu.org/copyleft/lesser.html LGPL
 */
class JobService extends ConfigClass {
    ...
    public function getJobCategory() {
        ...
        if ($this->getVendor() == self::MYSQL) {
         ...
         SELECT      JOBCATEGORYID AS \"jobCategoryId\",
                     JOBCATEGORYDESCRIPTION AS \"jobCategoryDescription\"
         FROM        JOBCATEGORY
         WHERE       ISACTIVE    =   1
         AND         COMPANYID   =   '" . $this->getCompanyId() . "'
         ORDER BY    ISDEFAULT";
        ...
        if ($result) {
            ...
            while (($row = $this->q->fetchArray($result)) == true) {
                if ($this->getServiceOutput() == 'option') {
                    $str .= "<option value='" . $row['jobCategoryId'] . "'>" . $d . ". " . $row['jobCategoryDescription'] . "</option>";
                } else {
