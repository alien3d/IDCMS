function checkDuplicate(leafId, page, securityToken) {
    var $jobCategoryCode = $("#jobCategoryCode");
    if ($jobCategoryCode.val().length === 0) {
    ...
    $.ajax({
        ...
        url: page,
        data: {
            jobCategoryCode: $jobCategoryCode.val(),
            method: 'duplicate',
        ...
        success: function(data) {
            ...
            if (success === true) {
                if (total !== 0) {
                    $("#jobCategoryCode").val('').focus();
                    $("#jobCategoryCodeForm").removeClass().addClass("form-group has-error");
                ...
                $("#jobCategoryForm").removeClass().addClass("form-group has-error");
            }
            if ($infoPanel.is(':hidden')) {
                $infoPanel.show();
            }
...
function showFormUpdate(leafId, url, urlList, securityToken, jobCategoryId, updateAccess, deleteAccess) {
    sleep(500);
...
function showModalDelete(jobCategoryId, jobCategoryCode, jobCategoryDescription) {
    $('#jobCategoryIdPreview').val('').val(decodeURIComponent(jobCategoryId));
    $('#jobCategoryCodePreview').val('').val(decodeURIComponent(jobCategoryCode));
    $('#jobCategoryDescriptionPreview').val('').val(decodeURIComponent(jobCategoryDescription));
...
function deleteGridRecord(leafId, url, urlList, securityToken) {
    $.ajax({
        ...
        data: {
            method: 'delete',
            output: 'json',
            jobCategoryId: $('#jobCategoryIdPreview').val(),
            securityToken: securityToken,
            leafId: leafId
        },
        beforeSend: function() {
...
function deleteGridRecordCheckbox(leafId, url, urlList, securityToken) {
    var stringText = '';
    var counter = 0;
    $('input:checkbox[name="jobCategoryId[]"]').each(function() {
        stringText = stringText + "&jobCategoryId[]=" + $(this).val();
    });
...
function newRecord(leafId, url, urlList, securityToken, type, createAccess, updateAccess, deleteAccess) {
    ...
    var $jobCategoryCode = $('#jobCategoryCode');
    var $jobCategoryDescription = $('#jobCategoryDescription');
    if (css.search('disabled') > 0) {
        return false;
    } else {
        if (type === 1) {
            if ($jobCategoryCode.val().length === 0) {
                $('#jobCategoryCodeHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['jobCategoryCodeLabel'] + " </span>");
                $('#jobCategoryCodeForm').removeClass().addClass('form-group has-error');
                $jobCategoryCode.focus();
                return false;
            }
            if ($jobCategoryDescription.val().length === 0) {
                $('#jobCategoryDescriptionHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['jobCategoryDescriptionLabel'] + " </span>");
                $('#jobCategoryDescriptionForm').removeClass().addClass('form-group has-error');
                $jobCategoryDescription.focus();
                return false;
            }
            $.ajax({
                type: 'POST',
                ...
                data: {
                    ...
                    output: 'json',
                    jobCategoryCode: $jobCategoryCode.val(),
                    jobCategoryDescription: $jobCategoryDescription.val(),
                    securityToken: securityToken,
                    leafId: leafId
                },
                beforeSend: function() {
                ...
                success: function(data) {
                    ...
                    if (success === true) {
                        ...
                        $jobCategoryCode.val('');
                        $('#jobCategoryCodeHelpMe').html('').empty();
                        $jobCategoryDescription.val('');
                        $('#jobCategoryDescriptionForm').removeClass().addClass('form-group');
                        $('#jobCategoryDescriptionHelpMe').html('').empty();
                    } else if (success === false) {
                        $infoPanel.html('').empty().html("<span class='label label-important'>&nbsp;" + message + "</span>");
                        if ($infoPanel.is(':hidden')) {
                            $infoPanel.show();
...
function resetRecord(leafId, url, urlList, securityToken, createAccess, updateAccess, deleteAccess) {
    ...
    $("#jobCategoryCode").val('');
    $("#jobCategoryCodeHelpMe").empty().html('');
    $("#jobCategoryDescription").val('');
    $("#jobCategoryDescriptionHelpMe").empty().html('');
    $('#jobCategoryDescription').empty().val('');
}
function postRecord() {
                ...
                if (success === true) {
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if (success === true) {
                                $('#jobCategoryId').val(data.data.jobCategoryId);
                                $('#jobCategoryCode').val(data.data.jobCategoryCode);
                                $('#jobCategoryDescription').val(data.data.jobCategoryDescription);
                                if (nextRecord > 0) {
                                    $('#previousRecordButton').removeClass().addClass('btn btn-default disabled').attr('onClick', '');
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($previousRecordCounter.val()) > 0 && parseFloat($previousRecordCounter.val()) < parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    jobCategoryId: $previousRecordCounter.val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($nextRecordCounter.val()) <= parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    jobCategoryId: $nextRecordCounter.val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
