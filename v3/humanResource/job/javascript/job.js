function getJobCategory(leafId, url, securityToken) {
    $.ajax({
        type: 'GET',
        url: url,
        data: {
        ...
        success: function(data) {
            ...
            if (success === false) {
                ...
                $("#jobCategoryId").html('').empty().html(data.data).trigger("chosen:updated");
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if (success === true) {
                if (total !== 0) {
                    $("#jobCode").val('').focus();
                    $("#jobCodeForm").removeClass().addClass("form-group has-error");
                ...
                $("#jobForm").removeClass().addClass("form-group has-error");
            }
            if ($infoPanel.is(':hidden')) {
                $infoPanel.show();
            }
...
function showFormUpdate(leafId, url, urlList, securityToken, jobId, updateAccess, deleteAccess) {
    sleep(500);
...
function showModalDelete(jobId, jobCategoryId, countryId, jobExperienceRequired, jobCode, jobTitle, jobMinimumSalary, jobMaximumSalary, jobDescription) {
    $('#jobIdPreview').val('').val(decodeURIComponent(jobId));
    $('#jobCategoryIdPreview').val('').val(decodeURIComponent(jobCategoryId));
    $('#countryIdPreview').val('').val(decodeURIComponent(countryId));
    $('#jobExperienceRequiredPreview').val('').val(decodeURIComponent(jobExperienceRequired));
    $('#jobCodePreview').val('').val(decodeURIComponent(jobCode));
    $('#jobTitlePreview').val('').val(decodeURIComponent(jobTitle));
    $('#jobMinimumSalaryPreview').val('').val(decodeURIComponent(jobMinimumSalary));
    $('#jobMaximumSalaryPreview').val('').val(decodeURIComponent(jobMaximumSalary));
    $('#jobDescriptionPreview').val('').val(decodeURIComponent(jobDescription));
...
function deleteGridRecordCheckbox(leafId, url, urlList, securityToken) {
    var stringText = '';
    var counter = 0;
    $('input:checkbox[name="jobId[]"]').each(function() {
        stringText = stringText + "&jobId[]=" + $(this).val();
    });
...
function newRecord(leafId, url, urlList, securityToken, type, createAccess, updateAccess, deleteAccess) {
    ...
    var $jobExperienceRequired = $('#jobExperienceRequired');
    var $jobCode = $('#jobCode');
    var $jobTitle = $('#jobTitle');
    var $jobMinimumSalary = $('#jobMinimumSalary');
    var $jobMaximumSalary = $('#jobMaximumSalary');
    ...
    if (css.search('disabled') > 0) {
        ...
        if (type === 1) {
            if ($jobCategoryId.val().length === 0) {
                $('#jobCategoryIdHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['jobCategoryIdLabel'] + " </span>");
                $jobCategoryId.data('chosen').activate_action();
                return false;
            }
            ...
            if ($jobExperienceRequired.val().length === 0) {
                $('#jobExperienceRequiredHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['jobExperienceRequiredLabel'] + " </span>");
                $('#jobExperienceRequiredForm').removeClass().addClass('form-group has-error');
                $jobExperienceRequired.focus();
                return false;
            }
            if ($jobCode.val().length === 0) {
                $('#jobCodeHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['jobCodeLabel'] + " </span>");
                $('#jobCodeForm').removeClass().addClass('form-group has-error');
                $jobCode.focus();
                return false;
            }
            if ($jobTitle.val().length === 0) {
                $('#jobTitleHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['jobTitleLabel'] + " </span>");
                $('#jobTitleForm').removeClass().addClass('form-group has-error');
                $jobTitle.focus();
                return false;
            }
            if ($jobMinimumSalary.val().length === 0) {
                $('#jobMinimumSalaryHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['jobMinimumSalaryLabel'] + " </span>");
                $('#jobMinimumSalaryForm').removeClass().addClass('form-group has-error');
                $jobMinimumSalary.focus();
                return false;
            }
            if ($jobMaximumSalary.val().length === 0) {
                $('#jobMaximumSalaryHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['jobMaximumSalaryLabel'] + " </span>");
                $('#jobMaximumSalaryForm').removeClass().addClass('form-group has-error');
                $jobMaximumSalary.focus();
                return false;
            }
            if ($jobDescription.val().length === 0) {
                $('#jobDescriptionHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['jobDescriptionLabel'] + " </span>");
                $('#jobDescriptionForm').removeClass().addClass('form-group has-error');
            ...
            $.ajax({
                ...
                data: {
                    ...
                    jobExperienceRequired: $jobExperienceRequired.val(),
                    jobCode: $jobCode.val(),
                    jobTitle: $jobTitle.val(),
                    jobMinimumSalary: $jobMinimumSalary.val(),
                    jobMaximumSalary: $jobMaximumSalary.val(),
                ...
                success: function(data) {
                    ...
                    if (success === true) {
                        ...
                        $jobCategoryId.val('');
                        $jobCategoryId.trigger("chosen:updated");
                        $('#jobCategoryIdHelpMe').html('').empty();
                        $countryId.val('');
                        $countryId.trigger("chosen:updated");
                        $('#countryIdHelpMe').html('').empty();
                        $jobExperienceRequired.val('');
                        $('#jobExperienceRequiredHelpMe').html('').empty();
                        $jobCode.val('');
                        $('#jobCodeHelpMe').html('').empty();
                        $jobTitle.val('');
                        $('#jobTitleHelpMe').html('').empty();
                        $jobMinimumSalary.val('');
                        $('#jobMinimumSalaryHelpMe').html('').empty();
                        $jobMaximumSalary.val('');
                        $('#jobMaximumSalaryHelpMe').html('').empty();
                        $jobDescription.val('');
                        $('#jobDescriptionForm').removeClass().addClass('form-group');
                        $('#jobDescriptionHelpMe').html('').empty();
                    } else if (success === false) {
                        $infoPanel.html('').empty().html("<span class='label label-important'>&nbsp;" + message + "</span>");
                        if ($infoPanel.is(':hidden')) {
                            $infoPanel.show();
...
function resetRecord(leafId, url, urlList, securityToken, createAccess, updateAccess, deleteAccess) {
    ...
    $("#jobCategoryId").val('');
    $("#jobCategoryIdHelpMe").empty().html('');
    $('#jobCategoryId').trigger("chosen:updated");
    $("#countryId").val('');
    $("#countryIdHelpMe").empty().html('');
    $('#countryId').trigger("chosen:updated");
    $("#jobExperienceRequired").val('');
    $("#jobExperienceRequiredHelpMe").empty().html('');
    $("#jobCode").val('');
    $("#jobCodeHelpMe").empty().html('');
    $("#jobTitle").val('');
    $("#jobTitleHelpMe").empty().html('');
    $("#jobMinimumSalary").val('');
    $("#jobMinimumSalaryHelpMe").empty().html('');
    $("#jobMaximumSalary").val('');
    $("#jobMaximumSalaryHelpMe").empty().html('');
    $("#jobDescription").val('');
    $("#jobDescriptionHelpMe").empty().html('');
    $('#jobDescription').empty().val('');
}
function postRecord() {
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if (success === true) {
                                ...
                                $('#jobCategoryId').val(data.data.jobCategoryId).trigger("chosen:updated");
                                $('#countryId').val(data.data.countryId).trigger("chosen:updated");
                                $('#jobExperienceRequired').val(data.data.jobExperienceRequired);
                                $('#jobCode').val(data.data.jobCode);
                                $('#jobTitle').val(data.data.jobTitle);
                                $('#jobMinimumSalary').val(data.data.jobMinimumSalary);
                                $('#jobMaximumSalary').val(data.data.jobMaximumSalary);
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($previousRecordCounter.val()) > 0 && parseFloat($previousRecordCounter.val()) < parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    jobId: $previousRecordCounter.val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
