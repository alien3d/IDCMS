...
function getJobRateType(leafId, url, securityToken) {
    $.ajax({
        type: 'GET',
        url: url,
        data: {
        ...
        success: function(data) {
            ...
            if (success === false) {
                ...
                $("#jobRateTypeId").html('').empty().html(data.data).trigger("chosen:updated");
                $infoPanel.html('').empty().html("<span class='label label-success'>&nbsp;<img src='" + smileyLol + "'>  " + decodeURIComponent(t['loadingCompleteTextLabel']) + "</span>").delay(5000).fadeOut();
            }
        },
        error: function(xhr) {
...
function checkDuplicate(leafId, page, securityToken) {
    var $jobRateCode = $("#jobRateCode");
    if ($jobRateCode.val().length === 0) {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if (success === true) {
                if (total !== 0) {
                    $("#jobRateCode").val('').focus();
                    $("#jobRateCodeForm").removeClass().addClass("form-group has-error");
                ...
                $("#jobRateForm").removeClass().addClass("form-group has-error");
            }
            if ($infoPanel.is(':hidden')) {
                $infoPanel.show();
            }
...
function showFormUpdate(leafId, url, urlList, securityToken, jobRateId, updateAccess, deleteAccess) {
    sleep(500);
...
function showModalDelete(jobRateId, jobId, jobRateTypeId, countryId, jobRateCode, jobRateAmount, jobRateDescription) {
    $('#jobRateIdPreview').val('').val(decodeURIComponent(jobRateId));
    $('#jobIdPreview').val('').val(decodeURIComponent(jobId));
    $('#jobRateTypeIdPreview').val('').val(decodeURIComponent(jobRateTypeId));
    $('#countryIdPreview').val('').val(decodeURIComponent(countryId));
    $('#jobRateCodePreview').val('').val(decodeURIComponent(jobRateCode));
    $('#jobRateAmountPreview').val('').val(decodeURIComponent(jobRateAmount));
    $('#jobRateDescriptionPreview').val('').val(decodeURIComponent(jobRateDescription));
...
function deleteGridRecord(leafId, url, urlList, securityToken) {
    $.ajax({
        ...
        data: {
            method: 'delete',
            output: 'json',
            jobRateId: $('#jobRateIdPreview').val(),
            securityToken: securityToken,
            leafId: leafId
        },
        beforeSend: function() {
...
function deleteGridRecordCheckbox(leafId, url, urlList, securityToken) {
    var stringText = '';
    var counter = 0;
    $('input:checkbox[name="jobRateId[]"]').each(function() {
        stringText = stringText + "&jobRateId[]=" + $(this).val();
    });
...
function newRecord(leafId, url, urlList, securityToken, type, createAccess, updateAccess, deleteAccess) {
    ...
    var $jobId = $('#jobId');
    var $jobRateTypeId = $('#jobRateTypeId');
    var $countryId = $('#countryId');
    var $jobRateCode = $('#jobRateCode');
    var $jobRateAmount = $('#jobRateAmount');
    var $jobRateDescription = $('#jobRateDescription');
    if (css.search('disabled') > 0) {
        return false;
    } else {
        if (type === 1) {
            ...
            if ($jobRateTypeId.val().length === 0) {
                $('#jobRateTypeIdHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['jobRateTypeIdLabel'] + " </span>");
                $jobRateTypeId.data('chosen').activate_action();
                return false;
            }
            ...
            if ($jobRateCode.val().length === 0) {
                $('#jobRateCodeHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['jobRateCodeLabel'] + " </span>");
                $('#jobRateCodeForm').removeClass().addClass('form-group has-error');
                $jobRateCode.focus();
                return false;
            }
            if ($jobRateAmount.val().length === 0) {
                $('#jobRateAmountHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['jobRateAmountLabel'] + " </span>");
                $('#jobRateAmountForm').removeClass().addClass('form-group has-error');
                $jobRateAmount.focus();
                return false;
            }
            if ($jobRateDescription.val().length === 0) {
                $('#jobRateDescriptionHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['jobRateDescriptionLabel'] + " </span>");
                $('#jobRateDescriptionForm').removeClass().addClass('form-group has-error');
                $jobRateDescription.focus();
                return false;
            }
            $.ajax({
                type: 'POST',
                ...
                data: {
                    ...
                    output: 'json',
                    jobId: $jobId.val(),
                    jobRateTypeId: $jobRateTypeId.val(),
                    countryId: $countryId.val(),
                    jobRateCode: $jobRateCode.val(),
                    jobRateAmount: $jobRateAmount.val(),
                    jobRateDescription: $jobRateDescription.val(),
                    securityToken: securityToken,
                    leafId: leafId
                },
                beforeSend: function() {
                ...
                success: function(data) {
                    ...
                    if (success === true) {
                        ...
                        $jobRateTypeId.val('');
                        $jobRateTypeId.trigger("chosen:updated");
                        $('#jobRateTypeIdHelpMe').html('').empty();
                        $countryId.val('');
                        $countryId.trigger("chosen:updated");
                        $('#countryIdHelpMe').html('').empty();
                        $jobRateCode.val('');
                        $('#jobRateCodeHelpMe').html('').empty();
                        $jobRateAmount.val('');
                        $('#jobRateAmountHelpMe').html('').empty();
                        $jobRateDescription.val('');
                        $('#jobRateDescriptionForm').removeClass().addClass('form-group');
                        $('#jobRateDescriptionHelpMe').html('').empty();
                    } else if (success === false) {
                        $infoPanel.html('').empty().html("<span class='label label-important'>&nbsp;" + message + "</span>");
                        if ($infoPanel.is(':hidden')) {
                            $infoPanel.show();
...
function resetRecord(leafId, url, urlList, securityToken, createAccess, updateAccess, deleteAccess) {
    ...
    $("#jobRateId").val('');
    $("#jobRateIdHelpMe").empty().html('');
    
    $("#jobId").val('');
    $("#jobIdHelpMe").empty().html('');
    $('#jobId').trigger("chosen:updated");
    $("#jobRateTypeId").val('');
    $("#jobRateTypeIdHelpMe").empty().html('');
    $('#jobRateTypeId').trigger("chosen:updated");
    $("#countryId").val('');
    $("#countryIdHelpMe").empty().html('');
    $('#countryId').trigger("chosen:updated");
    $("#jobRateCode").val('');
    $("#jobRateCodeHelpMe").empty().html('');
    $("#jobRateAmount").val('');
    $("#jobRateAmountHelpMe").empty().html('');
    $("#jobRateDescription").val('');
    $("#jobRateDescriptionHelpMe").empty().html('');
    $('#jobRateDescription').empty().val('');
}
function postRecord() {
                ...
                if (success === true) {
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if (success === true) {
                                $('#jobRateId').val(data.data.jobRateId);
                                $('#jobId').val(data.data.jobId).trigger("chosen:updated");
                                $('#jobRateTypeId').val(data.data.jobRateTypeId).trigger("chosen:updated");
                                $('#countryId').val(data.data.countryId).trigger("chosen:updated");
                                $('#jobRateCode').val(data.data.jobRateCode);
                                $('#jobRateAmount').val(data.data.jobRateAmount);
                                $('#jobRateDescription').val(data.data.jobRateDescription);
                                if (nextRecord > 0) {
                                    $('#previousRecordButton').removeClass().addClass('btn btn-default disabled').attr('onClick', '');
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($previousRecordCounter.val()) > 0 && parseFloat($previousRecordCounter.val()) < parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    jobRateId: $previousRecordCounter.val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($nextRecordCounter.val()) <= parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    jobRateId: $nextRecordCounter.val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
