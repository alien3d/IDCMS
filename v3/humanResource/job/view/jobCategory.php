...
require_once($newFakeDocumentRoot . "v3/humanResource/job/controller/jobCategoryController.php");
...
$translator->setCurrentTable('jobCategory');
...
if (isset($_POST)) {
    if (isset($_POST['method'])) {
        $jobCategory = new \Core\HumanResource\Job\JobCategory\Controller\JobCategoryClass();
        define('LIMIT', 10);
        ...
        if (isset($_POST ['query'])) {
            $jobCategory->setFieldQuery($_POST ['query']);
        }
        if (isset($_POST ['filter'])) {
            $jobCategory->setGridQuery($_POST ['filter']);
        }
        if (isset($_POST ['character'])) {
            $jobCategory->setCharacterQuery($_POST['character']);
        }
        if (isset($_POST ['dateRangeStart'])) {
            $jobCategory->setDateRangeStartQuery($_POST['dateRangeStart']);
            //explode the data to get day,month,year
            $start = explode('-', $_POST ['dateRangeStart']);
            $jobCategory->setStartDay($start[2]);
            $jobCategory->setStartMonth($start[1]);
            $jobCategory->setStartYear($start[0]);
        }
        if (isset($_POST ['dateRangeEnd']) && (strlen($_POST['dateRangeEnd']) > 0)) {
            $jobCategory->setDateRangeEndQuery($_POST['dateRangeEnd']);
            //explode the data to get day,month,year
            $start = explode('-', $_POST ['dateRangeEnd']);
            $jobCategory->setEndDay($start[2]);
            $jobCategory->setEndMonth($start[1]);
            $jobCategory->setEndYear($start[0]);
        }
        if (isset($_POST ['dateRangeType'])) {
            $jobCategory->setDateRangeTypeQuery($_POST['dateRangeType']);
        }
        if (isset($_POST ['dateRangeExtraType'])) {
            $jobCategory->setDateRangeExtraTypeQuery($_POST['dateRangeExtraType']);
        }
        $jobCategory->setServiceOutput('html');
        $jobCategory->setLeafId($leafId);
        $jobCategory->execute();
        if ($_POST['method'] == 'read') {
            $jobCategory->setStart($offset);
            $jobCategory->setLimit($limit); // normal system don't like paging..
            $jobCategory->setPageOutput('html');
            $jobCategoryArray = $jobCategory->read();
            if (isset($jobCategoryArray [0]['firstRecord'])) {
                $firstRecord = $jobCategoryArray [0]['firstRecord'];
            }
            if (isset($jobCategoryArray [0]['nextRecord'])) {
                $nextRecord = $jobCategoryArray [0]['nextRecord'];
            }
            if (isset($jobCategoryArray [0]['previousRecord'])) {
                $previousRecord = $jobCategoryArray [0]['previousRecord'];
            }
            if (isset($jobCategoryArray [0]['lastRecord'])) {
                $lastRecord = $jobCategoryArray [0]['lastRecord'];
                $endRecord = $jobCategoryArray [0]['lastRecord'];
            }
            $navigation = new \Core\Paging\HtmlPaging();
            $navigation->setLeafId($leafId);
            $navigation->setViewPath($jobCategory->getViewPath());
            ...
            if (isset($jobCategoryArray [0]['total'])) {
                $total = $jobCategoryArray [0]['total'];
            } else {
                $total = 0;
            }
                        ...
                        <div class="btn-group">
                            ...
                            <ul class="dropdown-menu" style="text-align:left">
                                <li>
                                    ...
                                    echo $jobCategory->getControllerPath();
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <form class="form-horizontal">
                                        <input type="hidden" name="jobCategoryIdPreview" id="jobCategoryIdPreview">

                                        <div class="form-group" id="jobCategoryCodeDiv">
                                            <label for="jobCategoryCodePreview"
                                                   class="control-label col-xs-4 col-sm-4 col-md-4"><?php echo $leafTranslation['jobCategoryCodeLabel']; ?></label>

                                            <div class="col-xs-8 col-sm-8 col-md-8">
                                                <input class="form-control" type="text" name="jobCategoryCodePreview"
                                                       id="jobCategoryCodePreview">
                                            </div>
                                        </div>
                                        <div class="form-group" id="jobCategoryDescriptionDiv">
                                            <label for="jobCategoryDescriptionPreview"
                                                   class="control-label col-xs-4 col-sm-4 col-md-4"><?php echo $leafTranslation['jobCategoryDescriptionLabel']; ?></label>

                                            <div class="col-xs-8 col-sm-8 col-md-8">
                                                <input class="form-control" type="text"
                                                       name="jobCategoryDescriptionPreview" id="jobCategoryDescriptionPreview">
                                            </div>
                                        </div>
                                    </form>
                                </div>
                        ...
                        <div class="col-xs-12 col-sm-12 col-md-12">
                            <div class="panel panel-default">
                                <table class="table table table-bordered table-striped table-condensed table-hover smart-form has-tickbox" id="tableData">
                                    ...
                                    <div align="center"><?php echo ucwords($leafTranslation['jobCategoryCodeLabel']); ?></div>
                                    </th>
                                    <th><?php echo ucwords($leafTranslation['jobCategoryDescriptionLabel']); ?></th>
                                    <th width="100px">
                                            ...
                                            if (is_array($jobCategoryArray)) {
                                                ...
                                                if ($totalRecord > 0) {
                                                    ...
                                                    for ($i = 0; $i < $totalRecord; $i++) {
                                                        ...
                                                        ?>
                                                        <tr <?php
                                                        if ($jobCategoryArray[$i]['isDelete'] == 1) {
                                                            echo "class=\"danger\"";
                                                        } else {
                                                            if ($jobCategoryArray[$i]['isDraft'] == 1) {
                                                                echo "class=\"warning\"";
                                                            }
                                                        }
                                                        ?>>
                                                            ...
                                                            <td align="center">
                                                                <div class="btn-group" align="center">
                                                                    <button type="button"  class="btn btn-info btn-sm" title="Edit"
                                                                            ...
                                                                            echo intval(
                                                                                    $jobCategoryArray [$i]['jobCategoryId']
                                                                            );
                                                                    ...
                                                                    <button type="button"  class="btn btn-danger btn-sm" title="Delete"
                                                                            ...
                                                                            ?>', '<?php
                                                                            echo rawurlencode(
                                                                                    $jobCategoryArray [$i]['jobCategoryCode']
                                                                            );
                                                                            ?>', '<?php
                                                                            echo rawurlencode(
                                                                                    $jobCategoryArray [$i]['jobCategoryDescription']
                                                                            );
                                                                            ?>');">
                                                            ...
                                                            <td>
                                                                <div align="center">
                                                                    <?php
                                                                    if (isset($jobCategoryArray[$i]['jobCategoryCode'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos(
                                                                                                strtolower($jobCategoryArray[$i]['jobCategoryCode']), strtolower($_POST['query'])
                                                                                        ) !== false
                                                                                ) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $jobCategoryArray[$i]['jobCategoryCode']
                                                                                    );
                                                                                } else {
                                                                                    echo $jobCategoryArray[$i]['jobCategoryCode'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                    if (strpos(
                                                                                                    strtolower($jobCategoryArray[$i]['jobCategoryCode']), strtolower($_POST['character'])
                                                                                            ) !== false
                                                                                    ) {
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $jobCategoryArray[$i]['jobCategoryCode']
                                                                                        );
                                                                                    } else {
                                                                                        echo $jobCategoryArray[$i]['jobCategoryCode'];
                                                                                    }
                                                                                } else {
                                                                                    if (!empty($jobCategoryArray[$i]['jobCategoryCode'])) {
                                                                                        echo $jobCategoryArray[$i]['jobCategoryCode'];
                                                                                    } else {
                                                                                        echo "-";
                                                                                    }
                                                            ...
                                                            <td>
                                                                <div class="pull-left">
                                                                    <?php
                                                                    if (isset($jobCategoryArray[$i]['jobCategoryDescription'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos(
                                                                                                strtolower($jobCategoryArray[$i]['jobCategoryDescription']), strtolower($_POST['query'])
                                                                                        ) !== false
                                                                                ) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $jobCategoryArray[$i]['jobCategoryDescription']
                                                                                    );
                                                                                } else {
                                                                                    echo $jobCategoryArray[$i]['jobCategoryDescription'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                    if (strpos(
                                                                                                    strtolower($jobCategoryArray[$i]['jobCategoryDescription']), strtolower($_POST['character'])
                                                                                            ) !== false
                                                                                    ) {
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $jobCategoryArray[$i]['jobCategoryDescription']
                                                                                        );
                                                                                    } else {
                                                                                        echo $jobCategoryArray[$i]['jobCategoryDescription'];
                                                                                    }
                                                                                } else {
                                                                                    if (!empty($jobCategoryArray[$i]['jobCategoryDescription'])) {
                                                                                        echo $jobCategoryArray[$i]['jobCategoryDescription'];
                                                                                    } else {
                                                                                        echo "-";
                                                                                    }
                                                            ...
                                                            <td>
                                                                <div align="center">
                                                                    <?php
                                                                    if (isset($jobCategoryArray[$i]['executeBy'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos($jobCategoryArray[$i]['staffName'], $_POST['query']) !== false) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $jobCategoryArray[$i]['staffName']
                                                                                    );
                                                                                } else {
                                                                                    echo $jobCategoryArray[$i]['staffName'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                    if (strpos(
                                                                                                    $jobCategoryArray[$i]['staffName'], $_POST['character']
                                                                                            ) !== false
                                                                                    ) {
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $jobCategoryArray[$i]['staffName']
                                                                                        );
                                                                                    } else {
                                                                                        echo $jobCategoryArray[$i]['staffName'];
                                                                                    }
                                                            ...
                                                            <?php
                                                            if (isset($jobCategoryArray[$i]['executeTime'])) {
                                                                $valueArray = $jobCategoryArray[$i]['executeTime'];
                                                            ...
                                                            <?php } ?>
                                                            <?php
                                                            if ($jobCategoryArray[$i]['isDelete']) {
                                                                $checked = "checked";
                                                            } else {
                                                                $checked = null;
                                                            }
                                                            ?>
                                                            <td>
                                                                <input style="display:none;" type="checkbox" name="jobCategoryId[]"
                                                                       value="<?php echo $jobCategoryArray[$i]['jobCategoryId']; ?>">
                                                                <input <?php echo $checked; ?> type="checkbox" name="isDelete[]"
                                                                                               value="<?php echo $jobCategoryArray[$i]['isDelete']; ?>">
                                                            </td>
                                                        </tr>
                                                        <?php
                                                    }
                        ...
                        <div class="col-xs-3 col-sm-3 col-md-3 pagination">
                            <div class="pull-right">
                                <button class="delete btn btn-warning" type="button" 
                                        ...
                                        echo $jobCategory->getControllerPath();
                                        ?>', '<?php echo $jobCategory->getViewPath(); ?>', '<?php echo $securityToken; ?>');">
...
if ((isset($_POST['method']) == 'new' || isset($_POST['method']) == 'read') && $_POST['type'] == 'form') {
            ...
            <div class="row">
                <div class="col-xs-12 col-sm-12 col-md-12">
                    <div class="panel panel-info">
                        ...
                        <div class="panel-body">
                            <input type="hidden" name="jobCategoryId" id="jobCategoryId" value="<?php
                            if (isset($_POST['jobCategoryId'])) {
                                echo $_POST['jobCategoryId'];
                            }
                            ?>">

                            <div class="row">
                                <div class="col-xs-12 col-sm-12 col-md-12">
                                    <div class="form-group" id="jobCategoryCodeForm">
                                        <label for="jobCategoryCode" class="control-label col-xs-4 col-sm-4 col-md-4"><strong><?php
                                                echo ucfirst(
                                                        $leafTranslation['jobCategoryCodeLabel']
                                                );
                                                ?></strong></label>

                                        <div class="col-xs-8 col-sm-8 col-md-8">
                                            <input type="text" class="form-control" name="jobCategoryCode"
                                                   id="jobCategoryCode" onKeyUp="removeMeError('jobCategoryCode');"
                                                   class="col-xs-11 col-sm-11 col-md-11"
                                                   value="<?php
                                                   if (isset($jobCategoryArray) && is_array($jobCategoryArray)) {
                                                       if (isset($jobCategoryArray[0]['jobCategoryCode'])) {
                                                           echo htmlentities($jobCategoryArray[0]['jobCategoryCode']);
                                                       }
                                                   }
                                                   ?>" maxlength="16"> <span class="help-block" id="jobCategoryCodeHelpMe"></span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-xs-12 col-sm-12 col-md-12">
                                    <div class="form-group" id="jobCategoryDescriptionForm">
                                        <div class="col-xs-12 col-sm-12 col-md-12">
                                            <textarea class="form-control" name="jobCategoryDescription"
                                                      id="jobCategoryDescription"><?php
                                                          if (isset($jobCategoryArray[0]['jobCategoryDescription'])) {
                                                              echo htmlentities($jobCategoryArray[0]['jobCategoryDescription']);
                                                          }
                                                          ?></textarea> <span class="help-block" id="jobCategoryDescriptionHelpMe"></span>
                                        </div>
                                    </div>
                                </div>
                            </div>
    ...
    <?php if ($leafAccess['leafAccessCreateValue'] == 1) { ?>
                        ...
                        if (e.which === 78 && e.which === 18  && e.shiftKey) {
                            ...
                            newRecord(<?php echo $leafId; ?>, '<?php echo $jobCategory->getControllerPath(); ?>', '<?php echo $jobCategory->getViewPath(); ?>', '<?php echo $securityToken; ?>', 1);
    ...
    <?php if ($leafAccess['leafAccessUpdateValue'] == 1) { ?>
                        ...
                        if (e.which === 83 && e.which === 18  && e.shiftKey) {
                            ...
                            updateRecord(<?php echo $leafId; ?>, '<?php echo $jobCategory->getControllerPath(); ?>', '<?php echo $jobCategory->getViewPath(); ?>', '<?php echo $securityToken; ?>', 1, <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
    ...
    <?php if ($leafAccess['leafAccessDeleteValue'] == 1) { ?>
                        ...
                        if (e.which === 88 && e.which === 18 && e.shiftKey) {
                            ...
                            deleteRecord(<?php echo $leafId; ?>, '<?php echo $jobCategory->getControllerPath(); ?>', '<?php echo $jobCategory->getViewPath(); ?>', '<?php echo $securityToken; ?>', <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
    ...
    <?php } ?>
                    switch (e.keyCode) {
                        case 37:
                            previousRecord(<?php echo $leafId; ?>, '<?php echo $jobCategory->getControllerPath(); ?>', '<?php echo $jobCategory->getViewPath(); ?>', '<?php echo $securityToken; ?>', <?php echo $leafAccess['leafAccessUpdateValue']; ?>, <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
                            
                            return false;
                            break;
                        case 39:
                            nextRecord(<?php echo $leafId; ?>, '<?php echo $jobCategory->getControllerPath(); ?>', '<?php echo $jobCategory->getViewPath(); ?>', '<?php echo $securityToken; ?>', <?php echo $leafAccess['leafAccessUpdateValue']; ?>, <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
                ...
                $(document).ready(function() {
                    ...
                    validateMeAlphaNumeric('jobCategoryCode');
    <?php if ($_POST['method'] == "new") { ?>
                        $('#resetRecordButton')
                                .removeClass().addClass('btn btn-default');
        <?php if ($leafAccess['leafAccessCreateValue'] == 1) { ?>
                            $('#newRecordButton1').removeClass().addClass('btn btn-success').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $jobCategory->getControllerPath(); ?>','<?php echo $jobCategory->getViewPath(); ?>','<?php echo $securityToken; ?>',1)");
                            $('#newRecordButton2').removeClass().addClass('btn dropdown-toggle btn-success');
                            $('#newRecordButton3').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $jobCategory->getControllerPath(); ?>','<?php echo $jobCategory->getViewPath(); ?>','<?php echo $securityToken; ?>',1)");
                            $('#newRecordButton4').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $jobCategory->getControllerPath(); ?>','<?php echo $jobCategory->getViewPath(); ?>','<?php echo $securityToken; ?>',2)");
                            $('#newRecordButton5').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $jobCategory->getControllerPath(); ?>','<?php echo $jobCategory->getViewPath(); ?>','<?php echo $securityToken; ?>',3)");
                            $('#newRecordButton6').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $jobCategory->getControllerPath(); ?>','<?php echo $jobCategory->getViewPath(); ?>','<?php echo $securityToken; ?>',4)");
                            $('#newRecordButton7').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $jobCategory->getControllerPath(); ?>','<?php echo $jobCategory->getViewPath(); ?>','<?php echo $securityToken; ?>',5)");
        <?php } else { ?>
                            $('#newRecordButton1')
        ...
        if ($_POST['jobCategoryId']) {
            ...
            <?php if ($leafAccess['leafAccessUpdateValue'] == 1) { ?>
                                $('#updateRecordButton1')
                                        .removeClass().addClass('btn btn-info').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $jobCategory->getControllerPath(); ?>','<?php echo $jobCategory->getViewPath(); ?>','<?php echo $securityToken; ?>',1,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                                $('#updateRecordButton2')
                                        .removeClass().addClass('btn dropdown-toggle btn-info');
                                $('#updateRecordButton3').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $jobCategory->getControllerPath(); ?>','<?php echo $jobCategory->getViewPath(); ?>','<?php echo $securityToken; ?>',1,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                                $('#updateRecordButton4').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $jobCategory->getControllerPath(); ?>','<?php echo $jobCategory->getViewPath(); ?>','<?php echo $securityToken; ?>',2,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                                $('#updateRecordButton5').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $jobCategory->getControllerPath(); ?>','<?php echo $jobCategory->getViewPath(); ?>','<?php echo $securityToken; ?>',3,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
            <?php } else { ?>
                                $('#updateRecordButton1')
                                        .removeClass().addClass(' btn btn-info disabled');
                                $('#updateRecordButton2')
            ...
            <?php if ($leafAccess['leafAccessDeleteValue'] == 1) { ?>
                                $('#deleteRecordButton')
                                        ...
                                        .attr('onClick', "deleteRecord(<?php echo $leafId; ?>,'<?php echo $jobCategory->getControllerPath(); ?>','<?php echo $jobCategory->getViewPath(); ?>','<?php echo $securityToken; ?>',<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
            <?php } else { ?>
                                $('#deleteRecordButton')
                                        .removeClass().addClass('btn btn-danger disabled')
                                        .attr('onClick', '');
...
<?php } ?>
<script type="text/javascript" src="./v3/humanResource/job/javascript/jobCategory.js"></script>
<hr>
