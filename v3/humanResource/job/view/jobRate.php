...
require_once($newFakeDocumentRoot . "v3/humanResource/job/controller/jobRateController.php");
...
$translator->setCurrentTable('jobRate');
...
$jobRateArray = array();
$jobArray = array();
$jobRateTypeArray = array();
if (isset($_POST)) {
    if (isset($_POST['method'])) {
        $jobRate = new \Core\HumanResource\Job\JobRate\Controller\JobRateClass();
        define('LIMIT', 10);
        ...
        if (isset($_POST ['query'])) {
            $jobRate->setFieldQuery($_POST ['query']);
        }
        if (isset($_POST ['filter'])) {
            $jobRate->setGridQuery($_POST ['filter']);
        }
        if (isset($_POST ['character'])) {
            $jobRate->setCharacterQuery($_POST['character']);
        }
        if (isset($_POST ['dateRangeStart'])) {
            $jobRate->setDateRangeStartQuery($_POST['dateRangeStart']);
            //explode the data to get day,month,year
            $start = explode('-', $_POST ['dateRangeStart']);
            $jobRate->setStartDay($start[2]);
            $jobRate->setStartMonth($start[1]);
            $jobRate->setStartYear($start[0]);
        }
        if (isset($_POST ['dateRangeEnd']) && (strlen($_POST['dateRangeEnd']) > 0)) {
            $jobRate->setDateRangeEndQuery($_POST['dateRangeEnd']);
            //explode the data to get day,month,year
            $start = explode('-', $_POST ['dateRangeEnd']);
            $jobRate->setEndDay($start[2]);
            $jobRate->setEndMonth($start[1]);
            $jobRate->setEndYear($start[0]);
        }
        if (isset($_POST ['dateRangeType'])) {
            $jobRate->setDateRangeTypeQuery($_POST['dateRangeType']);
        }
        if (isset($_POST ['dateRangeExtraType'])) {
            $jobRate->setDateRangeExtraTypeQuery($_POST['dateRangeExtraType']);
        }
        $jobRate->setServiceOutput('html');
        $jobRate->setLeafId($leafId);
        $jobRate->execute();
        $jobArray = $jobRate->getJob();
        $jobRateTypeArray = $jobRate->getJobRateType();
        if ($_POST['method'] == 'read') {
            $jobRate->setStart($offset);
            $jobRate->setLimit($limit); // normal system don't like paging..
            $jobRate->setPageOutput('html');
            $jobRateArray = $jobRate->read();
            if (isset($jobRateArray [0]['firstRecord'])) {
                $firstRecord = $jobRateArray [0]['firstRecord'];
            }
            if (isset($jobRateArray [0]['nextRecord'])) {
                $nextRecord = $jobRateArray [0]['nextRecord'];
            }
            if (isset($jobRateArray [0]['previousRecord'])) {
                $previousRecord = $jobRateArray [0]['previousRecord'];
            }
            if (isset($jobRateArray [0]['lastRecord'])) {
                $lastRecord = $jobRateArray [0]['lastRecord'];
                $endRecord = $jobRateArray [0]['lastRecord'];
            }
            $navigation = new \Core\Paging\HtmlPaging();
            $navigation->setLeafId($leafId);
            $navigation->setViewPath($jobRate->getViewPath());
            ...
            if (isset($jobRateArray [0]['total'])) {
                $total = $jobRateArray [0]['total'];
            } else {
                $total = 0;
            }
                        ...
                        <div class="btn-group">
                            ...
                            <ul class="dropdown-menu" style="text-align:left">
                                <li>
                                    ...
                                    echo $jobRate->getControllerPath();
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <form class="form-horizontal">
                                        <input type="hidden" name="jobRateIdPreview" id="jobRateIdPreview">
                                        ...
                                        <div class="form-group" id="jobRateTypeIdDiv">
                                            <label for="jobRateTypeIdPreview"
                                                   class="control-label col-xs-4 col-sm-4 col-md-4"><?php echo $leafTranslation['jobRateTypeIdLabel']; ?></label>

                                            <div class="col-xs-8 col-sm-8 col-md-8">
                                                <input class="form-control" type="text" name="jobRateTypeIdPreview"
                                                       id="jobRateTypeIdPreview">
                                            </div>
                                        </div>
                                        <div class="form-group" id="jobRateCodeDiv">
                                            <label for="jobRateCodePreview"
                                                   class="control-label col-xs-4 col-sm-4 col-md-4"><?php echo $leafTranslation['jobRateCodeLabel']; ?></label>

                                            <div class="col-xs-8 col-sm-8 col-md-8">
                                                <input class="form-control" type="text" name="jobRateCodePreview"
                                                       id="jobRateCodePreview">
                                            </div>
                                        </div>
                                        <div class="form-group" id="jobRateAmountDiv">
                                            <label for="jobRateAmountPreview"
                                                   class="control-label col-xs-4 col-sm-4 col-md-4"><?php echo $leafTranslation['jobRateAmountLabel']; ?></label>

                                            <div class="col-xs-8 col-sm-8 col-md-8">
                                                <input class="form-control" type="text" name="jobRateAmountPreview"
                                                       id="jobRateAmountPreview">
                                            </div>
                                        </div>
                                        <div class="form-group" id="jobRateDescriptionDiv">
                                            <label for="jobRateDescriptionPreview"
                                                   class="control-label col-xs-4 col-sm-4 col-md-4"><?php echo $leafTranslation['jobRateDescriptionLabel']; ?></label>

                                            <div class="col-xs-8 col-sm-8 col-md-8">
                                                <input class="form-control" type="text" name="jobRateDescriptionPreview"
                                                       id="jobRateDescriptionPreview">
                                            </div>
                                        </div>
                                    </form>
                                </div>
                        ...
                        <div class="col-xs-12 col-sm-12 col-md-12">
                            <div class="panel panel-default">
                                <table class="table table table-bordered table-striped table-condensed table-hover smart-form has-tickbox" id="tableData">
                                    ...
                                    <th width="75px"><?php echo ucfirst($t['actionTextLabel']); ?></th>
                                    <th width="125px"><?php echo ucwords($leafTranslation['jobIdLabel']); ?></th>
                                    <th width="125px"><?php echo ucwords($leafTranslation['jobRateTypeIdLabel']); ?></th>
                                    <th><?php echo ucwords($leafTranslation['jobRateDescriptionLabel']); ?></th>
                                    <th width="100px"><?php echo ucwords($leafTranslation['jobRateAmountLabel']); ?></th>
                                    ...
                                    <tbody id="tableBody">
                                        ...
                                        if ($_POST['method'] == 'read' && $_POST['type'] == 'list' && $_POST['detail'] == 'body') {
                                            if (is_array($jobRateArray)) {
                                                $totalRecord = intval(count($jobRateArray));
                                                if ($totalRecord > 0) {
                                                    $counter = 0;
                                                    for ($i = 0; $i < $totalRecord; $i++) {
                                                        ...
                                                        ?>
                                                        <tr <?php
                                                        if ($jobRateArray[$i]['isDelete'] == 1) {
                                                            echo "class=\"danger\"";
                                                        } else {
                                                            if ($jobRateArray[$i]['isDraft'] == 1) {
                                                                echo "class=\"warning\"";
                                                            }
                                                        }
                                                        ?>>
                                                            ...
                                                            <td align="center">
                                                                <div class="btn-group" align="center">
                                                                    ...
                                                                    <button type="button"  class="btn btn-danger btn-sm" title="Delete"
                                                                            ...
                                                                            ?>', '<?php
                                                                            echo rawurlencode(
                                                                                    $jobRateArray [$i]['jobTitle']
                                                                            );
                                                                            ?>', '<?php
                                                                            echo rawurlencode(
                                                                                    $jobRateArray [$i]['jobRateTypeDescription']
                                                                            );
                                                                            ?>', '<?php
                                                                            echo rawurlencode(
                                                                                    $jobRateArray [$i]['jobRateCode']
                                                                            );
                                                                            ?>', '<?php
                                                                            echo rawurlencode(
                                                                                    $jobRateArray [$i]['jobRateAmount']
                                                                            );
                                                                            ?>', '<?php
                                                                            echo rawurlencode(
                                                                                    $jobRateArray [$i]['jobRateDescription']
                                                                            );
                                                                            ?>');">
                                                            ...
                                                            <td>
                                                                <div class="pull-left">
                                                                    <?php
                                                                    if (isset($jobRateArray[$i]['jobTitle'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos($jobRateArray[$i]['jobTitle'], $_POST['query']) !== false) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $jobRateArray[$i]['jobTitle']
                                                                                    );
                                                                                } else {
                                                                                    echo $jobRateArray[$i]['jobTitle'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                    if (strpos(
                                                                                                    $jobRateArray[$i]['jobTitle'], $_POST['character']
                                                                                            ) !== false
                                                                                    ) {
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $jobRateArray[$i]['jobTitle']
                                                                                        );
                                                                                    } else {
                                                                                        echo $jobRateArray[$i]['jobTitle'];
                                                                                    }
                                                            ...
                                                            <td>
                                                                <div class="pull-left">
                                                                    <?php
                                                                    if (isset($jobRateArray[$i]['jobRateTypeDescription'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos(
                                                                                                strtolower($jobRateArray[$i]['jobRateTypeDescription']), strtolower($_POST['query'])
                                                                                        ) !== false
                                                                                ) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $jobRateArray[$i]['jobRateTypeDescription']
                                                                                    );
                                                                                } else {
                                                                                    echo $jobRateArray[$i]['jobRateTypeMultiplier'] . " " . $jobRateArray[$i]['jobRateTypeDescription'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                    if (strpos(
                                                                                                    strtolower($jobRateArray[$i]['jobRateTypeDescription']), strtolower($_POST['character'])
                                                                                            ) !== false
                                                                                    ) {
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $jobRateArray[$i]['jobRateTypeDescription']
                                                                                        );
                                                                                    } else {
                                                                                        echo $jobRateArray[$i]['jobRateTypeMultiplier'] . " " . $jobRateArray[$i]['jobRateTypeDescription'];
                                                                                    }
                                                            ...
                                                            <td>
                                                                <div class="pull-left">
                                                                    <?php
                                                                    if (isset($jobRateArray[$i]['jobRateDescription'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos(
                                                                                                strtolower($jobRateArray[$i]['jobRateDescription']), strtolower($_POST['query'])
                                                                                        ) !== false
                                                                                ) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $jobRateArray[$i]['jobRateDescription']
                                                                                    );
                                                                                } else {
                                                                                    echo $jobRateArray[$i]['jobRateDescription'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                    if (strpos(
                                                                                                    strtolower($jobRateArray[$i]['jobRateDescription']), strtolower($_POST['character'])
                                                                                            ) !== false
                                                                                    ) {
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $jobRateArray[$i]['jobRateDescription']
                                                                                        );
                                                                                    } else {
                                                                                        echo $jobRateArray[$i]['jobRateDescription'];
                                                                                    }
                                                            ...
                                                            <?php
                                                            $d = $jobRateArray[$i]['jobRateAmount'];
                                                            ...
                                                            <td>
                                                                <div class="pull-left">
                                                                    <?php
                                                                    if (isset($jobRateArray[$i]['executeBy'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos($jobRateArray[$i]['staffName'], $_POST['query']) !== false) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $jobRateArray[$i]['staffName']
                                                                                    );
                                                                                } else {
                                                                                    echo $jobRateArray[$i]['staffName'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                    if (strpos(
                                                                                                    $jobRateArray[$i]['staffName'], $_POST['character']
                                                                                            ) !== false
                                                                                    ) {
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $jobRateArray[$i]['staffName']
                                                                                        );
                                                                                    } else {
                                                                                        echo $jobRateArray[$i]['staffName'];
                                                                                    }
                                                            ...
                                                            <?php
                                                            if (isset($jobRateArray[$i]['executeTime'])) {
                                                                $valueArray = $jobRateArray[$i]['executeTime'];
                                                            ...
                                                            ?>
                                                            <td>
                                                                <input style="display:none;" type="checkbox" name="jobRateId[]"
                                                                       value="<?php echo $jobRateArray[$i]['jobRateId']; ?>">
                                                                <input <?php echo $checked; ?> type="checkbox" name="isDelete[]"
                                                                                               value="<?php echo $jobRateArray[$i]['isDelete']; ?>">
                                                            </td>
                                                        </tr>
                                                        <?php
                                                    }
                        ...
                        <div class="col-xs-3 col-sm-3 col-md-3 pagination">
                            <div class="pull-right">
                                <button class="delete btn btn-warning" type="button" 
                                        ...
                                        echo $jobRate->getControllerPath();
                                        ?>', '<?php echo $jobRate->getViewPath(); ?>', '<?php echo $securityToken; ?>');">
...
if ((isset($_POST['method']) == 'new' || isset($_POST['method']) == 'read') && $_POST['type'] == 'form') {
            ...
            <div class="row">
                <div class="col-xs-12 col-sm-12 col-md-12">
                    <div class="panel panel-info">
                        ...
                        <div class="panel-body">
                            <input type="hidden" name="jobRateId" id="jobRateId" value="<?php
                            if (isset($_POST['jobRateId'])) {
                                echo $_POST['jobRateId'];
                            }
                            ?>">
                            ...
                            <div class="row">
                                <div class="col-xs-12 col-sm-12 col-md-12">
                                            ...
                                            <select name="jobId" id="jobId" class="chzn-select form-control">
                                                ...
                                                if (is_array($jobArray)) {
                                                    ...
                                                    if ($totalRecord > 0) {
                                                        $d = 1;
                                                        for ($i = 0; $i < $totalRecord; $i++) {
                                                            if (isset($jobRateArray[0]['jobId'])) {
                                                                if ($jobRateArray[0]['jobId'] == $jobArray[$i]['jobId']) {
                                                                    $selected = "selected";
                                                                } else {
                                                                    $selected = null;
                                                                }
                                    ...
                                    <div class="col-xs-6 col-sm-6 col-md-6 form-group" id="jobRateTypeIdForm">
                                        <label for="jobRateTypeId" class="control-label col-xs-4 col-sm-4 col-md-4"><strong><?php
                                                echo ucfirst(
                                                        $leafTranslation['jobRateTypeIdLabel']
                                                );
                                                ?></strong></label>

                                        <div class="col-xs-8 col-sm-8 col-md-8">
                                            <select name="jobRateTypeId" id="jobRateTypeId" class="chzn-select form-control">
                                                <option value=""></option>
                                                <?php
                                                if (is_array($jobRateTypeArray)) {
                                                    $totalRecord = intval(count($jobRateTypeArray));
                                                    if ($totalRecord > 0) {
                                                        $d = 1;
                                                        for ($i = 0; $i < $totalRecord; $i++) {
                                                            if (isset($jobRateArray[0]['jobRateTypeId'])) {
                                                                if ($jobRateArray[0]['jobRateTypeId'] == $jobRateTypeArray[$i]['jobRateTypeId']) {
                                                                    $selected = "selected";
                                                                } else {
                                                                    $selected = null;
                                                                }
                                                            ...
                                                            ?>
                                                            <option
                                                                value="<?php echo $jobRateTypeArray[$i]['jobRateTypeId']; ?>" <?php echo $selected; ?>><?php echo $d; ?>
                                                                . <?php echo $jobRateTypeArray[$i]['jobRateTypeDescription']; ?></option>
                                                            <?php
                                                            $d++;
                                                        }
                                                    } else {
                                                    ...
                                                    <option value=""><?php echo $t['notAvailableTextLabel']; ?></option>
                                                <?php } ?>
                                            </select> <span class="help-block" id="jobRateTypeIdHelpMe"></span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-xs-12 col-sm-12 col-md-12">
                                    <div class="col-xs-6 col-sm-6 col-md-6 form-group" id="jobRateCodeForm">
                                        <label for="jobRateCode" class="control-label col-xs-4 col-sm-4 col-md-4"><strong><?php
                                                echo ucfirst(
                                                        $leafTranslation['jobRateCodeLabel']
                                                );
                                                ?></strong></label>

                                        <div class="col-xs-8 col-sm-8 col-md-8">
                                            <input type="text" class="form-control" name="jobRateCode" id="jobRateCode"
                                                   onKeyUp="removeMeError('jobRateCode');" value="<?php
                                                   if (isset($jobRateArray) && is_array($jobRateArray)) {
                                                       if (isset($jobRateArray[0]['jobRateCode'])) {
                                                           echo htmlentities($jobRateArray[0]['jobRateCode']);
                                                       }
                                                   }
                                                   ?>" maxlength="16"> <span class="help-block" id="jobRateCodeHelpMe"></span>
                                        </div>
                                    </div>
                                    <div class="col-xs-6 col-sm-6 col-md-6 form-group" id="jobRateAmountForm">
                                        <label for="jobRateAmount" class="control-label col-xs-4 col-sm-4 col-md-4"><strong><?php
                                                echo ucfirst(
                                                        $leafTranslation['jobRateAmountLabel']
                                                );
                                                ?></strong></label>

                                        <div class="col-xs-8 col-sm-8 col-md-8">
                                            <input type="text" class="form-control" name="jobRateAmount" id="jobRateAmount"
                                                   value="<?php
                                                   if (isset($jobRateArray[0]['jobRateAmount'])) {
                                                       if (isset($jobRateArray[0]['jobRateAmount'])) {
                                                           echo htmlentities($jobRateArray[0]['jobRateAmount']);
                                                       }
                                                   }
                                                   ?>"> <span class="help-block" id="jobRateAmountHelpMe"></span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-xs-12 col-sm-12 col-md-12">
                                    <div class="form-group" id="jobRateDescriptionForm">
                                        <div class="col-xs-12 col-sm-12 col-md-12">
                                            <textarea name="jobRateDescription" id="jobRateDescription" class="form-control"><?php
                                                if (isset($jobRateArray[0]['jobRateDescription'])) {
                                                    echo htmlentities($jobRateArray[0]['jobRateDescription']);
                                                }
                                                ?></textarea> <span class="help-block" id="jobRateDescriptionHelpMe"></span>
                                        </div>
                                    </div>
                                </div>
                            </div>
    ...
    <?php if ($leafAccess['leafAccessCreateValue'] == 1) { ?>
                        ...
                        if (e.which === 78 && e.which === 18  && e.shiftKey) {
                            ...
                            newRecord(<?php echo $leafId; ?>, '<?php echo $jobRate->getControllerPath(); ?>', '<?php echo $jobRate->getViewPath(); ?>', '<?php echo $securityToken; ?>', 1);
    ...
    <?php if ($leafAccess['leafAccessUpdateValue'] == 1) { ?>
                        ...
                        if (e.which === 83 && e.which === 18  && e.shiftKey) {
                            ...
                            updateRecord(<?php echo $leafId; ?>, '<?php echo $jobRate->getControllerPath(); ?>', '<?php echo $jobRate->getViewPath(); ?>', '<?php echo $securityToken; ?>', 1, <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
    ...
    <?php if ($leafAccess['leafAccessDeleteValue'] == 1) { ?>
                        ...
                        if (e.which === 88 && e.which === 18 && e.shiftKey) {
                            ...
                            deleteRecord(<?php echo $leafId; ?>, '<?php echo $jobRate->getControllerPath(); ?>', '<?php echo $jobRate->getViewPath(); ?>', '<?php echo $securityToken; ?>', <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
    ...
    <?php } ?>
                    switch (e.keyCode) {
                        case 37:
                            previousRecord(<?php echo $leafId; ?>, '<?php echo $jobRate->getControllerPath(); ?>', '<?php echo $jobRate->getViewPath(); ?>', '<?php echo $securityToken; ?>', <?php echo $leafAccess['leafAccessUpdateValue']; ?>, <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
                            
                            return false;
                            break;
                        case 39:
                            nextRecord(<?php echo $leafId; ?>, '<?php echo $jobRate->getControllerPath(); ?>', '<?php echo $jobRate->getViewPath(); ?>', '<?php echo $securityToken; ?>', <?php echo $leafAccess['leafAccessUpdateValue']; ?>, <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
                ...
                $(document).ready(function() {
                    ...
                    validateMeNumeric('jobRateId');
                    validateMeNumeric('jobId');
                    validateMeNumeric('jobRateTypeId');
                    validateMeAlphaNumeric('jobRateCode');
                    validateMeNumeric('jobRateAmount');
    <?php if ($_POST['method'] == "new") { ?>
                        $('#resetRecordButton')
                                .removeClass().addClass('btn btn-default');
        <?php if ($leafAccess['leafAccessCreateValue'] == 1) { ?>
                            $('#newRecordButton1')
                                    .removeClass().addClass('btn btn-success').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $jobRate->getControllerPath(); ?>','<?php echo $jobRate->getViewPath(); ?>','<?php echo $securityToken; ?>',1)");
                            $('#newRecordButton2')
                                    .removeClass().addClass('btn dropdown-toggle btn-success');


                            $('#newRecordButton3').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $jobRate->getControllerPath(); ?>','<?php echo $jobRate->getViewPath(); ?>','<?php echo $securityToken; ?>',1)");
                            $('#newRecordButton4').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $jobRate->getControllerPath(); ?>','<?php echo $jobRate->getViewPath(); ?>','<?php echo $securityToken; ?>',2)");
                            $('#newRecordButton5').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $jobRate->getControllerPath(); ?>','<?php echo $jobRate->getViewPath(); ?>','<?php echo $securityToken; ?>',3)");
                            $('#newRecordButton6').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $jobRate->getControllerPath(); ?>','<?php echo $jobRate->getViewPath(); ?>','<?php echo $securityToken; ?>',4)");
                            $('#newRecordButton7').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $jobRate->getControllerPath(); ?>','<?php echo $jobRate->getViewPath(); ?>','<?php echo $securityToken; ?>',5)");
        <?php } else { ?>
        ...
        if ($_POST['jobRateId']) {
            ...
            <?php if ($leafAccess['leafAccessUpdateValue'] == 1) { ?>
                                $('#updateRecordButton1')
                                        .removeClass().addClass('btn btn-info').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $jobRate->getControllerPath(); ?>','<?php echo $jobRate->getViewPath(); ?>','<?php echo $securityToken; ?>',1,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                                $('#updateRecordButton2')
                                        .removeClass().addClass('btn dropdown-toggle btn-info');
                                $('#updateRecordButton3').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $jobRate->getControllerPath(); ?>','<?php echo $jobRate->getViewPath(); ?>','<?php echo $securityToken; ?>',1,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                                $('#updateRecordButton4').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $jobRate->getControllerPath(); ?>','<?php echo $jobRate->getViewPath(); ?>','<?php echo $securityToken; ?>',2,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                                $('#updateRecordButton5').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $jobRate->getControllerPath(); ?>','<?php echo $jobRate->getViewPath(); ?>','<?php echo $securityToken; ?>',3,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
            <?php } else { ?>
                                $('#updateRecordButton1')
                                        .removeClass().addClass(' btn btn-info disabled');
                                $('#updateRecordButton2')
            ...
            <?php if ($leafAccess['leafAccessDeleteValue'] == 1) { ?>
                                $('#deleteRecordButton')
                                        .removeClass().addClass('btn btn-danger').attr('onClick', "deleteRecord(<?php echo $leafId; ?>,'<?php echo $jobRate->getControllerPath(); ?>','<?php echo $jobRate->getViewPath(); ?>','<?php echo $securityToken; ?>',<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
            <?php } else { ?>
                                $('#deleteRecordButton')
                                        .removeClass().addClass('btn btn-danger disabled')
                                        .attr('onClick', '');
...
<?php } ?>
<script type="text/javascript" src="./v3/humanResource/job/javascript/jobRate.js"></script>
<hr>
