...
require_once($newFakeDocumentRoot . "v3/humanResource/job/controller/jobController.php");
...
$translator->setCurrentTable('job');
...
if (isset($_POST)) {
    if (isset($_POST['method'])) {
        $job = new \Core\HumanResource\Job\Job\Controller\JobClass();
        define('LIMIT', 10);
        ...
        if (isset($_POST ['query'])) {
            $job->setFieldQuery($_POST ['query']);
        }
        if (isset($_POST ['filter'])) {
            $job->setGridQuery($_POST ['filter']);
        }
        if (isset($_POST ['character'])) {
            $job->setCharacterQuery($_POST['character']);
        }
        if (isset($_POST ['dateRangeStart'])) {
            $job->setDateRangeStartQuery($_POST['dateRangeStart']);
            //explode the data to get day,month,year
            $start = explode('-', $_POST ['dateRangeStart']);
            $job->setStartDay($start[2]);
            $job->setStartMonth($start[1]);
            $job->setStartYear($start[0]);
        }
        if (isset($_POST ['dateRangeEnd']) && (strlen($_POST['dateRangeEnd']) > 0)) {
            $job->setDateRangeEndQuery($_POST['dateRangeEnd']);
            //explode the data to get day,month,year
            $start = explode('-', $_POST ['dateRangeEnd']);
            $job->setEndDay($start[2]);
            $job->setEndMonth($start[1]);
            $job->setEndYear($start[0]);
        }
        if (isset($_POST ['dateRangeType'])) {
            $job->setDateRangeTypeQuery($_POST['dateRangeType']);
        }
        if (isset($_POST ['dateRangeExtraType'])) {
            $job->setDateRangeExtraTypeQuery($_POST['dateRangeExtraType']);
        }
        $job->setServiceOutput('html');
        $job->setLeafId($leafId);
        $job->execute();
        $jobCategoryArray = $job->getJobCategory();
        $countryArray = $job->getCountry();
        if ($_POST['method'] == 'read') {
            $job->setStart($offset);
            $job->setLimit($limit); // normal system don't like paging..
            $job->setPageOutput('html');
            $jobArray = $job->read();
            if (isset($jobArray [0]['firstRecord'])) {
                $firstRecord = $jobArray [0]['firstRecord'];
            }
            if (isset($jobArray [0]['nextRecord'])) {
                $nextRecord = $jobArray [0]['nextRecord'];
            }
            if (isset($jobArray [0]['previousRecord'])) {
                $previousRecord = $jobArray [0]['previousRecord'];
            }
            if (isset($jobArray [0]['lastRecord'])) {
                $lastRecord = $jobArray [0]['lastRecord'];
                $endRecord = $jobArray [0]['lastRecord'];
            }
            $navigation = new \Core\Paging\HtmlPaging();
            $navigation->setLeafId($leafId);
            $navigation->setViewPath($job->getViewPath());
            ...
            if (isset($jobArray [0]['total'])) {
                $total = $jobArray [0]['total'];
            } else {
                $total = 0;
            }
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <form class="form-horizontal">
                                        <input type="hidden" name="jobIdPreview" id="jobIdPreview">

                                        <div class="form-group" id="jobCategoryIdDiv">
                                            <label for="jobCategoryIdPreview"
                                                   class="control-label col-xs-4 col-sm-4 col-md-4"><?php echo $leafTranslation['jobCategoryIdLabel']; ?></label>

                                            <div class="col-xs-8 col-sm-8 col-md-8">
                                                <input class="form-control" type="text" name="jobCategoryIdPreview"
                                                       id="jobCategoryIdPreview">
                                            </div>
                                        </div>
                                        ...
                                        <div class="form-group" id="jobExperinceRequiredDiv">
                                            <label for="jobExperinceRequiredPreview"
                                                   class="control-label col-xs-4 col-sm-4 col-md-4"><?php echo $leafTranslation['jobExperinceRequiredLabel']; ?></label>

                                            <div class="col-xs-8 col-sm-8 col-md-8">
                                                <input class="form-control" type="text" name="jobExperinceRequiredPreview"
                                                       id="jobExperinceRequiredPreview">
                                            </div>
                                        </div>
                                        <div class="form-group" id="jobCodeDiv">
                                            <label for="jobCodePreview"
                                                   class="control-label col-xs-4 col-sm-4 col-md-4"><?php echo $leafTranslation['jobCodeLabel']; ?></label>

                                            <div class="col-xs-8 col-sm-8 col-md-8">
                                                <input class="form-control" type="text" name="jobCodePreview"
                                                       id="jobCodePreview">
                                            </div>
                                        </div>
                                        <div class="form-group" id="jobTitleDiv">
                                            <label for="jobTitlePreview"
                                                   class="control-label col-xs-4 col-sm-4 col-md-4"><?php echo $leafTranslation['jobTitleLabel']; ?></label>

                                            <div class="col-xs-8 col-sm-8 col-md-8">
                                                <input class="form-control" type="text" name="jobTitlePreview"
                                                       id="jobTitlePreview">
                                            </div>
                                        </div>
                                        <div class="form-group" id="jobMinimumSalaryDiv">
                                            <label for="jobMinimumSalaryPreview"
                                                   class="control-label col-xs-4 col-sm-4 col-md-4"><?php echo $leafTranslation['jobMinimumSalaryLabel']; ?></label>

                                            <div class="col-xs-8 col-sm-8 col-md-8">
                                                <input class="form-control" type="text" name="jobMinimumSalaryPreview"
                                                       id="jobMinimumSalaryPreview">
                                            </div>
                                        </div>
                                        <div class="form-group" id="jobMaximumSalaryDiv">
                                            <label for="jobMaximumSalaryPreview"
                                                   class="control-label col-xs-4 col-sm-4 col-md-4"><?php echo $leafTranslation['jobMaximumSalaryLabel']; ?></label>

                                            <div class="col-xs-8 col-sm-8 col-md-8">
                                                <input class="form-control" type="text" name="jobMaximumSalaryPreview"
                                                       id="jobMaximumSalaryPreview">
                                            </div>
                                        </div>
                                        <div class="form-group" id="jobDescriptionDiv">
                                            <label for="jobDescriptionPreview"
                                                   class="control-label col-xs-4 col-sm-4 col-md-4"><?php echo $leafTranslation['jobDescriptionLabel']; ?></label>

                                            <div class="col-xs-8 col-sm-8 col-md-8">
                                                <textarea class="form-control" name="jobDescriptionPreview"
                                                          id="jobDescriptionPreview"></textarea>
                                            </div>
                                        </div>
                                    </form>
                                </div>
                        ...
                        <div class="col-xs-12 col-sm-12 col-md-12">
                            <div class="panel panel-default">
                                <table class="table table table-bordered table-striped table-condensed table-hover smart-form has-tickbox" id="tableData">
                                    ...
                                    <th width="75px"><?php echo ucwords($leafTranslation['jobCodeLabel']); ?></th>
                                    <th width="125px"><?php echo ucwords($leafTranslation['jobTitleLabel']); ?></th>
                                    <th><?php echo ucwords($leafTranslation['jobDescriptionLabel']); ?></th>
                                            ...
                                            if (is_array($jobArray)) {
                                                ...
                                                if ($totalRecord > 0) {
                                                    ...
                                                    for ($i = 0; $i < $totalRecord; $i++) {
                                                        ...
                                                        ?>
                                                        <tr <?php
                                                        if ($jobArray[$i]['isDelete'] == 1) {
                                                            echo "class=\"danger\"";
                                                        } else {
                                                            if ($jobArray[$i]['isDraft'] == 1) {
                                                                echo "class=\"warning\"";
                                                            }
                                                        }
                                                        ?>>
                                                            ...
                                                            <td align="center">
                                                                <div class="btn-group" align="center">
                                                                    ...
                                                                    <button type="button"  class="btn btn-danger btn-sm" title="Delete"
                                                                            ...
                                                                            ?>', '<?php
                                                                            echo rawurlencode(
                                                                                    $jobArray [$i]['jobCategoryDescription']
                                                                            );
                                                                            ?>', '<?php
                                                                            echo rawurlencode(
                                                                                    $jobArray [$i]['countryDescription']
                                                                            );
                                                                            ?>', '<?php
                                                                            echo rawurlencode(
                                                                                    $jobArray [$i]['jobExperienceRequired']
                                                                            );
                                                                            ?>', '<?php
                                                                            echo rawurlencode(
                                                                                    $jobArray [$i]['jobCode']
                                                                            ...
                                                                            ?>', '<?php
                                                                            echo rawurlencode(
                                                                                    $jobArray [$i]['jobMinimumSalary']
                                                                            );
                                                                            ?>', '<?php
                                                                            echo rawurlencode(
                                                                                    $jobArray [$i]['jobMaximumSalary']
                                                                            );
                                                                            ?>', '<?php echo rawurlencode($jobArray [$i]['jobDescription']); ?>');">
                                                            ...
                                                            <td>
                                                                <div class="pull-left">
                                                                    <?php
                                                                    if (isset($jobArray[$i]['jobCode'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos(
                                                                                                strtolower($jobArray[$i]['jobCode']), strtolower($_POST['query'])
                                                                                        ) !== false
                                                                                ) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $jobArray[$i]['jobCode']
                                                                                    );
                                                                                } else {
                                                                                    echo $jobArray[$i]['jobCode'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                    if (strpos(
                                                                                                    strtolower($jobArray[$i]['jobCode']), strtolower($_POST['character'])
                                                                                            ) !== false
                                                                                    ) {
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $jobArray[$i]['jobCode']
                                                                                        );
                                                                                    } else {
                                                                                        echo $jobArray[$i]['jobCode'];
                                                                                    }
                                                            ...
                                                            <td>
                                                                <div class="pull-left">
                                                                    <?php
                                                                    if (isset($jobArray[$i]['jobTitle'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos(
                                                                                                strtolower($jobArray[$i]['jobTitle']), strtolower($_POST['query'])
                                                                                        ) !== false
                                                                                ) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $jobArray[$i]['jobTitle']
                                                                                    );
                                                                                } else {
                                                                                    echo $jobArray[$i]['jobTitle'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                    if (strpos(
                                                                                                    strtolower($jobArray[$i]['jobTitle']), strtolower($_POST['character'])
                                                                                            ) !== false
                                                                                    ) {
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $jobArray[$i]['jobTitle']
                                                                                        );
                                                                                    } else {
                                                                                        echo $jobArray[$i]['jobTitle'];
                                                                                    }
                                                            ...
                                                            <td>
                                                                <div class="pull-left">
                                                                    <?php
                                                                    if (isset($jobArray[$i]['jobDescription'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos(
                                                                                                strtolower($jobArray[$i]['jobDescription']), strtolower($_POST['query'])
                                                                                        ) !== false
                                                                                ) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $jobArray[$i]['jobDescription']
                                                                                    );
                                                                                } else {
                                                                                    echo $jobArray[$i]['jobDescription'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                    if (strpos(
                                                                                                    strtolower($jobArray[$i]['jobDescription']), strtolower($_POST['character'])
                                                                                            ) !== false
                                                                                    ) {
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $jobArray[$i]['jobDescription']
                                                                                        );
                                                                                    } else {
                                                                                        echo $jobArray[$i]['jobDescription'];
                                                                                    }
                                                            ...
                                                            <td>
                                                                <div align="center">
                                                                    <?php
                                                                    if (isset($jobArray[$i]['executeBy'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos($jobArray[$i]['staffName'], $_POST['query']) !== false) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $jobArray[$i]['staffName']
                                                                                    );
                                                                                } else {
                                                                                    echo $jobArray[$i]['staffName'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                    if (strpos($jobArray[$i]['staffName'], $_POST['character']) !== false) {
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $jobArray[$i]['staffName']
                                                                                        );
                                                                                    } else {
                                                                                        echo $jobArray[$i]['staffName'];
                                                                                    }
                                                            ...
                                                            <?php
                                                            if (isset($jobArray[$i]['executeTime'])) {
                                                                $valueArray = $jobArray[$i]['executeTime'];
                                                            ...
                                                            ?>
                                                            <td>
                                                                <input style="display:none;" type="checkbox" name="jobId[]"
                                                                       value="<?php echo $jobArray[$i]['jobId']; ?>">
                                                                <input <?php echo $checked; ?> type="checkbox" name="isDelete[]"
                                                                                               value="<?php echo $jobArray[$i]['isDelete']; ?>">
                                                            </td>
                                                        </tr>
                                                        <?php
                                                    }
                        ...
                        <div class="col-xs-3 col-sm-3 col-md-3 pagination">
                            <div class="pull-right">
                                <button class="delete btn btn-warning" type="button" 
                                        ...
                                        echo $job->getControllerPath();
                                        ?>', '<?php echo $job->getViewPath(); ?>', '<?php echo $securityToken; ?>');">
...
if ((isset($_POST['method']) == 'new' || isset($_POST['method']) == 'read') && $_POST['type'] == 'form') {
            ...
            <div class="row">
                <div class="col-xs-12 col-sm-12 col-md-12">
                    <div class="panel panel-info">
                        ...
                        <div class="panel-body">
                            <input type="hidden" name="jobId" id="jobId" value="<?php
                            ...
                            <div class="row">
                                <div class="col-xs-12 col-sm-12 col-md-12">
                                    <div class="form-group" id="jobCategoryIdForm">
                                        <label for="jobCategoryId" class="control-label col-xs-4 col-sm-4 col-md-4"><strong><?php
                                                echo ucfirst(
                                                        $leafTranslation['jobCategoryIdLabel']
                                                );
                                                ?></strong></label>

                                        <div class="col-xs-8 col-sm-8 col-md-8">
                                            <select name="jobCategoryId" id="jobCategoryId" class="chzn-select form-control">
                                                <option value=""></option>
                                                <?php
                                                if (is_array($jobCategoryArray)) {
                                                    $totalRecord = intval(count($jobCategoryArray));
                                                    if ($totalRecord > 0) {
                                                        $d = 1;
                                                        for ($i = 0; $i < $totalRecord; $i++) {
                                                            if (isset($jobArray[0]['jobCategoryId'])) {
                                                                if ($jobArray[0]['jobCategoryId'] == $jobCategoryArray[$i]['jobCategoryId']) {
                                                                    $selected = "selected";
                                                                } else {
                                                                    $selected = null;
                                                                }
                                                            ...
                                                            ?>
                                                            <option
                                                                value="<?php echo $jobCategoryArray[$i]['jobCategoryId']; ?>" <?php echo $selected; ?>><?php echo $d; ?>
                                                                . <?php echo $jobCategoryArray[$i]['jobCategoryDescription']; ?></option>
                                                            <?php
                                                            $d++;
                                                        }
                                                    } else {
                                                    ...
                                                    <option value=""><?php echo $t['notAvailableTextLabel']; ?></option>
                                                <?php } ?>
                                            </select> <span class="help-block" id="jobCategoryIdHelpMe"></span>
                                        </div>
                                    </div>
                                            ...
                                            <select name="countryId" id="countryId" class="chzn-select form-control">
                                                ...
                                                if (is_array($countryArray)) {
                                                    ...
                                                    if ($totalRecord > 0) {
                                                        $d = 1;
                                                        for ($i = 0; $i < $totalRecord; $i++) {
                                                            if (isset($jobArray[0]['countryId'])) {
                                                                if ($jobArray[0]['countryId'] == $countryArray[$i]['countryId']) {
                                                                    $selected = "selected";
                                                                } else {
                                                                    $selected = null;
                                                                }
                            ...
                            <div class="row">
                                <div class="col-xs-12 col-sm-12 col-md-12">
                                    <div class="col-xs-6 col-sm-6 col-md-6 form-group" id="jobExperinceRequiredForm">
                                        <label for="jobExperinceRequired" class="control-label col-xs-4 col-sm-4 col-md-4"><strong><?php
                                                echo ucfirst(
                                                        $leafTranslation['jobExperinceRequiredLabel']
                                                );
                                                ?></strong></label>

                                        <div class="col-xs-8 col-sm-8 col-md-8">
                                            <input type="text" class="form-control" name="jobExperinceRequired"
                                                   id="jobExperinceRequired" value="<?php
                                                   if (isset($jobArray) && is_array($jobArray)) {
                                                       if (isset($jobArray[0]['jobExperienceRequired'])) {
                                                           echo htmlentities($jobArray[0]['jobExperienceRequired']);
                                                       }
                                                   }
                                                   ?>"> <span class="help-block" id="jobExperinceRequiredHelpMe"></span>
                                        </div>
                                    </div>
                                    <div class="col-xs-6 col-sm-6 col-md-6 form-group" id="jobCodeForm">
                                        <label for="jobCode" class="control-label col-xs-4 col-sm-4 col-md-4"><strong><?php
                                                echo ucfirst(
                                                        $leafTranslation['jobCodeLabel']
                                                );
                                                ?></strong></label>

                                        <div class="col-xs-8 col-sm-8 col-md-8">
                                            <input type="text" class="form-control" name="jobCode" id="jobCode"
                                                   onKeyUp="removeMeError('jobCode');" value="<?php
                                                   if (isset($jobArray) && is_array($jobArray)) {
                                                       if (isset($jobArray[0]['jobCode'])) {
                                                           echo htmlentities($jobArray[0]['jobCode']);
                                                       }
                                                   }
                                                   ?>" maxlength="16"> <span class="help-block" id="jobCodeHelpMe"></span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-xs-12 col-sm-12 col-md-12">
                                    <div class="col-xs-6 col-sm-6 col-md-6 form-group" id="jobTitleForm">
                                        <label for="jobTitle" class="control-label col-xs-4 col-sm-4 col-md-4"><strong><?php
                                                echo ucfirst(
                                                        $leafTranslation['jobTitleLabel']
                                                );
                                                ?></strong></label>

                                        <div class="col-xs-8 col-sm-8 col-md-8">
                                            <input type="text" class="form-control" name="jobTitle" id="jobTitle"
                                                   onKeyUp="removeMeError('jobTitle');" value="<?php
                                                   if (isset($jobArray) && is_array($jobArray)) {
                                                       if (isset($jobArray[0]['jobTitle'])) {
                                                           echo htmlentities($jobArray[0]['jobTitle']);
                                                       }
                                                   }
                                                   ?>"> <span class="help-block" id="jobTitleHelpMe"></span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-xs-12 col-sm-12 col-md-12">
                                    <div class="col-xs-6 col-sm-6 col-md-6 form-group" id="jobMinimumSalaryForm">
                                        <label for="jobMinimumSalary" class="control-label col-xs-4 col-sm-4 col-md-4"
                                               class="control-label col-xs-4 col-sm-4 col-md-4"><strong><?php
                                                       echo ucfirst(
                                                               $leafTranslation['jobMinimumSalaryLabel']
                                                       );
                                                       ?></strong></label>

                                        <div class="col-xs-8 col-sm-8 col-md-8">
                                            <input type="text" class="form-control" name="jobMinimumSalary" id="jobMinimumSalary"
                                                  value="<?php
                                                   if (isset($jobArray) && is_array($jobArray)) {
                                                       if (isset($jobArray[0]['jobMinimumSalary'])) {
                                                           echo htmlentities($jobArray[0]['jobMinimumSalary']);
                                                       }
                                                   }
                                                   ?>"> <span class="help-block" id="jobMinimumSalaryHelpMe"></span>
                                        </div>
                                    </div>
                                    <div class="col-xs-6 col-sm-6 col-md-6 form-group" id="jobMaximumSalaryForm">
                                        <label for="jobMaximumSalary" class="control-label col-xs-4 col-sm-4 col-md-4"><strong><?php
                                                echo ucfirst(
                                                        $leafTranslation['jobMaximumSalaryLabel']
                                                );
                                                ?></strong></label>

                                        <div class="col-xs-8 col-sm-8 col-md-8">
                                            <input type="text" class="form-control" name="jobMaximumSalary" id="jobMaximumSalary"
                                                  value="<?php
                                                   if (isset($jobArray) && is_array($jobArray)) {
                                                       if (isset($jobArray[0]['jobMaximumSalary'])) {
                                                           echo htmlentities($jobArray[0]['jobMaximumSalary']);
                                                       }
                                                   }
                                                   ?>"> <span class="help-block" id="jobMaximumSalaryHelpMe"></span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-xs-12 col-sm-12 col-md-12">
                                    <div class="form-group" id="jobDescriptionForm">
                                        <div class="col-xs-12 col-sm-12 col-md-12">
                                            <textarea class="form-control" rows="5" name="jobDescription" id="jobDescription"
                                                     ><?php
                                                          if (isset($jobArray[0]['jobDescription'])) {
                                                              echo htmlentities($jobArray[0]['jobDescription']);
                                                          }
                                                          ?></textarea> <span class="help-block" id="jobDescriptionHelpMe"></span>
                                        </div>
                                    </div>
                                </div>
                            </div>
    ...
    <?php if ($leafAccess['leafAccessCreateValue'] == 1) { ?>
                        ...
                        if (e.which === 78 && e.which === 18  && e.shiftKey) {
                            ...
                            newRecord(<?php echo $leafId; ?>, '<?php echo $job->getControllerPath(); ?>', '<?php echo $job->getViewPath(); ?>', '<?php echo $securityToken; ?>', 1);
    ...
    <?php if ($leafAccess['leafAccessUpdateValue'] == 1) { ?>
                        ...
                        if (e.which === 83 && e.which === 18  && e.shiftKey) {
                            ...
                            updateRecord(<?php echo $leafId; ?>, '<?php echo $job->getControllerPath(); ?>', '<?php echo $job->getViewPath(); ?>', '<?php echo $securityToken; ?>', 1, <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
    ...
    <?php if ($leafAccess['leafAccessDeleteValue'] == 1) { ?>
                        ...
                        if (e.which === 88 && e.which === 18 && e.shiftKey) {
                            ...
                            deleteRecord(<?php echo $leafId; ?>, '<?php echo $job->getControllerPath(); ?>', '<?php echo $job->getViewPath(); ?>', '<?php echo $securityToken; ?>', <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
    ...
    <?php } ?>
                    switch (e.keyCode) {
                        case 37:
                            previousRecord(<?php echo $leafId; ?>, '<?php echo $job->getControllerPath(); ?>', '<?php echo $job->getViewPath(); ?>', '<?php echo $securityToken; ?>', <?php echo $leafAccess['leafAccessUpdateValue']; ?>, <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
                            
                            return false;
                            break;
                        case 39:
                            nextRecord(<?php echo $leafId; ?>, '<?php echo $job->getControllerPath(); ?>', '<?php echo $job->getViewPath(); ?>', '<?php echo $securityToken; ?>', <?php echo $leafAccess['leafAccessUpdateValue']; ?>, <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
                ...
                $(document).ready(function() {
                    ...
                    validateMeNumeric('jobCategoryId');
                    validateMeNumeric('countryId');
                    validateMeCurrency('jobExperinceRequired');
                    validateMeAlphaNumeric('jobCode');
                    validateMeAlphaNumeric('jobTitle');
                    validateMeCurrency('jobMinimumSalary');
                    validateMeCurrency('jobMaximumSalary');
    <?php if ($_POST['method'] == "new") { ?>
                        $('#resetRecordButton')
                                .removeClass().addClass('btn btn-default');
        <?php if ($leafAccess['leafAccessCreateValue'] == 1) { ?>
                            $('#newRecordButton1')
                                    .removeClass().addClass('btn btn-success').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $job->getControllerPath(); ?>','<?php echo $job->getViewPath(); ?>','<?php echo $securityToken; ?>',1)");
                            $('#newRecordButton2')
                                    .removeClass().addClass('btn dropdown-toggle btn-success');
                            $('#newRecordButton3').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $job->getControllerPath(); ?>','<?php echo $job->getViewPath(); ?>','<?php echo $securityToken; ?>',1)");
                            $('#newRecordButton4').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $job->getControllerPath(); ?>','<?php echo $job->getViewPath(); ?>','<?php echo $securityToken; ?>',2)");
                            $('#newRecordButton5').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $job->getControllerPath(); ?>','<?php echo $job->getViewPath(); ?>','<?php echo $securityToken; ?>',3)");
                            $('#newRecordButton6').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $job->getControllerPath(); ?>','<?php echo $job->getViewPath(); ?>','<?php echo $securityToken; ?>',4)");
                            $('#newRecordButton7').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $job->getControllerPath(); ?>','<?php echo $job->getViewPath(); ?>','<?php echo $securityToken; ?>',5)");
        <?php } else { ?>
                            $('#newRecordButton1')
        ...
        if ($_POST['jobId']) {
            ...
            <?php if ($leafAccess['leafAccessUpdateValue'] == 1) { ?>
                                $('#updateRecordButton1')
                                        .removeClass().addClass('btn btn-info').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $job->getControllerPath(); ?>','<?php echo $job->getViewPath(); ?>','<?php echo $securityToken; ?>',1,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                                $('#updateRecordButton2')
                                        .removeClass().addClass('btn dropdown-toggle btn-info');
                                $('#updateRecordButton3').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $job->getControllerPath(); ?>','<?php echo $job->getViewPath(); ?>','<?php echo $securityToken; ?>',1,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                                $('#updateRecordButton4').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $job->getControllerPath(); ?>','<?php echo $job->getViewPath(); ?>','<?php echo $securityToken; ?>',2,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                                $('#updateRecordButton5').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $job->getControllerPath(); ?>','<?php echo $job->getViewPath(); ?>','<?php echo $securityToken; ?>',3,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
            <?php } else { ?>
                                $('#updateRecordButton1')
                                        .removeClass().addClass(' btn btn-info disabled');
                                $('#updateRecordButton2')
            ...
            <?php } ?>
            <?php if ($leafAccess['leafAccessDeleteValue'] == 1) { ?>
                                $('#deleteRecordButton').removeClass().addClass('btn btn-danger').attr('onClick', "deleteRecord(<?php echo $leafId; ?>,'<?php echo $job->getControllerPath(); ?>','<?php echo $job->getViewPath(); ?>','<?php echo $securityToken; ?>',<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
            <?php } else { ?>
                                $('#deleteRecordButton')
                                        .removeClass().addClass('btn btn-danger disabled')
                                        .attr('onClick', '');
...
<?php } ?>
<script type="text/javascript" src="./v3/humanResource/job/javascript/job.js"></script>
<hr>
