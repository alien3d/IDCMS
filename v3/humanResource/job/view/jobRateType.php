...
require_once($newFakeDocumentRoot . "v3/humanResource/job/controller/jobRateTypeController.php");
...
$translator->setCurrentTable('jobRateType');
...
if (isset($_POST)) {
    if (isset($_POST['method'])) {
        $jobRateType = new \Core\HumanResource\Job\JobRateType\Controller\JobRateTypeClass();
        define('LIMIT', 10);
        ...
        if (isset($_POST ['query'])) {
            $jobRateType->setFieldQuery($_POST ['query']);
        }
        if (isset($_POST ['filter'])) {
            $jobRateType->setGridQuery($_POST ['filter']);
        }
        if (isset($_POST ['character'])) {
            $jobRateType->setCharacterQuery($_POST['character']);
        }
        if (isset($_POST ['dateRangeStart'])) {
            $jobRateType->setDateRangeStartQuery($_POST['dateRangeStart']);
            //explode the data to get day,month,year
            $start = explode('-', $_POST ['dateRangeStart']);
            $jobRateType->setStartDay($start[2]);
            $jobRateType->setStartMonth($start[1]);
            $jobRateType->setStartYear($start[0]);
        }
        if (isset($_POST ['dateRangeEnd']) && (strlen($_POST['dateRangeEnd']) > 0)) {
            $jobRateType->setDateRangeEndQuery($_POST['dateRangeEnd']);
            //explode the data to get day,month,year
            $start = explode('-', $_POST ['dateRangeEnd']);
            $jobRateType->setEndDay($start[2]);
            $jobRateType->setEndMonth($start[1]);
            $jobRateType->setEndYear($start[0]);
        }
        if (isset($_POST ['dateRangeType'])) {
            $jobRateType->setDateRangeTypeQuery($_POST['dateRangeType']);
        }
        if (isset($_POST ['dateRangeExtraType'])) {
            $jobRateType->setDateRangeExtraTypeQuery($_POST['dateRangeExtraType']);
        }
        $jobRateType->setServiceOutput('html');
        $jobRateType->setLeafId($leafId);
        $jobRateType->execute();
        if ($_POST['method'] == 'read') {
            $jobRateType->setStart($offset);
            $jobRateType->setLimit($limit); // normal system don't like paging..
            $jobRateType->setPageOutput('html');
            $jobRateTypeArray = $jobRateType->read();
            if (isset($jobRateTypeArray [0]['firstRecord'])) {
                $firstRecord = $jobRateTypeArray [0]['firstRecord'];
            }
            if (isset($jobRateTypeArray [0]['nextRecord'])) {
                $nextRecord = $jobRateTypeArray [0]['nextRecord'];
            }
            if (isset($jobRateTypeArray [0]['previousRecord'])) {
                $previousRecord = $jobRateTypeArray [0]['previousRecord'];
            }
            if (isset($jobRateTypeArray [0]['lastRecord'])) {
                $lastRecord = $jobRateTypeArray [0]['lastRecord'];
                $endRecord = $jobRateTypeArray [0]['lastRecord'];
            }
            $navigation = new \Core\Paging\HtmlPaging();
            $navigation->setLeafId($leafId);
            $navigation->setViewPath($jobRateType->getViewPath());
            ...
            if (isset($jobRateTypeArray [0]['total'])) {
                $total = $jobRateTypeArray [0]['total'];
            } else {
                $total = 0;
            }
                        ...
                        <div class="btn-group">
                            ...
                            <ul class="dropdown-menu" style="text-align:left">
                                <li>
                                    ...
                                    echo $jobRateType->getControllerPath();
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <form class="form-horizontal">
                                        <input type="hidden" name="jobRateTypeIdPreview" id="jobRateTypeIdPreview">

                                        <div class="form-group" id="jobRateTypeCodeDiv">
                                            <label for="jobRateTypeCodePreview"
                                                   class="control-label col-xs-4 col-sm-4 col-md-4"><?php echo $leafTranslation['jobRateTypeCodeLabel']; ?></label>

                                            <div class="col-xs-8 col-sm-8 col-md-8">
                                                <input class="form-control" type="text" name="jobRateTypeCodePreview"
                                                       id="jobRateTypeCodePreview">
                                            </div>
                                        </div>
                                        <div class="form-group" id="jobRateMultiplierDiv">
                                            <label for="jobRateMultiplierPreview"
                                                   class="control-label col-xs-4 col-sm-4 col-md-4"><?php echo $leafTranslation['jobRateMultiplierLabel']; ?></label>

                                            <div class="col-xs-8 col-sm-8 col-md-8">
                                                <input class="form-control" type="text" name="jobRateMultiplierPreview"
                                                       id="jobRateMultiplierPreview">
                                            </div>
                                        </div>
                                        <div class="form-group" id="jobRateTypeDescriptionDiv">
                                            <label for="jobRateTypeDescriptionPreview"
                                                   class="control-label col-xs-4 col-sm-4 col-md-4"><?php echo $leafTranslation['jobRateTypeDescriptionLabel']; ?></label>

                                            <div class="col-xs-8 col-sm-8 col-md-8">
                                                <input class="form-control" type="text"
                                                       name="jobRateTypeDescriptionPreview" id="jobRateTypeDescriptionPreview">
                                            </div>
                                        </div>
                                    </form>
                                </div>
                        ...
                        <div class="col-xs-12 col-sm-12 col-md-12">
                            <div class="panel panel-default">
                                <table class="table table table-bordered table-striped table-condensed table-hover smart-form has-tickbox" id="tableData">
                                    ...
                                    <th width="75px"><?php echo ucwords($leafTranslation['jobRateTypeCodeLabel']); ?></th>
                                    <th width="125px"><?php echo ucwords($leafTranslation['jobRateMultiplierLabel']); ?></th>
                                    <th><?php echo ucwords($leafTranslation['jobRateTypeDescriptionLabel']); ?></th>
                                            ...
                                            if (is_array($jobRateTypeArray)) {
                                                ...
                                                if ($totalRecord > 0) {
                                                    ...
                                                    for ($i = 0; $i < $totalRecord; $i++) {
                                                        ...
                                                        ?>
                                                        <tr <?php
                                                        if ($jobRateTypeArray[$i]['isDelete'] == 1) {
                                                            echo "class=\"danger\"";
                                                        } else {
                                                            if ($jobRateTypeArray[$i]['isDraft'] == 1) {
                                                                echo "class=\"warning\"";
                                                            }
                                                        }
                                                        ?>>
                                                            ...
                                                            <td align="center">
                                                                <div class="btn-group" align="center">
                                                                    <button type="button"  class="btn btn-info btn-sm" title="Edit"
                                                                            ...
                                                                            echo intval(
                                                                                    $jobRateTypeArray [$i]['jobRateTypeId']
                                                                            );
                                                                    ...
                                                                    <button type="button"  class="btn btn-danger btn-sm" title="Delete"
                                                                            ...
                                                                            ?>', '<?php
                                                                            echo rawurlencode(
                                                                                    $jobRateTypeArray [$i]['jobRateTypeCode']
                                                                            );
                                                                            ?>', '<?php
                                                                            echo rawurlencode(
                                                                                    $jobRateTypeArray [$i]['jobRateMultiplier']
                                                                            );
                                                                            ?>', '<?php
                                                                            echo rawurlencode(
                                                                                    $jobRateTypeArray [$i]['jobRateTypeDescription']
                                                                            );
                                                                            ?>');">
                                                            ...
                                                            <td>
                                                                <div class="pull-left">
                                                                    <?php
                                                                    if (isset($jobRateTypeArray[$i]['jobRateTypeCode'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos(
                                                                                                strtolower($jobRateTypeArray[$i]['jobRateTypeCode']), strtolower($_POST['query'])
                                                                                        ) !== false
                                                                                ) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $jobRateTypeArray[$i]['jobRateTypeCode']
                                                                                    );
                                                                                } else {
                                                                                    echo $jobRateTypeArray[$i]['jobRateTypeCode'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                    if (strpos(
                                                                                                    strtolower($jobRateTypeArray[$i]['jobRateTypeCode']), strtolower($_POST['character'])
                                                                                            ) !== false
                                                                                    ) {
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $jobRateTypeArray[$i]['jobRateTypeCode']
                                                                                        );
                                                                                    } else {
                                                                                        echo $jobRateTypeArray[$i]['jobRateTypeCode'];
                                                                                    }
                                                            ...
                                                            <?php
                                                            $d = $jobRateTypeArray[$i]['jobRateMultiplier'];
                                                            if (class_exists('NumberFormatter')) {
                                                                if (is_array($systemFormat) && $systemFormat['languageCode'] != '') {
                                                                    ...
                                                                    $d = $a->format($jobRateTypeArray[$i]['jobRateMultiplier']);
                                                                } else {
                                                            ...
                                                            <td>
                                                                <div class="pull-left">
                                                                    <?php
                                                                    if (isset($jobRateTypeArray[$i]['jobRateTypeDescription'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos(
                                                                                                strtolower($jobRateTypeArray[$i]['jobRateTypeDescription']), strtolower($_POST['query'])
                                                                                        ) !== false
                                                                                ) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $jobRateTypeArray[$i]['jobRateTypeDescription']
                                                                                    );
                                                                                } else {
                                                                                    echo $jobRateTypeArray[$i]['jobRateTypeDescription'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                    if (strpos(
                                                                                                    strtolower($jobRateTypeArray[$i]['jobRateTypeDescription']), strtolower($_POST['character'])
                                                                                            ) !== false
                                                                                    ) {
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $jobRateTypeArray[$i]['jobRateTypeDescription']
                                                                                        );
                                                                                    } else {
                                                                                        echo $jobRateTypeArray[$i]['jobRateTypeDescription'];
                                                                                    }
                                                            ...
                                                            <td>
                                                                <div class="pull-left">
                                                                    <?php
                                                                    if (isset($jobRateTypeArray[$i]['executeBy'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos($jobRateTypeArray[$i]['staffName'], $_POST['query']) !== false) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $jobRateTypeArray[$i]['staffName']
                                                                                    );
                                                                                } else {
                                                                                    echo $jobRateTypeArray[$i]['staffName'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                    if (strpos(
                                                                                                    $jobRateTypeArray[$i]['staffName'], $_POST['character']
                                                                                            ) !== false
                                                                                    ) {
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $jobRateTypeArray[$i]['staffName']
                                                                                        );
                                                                                    } else {
                                                                                        echo $jobRateTypeArray[$i]['staffName'];
                                                                                    }
                                                            ...
                                                            <?php
                                                            if (isset($jobRateTypeArray[$i]['executeTime'])) {
                                                                $valueArray = $jobRateTypeArray[$i]['executeTime'];
                                                            ...
                                                            <?php } ?>
                                                            <?php
                                                            if ($jobRateTypeArray[$i]['isDelete']) {
                                                                $checked = "checked";
                                                            } else {
                                                                $checked = null;
                                                            }
                                                            ?>
                                                            <td>
                                                                <input style="display:none;" type="checkbox" name="jobRateTypeId[]"
                                                                       value="<?php echo $jobRateTypeArray[$i]['jobRateTypeId']; ?>">
                                                                <input <?php echo $checked; ?> type="checkbox" name="isDelete[]"
                                                                                               value="<?php echo $jobRateTypeArray[$i]['isDelete']; ?>">
                                                            </td>
                                                        </tr>
                                                        <?php
                                                    }
                        ...
                        <div class="col-xs-3 col-sm-3 col-md-3 pagination">
                            <div class="pull-right">
                                <button class="delete btn btn-warning" type="button" 
                                        ...
                                        echo $jobRateType->getControllerPath();
                                        ?>', '<?php echo $jobRateType->getViewPath(); ?>', '<?php echo $securityToken; ?>');">
        ...
        <div id="content" style="opacity: 1;">
            <div class="row">
                <div class="col-xs-12 col-sm-12 col-md-12">
                    <div class="panel panel-info">
                        ...
                        <div class="panel-body">
                            <input type="hidden" name="jobRateTypeId" id="jobRateTypeId" value="<?php
                            if (isset($_POST['jobRateTypeId'])) {
                                echo $_POST['jobRateTypeId'];
                            }
                            ?>">

                            <div class="row">
                                <div class="col-xs-12 col-sm-12 col-md-12">
                                    <div class="col-xs-6 col-sm-6 col-md-6 form-group" id="jobRateTypeCodeForm">
                                        <label for="jobRateTypeCode" class="control-label col-xs-4 col-sm-4 col-md-4"><strong><?php
                                                echo ucfirst(
                                                        $leafTranslation['jobRateTypeCodeLabel']
                                                );
                                                ?></strong></label>

                                        <div class="col-xs-8 col-sm-8 col-md-8">
                                            <input type="text" class="form-control" name="jobRateTypeCode"
                                                   id="jobRateTypeCode" onKeyUp="removeMeError('jobRateTypeCode');" value="<?php
                                                   if (isset($jobRateTypeArray) && is_array($jobRateTypeArray)) {
                                                       if (isset($jobRateTypeArray[0]['jobRateTypeCode'])) {
                                                           echo htmlentities($jobRateTypeArray[0]['jobRateTypeCode']);
                                                       }
                                                   }
                                                   ?>" maxlength="16"> <span class="help-block" id="jobRateTypeCodeHelpMe"></span>
                                        </div>
                                    </div>
                                    <div class="col-xs-6 col-sm-6 col-md-6 form-group" id="jobRateMultiplierForm">
                                        <label for="jobRateMultiplier" class="control-label col-xs-4 col-sm-4 col-md-4"><strong><?php
                                                echo ucfirst(
                                                        $leafTranslation['jobRateMultiplierLabel']
                                                );
                                                ?></strong></label>

                                        <div class="col-xs-8 col-sm-8 col-md-8">
                                            <input type="text" class="form-control" name="jobRateMultiplier"
                                                   id="jobRateMultiplier" onKeyUp="removeMeError('jobRateMultiplier');" value="<?php
                                                   if (isset($jobRateTypeArray) && is_array($jobRateTypeArray)) {
                                                       if (isset($jobRateTypeArray[0]['jobRateMultiplier'])) {
                                                           echo htmlentities($jobRateTypeArray[0]['jobRateMultiplier']);
                                                       }
                                                   }
                                                   ?>"> <span class="help-block" id="jobRateMultiplierHelpMe"></span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-xs-12 col-sm-12 col-md-12">
                                    <div class="form-group" id="jobRateTypeDescriptionForm">
                                        <div class="col-xs-12 col-sm-12 col-md-12">
                                            <textarea name="jobRateTypeDescription" id="jobRateTypeDescription"
                                                      class="form-control" rows="5"><?php
                                                          if (isset($jobRateTypeArray[0]['jobRateTypeDescription'])) {
                                                              echo htmlentities($jobRateTypeArray[0]['jobRateTypeDescription']);
                                                          }
                                                          ?></textarea> <span class="help-block" id="jobRateTypeDescriptionHelpMe"></span>
                                        </div>
                                    </div>
                                </div>
                            </div>
            ...
            <script type="text/javascript">
                $(document).ready(function() {
                    ...
                    validateMeAlphaNumeric('jobRateTypeCode');
                    validateMeCurrency('jobRateMultiplier');
    <?php if ($_POST['method'] == "new") { ?>
                        $('#resetRecordButton')
                                .removeClass().addClass('btn btn-default');
        <?php if ($leafAccess['leafAccessCreateValue'] == 1) { ?>
                            $('#newRecordButton1')
                                    .removeClass().addClass('btn btn-success').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $jobRateType->getControllerPath(); ?>','<?php echo $jobRateType->getViewPath(); ?>','<?php echo $securityToken; ?>',1)");
                            $('#newRecordButton2')
                                    .removeClass().addClass('btn dropdown-toggle btn-success');
                            $('#newRecordButton3').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $jobRateType->getControllerPath(); ?>','<?php echo $jobRateType->getViewPath(); ?>','<?php echo $securityToken; ?>',1)");
                            $('#newRecordButton4').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $jobRateType->getControllerPath(); ?>','<?php echo $jobRateType->getViewPath(); ?>','<?php echo $securityToken; ?>',2)");
                            $('#newRecordButton5').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $jobRateType->getControllerPath(); ?>','<?php echo $jobRateType->getViewPath(); ?>','<?php echo $securityToken; ?>',3)");
                            $('#newRecordButton6').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $jobRateType->getControllerPath(); ?>','<?php echo $jobRateType->getViewPath(); ?>','<?php echo $securityToken; ?>',4)");
                            $('#newRecordButton7').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $jobRateType->getControllerPath(); ?>','<?php echo $jobRateType->getViewPath(); ?>','<?php echo $securityToken; ?>',5)");
        <?php } else { ?>
                            $('#newRecordButton1')
        ...
        if ($_POST['jobRateTypeId']) {
            ...
            <?php if ($leafAccess['leafAccessUpdateValue'] == 1) { ?>
                                $('#updateRecordButton1')
                                        .removeClass().addClass('btn btn-info').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $jobRateType->getControllerPath(); ?>','<?php echo $jobRateType->getViewPath(); ?>','<?php echo $securityToken; ?>',1,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                                $('#updateRecordButton2')
                                        .removeClass().addClass('btn dropdown-toggle btn-info');
                                $('#updateRecordButton3').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $jobRateType->getControllerPath(); ?>','<?php echo $jobRateType->getViewPath(); ?>','<?php echo $securityToken; ?>',1,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                                $('#updateRecordButton4').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $jobRateType->getControllerPath(); ?>','<?php echo $jobRateType->getViewPath(); ?>','<?php echo $securityToken; ?>',2,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                                $('#updateRecordButton5').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $jobRateType->getControllerPath(); ?>','<?php echo $jobRateType->getViewPath(); ?>','<?php echo $securityToken; ?>',3,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
            <?php } else { ?>
                                $('#updateRecordButton1')
                                        .removeClass().addClass(' btn btn-info disabled');
                                $('#updateRecordButton2')
            ...
            <?php if ($leafAccess['leafAccessDeleteValue'] == 1) { ?>
                                $('#deleteRecordButton')
                                        .removeClass().addClass('btn btn-danger').attr('onClick', "deleteRecord(<?php echo $leafId; ?>,'<?php echo $jobRateType->getControllerPath(); ?>','<?php echo $jobRateType->getViewPath(); ?>','<?php echo $securityToken; ?>',<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
            <?php } else { ?>
                                $('#deleteRecordButton')
                                        .removeClass().addClass('btn btn-danger disabled')
                                        .attr('onClick', '');
...
<?php } ?>
<script type="text/javascript" src="./v3/humanResource/job/javascript/jobRateType.js"></script>
<hr>
