function checkDuplicate(leafId, page, securityToken) {
    var $loanCategoryCode = $("#loanCategoryCode");
    if ($loanCategoryCode.val().length === 0) {
    ...
    $.ajax({
        ...
        url: page,
        data: {
            loanCategoryCode: $loanCategoryCode.val(),
            method: 'duplicate',
        ...
        success: function(data) {
            ...
            if (success === true) {
                if (total !== 0) {
                    $("#loanCategoryCode").val('').focus();
                    $("#loanCategoryCodeForm").removeClass().addClass("form-group has-error");
                ...
                $("#loanCategoryForm").removeClass().addClass("form-group has-error");
            }
            if ($infoPanel.is(':hidden')) {
                $infoPanel.show();
            }
...
function showFormUpdate(leafId, url, urlList, securityToken, loanCategoryId, updateAccess, deleteAccess) {
    sleep(500);
...
function showModalDelete(loanCategoryId, loanCategoryCode, loanCategoryDescription) {
    $('#loanCategoryIdPreview').val('').val(decodeURIComponent(loanCategoryId));
    $('#loanCategoryCodePreview').val('').val(decodeURIComponent(loanCategoryCode));
    $('#loanCategoryDescriptionPreview').val('').val(decodeURIComponent(loanCategoryDescription));
...
function deleteGridRecord(leafId, url, urlList, securityToken) {
    $.ajax({
        ...
        data: {
            method: 'delete',
            output: 'json',
            loanCategoryId: $('#loanCategoryIdPreview').val(),
            securityToken: securityToken,
            leafId: leafId
        },
        beforeSend: function() {
...
function deleteGridRecordCheckbox(leafId, url, urlList, securityToken) {
    var stringText = '';
    var counter = 0;
    $('input:checkbox[name="loanCategoryId[]"]').each(function() {
        stringText = stringText + "&loanCategoryId[]=" + $(this).val();
    });
...
function newRecord(leafId, url, urlList, securityToken, type, createAccess, updateAccess, deleteAccess) {
    ...
    var $loanCategoryCode = $('#loanCategoryCode');
    var $loanCategoryDescription = $('#loanCategoryDescription');
    if (css.search('disabled') > 0) {
        return false;
    } else {
        if (type === 1) {
            if ($loanCategoryCode.val().length === 0) {
                $('#loanCategoryCodeHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['loanCategoryCodeLabel'] + " </span>");
                $('#loanCategoryCodeForm').removeClass().addClass('form-group has-error');
                $loanCategoryCode.focus();
                return false;
            }
            if ($loanCategoryDescription.val().length === 0) {
                $('#loanCategoryDescriptionHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['loanCategoryDescriptionLabel'] + " </span>");
                $('#loanCategoryDescriptionForm').removeClass().addClass('form-group has-error');
            ...
            $.ajax({
                ...
                data: {
                    ...
                    output: 'json',
                    loanCategoryCode: $loanCategoryCode.val(),
                    loanCategoryDescription: $loanCategoryDescription.val(),
                    securityToken: securityToken,
                    leafId: leafId
                },
                beforeSend: function() {
                ...
                success: function(data) {
                    ...
                    if (success === true) {
                        ...
                        $loanCategoryCode.val('');
                        $('#loanCategoryCodeHelpMe').html('').empty();
                        $loanCategoryDescription.val('');
                        $('#loanCategoryDescriptionForm').removeClass().addClass('form-group');
                        $('#loanCategoryDescriptionHelpMe').html('').empty();
                    } else if (success === false) {
                        $infoPanel.html('').empty().html("<span class='label label-important'>&nbsp;" + message + "</span>");
                        if ($infoPanel.is(':hidden')) {
                            $infoPanel.show();
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (success === true) {
                        ...
                        $('#loanCategoryId').val(data.loanCategoryId);
                        $('#newRecordButton1').removeClass().addClass('btn btn-success disabled').attr('onClick', '');
                        $('#newRecordButton2').removeClass().addClass('btn dropdown-toggle btn-success disabled');
                        $('#newRecordButton3').attr('onClick', '');
                        $('#newRecordButton4').attr('onClick', '');
...
function resetRecord(leafId, url, urlList, securityToken, createAccess, updateAccess, deleteAccess) {
    ...
    $("#loanCategoryCode").val('');
    $("#loanCategoryCodeHelpMe").empty().html('');
    $("#loanCategoryDescription").val('');
    $("#loanCategoryDescriptionHelpMe").empty().html('');
    $('#loanCategoryDescription').empty().val('');
}
function postRecord() {
                ...
                if (success === true) {
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if (success === true) {
                                $('#loanCategoryId').val(data.data.loanCategoryId);
                                $('#loanCategoryCode').val(data.data.loanCategoryCode);
                                $('#loanCategoryDescription').val(data.data.loanCategoryDescription);
                                if (nextRecord > 0) {
                                    $('#previousRecordButton').removeClass().addClass('btn btn-default disabled').attr('onClick', '');
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($previousRecordCounter.val()) > 0 && parseFloat($previousRecordCounter.val()) < parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    loanCategoryId: $previousRecordCounter.val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($nextRecordCounter.val()) <= parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    loanCategoryId: $nextRecordCounter.val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
