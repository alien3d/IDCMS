...
function checkDuplicate(leafId, page, securityToken) {
    var $loanTypeCode = $("#loanTypeCode");
    if ($loanTypeCode.val().length === 0) {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if (success === true) {
                if (total !== 0) {
                    $("#loanTypeCode").val('').focus();
                    $("#loanTypeCodeForm").removeClass().addClass("form-group has-error");
                ...
                $("#loanTypeForm").removeClass().addClass("form-group has-error");
            }
            if ($infoPanel.is(':hidden')) {
                $infoPanel.show();
            }
...
function showFormUpdate(leafId, url, urlList, securityToken, loanTypeId, updateAccess, deleteAccess) {
    sleep(500);
...
function showModalDelete(loanTypeId, loanCategoryId, loanTypeDescription) {
    $('#loanTypeIdPreview').val('').val(decodeURIComponent(loanTypeId));
    $('#loanCategoryIdPreview').val('').val(decodeURIComponent(loanCategoryId));
    $('#loanTypeDescriptionPreview').val('').val(decodeURIComponent(loanTypeDescription));
...
function deleteGridRecord(leafId, url, urlList, securityToken) {
    $.ajax({
        ...
        data: {
            method: 'delete',
            output: 'json',
            loanTypeId: $('#loanTypeIdPreview').val(),
            securityToken: securityToken,
            leafId: leafId
        },
        beforeSend: function() {
...
function deleteGridRecordCheckbox(leafId, url, urlList, securityToken) {
    var stringText = '';
    var counter = 0;
    $('input:checkbox[name="loanTypeId[]"]').each(function() {
        stringText = stringText + "&loanTypeId[]=" + $(this).val();
    });
...
function newRecord(leafId, url, urlList, securityToken, type, createAccess, updateAccess, deleteAccess) {
    ...
    var $loanTypeDescription = $('#loanTypeDescription');
    if (css.search('disabled') > 0) {
        return false;
    } else {
        if (type === 1) {
            ...
            if ($loanTypeDescription.val().length === 0) {
                $('#loanTypeDescriptionHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['loanTypeDescriptionLabel'] + " </span>");
                $('#loanTypeDescriptionForm').removeClass().addClass('form-group has-error');
                $loanTypeDescription.focus();
                return false;
            }
            $.ajax({
                type: 'POST',
                ...
                data: {
                    ...
                    loanTypeDescription: $loanTypeDescription.val(),
                    securityToken: securityToken,
                    leafId: leafId
                },
                beforeSend: function() {
                ...
                success: function(data) {
                    ...
                    if (success === true) {
                        ...
                        $loanTypeDescription.val('');
                        $('#loanTypeDescriptionForm').removeClass().addClass('form-group');
                        $('#loanTypeDescriptionHelpMe').html('').empty();
                    } else if (success === false) {
                        $infoPanel.html('').empty().html("<span class='label label-important'>&nbsp;" + message + "</span>");
                        if ($infoPanel.is(':hidden')) {
                            $infoPanel.show();
...
function resetRecord(leafId, url, urlList, securityToken, createAccess, updateAccess, deleteAccess) {
    ...
    $('#loanCategoryId').trigger("chosen:updated");
    $("#loanTypeDescription").val('');
    $("#loanTypeDescriptionHelpMe").empty().html('');
    $('#loanTypeDescription').empty().val('');
}
function postRecord() {
                ...
                if (success === true) {
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if (success === true) {
                                $('#loanTypeId').val(data.data.loanTypeId);
                                $('#loanCategoryId').val(data.data.loanCategoryId).trigger("chosen:updated");
                                $('#loanTypeDescription').val(data.data.loanTypeDescription);
                                if (nextRecord > 0) {
                                    $('#previousRecordButton').removeClass().addClass('btn btn-default disabled').attr('onClick', '');
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($previousRecordCounter.val()) > 0 && parseFloat($previousRecordCounter.val()) < parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    loanTypeId: $previousRecordCounter.val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($nextRecordCounter.val()) <= parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    loanTypeId: $nextRecordCounter.val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
