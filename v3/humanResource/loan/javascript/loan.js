function getLoanCategory(leafId, url, securityToken) {
    $.ajax({
        type: 'GET',
        url: url,
        data: {
        ...
        success: function(data) {
            ...
            if (success === false) {
                ...
                $("#loanCategoryId").html('').empty().html(data.data).trigger("chosen:updated");
...
function getLoanType(leafId, url, securityToken) {
    $.ajax({
        type: 'GET',
        url: url,
        data: {
        ...
        success: function(data) {
            ...
            if (success === false) {
                ...
                $("#loanTypeId").html('').empty().html(data.data).trigger("chosen:updated");
                $infoPanel.html('').empty().html("<span class='label label-success'>&nbsp;<img src='" + smileyLol + "'>  " + decodeURIComponent(t['loadingCompleteTextLabel']) + "</span>").delay(5000).fadeOut();
            }
        },
        error: function(xhr) {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if (success === true) {
                if (total !== 0) {
                    $("#loanCode").val('').focus();
                    $("#loanCodeForm").removeClass().addClass("form-group has-error");
                ...
                $("#loanForm").removeClass().addClass("form-group has-error");
            }
            if ($infoPanel.is(':hidden')) {
                $infoPanel.show();
            }
...
function showFormUpdate(leafId, url, urlList, securityToken, loanId, updateAccess, deleteAccess) {
    sleep(500);
...
function showModalDelete(loanId, loanCategoryId, loanTypeId, employeeId, loanAmount, loanStartDate, loanEndDate, loanMonthlyPayment, loanFinalPayment, loanDescription) {
    $('#loanIdPreview').val('').val(decodeURIComponent(loanId));
    $('#loanCategoryIdPreview').val('').val(decodeURIComponent(loanCategoryId));
    $('#loanTypeIdPreview').val('').val(decodeURIComponent(loanTypeId));
    $('#employeeIdPreview').val('').val(decodeURIComponent(employeeId));
    $('#loanAmountPreview').val('').val(decodeURIComponent(loanAmount));
    $('#loanStartDatePreview').val('').val(decodeURIComponent(loanStartDate));
    $('#loanEndDatePreview').val('').val(decodeURIComponent(loanEndDate));
    $('#loanMonthlyPaymentPreview').val('').val(decodeURIComponent(loanMonthlyPayment));
    $('#loanFinalPaymentPreview').val('').val(decodeURIComponent(loanFinalPayment));
    $('#loanDescriptionPreview').val('').val(decodeURIComponent(loanDescription));
...
function deleteGridRecordCheckbox(leafId, url, urlList, securityToken) {
    var stringText = '';
    var counter = 0;
    $('input:checkbox[name="loanId[]"]').each(function() {
        stringText = stringText + "&loanId[]=" + $(this).val();
    });
...
function newRecord(leafId, url, urlList, securityToken, type, createAccess, updateAccess, deleteAccess) {
    ...
    var $loanCategoryId = $('#loanCategoryId');
    var $loanTypeId = $('#loanTypeId');
    var $employeeId = $('#employeeId');
    var $loanAmount = $('#loanAmount');
    var $loanStartDate = $('#loanStartDate');
    var $loanEndDate = $('#loanEndDate');
    var $loanMonthlyPayment = $('#loanMonthlyPayment');
    var $loanFinalPayment = $('#loanFinalPayment');
    var $loanDescription = $('#loanDescription');
    if (css.search('disabled') > 0) {
        return false;
    } else {
        if (type === 1) {
            if ($loanCategoryId.val().length === 0) {
                $('#loanCategoryIdHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['loanCategoryIdLabel'] + " </span>");
                $loanCategoryId.data('chosen').activate_action();
                return false;
            }
            if ($loanTypeId.val().length === 0) {
                $('#loanTypeIdHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['loanTypeIdLabel'] + " </span>");
                $loanTypeId.data('chosen').activate_action();
                return false;
            }
            ...
            if ($loanAmount.val().length === 0) {
                $('#loanAmountHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['loanAmountLabel'] + " </span>");
                $('#loanAmountForm').removeClass().addClass('form-group has-error');
                $loanAmount.focus();
                return false;
            }
            if ($loanStartDate.val().length === 0) {
                $('#loanStartDateHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['loanStartDateLabel'] + " </span>");
                $('#loanStartDateForm').removeClass().addClass('form-group has-error');
                $loanStartDate.focus();
                return false;
            }
            if ($loanEndDate.val().length === 0) {
                $('#loanEndDateHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['loanEndDateLabel'] + " </span>");
                $('#loanEndDateForm').removeClass().addClass('form-group has-error');
                $loanEndDate.focus();
                return false;
            }
            if ($loanMonthlyPayment.val().length === 0) {
                $('#loanMonthlyPaymentHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['loanMonthlyPaymentLabel'] + " </span>");
                $('#loanMonthlyPaymentForm').removeClass().addClass('form-group has-error');
                $loanMonthlyPayment.focus();
                return false;
            }
            if ($loanFinalPayment.val().length === 0) {
                $('#loanFinalPaymentHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['loanFinalPaymentLabel'] + " </span>");
                $('#loanFinalPaymentForm').removeClass().addClass('form-group has-error');
                $loanFinalPayment.focus();
                return false;
            }
            if ($loanDescription.val().length === 0) {
                $('#loanDescriptionHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['loanDescriptionLabel'] + " </span>");
                $('#loanDescriptionForm').removeClass().addClass('form-group has-error');
            ...
            $.ajax({
                ...
                data: {
                    method: 'create',
                    output: 'json',
                    loanCategoryId: $loanCategoryId.val(),
                    loanTypeId: $loanTypeId.val(),
                    employeeId: $employeeId.val(),
                    loanAmount: $loanAmount.val(),
                    loanStartDate: $loanStartDate.val(),
                    loanEndDate: $loanEndDate.val(),
                    loanMonthlyPayment: $loanMonthlyPayment.val(),
                    loanFinalPayment: $loanFinalPayment.val(),
                    loanDescription: $loanDescription.val(),
                    securityToken: securityToken,
                    leafId: leafId
                },
                beforeSend: function() {
                ...
                success: function(data) {
                    ...
                    if (success === true) {
                        ...
                        $loanCategoryId.val('');
                        $loanCategoryId.trigger("chosen:updated");
                        $('#loanCategoryIdHelpMe').html('').empty();
                        $loanTypeId.val('');
                        $loanTypeId.trigger("chosen:updated");
                        $('#loanTypeIdHelpMe').html('').empty();
                        $employeeId.val('');
                        $employeeId.trigger("chosen:updated");
                        $('#employeeIdHelpMe').html('').empty();
                        $loanAmount.val('');
                        $('#loanAmountHelpMe').html('').empty();
                        $loanStartDate.val('');
                        $('#loanStartDateHelpMe').html('').empty();
                        $loanEndDate.val('');
                        $('#loanEndDateHelpMe').html('').empty();
                        $loanMonthlyPayment.val('');
                        $('#loanMonthlyPaymentHelpMe').html('').empty();
                        $loanFinalPayment.val('');
                        $('#loanFinalPaymentHelpMe').html('').empty();
                        $loanDescription.val('');
                        $('#loanDescriptionForm').removeClass().addClass('form-group');
                        $('#loanDescriptionHelpMe').html('').empty();
                    } else if (success === false) {
                        $infoPanel.html('').empty().html("<span class='label label-important'>&nbsp;" + message + "</span>");
                        if ($infoPanel.is(':hidden')) {
                            $infoPanel.show();
...
function resetRecord(leafId, url, urlList, securityToken, createAccess, updateAccess, deleteAccess) {
    ...
    $("#loanId").val('');
    $("#loanIdHelpMe").empty().html('');
    
    $("#loanCategoryId").val('');
    $("#loanCategoryIdHelpMe").empty().html('');
    $('#loanCategoryId').trigger("chosen:updated");
    $("#loanTypeId").val('');
    $("#loanTypeIdHelpMe").empty().html('');
    $('#loanTypeId').trigger("chosen:updated");
    $("#employeeId").val('');
    $("#employeeIdHelpMe").empty().html('');
    $('#employeeId').trigger("chosen:updated");
    $("#loanAmount").val('');
    $("#loanAmountHelpMe").empty().html('');
    $("#loanStartDate").val('');
    $("#loanStartDateHelpMe").empty().html('');
    $("#loanEndDate").val('');
    $("#loanEndDateHelpMe").empty().html('');
    $("#loanMonthlyPayment").val('');
    $("#loanMonthlyPaymentHelpMe").empty().html('');
    $("#loanFinalPayment").val('');
    $("#loanFinalPaymentHelpMe").empty().html('');
    $("#loanDescription").val('');
    $("#loanDescriptionHelpMe").empty().html('');
    $('#loanDescription').empty().val('');
}
function postRecord() {
                ...
                if (success === true) {
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if (success === true) {
                                $('#loanId').val(data.data.loanId);
                                $('#loanCategoryId').val(data.data.loanCategoryId).trigger("chosen:updated");
                                $('#loanTypeId').val(data.data.loanTypeId).trigger("chosen:updated");
                                $('#employeeId').val(data.data.employeeId).trigger("chosen:updated");
                                $('#loanAmount').val(data.data.loanAmount);
                                x = data.data.loanStartDate;
                                x = x.split("-");
                                output = x[2] + "-" + x[1] + "-" + x[0];
                                output = output.toString();
                                $('#loanStartDate').val(output);
                                x = data.data.loanEndDate;
                                x = x.split("-");
                                output = x[2] + "-" + x[1] + "-" + x[0];
                                output = output.toString();
                                $('#loanEndDate').val(output);
                                $('#loanMonthlyPayment').val(data.data.loanMonthlyPayment);
                                $('#loanFinalPayment').val(data.data.loanFinalPayment);
                                $('#loanDescription').val(data.data.loanDescription);
                                if (nextRecord > 0) {
                                    $('#previousRecordButton').removeClass().addClass('btn btn-default disabled').attr('onClick', '');
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($previousRecordCounter.val()) > 0 && parseFloat($previousRecordCounter.val()) < parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    loanId: $previousRecordCounter.val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
