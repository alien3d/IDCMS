...
require_once($newFakeDocumentRoot . "v3/humanResource/loan/controller/loanTypeController.php");
...
$translator->setCurrentTable('loanType');
...
if (isset($_POST)) {
    if (isset($_POST['method'])) {
        $loanType = new \Core\HumanResource\Loan\LoanType\Controller\LoanTypeClass();
        define('LIMIT', 10);
        ...
        if (isset($_POST ['query'])) {
            $loanType->setFieldQuery($_POST ['query']);
        }
        if (isset($_POST ['filter'])) {
            $loanType->setGridQuery($_POST ['filter']);
        }
        if (isset($_POST ['character'])) {
            $loanType->setCharacterQuery($_POST['character']);
        }
        if (isset($_POST ['dateRangeStart'])) {
            $loanType->setDateRangeStartQuery($_POST['dateRangeStart']);
            //explode the data to get day,month,year
            $start = explode('-', $_POST ['dateRangeStart']);
            $loanType->setStartDay($start[2]);
            $loanType->setStartMonth($start[1]);
            $loanType->setStartYear($start[0]);
        }
        if (isset($_POST ['dateRangeEnd']) && (strlen($_POST['dateRangeEnd']) > 0)) {
            $loanType->setDateRangeEndQuery($_POST['dateRangeEnd']);
            //explode the data to get day,month,year
            $start = explode('-', $_POST ['dateRangeEnd']);
            $loanType->setEndDay($start[2]);
            $loanType->setEndMonth($start[1]);
            $loanType->setEndYear($start[0]);
        }
        if (isset($_POST ['dateRangeType'])) {
            $loanType->setDateRangeTypeQuery($_POST['dateRangeType']);
        }
        if (isset($_POST ['dateRangeExtraType'])) {
            $loanType->setDateRangeExtraTypeQuery($_POST['dateRangeExtraType']);
        }
        $loanType->setServiceOutput('html');
        $loanType->setLeafId($leafId);
        $loanType->execute();
        $loanCategoryArray = $loanType->getLoanCategory();
        if ($_POST['method'] == 'read') {
            $loanType->setStart($offset);
            $loanType->setLimit($limit); // normal system don't like paging..
            $loanType->setPageOutput('html');
            $loanTypeArray = $loanType->read();
            if (isset($loanTypeArray [0]['firstRecord'])) {
                $firstRecord = $loanTypeArray [0]['firstRecord'];
            }
            if (isset($loanTypeArray [0]['nextRecord'])) {
                $nextRecord = $loanTypeArray [0]['nextRecord'];
            }
            if (isset($loanTypeArray [0]['previousRecord'])) {
                $previousRecord = $loanTypeArray [0]['previousRecord'];
            }
            if (isset($loanTypeArray [0]['lastRecord'])) {
                $lastRecord = $loanTypeArray [0]['lastRecord'];
                $endRecord = $loanTypeArray [0]['lastRecord'];
            }
            $navigation = new \Core\Paging\HtmlPaging();
            $navigation->setLeafId($leafId);
            $navigation->setViewPath($loanType->getViewPath());
            ...
            if (isset($loanTypeArray [0]['total'])) {
                $total = $loanTypeArray [0]['total'];
            } else {
                $total = 0;
            }
                        ...
                        <div class="btn-group">
                            ...
                            <ul class="dropdown-menu" style="text-align:left">
                                <li>
                                    ...
                                    echo $loanType->getControllerPath();
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <form class="form-horizontal">
                                        <input type="hidden" name="loanTypeIdPreview" id="loanTypeIdPreview">
                                        ...
                                        <div class="form-group" id="loanTypeDescriptionDiv">
                                            <label for="loanTypeDescriptionPreview"
                                                   class="control-label col-xs-4 col-sm-4 col-md-4"><?php echo $leafTranslation['loanTypeDescriptionLabel']; ?></label>

                                            <div class="col-xs-8 col-sm-8 col-md-8">
                                                <input class="form-control" type="text" name="loanTypeDescriptionPreview"
                                                       id="loanTypeDescriptionPreview">
                                            </div>
                                        </div>
                                    </form>
                                </div>
                        ...
                        <div class="col-xs-12 col-sm-12 col-md-12">
                            <div class="panel panel-default">
                                <table class="table table table-bordered table-striped table-condensed table-hover smart-form has-tickbox" id="tableData">
                                    ...
                                    <th vAlign="top" width="100px"><?php echo ucwords($leafTranslation['loanCategoryIdLabel']); ?></th>
                                    <th vAlign="top"><?php echo ucwords($leafTranslation['loanTypeDescriptionLabel']); ?></th>
                                            ...
                                            if (is_array($loanTypeArray)) {
                                                ...
                                                if ($totalRecord > 0) {
                                                    ...
                                                    for ($i = 0; $i < $totalRecord; $i++) {
                                                        ...
                                                        ?>
                                                        <tr <?php
                                                        if ($loanTypeArray[$i]['isDelete'] == 1) {
                                                            echo "class=\"danger\"";
                                                        } else {
                                                            if ($loanTypeArray[$i]['isDraft'] == 1) {
                                                                echo "class=\"warning\"";
                                                            }
                                                        }
                                                        ?>>
                                                            ...
                                                            <td align="center">
                                                                <div class="btn-group" align="center">
                                                                    ...
                                                                    <button type="button"  class="btn btn-danger btn-sm" title="Delete"
                                                                            ...
                                                                            ?>', '<?php
                                                                            echo rawurlencode(
                                                                                    $loanTypeArray [$i]['loanCategoryDescription']
                                                                            );
                                                                            ?>', '<?php
                                                                            echo rawurlencode(
                                                                                    $loanTypeArray [$i]['loanTypeDescription']
                                                                            );
                                                                            ?>');">
                                                            ...
                                                            <td>
                                                                <div class="pull-left">
                                                                    <?php
                                                                    if (isset($loanTypeArray[$i]['loanCategoryDescription'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos(
                                                                                                $loanTypeArray[$i]['loanCategoryDescription'], $_POST['query']
                                                                                        ) !== false
                                                                                ) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $loanTypeArray[$i]['loanCategoryDescription']
                                                                                    );
                                                                                } else {
                                                                                    echo $loanTypeArray[$i]['loanCategoryDescription'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                    if (strpos(
                                                                                                    $loanTypeArray[$i]['loanCategoryDescription'], $_POST['character']
                                                                                            ) !== false
                                                                                    ) {
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $loanTypeArray[$i]['loanCategoryDescription']
                                                                                        );
                                                                                    } else {
                                                                                        echo $loanTypeArray[$i]['loanCategoryDescription'];
                                                                                    }
                                                            ...
                                                            <td>
                                                                <div class="pull-left">
                                                                    <?php
                                                                    if (isset($loanTypeArray[$i]['loanTypeDescription'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos(
                                                                                                strtolower($loanTypeArray[$i]['loanTypeDescription']), strtolower($_POST['query'])
                                                                                        ) !== false
                                                                                ) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $loanTypeArray[$i]['loanTypeDescription']
                                                                                    );
                                                                                } else {
                                                                                    echo $loanTypeArray[$i]['loanTypeDescription'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                    if (strpos(
                                                                                                    strtolower($loanTypeArray[$i]['loanTypeDescription']), strtolower($_POST['character'])
                                                                                            ) !== false
                                                                                    ) {
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $loanTypeArray[$i]['loanTypeDescription']
                                                                                        );
                                                                                    } else {
                                                                                        echo $loanTypeArray[$i]['loanTypeDescription'];
                                                                                    }
                                                            ...
                                                            <td>
                                                                <div align="center">
                                                                    <?php
                                                                    if (isset($loanTypeArray[$i]['executeBy'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos($loanTypeArray[$i]['staffName'], $_POST['query']) !== false) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $loanTypeArray[$i]['staffName']
                                                                                    );
                                                                                } else {
                                                                                    echo $loanTypeArray[$i]['staffName'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                    if (strpos(
                                                                                                    $loanTypeArray[$i]['staffName'], $_POST['character']
                                                                                            ) !== false
                                                                                    ) {
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $loanTypeArray[$i]['staffName']
                                                                                        );
                                                                                    } else {
                                                                                        echo $loanTypeArray[$i]['staffName'];
                                                                                    }
                                                            ...
                                                            <?php
                                                            if (isset($loanTypeArray[$i]['executeTime'])) {
                                                                $valueArray = $loanTypeArray[$i]['executeTime'];
                                                            ...
                                                            ?>
                                                            <td>
                                                                <input style="display:none;" type="checkbox" name="loanTypeId[]"
                                                                       value="<?php echo $loanTypeArray[$i]['loanTypeId']; ?>">
                                                                <input <?php echo $checked; ?> type="checkbox" name="isDelete[]"
                                                                                               value="<?php echo $loanTypeArray[$i]['isDelete']; ?>">
                                                            </td>
                                                        </tr>
                                                        <?php
                                                    }
                        ...
                        <div class="col-xs-3 col-sm-3 col-md-3 pagination">
                            <div class="pull-right">
                                <button class="delete btn btn-warning" type="button" 
                                        ...
                                        echo $loanType->getControllerPath();
                                        ?>', '<?php echo $loanType->getViewPath(); ?>', '<?php echo $securityToken; ?>');">
...
if ((isset($_POST['method']) == 'new' || isset($_POST['method']) == 'read') && $_POST['type'] == 'form') {
            ...
            <div class="row">
                <div class="col-xs-12 col-sm-12 col-md-12">
                    <div class="panel panel-info">
                        ...
                        <div class="panel-body">
                            <input type="hidden" name="loanTypeId" id="loanTypeId" value="<?php
                            if (isset($_POST['loanTypeId'])) {
                                echo $_POST['loanTypeId'];
                            }
                            ?>">

                            <div class="form-group" id="loanCategoryIdForm">
                                <label for="loanCategoryId" class="control-label col-xs-2 col-sm-2 col-md-2"><strong><?php
                                        echo ucfirst(
                                    ...
                                    <select name="loanCategoryId" id="loanCategoryId" class="chzn-select form-control">
                                        ...
                                        if (is_array($loanCategoryArray)) {
                                            ...
                                            if ($totalRecord > 0) {
                                                $d = 1;
                                                for ($i = 0; $i < $totalRecord; $i++) {
                                                    if (isset($loanTypeArray[0]['loanCategoryId'])) {
                                                        if ($loanTypeArray[0]['loanCategoryId'] == $loanCategoryArray[$i]['loanCategoryId']) {
                                                            $selected = "selected";
                                                        } else {
                                                            $selected = null;
                                                        }
                            ...
                            <div class="form-group" id="loanTypeDescriptionForm">
                                <div class="col-xs-12 col-sm-12 col-md-12">
                                    <textarea class="form-control" rows="5" name="loanTypeDescription"
                                              id="loanTypeDescription"><?php
                                                  if (isset($loanTypeArray[0]['loanTypeDescription'])) {
                                                      echo htmlentities($loanTypeArray[0]['loanTypeDescription']);
                                                  }
                                                  ?></textarea> <span class="help-block" id="loanTypeDescriptionHelpMe"></span>
                                </div>
                            </div>
                        </div>
    ...
    <?php if ($leafAccess['leafAccessCreateValue'] == 1) { ?>
                            newRecord(<?php echo $leafId; ?>, '<?php echo $loanType->getControllerPath(); ?>', '<?php echo $loanType->getViewPath(); ?>', '<?php echo $securityToken; ?>', 1);
    <?php } ?>
                        return false;
                    }
                    // shift+s save event
    ...
    <?php if ($leafAccess['leafAccessUpdateValue'] == 1) { ?>
                            updateRecord(<?php echo $leafId; ?>, '<?php echo $loanType->getControllerPath(); ?>', '<?php echo $loanType->getViewPath(); ?>', '<?php echo $securityToken; ?>', 1, <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
    <?php } ?>
                        return false;
                    }
    ...
    <?php if ($leafAccess['leafAccessDeleteValue'] == 1) { ?>
                            deleteRecord(<?php echo $leafId; ?>, '<?php echo $loanType->getControllerPath(); ?>', '<?php echo $loanType->getViewPath(); ?>', '<?php echo $securityToken; ?>', <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
    ...
    <?php } ?>
                    ...
                    switch (e.keyCode) {
                        case 37:
                            previousRecord(<?php echo $leafId; ?>, '<?php echo $loanType->getControllerPath(); ?>', '<?php echo $loanType->getViewPath(); ?>', '<?php echo $securityToken; ?>', <?php echo $leafAccess['leafAccessUpdateValue']; ?>, <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
                            
                            return false;
                            break;
                        case 39:
                            nextRecord(<?php echo $leafId; ?>, '<?php echo $loanType->getControllerPath(); ?>', '<?php echo $loanType->getViewPath(); ?>', '<?php echo $securityToken; ?>', <?php echo $leafAccess['leafAccessUpdateValue']; ?>, <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
    ...
    <?php if ($_POST['method'] == "new") { ?>
        ...
        <?php if ($leafAccess['leafAccessCreateValue'] == 1) { ?>
                            $('#newRecordButton1').removeClass().addClass('btn btn-success').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $loanType->getControllerPath(); ?>','<?php echo $loanType->getViewPath(); ?>','<?php echo $securityToken; ?>',1)");
                            $('#newRecordButton2').removeClass().addClass('btn dropdown-toggle btn-success');
                            $('#newRecordButton3').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $loanType->getControllerPath(); ?>','<?php echo $loanType->getViewPath(); ?>','<?php echo $securityToken; ?>',1)");
                            $('#newRecordButton4').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $loanType->getControllerPath(); ?>','<?php echo $loanType->getViewPath(); ?>','<?php echo $securityToken; ?>',2)");
                            $('#newRecordButton5').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $loanType->getControllerPath(); ?>','<?php echo $loanType->getViewPath(); ?>','<?php echo $securityToken; ?>',3)");
                            $('#newRecordButton6').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $loanType->getControllerPath(); ?>','<?php echo $loanType->getViewPath(); ?>','<?php echo $securityToken; ?>',4)");
                            $('#newRecordButton7').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $loanType->getControllerPath(); ?>','<?php echo $loanType->getViewPath(); ?>','<?php echo $securityToken; ?>',5)");
        <?php } else { ?>
        ...
        if ($_POST['loanTypeId']) {
            ?>
                            ...
                            $('#updateRecordButton5');
            <?php if ($leafAccess['leafAccessUpdateValue'] == 1) { ?>
                                $('#updateRecordButton1').removeClass().addClass('btn btn-info').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $loanType->getControllerPath(); ?>','<?php echo $loanType->getViewPath(); ?>','<?php echo $securityToken; ?>',1,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                                $('#updateRecordButton2').removeClass().addClass('btn dropdown-toggle btn-info');
                                $('#updateRecordButton3').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $loanType->getControllerPath(); ?>','<?php echo $loanType->getViewPath(); ?>','<?php echo $securityToken; ?>',1,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                                $('#updateRecordButton4').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $loanType->getControllerPath(); ?>','<?php echo $loanType->getViewPath(); ?>','<?php echo $securityToken; ?>',2,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                                $('#updateRecordButton5').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $loanType->getControllerPath(); ?>','<?php echo $loanType->getViewPath(); ?>','<?php echo $securityToken; ?>',3,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
            <?php } else { ?>
                                $('#updateRecordButton1').removeClass().addClass(' btn btn-info disabled').attr('onClick', '');
                                $('#updateRecordButton2').removeClass().addClass('btn dropdown-toggle btn-info disabled');
                                $('#updateRecordButton3').attr('onClick', '');
            ...
            <?php } ?>
            <?php if ($leafAccess['leafAccessDeleteValue'] == 1) { ?>
                                $('#deleteRecordButton').removeClass().addClass('btn btn-danger').attr('onClick', "deleteRecord(<?php echo $leafId; ?>,'<?php echo $loanType->getControllerPath(); ?>','<?php echo $loanType->getViewPath(); ?>','<?php echo $securityToken; ?>',<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
            <?php } else { ?>
                                $('#deleteRecordButton').removeClass().addClass('btn btn-danger disabled').attr('onClick', '');
            <?php } ?>
            <?php
...
<?php } ?>
<script type="text/javascript" src="./v3/humanResource/loan/javascript/loanType.js"></script>
<hr>
