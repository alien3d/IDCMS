...
require_once($newFakeDocumentRoot . "v3/humanResource/loan/controller/loanCategoryController.php");
...
$translator->setCurrentTable('loanCategory');
...
if (isset($_POST)) {
    if (isset($_POST['method'])) {
        $loanCategory = new \Core\HumanResource\Loan\LoanCategory\Controller\LoanCategoryClass();
        define('LIMIT', 10);
        ...
        if (isset($_POST ['query'])) {
            $loanCategory->setFieldQuery($_POST ['query']);
        }
        if (isset($_POST ['filter'])) {
            $loanCategory->setGridQuery($_POST ['filter']);
        }
        if (isset($_POST ['character'])) {
            $loanCategory->setCharacterQuery($_POST['character']);
        }
        if (isset($_POST ['dateRangeStart'])) {
            $loanCategory->setDateRangeStartQuery($_POST['dateRangeStart']);
            //explode the data to get day,month,year
            $start = explode('-', $_POST ['dateRangeStart']);
            $loanCategory->setStartDay($start[2]);
            $loanCategory->setStartMonth($start[1]);
            $loanCategory->setStartYear($start[0]);
        }
        if (isset($_POST ['dateRangeEnd']) && (strlen($_POST['dateRangeEnd']) > 0)) {
            $loanCategory->setDateRangeEndQuery($_POST['dateRangeEnd']);
            //explode the data to get day,month,year
            $start = explode('-', $_POST ['dateRangeEnd']);
            $loanCategory->setEndDay($start[2]);
            $loanCategory->setEndMonth($start[1]);
            $loanCategory->setEndYear($start[0]);
        }
        if (isset($_POST ['dateRangeType'])) {
            $loanCategory->setDateRangeTypeQuery($_POST['dateRangeType']);
        }
        if (isset($_POST ['dateRangeExtraType'])) {
            $loanCategory->setDateRangeExtraTypeQuery($_POST['dateRangeExtraType']);
        }
        $loanCategory->setServiceOutput('html');
        $loanCategory->setLeafId($leafId);
        $loanCategory->execute();
        if ($_POST['method'] == 'read') {
            $loanCategory->setStart($offset);
            $loanCategory->setLimit($limit); // normal system don't like paging..
            $loanCategory->setPageOutput('html');
            $loanCategoryArray = $loanCategory->read();
            if (isset($loanCategoryArray [0]['firstRecord'])) {
                $firstRecord = $loanCategoryArray [0]['firstRecord'];
            }
            if (isset($loanCategoryArray [0]['nextRecord'])) {
                $nextRecord = $loanCategoryArray [0]['nextRecord'];
            }
            if (isset($loanCategoryArray [0]['previousRecord'])) {
                $previousRecord = $loanCategoryArray [0]['previousRecord'];
            }
            if (isset($loanCategoryArray [0]['lastRecord'])) {
                $lastRecord = $loanCategoryArray [0]['lastRecord'];
                $endRecord = $loanCategoryArray [0]['lastRecord'];
            }
            $navigation = new \Core\Paging\HtmlPaging();
            $navigation->setLeafId($leafId);
            $navigation->setViewPath($loanCategory->getViewPath());
            ...
            if (isset($loanCategoryArray [0]['total'])) {
                $total = $loanCategoryArray [0]['total'];
            } else {
                $total = 0;
            }
                        ...
                        <div class="btn-group">
                            ...
                            <ul class="dropdown-menu" style="text-align:left">
                                <li>
                                    ...
                                    echo $loanCategory->getControllerPath();
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <form class="form-horizontal">
                                        <input type="hidden" name="loanCategoryIdPreview" id="loanCategoryIdPreview">

                                        <div class="form-group" id="loanCategoryCodeDiv">
                                            <label for="loanCategoryCodePreview"
                                                   class="control-label col-xs-4 col-sm-4 col-md-4"><?php echo $leafTranslation['loanCategoryCodeLabel']; ?></label>

                                            <div class="col-xs-8 col-sm-8 col-md-8">
                                                <input class="form-control" type="text" name="loanCategoryCodePreview"
                                                       id="loanCategoryCodePreview">
                                            </div>
                                        </div>
                                        <div class="form-group" id="loanCategoryDescriptionDiv">
                                            <label for="loanCategoryDescriptionPreview"
                                                   class="control-label col-xs-4 col-sm-4 col-md-4"><?php echo $leafTranslation['loanCategoryDescriptionLabel']; ?></label>

                                            <div class="col-xs-8 col-sm-8 col-md-8">
                                                <input class="form-control" type="text"
                                                       name="loanCategoryDescriptionPreview" id="loanCategoryDescriptionPreview">
                                            </div>
                                        </div>
                                    </form>
                                </div>
                        ...
                        <div class="col-xs-12 col-sm-12 col-md-12">
                            <div class="panel panel-default">
                                <table class="table table table-bordered table-striped table-condensed table-hover smart-form has-tickbox" id="tableData">
                                    ...
                                    <th vAlign="top" width="175px">
                                    <div align="center"><?php echo ucwords($leafTranslation['loanCategoryCodeLabel']); ?></div>
                                    </th>
                                    <th vAlign="top"><?php echo ucwords($leafTranslation['loanCategoryDescriptionLabel']); ?></th>
                                            ...
                                            if (is_array($loanCategoryArray)) {
                                                ...
                                                if ($totalRecord > 0) {
                                                    ...
                                                    for ($i = 0; $i < $totalRecord; $i++) {
                                                        ...
                                                        ?>
                                                        <tr <?php
                                                        if ($loanCategoryArray[$i]['isDelete'] == 1) {
                                                            echo "class=\"danger\"";
                                                        } else {
                                                            if ($loanCategoryArray[$i]['isDraft'] == 1) {
                                                                echo "class=\"warning\"";
                                                            }
                                                        }
                                                        ?>>
                                                            ...
                                                            <td align="center">
                                                                <div class="btn-group" align="center">
                                                                    <button type="button"  class="btn btn-info btn-sm" title="Edit"
                                                                            ...
                                                                            echo intval(
                                                                                    $loanCategoryArray [$i]['loanCategoryId']
                                                                            );
                                                                    ...
                                                                    <button type="button"  class="btn btn-danger btn-sm" title="Delete"
                                                                            ...
                                                                            ?>', '<?php
                                                                            echo rawurlencode(
                                                                                    $loanCategoryArray [$i]['loanCategoryCode']
                                                                            );
                                                                            ?>', '<?php
                                                                            echo rawurlencode(
                                                                                    $loanCategoryArray [$i]['loanCategoryDescription']
                                                                            );
                                                                            ?>');">
                                                            ...
                                                            <td>
                                                                <div align="center">
                                                                    <?php
                                                                    if (isset($loanCategoryArray[$i]['loanCategoryCode'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos(
                                                                                                strtolower($loanCategoryArray[$i]['loanCategoryCode']), strtolower($_POST['query'])
                                                                                        ) !== false
                                                                                ) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $loanCategoryArray[$i]['loanCategoryCode']
                                                                                    );
                                                                                } else {
                                                                                    echo $loanCategoryArray[$i]['loanCategoryCode'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                    if (strpos(
                                                                                                    strtolower($loanCategoryArray[$i]['loanCategoryCode']), strtolower($_POST['character'])
                                                                                            ) !== false
                                                                                    ) {
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $loanCategoryArray[$i]['loanCategoryCode']
                                                                                        );
                                                                                    } else {
                                                                                        echo $loanCategoryArray[$i]['loanCategoryCode'];
                                                                                    }
                                                            ...
                                                            <td>
                                                                <div class="pull-left">
                                                                    <?php
                                                                    if (isset($loanCategoryArray[$i]['loanCategoryDescription'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos(
                                                                                                strtolower($loanCategoryArray[$i]['loanCategoryDescription']), strtolower($_POST['query'])
                                                                                        ) !== false
                                                                                ) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $loanCategoryArray[$i]['loanCategoryDescription']
                                                                                    );
                                                                                } else {
                                                                                    echo $loanCategoryArray[$i]['loanCategoryDescription'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                    if (strpos(
                                                                                                    strtolower($loanCategoryArray[$i]['loanCategoryDescription']), strtolower($_POST['character'])
                                                                                            ) !== false
                                                                                    ) {
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $loanCategoryArray[$i]['loanCategoryDescription']
                                                                                        );
                                                                                    } else {
                                                                                        echo $loanCategoryArray[$i]['loanCategoryDescription'];
                                                                                    }
                                                            ...
                                                            <td>
                                                                <div align="center">
                                                                    <?php
                                                                    if (isset($loanCategoryArray[$i]['executeBy'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos(
                                                                                                $loanCategoryArray[$i]['staffName'], $_POST['query']
                                                                                        ) !== false
                                                                                ) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $loanCategoryArray[$i]['staffName']
                                                                                    );
                                                                                } else {
                                                                                    echo $loanCategoryArray[$i]['staffName'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen($_POST['character']) > 0) {
                                                                                    if (strpos(
                                                                                                    $loanCategoryArray[$i]['staffName'], $_POST['character']
                                                                                            ) !== false
                                                                                    ) {
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $loanCategoryArray[$i]['staffName']
                                                                                        );
                                                                                    } else {
                                                                                        echo $loanCategoryArray[$i]['staffName'];
                                                                                    }
                                                            ...
                                                            <?php
                                                            if (isset($loanCategoryArray[$i]['executeTime'])) {
                                                                $valueArray = $loanCategoryArray[$i]['executeTime'];
                                                            ...
                                                            <?php } ?>
                                                            <?php
                                                            if ($loanCategoryArray[$i]['isDelete']) {
                                                                $checked = "checked";
                                                            } else {
                                                                $checked = null;
                                                            }
                                                            ?>
                                                            <td>
                                                                <input style="display:none;" type="checkbox" name="loanCategoryId[]"
                                                                       value="<?php echo $loanCategoryArray[$i]['loanCategoryId']; ?>">
                                                                <input <?php echo $checked; ?> type="checkbox" name="isDelete[]"
                                                                                               value="<?php echo $loanCategoryArray[$i]['isDelete']; ?>">
                                                            </td>
                                                        </tr>
                                                        <?php
                                                    }
                        ...
                        <div class="col-xs-3 col-sm-3 col-md-3 pagination">
                            <div class="pull-right">
                                <button class="delete btn btn-warning" type="button" 
                                        ...
                                        echo $loanCategory->getControllerPath();
                                        ?>', '<?php echo $loanCategory->getViewPath(); ?>', '<?php echo $securityToken; ?>');">
...
if ((isset($_POST['method']) == 'new' || isset($_POST['method']) == 'read') && $_POST['type'] == 'form') {
        ...
        <div id="content" style="opacity: 1;">
            <div class="row">
                <div class="panel panel-info">
                    ...
                    <div class="panel-body">
                        <input type="hidden" name="loanCategoryId" id="loanCategoryId" value="<?php
                        if (isset($_POST['loanCategoryId'])) {
                            echo $_POST['loanCategoryId'];
                        }
                        ?>">

                        <div class="form-group" id="loanCategoryCodeForm">
                            <label for="loanCategoryCode" class="control-label col-xs-2 col-sm-2 col-md-2"><strong><?php
                                    echo ucfirst(
                                            $leafTranslation['loanCategoryCodeLabel']
                                    );
                                    ?></strong></label>

                            <div class="col-xs-10 col-sm-10 col-md-10">
                                <input class="form-control" type="text" name="loanCategoryCode" id="loanCategoryCode"
                                       onKeyUp="removeMeError('loanCategoryCode');" value="<?php
                                       if (isset($loanCategoryArray) && is_array($loanCategoryArray)) {
                                           if (isset($loanCategoryArray[0]['loanCategoryCode'])) {
                                               echo htmlentities($loanCategoryArray[0]['loanCategoryCode']);
                                           }
                                       }
                                       ?>" maxlength="16"> <span class="help-block" id="loanCategoryCodeHelpMe"></span>
                            </div>
                        </div>
                        <div class="form-group" id="loanCategoryDescriptionForm">
                            <div class="col-xs-12 col-sm-12 col-md-12">
                                <textarea class="form-control" rows="5" name="loanCategoryDescription"
                                          id="loanCategoryDescription"><?php
                                              if (isset($loanCategoryArray[0]['loanCategoryDescription'])) {
                                                  echo htmlentities($loanCategoryArray[0]['loanCategoryDescription']);
                                              }
                                              ?></textarea> <span class="help-block" id="loanCategoryDescriptionHelpMe"></span>
                            </div>
                        </div>
                    </div>
    ...
    <?php if ($leafAccess['leafAccessCreateValue'] == 1) { ?>
                            newRecord(<?php echo $leafId; ?>, '<?php echo $loanCategory->getControllerPath(); ?>', '<?php echo $loanCategory->getViewPath(); ?>', '<?php echo $securityToken; ?>', 1);
    <?php } ?>
                        return false;
                    }
                    // shift+s save event
    ...
    <?php if ($leafAccess['leafAccessUpdateValue'] == 1) { ?>
                            updateRecord(<?php echo $leafId; ?>, '<?php echo $loanCategory->getControllerPath(); ?>', '<?php echo $loanCategory->getViewPath(); ?>', '<?php echo $securityToken; ?>', 1, <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
    <?php } ?>
                        return false;
                    }
    ...
    <?php if ($leafAccess['leafAccessDeleteValue'] == 1) { ?>
                            deleteRecord(<?php echo $leafId; ?>, '<?php echo $loanCategory->getControllerPath(); ?>', '<?php echo $loanCategory->getViewPath(); ?>', '<?php echo $securityToken; ?>', <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
    ...
    <?php } ?>
                    ...
                    switch (e.keyCode) {
                        case 37:
                            previousRecord(<?php echo $leafId; ?>, '<?php echo $loanCategory->getControllerPath(); ?>', '<?php echo $loanCategory->getViewPath(); ?>', '<?php echo $securityToken; ?>', <?php echo $leafAccess['leafAccessUpdateValue']; ?>, <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
                            
                            return false;
                            break;
                        case 39:
                            nextRecord(<?php echo $leafId; ?>, '<?php echo $loanCategory->getControllerPath(); ?>', '<?php echo $loanCategory->getViewPath(); ?>', '<?php echo $securityToken; ?>', <?php echo $leafAccess['leafAccessUpdateValue']; ?>, <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
                ...
                $(document).ready(function() {
                    ...
                    validateMeAlphaNumeric('loanCategoryCode');
    <?php if ($_POST['method'] == "new") { ?>
                        $('#resetRecordButton').removeClass().addClass('btn btn-default');
        <?php if ($leafAccess['leafAccessCreateValue'] == 1) { ?>
                            $('#newRecordButton1').removeClass().addClass('btn btn-success').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $loanCategory->getControllerPath(); ?>','<?php echo $loanCategory->getViewPath(); ?>','<?php echo $securityToken; ?>',1)");
                            $('#newRecordButton2').removeClass().addClass('btn dropdown-toggle btn-success');
                            $('#newRecordButton3').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $loanCategory->getControllerPath(); ?>','<?php echo $loanCategory->getViewPath(); ?>','<?php echo $securityToken; ?>',1)");
                            $('#newRecordButton4').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $loanCategory->getControllerPath(); ?>','<?php echo $loanCategory->getViewPath(); ?>','<?php echo $securityToken; ?>',2)");
                            $('#newRecordButton5').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $loanCategory->getControllerPath(); ?>','<?php echo $loanCategory->getViewPath(); ?>','<?php echo $securityToken; ?>',3)");
                            $('#newRecordButton6').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $loanCategory->getControllerPath(); ?>','<?php echo $loanCategory->getViewPath(); ?>','<?php echo $securityToken; ?>',4)");
                            $('#newRecordButton7').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $loanCategory->getControllerPath(); ?>','<?php echo $loanCategory->getViewPath(); ?>','<?php echo $securityToken; ?>',5)");
        <?php } else { ?>
        ...
        if ($_POST['loanCategoryId']) {
            ...
            <?php if ($leafAccess['leafAccessUpdateValue'] == 1) { ?>
                                $('#updateRecordButton1').removeClass().addClass('btn btn-info').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $loanCategory->getControllerPath(); ?>','<?php echo $loanCategory->getViewPath(); ?>','<?php echo $securityToken; ?>',1,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                                $('#updateRecordButton2').removeClass().addClass('btn dropdown-toggle btn-info');
                                $('#updateRecordButton3').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $loanCategory->getControllerPath(); ?>','<?php echo $loanCategory->getViewPath(); ?>','<?php echo $securityToken; ?>',1,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                                $('#updateRecordButton4').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $loanCategory->getControllerPath(); ?>','<?php echo $loanCategory->getViewPath(); ?>','<?php echo $securityToken; ?>',2,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                                $('#updateRecordButton5').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $loanCategory->getControllerPath(); ?>','<?php echo $loanCategory->getViewPath(); ?>','<?php echo $securityToken; ?>',3,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
            <?php } else { ?>
                                $('#updateRecordButton1').removeClass().addClass(' btn btn-info disabled').attr('onClick', '');
                                $('#updateRecordButton2').removeClass().addClass('btn dropdown-toggle btn-info disabled');
                                $('#updateRecordButton3').attr('onClick', '');
            ...
            <?php } ?>
            <?php if ($leafAccess['leafAccessDeleteValue'] == 1) { ?>
                                $('#deleteRecordButton').removeClass().addClass('btn btn-danger').attr('onClick', "deleteRecord(<?php echo $leafId; ?>,'<?php echo $loanCategory->getControllerPath(); ?>','<?php echo $loanCategory->getViewPath(); ?>','<?php echo $securityToken; ?>',<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
            <?php } else { ?>
                                $('#deleteRecordButton').removeClass().addClass('btn btn-danger disabled').attr('onClick', '');
            <?php } ?>
            <?php
...
<?php } ?>
<script type="text/javascript" src="./v3/humanResource/loan/javascript/loanCategory.js"></script>
<hr>
