...
function checkDuplicate(leafId, page, securityToken) {
    var $vacancyCode = $("#vacancyCode");
    if ($vacancyCode.val().length === 0) {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if (success === true) {
                if (total !== 0) {
                    $("#vacancyCode").val('').focus();
                    $("#vacancyCodeForm").removeClass().addClass("form-group has-error");
                ...
                $("#vacancyForm").removeClass().addClass("form-group has-error");
            }
            if ($infoPanel.is(':hidden')) {
                $infoPanel.show();
            }
...
function showFormUpdate(leafId, url, urlList, securityToken, vacancyId, updateAccess, deleteAccess) {
    sleep(500);
...
function showModalDelete(vacancyId, jobId, documentNumber, vacancyTitle, vacancyRequiredDate, vacancyRequiredPeople, vacancyDescription) {
    $('#vacancyIdPreview').val('').val(decodeURIComponent(vacancyId));
    $('#jobIdPreview').val('').val(decodeURIComponent(jobId));
    $('#documentNumberPreview').val('').val(decodeURIComponent(documentNumber));
    $('#vacancyTitlePreview').val('').val(decodeURIComponent(vacancyTitle));
    $('#vacancyRequiredDatePreview').val('').val(decodeURIComponent(vacancyRequiredDate));
    $('#vacancyRequiredPeoplePreview').val('').val(decodeURIComponent(vacancyRequiredPeople));
    $('#vacancyDescriptionPreview').val('').val(decodeURIComponent(vacancyDescription));
...
function deleteGridRecord(leafId, url, urlList, securityToken) {
    $.ajax({
        ...
        data: {
            method: 'delete',
            output: 'json',
            vacancyId: $('#vacancyIdPreview').val(),
            securityToken: securityToken,
            leafId: leafId
        },
        beforeSend: function() {
...
function deleteGridRecordCheckbox(leafId, url, urlList, securityToken) {
    var stringText = '';
    var counter = 0;
    $('input:checkbox[name="vacancyId[]"]').each(function() {
        stringText = stringText + "&vacancyId[]=" + $(this).val();
    });
...
function newRecord(leafId, url, urlList, securityToken, type, createAccess, updateAccess, deleteAccess) {
    ...
    var $jobId = $('#jobId');
    var $vacancyTitle = $('#vacancyTitle');
    var $vacancyRequiredDate = $('#vacancyRequiredDate');
    var $vacancyRequiredPeople = $('#vacancyRequiredPeople');
    var $vacancyDescription = $('#vacancyDescription');
    if (css.search('disabled') > 0) {
        return false;
    } else {
        if (type === 1) {
            ...
            if ($vacancyTitle.val().length === 0) {
                $('#vacancyTitleHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['vacancyTitleLabel'] + " </span>");
                $('#vacancyTitleForm').removeClass().addClass('form-group has-error');
                $vacancyTitle.focus();
                return false;
            }
            if ($vacancyRequiredDate.val().length === 0) {
                $('#vacancyRequiredDateHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['vacancyRequiredDateLabel'] + " </span>");
                $('#vacancyRequiredDateForm').removeClass().addClass('form-group has-error');
                $vacancyRequiredDate.focus();
                return false;
            }
            if ($vacancyRequiredPeople.val().length === 0) {
                $('#vacancyRequiredPeopleHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['vacancyRequiredPeopleLabel'] + " </span>");
                $('#vacancyRequiredPeopleForm').removeClass().addClass('form-group has-error');
                $vacancyRequiredPeople.focus();
                return false;
            }
            if ($vacancyDescription.val().length === 0) {
                $('#vacancyDescriptionHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['vacancyDescriptionLabel'] + " </span>");
                $('#vacancyDescriptionForm').removeClass().addClass('form-group has-error');
                $vacancyDescription.focus();
                return false;
            }
            $.ajax({
                type: 'POST',
                ...
                data: {
                    ...
                    vacancyTitle: $vacancyTitle.val(),
                    vacancyRequiredDate: $vacancyRequiredDate.val(),
                    vacancyRequiredPeople: $vacancyRequiredPeople.val(),
                ...
                success: function(data) {
                    ...
                    if (success === true) {
                        ...
                        $vacancyTitle.val('');
                        $('#vacancyTitleHelpMe').html('').empty();
                        $vacancyRequiredDate.val('');
                        $('#vacancyRequiredDateHelpMe').html('').empty();
                        $vacancyRequiredPeople.val('');
                        $('#vacancyRequiredPeopleHelpMe').html('').empty();
                        $vacancyDescription.val('');
                        $('#vacancyDescriptionForm').removeClass().addClass('form-group');
                        $('#vacancyDescriptionHelpMe').html('').empty();
                    } else if (success === false) {
                        $infoPanel.html('').empty().html("<span class='label label-important'>&nbsp;" + message + "</span>");
                        if ($infoPanel.is(':hidden')) {
                            $infoPanel.show();
...
function resetRecord(leafId, url, urlList, securityToken, createAccess, updateAccess, deleteAccess) {
    ...
    $("#vacancyTitle").val('');
    $("#vacancyTitleHelpMe").empty().html('');
    $("#vacancyRequiredDate").val('');
    $("#vacancyRequiredDateHelpMe").empty().html('');
    $("#vacancyRequiredPeople").val('');
    $("#vacancyRequiredPeopleHelpMe").empty().html('');
    $("#vacancyDescription").val('');
    $("#vacancyDescriptionHelpMe").empty().html('');
    $('#vacancyDescription').empty().val('');
}
function postRecord() {
                ...
                if (success === true) {
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if (success === true) {
                                $('#vacancyId').val(data.data.vacancyId);
                                $('#jobId').val(data.data.jobId).trigger("chosen:updated");
                                $('#documentNumber').val(data.data.documentNumber);
                                $('#vacancyTitle').val(data.data.vacancyTitle);
                                x = data.data.vacancyRequiredDate;
                                x = x.split("-");
                                output = x[2] + "-" + x[1] + "-" + x[0];
                                output = output.toString();
                                $('#vacancyRequiredDate').val(output);
                                $('#vacancyRequiredPeople').val(data.data.vacancyRequiredPeople);
                                $('#vacancyDescription').val(data.data.vacancyDescription);
                                if (nextRecord > 0) {
                                    $('#previousRecordButton').removeClass().addClass('btn btn-default disabled').attr('onClick', '');
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($previousRecordCounter.val()) > 0 && parseFloat($previousRecordCounter.val()) < parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    vacancyId: $previousRecordCounter.val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($nextRecordCounter.val()) <= parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    vacancyId: $nextRecordCounter.val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
