function getTraining(leafId, url, securityToken) {
    $.ajax({
        type: 'GET',
        url: url,
        data: {
        ...
        success: function(data) {
            ...
            if (success === false) {
                ...
                $("#trainingId").html('').empty().html(data.data).trigger("chosen:updated");
...
function checkDuplicate(leafId, page, securityToken) {
    var $trainingEmployeeCode = $("#trainingEmployeeCode");
    if ($trainingEmployeeCode.val().length === 0) {
    ...
    $.ajax({
        ...
        url: page,
        data: {
            trainingEmployeeCode: $trainingEmployeeCode.val(),
            method: 'duplicate',
            securityToken: securityToken,
            leafId: leafId
        },
        ...
        success: function(data) {
            ...
            if (success === true) {
                if (total !== 0) {
                    $("#trainingEmployeeCode").val('').focus();
                    $("#trainingEmployeeCodeForm").removeClass().addClass("form-group has-error");
                ...
                $("#trainingEmployeeForm").removeClass().addClass("form-group has-error");
            }
            if ($infoPanel.is(':hidden')) {
                $infoPanel.show();
            }
...
function showFormUpdate(leafId, url, urlList, securityToken, trainingEmployeeId, updateAccess, deleteAccess) {
    sleep(500);
...
function showModalDelete(trainingEmployeeId, trainingId, employeeId) {
    $('#trainingEmployeeIdPreview').val('').val(decodeURIComponent(trainingEmployeeId));
...
function deleteGridRecordCheckbox(leafId, url, urlList, securityToken) {
    var stringText = '';
    var counter = 0;
    $('input:checkbox[name="trainingEmployeeId[]"]').each(function() {
        stringText = stringText + "&trainingEmployeeId[]=" + $(this).val();
    });
...
function newRecord(leafId, url, urlList, securityToken, type, createAccess, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        if (type === 1) {
            if ($trainingId.val().length === 0) {
                $('#trainingIdHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['trainingIdLabel'] + " </span>");
                $trainingId.data('chosen').activate_action();
                return false;
            }
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (success === true) {
                        ...
                        $trainingId.val('');
                        $trainingId.trigger("chosen:updated");
                        $('#trainingIdHelpMe').html('').empty();
                        $employeeId.val('');
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (success === true) {
                        ...
                        $('#trainingEmployeeId').val(data.trainingEmployeeId);
                        $('#newRecordButton1').removeClass().addClass('btn btn-success disabled').attr('onClick', '');
                        $('#newRecordButton2').removeClass().addClass('btn dropdown-toggle btn-success disabled');
                        $('#newRecordButton3').attr('onClick', '');
                        $('#newRecordButton4').attr('onClick', '');
        ...
        if (type === 1) {
            ...
            $.ajax({
                ...
                data: {
                    method: 'save',
                    output: 'json',
                    trainingEmployeeId: $trainingEmployeeId.val(),
                    trainingId: $trainingId.val(),
                    employeeId: $employeeId.val(),
                    securityToken: securityToken,
                    leafId: leafId
...
function deleteRecord(leafId, url, urlList, securityToken, deleteAccess) {
    var $infoPanel = $('#infoPanel');
    var $trainingEmployeeId = $('#trainingEmployeeId');
    ...
    if (css.search('disabled') > 0) {
        ...
        if (deleteAccess === 1) {
            if (confirm(decodeURIComponent(t['deleteRecordMessageLabel']))) {
                var value = $trainingEmployeeId.val();
                if (!value) {
...
function resetRecord(leafId, url, urlList, securityToken, createAccess, updateAccess, deleteAccess) {
    ...
    $("#trainingEmployeeId").val('');
    $("#trainingEmployeeIdHelpMe").empty().html('');
            ...
            success: function(data) {
                ...
                if (success === true) {
                    $.ajax({
                        ...
                        data: {
                            method: 'read',
                            trainingEmployeeId: firstRecord,
                            output: 'json',
                            securityToken: securityToken,
                            leafId: leafId
                        },
                        ...
                        success: function(data) {
                            ...
                            if (success === true) {
                                $('#trainingEmployeeId').val(data.data.trainingEmployeeId);
                                $('#trainingId').val(data.data.trainingId).trigger("chosen:updated");
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($previousRecordCounter.val()) > 0 && parseFloat($previousRecordCounter.val()) < parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    trainingEmployeeId: $previousRecordCounter.val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($nextRecordCounter.val()) <= parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    trainingEmployeeId: $nextRecordCounter.val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
