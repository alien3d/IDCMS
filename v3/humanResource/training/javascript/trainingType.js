function checkDuplicate(leafId, page, securityToken) {
    var $trainingTypeCode = $("#trainingTypeCode");
    if ($trainingTypeCode.val().length === 0) {
    ...
    $.ajax({
        ...
        url: page,
        data: {
            trainingTypeCode: $trainingTypeCode.val(),
            method: 'duplicate',
        ...
        success: function(data) {
            ...
            if (success === true) {
                if (total !== 0) {
                    $("#trainingTypeCode").val('').focus();
                    $("#trainingTypeCodeForm").removeClass().addClass("form-group has-error");
                ...
                $("#trainingTypeForm").removeClass().addClass("form-group has-error");
            }
            if ($infoPanel.is(':hidden')) {
                $infoPanel.show();
            }
...
function showFormUpdate(leafId, url, urlList, securityToken, trainingTypeId, updateAccess, deleteAccess) {
    sleep(500);
...
function showModalDelete(trainingTypeId, trainingTypeDescription) {
    $('#trainingTypeIdPreview').val('').val(decodeURIComponent(trainingTypeId));
    $('#trainingTypeDescriptionPreview').val('').val(decodeURIComponent(trainingTypeDescription));
...
function deleteGridRecord(leafId, url, urlList, securityToken) {
    $.ajax({
        ...
        data: {
            method: 'delete',
            output: 'json',
            trainingTypeId: $('#trainingTypeIdPreview').val(),
            securityToken: securityToken,
            leafId: leafId
        },
        beforeSend: function() {
...
function deleteGridRecordCheckbox(leafId, url, urlList, securityToken) {
    var stringText = '';
    var counter = 0;
    $('input:checkbox[name="trainingTypeId[]"]').each(function() {
        stringText = stringText + "&trainingTypeId[]=" + $(this).val();
    });
...
function newRecord(leafId, url, urlList, securityToken, type, createAccess, updateAccess, deleteAccess) {
    ...
    var $trainingTypeDescription = $('#trainingTypeDescription');
    if (css.search('disabled') > 0) {
        return false;
    } else {
        if (type === 1) {
            if ($trainingTypeDescription.val().length === 0) {
                $('#trainingTypeDescriptionHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['trainingTypeDescriptionLabel'] + " </span>");
                $('#trainingTypeDescriptionForm').removeClass().addClass('form-group has-error');
                $trainingTypeDescription.focus();
                return false;
            }
            $.ajax({
                type: 'POST',
                ...
                data: {
                    method: 'create',
                    output: 'json',
                    trainingTypeDescription: $trainingTypeDescription.val(),
                    securityToken: securityToken,
                    leafId: leafId
                },
                beforeSend: function() {
                ...
                success: function(data) {
                    ...
                    if (success === true) {
                        ...
                        $trainingTypeDescription.val('');
                        $('#trainingTypeDescriptionForm').removeClass().addClass('form-group');
                        $('#trainingTypeDescriptionHelpMe').html('').empty();
                    } else if (success === false) {
                        $infoPanel.html('').empty().html("<span class='label label-important'>&nbsp;" + message + "</span>");
                        if ($infoPanel.is(':hidden')) {
                            $infoPanel.show();
...
function resetRecord(leafId, url, urlList, securityToken, createAccess, updateAccess, deleteAccess) {
    ...
    $("#trainingTypeDescription").val('');
    $("#trainingTypeDescriptionHelpMe").empty().html('');
    $('#trainingTypeDescription').empty().val('');
}
function postRecord() {
                ...
                if (success === true) {
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if (success === true) {
                                $('#trainingTypeId').val(data.data.trainingTypeId);
                                $('#trainingTypeDescription').val(data.data.trainingTypeDescription);
                                if (nextRecord > 0) {
                                    $('#previousRecordButton').removeClass().addClass('btn btn-default disabled').attr('onClick', '');
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($previousRecordCounter.val()) > 0 && parseFloat($previousRecordCounter.val()) < parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    trainingTypeId: $previousRecordCounter.val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($nextRecordCounter.val()) <= parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    trainingTypeId: $nextRecordCounter.val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
