function getTrainingType(leafId, url, securityToken) {
    $.ajax({
        type: 'GET',
        url: url,
        data: {
        ...
        success: function(data) {
            ...
            if (success === false) {
                ...
                $("#trainingTypeId").html('').empty().html(data.data).trigger("chosen:updated");
...
function checkDuplicate(leafId, page, securityToken) {
    var $trainingCode = $("#trainingCode");
    if ($trainingCode.val().length === 0) {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if (success === true) {
                if (total !== 0) {
                    $("#trainingCode").val('').focus();
                    $("#trainingCodeForm").removeClass().addClass("form-group has-error");
                ...
                $("#trainingForm").removeClass().addClass("form-group has-error");
            }
            if ($infoPanel.is(':hidden')) {
                $infoPanel.show();
            }
...
function showFormUpdate(leafId, url, urlList, securityToken, trainingId, updateAccess, deleteAccess) {
    sleep(500);
...
function showModalDelete(trainingId, trainingTypeId, trainingTitle, trainingLocation, trainingDescription, trainingDate) {
    $('#trainingIdPreview').val('').val(decodeURIComponent(trainingId));
    $('#trainingTypeIdPreview').val('').val(decodeURIComponent(trainingTypeId));
    $('#trainingTitlePreview').val('').val(decodeURIComponent(trainingTitle));
    $('#trainingLocationPreview').val('').val(decodeURIComponent(trainingLocation));
    $('#trainingDescriptionPreview').val('').val(decodeURIComponent(trainingDescription));
    $('#trainingDatePreview').val('').val(decodeURIComponent(trainingDate));
...
function deleteGridRecord(leafId, url, urlList, securityToken) {
    $.ajax({
        ...
        data: {
            method: 'delete',
            output: 'json',
            trainingId: $('#trainingIdPreview').val(),
            securityToken: securityToken,
            leafId: leafId
        },
        beforeSend: function() {
...
function showFormCreateDetail(leafId, url, securityToken) {
    var $infoPanel = $('#infoPanel');
    if ($('#employeeId9999').val().length === 0) {
        $infoPanel.html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['employeeIdLabel'] + "</span>");
        $('#employeeId9999HelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['employeeIdLabel'] + "</span>");
        $('#employeeId9999Form').removeClass().addClass('form-group has-error');
        $('#employeeId9999').focus();
        return false;
    }
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        if (success === true) {
                            ...
                            $("#trainingId9999").prop("disabled", "false").removeAttr("disabled", "").val('').trigger("chosen:updated");
                            $("#employeeId9999").prop("disabled", "false").removeAttr("disabled", "").val('').trigger("chosen:updated");
                            $(".chzn-select").chosen();
                            $infoPanel.html('').empty().html("<span class='label label-success'>&nbsp;<img src='" + smileyLol + "'> " + decodeURIComponent(t['loadingCompleteTextLabel']) + "</span>").delay(1000).fadeOut();
                            if ($infoPanel.is(':hidden')) {
                                $infoPanel.show();
...
function showFormUpdateDetail(leafId, url, securityToken, trainingEmployeeId) {
    var $infoPanel = $('#infoPanel');
    if ($('#employeeId' + trainingEmployeeId).val().length === 0) {
        $infoPanel.html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + ' :' + leafTranslation['employeeIdLabel'] + "</span>");
        $('#employeeId' + trainingEmployeeId + 'HelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + ' :' + leafTranslation['employeeIdLabel'] + "</span>");
        $('#employeeId' + trainingEmployeeId).removeClass().addClass('form-group has-error');
        $('#employeeId' + trainingEmployeeId).focus();
        return false;
    }
    $.ajax({
        type: 'POST',
        ...
        data: {
            method: 'save',
            output: 'json',
            trainingEmployeeId: $('#trainingEmployeeId' + trainingEmployeeId).val(),
            trainingId: $('#trainingId').val(),
            employeeId: $('#employeeId' + trainingEmployeeId).val(),
            securityToken: securityToken,
            leafId: leafId
        },
        beforeSend: function() {
            ...
            var $infoPanel = $('#infoPanel');
            $infoPanel.html('').empty().html("<span class='label label-warning'>&nbsp;<img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
            $('#miniInfoPanel' + trainingEmployeeId).html('').empty().html("<span class='label label-warning'> <img src='" + smileyRoll + "'> " + decodeURIComponent(t['loadingTextLabel']) + "</span>");
            if ($infoPanel.is(':hidden')) {
                $infoPanel.show();
            }
        },
        success: function(data) {
            var $infoPanel = $('#infoPanel');
            var $miniInfoPanel = $('#miniInfoPanel' + trainingEmployeeId);
            var smileyLol = './images/icons/smiley-lol.png';
            var success = data.success;
            var message = data.message;
            if (success === true) {
...
function showModalDeleteDetail(trainingEmployeeId) {
    $('#trainingEmployeeIdPreview').val('').val(decodeURIComponent($("#trainingEmployeeId" + trainingEmployeeId).val()));
    $('#employeeIdPreview').val('').val(decodeURIComponent($("#employeeId" + trainingEmployeeId + " option:selected").text()));
...
function deleteGridRecordDetail(leafId, url, urlList, securityToken) {
    $.ajax({
        ...
        data: {
            method: 'delete',
            output: 'json',
            trainingEmployeeId: $('#trainingEmployeeIdPreview').val(),
            securityToken: securityToken,
            leafId: leafId
        },
        beforeSend: function() {
        ...
        success: function(data) {
            ...
            if (success === true) {
                ...
                removeMeTr($('#trainingEmployeeIdPreview').val())
            } else if (success === false) {
                $infoPanel.html('').empty().html("<span class='label label-important'> " + message + "</span>");
                if ($infoPanel.is(':hidden')) {
                    $infoPanel.show();
...
function deleteGridRecordCheckbox(leafId, url, urlList, securityToken) {
    var stringText = '';
    var counter = 0;
    $('input:checkbox[name="trainingId[]"]').each(function() {
        stringText = stringText + "&trainingId[]=" + $(this).val();
    });
...
function newRecord(leafId, url, urlList, securityToken, type, createAccess, updateAccess, deleteAccess) {
    ...
    var $trainingTypeId = $('#trainingTypeId');
    var $trainingTitle = $('#trainingTitle');
    ...
    if (css.search('disabled') > 0) {
        ...
        if (type === 1) {
            if ($trainingTypeId.val().length === 0) {
                $('#trainingTypeIdHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['trainingTypeIdLabel'] + " </span>");
                $trainingTypeId.data('chosen').activate_action();
                return false;
            }
            if ($trainingTitle.val().length === 0) {
                $('#trainingTitleHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['trainingTitleLabel'] + " </span>");
                $('#trainingTitleForm').removeClass().addClass('form-group has-error');
                $trainingTitle.focus();
                return false;
            }
            if ($trainingLocation.val().length === 0) {
                $('#trainingLocationHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['trainingLocationLabel'] + " </span>");
                $('#trainingLocationForm').removeClass().addClass('form-group has-error');
                $trainingLocation.focus();
                return false;
            }
            if ($trainingDescription.val().length === 0) {
                $('#trainingDescriptionHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['trainingDescriptionLabel'] + " </span>");
                $('#trainingDescriptionForm').removeClass().addClass('form-group has-error');
                $trainingDescription.focus();
                return false;
            }
            if ($trainingDate.val().length === 0) {
                $('#trainingDateHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['trainingDateLabel'] + " </span>");
                $('#trainingDateForm').removeClass().addClass('form-group has-error');
                $trainingDate.focus();
                return false;
            }
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (success === true) {
                        ...
                        $trainingTypeId.val('');
                        $trainingTypeId.trigger("chosen:updated");
                        $('#trainingTypeIdHelpMe').html('').empty();
                        $trainingTitle.val('');
                        $('#trainingTitleHelpMe').html('').empty();
                        $trainingLocation.val('');
                        $('#trainingLocationHelpMe').html('').empty();
                        $trainingDescription.val('');
                        $('#trainingDescriptionForm').removeClass().addClass('form-group');
                        $('#trainingDescriptionHelpMe').html('').empty();
                        $trainingDate.val('');
                        $('#trainingDateHelpMe').html('').empty();
                    } else if (success === false) {
                        $infoPanel.html('').empty().html("<span class='label label-important'>&nbsp;" + message + "</span>");
                        if ($infoPanel.is(':hidden')) {
                            $infoPanel.show();
...
function resetRecord(leafId, url, urlList, urlTrainingEmployee, securityToken, createAccess, updateAccess, deleteAccess) {
    var $infoPanel = $('#infoPanel');
    ...
    $('#firstRecordButton').removeClass().addClass('btn btn-default').attr('onClick', "firstRecord(" + leafId + ",'" + url + "','" + urlList + "','" + urlTrainingEmployee + "','" + securityToken + "'," + updateAccess + "," + deleteAccess + ")");
    ...
    $('#endRecordButton').removeClass().addClass('btn btn-default').attr('onClick', "endRecord(" + leafId + ",'" + url + "','" + urlList + "','" + urlTrainingEmployee + "','" + securityToken + "'," + updateAccess + ")");
    $("#trainingId").val('');
    $("#trainingIdHelpMe").empty().html('');
    
    $("#trainingTypeId").val('');
    $("#trainingTypeIdHelpMe").empty().html('');
    $('#trainingTypeId').trigger("chosen:updated");
    $("#trainingTitle").val('');
    $("#trainingTitleHelpMe").empty().html('');
    $("#trainingLocation").val('');
    $("#trainingLocationHelpMe").empty().html('');
    $("#trainingDescription").val('');
    $("#trainingDescriptionHelpMe").empty().html('');
    $('#trainingDescription').empty().val('');
    $("#trainingDate").val('');
    $("#trainingDateHelpMe").empty().html('');
    $("#trainingEmployeeId9999").prop("disabled", "true").attr("disabled", "disabled").val('');
    $("#trainingId9999").prop("disabled", "true").attr("disabled", "disabled").val('').trigger("chosen:updated");
    $("#employeeId9999").prop("disabled", "true").attr("disabled", "disabled").val('').trigger("chosen:updated");
...
function firstRecord(leafId, url, urlList, urlTrainingEmployee, securityToken, updateAccess, deleteAccess) {
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if (success === true) {
                                ...
                                $('#trainingTypeId').val(data.data.trainingTypeId).trigger("chosen:updated");
                                $('#trainingTitle').val(data.data.trainingTitle);
                                $('#trainingLocation').val(data.data.trainingLocation);
                                $('#trainingDescription').val(data.data.trainingDescription);
                                x = data.data.trainingDate;
                                x = x.split("-");
                                output = x[2] + "-" + x[1] + "-" + x[0];
                                output = output.toString();
                                $('#trainingDate').val(output);
                                $("#trainingId9999").prop("disabled", "false").removeAttr("disabled", "").val('').trigger("chosen:updated");
                                $("#employeeId9999").prop("disabled", "false").removeAttr("disabled", "").val('').trigger("chosen:updated");

                                $.ajax({
                                    type: 'POST',
                                    url: urlTrainingEmployee,
                                    data: {
                                        method: 'read',
                                        trainingId: data.firstRecord,
                                        output: 'table',
                                        securityToken: securityToken,
                                        leafId: leafId
                                    },
                                ...
                                if (nextRecord > 0) {
                                    $('#previousRecordButton').removeClass().addClass('btn btn-default disabled').attr('onClick', '');
                                    $('#nextRecordButton').removeClass().addClass('btn btn-default').attr('onClick', '').attr('onClick', "nextRecord(" + leafId + ",'" + url + "','" + urlList + "','" + urlTrainingEmployee + "','" + securityToken + "'," + updateAccess + "," + deleteAccess + ")");
...
function endRecord(leafId, url, urlList, urlTrainingEmployee, securityToken, updateAccess, deleteAccess) {
    var $infoPanel = $('#infoPanel');
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if (success === true) {
                                ...
                                if (lastRecord !== 0) {
                                    $('#previousRecordButton').removeClass().addClass('btn btn-default').attr('onClick', "previousRecord(" + leafId + ",'" + url + "','" + urlList + "','" + urlTrainingEmployee + "','" + securityToken + "'," + updateAccess + "," + deleteAccess + ")");
                                    $('#nextRecordButton').removeClass().addClass('btn btn-default disabled').attr('onClick', '');
                                    $('#firstRecordCounter').val(firstRecord);
                                    $('#previousRecordCounter').val(previousRecord);
                                    $('#nextRecordCounter').val(nextRecord);
...
function previousRecord(leafId, url, urlList, urlTrainingEmployee, securityToken, updateAccess, deleteAccess) {
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($previousRecordCounter.val()) > 0 && parseFloat($previousRecordCounter.val()) < parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    trainingId: $previousRecordCounter.val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
...
function nextRecord(leafId, url, urlList, urlTrainingEmployee, securityToken, updateAccess, deleteAccess) {
    var $infoPanel = $('#infoPanel');
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($nextRecordCounter.val()) <= parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    trainingId: $nextRecordCounter.val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
