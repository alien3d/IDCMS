...
require_once($newFakeDocumentRoot . "v3/humanResource/training/controller/trainingTypeController.php");
...
$translator->setCurrentTable('trainingType');
...
if (isset($_POST)) {
    if (isset($_POST['method'])) {
        $trainingType = new \Core\HumanResource\Training\TrainingType\Controller\TrainingTypeClass();
        define('LIMIT', 10);
        ...
        if (isset($_POST ['query'])) {
            $trainingType->setFieldQuery($_POST ['query']);
        }
        if (isset($_POST ['filter'])) {
            $trainingType->setGridQuery($_POST ['filter']);
        }
        if (isset($_POST ['character'])) {
            $trainingType->setCharacterQuery($_POST['character']);
        }
        if (isset($_POST ['dateRangeStart'])) {
            $trainingType->setDateRangeStartQuery($_POST['dateRangeStart']);
            //explode the data to get day,month,year
            $start = explode('-', $_POST ['dateRangeStart']);
            $trainingType->setStartDay($start[2]);
            $trainingType->setStartMonth($start[1]);
            $trainingType->setStartYear($start[0]);
        }
        if (isset($_POST ['dateRangeEnd']) && (strlen($_POST['dateRangeEnd']) > 0)) {
            $trainingType->setDateRangeEndQuery($_POST['dateRangeEnd']);
            //explode the data to get day,month,year
            $start = explode('-', $_POST ['dateRangeEnd']);
            $trainingType->setEndDay($start[2]);
            $trainingType->setEndMonth($start[1]);
            $trainingType->setEndYear($start[0]);
        }
        if (isset($_POST ['dateRangeType'])) {
            $trainingType->setDateRangeTypeQuery($_POST['dateRangeType']);
        }
        if (isset($_POST ['dateRangeExtraType'])) {
            $trainingType->setDateRangeExtraTypeQuery($_POST['dateRangeExtraType']);
        }
        $trainingType->setServiceOutput('html');
        $trainingType->setLeafId($leafId);
        $trainingType->execute();
        if ($_POST['method'] == 'read') {
            $trainingType->setStart($offset);
            $trainingType->setLimit($limit); // normal system don't like paging..
            $trainingType->setPageOutput('html');
            $trainingTypeArray = $trainingType->read();
            if (isset($trainingTypeArray [0]['firstRecord'])) {
                $firstRecord = $trainingTypeArray [0]['firstRecord'];
            }
            if (isset($trainingTypeArray [0]['nextRecord'])) {
                $nextRecord = $trainingTypeArray [0]['nextRecord'];
            }
            if (isset($trainingTypeArray [0]['previousRecord'])) {
                $previousRecord = $trainingTypeArray [0]['previousRecord'];
            }
            if (isset($trainingTypeArray [0]['lastRecord'])) {
                $lastRecord = $trainingTypeArray [0]['lastRecord'];
                $endRecord = $trainingTypeArray [0]['lastRecord'];
            }
            $navigation = new \Core\Paging\HtmlPaging();
            $navigation->setLeafId($leafId);
            $navigation->setViewPath($trainingType->getViewPath());
            ...
            if (isset($trainingTypeArray [0]['total'])) {
                $total = $trainingTypeArray [0]['total'];
            } else {
                $total = 0;
            }
                        ...
                        <div class="btn-group">
                            ...
                            <ul class="dropdown-menu" style="text-align:left">
                                <li>
                                    ...
                                    echo $trainingType->getControllerPath();
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <form class="form-horizontal">
                                        <input type="hidden" name="trainingTypeIdPreview" id="trainingTypeIdPreview">

                                        <div class="form-group" id="trainingTypeDescriptionDiv">
                                            <label for="trainingTypeDescriptionPreview"
                                                   class="control-label col-xs-4 col-sm-4 col-md-4"><?php echo $leafTranslation['trainingTypeDescriptionLabel']; ?></label>

                                            <div class="col-xs-8 col-sm-8 col-md-8">
                                                <input class="form-control" type="text"
                                                       name="trainingTypeDescriptionPreview" id="trainingTypeDescriptionPreview">
                                            </div>
                                        </div>
                                    </form>
                                </div>
                        ...
                        <div class="col-xs-12 col-sm-12 col-md-12">
                            <div class="panel panel-default">
                                <table class="table table table-bordered table-striped table-condensed table-hover smart-form has-tickbox" id="tableData">
                                    ...
                                    <th><?php echo ucwords($leafTranslation['trainingTypeDescriptionLabel']); ?></th>
                                    <th width="100px">
                                            ...
                                            if (is_array($trainingTypeArray)) {
                                                ...
                                                if ($totalRecord > 0) {
                                                    ...
                                                    for ($i = 0; $i < $totalRecord; $i++) {
                                                        ...
                                                        ?>
                                                        <tr <?php
                                                        if ($trainingTypeArray[$i]['isDelete'] == 1) {
                                                            echo "class=\"danger\"";
                                                        } else {
                                                            if ($trainingTypeArray[$i]['isDraft'] == 1) {
                                                                echo "class=\"warning\"";
                                                            }
                                                        }
                                                        ?>>
                                                            ...
                                                            <td align="center">
                                                                <div class="btn-group" align="center">
                                                                    <button type="button"  class="btn btn-info btn-sm" title="Edit"
                                                                            ...
                                                                            echo intval(
                                                                                    $trainingTypeArray [$i]['trainingTypeId']
                                                                            );
                                                                    ...
                                                                    <button type="button"  class="btn btn-danger btn-sm" title="Delete"
                                                                            ...
                                                                            ?>', '<?php
                                                                            echo rawurlencode(
                                                                                    $trainingTypeArray [$i]['trainingTypeDescription']
                                                                            );
                                                                            ?>');">
                                                            ...
                                                            <td>
                                                                <div class="pull-left">
                                                                    <?php
                                                                    if (isset($trainingTypeArray[$i]['trainingTypeDescription'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos(
                                                                                    ...
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $trainingTypeArray[$i]['trainingTypeDescription']
                                                                                    );
                                                                                } else {
                                                                                    echo $trainingTypeArray[$i]['trainingTypeDescription'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen(
                                                                                    ...
                                                                                    if (strpos(
                                                                                        ...
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $trainingTypeArray[$i]['trainingTypeDescription']
                                                                                        );
                                                                                    } else {
                                                                                        echo $trainingTypeArray[$i]['trainingTypeDescription'];
                                                                                    }
                                                            ...
                                                            <td>
                                                                <div align="center">
                                                                    <?php
                                                                    if (isset($trainingTypeArray[$i]['executeBy'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos(
                                                                                                $trainingTypeArray[$i]['staffName'], $_POST['query']
                                                                                        ) !== false
                                                                                ) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $trainingTypeArray[$i]['staffName']
                                                                                    );
                                                                                } else {
                                                                                    echo $trainingTypeArray[$i]['staffName'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen(
                                                                                                $_POST['character']
                                                                                    ...
                                                                                    if (strpos(
                                                                                                    $trainingTypeArray[$i]['staffName'], $_POST['character']
                                                                                            ) !== false
                                                                                    ) {
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $trainingTypeArray[$i]['staffName']
                                                                                        );
                                                                                    } else {
                                                                                        echo $trainingTypeArray[$i]['staffName'];
                                                                                    }
                                                            ...
                                                            <?php
                                                            if (isset($trainingTypeArray[$i]['executeTime'])) {
                                                                $valueArray = $trainingTypeArray[$i]['executeTime'];
                                                            ...
                                                            <?php } ?>
                                                            <?php
                                                            if ($trainingTypeArray[$i]['isDelete']) {
                                                                $checked = "checked";
                                                            } else {
                                                                $checked = null;
                                                            }
                                                            ?>
                                                            <td>
                                                                <input style="display:none;" type="checkbox" name="trainingTypeId[]"
                                                                       value="<?php echo $trainingTypeArray[$i]['trainingTypeId']; ?>">
                                                                <input <?php echo $checked; ?> type="checkbox" name="isDelete[]"
                                                                                               value="<?php echo $trainingTypeArray[$i]['isDelete']; ?>">
                                                            </td>
                                                        </tr>
                                                        <?php
                                                    }
                        ...
                        <div class="col-xs-3 col-sm-3 col-md-3 pagination">
                            <div class="pull-right">
                                <button class="delete btn btn-warning" type="button" 
                                        ...
                                        echo $trainingType->getControllerPath();
                                        ?>', '<?php echo $trainingType->getViewPath(); ?>', '<?php echo $securityToken; ?>');">
...
if ((isset($_POST['method']) == 'new' || isset($_POST['method']) == 'read') && $_POST['type'] == 'form') {
            ...
            <div class="row">
                <div class="col-xs-12 col-sm-12 col-md-12">
                    <div class="panel panel-info">
                        ...
                        <div class="panel-body">
                            <input type="hidden" name="trainingTypeId" id="trainingTypeId" value="<?php
                            if (isset($_POST['trainingTypeId'])) {
                                echo $_POST['trainingTypeId'];
                            }
                            ?>">

                            <div class="row">
                                <div class="col-xs-12 col-sm-12 col-md-12">
                                    <div class="form-group" id="trainingTypeDescriptionForm">
                                        <label for="trainingTypeDescription"><strong><?php
                                                echo ucfirst(
                                                        $leafTranslation['trainingTypeDescriptionLabel']
                                                );
                                                ?></strong></label>

                                        <div class="col-xs-12 col-sm-12 col-md-12">
                                            <textarea class="form-control" rows="5" name="trainingTypeDescription"
                                                      id="trainingTypeDescription"
                                                      onKeyUp="removeMeError('trainingTypeDescription');"><?php
                                                          if (isset($trainingTypeArray[0]['trainingTypeDescription'])) {
                                                              echo htmlentities($trainingTypeArray[0]['trainingTypeDescription']);
                                                          }
                                                          ?></textarea> <span class="help-block" id="trainingTypeDescriptionHelpMe"></span>
                                        </div>
                                    </div>
                                </div>
                            </div>
    ...
    <?php if ($leafAccess['leafAccessCreateValue'] == 1) { ?>
                            newRecord(<?php echo $leafId; ?>, '<?php echo $trainingType->getControllerPath(); ?>', '<?php echo $trainingType->getViewPath(); ?>', '<?php echo $securityToken; ?>', 1);
    <?php } ?>
                        return false;
                    }
                    // shift+s save event
    ...
    <?php if ($leafAccess['leafAccessUpdateValue'] == 1) { ?>
                            updateRecord(<?php echo $leafId; ?>, '<?php echo $trainingType->getControllerPath(); ?>', '<?php echo $trainingType->getViewPath(); ?>', '<?php echo $securityToken; ?>', 1, <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
    <?php } ?>
                        return false;
                    }
    ...
    <?php if ($leafAccess['leafAccessDeleteValue'] == 1) { ?>
                            deleteRecord(<?php echo $leafId; ?>, '<?php echo $trainingType->getControllerPath(); ?>', '<?php echo $trainingType->getViewPath(); ?>', '<?php echo $securityToken; ?>', <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
    ...
    <?php } ?>
                    ...
                    switch (e.keyCode) {
                        case 37:
                            previousRecord(<?php echo $leafId; ?>, '<?php echo $trainingType->getControllerPath(); ?>', '<?php echo $trainingType->getViewPath(); ?>', '<?php echo $securityToken; ?>', <?php echo $leafAccess['leafAccessUpdateValue']; ?>, <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
                            
                            return false;
                            break;
                        case 39:
                            nextRecord(<?php echo $leafId; ?>, '<?php echo $trainingType->getControllerPath(); ?>', '<?php echo $trainingType->getViewPath(); ?>', '<?php echo $securityToken; ?>', <?php echo $leafAccess['leafAccessUpdateValue']; ?>, <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
    ...
    <?php if ($_POST['method'] == "new") { ?>
        ...
        <?php if ($leafAccess['leafAccessCreateValue'] == 1) { ?>
                            $('#newRecordButton1').removeClass().addClass('btn btn-success').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $trainingType->getControllerPath(); ?>','<?php echo $trainingType->getViewPath(); ?>','<?php echo $securityToken; ?>',1)");
                            $('#newRecordButton2').removeClass().addClass('btn dropdown-toggle btn-success');
                            $('#newRecordButton3').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $trainingType->getControllerPath(); ?>','<?php echo $trainingType->getViewPath(); ?>','<?php echo $securityToken; ?>',1)");
                            $('#newRecordButton4').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $trainingType->getControllerPath(); ?>','<?php echo $trainingType->getViewPath(); ?>','<?php echo $securityToken; ?>',2)");
                            $('#newRecordButton5').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $trainingType->getControllerPath(); ?>','<?php echo $trainingType->getViewPath(); ?>','<?php echo $securityToken; ?>',3)");
                            $('#newRecordButton6').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $trainingType->getControllerPath(); ?>','<?php echo $trainingType->getViewPath(); ?>','<?php echo $securityToken; ?>',4)");
                            $('#newRecordButton7').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $trainingType->getControllerPath(); ?>','<?php echo $trainingType->getViewPath(); ?>','<?php echo $securityToken; ?>',5)");
        <?php } else { ?>
        ...
        if ($_POST['trainingTypeId']) {
            ...
            <?php if ($leafAccess['leafAccessUpdateValue'] == 1) { ?>
                                $('#updateRecordButton1').removeClass().addClass('btn btn-info').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $trainingType->getControllerPath(); ?>','<?php echo $trainingType->getViewPath(); ?>','<?php echo $securityToken; ?>',1,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                                $('#updateRecordButton2').removeClass().addClass('btn dropdown-toggle btn-info');
                                $('#updateRecordButton3').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $trainingType->getControllerPath(); ?>','<?php echo $trainingType->getViewPath(); ?>','<?php echo $securityToken; ?>',1,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                                $('#updateRecordButton4').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $trainingType->getControllerPath(); ?>','<?php echo $trainingType->getViewPath(); ?>','<?php echo $securityToken; ?>',2,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                                $('#updateRecordButton5').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $trainingType->getControllerPath(); ?>','<?php echo $trainingType->getViewPath(); ?>','<?php echo $securityToken; ?>',3,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
            <?php } else { ?>
                                $('#updateRecordButton1').removeClass().addClass(' btn btn-info disabled').attr('onClick', '');
                                $('#updateRecordButton2').removeClass().addClass('btn dropdown-toggle btn-info disabled');
                                $('#updateRecordButton3').attr('onClick', '');
            ...
            <?php } ?>
            <?php if ($leafAccess['leafAccessDeleteValue'] == 1) { ?>
                                $('#deleteRecordButton').removeClass().addClass('btn btn-danger').attr('onClick', "deleteRecord(<?php echo $leafId; ?>,'<?php echo $trainingType->getControllerPath(); ?>','<?php echo $trainingType->getViewPath(); ?>','<?php echo $securityToken; ?>',<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
            <?php } else { ?>
                                $('#deleteRecordButton').removeClass().addClass('btn btn-danger disabled').attr('onClick', '');
            <?php } ?>
            <?php
...
<?php } ?>
<script type="text/javascript" src="./v3/humanResource/training/javascript/trainingType.js"></script>
<hr>
