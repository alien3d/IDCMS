...
require_once($newFakeDocumentRoot . "v3/humanResource/leave/controller/leaveHolidaysController.php");
...
$translator->setCurrentTable('leaveHolidays');
...
$leaveHolidaysArray = array();
if (isset($_POST)) {
    if (isset($_POST['method'])) {
        $leaveHolidays = new \Core\HumanResource\Leave\LeaveHolidays\Controller\LeaveHolidaysClass();
        define('LIMIT', 10);
        ...
        if (isset($_POST ['query'])) {
            $leaveHolidays->setFieldQuery($_POST ['query']);
        }
        if (isset($_POST ['filter'])) {
            $leaveHolidays->setGridQuery($_POST ['filter']);
        }
        if (isset($_POST ['character'])) {
            $leaveHolidays->setCharacterQuery($_POST['character']);
        }
        if (isset($_POST ['dateRangeStart'])) {
            $leaveHolidays->setDateRangeStartQuery($_POST['dateRangeStart']);
            //explode the data to get day,month,year
            $start = explode('-', $_POST ['dateRangeStart']);
            $leaveHolidays->setStartDay($start[2]);
            $leaveHolidays->setStartMonth($start[1]);
            $leaveHolidays->setStartYear($start[0]);
        }
        if (isset($_POST ['dateRangeEnd']) && (strlen($_POST['dateRangeEnd']) > 0)) {
            $leaveHolidays->setDateRangeEndQuery($_POST['dateRangeEnd']);
            //explode the data to get day,month,year
            $start = explode('-', $_POST ['dateRangeEnd']);
            $leaveHolidays->setEndDay($start[2]);
            $leaveHolidays->setEndMonth($start[1]);
            $leaveHolidays->setEndYear($start[0]);
        }
        if (isset($_POST ['dateRangeType'])) {
            $leaveHolidays->setDateRangeTypeQuery($_POST['dateRangeType']);
        }
        if (isset($_POST ['dateRangeExtraType'])) {
            $leaveHolidays->setDateRangeExtraTypeQuery($_POST['dateRangeExtraType']);
        }
        $leaveHolidays->setServiceOutput('html');
        $leaveHolidays->setLeafId($leafId);
        $leaveHolidays->execute();
        if ($_POST['method'] == 'read') {
            $leaveHolidays->setStart($offset);
            $leaveHolidays->setLimit($limit); // normal system don't like paging..
            $leaveHolidays->setPageOutput('html');
            $leaveHolidaysArray = $leaveHolidays->read();
            if (isset($leaveHolidaysArray [0]['firstRecord'])) {
                $firstRecord = $leaveHolidaysArray [0]['firstRecord'];
            }
            if (isset($leaveHolidaysArray [0]['nextRecord'])) {
                $nextRecord = $leaveHolidaysArray [0]['nextRecord'];
            }
            if (isset($leaveHolidaysArray [0]['previousRecord'])) {
                $previousRecord = $leaveHolidaysArray [0]['previousRecord'];
            }
            if (isset($leaveHolidaysArray [0]['lastRecord'])) {
                $lastRecord = $leaveHolidaysArray [0]['lastRecord'];
                $endRecord = $leaveHolidaysArray [0]['lastRecord'];
            }
            $navigation = new \Core\Paging\HtmlPaging();
            $navigation->setLeafId($leafId);
            $navigation->setViewPath($leaveHolidays->getViewPath());
            ...
            if (isset($leaveHolidaysArray [0]['total'])) {
                $total = $leaveHolidaysArray [0]['total'];
            } else {
                $total = 0;
            }
            ...
            <div class="row">
                <div class="pull-left btn-group col-xs-10 col-sm-10 col-md-10">
                    <button title="A" class="btn btn-success btn-sm" type="button" 
                            ...
                            echo $leaveHolidays->getViewPath();
                        ...
                        <div class="btn-group">
                            ...
                            <ul class="dropdown-menu" style="text-align:left">
                                <li>
                                    ...
                                    echo $leaveHolidays->getControllerPath();
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <form class="form-horizontal">
                                        <input type="hidden" name="leaveHolidaysIdPreview" id="leaveHolidaysIdPreview">

                                        <div class="form-group" id="leaveHolidaysDateDiv">
                                            <label for="leaveHolidaysDatePreview"
                                                   class="control-label col-xs-4 col-sm-4 col-md-4"><?php echo $leafTranslation['leaveHolidaysDateLabel']; ?></label>

                                            <div class="col-xs-8 col-sm-8 col-md-8">
                                                <input class="form-control" type="text" name="leaveHolidaysDatePreview"
                                                       id="leaveHolidaysDatePreview">
                                            </div>
                                        </div>
                                        <div class="form-group" id="leaveHolidaysDescriptionDiv">
                                            <label for="leaveHolidaysDescriptionPreview"
                                                   class="control-label col-xs-4 col-sm-4 col-md-4"><?php echo $leafTranslation['leaveHolidaysDescriptionLabel']; ?></label>

                                            <div class="col-xs-8 col-sm-8 col-md-8">
                                                <input class="form-control" type="text"
                                                       name="leaveHolidaysDescriptionPreview" id="leaveHolidaysDescriptionPreview">
                                            </div>
                                        </div>
                                    </form>
                                </div>
                        ...
                        <div class="col-xs-12 col-sm-12 col-md-12">
                            <div class="panel panel-default">
                                <table class="table table table-bordered table-striped table-condensed table-hover smart-form has-tickbox" id="tableData">
                                    ...
                                    <div align="center"><?php echo ucwords($leafTranslation['leaveHolidaysDateLabel']); ?></div>
                                    </th>
                                    <th><?php echo ucwords($leafTranslation['leaveHolidaysDescriptionLabel']); ?></th>
                                    <th width="100px">
                                    ...
                                    <tbody id="tableBody">
                                        ...
                                        if ($_POST['method'] == 'read' && $_POST['type'] == 'list' && $_POST['detail'] == 'body') {
                                            if (is_array($leaveHolidaysArray)) {
                                                $totalRecord = intval(count($leaveHolidaysArray));
                                                if ($totalRecord > 0) {
                                                    $counter = 0;
                                                    for ($i = 0; $i < $totalRecord; $i++) {
                                                        ...
                                                        ?>
                                                        <tr <?php
                                                        if ($leaveHolidaysArray[$i]['isDelete'] == 1) {
                                                            echo "class=\"danger\"";
                                                        } else {
                                                            if ($leaveHolidaysArray[$i]['isDraft'] == 1) {
                                                                echo "class=\"warning\"";
                                                            }
                                                        }
                                                        ?>>
                                                            ...
                                                            <td align="center">
                                                                <div class="btn-group" align="center">
                                                                    <button type="button"  class="btn btn-info btn-sm" title="Edit"
                                                                            ...
                                                                            echo intval(
                                                                                    $leaveHolidaysArray [$i]['leaveHolidaysId']
                                                                            );
                                                                    ...
                                                                    <button type="button"  class="btn btn-danger btn-sm" title="Delete"
                                                                            ...
                                                                            ?>', '<?php
                                                                            echo rawurlencode(
                                                                                    $leaveHolidaysArray [$i]['leaveHolidaysDate']
                                                                            );
                                                                            ?>', '<?php
                                                                            echo rawurlencode(
                                                                                    $leaveHolidaysArray [$i]['leaveHolidaysDescription']
                                                                            );
                                                                            ?>');">
                                                            ...
                                                            <?php
                                                            if (isset($leaveHolidaysArray[$i]['leaveHolidaysDate'])) {
                                                                $valueArray = $leaveHolidaysArray[$i]['leaveHolidaysDate'];
                                                            ...
                                                            <td>
                                                                <div class="pull-left">
                                                                    <?php
                                                                    if (isset($leaveHolidaysArray[$i]['leaveHolidaysDescription'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos(
                                                                                    ...
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $leaveHolidaysArray[$i]['leaveHolidaysDescription']
                                                                                    );
                                                                                } else {
                                                                                    echo $leaveHolidaysArray[$i]['leaveHolidaysDescription'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen(
                                                                                    ...
                                                                                    if (strpos(
                                                                                        ...
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $leaveHolidaysArray[$i]['leaveHolidaysDescription']
                                                                                        );
                                                                                    } else {
                                                                                        echo $leaveHolidaysArray[$i]['leaveHolidaysDescription'];
                                                                                    }
                                                            ...
                                                            <td align="center">
                                                                <div class="center">
                                                                    <?php
                                                                    if (isset($leaveHolidaysArray[$i]['executeBy'])) {
                                                                        if (isset($_POST['query']) || isset($_POST['character'])) {
                                                                            if (isset($_POST['query']) && strlen($_POST['query']) > 0) {
                                                                                if (strpos(
                                                                                                $leaveHolidaysArray[$i]['staffName'], $_POST['query']
                                                                                        ) !== false
                                                                                ) {
                                                                                    echo str_replace(
                                                                                            $_POST['query'], "<span class=\"label label-info\">" . $_POST['query'] . "</span>", $leaveHolidaysArray[$i]['staffName']
                                                                                    );
                                                                                } else {
                                                                                    echo $leaveHolidaysArray[$i]['staffName'];
                                                                                }
                                                                            } else {
                                                                                if (isset($_POST['character']) && strlen(
                                                                                                $_POST['character']
                                                                                    ...
                                                                                    if (strpos(
                                                                                                    $leaveHolidaysArray[$i]['staffName'], $_POST['character']
                                                                                            ) !== false
                                                                                    ) {
                                                                                        echo str_replace(
                                                                                                $_POST['character'], "<span class=\"label label-info\">" . $_POST['character'] . "</span>", $leaveHolidaysArray[$i]['staffName']
                                                                                        );
                                                                                    } else {
                                                                                        echo $leaveHolidaysArray[$i]['staffName'];
                                                                                    }
                                                            ...
                                                            <?php
                                                            if (isset($leaveHolidaysArray[$i]['executeTime'])) {
                                                                $valueArray = $leaveHolidaysArray[$i]['executeTime'];
                                                            ...
                                                            <?php } ?>
                                                            <?php
                                                            if ($leaveHolidaysArray[$i]['isDelete']) {
                                                                $checked = "checked";
                                                            } else {
                                                                $checked = null;
                                                            }
                                                            ?>
                                                            <td>
                                                                <input style="display:none;" type="checkbox" name="leaveHolidaysId[]"
                                                                       value="<?php echo $leaveHolidaysArray[$i]['leaveHolidaysId']; ?>">
                                                                <input <?php echo $checked; ?> type="checkbox" name="isDelete[]"
                                                                                               value="<?php echo $leaveHolidaysArray[$i]['isDelete']; ?>">
                                                            </td>
                                                        </tr>
                                                        <?php
                                                    }
                                                    ...
                                                    <tr>
                                                        <td colspan="7" vAlign="top" align="center"><?php
                                                            $leaveHolidays->exceptionMessage(
                        ...
                        <div class="col-xs-3 col-sm-3 col-md-3 pagination">
                            <div class="pull-right">
                                <button class="delete btn btn-warning" type="button" 
                                        ...
                                        echo $leaveHolidays->getControllerPath();
                                        ?>', '<?php echo $leaveHolidays->getViewPath(); ?>', '<?php echo $securityToken; ?>');">
...
if ((isset($_POST['method']) == 'new' || isset($_POST['method']) == 'read') && $_POST['type'] == 'form') {
            ...
            <div class="row">
                <div class="col-xs-12 col-sm-12 col-md-12">
                    <div class="panel panel-info">
                        ...
                        <div class="panel-body">
                            <input type="hidden" name="leaveHolidaysId" id="leaveHolidaysId" value="<?php
                            if (isset($_POST['leaveHolidaysId'])) {
                                echo $_POST['leaveHolidaysId'];
                            }
                            ?>">
                            ...
                            <div class="row">
                                <div class="col-xs-12 col-sm-12 col-md-12">
                                    <?php
                                    if (isset($leaveHolidaysArray) && is_array($leaveHolidaysArray)) {

                                        $valueArray = $leaveHolidaysArray[0]['leaveHolidaysDate'];
                                        if ($dateConvert->checkDateTime($valueArray)) {
                                            $valueData = explode('-', $valueArray);
                                            $year = $valueData[0];
                                            $month = $valueData[1];
                                    ...
                                    ?>
                                    <div class="col-xs-6 col-sm-6 col-md-6 form-group" id="leaveHolidaysDateForm">
                                        <label for="leaveHolidaysDate" class="control-label col-xs-4 col-sm-4 col-md-4"><strong><?php
                                                echo ucfirst(
                                                        $leafTranslation['leaveHolidaysDateLabel']
                                                );
                                                ?></strong></label>

                                        <div class="col-xs-8 col-sm-8 col-md-8">
                                            <div class="input-group">
                                                <input type="text" class="form-control" name="leaveHolidaysDate"
                                                       id="leaveHolidaysDate" value="<?php
                                                       if (isset($value)) {
                                                           echo $value;
                                                       }
                                                       ?>"><span class="input-group-addon"><img src="./images/icons/calendar.png"
                                                                                       id="leaveHolidaysDateImage"></span>
                                            </div>
                                            <span class="help-block" id="leaveHolidaysDateHelpMe"></span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-xs-12 col-sm-12 col-md-12">
                                    <div class="form-group" id="leaveHolidaysDescriptionForm">
                                        <div class="col-xs-12 col-sm-12 col-md-12">
                                            <textarea class="form-control" name="leaveHolidaysDescription"
                                                      id="leaveHolidaysDescription" rows="5"><?php
                                                          if (isset($leaveHolidaysArray[0]['leaveHolidaysDescription'])) {
                                                              echo htmlentities($leaveHolidaysArray[0]['leaveHolidaysDescription']);
                                                          }
                                                          ?></textarea> <span class="help-block" id="leaveHolidaysDescriptionHelpMe"></span>
                                        </div>
                                    </div>
                                </div>
                            </div>
    ...
    <?php if ($leafAccess['leafAccessCreateValue'] == 1) { ?>
                        ...
                        if (e.which === 78 && e.which === 18  && e.shiftKey) {
                            ...
                            newRecord(<?php echo $leafId; ?>, '<?php echo $leaveHolidays->getControllerPath(); ?>', '<?php echo $leaveHolidays->getViewPath(); ?>', '<?php echo $securityToken; ?>', 1);
    ...
    <?php if ($leafAccess['leafAccessUpdateValue'] == 1) { ?>
                        ...
                        if (e.which === 83 && e.which === 18  && e.shiftKey) {
                            ...
                            updateRecord(<?php echo $leafId; ?>, '<?php echo $leaveHolidays->getControllerPath(); ?>', '<?php echo $leaveHolidays->getViewPath(); ?>', '<?php echo $securityToken; ?>', 1, <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
    ...
    <?php if ($leafAccess['leafAccessDeleteValue'] == 1) { ?>
                        ...
                        if (e.which === 88 && e.which === 18 && e.shiftKey) {
                            ...
                            deleteRecord(<?php echo $leafId; ?>, '<?php echo $leaveHolidays->getControllerPath(); ?>', '<?php echo $leaveHolidays->getViewPath(); ?>', '<?php echo $securityToken; ?>', <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
    ...
    <?php } ?>
                    switch (e.keyCode) {
                        case 37:
                            previousRecord(<?php echo $leafId; ?>, '<?php echo $leaveHolidays->getControllerPath(); ?>', '<?php echo $leaveHolidays->getViewPath(); ?>', '<?php echo $securityToken; ?>', <?php echo $leafAccess['leafAccessUpdateValue']; ?>, <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
                            
                            return false;
                            break;
                        case 39:
                            nextRecord(<?php echo $leafId; ?>, '<?php echo $leaveHolidays->getControllerPath(); ?>', '<?php echo $leaveHolidays->getViewPath(); ?>', '<?php echo $securityToken; ?>', <?php echo $leafAccess['leafAccessUpdateValue']; ?>, <?php echo $leafAccess['leafAccessDeleteValue']; ?>);
                ...
                $(document).ready(function() {
                    ...
                    validateMeNumeric('leaveHolidaysId');
                    var a = $('#leaveHolidaysDate').datepicker({
                        format: "<?php echo $systemFormat['systemSettingDateFormat']; ?>"
                    }).on('changeDate', function() {
                        $(this).datepicker('hide');
                    });
                    $("#leaveHolidaysDate").on('click', function() {
                        a.datepicker('show');
                    });
    <?php if ($_POST['method'] == "new") { ?>
                        $('#resetRecordButton').removeClass().addClass('btn btn-default');
        <?php if ($leafAccess['leafAccessCreateValue'] == 1) { ?>
                            $('#newRecordButton1').removeClass().addClass('btn btn-success').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $leaveHolidays->getControllerPath(); ?>','<?php echo $leaveHolidays->getViewPath(); ?>','<?php echo $securityToken; ?>',1)");
                            $('#newRecordButton2').removeClass().addClass('btn dropdown-toggle btn-success');
                            $('#newRecordButton3').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $leaveHolidays->getControllerPath(); ?>','<?php echo $leaveHolidays->getViewPath(); ?>','<?php echo $securityToken; ?>',1)");
                            $('#newRecordButton4').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $leaveHolidays->getControllerPath(); ?>','<?php echo $leaveHolidays->getViewPath(); ?>','<?php echo $securityToken; ?>',2)");
                            $('#newRecordButton5').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $leaveHolidays->getControllerPath(); ?>','<?php echo $leaveHolidays->getViewPath(); ?>','<?php echo $securityToken; ?>',3)");
                            $('#newRecordButton6').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $leaveHolidays->getControllerPath(); ?>','<?php echo $leaveHolidays->getViewPath(); ?>','<?php echo $securityToken; ?>',4)");
                            $('#newRecordButton7').attr('onClick', "newRecord(<?php echo $leafId; ?>,'<?php echo $leaveHolidays->getControllerPath(); ?>','<?php echo $leaveHolidays->getViewPath(); ?>','<?php echo $securityToken; ?>',5)");
        <?php } else { ?>
        ...
        if ($_POST['leaveHolidaysId']) {
            ...
            <?php if ($leafAccess['leafAccessUpdateValue'] == 1) { ?>
                                $('#updateRecordButton1').removeClass().addClass('btn btn-info').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $leaveHolidays->getControllerPath(); ?>','<?php echo $leaveHolidays->getViewPath(); ?>','<?php echo $securityToken; ?>',1,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                                $('#updateRecordButton2').removeClass().addClass('btn dropdown-toggle btn-info');
                                $('#updateRecordButton3').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $leaveHolidays->getControllerPath(); ?>','<?php echo $leaveHolidays->getViewPath(); ?>','<?php echo $securityToken; ?>',1,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                                $('#updateRecordButton4').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $leaveHolidays->getControllerPath(); ?>','<?php echo $leaveHolidays->getViewPath(); ?>','<?php echo $securityToken; ?>',2,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
                                $('#updateRecordButton5').attr('onClick', "updateRecord(<?php echo $leafId; ?>,'<?php echo $leaveHolidays->getControllerPath(); ?>','<?php echo $leaveHolidays->getViewPath(); ?>','<?php echo $securityToken; ?>',3,<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
            <?php } else { ?>
                                $('#updateRecordButton1').removeClass().addClass(' btn btn-info disabled').attr('onClick', '');
                                $('#updateRecordButton2').removeClass().addClass('btn dropdown-toggle btn-info disabled');
                                $('#updateRecordButton3').attr('onClick', '');
            ...
            <?php } ?>
            <?php if ($leafAccess['leafAccessDeleteValue'] == 1) { ?>
                                $('#deleteRecordButton').removeClass().addClass('btn btn-danger').attr('onClick', "deleteRecord(<?php echo $leafId; ?>,'<?php echo $leaveHolidays->getControllerPath(); ?>','<?php echo $leaveHolidays->getViewPath(); ?>','<?php echo $securityToken; ?>',<?php echo $leafAccess['leafAccessDeleteValue']; ?>);");
            <?php } else { ?>
                                $('#deleteRecordButton').removeClass().addClass('btn btn-danger disabled').attr('onClick', '');
            <?php } ?>
            <?php
...
<?php } ?>
<script type="text/javascript" src="./v3/humanResource/leave/javascript/leaveHolidays.js"></script>
<hr>
