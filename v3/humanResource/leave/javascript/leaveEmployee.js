function checkDuplicate(leafId, page, securityToken) {
    var $leaveEmployeeCode = $("#leaveEmployeeCode");
    if ($leaveEmployeeCode.val().length === 0) {
    ...
    $.ajax({
        ...
        url: page,
        data: {
            leaveEmployeeCode: $leaveEmployeeCode.val(),
            method: 'duplicate',
        ...
        success: function(data) {
            ...
            if (success === true) {
                if (total !== 0) {
                    $("#leaveEmployeeCode").val('').focus();
                    $("#leaveEmployeeCodeForm").removeClass().addClass("form-group has-error");
                ...
                $("#leaveEmployeeForm").removeClass().addClass("form-group has-error");
            }
            if ($infoPanel.is(':hidden')) {
                $infoPanel.show();
            }
...
function showFormUpdate(leafId, url, urlList, securityToken, leaveEmployeeId, updateAccess, deleteAccess) {
    sleep(500);
...
function showModalDelete(leaveEmployeeId, leaveTypeId, employeeId, leaveEmployeeYear, leaveEmployeeBalance) {
    $('#leaveEmployeeIdPreview').val('').val(decodeURIComponent(leaveEmployeeId));
    $('#leaveTypeIdPreview').val('').val(decodeURIComponent(leaveTypeId));
    $('#employeeIdPreview').val('').val(decodeURIComponent(employeeId));
    $('#leaveEmployeeYearPreview').val('').val(decodeURIComponent(leaveEmployeeYear));
    $('#leaveEmployeeBalancePreview').val('').val(decodeURIComponent(leaveEmployeeBalance));
...
function deleteGridRecord(leafId, url, urlList, securityToken) {
    $.ajax({
        ...
        data: {
            method: 'delete',
            output: 'json',
            leaveEmployeeId: $('#leaveEmployeeIdPreview').val(),
            securityToken: securityToken,
            leafId: leafId
        },
        beforeSend: function() {
...
function deleteGridRecordCheckbox(leafId, url, urlList, securityToken) {
    var stringText = '';
    var counter = 0;
    $('input:checkbox[name="leaveEmployeeId[]"]').each(function() {
        stringText = stringText + "&leaveEmployeeId[]=" + $(this).val();
    });
...
function newRecord(leafId, url, urlList, securityToken, type, createAccess, updateAccess, deleteAccess) {
    ...
    var $leaveEmployeeYear = $('#leaveEmployeeYear');
    var $leaveEmployeeBalance = $('#leaveEmployeeBalance');
    if (css.search('disabled') > 0) {
        return false;
    } else {
        if (type === 1) {
            if ($leaveTypeId.val().length === 0) {
                ...
                $('#leaveTypeIdForm').removeClass().addClass('form-group has-error');
                $leaveTypeId.focus();
                return false;
            }
            if ($employeeId.val().length === 0) {
                ...
                $('#employeeIdForm').removeClass().addClass('form-group has-error');
                $employeeId.focus();
                return false;
            }
            if ($leaveEmployeeYear.val().length === 0) {
                $('#leaveEmployeeYearHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['leaveEmployeeYearLabel'] + " </span>");
                $('#leaveEmployeeYearForm').removeClass().addClass('form-group has-error');
                $leaveEmployeeYear.focus();
                return false;
            }
            if ($leaveEmployeeBalance.val().length === 0) {
                $('#leaveEmployeeBalanceHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['leaveEmployeeBalanceLabel'] + " </span>");
                $('#leaveEmployeeBalanceForm').removeClass().addClass('form-group has-error');
                $leaveEmployeeBalance.focus();
                return false;
            }
            $.ajax({
                type: 'POST',
                ...
                data: {
                    ...
                    leaveEmployeeYear: $leaveEmployeeYear.val(),
                    leaveEmployeeBalance: $leaveEmployeeBalance.val(),
                    securityToken: securityToken,
                    leafId: leafId
                },
                beforeSend: function() {
                ...
                success: function(data) {
                    ...
                    if (success === true) {
                        ...
                        $leaveEmployeeYear.val('');
                        $('#leaveEmployeeYearHelpMe').html('').empty();
                        $leaveEmployeeBalance.val('');
                        $('#leaveEmployeeBalanceHelpMe').html('').empty();
                    } else if (success === false) {
                        $infoPanel.html('').empty().html("<span class='label label-important'>&nbsp;" + message + "</span>");
                        if ($infoPanel.is(':hidden')) {
                            $infoPanel.show();
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (success === true) {
                        ...
                        $('#leaveEmployeeId').val(data.leaveEmployeeId);
                        $('#newRecordButton1').removeClass().addClass('btn btn-success disabled').attr('onClick', '');
                        $('#newRecordButton2').removeClass().addClass('btn dropdown-toggle btn-success disabled');
                        $('#newRecordButton3').attr('onClick', '');
                        $('#newRecordButton4').attr('onClick', '');
        ...
        if (type === 1) {
            ...
            $.ajax({
                ...
                data: {
                    method: 'save',
                    output: 'json',
                    leaveEmployeeId: $leaveEmployeeId.val(),
                    leaveTypeId: $leaveTypeId.val(),
                    employeeId: $employeeId.val(),
                    leaveEmployeeYear: $leaveEmployeeYear.val(),
                    leaveEmployeeBalance: $leaveEmployeeBalance.val(),
...
function deleteRecord(leafId, url, urlList, securityToken, deleteAccess) {
    var $infoPanel = $('#infoPanel');
    var $leaveEmployeeId = $('#leaveEmployeeId');
...
function resetRecord(leafId, url, urlList, securityToken, createAccess, updateAccess, deleteAccess) {
    ...
    $("#leaveEmployeeId").val('');
    $("#leaveEmployeeIdHelpMe").empty().html('');
    ...
    $('#employeeId').trigger("chosen:updated");
    $("#leaveEmployeeYear").val('');
    $("#leaveEmployeeYearHelpMe").empty().html('');
    $("#leaveEmployeeBalance").val('');
    $("#leaveEmployeeBalanceHelpMe").empty().html('');
}
function postRecord() {
                ...
                if (success === true) {
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if (success === true) {
                                $('#leaveEmployeeId').val(data.data.leaveEmployeeId);
                                $('#leaveTypeId').val(data.data.leaveTypeId).trigger("chosen:updated");
                                $('#employeeId').val(data.data.employeeId).trigger("chosen:updated");
                                $('#leaveEmployeeYear').val(data.data.leaveEmployeeYear);
                                $('#leaveEmployeeBalance').val(data.data.leaveEmployeeBalance);
                                if (nextRecord > 0) {
                                    $('#previousRecordButton').removeClass().addClass('btn btn-default disabled').attr('onClick', '');
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($previousRecordCounter.val()) > 0 && parseFloat($previousRecordCounter.val()) < parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    leaveEmployeeId: $previousRecordCounter.val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($nextRecordCounter.val()) <= parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    leaveEmployeeId: $nextRecordCounter.val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
