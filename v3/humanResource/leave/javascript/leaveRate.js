...
function checkDuplicate(leafId, page, securityToken) {
    var $leaveRateCode = $("#leaveRateCode");
    if ($leaveRateCode.val().length === 0) {
    ...
    $.ajax({
        ...
        url: page,
        data: {
            leaveRateCode: $leaveRateCode.val(),
            method: 'duplicate',
            securityToken: securityToken,
            leafId: leafId
        },
        ...
        success: function(data) {
            ...
            if (success === true) {
                if (total !== 0) {
                    $("#leaveRateCode").val('').focus();
                    $("#leaveRateCodeForm").removeClass().addClass("form-group has-error");
                ...
                $("#leaveRateForm").removeClass().addClass("form-group has-error");
            }
            if ($infoPanel.is(':hidden')) {
                $infoPanel.show();
            }
...
function showFormUpdate(leafId, url, urlList, securityToken, leaveRateId, updateAccess, deleteAccess) {
    sleep(500);
...
function showModalDelete(leaveRateId, countryId, leaveTypeId, jobId, leaveRateAmount) {
    $('#leaveRateIdPreview').val('').val(decodeURIComponent(leaveRateId));
    ...
    $('#leaveRateAmountPreview').val('').val(decodeURIComponent(leaveRateAmount));
...
function deleteGridRecord(leafId, url, urlList, securityToken) {
    $.ajax({
        ...
        data: {
            method: 'delete',
            output: 'json',
            leaveRateId: $('#leaveRateIdPreview').val(),
            securityToken: securityToken,
            leafId: leafId
        },
        beforeSend: function() {
...
function deleteGridRecordCheckbox(leafId, url, urlList, securityToken) {
    var stringText = '';
    var counter = 0;
    $('input:checkbox[name="leaveRateId[]"]').each(function() {
        stringText = stringText + "&leaveRateId[]=" + $(this).val();
    });
...
function newRecord(leafId, url, urlList, securityToken, type, createAccess, updateAccess, deleteAccess) {
    ...
    var $jobId = $('#jobId');
    var $leaveRateAmount = $('#leaveRateAmount');
    if (css.search('disabled') > 0) {
        return false;
    } else {
        if (type === 1) {
            ...
            if ($leaveRateAmount.val().length === 0) {
                $('#leaveRateAmountHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['leaveRateAmountLabel'] + " </span>");
                $('#leaveRateAmountForm').removeClass().addClass('form-group has-error');
                $leaveRateAmount.focus();
                return false;
            }
            $.ajax({
                type: 'POST',
                ...
                data: {
                    ...
                    jobId: $jobId.val(),
                    leaveRateAmount: $leaveRateAmount.val(),
                    securityToken: securityToken,
                    leafId: leafId
                },
                beforeSend: function() {
                ...
                success: function(data) {
                    ...
                    if (success === true) {
                        ...
                        $leaveRateAmount.val('');
                        $('#leaveRateAmountHelpMe').html('').empty();
                    } else if (success === false) {
                        $infoPanel.html('').empty().html("<span class='label label-important'>&nbsp;" + message + "</span>");
                        if ($infoPanel.is(':hidden')) {
                            $infoPanel.show();
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (success === true) {
                        ...
                        $('#leaveRateId').val(data.leaveRateId);
                        $('#newRecordButton1').removeClass().addClass('btn btn-success disabled').attr('onClick', '');
                        $('#newRecordButton2').removeClass().addClass('btn dropdown-toggle btn-success disabled');
                        $('#newRecordButton3').attr('onClick', '');
                        $('#newRecordButton4').attr('onClick', '');
...
function resetRecord(leafId, url, urlList, securityToken, createAccess, updateAccess, deleteAccess) {
    ...
    $('#jobId').trigger("chosen:updated");
    $("#leaveRateAmount").val('');
    $("#leaveRateAmountHelpMe").empty().html('');
}
function postRecord() {
                ...
                if (success === true) {
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if (success === true) {
                                $('#leaveRateId').val(data.data.leaveRateId);
                                ...
                                $('#leaveRateAmount').val(data.data.leaveRateAmount);
                                if (nextRecord > 0) {
                                    $('#previousRecordButton').removeClass().addClass('btn btn-default disabled').attr('onClick', '');
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($previousRecordCounter.val()) > 0 && parseFloat($previousRecordCounter.val()) < parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    leaveRateId: $previousRecordCounter.val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($nextRecordCounter.val()) <= parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    leaveRateId: $nextRecordCounter.val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
