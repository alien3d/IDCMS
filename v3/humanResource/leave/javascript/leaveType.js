function getLeaveType(leafId, url, securityToken) {
    $.ajax({
        type: 'GET',
        url: url,
        data: {
        ...
        success: function(data) {
            ...
            if (success === false) {
                ...
                $("#leaveTypeId").html('').empty().html(data.data).trigger("chosen:updated");
...
function checkDuplicate(leafId, page, securityToken) {
    var $leaveCode = $("#leaveCode");
    if ($leaveCode.val().length === 0) {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if (success === true) {
                if (total !== 0) {
                    $("#leaveCode").val('').focus();
                    $("#leaveCodeForm").removeClass().addClass("form-group has-error");
                ...
                $("#leaveForm").removeClass().addClass("form-group has-error");
            }
            if ($infoPanel.is(':hidden')) {
                $infoPanel.show();
            }
...
function showFormUpdate(leafId, url, urlList, securityToken, leaveId, updateAccess, deleteAccess) {
    sleep(500);
...
function showModalDelete(leaveId, leaveTypeId, employeeId, documentNumber, leaveStartDate, leaveEndDate, leaveDescription, isRejected) {
    $('#leaveIdPreview').val('').val(decodeURIComponent(leaveId));
    $('#leaveTypeIdPreview').val('').val(decodeURIComponent(leaveTypeId));
    $('#employeeIdPreview').val('').val(decodeURIComponent(employeeId));
    $('#documentNumberPreview').val('').val(decodeURIComponent(documentNumber));
    $('#leaveStartDatePreview').val('').val(decodeURIComponent(leaveStartDate));
    $('#leaveEndDatePreview').val('').val(decodeURIComponent(leaveEndDate));
    $('#leaveDescriptionPreview').val('').val(decodeURIComponent(leaveDescription));
    $('#isRejectedPreview').val('').val(decodeURIComponent(isRejected));
...
function deleteGridRecordCheckbox(leafId, url, urlList, securityToken) {
    var stringText = '';
    var counter = 0;
    $('input:checkbox[name="leaveId[]"]').each(function() {
        stringText = stringText + "&leaveId[]=" + $(this).val();
    });
...
function newRecord(leafId, url, urlList, securityToken, type, createAccess, updateAccess, deleteAccess) {
    ...
    var $leaveStartDate = $('#leaveStartDate');
    var $leaveEndDate = $('#leaveEndDate');
    var $leaveDescription = $('#leaveDescription');
    var $isRejected = $('#isRejected');
    if (css.search('disabled') > 0) {
        return false;
    } else {
        if (type === 1) {
            if ($leaveTypeId.val().length === 0) {
                $('#leaveTypeIdHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['leaveTypeIdLabel'] + " </span>");
                $leaveTypeId.data('chosen').activate_action();
                return false;
            }
            ...
            if ($leaveStartDate.val().length === 0) {
                $('#leaveStartDateHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['leaveStartDateLabel'] + " </span>");
                $('#leaveStartDateForm').removeClass().addClass('form-group has-error');
                $leaveStartDate.focus();
                return false;
            }
            if ($leaveEndDate.val().length === 0) {
                $('#leaveEndDateHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['leaveEndDateLabel'] + " </span>");
                $('#leaveEndDateForm').removeClass().addClass('form-group has-error');
                $leaveEndDate.focus();
                return false;
            }
            if ($leaveDescription.val().length === 0) {
                $('#leaveDescriptionHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['leaveDescriptionLabel'] + " </span>");
                $('#leaveDescriptionForm').removeClass().addClass('form-group has-error');
                $leaveDescription.focus();
                return false;
            }
            if ($isRejected.val().length === 0) {
                $('#isRejectedHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['isRejectedLabel'] + " </span>");
                $('#isRejectedForm').removeClass().addClass('form-group has-error');
                $isRejected.focus();
                return false;
            }
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (success === true) {
                        ...
                        $leaveTypeId.val('');
                        $leaveTypeId.trigger("chosen:updated");
                        $('#leaveTypeIdHelpMe').html('').empty();
                        $employeeId.val('');
                        ...
                        $leaveStartDate.val('');
                        $('#leaveStartDateHelpMe').html('').empty();
                        $leaveEndDate.val('');
                        $('#leaveEndDateHelpMe').html('').empty();
                        $leaveDescription.val('');
                        $('#leaveDescriptionForm').removeClass().addClass('form-group');
                        $('#leaveDescriptionHelpMe').html('').empty();
                        $isRejected.val('');
                        $('#isRejectedHelpMe').html('').empty();
                    } else if (success === false) {
                        $infoPanel.html('').empty().html("<span class='label label-important'>&nbsp;" + message + "</span>");
                        if ($infoPanel.is(':hidden')) {
                            $infoPanel.show();
...
function resetRecord(leafId, url, urlList, securityToken, createAccess, updateAccess, deleteAccess) {
    ...
    $("#leaveId").val('');
    $("#leaveIdHelpMe").empty().html('');
    
    $("#leaveTypeId").val('');
    $("#leaveTypeIdHelpMe").empty().html('');
    $('#leaveTypeId').trigger("chosen:updated");
    $("#employeeId").val('');
    ...
    $("#leaveStartDate").val('');
    $("#leaveStartDateHelpMe").empty().html('');
    $("#leaveEndDate").val('');
    $("#leaveEndDateHelpMe").empty().html('');
    $("#leaveDescription").val('');
    $("#leaveDescriptionHelpMe").empty().html('');
    $('#leaveDescription').empty().val('');
    $("#isRejected").val('');
    $("#isRejectedHelpMe").empty().html('');
}
function postRecord() {
                ...
                if (success === true) {
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if (success === true) {
                                $('#leaveId').val(data.data.leaveId);
                                $('#leaveTypeId').val(data.data.leaveTypeId).trigger("chosen:updated");
                                $('#employeeId').val(data.data.employeeId).trigger("chosen:updated");
                                $('#documentNumber').val(data.data.documentNumber);
                                x = data.data.leaveStartDate;
                                x = x.split("-");
                                output = x[2] + "-" + x[1] + "-" + x[0];
                                output = output.toString();
                                $('#leaveStartDate').val(output);
                                x = data.data.leaveEndDate;
                                x = x.split("-");
                                output = x[2] + "-" + x[1] + "-" + x[0];
                                output = output.toString();
                                $('#leaveEndDate').val(output);
                                $('#leaveDescription').val(data.data.leaveDescription);
                                $('#isRejected').val(data.data.isRejected);
                                if (nextRecord > 0) {
                                    $('#previousRecordButton').removeClass().addClass('btn btn-default disabled').attr('onClick', '');
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($previousRecordCounter.val()) > 0 && parseFloat($previousRecordCounter.val()) < parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    leaveId: $previousRecordCounter.val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
