function checkDuplicate(leafId, page, securityToken) {
    var $leaveHolidaysCode = $("#leaveHolidaysCode");
    if ($leaveHolidaysCode.val().length === 0) {
    ...
    $.ajax({
        ...
        url: page,
        data: {
            leaveHolidaysCode: $leaveHolidaysCode.val(),
            method: 'duplicate',
        ...
        success: function(data) {
            ...
            if (success === true) {
                if (total !== 0) {
                    $("#leaveHolidaysCode").val('').focus();
                    $("#leaveHolidaysCodeForm").removeClass().addClass("form-group has-error");
                ...
                $("#leaveHolidaysForm").removeClass().addClass("form-group has-error");
            }
            if ($infoPanel.is(':hidden')) {
                $infoPanel.show();
            }
...
function showFormUpdate(leafId, url, urlList, securityToken, leaveHolidaysId, updateAccess, deleteAccess) {
    sleep(500);
...
function showModalDelete(leaveHolidaysId, leaveHolidaysDate, leaveHolidaysDescription) {
    $('#leaveHolidaysIdPreview').val('').val(decodeURIComponent(leaveHolidaysId));
    $('#leaveHolidaysDatePreview').val('').val(decodeURIComponent(leaveHolidaysDate));
    $('#leaveHolidaysDescriptionPreview').val('').val(decodeURIComponent(leaveHolidaysDescription));
...
function deleteGridRecord(leafId, url, urlList, securityToken) {
    $.ajax({
        ...
        data: {
            method: 'delete',
            output: 'json',
            leaveHolidaysId: $('#leaveHolidaysIdPreview').val(),
            securityToken: securityToken,
            leafId: leafId
        },
        beforeSend: function() {
...
function deleteGridRecordCheckbox(leafId, url, urlList, securityToken) {
    var stringText = '';
    var counter = 0;
    $('input:checkbox[name="leaveHolidaysId[]"]').each(function() {
        stringText = stringText + "&leaveHolidaysId[]=" + $(this).val();
    });
...
function newRecord(leafId, url, urlList, securityToken, type, createAccess, updateAccess, deleteAccess) {
    ...
    var $leaveHolidaysDate = $('#leaveHolidaysDate');
    var $leaveHolidaysDescription = $('#leaveHolidaysDescription');
    if (css.search('disabled') > 0) {
        return false;
    } else {
        if (type === 1) {
            if ($leaveHolidaysDate.val().length === 0) {
                $('#leaveHolidaysDateHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['leaveHolidaysDateLabel'] + " </span>");
                $('#leaveHolidaysDateForm').removeClass().addClass('form-group has-error');
                $leaveHolidaysDate.focus();
                return false;
            }
            if ($leaveHolidaysDescription.val().length === 0) {
                $('#leaveHolidaysDescriptionHelpMe').html('').empty().html("<span class='label label-important'>&nbsp;" + decodeURIComponent(t['requiredTextLabel']) + " : " + leafTranslation['leaveHolidaysDescriptionLabel'] + " </span>");
                $('#leaveHolidaysDescriptionForm').removeClass().addClass('form-group has-error');
                $leaveHolidaysDescription.focus();
                return false;
            }
            $.ajax({
                type: 'POST',
                ...
                data: {
                    method: 'create',
                    output: 'json',
                    leaveHolidaysDate: $leaveHolidaysDate.val(),
                    leaveHolidaysDescription: $leaveHolidaysDescription.val(),
                    securityToken: securityToken,
                    leafId: leafId
                },
                beforeSend: function() {
                ...
                success: function(data) {
                    ...
                    if (success === true) {
                        ...
                        $leaveHolidaysDate.val('');
                        $('#leaveHolidaysDateHelpMe').html('').empty();
                        $leaveHolidaysDescription.val('');
                        $('#leaveHolidaysDescriptionForm').removeClass().addClass('form-group');
                        $('#leaveHolidaysDescriptionHelpMe').html('').empty();
                    } else if (success === false) {
                        $infoPanel.html('').empty().html("<span class='label label-important'>&nbsp;" + message + "</span>");
                        if ($infoPanel.is(':hidden')) {
                            $infoPanel.show();
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (success === true) {
                        ...
                        $('#leaveHolidaysId').val(data.leaveHolidaysId);
                        $('#newRecordButton1').removeClass().addClass('btn btn-success disabled').attr('onClick', '');
                        $('#newRecordButton2').removeClass().addClass('btn dropdown-toggle btn-success disabled');
                        $('#newRecordButton3').attr('onClick', '');
                        $('#newRecordButton4').attr('onClick', '');
        ...
        if (type === 1) {
            ...
            $.ajax({
                ...
                data: {
                    method: 'save',
                    output: 'json',
                    leaveHolidaysId: $leaveHolidaysId.val(),
                    leaveHolidaysDate: $leaveHolidaysDate.val(),
                    leaveHolidaysDescription: $leaveHolidaysDescription.val(),
                    securityToken: securityToken,
                    leafId: leafId
...
function deleteRecord(leafId, url, urlList, securityToken, deleteAccess) {
    var $infoPanel = $('#infoPanel');
    var $leaveHolidaysId = $('#leaveHolidaysId');
...
function resetRecord(leafId, url, urlList, securityToken, createAccess, updateAccess, deleteAccess) {
    ...
    $("#leaveHolidaysId").val('');
    $("#leaveHolidaysIdHelpMe").empty().html('');
    
    $("#leaveHolidaysDate").val('');
    $("#leaveHolidaysDateHelpMe").empty().html('');
    $("#leaveHolidaysDescription").val('');
    $("#leaveHolidaysDescriptionHelpMe").empty().html('');
    $('#leaveHolidaysDescription').empty().val('');
}
function postRecord() {
                ...
                if (success === true) {
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if (success === true) {
                                $('#leaveHolidaysId').val(data.data.leaveHolidaysId);
                                x = data.data.leaveHolidaysDate;
                                x = x.split("-");
                                output = x[2] + "-" + x[1] + "-" + x[0];
                                output = output.toString();
                                $('#leaveHolidaysDate').val(output);
                                $('#leaveHolidaysDescription').val(data.data.leaveHolidaysDescription);
                                if (nextRecord > 0) {
                                    $('#previousRecordButton').removeClass().addClass('btn btn-default disabled').attr('onClick', '');
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($previousRecordCounter.val()) > 0 && parseFloat($previousRecordCounter.val()) < parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    leaveHolidaysId: $previousRecordCounter.val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
    ...
    if (css.search('disabled') > 0) {
        ...
        if (parseFloat($nextRecordCounter.val()) <= parseFloat($('#lastRecordCounter').val())) {
            $.ajax({
                ...
                data: {
                    method: 'read',
                    leaveHolidaysId: $nextRecordCounter.val(),
                    output: 'json',
                    securityToken: securityToken,
                    leafId: leafId
                },
