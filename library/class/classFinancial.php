 ...
 * described in Mayle (1993, 17?23, A3?A15).
 * See Public Securities Association (1990)
...
class Financial {
	 ...
	 *        	is the value used to increment datepart. If you specify a value that is not an integer, the fractional part of the value is discarded.
	 * @param integer $date
	 ...
	 *        	is the beginning date (Unix timestamp) for the calculation.
	 * @param integer $enddate
	 *        	is the ending date (Unix timestamp) for the calculation.
	 ...
	 *        	is the ending date (Unix timestamp) for the calculation.
	 * @param bool $method
	 *        	is a logical value that specifies whether to use the U.S. or European method in the calculation.
	 ...
	 * @param integer $enddate
	 *        	is the ending date (Unix timestamp) for the calculation.
	 * @param date $refPeriodStart
	 *        	is the reference beginning date (Unix timestamp) for the inner calculation.
	 * @param date $refPeriodEnd
	 *        	is the reference ending date (Unix timestamp) for the inner calculation.
	 * @return float the fraction of years between two dates
	 */
	function ActualActualyearFraction($startdate, $enddate, $refPeriodStart = null, $refPeriodEnd = null) {
		...
		/*
		 * if ($this->DATEDIFF('day', $startdate, $enddate) == 0) return 0.0; $d1 = getdate($startdate); $d2 = getdate($enddate); $dib1 = ((date('L', $startdate) == 1) ? 366.0 : 365.0); $dib2 = ((date('L', $enddate) == 1) ? 366.0 : 365.0); $sum = $d2['year'] - $d1['year'] - 1; $sum += $this->ActualActualdayCount($startdate, mktime(0,0,0,1,1,$d1['year'] + 1)) / $dib1; $sum += $this->ActualActualdayCount(mktime(0,0,0,1,1,$d2['year']), $enddate) / $dib2; return $sum;
		 */
	 ...
	 *        	is the starting period for which you want to calculate the depreciation. Start_period must use the same units as life.
	 * @param integer $end_period
	 *        	is the ending period for which you want to calculate the depreciation. End_period must use the same units as life.
	 * @param float $factor
	 *        	is the rate at which the balance declines. If factor is omitted, it is assumed to be 2 (the double-declining balance method). Change factor if you do not want to use the double-declining balance method.
	 * @param bool $no_switch
	...
	function VDB($cost, $salvage, $life, $start_period, $end_period, $factor = 2.0, $no_switch = false) {
		// pre-validations
		if (($start_period < 0) || ($end_period < $start_period) || ($end_period > $life) || ($cost < 0) || ($salvage > $cost) || ($factor <= 0)) {
			return null;
		}
	...
	function _calculate_interest_part($pv, $pmt, $rate, $per) {
		return - ($pv * pow ( 1 + $rate, $per ) * $rate + $pmt * (pow ( 1 + $rate, $per ) - 1));
	}
	 ...
	 * @param float $pmt
	 *        	is the payment made each period and cannot change over the life of the annuity.
	 * @param float $fv
	...
	function FV($rate, $nper, $pmt, $pv = 0.0, $type = 0) {
		...
		$fv = (- (($pv * $pvif) + $pmt * (1.0 + $rate * $type) * $fvifa));
	 ...
	 * MIRR considers both the cost of
	...
	function MIRR($values, $finance_rate, $reinvert_rate) {
		...
		/*
		 * I have my doubts about this formula, but it sort of looks like the one Microsoft claims to use and it produces the results that Excel does. -- MW.
		 */
	 ...
	 * To calculate the internal rate
	 ...
	 * 4 European 30/360
	 */
	function yieldx($settlement, $maturity, $rate, $pr, $redemption, $frequency, $basis = FINANCIAL_BASIS_MSRB_30_360) {
		if (! $this->_is_valid_basis ( $basis )) {
			return null;
		}
